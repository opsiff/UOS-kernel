 ...
 */
/*
 * Must stay below PAGEVEC_SIZE (15 on this branch): if the threshold is not
 * reached before the batch fills, the fault path drains it instead and we are
 * back to a multi-microsecond pte-lock critical section.
 */
unsigned int lru_add_drain_min_nr __read_mostly = 8;
module_param_named(lru_drain_min_nr, lru_add_drain_min_nr, uint, 0644);

void lru_add_drain_min(void)
{
	if (!lru_add_drain_min_nr || lru_add_drain_min_nr >= PAGEVEC_SIZE)
		return;
