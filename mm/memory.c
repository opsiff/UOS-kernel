...
#ifdef CONFIG_MM_AMA
#define MAX_CMA_REUSE_RAM_PAGES 0x280000 /* 10G pages */
#else
#define MAX_CMA_REUSE_RAM_PAGES 0x800000 /* 32G pages */
#endif
...
#ifdef CONFIG_MM_SMMU_V3
	/*
	 * Flush svm task's pgtable cache and SMMU tlb.
	 * Otherwise, hardware UAF may occur under following circumstance:
	 *   1.svm task map VA1
	 *   2.svm task let NPU hardware access VA1 (generate SMMU tlb)
	 *   3.svm task unmap VA1
	 *   4.svm task let NPU hardware access VA1 (tlb hit and UAF occurs)
	 */
	if (first_vma && first_vma->vm_mm && test_bit(MMF_SVM, &first_vma->vm_mm->flags))
		mm_svm_flush_cache(first_vma->vm_mm, start_addr, end_addr - start_addr);
#endif
...
#ifdef CONFIG_MM_AMA
bool pte_spinlock_exp(struct vm_fault *vmf)
{
	return pte_spinlock(vmf);
}
#endif
...
static pmd_t mm_pmdp_invalidate(pmd_t *pmdp)
{
	pmd_t inv_pmd;
	pmd_t old;

	inv_pmd = pmd_mkinvalid(*pmdp);
	old = __pmd(xchg_relaxed(&pmd_val(*pmdp), pmd_val(inv_pmd)));
	flush_tlb_all();
	return old;
}
...
static vm_fault_t wp_page_copy(struct vm_fault *vmf)
{
	...
	if (is_zero_pfn(pte_pfn(vmf->orig_pte))) {
		...
		if (totalram_pages() > MAX_CMA_REUSE_RAM_PAGES)
			new_page = alloc_page_vma(GFP_HIGHUSER_MOVABLE, vma,
					vmf->address);
		else
			new_page = alloc_page_vma(GFP_HIGHUSER_MOVABLE | __GFP_CMA, vma,
					vmf->address);
		if (!new_page)
			goto out;
...
vm_fault_t do_swap_page(struct vm_fault *vmf)
{
	...
	if (!page) {
		...
		if (data_race(si->flags & SWP_SYNCHRONOUS_IO) &&
			...
			/* skip swapcache */
			if (totalram_pages() > MAX_CMA_REUSE_RAM_PAGES)
				flags = GFP_HIGHUSER_MOVABLE;
			else
				flags = GFP_HIGHUSER_MOVABLE | __GFP_CMA;
...
static vm_fault_t do_cow_fault(struct vm_fault *vmf)
{
	...
	if (totalram_pages() > MAX_CMA_REUSE_RAM_PAGES)
		...
		vmf->cow_page = alloc_page_vma(GFP_HIGHUSER_MOVABLE | __GFP_CMA, vma, vmf->address);
	if (!vmf->cow_page)
		return VM_FAULT_OOM;
