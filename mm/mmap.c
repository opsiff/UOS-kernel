...
#ifdef CONFIG_BG_DMABUF_RECLAIM
	if (vma_is_bg_buffer_vma(vma))
		bg_vma_lock_state = bg_buffer_vma_unmap_lock(vma, &buf_ptr);
#endif
...
#ifdef CONFIG_BG_DMABUF_RECLAIM
	if (bg_vma_lock_state == 0)
		bg_buffer_vma_unmap_unlock(buf_ptr);
#endif
