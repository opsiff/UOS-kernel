...
#ifdef CONFIG_HYPERHOLD_FILE_LRU
	if (!is_prot_page(page) && !PageSwapBacked(page)) {
		lruvec = &pgdat->__lruvec;
		goto out;
	}
#endif
...
static void mem_cgroup_css_free(struct cgroup_subsys_state *css)
{
	...
	/*
	 * Hyperhold moves mem_cgroup_id_remove from mem_cgroup_id_put_many.
	 * As a result, rcu lock can no longer protect memcg from being released
	 * when we get memcg from memcg id. Just use synchronize_rcu to wait for
	 * a full grace period elapsed.
	 */
	synchronize_rcu();
#endif
...
int __mem_cgroup_charge_nonprotect(struct page *page, struct mm_struct *mm,
			gfp_t gfp_mask, bool protect)
#else
