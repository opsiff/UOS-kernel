...
#ifdef CONFIG_HKIP_PRMEM
		if (likely(!is_pmalloc((void *)addr, 1)))
			cond_resched();
#else
		cond_resched();
#endif
...
{
	unmap_kernel_range_noflush(addr, end - addr);
}
#endif
...
int prmem_vmap_page_range(unsigned long start, unsigned long end,
			  pgprot_t prot, struct page **pages)
{
	return map_kernel_range(start, end - start, prot, pages);
}
#endif
...
void vfree(const void *addr)
{
	...
	CALL_HCK_VH(hck_dfx_bbox_vmalloc_trace_hook, (u8)0,
		(u64)_RET_IP_, (u64)addr, 0, (u64)0);
}
EXPORT_SYMBOL(vfree);
		...
		unsigned long flags, pgprot_t prot)
{
	...
	vm_end = get_vm_area_size(area);
	vm_mid = (unsigned long)(offset << PAGE_SHIFT);

	/* For pages from last buffer, they mapped from the beginning of vm area.
	 * Build the prot of last buffer first.
	 */
	lb_prot_build(pages[0], &prot);
	ret = map_kernel_range_noflush(vm_start, vm_mid, prot, pages);
	if (ret <= 0) {
		vunmap(area->addr);
		return NULL;
	}
	...
	ret = map_kernel_range_noflush((vm_start + vm_mid), vm_end, prot, pages + offset);
	if (ret <= 0) {
		vunmap(area->addr);
		return NULL;
	}
			...
			const void *caller)
{
	...
	CALL_HCK_VH(hck_dfx_bbox_vmalloc_trace_hook, (u8)1,
		(u64)caller, (u64)addr, (struct page *)area->pages[0], (u64)size);
	return addr;
...
#ifdef CONFIG_H_MM_PAGE_TRACE
#include "../platform_source/basicplatform/mm/vmalloc_track.c"
#endif
