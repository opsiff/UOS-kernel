...
static int oled_spi_aod_on(void)
{
	...
	if (ret)
		return ret;
	buf = OLED_SPI_PAGE2_VAL;
	ret = oled_data_cmd_transfer(&buf, OLED_SPI_CMD_LEN, DCX_DATA_MODE);
	if (ret)
		return ret;
	buf = OLED_SPI_FRAME_RATE_REG;
	oled_data_cmd_transfer(&buf, OLED_SPI_CMD_LEN, DCX_CMD_MODE);
	buf = OLED_SPI_30HZ_FRAME;
	oled_data_cmd_transfer(&buf, OLED_SPI_CMD_LEN, DCX_DATA_MODE);
	buf = OLED_SPI_PAGE_REG;
	ret = oled_data_cmd_transfer(&buf, OLED_SPI_CMD_LEN, DCX_CMD_MODE);
	if (ret)
		return ret;
...
{
	uint32_t brightness;
	if (!data || !data->spi_bl_cali) {
		hwlog_err("invalid data, return.\n");
		return LCD_KIT_FAIL;
	}
	if (lcd_kit_get_alpm_setting_status() == ALPM_ON_MIDDLE_LIGHT)
		return LCD_KIT_OK;
	brightness = *(uint32_t *)data->spi_bl_cali;
