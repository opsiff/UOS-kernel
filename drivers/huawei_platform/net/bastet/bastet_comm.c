...
ssize_t bastet_kernel_read(struct file *file, void *buf, size_t count, loff_t *pos)
{
	mm_segment_t old_fs;
	ssize_t result;
...
ssize_t bastet_kernel_write(struct file *file, const void *buf, size_t count,
			   loff_t *pos)
{
	mm_segment_t old_fs;
	ssize_t res;
...
int bastet_modem_dev_write(uint8_t *msg, uint32_t len)
{
	...
	ret = bastet_kernel_write(dev_filp, msg, len, &offset);
...
static void handle_bst_acore_core_msg_type_dspp(uint8_t *msg, uint32_t len)
{
	...
	if (len != (unsigned)hdrlen + acom_msg->ul_len) {
...
{
	struct bst_acom_msg *acom_msg = (struct bst_acom_msg *)msg;
	unsigned long hdrlen = (uint8_t *)(acom_msg->auc_value) - (uint8_t *)acom_msg;
...
static int read_data_from_file(uint32_t *len, struct list_head *buffers)
{
	...
	while (1) {
		...
		ret = bastet_kernel_read(dev_filp, buf->data,
...
static int get_event(void)
{
	...
	while (1) {
		size = bastet_kernel_read(dev_filp, buf,
			BST_MAX_READ_PAYLOAD, &offset);
