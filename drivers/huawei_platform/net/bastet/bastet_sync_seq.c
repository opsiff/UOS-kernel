...
static int g_comm_prop_cnt = 0;

static int g_comm_prop_contain_mac_cnt = 0;

static struct bst_mac_address_comm_prop g_comm_prop_array[BST_COMM_PROP_NUMBER] = {0};

static bool g_bastet_wifi_work = false;

static DEFINE_SPINLOCK(create_bastet_lock);

static DEFINE_SPINLOCK(comm_prop_array_lock);
...
static void process_sock_send_and_recv(struct sock *sk)
{
	...
	if (tcp_send_head(sk) != NULL) {
		mss_now = (signed)tcp_current_mss(sk);
...
/*
 * reset_comm_prop_array () - reset the comm prop array
 *
 * Reset commom prop array and clear the count
 *
 * Returns:No
 */
static void reset_comm_prop_array()
{
	spin_lock_bh(&comm_prop_array_lock);

	g_comm_prop_cnt = 0;
	g_comm_prop_contain_mac_cnt = 0;
	memset_s(g_comm_prop_array, BST_COMM_PROP_NUMBER * sizeof(struct bst_mac_address_comm_prop),
		0, BST_COMM_PROP_NUMBER * sizeof(struct bst_mac_address_comm_prop));

	spin_unlock_bh(&comm_prop_array_lock);
}
...
static int add_rcvqueues_queue(struct sock *sk, struct sk_buff *skb)
{
	...
	sk->bastet->recv_len += (signed)skb->truesize;
	return 0;
}
...
static int post_sock_prop_ipv4(struct sock *sk)
{
	...
	spin_lock_bh(&comm_prop_array_lock);
	bastet_get_mac_header(g_comm_prop_array, g_comm_prop_cnt, &sock_p);
	spin_unlock_bh(&comm_prop_array_lock);
	#endif
...
static int post_sock_prop_ipv6(struct sock *sk)
{
	...
	#ifdef CONFIG_BASTET_IN_SENSORHUB
	spin_lock_bh(&comm_prop_array_lock);
	bastet_get_mac_header_ipv6(g_comm_prop_array, g_comm_prop_cnt, &sock_p);
	spin_unlock_bh(&comm_prop_array_lock);
	#endif
...
/*
 * update_comm_prop_array() - update comm prop array
 * @bst_sock_comm_prop_array: array stored comm prop
 * @comm_prop: comm prop data pointer.
 *
 * update comm prop array with new comm prop data
 * If array already contains the comm prop, skip
 *
 * Returns:No
 */
static void update_comm_prop_array(struct bst_mac_address_comm_prop *bst_sock_comm_prop_array,
		struct bst_sock_comm_prop *comm_prop)
{
	if (IS_ERR_OR_NULL(bst_sock_comm_prop_array) || IS_ERR_OR_NULL(comm_prop)) {
		bastet_loge("invalid parameter");
		return;
	}
	...
	if (g_comm_prop_cnt >= BST_COMM_PROP_NUMBER) {
		spin_unlock_bh(&comm_prop_array_lock);
		bastet_loge("bst_sock_comm_prop_array range out of boundary");
		return;
	}

	for (int i = 0; i < g_comm_prop_cnt; i++) {
		if (memcmp(&(bst_sock_comm_prop_array[i].comm_prop), comm_prop, sizeof(struct bst_sock_comm_prop)) == 0) {
		    bastet_logi("current bastet_comm_prop already in the array");
		    contains_flag = true;
		    break;
		}
	}
	if (contains_flag == false) {
		bst_sock_comm_prop_array[g_comm_prop_cnt].comm_prop = *comm_prop;
		bst_sock_comm_prop_array[g_comm_prop_cnt].contain_mac = false;
		g_comm_prop_cnt++;
	}
	spin_unlock_bh(&comm_prop_array_lock);
}

static void update_comm_prop_array_ipv6(struct bst_mac_address_comm_prop *bst_sock_comm_prop_array,
		struct bst_sock_comm_prop_ipv6 *comm_prop_ipv6)
{
	if (IS_ERR_OR_NULL(bst_sock_comm_prop_array) || IS_ERR_OR_NULL(comm_prop_ipv6)) {
		bastet_loge("invalid parameter");
		return;
	}
	...
	for (int i = 0; i < g_comm_prop_cnt; i++) {
		if (memcmp(&(bst_sock_comm_prop_array[i].comm_prop_ipv6), comm_prop_ipv6,
			sizeof(struct bst_sock_comm_prop_ipv6)) == 0) {
			bastet_logi("current bastet_comm_prop_ipv6 already in the array");
			contains_flag = true;
			break;
		}
	}
	if (contains_flag == false) {
		bst_sock_comm_prop_array[g_comm_prop_cnt].comm_prop_ipv6 = *comm_prop_ipv6;
		bst_sock_comm_prop_array[g_comm_prop_cnt].contain_mac = false;
		g_comm_prop_cnt++;
	}
	spin_unlock_bh(&comm_prop_array_lock);
...
/*
 * bastet_hook_sk_buff() - bst hook sk_buff from netfilter
 * @skb: the sk_buff from netfilter
 *
 * hook the ip address and port no. from sk_buff
 * compare to the comm prop in comm prop array
 * copy the mac address to the array if the comm prop is identical
 *
 * Returns No.
 */
void bastet_hook_sk_buff(struct sk_buff *skb)
{
	if (!g_bastet_wifi_work)
		return;
	...
	if (g_comm_prop_contain_mac_cnt >= g_comm_prop_cnt) {
		spin_unlock_bh(&comm_prop_array_lock);
		return;
	}

	unsigned short protocol_ip = skb->protocol;

	if (protocol_ip == htons(ETH_P_IP)) {
		...
		if (ip_header->protocol == IPPROTO_TCP) {
			...
			unsigned int src_port = tcp_header->source;
			unsigned int dest_port = tcp_header->dest;
			for (int i = 0; i < g_comm_prop_cnt; i++) {
				struct bst_sock_comm_prop *comm_prop = &g_comm_prop_array[i].comm_prop;
				struct bst_sock_comm_prop skb_comm_prop = {src_ip, dest_ip, src_port, dest_port};
				if (memcmp(comm_prop, &skb_comm_prop, sizeof(struct bst_sock_comm_prop)) == 0) {
					 struct ethhdr *mac_header = (struct ethhdr *) skb_mac_header(skb);
					if (mac_header != NULL &&
						check_mac_address_valid(mac_header->h_source, ETH_ALEN) &&
						check_mac_address_valid(mac_header->h_dest, ETH_ALEN)) {
						struct bst_mac_address_comm_prop *mac_address_comm_prop = &g_comm_prop_array[i];
						memcpy(mac_address_comm_prop->mac_source, mac_header->h_source, ETH_ALEN);
						memcpy(mac_address_comm_prop->mac_dest, mac_header->h_dest, ETH_ALEN);
						mac_address_comm_prop->contain_mac = true;
					}
				}
			}
		}
	...
	if (protocol_ip == ETH_P_IPV6) {
		struct ipv6hdr *ipv6_header = ipv6_hdr(skb);
		struct in6_addr src_ip = ipv6_header->saddr;
		struct in6_addr dest_ip = ipv6_header->daddr;

		if (ipv6_header->nexthdr == NEXTHDR_TCP) {
			struct tcphdr *tcp_header = (struct tcphdr *) skb_transport_header(skb);
			unsigned int src_port = tcp_header->source;
			unsigned int dest_port = tcp_header->dest;
			for (int i = 0; i < g_comm_prop_cnt; i++) {
				struct bst_sock_comm_prop_ipv6 *comm_prop = &g_comm_prop_array[i].comm_prop_ipv6;
				struct bst_sock_comm_prop_ipv6 skb_comm_prop_ipv6 = {
					.local_port = src_port,
					.remote_port = dest_port
				};
				memcpy(skb_comm_prop_ipv6.local_ip, &src_ip, IPV6_ADDR_LENGTH);
				memcpy(skb_comm_prop_ipv6.remote_ip, &dest_ip, IPV6_ADDR_LENGTH);
				if (memcmp(comm_prop, &skb_comm_prop_ipv6, sizeof(struct bst_sock_comm_prop_ipv6)) == 0) {
					 struct ethhdr *mac_header = (struct ethhdr *) skb_mac_header(skb);
					 if (mac_header != NULL &&
						check_mac_address_valid(mac_header->h_source, ETH_ALEN) &&
						check_mac_address_valid(mac_header->h_dest, ETH_ALEN)) {
	...
	int contain_mac_num = 0;
	for (int i = 0; i < g_comm_prop_cnt; i++) {
		if (g_comm_prop_array[i].contain_mac)
			contain_mac_num++;
	}
	g_comm_prop_contain_mac_cnt = contain_mac_num;

	spin_unlock_bh(&comm_prop_array_lock);
}

/*
 * bastet_hook_roamed_event() - bst hook mac address after wifi roamed
 * @mac_addr: the previous mac address
 *
 * hook the mac address from the roamed event and send comm prop data
 *
 * Returns No.
 */
void bastet_hook_roamed_event(unsigned char *mac_addr)
{
	if (!g_bastet_wifi_work)
		return;
	...
	if (g_comm_prop_cnt <= 0) {
		bastet_loge("comm prop array is empty");
		spin_unlock_bh(&comm_prop_array_lock);
		return;
	}

	if (!check_mac_address_valid(mac_addr, ETH_ALEN)) {
		bastet_loge("bst wifi roamed mac address is invalid");
		spin_unlock_bh(&comm_prop_array_lock);
		return;
	}

	for (int i = 0; i < g_comm_prop_cnt; i++) {
		if (memcmp(g_comm_prop_array[i].mac_source, mac_addr, ETH_ALEN) == 0) {
			struct bst_sock_comm_prop *guide = &g_comm_prop_array[i].comm_prop;
			struct bst_sock_comm_prop_ipv6 *prop_ipv6 = &g_comm_prop_array[i].comm_prop_ipv6;
			spin_unlock_bh(&comm_prop_array_lock);
			if (guide != NULL)
				post_indicate_packet(BST_IND_WIFI_ROAMED_EVENT, guide, sizeof(*guide));
			if (prop_ipv6 != NULL)
				post_indicate_packet(BST_IND_WIFI_ROAMED_EVENT, prop_ipv6, sizeof(*prop_ipv6));
			spin_lock_bh(&comm_prop_array_lock);
		}
	}
...
#else
static void bastet_sock_bastet_timeout(struct timer_list *data)
#endif
{
	int event;
#if LINUX_VERSION_CODE < KERNEL_VERSION(4,19,0)
...
#else
	struct sock *sk = (struct sock *)data->data;
#endif
	struct bastet_sock *bsk = sk->bastet;
...
#else
	bsk->bastet_timer.data = (unsigned long)sk;
	timer_setup(&bsk->bastet_timer, bastet_sock_bastet_timeout, 0);
#endif
...
int stop_bastet_sock(struct bst_sock_id *guide)
{
	...
	#ifdef CONFIG_BASTET_IN_SENSORHUB
	reset_comm_prop_array();
	bastet_logi("stop_bastet_sock reset g_comm_prop_array");
	#endif
...
int prepare_bastet_sock(struct bst_set_sock_sync_delay *sync_prop)
{
	...
	#ifdef CONFIG_BASTET_IN_SENSORHUB
	g_bastet_wifi_work = true;

	if (!is_ipv6_addr(sk)) {
		struct bst_set_sock_sync_prop sock_p;
		memset(&sock_p, 0, sizeof(sock_p));
		bastet_get_comm_prop(sk, &sock_p.guide);
		update_comm_prop_array(g_comm_prop_array, &sock_p.guide);
	} else {
		struct bst_sock_comm_prop_ipv6 prop_ipv6;
		memset(&prop_ipv6, 0, sizeof(prop_ipv6));
		bastet_get_comm_prop_ipv6(sk, &prop_ipv6);
		update_comm_prop_array_ipv6(g_comm_prop_array, &prop_ipv6);
	}
	#endif
...
/*
 * check_bastet_mac_address_available() - check the mac address is available
 * @guide: sock information,include pid and fd.
 *
 * Function is called when application check is bastet available
 *
 * Returns true, if comm prop contains source and dest mac address
 * Otherwise return false.
 */
bool check_bastet_mac_address_available(struct bst_sock_id *guide)
{
	struct sock *sk = NULL;
	...
	if (!is_ipv6_addr(sk)) {
		...
		for (int i = 0; i < g_comm_prop_cnt; i++) {
			struct bst_sock_comm_prop *comm_prop = &g_comm_prop_array[i].comm_prop;
			if (memcmp(comm_prop, &sock_p.guide, sizeof(struct bst_sock_comm_prop)) == 0) {
				if (check_mac_address_valid(g_comm_prop_array[i].mac_source, ETH_ALEN) &&
				    check_mac_address_valid(g_comm_prop_array[i].mac_dest, ETH_ALEN))
					return true;
			}
		}
	} else {
		...
		for (int i = 0; i < g_comm_prop_cnt; i++) {
			struct bst_sock_comm_prop_ipv6 *comm_prop = &g_comm_prop_array[i].comm_prop_ipv6;
			if (memcmp(comm_prop, &prop_ipv6, sizeof(struct bst_sock_comm_prop_ipv6)) == 0) {
				if (check_mac_address_valid(g_comm_prop_array[i].mac_source, ETH_ALEN) &&
				    check_mac_address_valid(g_comm_prop_array[i].mac_dest, ETH_ALEN))
					return true;
			}
...
int set_tcp_sock_closed(struct bst_sock_comm_prop *guide)
{
	...
	#ifdef CONFIG_BASTET_IN_SENSORHUB
	remove_comm_prop_from_array(guide);
	#endif
...
int set_tcp_sock_closed_ipv6(struct bst_sock_comm_prop_ipv6 *guide)
{
	...
	#ifdef CONFIG_BASTET_IN_SENSORHUB
	remove_comm_prop_from_array_ipv6(guide);
	#endif
...
int get_tcp_bastet_sock_state(struct bst_get_bastet_sock_state *get_prop)
{
	...
	get_prop->sync_state = (unsigned)get_bastet_sock_state(sk);
...
/*
 * remove_comm_prop_from_array() - remove comm prop from sock comm prop array
 * @sock_comm_prop: the sock comm prop pointer.
 *
 * remove comm prop from g_sock_comm_prop_array when the socket is disconnected.
 * decrease the length of g_sock_comm_prop_array.
 *
 * Returns 0, get success
 * Otherwise failed.
 */
int remove_comm_prop_from_array(struct bst_sock_comm_prop *sock_comm_prop)
{
	if (IS_ERR_OR_NULL(sock_comm_prop)) {
	...
	for (int i = 0; i < g_comm_prop_cnt; i++) {
		struct bst_sock_comm_prop *comm_prop = &g_comm_prop_array[i].comm_prop;
		if (memcmp(comm_prop, sock_comm_prop, sizeof(struct bst_sock_comm_prop)) == 0) {
			for (int j = i; j < g_comm_prop_cnt - 1; j++)
				g_comm_prop_array[j] = g_comm_prop_array[j+1];
...
int remove_comm_prop_from_array_ipv6(struct bst_sock_comm_prop_ipv6 *sock_comm_prop_ipv6)
{
	if (IS_ERR_OR_NULL(sock_comm_prop_ipv6)) {
		bastet_loge("invalid patameter");
		return -1;
	}
	...
	spin_lock_bh(&comm_prop_array_lock);
	for (int i = 0; i < g_comm_prop_cnt; i++) {
		struct bst_sock_comm_prop_ipv6 *comm_prop_ipv6 = &g_comm_prop_array[i].comm_prop_ipv6;
		if (memcmp(comm_prop_ipv6, sock_comm_prop_ipv6, sizeof(struct bst_sock_comm_prop_ipv6)) == 0) {
			for (int j = i; j < g_comm_prop_cnt - 1; j++)
				g_comm_prop_array[j] = g_comm_prop_array[j+1];
