...
static void sw_update_pid_vid(char *uevent[])
{
	...
	pcore_data = pdisc_data->core_data;
	if (!pcore_data || !pcore_data->device)
		return;
...
static void sw_notify_android_uevent(int is_conn_or_disconn)
{
	...
	char *connected[KB_STATE_CONNECT_MAX] = { "KB_STATE=CONNECTED", NULL, NULL, NULL };
	char *nfc_req[KB_STATE_MAX] = { "KB_STATE=RECV_NFC_REQ", NULL };
	char *nfc_near[KB_STATE_NFC_NEAR_MAX] = { "KB_NFC_STATE=NEAR", NULL, NULL, NULL };
...
static int sw_probe(struct platform_device *pdev)
{
	...
	pdisc_data->detect_ops.detect_nb.notifier_call = sw_detect_notifier_call;
	pdisc_data->detect_ops.notify = NULL;

	if ((ret = sw_gpio_detect_register(pdev, &pdisc_data->detect_ops)) < 0) {
		sw_print_err("sw_gpio_detect_register failed\n");
		goto err_core_init;
	}

	if ((ret = sw_core_init(&pdisc_data->core_data)) != 0) {
	...
	pdisc_data->kb_wakelock = wakeup_source_register(&pdev->dev ,"swkb_wakelock");
#else
	pdisc_data->kb_wakelock = wakeup_source_register("swkb_wakelock");
#endif
	if (!pdisc_data->kb_wakelock) {
		sw_print_err("wakeup source register fail\n");
		goto err_exception;
	}
...
err_core_init:
	wakeup_source_unregister(pdisc_data->kb_wakelock);
	pdisc_data->kb_wakelock = NULL;
...
int sw_register(struct sw_driver *drv)
{
	return sw_register_driver(drv, THIS_MODULE, KBUILD_MODNAME);
}
