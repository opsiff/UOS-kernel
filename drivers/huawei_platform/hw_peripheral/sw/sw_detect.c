...
static int sw_gpio_detect_notifier_register(struct platform_device *pdev,
	struct sw_gpio_detector *detector, struct notifier_block *nb)
{
	...
	detector->kb_wlock = wakeup_source_register(&pdev->dev,"kb wakelock");
#else
	detector->kb_wlock = wakeup_source_register("kb wakelock");
#endif
	if (!detector->kb_wlock) {
	...
	struct sw_gpio_detector *detector, struct notifier_block *nb)
{
	...
	wakeup_source_unregister(detector->kb_wlock);
	detector->kb_wlock = NULL;
	return ret;
}
	...
	struct sw_detector_ops *ops)
{
	...
	sw_gpio_detect_notifier_register(pdev, detector, &ops->detect_nb);
