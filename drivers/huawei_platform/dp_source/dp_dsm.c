...
// true:  only time
// false: date and time
static int dp_timespec64_to_str(struct timespec64 time, const char *comment,
...
#ifdef DP_DEBUG_ENABLE
int dp_timespec64_to_time_str(struct timespec64 time, const char *comment,
	char *str, int size)
{
	return dp_timespec64_to_str(time, comment, str, size, true);
}
#endif

int dp_timespec64_to_datetime_str(struct timespec64 time, const char *comment,
	char *str, int size)
{
	return dp_timespec64_to_str(time, comment, str, size, false);
}

#ifdef DP_LINK_TIMEVAL_FORMAT
static int dp_timespec64_sec_to_str(unsigned long time, const char *comment,
	char *str, int size)
{
	struct rtc_time tm;
	int ret = 0;
...
static void dp_add_hotplug_info_to_list(struct dp_dsm_priv *priv, bool stop)
{
	...
	if (stop) {
		...
		sec.tv_nsec = (sec.tv_sec * DP_DSM_TIME_MS_OF_SEC) +
			(sec.tv_nsec / DP_DSM_TIME_US_OF_MS / DP_DSM_TIME_NS_OF_US);
...
static void dp_set_typec_in_out_time(struct dp_dsm_priv *priv, bool in)
{
	...
	if (in) {
		ktime_get_real_ts64(&(priv->tpyec_in_time));
		g_typec_in_out.in_time[index] = priv->tpyec_in_time;
	} else { // out
		ktime_get_real_ts64(&(priv->tpyec_out_time));
				 ...
				 void *data)
{
	...
	// typec in & out time
	dp_timespec64_to_datetime_str(priv->tpyec_in_time,
		"tpyec_in", in_time, DP_DSM_DATETIME_SIZE);
	dp_timespec64_to_datetime_str(priv->tpyec_out_time,
	...
	dp_timespec64_to_datetime_str(priv->same_mode_time,
		"same_mode_time", same_time, DP_DSM_DATETIME_SIZE);
	dp_timespec64_to_datetime_str(priv->diff_mode_time,
	...
	dp_timespec64_to_datetime_str(priv->link_max_time,
