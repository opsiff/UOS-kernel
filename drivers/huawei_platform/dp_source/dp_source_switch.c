...
#include <platform_include/display/dkmd/dkmd_dp_interface.h>
#endif
...
#define PRODUCT_ID_HUASHAN_0      0x22 /* EDID Byte10 */
#define PRODUCT_ID_HUASHAN_1      0x6E /* EDID Byte11 */
#define EDID_BYTE_10              10
#define EDID_BYTE_11              11
#define HACTIVE_PIXELS_RATIO      3
#define VACTIVE_PIXELS_RATIO      2
...
#ifdef CONFIG_DKMD_DPU_DP
static struct dp_hook_ops dpu_dp_hook_ops = {
	.save_dp_edid = save_dp_edid,
	.is_valid_timing_info = is_valid_timing_info,
};
#endif
...
static enum huawei_vision_type get_smart_display_type()
{
	if (g_dp_priv.edid[EDID_BYTE_10] == PRODUCT_ID_HUASHAN_0 &&
		g_dp_priv.edid[EDID_BYTE_11] == PRODUCT_ID_HUASHAN_1)
		return VISION_TYPE_HUASHAN;
...
bool is_valid_timing_info(uint16_t hactive_pixels, uint16_t vactive_pixels, uint32_t fps)
{
	...
	if (hactive_pixels * VACTIVE_PIXELS_RATIO != vactive_pixels * HACTIVE_PIXELS_RATIO &&
		get_smart_display_type() == VISION_TYPE_HUASHAN) {
		hwlog_info("invalid_timing -- xres: %u, yres: %u\n", hactive_pixels, vactive_pixels);
		return false;
	}

	hwlog_info("valid_timing -- xres : %u, yres: %u\n", hactive_pixels, vactive_pixels);
	return true;
}
...
static void dp_device_create(void)
{
	hwlog_info("%s enter\n", __func__);
...
#ifdef CONFIG_DKMD_DPU_DP
	dpu_dp_hook_register(&dpu_dp_hook_ops);
#endif
