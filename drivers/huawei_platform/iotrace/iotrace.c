...
static int trace_mm_init(struct io_trace_ctrl *trace_ctrl)
{
	...
	trace_ctrl->all_trace[0] = (struct io_trace_pid *)kvzalloc(
		sizeof(struct io_trace_pid) * IO_MAX_PROCESS, GFP_KERNEL);
	if (trace_ctrl->all_trace[0] == NULL) {
		io_trace_print("all trace kvzalloc failed!\n");
		return -1;
	}
	for (i = 1; i < IO_MAX_PROCESS; i++)
		trace_ctrl->all_trace[i] = trace_ctrl->all_trace[0] + i;
...
failed:
	kvfree(trace_ctrl->all_trace[0]);
	for (i = 0; i < IO_MAX_PROCESS; i++)
		trace_ctrl->all_trace[i] = NULL;
