...
do { \
	if ((!(data)) || \
		(!((data)->tag >= TAG_SENSOR_BEGIN && ((data)->tag) < TAG_SENSOR_END)) || \
		(!((data)->name))) { \
...
do {\
	if ((!(data)) || \
		(!(((data)->tag >= TAG_SENSOR_BEGIN && ((data)->tag) < TAG_SENSOR_END) || \
		   ((data)->tag >= TAG_EXTEND_BEGIN && ((data)->tag) < TAG_EXTEND_END))) || \
		(!((data)->name))) { \
		hwlog_err("error in %s\n", __func__); \
		return -EINVAL; \
	} \
...
static void split_motion_param(char *buf, unsigned long *cmd, unsigned long *type)
{
	char *temp;
	char *p;
	temp = strtok_s(buf, " ", &p);
	if (kstrtoul(temp, TO_DECIMALISM, type))
		hwlog_err("%s: rpc motion request val %lu invalid",
			__func__, *type);
	temp = strtok_s(NULL, " ", &p);
	if (kstrtoul(temp, TO_DECIMALISM, cmd))
		hwlog_err("%s: rpc motion request val %lu invalid",
			__func__, *cmd);
}
	...
	struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if (size > MOTION_CTRL_PAR) {
		char temp[MOTION_CTRL_NUM] = {0};
		if (memcpy_s(temp, sizeof(temp), buf, strlen(buf))) {
			hwlog_err("%s:memcpy_s failed\n", __func__);
			return false;
		}
		unsigned long type = 0;
		unsigned long cmd = 0;
		split_motion_param(temp, &cmd, &type);
		send_motion_type_switch(type, cmd);
		return size;
	}
...
static ssize_t show_fix_2_i2c_info(struct device *dev, struct device_attribute *attr,
	char *buf)
{
...
static ssize_t show_get_data(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	if (data->tag < TAG_EXTEND_BEGIN) {
		tag_type = get_tag_to_hal_sensor_type();
		hal_sensor_tag = tag_type[data->tag];
	} else {
		tag_type = get_tag_to_hal_sensor_type_extend();
		hal_sensor_tag = tag_type[data->tag - TAG_EXTEND_BEGIN];
	}
#endif
...
static DEVICE_ATTR(acc1_sensorlist_info, 0664, show_acc1_sensorlist_info, NULL);
static DEVICE_ATTR(acc2_sensorlist_info, 0664, show_acc2_sensorlist_info, NULL);
...
static DEVICE_ATTR(gyro1_sensorlist_info, 0664, show_gyro1_sensorlist_info, NULL);
static DEVICE_ATTR(gyro2_sensorlist_info, 0664, show_gyro2_sensorlist_info, NULL);
...
static DEVICE_ATTR(fix_2_i2c, 0664, show_fix_2_i2c_info, NULL);
...
ssize_t show_gyro_do_ois_offset_in_ui(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	int flag = 0;
	flag = get_do_ois_offset_in_ui_val();
	hwlog_info("gyro_do_ois_offset_in_ui:%d\n", flag);
	snprintf_s(buf, MAX_STR_SIZE, MAX_STR_SIZE - 1, "%d\n", flag);
	return flag;
}
...
static DEVICE_ATTR(acc2_offset_data, 0444, show_acc2_offset_data, NULL);
...
static DEVICE_ATTR(gyro2_offset_data, 0444, show_gyro2_offset_data, NULL);
static DEVICE_ATTR(gyro_do_ois_offset_in_ui, 0444, show_gyro_do_ois_offset_in_ui, NULL);
...
static ssize_t attr_als_calibrate_zhongtongline_store(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
	struct sensor_cookie *data =
		(struct sensor_cookie *)dev_get_drvdata(dev);
	...
	CHECK_SENSOR_COOKIE(data);
	return als_calibrate_zhongtongline_store(data->tag, dev, attr, buf, count);
}

static DEVICE_ATTR(als_calibrate_zhongtongline, 0660,
	NULL, attr_als_calibrate_zhongtongline_store);
...
static ssize_t attr_als_calibrate_product_type_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	struct sensor_cookie *data =
		(struct sensor_cookie *)dev_get_drvdata(dev);
	...
	CHECK_SENSOR_COOKIE(data);
	return als_calibrate_product_type_show(data->tag, dev, attr, buf);
}

static DEVICE_ATTR(als_calibrate_product_type, 0664,
	attr_als_calibrate_product_type_show, NULL);
#endif
...
static struct attribute *acc1_sensor_attrs[] = {
	...
	&dev_attr_calibrate.attr,
	&dev_attr_calibrate_timeout.attr,
	&dev_attr_acc1_sensorlist_info.attr,
...
// acc2
static struct attribute *acc2_sensor_attrs[] = {
	&dev_attr_get_data.attr,
	&dev_attr_self_test.attr,
	&dev_attr_self_test_timeout.attr,
	&dev_attr_calibrate.attr,
	&dev_attr_calibrate_timeout.attr,
	&dev_attr_acc2_sensorlist_info.attr,
	&dev_attr_acc2_offset_data.attr,
	&dev_attr_unacc_time_offset.attr,
	NULL,
};

static const struct attribute_group acc2_sensor_attrs_grp = {
	.attrs = acc2_sensor_attrs,
};
...
static struct attribute *als_sensor_attrs[] = {
	...
	&dev_attr_als_calibrate_zhongtongline.attr,
	...
	&dev_attr_als_calibrate_product_type.attr,
#endif
	NULL,
};
...
static struct attribute *gyro_sensor_attrs[] = {
	...
	&dev_attr_get_sensor_id.attr,
	&dev_attr_gyro_do_ois_offset_in_ui.attr,
	NULL,
};
...
static struct attribute *gyro1_sensor_attrs[] = {
	...
	&dev_attr_calibrate.attr,
	&dev_attr_calibrate_timeout.attr,
	&dev_attr_gyro1_sensorlist_info.attr,
	&dev_attr_gyro_position_info.attr,
...
static struct attribute *gyro2_sensor_attrs[] = {
	&dev_attr_get_data.attr,
	&dev_attr_self_test.attr,
	&dev_attr_self_test_timeout.attr,
	&dev_attr_calibrate.attr,
	&dev_attr_calibrate_timeout.attr,
	&dev_attr_gyro2_sensorlist_info.attr,
	&dev_attr_gyro_position_info.attr,
	&dev_attr_gyro2_offset_data.attr,
	&dev_attr_ungyro_time_offset.attr,
	NULL,
};

static const struct attribute_group gyro2_sensor_attrs_grp = {
	.attrs = gyro2_sensor_attrs,
};
...
static ssize_t store_enable_motion_type(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t size)
{
	char temp[MOTION_CTRL_NUM] = {0};
	if (memcpy_s(temp, sizeof(temp), buf, strlen(buf))) {
	...
	unsigned long cmd = 0;
	split_motion_param(temp, &cmd, &type);
	if (type != MOTION_TYPE_SHAKE) {
		hwlog_err("wrong open type\n");
		return -EINVAL;
	}
	update_shake_info(cmd, type);
	send_motion_type_open(cmd, type);
	return size;
}

static DEVICE_ATTR(enable_motion_type, 0660, NULL, store_enable_motion_type);

static struct attribute *motion_sensor_attrs[] = {
	&dev_attr_enable_motion_type.attr,
	NULL,
};

static const struct attribute_group motion_sensor_attrs_grp = {
	.attrs = motion_sensor_attrs,
};
...
static struct sensor_cookie all_sensors[] = {
	...
	},
	{
		.tag = TAG_ACC2,
		.name = "acc2_sensor",
		.attrs_group = &acc2_sensor_attrs_grp,
	},
	{
		.tag = TAG_GYRO1,
		.name = "gyro1_sensor",
	...
	},
	{
		.tag = TAG_GYRO2,
		.name = "gyro2_sensor",
		.attrs_group = &gyro2_sensor_attrs_grp,
	},
	{
		.tag = TAG_ALS1,
		.name = "als1_sensor",
	...
	},
	{
		.tag = TAG_MOTION,
		.name = "motion",
		.attrs_group = &motion_sensor_attrs_grp,
	},
};
...
static void init_sensors_get_data(void)
{
	...
	for (tag = TAG_EXTEND_BEGIN; tag < TAG_EXTEND_END; ++tag) {
		int i = tag - TAG_EXTEND_BEGIN;
		/* 1 means fail */
		if (memcpy_s(sensor_status.selftest_result_extend[i],
			sizeof(sensor_status.selftest_result_extend[i]),
			"1", sizeof("1")) != EOK) {
			hwlog_err("%s memcpy_s error tag=%d\n", __func__, tag);
			return;
		}
