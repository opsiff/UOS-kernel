...
struct g_sensor_platform_data *acc_get_platform_data(int32_t tag)
{
	...
	else if (tag == TAG_ACC2)
		return (&(gsensor_data[ACC_DEVICE_ID_2]));
#endif
...
struct acc_device_info *acc_get_device_info(int32_t tag)
{
	if (tag == TAG_ACCEL)
		...
		return (&(g_acc_dev_info[ACC_DEVICE_ID_2]));
#endif
	hwlog_info("%s error, please check tag %d\n", __func__, tag);
	return NULL;
}
	...
	struct sensor_detect_manager *sm)
{
	...
	if (of_property_read_u32(dn, "need_fix_i2c", &temp)) {
		hwlog_err("%s:read %s need_fix_i2c fail\n",
			__func__, sm->sensor_name_str);
	} else {
		pf_data->need_fix_i2c = (uint8_t)temp;
		g_fix_2_i2c = (uint8_t)temp;
	}

	hwlog_info("read_acc_config4_from_dts need_fix_i2c %d\n", (int)(pf_data->need_fix_i2c));
}
...
int acc2_data_from_mcu(const struct pkt_header *head)
{
	hwlog_info("acc2_data_from_mcu\n");
	switch (((pkt_acc2_calibrate_data_req_t *)head)->subcmd) {
	case SUB_CMD_SELFCALI_REQ:
		return write_gsensor2_offset_to_nv(
			((pkt_acc2_calibrate_data_req_t *)head)->calibrate_data,
			head->length-SUBCMD_LEN);
	default:
		hwlog_info("uncorrect subcmd 0x%x\n",
			((pkt_acc2_calibrate_data_req_t *)head)->subcmd);
	}
	return 0;
}

uint8_t get_fix_2_i2c(void)
{
	return g_fix_2_i2c;
}

void set_fix_2_i2c_2_finishsed(void)
{
	g_fix_2_i2c = FIX_2_I2C_SENSOR_INIT_FIINSHED;
}

void set_fix_2_i2c_to_no_grl_dedicated_board(void)
{
	g_fix_2_i2c = FIX_2_I2C_SENSOR_NO_GRL_DEDICATED_BOARD;
}

int acc_data_from_mcu(const struct pkt_header *head)
{
	hwlog_info("acc_data_from_mcu\n");
	switch (((pkt_acc_calibrate_data_req_t *)head)->subcmd) {
	case SUB_CMD_SELFCALI_REQ:
		return write_gsensor_offset_to_nv(
			((pkt_acc_calibrate_data_req_t *)head)->calibrate_data,
			head->length-SUBCMD_LEN);
	default:
		hwlog_info("uncorrect subcmd 0x%x\n",
			((pkt_acc_calibrate_data_req_t *)head)->subcmd);
	}
	return 0;
}