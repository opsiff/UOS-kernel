...
ssize_t show_acc1_sensorlist_info(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	if (memcpy_s(buf, MAX_STR_SIZE, get_sensorlist_info_by_index(ACC1),
		sizeof(struct sensorlist_info)) != EOK)
		return -1;
	return sizeof(struct sensorlist_info);
}

ssize_t show_acc2_sensorlist_info(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	if (memcpy_s(buf, MAX_STR_SIZE, get_sensorlist_info_by_index(ACC2),
		sizeof(struct sensorlist_info)) != EOK)
		return -1;
	return sizeof(struct sensorlist_info);
}

static long unacc_timestamp_offset;
	...
	struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if (kstrtol(buf, 10, &val)) { /* change to 10 type */
		hwlog_err("%s: read unacc_timestamp_offset val %ld invalid",
			__func__, val);
		return -EINVAL;
	}
	...
	struct device_attribute *attr, char *buf)
{
	...
	hwlog_info("%s: acc_time_offset is %ld\n",
...
ssize_t show_acc2_offset_data(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	return snprintf_s(buf, MAX_STR_SIZE, MAX_STR_SIZE - 1,
		"acc offset:%d %d %d\n",
