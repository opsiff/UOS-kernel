...
static char *acc_test_name[ACC_DEV_COUNT_MAX][ACC_CAL_NUM] = {
	...
	{
	"ACC2_CALI_X_OFFSET_MSG", "ACC2_CALI_Y_OFFSET_MSG",
	"ACC2_CALI_Z_OFFSET_MSG",
	"ACC2_CALI_X_SEN_MSG", "ACC2_CALI_Y_SEN_MSG", "ACC2_CALI_Z_SEN_MSG",
	"ACC2_CALI_XIS_00_MSG", "ACC2_CALI_XIS_01_MSG", "ACC2_CALI_XIS_02_MSG",
	"ACC2_CALI_XIS_10_MSG", "ACC2_CALI_XIS_11_MSG", "ACC2_CALI_XIS_12_MSG",
	"ACC2_CALI_XIS_20_MSG", "ACC2_CALI_XIS_21_MSG", "ACC2_CALI_XIS_22_MSG"
	}
};
#endif
...
static int32_t set_acc2_calib_data[ACC2_CALIDATA_LEN];
...
int acc2_calibrate_save(const void *buf, int length)
{
	struct acc_device_info *dev_info = NULL;
	const int32_t *poffset_data = (const int32_t *)buf;
	int ret;
	...
	hwlog_info("%s:acc2 calibrate ok, %d  %d  %d\n",
		__func__, *poffset_data, *(poffset_data + 1), *(poffset_data + 2));

	for (i = 0; i < ACC_OFFSET_CALIBRATE_LENGTH; i++) {
		if (*(poffset_data + i) < acc_calib_threshold[i].low_threshold ||
			*(poffset_data + i) > acc_calib_threshold[i].high_threshold) {
			hwlog_err("%s: acc2 calibrated_data is out of range. i = %d, num = %d\n",
				__FUNCTION__, i, *(poffset_data + i));
			dev_info->acc_calibration_res = NV_FAIL;
			return -1;
		}
	...
	ret = write_gsensor2_offset_to_nv((char *)buf, length);
	if (ret) {
		hwlog_err("nv write fail\n");
		dev_info->acc_calibration_res = NV_FAIL;
		return -1;
...
static int acc2_calibrate_on_enter(struct acc_device_info *dev_info)
{
	int ret;
	interval_param_t param = { 0 };

	/* if assistant_acc is not opened, open first */
	if (sensor_status.opened[TAG_ACC2] == 0) {
		dev_info->acc_opened_before_calibrate = 0;
		hwlog_info("send acc2 open cmd(during calibrate) to mcu.\n");
		ret = inputhub_sensor_enable(TAG_ACC2, true);
		if (ret) {
			dev_info->acc_calibration_res = COMMU_FAIL;
			hwlog_err("send acc2 open cmd(during calibrate) to mcu fail,ret=%d\n",
				ret);
			return -1;
		}
	} else {
	...
	/* period must <= 100 ms */
	if ((sensor_status.delay[TAG_ACC2] == 0) ||
		(sensor_status.delay[TAG_ACC2] > ACC_PERIOD_MAX)) {
		hwlog_info("send acc2 setdelay cmd(during calibrate) to mcu\n");
		param.period = ACC_PERIOD_MAX;
		ret = inputhub_sensor_setdelay(TAG_ACC2, &param);
		if (ret) {
			dev_info->acc_calibration_res = COMMU_FAIL;
			hwlog_err("send acc2 set delay cmd(during calibrate) to mcu fail,ret=%d\n",
				ret);
			return -1;
		}
	}
...
static void acc2_calibrate_on_exit(struct acc_device_info *dev_info)
{
	int ret;

	if (dev_info->acc_opened_before_calibrate == 0) {
		dev_info->acc_opened_before_calibrate = 1;
		hwlog_info("send acc2 close cmd(during calibrate) to mcu\n");
		ret = inputhub_sensor_enable(TAG_ACC2, false);
		if (ret) {
			dev_info->acc_calibration_res = COMMU_FAIL;
			hwlog_err("send acc2 close cmd(during calibrate) to mcu fail,ret=%d\n",
				ret);
			return;
		}
	}
...
ssize_t attr_acc2_calibrate_write(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
	struct g_sensor_platform_data *pf_data = NULL;
	struct acc_device_info *dev_info = NULL;
	...
	pf_data = acc_get_platform_data(TAG_ACC2);
	dev_info = acc_get_device_info(TAG_ACC2);
	if (pf_data == NULL || dev_info == NULL)
		return -1;
	if (kstrtoul(buf, TO_DECIMALISM, &val))
	...
	if (val != 1) {
		hwlog_err("send acc2 calibrate value is error value=%lu\n", val);
		return count;
	}

	if (acc2_calibrate_on_enter(dev_info))
		return count;

	msleep(300); /* sleep 300 ms */
	/* send calibrate command, need set delay first */
	read_pkg = send_calibrate_cmd(TAG_ACC2, val, &rtype);
	dev_info->acc_calibration_res = rtype;
	if (dev_info->acc_calibration_res == COMMU_FAIL)
		return count;
	else if (read_pkg.errno == 0)
		acc2_calibrate_save(read_pkg.data, read_pkg.data_length);
	...
	acc2_calibrate_on_exit(dev_info);
	return count;
}
...
ssize_t show_acc2_set_calidata(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	struct g_sensor_platform_data *pf_data = NULL;

	hwlog_info("%s: inter show acc2 set calidata node\n", __func__);
	pf_data = acc_get_platform_data(TAG_ACC2);
	if (pf_data == NULL)
		return -1;
	...
	if (strlen(get_sensor_chip_info_address(ACC2)) != 0)
		return snprintf_s(buf, MAX_STR_SIZE, MAX_STR_SIZE - 1,
			"%d %d %d %d %d %d %d %d %d %d %d %d %d %d %d\n",
			/* offset:data[0]-data[2] sensitivity:data[3]-data[5] */
			set_acc2_calib_data[0], set_acc2_calib_data[1],
			set_acc2_calib_data[2], set_acc2_calib_data[3],
			set_acc2_calib_data[4], set_acc2_calib_data[5],
			/* six_angle:data[6] - data[14] */
			set_acc2_calib_data[6], set_acc2_calib_data[7],
			set_acc2_calib_data[8], set_acc2_calib_data[9],
			set_acc2_calib_data[10], set_acc2_calib_data[11],
			set_acc2_calib_data[12], set_acc2_calib_data[13],
			set_acc2_calib_data[14]);
...
ssize_t store_acc2_set_calidata(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t size)
{
	struct g_sensor_platform_data *pf_data = NULL;
	int32_t set_acc2_sensor_offset[ACC2_OFFSET_DATA_LENGTH] = { 0 };
	int i;

	hwlog_info("%s: inter acc2 set calidata node\n", __func__);
	pf_data = acc_get_platform_data(TAG_ACC2);
	if (pf_data == NULL)
		return -1;

	if (pf_data->calibrate_way == 0) {
		hwlog_info("%s: now is acc2 self_calibreate\n", __func__);
		return size;
	}
	if (strlen(get_sensor_chip_info_address(ACC2)) == 0) {
		hwlog_err("ACC2 not exits !!\n");
		return -1;
	}
	if (size != sizeof(set_acc2_calib_data)) {
		hwlog_err("%s:size %lu is not equal 15*4\n", __func__, size);
		return -1;
	}
	if (memcpy_s(set_acc2_calib_data, sizeof(set_acc2_calib_data),
		buf, size) != EOK)
		return -1;
	for (i = 0; i < CALIBRATE_DATA_LENGTH; i++) {
		if (set_acc2_calib_data[i] < acc_calib_threshold[i].low_threshold ||
		    set_acc2_calib_data[i] > acc_calib_threshold[i].high_threshold) {
			hwlog_err("%s: acc2 calibrated_data is out of range. i = %d, num = %d\n",
				__FUNCTION__, i, set_acc2_calib_data[i]);
			return -1;
		}
	}
	acc2_calibrate_save(set_acc2_calib_data, sizeof(set_acc2_calib_data));
	for (i = 0; i < CALIBRATE_DATA_LENGTH; i++)
		hwlog_info("set acc2 calibrate success data[%d] value is %d\n",
			i, set_acc2_calib_data[i]);
	...
	/* copy to gsensor_offset by pass */
	if (memcpy_s(set_acc2_sensor_offset, sizeof(set_acc2_sensor_offset),
		user_info.nv_data, sizeof(set_acc2_sensor_offset)) != EOK)
		return -1;
	hwlog_info("nve_direct_access read acc2_sensor offset: %d %d %d  sensitity:%d %d %d\n",
		set_acc2_sensor_offset[0], set_acc2_sensor_offset[1],
		set_acc2_sensor_offset[2], set_acc2_sensor_offset[3],
		set_acc2_sensor_offset[4], set_acc2_sensor_offset[5]);
	hwlog_info("nve_direct_access read acc2_sensor xis_angle: %d %d %d  %d %d %d %d %d %d\n",
		set_acc2_sensor_offset[6], set_acc2_sensor_offset[7],
		set_acc2_sensor_offset[8], set_acc2_sensor_offset[9],
		set_acc2_sensor_offset[10], set_acc2_sensor_offset[11],
		set_acc2_sensor_offset[12], set_acc2_sensor_offset[13],
		set_acc2_sensor_offset[14]);

	if (send_calibrate_data_to_mcu(TAG_ACC2, SUB_CMD_SET_OFFSET_REQ,
		set_acc2_sensor_offset, sizeof(set_acc2_sensor_offset), false))
		return -1;
	return size;
}
