...
static char *gyro_test_name[GYRO_DEV_COUNT_MAX][GYRO_CAL_NUM] = {
	...
	{
	"GYRO2_CALI_X_OFFSET_MSG", "GYRO2_CALI_Y_OFFSET_MSG",
	"GYRO2_CALI_Z_OFFSET_MSG",
	"GYRO2_CALI_X_SEN_MSG", "GYRO2_CALI_Y_SEN_MSG", "GYRO2_CALI_Z_SEN_MSG",
	"GYRO2_CALI_XIS_00_MSG", "GYRO2_CALI_XIS_01_MSG",
	"GYRO2_CALI_XIS_02_MSG", "GYRO2_CALI_XIS_10_MSG",
	"GYRO2_CALI_XIS_11_MSG", "GYRO2_CALI_XIS_12_MSG",
	"GYRO2_CALI_XIS_20_MSG", "GYRO2_CALI_XIS_21_MSG",
	"GYRO2_CALI_XIS_22_MSG"
	}
};
#endif
...
static int32_t set_gyro2_calib_data[GYRO2_CALIDATA_LEN];
...
static int gyro2_calibrate_save(const void *buf, int length)
{
	struct gyro_device_info *dev_info = NULL;
	...
	hwlog_info("%s:gyro2 calibrate ok, %d %d %d\n", __func__,
	...
	for (i = 0; i < GYRO_OFFSET_CALIBRATE_LENGTH; i++) {
		if (*(poffset_data + i) < gyro_calib_threshold[i].low_threshold ||
			*(poffset_data + i) > gyro_calib_threshold[i].high_threshold) {
			hwlog_err("%s: gyro2 calibrated_data is out of range. i = %d, num = %d\n",
				__func__, i, *(poffset_data + i));
			dev_info->gyro_calibration_res = NV_FAIL;
			return -1;
		}
	}
	ret = write_gyro2_sensor_offset_to_nv((char *)buf, length);
	if (ret) {
...
static int gyro2_calibrate_on_write(struct gyro_platform_data *pf_data,
	struct gyro_device_info *dev_info, unsigned long val)
{
	...
	read_pkg = send_calibrate_cmd(TAG_GYRO2, val, &rtype);
	...
	if (dev_info->gyro_calibration_res == COMMU_FAIL) {
		...
		gyro2_calibrate_save(dev_info->gyro_calib_data,
...
static int gyro2_calibrate_on_enter(struct gyro_device_info *dev_info)
{
	int ret;
	interval_param_t param = { 0 };

	/* if gyro2 is not opened, open first */
	if (sensor_status.opened[TAG_GYRO2] == 0) {
		dev_info->gyro_opened_before_calibrate = 0;
		hwlog_info("send gyro2 open cmd(during calibrate) to mcu.\n");
		ret = inputhub_sensor_enable(TAG_GYRO2, true);
		if (ret) {
	...
	if ((sensor_status.delay[TAG_GYRO2] == 0) ||
		(sensor_status.delay[TAG_GYRO2] > GYRO_PERIOD_MAX)) {
		hwlog_info("send gyro2 setdelay cmd(during calibrate) to mcu\n");
		param.period = GYRO_PERIOD_MAX;
		ret = inputhub_sensor_setdelay(TAG_GYRO2, &param);
		if (ret) {
			dev_info->gyro_calibration_res = COMMU_FAIL;
			hwlog_err("send gyro2 set delay cmd(during calibrate) to mcu fail,ret=%d\n",
				ret);
			return -1;
		}
	}
...
static void gyro2_calibrate_on_exit(struct gyro_device_info *dev_info)
{
	int ret;
	...
	if (dev_info->gyro_opened_before_calibrate == 0) {
		...
		ret = inputhub_sensor_enable(TAG_GYRO2, false);
		if (ret) {
			dev_info->gyro_calibration_res = COMMU_FAIL;
...
ssize_t attr_gyro2_calibrate_write(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	pf_data = gyro_get_platform_data(TAG_GYRO2);
	dev_info = gyro_get_device_info(TAG_GYRO2);
	if (pf_data == NULL || dev_info == NULL)
		return -1;
	...
	if (val != 1) {
		hwlog_err("set gyro2 calibrate val invalid,val=%lu\n", val);
		return count;
	}

	if (gyro2_calibrate_on_enter(dev_info))
		return count;

	msleep(300); /* sleep 300 ms */

	if (gyro2_calibrate_on_write(pf_data, dev_info, val))
		return count;
	gyro2_calibrate_on_exit(dev_info);
	return count;
}
...
ssize_t show_gyro2_set_calidata(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	struct gyro_platform_data *pf_data = NULL;

	hwlog_info("%s: inter show gyro2 set calidata node\n", __func__);
	pf_data = gyro_get_platform_data(TAG_GYRO2);
	if (pf_data == NULL)
		return -1;
	if (pf_data->calibrate_way == 0)
	...
	if (strlen(get_sensor_chip_info_address(GYRO2)) != 0)
		return snprintf_s(buf, MAX_STR_SIZE, MAX_STR_SIZE - 1,
			...
			set_gyro2_calib_data[0], set_gyro2_calib_data[1],
			set_gyro2_calib_data[2], set_gyro2_calib_data[3],
			set_gyro2_calib_data[4], set_gyro2_calib_data[5],
			/* six_angle: data[6]-data[14] */
			set_gyro2_calib_data[6], set_gyro2_calib_data[7],
			set_gyro2_calib_data[8], set_gyro2_calib_data[9],
			set_gyro2_calib_data[10], set_gyro2_calib_data[11],
			set_gyro2_calib_data[12], set_gyro2_calib_data[13],
			set_gyro2_calib_data[14]);
	else
		return -1;
}
...
ssize_t store_gyro2_set_calidata(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t size)
{
	struct gyro_platform_data *pf_data = NULL;
	int set_gyro2_sensor_offset[GYRO2_CALIBRATE_DATA_LENGTH] = { 0 };
	int i;

	hwlog_info("%s: inter gyro2 set calidata node\n", __func__);
	pf_data = gyro_get_platform_data(TAG_GYRO2);
	if (pf_data == NULL)
		return -1;
	if (pf_data->calibrate_way == 0) {
		hwlog_info("%s: now is gyro2 self_calibreate\n", __func__);
		return size;
	}
	if (strlen(get_sensor_chip_info_address(GYRO2)) == 0) {
		hwlog_err("GYRO2 not exits !!\n");
		return -1;
	}
	if (size != sizeof(set_gyro2_calib_data)) {
		hwlog_err("%s:size %lu is not equal 15*4\n", __func__, size);
		return -1;
	}
	if (memcpy_s(set_gyro2_calib_data, sizeof(set_gyro2_calib_data),
		buf, size) != EOK)
		return -1;
	for (i = 0; i < CALIBRATE_DATA_LENGTH; i++) {
		if (set_gyro2_calib_data[i] < gyro_calib_threshold[i].low_threshold ||
		    set_gyro2_calib_data[i] > gyro_calib_threshold[i].high_threshold) {
			hwlog_err("%s: gyro calibrated_data is out of range. i = %d, num = %d\n",
				__func__, i, set_gyro2_calib_data[i]);
			return -1;
		}
	}
	gyro2_calibrate_save(set_gyro2_calib_data, sizeof(set_gyro2_calib_data));
	if (read_calibrate_data_from_nv(GYRO2_OFFSET_NV_NUM,
		GYRO2_OFFSET_NV_SIZE, GYRO2_NV_NAME))
		return -1;

	/* copy to gyro_offset by pass */
	if (memcpy_s(set_gyro2_sensor_offset, sizeof(set_gyro2_sensor_offset),
		user_info.nv_data, sizeof(set_gyro2_sensor_offset)) != EOK)
		return -1;
	hwlog_info("nve_direct_access read gyro2_sensor offset: %d %d %d  sensitity:%d %d %d\n",
		set_gyro2_sensor_offset[0], set_gyro2_sensor_offset[1],
		set_gyro2_sensor_offset[2], set_gyro2_sensor_offset[3],
		set_gyro2_sensor_offset[4], set_gyro2_sensor_offset[5]);
	hwlog_info("nve_direct_access read gyro2_sensor xis_angle: %d %d %d  %d %d %d %d %d %d\n",
		set_gyro2_sensor_offset[6], set_gyro2_sensor_offset[7],
		set_gyro2_sensor_offset[8], set_gyro2_sensor_offset[9],
		set_gyro2_sensor_offset[10], set_gyro2_sensor_offset[11],
		set_gyro2_sensor_offset[12], set_gyro2_sensor_offset[13],
		set_gyro2_sensor_offset[14]);
	if (send_calibrate_data_to_mcu(TAG_GYRO2, SUB_CMD_SET_OFFSET_REQ,
		set_gyro2_sensor_offset, sizeof(set_gyro2_sensor_offset), false))
		return -1;
	return size;
}
