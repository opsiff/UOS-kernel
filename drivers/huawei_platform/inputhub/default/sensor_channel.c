...
static bool sensor_switch_flags_extend[EXTEND_TAGS_NUM] = {false};
#endif
...
static const struct sensors_cmd_map sensors_cmd_map_tab[] = {
	...
	{ SENSORHUB_TYPE_AUDIO_FUSION, TAG_AUDIO_FUSION },
#endif
	{ SENSORHUB_TYPE_DEBUGS, TAG_DEBUGS },
};
...
#ifdef CONFIG_SUPPORT_EXTEND_TAGS
	uint8_t *tag_type_extend = get_tag_to_hal_sensor_type_extend();
#endif
...
#else
		if (sensors_cmd_map_tab[i].tag < TAG_EXTEND_BEGIN)
			tag_type[sensors_cmd_map_tab[i].tag] =
				sensors_cmd_map_tab[i].hal_sensor_type;
		else
			tag_type_extend[sensors_cmd_map_tab[i].tag - TAG_EXTEND_BEGIN] =
				sensors_cmd_map_tab[i].hal_sensor_type;
#endif
...
static bool switch_sensor(int tag, bool enable)
{
	...
	if (tag < TAG_SENSOR_END) {
		...
		if ((enable && !sensor_switch_flags_extend[tag - TAG_EXTEND_BEGIN]) || (!enable)) {
			sensor_switch_flags_extend[tag - TAG_EXTEND_BEGIN] = enable;
			result = true;
		}
	}
#endif
...
static bool ap_sensor_flush(int tag)
{
	...
	t_ap_sensor_ops_record *record = NULL;
	bool work_on_ap = false;
#ifndef CONFIG_SUPPORT_EXTEND_TAGS
	record = get_all_ap_sensor_operations();
	work_on_ap = record[tag].work_on_ap;
#else
	if (tag < TAG_EXTEND_BEGIN) {
		record = get_all_ap_sensor_operations();
		work_on_ap = record[tag].work_on_ap;
	} else {
		record = get_all_ap_sensor_operations_extend();
		work_on_ap = record[tag - TAG_EXTEND_BEGIN].work_on_ap;
	}
#endif
	uint8_t *tag_type = get_tag_to_hal_sensor_type();
#ifdef CONFIG_SUPPORT_EXTEND_TAGS
	...
	if (work_on_ap) {
...
#else
		if (record[tag - TAG_EXTEND_BEGIN].ops.enable) {
			record[tag - TAG_EXTEND_BEGIN].ops.enable(1);
#endif
		...
		if (tag < TAG_EXTEND_BEGIN)
			...
			event.value[0] = tag_type_extend[tag - TAG_EXTEND_BEGIN];
#endif
...
void send_dumpinfor_to_mcu(int tag)
{
	int ret;
	...
	int buf = SUB_CMD_DUMP_REQ;
	pkg_ap.wr_buf = (const void *)(&buf);
	pkg_ap.wr_len = sizeof(int);
	ret = write_customize_cmd(&pkg_ap, NULL, false);
	if (ret)
		hwlog_info("send_dumpinfor_to_mcu fail\n");
	hwlog_info("send_debugs_command command \n");
}
...
void send_debugs_command(int tag, int32_t command)
{
	if (tag != TAG_DEBUGS)
		return;
	hwlog_info("send_debugs_command command %d\n", command);
	int tag1 = -1;
	int argv[1] = {0};
	int argc = 1;
	char buf[MAX_STR_SIZE] = {0};
	switch (command) {
	case SET_DEBUG_MODEL:
		argv[0] = DEBUG;
		set_log_level(tag1, argv, argc);
		break;
	case SET_NORMAL_MODEL:
		...
		break;
	case SET_DUMP_ACC:
		send_dumpinfor_to_mcu(TAG_ACCEL);
		break;
	case SET_DUMP_ACC1:
		send_dumpinfor_to_mcu(TAG_ACC1);
		break;
	case SET_DUMP:
		cls_attr_dump_show_func(NULL, NULL, buf);
		break;
	default:
		hwlog_info("send_debugs_command no def command %d\n", command);
	}
}
...
static void get_acc_calibrate_data(void)
{
	...
	if (dev_info->acc_first_start_flag == 0) {
		...
		if (ret)
			...
			hwlog_info("acc read nv success\n");
	}
}
...
static void get_acc1_calibrate_data(void)
{
	...
	if (dev_info->acc_first_start_flag == 0) {
		...
		if (ret)
			...
			hwlog_info("acc1 read nv success\n");
	}
}

static void get_acc2_calibrate_data(void)
{
	int ret;
	struct acc_device_info *dev_info = NULL;
	...
	if (dev_info == NULL)
		return;
	if (dev_info->acc_first_start_flag == 0) {
		ret = send_gsensor2_calibrate_data_to_mcu();
		if (ret)
			hwlog_err("%s read from nv fail,ret=%d", __func__, ret);
		else
			hwlog_info("acc2 read nv success\n");
	}
}
...
static void get_gyro1_calibrate_data(void)
{
	...
	if (dev_info->gyro_first_start_flag == 0) {
		...
		if (ret)
			...
			hwlog_info("gyro1_sensor read nv success\n");
	}
}

static void get_gyro2_calibrate_data(void)
{
	int ret;
	struct gyro_device_info *dev_info = NULL;
	...
	if (dev_info == NULL)
		return;
	if (dev_info->gyro_first_start_flag == 0) {
		ret = send_gyro2_calibrate_data_to_mcu();
		if (ret)
			hwlog_err("get_gyro2_calibrate_data read from nv fail,ret=%d",
				ret);
		else
			hwlog_info("gyro2_sensor read nv success\n");
	}
}
...
static void enable_sensors_when_recovery_iom3(void)
{
	...
	for (tag = TAG_EXTEND_BEGIN; tag < TAG_EXTEND_END; ++tag) {
		...
		if (sensor_status.status_extend[i]) {
			/* default delay_ms */
			interval_param.period = sensor_status.delay_extend[i];
			interval_param.batch_count =
				sensor_status.batch_cnt_extend[i];
			interval_param.mode = AUTO_MODE;
			interval_param.reserved[0] = TYPE_STANDARD;
			inputhub_sensor_enable_nolock(tag, true);
			inputhub_sensor_setdelay_nolock(tag, &interval_param);
