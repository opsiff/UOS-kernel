...
static const char *str_two_soft_para = "softiron_two_parameter";
static const char *str_soft_para_far = "softiron_parameter_far"; /* fold phone flag */
...
static void read_mag_config4_from_dts(struct device_node *dn,
	struct sensor_detect_manager *sm)
{
	struct compass_platform_data *pf_data =
		(struct compass_platform_data *)sm->spara;
	int temp = 0;
	if (of_property_read_u32(dn, "hall_gpio", &temp)) {
		hwlog_err("%s:read %s mag hall_gpio fail\n",
			__func__, sm->sensor_name_str);
		temp = HALL_STATE_GPIO;
	}
	pf_data->hall_gpio = (uint8_t)temp;

	if (of_property_read_u32(dn, "hall1_gpio", &temp)) {
		hwlog_err("%s:read %s mag hall1_gpio fail\n",
			__func__, sm->sensor_name_str);
		temp = HALL_1_STATE_GPIO;
	}
	pf_data->hall_1_gpio = (uint8_t)temp;
}
#endif
	...
	struct sensor_detect_manager *sm)
{
	...
	read_mag_config4_from_dts(dn, sm);
#endif
	...
	if (sm->sensor_id != MAG)
		return;
	ret = fill_extend_data_in_dts(dn, str_soft_para, pf_data->pdc_data, PDC_SIZE, EXTEND_DATA_TYPE_IN_DTS_BYTE);
	if (ret)
		hwlog_err("%s:fill_extend_data_in_dts fail\n", str_soft_para);
#if defined(CHA_MAG_DETACH) || defined(BBK_MAG_DETACH)
	ret = fill_extend_data_in_dts(dn, str_one_soft_para, pf_data->pdc_one_data, PDC_SIZE, EXTEND_DATA_TYPE_IN_DTS_BYTE);
	if (ret)
		hwlog_err("%s:fill_extend_data_in_dts fail\n", str_one_soft_para);

	ret = fill_extend_data_in_dts(dn, str_two_soft_para, pf_data->pdc_two_data, PDC_SIZE, EXTEND_DATA_TYPE_IN_DTS_BYTE);
	if (ret)
		hwlog_err("%s:fill_extend_data_in_dts fail\n", str_two_soft_para);
#endif

	ret = fill_extend_data_in_dts(dn, str_soft_para_far, mag_iron_data.pdc_data_far, PDC_SIZE,
