	...
	struct device_attribute *attr, char *buf)
{
	...
	for (int i = 0; i < EXTEND_TAGS_NUM; i++)
		hwlog_info("%s\t %s\t %d\n", get_ext_tag_str(i),
			sensor_status.opened_extend[i] ? "open" : "close",
	...
	struct device_attribute *attr, const char *buf, size_t size)
{
	...
	/* set_data_type[0]: 1.ACC 2.GYRO 35.ACC1 36.GYRO1  21.ACC2 38.GYRO2 */
	...
	} else if (set_data_type[0] == TAG_ACC2 ||
		set_data_type[0] == TAG_GYRO2) {
	...
	struct device_attribute *attr, char *buf)
{
	...
	case TAG_PS:
		return attr_get_ps_sensor_id(dev, attr, buf);
	default:
	...
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	case TAG_ACC2:
		return attr_acc2_calibrate_write(dev, attr, buf, count);
	case TAG_GYRO2:
		return attr_gyro2_calibrate_write(dev, attr, buf, count);
	case TAG_CAP_PROX1:
