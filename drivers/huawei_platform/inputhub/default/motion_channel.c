...
#define MOTION_OPEN_CMD_REQ 1
#define MOTION_CLOSE_CMD_REQ 3
...
static int MOTION_TYPE_BLE_RANGE = 25;
...
static const struct motions_cmd_map motions_cmd_map_tab[] = {
	...
	{ MHB_IOCTL_MOTION_CONFIG, -1, TAG_MOTION,
		CMD_CMN_CONFIG_REQ, SUB_CMD_NULL_REQ },
};
...
static int send_motion_config_cmd_internal(int tag, enum obj_cmd cmd,
	enum obj_sub_cmd subcmd, uint8_t* data, bool use_lock)
{
	hwlog_info("Enter cmd:%d in %s\n", cmd, __func__);
	uint8_t app_config[44] = { 0 };

	if (memcpy_s((void *)app_config, sizeof(app_config),
		 data, sizeof(app_config)) != 0) {
		hwlog_err("memcpy_s failed!\n");
		return -EINVAL;
	}
	send_app_config_cmd(TAG_MOTION, app_config, use_lock);
	hwlog_info("Exit cmd:%d in %s\n", cmd, __func__);
	return 0;
}
...
static int send_motion_cmd(unsigned int cmd, unsigned long arg)
{
	...
	if (argvalue != MOTION_TYPE_BLE_RANGE) {
	...
	data[0] = (motion_type_t)argvalue;
	hwlog_info("copy_from_user %s cmd %d argvalue %d\n", __func__, cmd, argvalue);
	if (copy_from_user(data_tmp, argp + 4, sizeof(data_tmp))) {
		hwlog_err("copy_from_user failed %s \n", __func__);
		return -EFAULT;
	}

	if (memcpy_s(&data[1], sizeof(data) - 1, data_tmp,  sizeof(data_tmp)) != 0) {
		hwlog_err("memcpy_s failed %s \n", __func__);
		return -EFAULT;
	}

	return send_motion_config_cmd_internal(motions_cmd_map_tab[i].tag,
		motions_cmd_map_tab[i].cmd, motions_cmd_map_tab[i].subcmd,
		data, true);
}
...
{
	update_motion_info(cmd, arg);
}

void send_motion_type_open(unsigned int cmd, unsigned long arg)
{
	if (cmd == MOTION_OPEN_CMD_REQ)
		send_motion_cmd_internal(motions_cmd_map_tab[0].tag,
			motions_cmd_map_tab[0].cmd, motions_cmd_map_tab[0].subcmd,
			arg, true);
	if (cmd == MOTION_CLOSE_CMD_REQ)
		send_motion_cmd_internal(motions_cmd_map_tab[1].tag,
			motions_cmd_map_tab[1].cmd, motions_cmd_map_tab[1].subcmd,
			arg, true);
}
