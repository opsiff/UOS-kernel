...
void disable_sensors_when_suspend(void)
{
	...
	int i = 0;
	for (i = 0; i < EXTEND_TAGS_NUM; ++i) {
		tag = TAG_EXTEND_BEGIN + i;
		if ((sensor_status_backup.status_extend[i] ||
			sensor_status_backup.opened_extend[i]) &&
			!(get_hifi_supported() == 1 &&
			(sensor_status.batch_cnt_extend[i] > 1))) {
			if (should_be_processed_when_sr(tag))
				inputhub_sensor_enable(tag, false);
		}
	}
...
#ifdef CONFIG_SUPPORT_EXTEND_TAGS
static void enable_sensors_when_resume_ext_tags(void)
{
	int tag;
	...
	for (int i = 0; i < EXTEND_TAGS_NUM; i++) {
		tag = TAG_EXTEND_BEGIN + i;
		if ((sensor_status_backup.status_extend[i] ||
			 sensor_status_backup.opened_extend[i]) &&
			!(get_hifi_supported() == 1 &&
			...
			if (sensor_status_backup.opened_extend[i] &&
				(sensor_status.opened_extend[i] == 0))
				inputhub_sensor_enable(tag, true);
			if (sensor_status_backup.status_extend[i]) {
				delay_param.period =
					sensor_status.status_extend[i] ?
					sensor_status.delay_extend[i] :
					sensor_status_backup.delay_extend[i];
				delay_param.batch_count =
					sensor_status.status_extend[i] ?
					sensor_status.batch_cnt_extend[i] :
					sensor_status_backup.batch_cnt_extend[i];
...
#ifdef CONFIG_SUPPORT_EXTEND_TAGS
	enable_sensors_when_resume_ext_tags();
#endif
}
