...
static int get_nv(int tag, int argv[], int argc)
{
	...
	user_info.nv_number = argv[0];
	user_info.nv_operation = argv[1];
	switch (argv[0]) {
	case ACC_OFFSET_NV_NUM:
		strncpy_s(user_info.nv_name, sizeof(user_info.nv_name), "gsensor", sizeof(user_info.nv_name));
		user_info.valid_size = ACC_OFFSET_NV_SIZE;
		break;
	case ACC1_OFFSET_NV_NUM:
		strncpy_s(user_info.nv_name, sizeof(user_info.nv_name), "GSENSOR1", sizeof(user_info.nv_name));
		user_info.valid_size = ACC1_OFFSET_NV_SIZE;
		break;
	case GYRO_CALIDATA_NV_NUM:
		strncpy_s(user_info.nv_name, sizeof(user_info.nv_name), "GYRO", sizeof(user_info.nv_name));
		user_info.valid_size = GYRO_CALIDATA_NV_SIZE;
		break;
	case GYRO1_OFFSET_NV_NUM:
		strncpy_s(user_info.nv_name, sizeof(user_info.nv_name), "GYRO1", sizeof(user_info.nv_name));
		user_info.valid_size = GYRO1_OFFSET_NV_SIZE;
		break;
	default:
		break;
	}
	if (user_info.nv_operation == NV_READ_TAG)
		ret = nve_direct_access_interface(&user_info);
	if (user_info.nv_operation == NV_WRITE_TAG) {
		int k;
		for (k = 2; k < argc; k++)
			user_info.nv_data[k - 2] = (u_char)argv[k];
		ret = nve_direct_access_interface(&user_info);
	}
	if (ret != 0) {
		hwlog_err("nve_direct_access_interface read %d error %d\n", user_info.nv_name, ret);
		return -1;
	}
...
static int als_param_write(int tag, int argv[], int argc)
{
	...
	if (argv[0] > 29 || argv[0] < 0) { /* 29:para num threshold */
...
static void register_my_debug_operations(void)
{
	...
	register_sensorhub_debug_op(get_nv);
}
...
static void unregister_my_debug_operations(void)
{
	...
	ret |= (unsigned int)unregister_sensorhub_debug_operation(get_nv);
	if (ret != 0)
		hwlog_err("%s err\n", __func__);
}
...
ssize_t cls_attr_dump_show_func(struct class *cls, struct class_attribute *attr, char *buf)
{
