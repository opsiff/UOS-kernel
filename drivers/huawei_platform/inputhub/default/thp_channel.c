...
static uint8_t g_tp_ud_open_flag;
...
int send_tp_ap_event(int inpara_len, const char *inpara, uint8_t cmd)
{
	...
	hwlog_info("thp: %s, inpara[0]= %d\n", __func__, inpara[0]);

	/* inpara[0] is screen on/off flag */
	/* g_tp_ud_open_flag is the gesture status used in recovery mode */
	if (inpara[0])
		g_tp_ud_open_flag = 1;
	else
		g_tp_ud_open_flag = 0;
...
static void enable_tp_ud_when_recovery_iom3(void)
{
	...
	hwlog_info("%s:tsa_event_to_udfp_switch_enable, ret = %d\n", __func__, ret);
}
			...
			unsigned long foo, void *bar)
{
	...
	case IOM3_RECOVERY_IDLE:
		...
		if (g_tp_ud_open_flag)
			enable_tp_ud_when_recovery_iom3();
		break;
