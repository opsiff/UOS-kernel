...
static char *als_test_name[ALS_DEV_COUNT_MAX][ALS_CAL_NUM] = {
	{
	"ALS_R", "ALS_G", "ALS_B",
	"ALS_C", "ALS_LUX", "ALS_CCT"
	},
	{
	"ALS1_R", "ALS1_G", "ALS1_B",
	"ALS1_C", "ALS1_LUX", "ALS1_CCT"
	},
	{
	"ALS2_R", "ALS2_G", "ALS2_B",
	"ALS2_C", "ALS2_LUX", "ALS2_CCT"
	}
};
...
#ifdef SENSOR_DATA_ACQUISITION
static const char *als_get_lcd_model_by_dev_id(int8_t dev_id)
{
	int ret;
	...
	if (dev_id != 0 && dev_id != 1) {
		hwlog_err("%s donot have this devid %d\n", __func__, dev_id);
		return target_model;
	}
	pf_data = als_get_platform_data(TAG_ALS);
	if (!pf_data) {
		hwlog_err("%s: als under tp no tag\n", __func__);
		return target_model;
	}
	hwlog_info("%s als_phone_type = %d\n", __func__, pf_data->als_phone_type);
	...
	hwlog_info("%s lcd model %s\n", __func__, lcd_model);

	target_model = (dev_id == 0 ? lcd_model : sec_lcd_model);
	if (pf_data->als_phone_type == ALTB)
		target_model = (dev_id == 0 ? sec_lcd_model : lcd_model);
	return target_model;
}
...
static void als_test_name_join_with_device(struct als_device_info *dev_info)
{
	...
	char lcd_name[ALS_TEST_NAME_MAX_LEN] = "_L_";
	...
	const char *lcd_model = als_get_lcd_model_by_dev_id(dev_id);
	if (lcd_model != NULL) {
		ret = strncat_s(lcd_name, sizeof(lcd_name), lcd_model, ALS_TEST_ADD_LEN);
		if (ret)
			hwlog_err("%s memcpy lcdmodel fail\n", __func__);
	}
	for (i = 0; i < ALS_CAL_NUM; i++) {
		...
		ret = strncat_s(als_test_with_device_name[dev_id][i],
			sizeof(als_test_with_device_name[dev_id][i]), lcd_name, strlen(lcd_name));
		if (ret)
			hwlog_err("%s strncat fail dev id %d %d, ret %d\n", __func__, dev_id, i, ret);
