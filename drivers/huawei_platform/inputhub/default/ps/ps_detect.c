...
#define RET_FAIL              (-1)
#define RET_SUCC              0
#define PS_SENSOR_DEFAULT_ID "8"

static char *g_ps_sensor_id = PS_SENSOR_DEFAULT_ID;
enum {
	TP_REPLACE_PS = 1,
	SONIC_REPLACE_PS = 2,
};
...
char *ps_get_sensors_id_string(void)
{
	return g_ps_sensor_id;
}

void ps_get_sensors_id_from_dts(struct device_node *dn)
{
	if (dn == NULL)
		return;
	if (of_property_read_string(dn,
		"ps_sensor_id", (const char **)&g_ps_sensor_id)) {
		g_ps_sensor_id = PS_SENSOR_DEFAULT_ID;
		hwlog_err("ps_sensor_id err\n");
	}
	hwlog_info("ps_sensor_id:%s\n", g_ps_sensor_id);
}
...
static int send_ps_para_to_mcu(int32_t tag, const unsigned char *input, int len, enum obj_sub_cmd subcmd)
{
...
void ps_set_config_to_mcu(int32_t tag)
{
	...
	ret = send_ps_para_to_mcu(tag, ps_para, sizeof(int), SUB_CMD_PS_NEED_FLAG_REQ);
	if (ret != EOK) {
...
void ps_set_foldtype_to_mcu(int32_t tag)
{
	int ret;
	unsigned char fold_type[sizeof(int)] = { 0 };

	ret = memcpy_s(fold_type, sizeof(int), &device_fold_type, sizeof(int));
	if (ret != EOK) {
		hwlog_err("ps_set_foldtype_to_mcu memcpy_s para err\n");
		return;
	}
	ret = send_ps_para_to_mcu(tag, fold_type, sizeof(int), SUB_CMD_PS_SET_FOLD_TYPE);
	if (ret != EOK) {
		hwlog_err("ps_set_foldtype_to_mcu failed\n");
		return;
	}
}

void ps_send_not_replaced_to_mcu(int32_t tag)
{
	int ret;
	unsigned char replace_type[sizeof(int)] = { 0 };

	if (replace_ps_type) {
		hwlog_info("ps is replaced!\n");
		return;
	}
	ret = memcpy_s(replace_type, sizeof(int), &replace_ps_type, sizeof(int));
	if (ret != EOK) {
		hwlog_err("ps_send_not_replaced_to_mcu memcpy_s para err\n");
		return;
	}
	ret = send_ps_para_to_mcu(tag, replace_type, sizeof(int), SUB_CMD_PS_SEND_NOT_REPLACED);
	if (ret != EOK) {
		hwlog_err("ps_send_not_replaced_to_mcu failed\n");
		return;
	}
}
	...
	struct sensor_detect_manager *sm, int index)
{
	...
	if (of_property_read_u32(dn, "device_fold_type", &device_fold_type))
		hwlog_err("%s: read device_fold_type fail\n", __func__);
