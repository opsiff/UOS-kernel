...
static int nfc_nv_opera(int opr, int idx, const char *name, int len, void *data)
{
	...
	(void)memset_s(&user_info, sizeof(user_info), 0, sizeof(user_info));
	user_info.nv_operation = opr;
	user_info.nv_number = idx;
	user_info.valid_size = len;

	err = strncpy_s(user_info.nv_name, sizeof(user_info.nv_name), name, sizeof(user_info.nv_name));
	if (err != EOK)
				...
				int out_string_len)
{
	...
	if (ret != 0) {
		...
		if (NULL == out_value) {
			...
			err = strncpy_s(out_string, out_string_len, out_value, strlen(out_value));
			if (err != EOK)
				hwlog_err("%s: strncpy_s fail\n", __func__);
			...
			struct huawei_nfc_data *p_nfc_data)
{
	...
	(void)memset_s(nfc_on_str, MAX_GPIO_PLATFROM_SIZE, 0, MAX_GPIO_PLATFROM_SIZE);
			...
			struct huawei_nfc_data *p_nfc_data)
{
	...
	(void)memset_s(nfc_ese_str, MAX_GPIO_PLATFROM_SIZE, 0, MAX_GPIO_PLATFROM_SIZE);
...
static ssize_t nfcc_chip_type_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	if (s_p_nfc_data == NULL) {
		...
		return (ssize_t)(snprintf_s(buf, MAX_ATTRIBUTE_BUFFER_SIZE, MAX_ATTRIBUTE_BUFFER_SIZE - 1, "%s\n", DEFAULT_NFCC_CHIP));
	}
	return (ssize_t)(snprintf_s(buf, MAX_ATTRIBUTE_BUFFER_SIZE, strlen(s_p_nfc_data->nfcc_chip_type) + 1, "%s",
...
static ssize_t nfcc_card_num_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	return (ssize_t)(snprintf_s(buf, MAX_ATTRIBUTE_BUFFER_SIZE, MAX_ATTRIBUTE_BUFFER_SIZE - 1, "%u\n",
		s_p_nfc_data->supported_card_num));
}
...
static ssize_t nfc_close_type_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	if (s_p_nfc_data == NULL) {
		hwlog_err("%s: para s_p_nfc_data is NULL\n", __func__);
		return (ssize_t)(snprintf_s(buf, MAX_ATTRIBUTE_BUFFER_SIZE, MAX_ATTRIBUTE_BUFFER_SIZE - 1, "%d", 0));
	}
	return (ssize_t)(snprintf_s(buf, MAX_ATTRIBUTE_BUFFER_SIZE, MAX_ATTRIBUTE_BUFFER_SIZE - 1, "%u",
		s_p_nfc_data->nfc_close_type));
}
...
static ssize_t nfcc_calibration_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	for (i = 0; i < cal_len + 1; i ++) {
		snprintf_s(&cali_str[i*2], (TEL_HUAWEI_NV_NFCCAL_LEGTH - i) * 2 + 1, 3, "%02X", cali_info[i]);
	}
	hwlog_info("%s: nfc cal info: [%s]!\n", __func__, cali_str);
	return (ssize_t)(snprintf_s(buf, TEL_HUAWEI_NV_NFCCAL_LEGTH * 2, TEL_HUAWEI_NV_NFCCAL_LEGTH * 2, "%s", cali_str));
}
                        ...
                        const char *buf, size_t count)
{
       ...
       if (sscanf(buf, "%1d", &val) == 1) {
               if (val == CLR_BIT || val == SET_BIT) {
                       ...
                       pmic_write_reg(g_nfc_addr_on_hisiXX[s_p_nfc_data->ven_on_gpio_type], val);
               } else {
...
static ssize_t nfcc_svdd_sw_show(struct device *dev, struct device_attribute *attr, char *buf)
{
       ...
       sw_status = pmic_read_reg(g_nfc_addr_on_hisiXX[nfc_on_type]);
...
int init_nfc_cfg_data_from_dts(struct huawei_nfc_data *p_nfc_data)
{
	...
	for_each_node_with_property(np, "nfc_chip_type") {
		...
		hwlog_info("%s : nfc chip: %s is %s \n",__func__, nfc_cy, nfc_st);
		/* copy chip type */
		(void)memset_s(p_nfc_data->nfcc_chip_type, CHIP_NAME_STR_LEN, 0, CHIP_NAME_STR_LEN);
		err = strncpy_s(p_nfc_data->nfcc_chip_type, CHIP_NAME_STR_LEN, nfc_cy, strlen(nfc_cy));
		if (err != EOK)
			hwlog_err("%s: strncpy_s fail\n", __func__);
...
void init_nfc_info_data(struct huawei_nfc_data *p_nfc_data) {
	...
	(void)memset_s(p_nfc_data, sizeof(struct huawei_nfc_data), 0, sizeof(struct huawei_nfc_data));
