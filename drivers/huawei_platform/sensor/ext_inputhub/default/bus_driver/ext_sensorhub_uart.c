...
#define TTY_NAME "ttyHS0"
#define MAX_UART_READ_LEN 10
#define COMMU_UART_FIRST_WRITE 128
#define UART_WORK_SPEED 4000000
#define UART_DIRECT_SPEED 115200
#define UART_READ_TIMEOUT_US 900
#define UART_READ_TIMEOUT_RETRY 20
...
static DEFINE_MUTEX(buf_list_lock);
...
static void uart_tty_receive_first_short(int count)
{
	int ret;
	...
	ret = wait_event_interruptible_timeout(g_next_wait, dma_ready() == 0,
...
static void uart_tty_receive_first_long(int count, unsigned short data_len)
{
	int ret;
	...
	if (!g_receive_data.is_reading || gpio_get_value(g_wakeup_ap_gpio) == 0) {
		pr_err("%s wakeup ap is 0 or timeout", __func__);
		uart_read_err_out();
		return;
	}
...
static void uart_tty_receive_first(const u8 *data, int count)
{
	...
	if (data_len <= COMMU_UART_FIRST_WRITE - FRAME_HEADER_LEN_UART) {
		uart_tty_receive_first_short(count);
		return;
	}
	pr_info("%s uart get in read long first pkg", __func__);

	if (g_receive_data.is_reading)
		uart_tty_receive_first_long(count, data_len);
}

static void uart_tty_receive_long(void)
{
	int ret;
...
int ext_sensorhub_uart_read(u8 *buf, u32 len)
{
	...
	for (i = 0; i < UART_READ_TIMEOUT_RETRY; ++i) {
		ret = wait_event_interruptible_timeout(
			...
			usecs_to_jiffies(UART_READ_TIMEOUT_US));
		if (ret == 0)
			try_flush_to_ldisc();
		else
			break;
...
bool ext_sensorhub_uart_has_probed(void)
{
	return g_uart_probed;
}

int ext_sensorhub_uart_late_init(void)
{
	int ret;
...
late_initcall(ext_sensorhub_uart_late_init);