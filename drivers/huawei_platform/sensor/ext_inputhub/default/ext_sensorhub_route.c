...
static struct wakeup_source *commux_wake_lock = NULL;
...
static struct ext_sensorhub_route ext_route_tbl[] = {
	...
	{   ROUTE_UWB_PORT, NULL,
		NULL, NULL, NULL,
		MAX_READ_LIST_LEN_UWB, 0
	},
	{   ROUTE_CHARGE_DETECT_PORT, NULL,
		NULL, NULL, NULL,
		MAX_READ_LIST_LEN_CHARGE_DETECT, 0
	},
};
...
static void ext_sensorhub_event_init(void)
{
	...
	commux_wake_lock = wakeup_source_register(NULL, "mcu_user_commux");
	/* for event callback */
	...
	struct ext_sensorhub_buf_list *resp_buf, int i)
{
	...
	/* if msg is commux 0x91, hold an extra 700 ms wake lock */
	if (buf_node->service_id == 0x91)
		__pm_wakeup_event(commux_wake_lock, 700);
}
