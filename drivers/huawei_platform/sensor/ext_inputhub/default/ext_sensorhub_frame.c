...
void write_reset_mcu(void)
{
	...
	if (g_handshake[mcu_driver].write_fail_retry_count == MAX_REHANDSHAKE_RETRY_COUNT)
		commu_self_test(TRANSACTION_ID_AP_TO_MCU, false);

	if (g_handshake[UART_DRIVER].write_fail_retry_count == MAX_REHANDSHAKE_RETRY_COUNT)
		commu_self_test(TRANSACTION_ID_AP_TO_BT, false);
