...
#define CMD_ID_REST 0x04
#define GET_BUFF_LEN 1
#define COMMAND_COUNT 1
#define MCU_TO_AP_BUFF 0x01
...
static int get_data_from_buffer(unsigned char service_id,
	unsigned char command_id, unsigned char *data, int data_len)
{
	int ret;
	unsigned char buff[GET_BUFF_LEN] = { 0x02 };
	struct command cmd;

	if ((data_len != GET_BUFF_LEN) || (data[0] != MCU_TO_AP_BUFF)) {
		pr_err("[%s] err: data is 0x%x, data_len is %d\n", __func__, data[0], data_len);
		return -EINVAL;
	}

	if ((service_id != SERVICE_ID) || (command_id != CMD_ID_REST)) {
		pr_err("[%s] service_id or command_id err\n", __func__);
		return -EINVAL;
	}

	cmd.service_id = SERVICE_ID;
	cmd.command_id = CMD_ID_REST;
	cmd.send_buffer = buff;
	cmd.send_buffer_len = sizeof(buff);

	ret = send_command(CHARGE_DETECT_CHANNEL, &cmd, false, NULL);
	if (ret < 0)
...
static int get_data_from_channel(void)
{
	int ret;
	...
	sub_cmd->cmds->command_id = CMD_ID_REST;

	ret = register_data_callback(CHARGE_DETECT_CHANNEL, sub_cmd, get_data_from_buffer);
	if (ret < 0)
...
static int ext_sensorhub_dbg_init(void)
{
	...
	/* monitor data from mcu in CHARGE_DETECT_CHANNEL */
	ret = get_data_from_channel();
	if (ret < 0)
		pr_err("[%s]get_data_from_channel failed\n", __func__);
