 ...
 * remote_fetch.c
 *
 * source file for remote fetch file processor
 *
...
static struct transfer_processor processor = { .name = "remote_fetch" };
...
static int __init init_processor(void)
{
	...
	register_transfer_processor(REMOTE_FETCH, &processor);
	return 0;
}
