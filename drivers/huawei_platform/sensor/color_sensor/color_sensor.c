...
static ssize_t polling_time_type_store(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t size)
{
	struct color_chip *chip = NULL;
	bool state = true;
	...
	if (chip->set_polling_time_type == NULL) {
		hwlog_info("%s set_polling_time_type NULL\n", __func__);
		return size;
	}
	hwlog_info("%s state = %d\n", __func__, (int)state);

	chip->set_polling_time_type(chip, (int)state);
	return size;
}

static ssize_t oeminfo_data_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	struct color_chip *chip = NULL;
	unsigned int len;
	char *oeminfo_data = NULL;
	...
	if (!chip->get_oeminfo_data)
		goto unsupport_oeminfo_data;

	oeminfo_data = chip->get_oeminfo_data();
	if (oeminfo_data == NULL)
		goto unsupport_oeminfo_data;

	len = strlen(oeminfo_data);
	hwlog_info("get oeminfo data = %s, len = %u\n", oeminfo_data, len);
	if (len >= MAX_IS_SUPPORT_OEMINFO_LEN)
		goto unsupport_oeminfo_data;

	return snprintf(buf, MAX_IS_SUPPORT_OEMINFO_LEN, "%s", oeminfo_data);

unsupport_oeminfo_data:
	return snprintf(buf, MAX_IS_SUPPORT_OEMINFO_LEN, "%s", "unsupport");
}
...
DEVICE_ATTR(polling_time_type, 0660, NULL, polling_time_type_store);
DEVICE_ATTR(oeminfo_data, 0660, oeminfo_data_show, NULL);
...
static struct attribute *color_sensor_attributes[] = {
	...
	&dev_attr_polling_time_type.attr,
	&dev_attr_oeminfo_data.attr,
	NULL,
};
