...
static int robot_soc_wakeup_mcu(struct device *dev)
{
	struct robot_mcu_dev_data *data;
	hwlog_info("%s: enter\n", __func__);
	...
	if (!gpio_is_valid(data->gpio_wakeup_mcu)) {
		hwlog_err("invalid gpio_wakeup_mcu %d.\n", data->gpio_wakeup_mcu);
		return -EINVAL;
	}

	if (gpio_request(data->gpio_wakeup_mcu, "gpio_int_soc_mcu")) {
		hwlog_err("gpio wakeup mcu %d request fail.\n", data->gpio_wakeup_mcu);
		return -EINVAL;
	}

	gpio_direction_output(data->gpio_wakeup_mcu, ROBOT_MCU_LEVEL_LOW);
	mdelay(10);
	gpio_direction_output(data->gpio_wakeup_mcu, ROBOT_MCU_LEVEL_HIGH);

	gpio_free(data->gpio_wakeup_mcu);
	hwlog_info("%s: exit\n", __func__);
	return 0;
}
				...
				size_t size)
{
	...
	case ROBOT_MCU_SYNC_UPGRADE_RESET:
		...
		break;
	case ROBOT_MCU_WAKEUP_SET:
		ret = robot_soc_wakeup_mcu(dev);
		break;
	default:
...
static int robot_mcu_parse_dts(struct device *dev, struct robot_mcu_dev_data *data)
{
	...
	ret = of_property_read_u32(dev->of_node, "gpio_int_soc_mcu", &data->gpio_wakeup_mcu);
	if (ret)
		hwlog_err("get int soc mcu gpio fail.\n");
