 ...
 * Description: This file contains the function required for communicating with dev/hwsdp
 * Create: 2020-12-11
 */
...
static long hwsdp_ioctl(struct file *fptr, unsigned int cmd, unsigned long arg)
{
	...
	case HWSDP_CMD_TEEC_OPS:
		...
		break;
	default:
		hwsdp_log_warn("Invalid command: %u", cmd);
		ret = -EINVAL;
	}
	(void)fptr;
	return (long)ret;
...
static s32 __init hwsdp_init(void)
{
	...
	if (IS_ERR(g_cdev_class)) {
		...
		return (s32)PTR_ERR(g_cdev_class);
	}
