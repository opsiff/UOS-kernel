 ...
 * Description: This file contains the function required for communicating with TEE
 * Create: 2020-12-14
 */
...
static void build_teec_operation(u8 *in_data, s32 in_len, s32 out_buff_len, TEEC_Operation *operation)
{
	...
	if ((in_len == sizeof(u8)) || (in_len == sizeof(u32)) || (in_len == sizeof(u64))) {
		...
		hwsdp_log_info("hwsdp: build_teec_operation, in buffer, size %u", operation->params[0].tmpref.size);
	}
	op_type1 = TEEC_NONE;
	op_type2 = TEEC_NONE;
	if (out_buff_len > 0) {
		...
		hwsdp_log_info("hwsdp: build_teec_operation, out buffer, size %u", operation->params[1].tmpref.size);
	}
...
static void hwsdp_open_session(struct work_struct *work)
{
	...
	hwsdp_log_info("TEEK_OpenSession returned %d and err_origin is %u", res, err_origin);
	if (res != TEEC_SUCCESS) {
...
static void hwsdp_invoke_cmd(struct work_struct *work)
{
	...
	if (tee_res != TEEC_SUCCESS) {
		hwsdp_log_err("InvokeCommand failed res=0x%x orig=0x%x cmd=%u",
...
static s32 out_data_to_user_buffer(hwsdp_teec_params *params)
{
	...
	hwsdp_log_info("work_invoke_cmd, out data to user space, length %u!", min_len);
	return HWSDP_IOCTL_SUCCESS;
}
...
static s32 work_invoke_cmd(hwsdp_ioctl_data_t *kdata)
{
	...
	if (kdata->data_len != sizeof(hwsdp_teec_params)) {
		hwsdp_log_err("User data length is bad, %llu/%llu",
	...
	hwsdp_log_info("work_invoke_cmd, tee params cmd %u", params.tee_cmd);
