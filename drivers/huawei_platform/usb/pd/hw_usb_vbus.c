...
#ifdef CONFIG_HW_PD_COMMON
static struct completion pd_get_typec_state_completion;
#else
...
#ifdef CONFIG_HW_PD_COMMON
static struct mutex typec_state_lock;
static struct mutex typec_wait_lock;
#else
...
static void vbus_connect_work(struct work_struct *w)
{
	...
	wait_finish = true;
#ifdef CONFIG_HW_PD_COMMON
	if (pd_state(cur_usb_event) == PD_DPM_TYPEC_ATTACHED_AUDIO) {
		pd_state(usb_audio_state) = PD_DPM_TYPEC_ATTACHED_AUDIO;
#else
...
static void vbus_disconnect_work(struct work_struct *w)
{
	...
	if (pd_state(usb_audio_state) == PD_DPM_TYPEC_ATTACHED_AUDIO) {
		pd_state(usb_audio_state) = PD_DPM_USB_TYPEC_NONE;
#else
