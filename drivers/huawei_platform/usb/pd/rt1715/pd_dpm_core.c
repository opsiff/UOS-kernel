...
int pd_dpm_send_source_caps(struct pd_port *pd_port)
{
	...
	if (tcpc->pd_port.ext_src_cap.otg_vol == SRC_CAP_HIGH_VOL &&
		tcpc->pd_port.ext_src_cap.otg_cur == SRC_CAP_HIGH_CUR) {
		    src_cap1->nr = SRC_PDO_THREE;
	} else if (tcpc->pd_port.ext_src_cap.otg_vol == SRC_CAP_HIGH_VOL &&
		tcpc->pd_port.ext_src_cap.otg_cur == SRC_CAP_LOW_CUR) {
