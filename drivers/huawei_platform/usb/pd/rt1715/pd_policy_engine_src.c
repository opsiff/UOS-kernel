...
#define PS_RDY_WAIT_RETRY_CNT    10
#define WAIT_ONCE_TIME           20
...
static void ps_ready_wait(struct pd_port *pd_port)
{
	int retry = PS_RDY_WAIT_RETRY_CNT;

	pr_err("%s data_role=%d, def_src_nr=%d, selected_cap=%d, ps_rdy_delay=%d\n", __func__,
		pd_port->data_role, pd_port->local_src_cap_default.nr, pd_port->pe_data.selected_cap, pd_port->ps_rdy_delay);

	if ((pd_port->data_role != PD_ROLE_DFP) ||
		(pd_port->local_src_cap_default.nr <= 1) ||
		(pd_port->pe_data.selected_cap <= 1)) {
		pr_err("%s: no need wait\n", __func__);
		return;
	}

	while (pd_port->ps_rdy_delay && (retry > 0)) {
		retry--;
		msleep(WAIT_ONCE_TIME);
		pr_err("%s: wait %d\n", __func__, retry);
	}
}
#endif
