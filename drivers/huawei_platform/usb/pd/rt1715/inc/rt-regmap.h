...
/* Declare a hnt_register by HNT_REG_DECL
 */
#ifdef CONFIG_HUAWEI_TCPC_HUSB311
#define HNT_REG_DECL(_addr, _reg_length, _reg_type, _mask_...) \
	static unsigned char hnt_writable_mask_##_addr[_reg_length] = _mask_; \
	static struct rt_register hnt_register_##_addr = { \
		.addr = _addr, \
		.size = _reg_length, \
		.reg_type = _reg_type, \
		.wbit_mask = hnt_writable_mask_##_addr, \
	}

#define HNT_REG(_addr) (&hnt_register_##_addr)
