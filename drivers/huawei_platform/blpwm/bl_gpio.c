...
static bool bl_gpio_adc_detect_v1(void)
{
	...
	while (loop--) {
		...
		g_bl_gpio_dev->volt = lpm_adc_get_value(ADC_CHANNEL);
...
static bool bl_gpio_adc_detect_v2(void)
{
	...
	while (loop > 0) {
		...
		while (cnt <= ADC_DETECT_NUMBER) {
			tmp = lpm_adc_get_value(ADC_CHANNEL);
			if (tmp > max_volt)
				max_volt = tmp;
...
static struct blpwm_ops g_ops = {
	.cover_detect = bl_gpio_cover_detect,
	.set_duty = bl_gpio_set_duty,
	.set_usage = bl_gpio_set_usage,
	.set_usage_stop = bl_gpio_set_usage_stop,
	.notiby_by_sns = bl_gpio_sensorhub_notify,
};
