	...
	const buffer_t *encoded_wfek, s32 flags)
{
	...
	if (g_enter_mode) {
		pr_info("%s id.task_uid is %u, id.uid is %u\n", __func__, id.task_uid, id.uid);
		id.task_uid = id.uid;
	}

#ifdef CONFIG_HWDPS_OHOS
	id.task_uid = id.uid;
	pr_info("hwdps: uid = %d, task_id = %d", id.uid, id.task_uid);
#endif
	...
	const buffer_t *encoded_wfek, secondary_buffer_t *fek, u32 flags)
{
	...
	if (g_enter_mode) {
		pr_info("%s ids.task_uid is %u, ids.uid is %u\n", __func__, ids.task_uid, ids.uid);
