	...
	s32 exe_path_len, pid_t pid)
{
	...
	if (!task) {
		rcu_read_unlock();
		hwdps_pr_err("Failed to lookup running task! pid %d\n", pid);
		return is_matched;
	}
	...
	bool copy_extinfo)
{
	...
	if (!src_info->app_policy ||
		...
		(dst_info->app_policy_len > src_info->app_policy_len)) {
		hwdps_pr_err("src_info->app_policy_len %llu, dst_info->app_policy_len %llu\n", src_info->app_policy_len,
