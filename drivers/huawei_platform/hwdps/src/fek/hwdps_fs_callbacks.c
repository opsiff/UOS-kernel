...
err:
	...
	*(encoded_wfek->data) = NULL;
	...
	kfree_sensitive(fek_temp.data);
	return res;
}

static hwdps_result_t refresh_fek_inner(const u8 *desc, uid_t uid,
	fek_efek_t *fek_efek)
{
	hwdps_result_t res;
	...
	*(fek_efek->efek_len) = sizeof(struct xattribs_v3_t);
	*(fek_efek->efek) = kzalloc(sizeof(u8) * sizeof(struct xattribs_v3_t),
		GFP_NOFS);
	if (!*(fek_efek->efek)) {
		res = HWDPS_ERR_NO_MEMORY;
		goto cleanup;
	}
	if (memcpy_s(*(fek_efek->efek), *(fek_efek->efek_len), encoded_cipher_buffer.data,
		encoded_cipher_buffer.len) != EOK) {
		hwdps_pr_err("%s: hwdps memcpy failed\n", __func__);
		kfree_sensitive(*(fek_efek->efek));
		*(fek_efek->efek) = NULL;
		res = HWDPS_ERR_NO_MEMORY;
	}
cleanup:
	...
	(void)memset_s(encoded_cipher_buffer.data, encoded_cipher_buffer.len,
		0, encoded_cipher_buffer.len);
...
static hwdps_result_t handle_refresh_fek(const u8 *desc, const u8 *fsname,
	const struct dentry *dentry, fek_efek_t *fek_efek, u32 parent_flags)
{
	hwdps_result_t res;
	uid_t uid;
	...
	res = refresh_fek_inner(desc, uid, fek_efek);
	if (res != HWDPS_SUCCESS)
		hwdps_pr_err("create_fek_inner returned %d\n", res);
	else
		hwdps_pr_debug("create_fek_inner returned success\n");
	...
	const buffer_t *encoded_wfek, secondary_buffer_t *fek)
{
	...
	res = precheck_uid_owner(id);
	if (res != HWDPS_SUCCESS) {
#ifdef CONFIG_HWDPS_OHOS
		if (id->task_uid == 0) {
			res = kernel_get_fek_v2(desc, id->uid, encoded_wfek, fek);
			hwdps_pr_info("hwdps: task-uid = %d, uid = %d, res = %d\n", id->task_uid, id->uid, res);
			check_ret_values(&res, fek);
			return res;
		}
#endif
...
struct hwdps_fs_callbacks_t g_fs_callbacks = {
	...
	.refresh_fek = handle_refresh_fek,
};
