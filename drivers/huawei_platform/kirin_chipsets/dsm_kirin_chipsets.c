...
#ifdef FACTORY_TEST
static void check_long_press_worker(struct work_struct *work)
{
	check_long_press_powe_key();
}

DECLARE_DELAYED_WORK(check_long_press_work, check_long_press_worker);
#endif
...
static int __init dsm_kirin_chipsets_init(void)
{
	...
	schedule_delayed_work(&check_long_press_work, msecs_to_jiffies(WORK_DELAY_10S));
#endif
	return OK;
}
