	...
	struct hwevext_mbhc_priv *mbhc)
{
	...
	flags = snd_soc_component_read(mbhc->component,
...
static int hwevext_codec_v1_probe(struct snd_soc_component *component)
{
	...
	flags = snd_soc_component_read(component, HWEVEXT_CODEC_V1_GPIO_FLAG);
...
static int hwevext_codec_v1_regmap_init(struct i2c_client *i2c_client,
	struct hwevext_codec_v1_priv *hwevext_codec)
{
	int ret;
	unsigned int status;
	...
	if (ret) {
		...
		ret = -ENXIO;
		goto hwevext_codec_v1_regmap_init_err;
	}
	hwlog_info("%s: get chip status: 0x%x\n", __func__, status);
	return 0;

hwevext_codec_v1_regmap_init_err:
	regmap_exit(hwevext_codec->regmap);
	return ret;
}

static int hwevext_codec_v1_register_component(struct device *dev)
{
	int ret = devm_snd_soc_register_component(dev,
	...
	const struct i2c_device_id *id)
{
	...
	ret = hwevext_codec_v1_regmap_init(i2c_client, hwevext_codec);
	if (ret < 0)
	...
	ret = hwevext_codec_v1_register_component(dev);
	if (ret < 0)
