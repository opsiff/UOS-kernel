...
static int hwevext_mbhc_cancel_long_btn_work(
	struct hwevext_mbhc_priv *mbhc_data)
{
...
static void hwevext_mbhc_btn_irq_control(
	struct hwevext_mbhc_priv *mbhc_data,
	int req)
{
...
static void hwevext_handle_usb_analog_hs(struct hwevext_mbhc_priv *mbhc_data,
	int *plug_type)
{
	int val;

	if (ana_hs_support_usb_sw() && ((*plug_type) == AUDIO_JACK_INVERT ||
		(*plug_type) == AUDIO_JACK_HEADPHONE)) {
		hwlog_info("%s: the swtich mic and gnd swap\n", __func__);
		ana_hs_mic_gnd_swap();
		msleep(20);
...
static void hwevext_mbhc_plug_in_detect(struct hwevext_mbhc_priv *mbhc_data)
{
	...
	hwevext_handle_usb_analog_hs(mbhc_data, (int *)&plug_type);
...
static void hwevext_mbhc_for_ext_plug_in_detect(void *para)
{
	struct hwevext_mbhc_priv *mbhc_data = (struct hwevext_mbhc_priv *)para;
	...
	IN_FUNCTION;
	mbhc_data = (struct hwevext_mbhc_priv *)para;
	mbhc_data->repeat_detect_work_stop = true;
	if (cancel_delayed_work_sync(&mbhc_data->repeat_detect_plug_work))
		hwlog_info("%s: repeat recognize work is canceled\n",
			__func__);
...
static void hwevext_mbhc_for_ext_plug_out_detect(void *para)
{
	struct hwevext_mbhc_priv *mbhc_data = (struct hwevext_mbhc_priv *)para;
...
static void hwevext_mbhc_btn_press_handle(
	struct hwevext_mbhc_priv *mbhc_data)
{
	mutex_lock(&mbhc_data->status_mutex);
	...
	hwlog_info("%s: btn press report type: 0x%x, status: %d",
		__func__, mbhc_data->btn_report, mbhc_data->hs_status);
	snd_soc_jack_report(&mbhc_data->button_jack,
...
static void hwevext_mbhc_btn_release_handle(
	struct hwevext_mbhc_priv *mbhc_data)
{
	hwevext_mbhc_cancel_long_btn_work(mbhc_data);
	mutex_lock(&mbhc_data->status_mutex);
...
static void hwevext_mbhc_btn_detect(struct hwevext_mbhc_priv *mbhc_data)
{
	...
	if (hwevext_mbhc_get_btn_press_status(mbhc_data) &&
		...
		if (!ret) {
			...
			hwevext_mbhc_btn_press_handle(mbhc_data);
		} else {
		...
		hwlog_info("%s : btn up report type: 0x%x release\n", __func__,
			mbhc_data->btn_report);
		hwevext_mbhc_btn_release_handle(mbhc_data);
	} else {
...
static void hwevext_mbhc_mutex_init(struct hwevext_mbhc_priv *mbhc_data)
{
	...
	mbhc_data->plug_irq_wake_lock = wakeup_source_register(NULL,
		"hwevext-mbhc-irq-plug");
	mbhc_data->btn_irq_wake_lock = wakeup_source_register(NULL,
...
static int hwevext_mbhc_create_singlethread_workqueue(
	struct workqueue_struct **wq, const char *name)
{
	struct workqueue_struct *req_wq = NULL;

	if (wq == NULL || name == NULL) {
		hwlog_err("%s : wq or name is NULL", __func__);
		return -EINVAL;
	}

	req_wq = create_singlethread_workqueue(name);
	if (!req_wq) {
...
static void hwevext_mbhc_destroy_delay_work(
	struct hwevext_mbhc_priv *mbhc_data)
{
...
static int hwevext_mbhc_create_delay_work(
	struct hwevext_mbhc_priv *mbhc_data)
{
	int ret;

	IN_FUNCTION;
	ret = hwevext_mbhc_create_singlethread_workqueue(
		&(mbhc_data->irq_plug_wq), "hwevext_mbhc_plug_wq");
	if (ret < 0)
		goto create_delay_work_err;
	INIT_DELAYED_WORK(&mbhc_data->plug_work, hwevext_mbhc_plug_work);

	ret = hwevext_mbhc_create_singlethread_workqueue(
		&(mbhc_data->irq_btn_handle_wq), "hwevext_mbhc_btn_delay_wq");
	if (ret < 0)
		goto create_delay_work_err;
	...
	ret = hwevext_mbhc_create_singlethread_workqueue(
		...
		"hwevext_mbhc_read_hs_status_wq");
	if (ret < 0)
		goto create_delay_work_err;
	...
	ret = hwevext_mbhc_create_singlethread_workqueue(
		...
		"hwevext_mbhc_repeat_detect_plug_wq");
	if (ret < 0)
		goto create_delay_work_err;
	...
	ret = hwevext_mbhc_create_singlethread_workqueue(
		...
		"hwevext_mbhc_long_press_btn_wq");
	if (ret < 0)
		goto create_delay_work_err;
...
static void hwcxext_mbhc_high_resistence_enable(void *priv,
	bool enable)
{
...
static int hwevext_get_mbhc_headset_type(void *para)
{
	struct hwevext_mbhc_priv *mbhc_data = (struct hwevext_mbhc_priv *)para;
...
static bool hwevext_mbhc_ext_check_status_headset_in(void *para)
{
	struct hwevext_mbhc_priv *mbhc_data = (struct hwevext_mbhc_priv *)para;
	...
	return mbhc_check_status_headset_in(mbhc_data);
}

static struct ana_hs_codec_dev g_ana_hs_dev = {
	.name = "ana_hs",
	.ops = {
		.check_headset_in = hwevext_mbhc_ext_check_status_headset_in,
		.plug_in_detect = hwevext_mbhc_for_ext_plug_in_detect,
		.plug_out_detect = hwevext_mbhc_for_ext_plug_out_detect,
		.get_headset_type = hwevext_get_mbhc_headset_type,
		.hs_high_resistence_enable = hwcxext_mbhc_high_resistence_enable,
	},
};

static int hwevext_mbhc_resource_init(struct hwevext_mbhc_priv *mbhc_pri,
	bool usb_ana_hs_support)
{
	...
	if (usb_ana_hs_support)
		goto hwevext_skip_rquest_plug_detect_irq;
...
hwevext_skip_rquest_plug_detect_irq:
	...
	struct hwevext_mbhc_cb *mbhc_cb)
{
	...
	usb_ana_hs_support = ana_hs_support_usb_sw();
	...
	ret = hwevext_mbhc_resource_init(mbhc_pri, usb_ana_hs_support);
	if (ret < 0)
		goto hwevext_mbhc_init_err1;
...
static void hwevext_mbhc_mutex_deinit(struct hwevext_mbhc_priv *mbhc_data)
{
	...
	wakeup_source_unregister(mbhc_data->plug_irq_wake_lock);
	wakeup_source_unregister(mbhc_data->btn_irq_wake_lock);
