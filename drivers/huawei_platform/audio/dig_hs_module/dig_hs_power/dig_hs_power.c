...
#define DIG_HS_VBST_LOWPOWER 3700000
...
static void pmu_buckboost_switch(int target_uv_min, int target_uv_max)
{
	int ret;

	/* when dts config, g_pmu_vbst not null */
	if (g_pmu_vbst) {
		ret = regulator_set_voltage(g_pmu_vbst, target_uv_min, target_uv_max);
		if (ret)
			hwlog_err("dig_hs %s: regulator_set_voltage err, ret=%d",
				__func__, ret);
		else
			hwlog_info("dig_hs %s: regulator_set_voltage %d succ",
				__func__, target_uv_min);
	}
}
...
static int buckboost_control(void)
{
	...
	if (g_pdata->gpio_one_ctrl > 0) {
		...
		pmu_buckboost_switch(DIG_HS_VBST_LOWPOWER, INT_MAX);
...
int dig_hs_normal_power(void)
{
	...
	if (g_pdata->audio_buckboost_enable) {
		if (g_pdata->gpio_one_ctrl > 0) {
			pmu_buckboost_switch(g_pdata->vbst_voltage_default, INT_MAX);
			dig_hs_power_gpio_set_value(g_pdata->gpio_low_power_ctrl,
				DIG_HS_POWER_GPIO_RESET);
		} else {
...
device_initcall(dig_hs_power_init);
