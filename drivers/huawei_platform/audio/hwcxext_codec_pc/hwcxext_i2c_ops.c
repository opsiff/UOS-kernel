	...
	struct hwcxext_codec_regmap_cfg **cfg)
{
	...
	hwcxext_kfree_ops((*cfg)->item_type);
	...
	struct hwcxext_codec_regmap_cfg *cfg_info)
{
	...
	const char *item_type_str       = "item_type";
	int ret;
	...
	ret += hwcxext_get_prop_of_u32_array(node, item_type_str,
		&cfg_info->item_type, &cfg_info->num_type);
	...
	unsigned int *value)
{
	...
	*value = le32_to_cpu(recv_buf);
	hwlog_info("%s: read reg:0x%x,value =0x%x \n", __func__, reg, *value);
	return 0;
}
	...
	struct hwcxext_codec_regmap_cfg *cfg)
{
	...
	hwlog_info("%s: regmap_cfg get w%d,%d,r%d,%d,v%d,%d,item%d,default%d\n",
		__func__, cfg->num_writeable, cfg->num_unwriteable,
		cfg->num_readable, cfg->num_unreadable, cfg->num_volatile,
		cfg->num_unvolatile, cfg->num_type, cfg->num_defaults / val_num);
