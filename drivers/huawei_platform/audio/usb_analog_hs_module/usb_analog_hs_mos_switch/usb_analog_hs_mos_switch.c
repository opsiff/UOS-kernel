...
static void usb_analog_hs_plugin_work(struct work_struct *work)
{
	...
	if (g_pdata_mos_switch->ldo_need_control &&
		...
		is_enabled = regulator_is_enabled(g_pdata_mos_switch->mos_switch_phy_ldo);
		hsl_state = pmic_read_reg(HI6555V3_CODEC_ANA_RW5_REG &
...
static void load_gpio_type_config(struct device_node *node)
{
	...
	if (!of_property_read_u32(node, "gpio_type", &temp))
		g_pdata_mos_switch->gpio_type = (int)temp;
	else
...
static int usb_ana_hs_mos_probe(struct platform_device *pdev)
{
	...
	g_pdata_mos_switch->wake_lock = wakeup_source_register(dev,
		"usb_analog_hs");
#else
    g_pdata_mos_switch->wake_lock = wakeup_source_register("usb_analog_hs");
#endif
    if (!g_pdata_mos_switch->wake_lock) {
...
mos_switch_err_regulator_get:
	wakeup_source_unregister(g_pdata_mos_switch->wake_lock);
