...
static int usb_ana_hs_isl54405_probe(struct platform_device *pdev)
{
	...
	isl54405_pdata->wake_lock = wakeup_source_register(dev, "usb_analog_hs");
#else
    isl54405_pdata->wake_lock = wakeup_source_register("usb_analog_hs");
#endif
    if (!isl54405_pdata->wake_lock) {
...
isl54405_err_misc_register:
	usb_ana_hs_isl54405_remove_workqueue();
isl54405_err_lock:
	wakeup_source_unregister(isl54405_pdata->wake_lock);
isl54405_err_out:
