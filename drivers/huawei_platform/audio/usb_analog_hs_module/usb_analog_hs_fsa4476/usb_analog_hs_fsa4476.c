...
static void load_gpio_type_config(struct device_node *node)
{
	...
	if (!of_property_read_u32(node, "gpio_type", &temp))
		fsa4476_pdata->gpio_type = (int)temp;
	else
...
static int usb_ana_hs_fsa4476_probe(struct platform_device *pdev)
{
	...
	fsa4476_pdata->wake_lock = wakeup_source_register(dev, "usb_analog_hs");
#else
    fsa4476_pdata->wake_lock = wakeup_source_register("usb_analog_hs");
#endif
    if (!fsa4476_pdata->wake_lock) {
...
fsa4476_err_lock:
	wakeup_source_unregister(fsa4476_pdata->wake_lock);
