...
static void ovp_dmd_report(void)
{
	...
	power_dsm_report_dmd(POWER_DSM_BATTERY, POWER_DSM_ERROR_NO_TYPEC_4480_OVP,
		(void *)msg_buf);
}
#endif
...
void ana_hs_fsa4480_init(void)
{
    ...
    extern_pdata->wake_lock = wakeup_source_register(NULL, "ana_hs_extern_ops");
#else
    extern_pdata->wake_lock = wakeup_source_register("ana_hs_extern_ops");
#endif
    if (!extern_pdata->wake_lock) {
