		...
		uint32_t len)
{
	...
	if (!data)
		return -ENOMEM; // Handling Memory Allocation Failures
...
static void aw_fine_grained_adapt_amp_gain(struct aw_haptic *aw_haptic)
{
	if (aw_haptic->amplitude > HAPTIC_GAIN_MAX_ADAPT_LEVELS)
		aw_haptic->amplitude = HAPTIC_GAIN_MAX_ADAPT_LEVELS;

	if (aw_haptic->amplitude < 0)
		return;

	aw_haptic->gain = HAPTIC_MAX_GAIN_REG * aw_haptic->amplitude / HAPTIC_GAIN_MAX_ADAPT_LEVELS;
	aw_info("gain = %u", aw_haptic->gain);
}

static void aw_play_effect_id(struct aw_haptic *aw_haptic, uint64_t type)
{
	int rtp_len = 0;
	...
	if (type > FINE_GRAINED_RAM_BASE) { // fine grained, amp is 0-99
		...
		aw_haptic->amplitude = type % FINE_GRAINED_AMP_DIV_COFF;
		aw_haptic->index = type / FINE_GRAINED_AMP_DIV_COFF - FINE_GRAINED_RAM_ID_BASE_VAL;
		aw_info("fine grained index = %d, amp = %d", aw_haptic->index, aw_haptic->amplitude);
		aw_haptic->state = 1;
		aw_fine_grained_adapt_amp_gain(aw_haptic);
		schedule_work(&aw_haptic->vibrator_work);
		...
		aw_info("long index = %d, amp = %d", aw_haptic->index, aw_haptic->amplitude);
		schedule_work(&aw_haptic->vibrator_work);
		...
		aw_info("short index = %d, amp = %d", aw_haptic->index, aw_haptic->amplitude);
		aw_haptic->state = 1;
		schedule_work(&aw_haptic->vibrator_work);
	...
	const char *buff, size_t len, loff_t *off)
{
	...
	aw_info("get haptic id = %llu", type);
	...
	aw_play_effect_id(aw_haptic, type);
	mutex_unlock(&aw_haptic->lock);
	return len;
}
...
static void ram_work_init(struct aw_haptic *aw_haptic)
{
	...
	if (vibrator_get_enter_recovery_mode()) {
		aw_info("recovery mode do not need load firmware");
		return;
	}
	schedule_delayed_work(&aw_haptic->ram_work,
						msecs_to_jiffies(ram_timer_val));
							...
							const char *buf, size_t count)
{
	...
	aw_haptic->gain = HAPTIC_MAX_GAIN_REG;
	mutex_unlock(&aw_haptic->lock);
	schedule_work(&aw_haptic->vibrator_work);
