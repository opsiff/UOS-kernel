...
static void byc_key_thread_func(struct batt_thread_work *twork)
{
	struct batt_chk_data *drv_data = container_of(twork,
		struct batt_chk_data, key_twork);
...
void byc_key_func(struct work_struct *work)
{
	...
	struct batt_thread_work *twork = &drv_data->key_twork;

	if (!batt_kthread_run("key_thread", twork))
		batt_kthread_wait_complete(twork);
}
...
static void byc_sn_thread_func(struct batt_thread_work *twork)
{
	struct batt_chk_data *drv_data = container_of(twork,
		struct batt_chk_data, sn_twork);
...
void byc_sn_func(struct work_struct *work)
{
	...
	struct batt_thread_work *twork = NULL;
	...
	twork = &drv_data->sn_twork;
	if (!batt_kthread_run("sn_thread", twork))
		batt_kthread_wait_complete(twork);
}
...
int byc_init_twork(struct batt_chk_data *drv_data)
{
	...
	if (!drv_data) {
		hwlog_err("drv_data is null in %s\n", __func__);
		return -EINVAL;
	}

	ret += init_thread_work(&drv_data->key_twork, byc_key_thread_func);
	ret += init_kwork_expeller(&drv_data->key_twork, true);
	ret += init_kwork_sche_param(&drv_data->key_twork, true,
		SCHED_FIFO, PRIORITY_MAX);
	ret += init_thread_work(&drv_data->sn_twork, byc_sn_thread_func);
	ret += init_kwork_expeller(&drv_data->sn_twork, true);
	ret += init_kwork_sche_param(&drv_data->sn_twork, true,
		SCHED_FIFO, PRIORITY_MAX);
	if (ret)
		hwlog_err("init twork error in %s\n", __func__);
	return ret;
}
