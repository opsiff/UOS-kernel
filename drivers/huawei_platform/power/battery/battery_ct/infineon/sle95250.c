...
static int sle95250_cyclk_set(void)
{
...
#ifdef BATTBD_FORCE_MATCH
	struct sle95250_nv_info nv_info = { 0 };
#endif /* BATTBD_FORCE_MATCH */
	...
	if (sle95250_is_act_sign_via_nv(di)) {
		...
		ret = power_nv_read(g_nv_ic_map[di->ic_index], &nv_info, sizeof(nv_info));
		if (ret) {
			hwlog_err("[%s] read nv failed\n", __func__);
			return ret;
		}
		nv_info.cyclk_set_flag = true;
		memcpy(nv_info.uid, di->mem.uid, SLE95250_UID_LEN);
		ret = power_nv_write(g_nv_ic_map[di->ic_index], &nv_info, sizeof(nv_info));
		if (ret) {
...
static int sle95250_write_act_sign_nv(struct sle95250_dev *di, uint8_t *act)
{
	...
	nv_info.sign_status = STATUS_BATT_SIGN_WRITING;
	memcpy(nv_info.act_sign, act, SLE95250_ACT_LEN);
...
static int sle95250_read_nv_info(struct sle95250_dev *di,
	struct sle95250_nv_info *nv_info)
{
	int ret;
...
static int sle95250_act_via_nv_read(struct sle95250_dev *di)
{
	...
	ret = sle95250_read_nv_info(di, &nv_info);
	if (ret) {
		hwlog_err("[%s] read sign in nv failed\n", __func__);
		return -EINVAL;
	}
	if (nv_info.sign_status > STATUS_BATT_SIGN_INIT) {
		memcpy(di->mem.act_sign, nv_info.act_sign, SLE95250_ACT_LEN);
...
#ifdef BATTBD_FORCE_MATCH
static void sle95250_lock_info_parse(struct sle95250_dev *di)
{
	uint8_t lock0[sizeof(struct batt_lock_para)] = { 0 };
	struct batt_lock_para *lock = batt_get_lock_para();

	if (!di)
		return;

	if (!lock) {
		hwlog_err("%s: lock info NULL\n", __func__);
		return;
	}

	if (!memcmp(lock0, (uint8_t *)lock, sizeof(struct batt_lock_para))) {
		hwlog_err("%s: lock info all zero\n", __func__);
		return;
	}
	...
	struct device_attribute *attr, char *buf)
{
	...
	struct sle95250_nv_info nv_act = { 0 };
	...
	case SLE95250_SYSFS_NV_ACT:
		hwlog_info("[%s] SLE95250_SYSFS_NV_ACT, ic_index:%u\n",
			__func__, di->ic_index);
		ret = sle95250_read_nv_info(di, &nv_act);
		if (ret) {
