...
static int batt_srv_state_cb(bool state)
{
	if (state)
...
static struct power_genl_node batt_info_genl_node = {
	...
	.srv_state_cb = batt_srv_state_cb,
};
...
static void exit_check_process(struct batt_info *drv_data)
{
...
static int check_backup_id_func(struct batt_chk_rslt *result)
{
	struct batt_checker_entry *temp = NULL;
	struct batt_chk_data *checker_data = NULL;
	int ret = 0;
	char buf[BACKUP_ID_BUF_LEN] = { 0 };
	char *flag_ptr = buf;
	char *val = NULL;

	if (get_battery_backup_id(buf, BACKUP_ID_BUF_LEN)) {
		list_for_each_entry(temp, &batt_checkers_head, node) {
			checker_data = platform_get_drvdata(temp->pdev);
			checker_data->result.sn_status = SN_SNS_UNMATCH;
			*result = checker_data->result;
		}
		return BACKUP_ID_CHECK_ERROR;
	}

	list_for_each_entry(temp, &batt_checkers_head, node) {
		checker_data = platform_get_drvdata(temp->pdev);
		val = strsep(&flag_ptr, "\n");
		if (!checker_data->backup_check_item.is_need_check || *val == '\0') {
			hwlog_err("%s no need check.\n", __func__);
			continue;
		}

		if ((int)(*val) != checker_data->backup_check_item.content) {
			hwlog_err("%s sn backup id content is %c not equal to required para %c.\n",
				__func__, *val, checker_data->backup_check_item.content);

			checker_data->result.sn_status = SN_SNS_UNMATCH;
			*result = checker_data->result;
			ret = BACKUP_ID_CHECK_ERROR;
		}
	}
...
static void check_thread_func(struct batt_thread_work *kwork)
{
	...
	if (check_backup_id_func(final_result))
		goto process_result_status;
...
static ssize_t battct_rt_check_store(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
	struct batt_info *drv_data = NULL;
	struct batt_checker_entry *temp = NULL;
	...
	list_for_each_entry(temp, &batt_checkers_head, node) {
		checker_data = platform_get_drvdata(temp->pdev);
		ret = checker_data->bco.rt_check(checker_data);
		if (ret)
			return ret;
	}
...
static DEVICE_ATTR_RO(nv_sn_good);
static DEVICE_ATTR_RO(batt_id_v);
static DEVICE_ATTR_RO(batt_num);
static DEVICE_ATTR_RO(check_mode);
static DEVICE_ATTR_RO(official);
static DEVICE_ATTR_RO(board);
static DEVICE_ATTR_RO(battery);
static DEVICE_ATTR_RO(board_runnable);
static DEVICE_ATTR_RO(battery_ct_shield);
static DEVICE_ATTR_WO(check_request);
static DEVICE_ATTR_RO(check_execute_state);
static DEVICE_ATTR_RO(can_check_in_running);
static DEVICE_ATTR_WO(powerct_error_code);
static DEVICE_ATTR_RW(battery_permit);
static DEVICE_ATTR_RW(final);
static DEVICE_ATTR_RW(must_real_check);
static DEVICE_ATTR_RO(current_group_sn);
#ifdef BATTERY_LIMIT_DEBUG
static DEVICE_ATTR_RO(ftime);
static DEVICE_ATTR_RO(ctime);
static DEVICE_ATTR_RW(bind_info);
static DEVICE_ATTR_RW(bind_uuid);
static DEVICE_ATTR_WO(check);
static DEVICE_ATTR_RW(sn_checker);
static DEVICE_ATTR_RW(check_result);
static DEVICE_ATTR_RO(batt_param);
static DEVICE_ATTR_WO(battct_rt_check);
#endif /* BATTERY_LIMIT_DEBUG */
