...
static struct batt_lock_para  g_lock_para = { 0 };
static char g_nv_sn[MAX_SN_LEN] = { 0 };
static char g_batt_name[BATT_MAX_NAME_LEN];
...
static int __init batt_parse_ic_name_cmdline(char *p)
{
	if (!p)
		return -1;
	strncpy(g_batt_name, p, strnlen(p, BATT_MAX_NAME_LEN - 1));
	return 0;
}
early_param("batt_name", batt_parse_ic_name_cmdline);
...
static int __init batt_parse_lock_cmdline(char *p)
{
	int value;
	char *token = NULL;
	...
	memset(&g_lock_para, 0, sizeof(struct batt_lock_para));
	token = strsep(&p, ",");
	if (!token)
		return -1;
	...
	if (kstrtoint(token, POWER_BASE_HEX, &value) != 0)
		return -1;
	g_lock_para.cyclk = value;

	if (kstrtoint(p, POWER_BASE_HEX, &value) != 0)
		return -1;
	g_lock_para.pglk = value;
	return 0;
}
early_param("battery_lock", batt_parse_lock_cmdline);
...
{
	if (strnlen(g_batt_name, BATT_MAX_NAME_LEN - 1) == 0)
		return NULL;
	return g_batt_name;
}
