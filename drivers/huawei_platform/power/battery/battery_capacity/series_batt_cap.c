...
static inline int sercap_vol2ocv(int vol, int cur)
{
	...
	return vol + cur * SERCAP_DEF_RBAT / POWER_UV_PER_MV;
}
...
static int sercap_adjust_cap(struct series_capacity *di, int state, int cap)
{
	...
	case POWER_SUPPLY_STATUS_FULL:
		if (charge_get_battery_current_avg() >= 0) {
...
{
	int cap = power_platform_get_battery_ui_capacity() * SERCAP_TENTH;
...
static int sercap_try_adjust_by_vol(struct series_capacity *di, int in_soc)
{
	...
	msleep(SERCAP_DEBOUNCE);
	v1 = sercap_vbat1_voltage();
	cur = -power_platform_get_battery_current();
	/* current is jumping */
...
static int sercap_adjust_bat1_soc(struct series_capacity *di, int bat1_soc)
{
	...
	int vbat1 = di->bat1_vol;
	int low_vol;
	int ocv = coul_drv_get_ocv() / POWER_UV_PER_MV;
...
void sercap_get_base_info(struct series_capacity *di)
{
	...
	di->avg_i = charge_get_battery_current_avg();
	di->cur = -power_platform_get_battery_current();
...
static void sercap_info_init(struct series_capacity *di)
{
	...
	if ((bat0_vol <= 0) || (bat1_vol <= 0) || !power_platform_is_battery_exit()) {
	...
	di->bat0_ocv = coul_drv_get_ocv() / POWER_UV_PER_MV;
	di->interval = SERCAP_INTERVAL_NOR;
...
{
	di->track.time = power_get_monotonic_boottime();
	di->track.type = type;
...
static void sercap_track_running(struct series_capacity *di)
{
	...
	if ((power_get_monotonic_boottime() - di->track.time) > SERCAP_TRACK_TIMEOUT) {
...
static int sercap_probe(struct platform_device *pdev)
{
	...
	di->wakelock = power_wakeup_source_register(di->dev, "series_batt_cap");
	sercap_data_init(di);
	sercap_info_init(di);
