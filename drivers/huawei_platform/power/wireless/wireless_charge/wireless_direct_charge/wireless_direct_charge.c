...
int wldc_get_bat_voltage(struct wldc_dev_info *di, int dc_mode)
{
	int btb_vol, dc_ratio;
	...
	dc_ratio = di->mode_para[dc_mode].init_para.dc_ratio;
	if (di->use_higher_vbat)
		btb_vol = dcm_get_ic_max_vbtb_with_comp(dc_ratio, di->comp_para.vbat_comp);
	else
		btb_vol = dcm_get_ic_vbtb_with_comp(dc_ratio, di->ic_data.cur_type,
...
bool wldc_need_cp_div2(void)
{
	...
	if (!di || !di->mode_para || !wldc_is_mode_para_valid(di, di->cur_dc_mode))
		return false;
...
bool wldc_support_scx_mode(void)
{
	struct wldc_dev_info *di = g_wldc_di;
...
static bool __wldc_is_volt_param_match(struct wldc_dev_info *di, int id,
...
static bool wldc_is_volt_param_match(struct wldc_dev_info *di, int id)
{
	...
	return __wldc_is_volt_param_match(di, id, batt_brand);
}

static bool wldc_is_dflt_volt_param_match(struct wldc_dev_info *di, int id)
{
	return __wldc_is_volt_param_match(di, id, POWER_SUPPLY_DEFAULT_BRAND);
}
	...
	struct wldc_regulation_data *data)
{
	...
	data->ls_vbatt = wldc_get_bat_voltage(di, di->cur_dc_mode);
...
static void wldc_check_recharge_path(struct wldc_dev_info *di)
{
	bool rechg_en = true;
	int rechg_ibat = 0;
	int pre_ibat = 0;
	...
	if (dc_rechg_is_enforce_en() && dc_rechg_is_ibus_support(ls_ibus)) {
		dc_rechg_enforce_en();
		return;
	}

	/* ibus >= open_ith */
	rechg_en = rechg_en && dc_rechg_is_ibus_support(ls_ibus);
	/* battery connect_mode is serial mode */
	rechg_en = rechg_en && (di->mode_para[di->cur_dc_mode].init_para.batt_conn_mode == BAT_SERIAL_MODE);
	/* match vol_diff level */
	rechg_en = rechg_en && bat_chg_balance_check_recharge(&rechg_ibat, &pre_ibat);
	dc_rechg_set_enable(rechg_en);
	if (rechg_en) {
		wlrx_ic_get_imax(WLTRX_IC_MAIN, &rx_imax);
		cp_ratio = di->mode_para[di->cur_dc_mode].init_para.cp_ratio;
		dc_rechg_regulation(rx_imax * cp_ratio * di->dc_ratio - pre_ibat, rechg_ibat);
		hwlog_info("rechg_ibat=%d, pre_ibat=%d", rechg_ibat, pre_ibat);
	}
}
...
static void wldc_select_charge_stage(struct wldc_dev_info *di)
{
	...
	vbatt = wldc_get_bat_voltage(di, di->cur_dc_mode);
...
static bool wldc_abnormal_iout_check(struct wldc_dev_info *di)
{
	...
	cp_iout_th = WLDC_IOUT_LTH * SC_DEFAULT_VOLT_RATIO / di->dc_ratio;
	if ((cp_iout < cp_iout_th) && (cp_iout_avg < cp_iout_th)) {
		cnt++;
...
static void wldc_check_imax_by_batt_balance(struct wldc_dev_info *di)
{
	...
	info[0].vol = dcm_get_ic_vbtb_with_comp(di->dc_ratio, CHARGE_IC_MAIN,
		di->comp_para.vbat_comp);
	...
	info[1].vol = dcm_get_ic_vbtb_with_comp(di->dc_ratio, CHARGE_IC_AUX,
		di->comp_para.vbat_comp);
	if (info[1].vol < 0)
		info[1].vol = info[0].vol;
	bat_temp_get_temperature(BTB_TEMP_1, &info[1].temp);

	if (bat_chg_balance_get_cur_info(info, 2, &result,
		di->mode_para[di->cur_dc_mode].init_para.batt_conn_mode)) /* 2: two battery */
		return;
...
static int wldc_get_cp_iin_thermal(struct wldc_dev_info *di)
{
	...
	dc_mode = wldc_get_mode_by_name(di->mode_para[di->cur_dc_mode].init_para.dc_name);
	if (dc_mode == WLDC_MODE_LVC)
		idx = WLDC_LVC_SINGLE_CHANNEL;
	else if (dc_mode == WLDC_MODE_SCX)
		idx = (di->ic_data.cur_type == CHARGE_MULTI_IC) ?
			WLDC_SC4_DUAL_CHANNEL : WLDC_SC4_SINGLE_CHANNEL;
	else
		idx = (di->ic_data.cur_type == CHARGE_MULTI_IC) ?
			WLDC_SC_DUAL_CHANNEL : WLDC_SC_SINGLE_CHANNEL;
...
static bool wldc_check_volt_para_by_stage(struct wldc_dev_info *di, int stage_id)
{
...
static void wldc_check_imax_by_pwr_limit(struct wldc_dev_info *di, int *i_limit_by_pwr)
{
...
static void wldc_check_imax_by_tx_evt_ilim(struct wldc_dev_info *di, int *tx_evt_ilim)
{
...
static void wldc_select_charge_param(struct wldc_dev_info *di)
{
	...
	if (!wldc_check_volt_para_by_stage(di, stage_id))
		return;
	...
	wldc_check_imax_by_tx_evt_ilim(di, &tx_evt_ilim);
	wldc_check_imax_by_pwr_limit(di, &i_limit_by_pwr);
...
static int wldc_stop_charge_status_check(struct wldc_dev_info *di)
{
	...
	if ((state == WIRED_CHANNEL_ON) ||
		...
		for (i = 0; i < di->total_dc_mode; i++) {
			if (!(di->mode_para[i].dis_flag & WLDC_DIS_BY_CHRG_DONE)) {
				wldc_set_charge_stage(WLDC_STAGE_DEFAULT);
				break;
			}
		}
		if (i >= di->total_dc_mode)
...
static void wldc_stop_limit_output_current(struct wldc_dev_info *di)
{
	...
	int rx_iout = 0;
	int rx_vout = 0;
	int cp_ratio, rx_vstep;

	if (!wldc_is_mode_para_valid(di, di->cur_dc_mode))
		return;

	cp_ratio = di->mode_para[di->cur_dc_mode].init_para.cp_ratio;
	rx_vstep = 75 * di->dc_ratio * cp_ratio; /* 75mv: limit current step volt */
...
void wldc_stop_charging(struct wldc_dev_info *di)
{
	...
	di->cur_dc_mode = di->total_dc_mode;
	hwlog_info("%s: --\n", __func__);
}
...
static void wldc_revise_mode_para(struct wldc_dev_info *di)
{
	...
	for (i = 0; i < di->total_dc_mode; i++) {
		...
		if (p_init->batt_conn_mode <= 0)
			p_init->batt_conn_mode = BAT_PARALLEL_MODE;
	}
}
