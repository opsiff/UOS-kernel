...
static void add_echo_info(void);
...
void handle_ipv4_info(struct sk_buff *skb, hw_wifi_filter_item *p_item)
{
    const struct iphdr *ip = NULL;
    ip = ip_hdr(skb);
    if (ip == NULL) {
		FILTER_LOGE("ipv4 is null");
        return;
    }

    if (ip->protocol == IPPROTO_TCP) {
		FILTER_LOGI("tcp_ipv4 dest=%d, source=%d", ntohs(tcp_hdr(skb)->dest), ntohs(tcp_hdr(skb)->source));
        p_item->port = tcp_hdr(skb)->dest;
        p_item->protocol = IPPROTO_TCP;
        p_item->filter_cnt = ntohs(ETH_P_IP);
		return;
    }

    if (ip->protocol == IPPROTO_UDP) {
		FILTER_LOGI("udp_ipv4 dest=%d, source=%d", ntohs(udp_hdr(skb)->dest), ntohs(udp_hdr(skb)->source));
        p_item->port = udp_hdr(skb)->dest;
        p_item->protocol = IPPROTO_UDP;
        p_item->filter_cnt = ntohs(ETH_P_IP);
		return;
    }
}

void handle_ipv6_info(struct sk_buff *skb, hw_wifi_filter_item *p_item)
{
	const struct ipv6hdr *ip = NULL;
	ip = ipv6_hdr(skb);
	if (ip == NULL) {
		FILTER_LOGE("ipv6 is null");
		return;
	}

	if (ip->nexthdr == NEXTHDR_TCP) {
		FILTER_LOGI("tcp_ipv6 dest=%d, source=%d", ntohs(tcp_hdr(skb)->dest), ntohs(tcp_hdr(skb)->source));
		p_item->port = tcp_hdr(skb)->dest;
		p_item->protocol = IPPROTO_TCP;
		p_item->filter_cnt = ntohs(ETH_P_IPV6);
		return;
	}

	if (ip->nexthdr == NEXTHDR_UDP) {
		FILTER_LOGI("udp_ipv6 dest=%d, source=%d", ntohs(udp_hdr(skb)->dest), ntohs(udp_hdr(skb)->source));
		p_item->port = udp_hdr(skb)->dest;
		p_item->protocol = IPPROTO_UDP;
		p_item->filter_cnt = ntohs(ETH_P_IPV6);
		return;
...
void get_wifi_filter_info(struct sk_buff *skb, hw_wifi_filter_item *p_item)
{
	if (skb == NULL) {
	...
	if (p_item == NULL) {
		FILTER_LOGE("p_item is null");
		return;
	}

	unsigned short protocol_ip = skb->protocol;
	FILTER_LOGI("protocol_ip=%d", skb->protocol);
	if (protocol_ip == ntohs(ETH_P_IP)) {
		handle_ipv4_info(skb, p_item);
	} else if (protocol_ip == ntohs(ETH_P_IPV6)) {
		handle_ipv6_info(skb, p_item);
	}
}
...
static void add_echo_info(void)
{
	...
	item.port = ntohs(ECHO_PORT);
	item.protocol = IPPROTO_TCP;
	add_to_screenoff_table(&item);
}
...
static void handle_wake_scrn_off(void)
{
...
static void handle_wake_exit_doze(void)
{
	bDozeEnable = false;
	hw_clear_doze_item();
...
static void handle_wake_enter_doze(void)
{
	bDozeEnable = true;
	if (gWlanFilterOps.set_filter_enable_ex != NULL) {
...
static void handle_wake_exit_night(void)
{
	bIsNight = false;
	if (gWlanFilterOps.set_filter_enable_ex != NULL) {
...
static int wifi_filter_threadfn(void *data)
{
    while(1)
    {
        ...
        case WAKE_SCRN_ON:
            ...
            break;
        case WAKE_SCRN_OFF:
			handle_wake_scrn_off();
            break;
        case WAKE_EXIT_DOZE:
			handle_wake_exit_doze();
            break;
        case WAKE_ENTER_DOZE:
			handle_wake_enter_doze();
            break;
