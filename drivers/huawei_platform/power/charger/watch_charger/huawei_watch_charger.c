...
#include "huawei_watch_charger.h"
...
#define WAKELOCK_TIMEOUT_MS 600
...
static struct wakeup_source *wake_lock;
...
static int watch_get_charge_status(void)
{
	...
	if (jiffies_to_msecs(jiffies - g_last_jiffies_charge_status) >
		...
		if (ret < 0)
			hwlog_err("query charging status fail use last value ret:%d\n", ret);
		else
			g_last_jiffies_charge_status = jiffies;

		hwlog_info("query charging status is %d\n", g_charge_info->charge_info.charge_st);
	}
...
static int watch_get_battery_voltage(void)
{
	...
	if (jiffies_to_msecs(jiffies - g_last_jiffies_voltage) >
		...
		if (ret < 0)
			hwlog_err("query voltage fail use last value ret:%d\n", ret);
		else
			g_last_jiffies_voltage = jiffies;

		hwlog_info("query battery voltage is %d uV\n",
		g_charge_info->charge_info.bat_vol * POWER_UV_PER_MV);
	}

	return g_charge_info->charge_info.bat_vol * POWER_UV_PER_MV;
}
...
static int watch_get_charge_current(void)
{
	...
	if (jiffies_to_msecs(jiffies - g_last_jiffies_current) >
		...
		if (ret < 0)
			hwlog_err("query current fail use last value ret:%d\n", ret);
		else
...
{
	hwlog_info("query battery capacity is %d\n",
...
static int watch_get_battery_temperature(void)
{
	...
	if (jiffies_to_msecs(jiffies - g_last_jiffies_temp) >
		...
		if (ret < 0)
			hwlog_err("query temp fail use last value ret:%d\n", ret);
		else
			g_last_jiffies_temp = jiffies;

		hwlog_info("query battery temperature is %d\n",
		g_charge_info->charge_info.bat_temp);
	}

	return g_charge_info->charge_info.bat_temp;
}
...
{
	return g_charge_info->charge_info.bat_charge_full * 1000; /* 1000: mAh to uAh */
}
...
{
	return g_charge_info->charge_info.bat_charge_full_design * 1000; /* 1000: mAh to uAh */
}
...
{
	hwlog_info("query charger online status is %d\n",
...
static int watch_set_enable_charge(unsigned int value)
{
	...
	hwlog_info("user set enable status:%d\n", value);
	g_charge_info->sysfs_data.charge_enable = value;
	if (value) {
		ret = watch_send_cmd_to_mcu_charge(SET_CHG_ENABLE, DEFAULT_CMD);
	} else {
		ret = watch_send_cmd_to_mcu_charge(SET_CHG_DISABLE, DEFAULT_CMD);
	}

	if (ret < 0)
		hwlog_err("set mcu charge enable fail ret:%d\n", ret);
	return 0;
}
...
{
	hwlog_info("mcu real enable charge result:%d\n", enable);
...
{
	g_charge_info->sysfs_data.iin_limit = value;
	hwlog_info("mcu real input current:%d mA\n", value);
}
...
{
	g_charge_info->sysfs_data.ichg_limit = value;
	hwlog_info("mcu real charge current:%d mA\n", value);
}
...
{
	g_charge_info->sysfs_data.vterm_limit = value;
	hwlog_info("mcu set real cut-off voltage:%d mV\n", value);
}
...
{
	hwlog_info("mcu real hiz enable result:%d\n", value);
...
static unsigned int watch_get_charge_done_status(void)
{
	...
	hwlog_info("mcu report capacity:%d, charge_done_status:%d\n",
		g_charge_info->charge_info.bat_cap,
...
{
	hwlog_info("wireless last aligned status:%d,mcu report status:%d\n",
...
static void watch_set_quick_charge_status(int status)
{
	int icon_type;

	hwlog_info("quick charge last status:%d,mcu report status:%d\n",
		g_charge_info->charge_info.quick_charge_status, status);

	if (g_charge_info->charge_info.quick_charge_status == status)
		return;

	g_charge_info->charge_info.quick_charge_status = status;

	if (status == WIRELESS_CHARGE_QUICK) {
		icon_type = ICON_TYPE_WIRELESS_SUPER;
		power_ui_event_notify(POWER_UI_NE_ICON_TYPE, &icon_type);
	} else if (status == WIRELESS_CHARGE_NORMAL) {
...
static void watch_get_charge_info(struct huawei_charge_info *info)
{
	if (info->mcu_report_flag == STATUS_FALSE) {
		...
		info->charge_info.wireless_aligned_status = WIRELESS_CHARGE_ALIGNED;
		info->low_power_off_flag = STATUS_FALSE;
		info->charge_info.quick_charge_status = WIRELESS_CHARGE_NORMAL;
		return;
	}
	...
	info->low_power_off_flag = STATUS_FALSE;
	info->charge_info.quick_charge_status =
	    g_charge_info->charge_info.quick_charge_status;
}
...
{
	hwlog_info("mcu report charge status:%d\n",
		g_charge_info->charge_info.charge_st);
	hwlog_info("mcu report battery_voltage:%d mV\n",
		g_charge_info->charge_info.bat_vol);
	hwlog_info("mcu report battery_current:%d mA\n",
		g_charge_info->charge_info.bat_cur);
	hwlog_info("mcu report battery_capacity:%d%%\n",
		g_charge_info->charge_info.bat_cap);
	hwlog_info("mcu report battery_temperature:%d\n",
		g_charge_info->charge_info.bat_temp);
	hwlog_info("mcu report battery_id_voltage:%d mV\n",
		g_charge_info->charge_info.bat_id);
	hwlog_info("mcu report charger_online_status:%d\n",
		g_charge_info->charge_info.online);
	hwlog_info("mcu report bat_cap_rm:%d mAh\n",
		g_charge_info->charge_info.bat_cap_rm);
	hwlog_info("mcu report bat_cap_fcc:%d mAh\n",
		g_charge_info->charge_info.bat_cap_fcc);
	hwlog_info("mcu report charge_cycle_cnt:%d\n",
		g_charge_info->charge_info.charge_cycle_cnt);
	hwlog_info("mcu report bat_charge_full:%d mAh\n",
		g_charge_info->charge_info.bat_charge_full);
	hwlog_info("mcu report bat_charge_full_design:%d mAh\n",
		g_charge_info->charge_info.bat_charge_full_design);
	hwlog_info("mcu report bat_voltage_max:%d mV\n",
		g_charge_info->charge_info.bat_voltage_max);
	hwlog_info("mcu report bat_capacity_level:%d\n",
		g_charge_info->charge_info.bat_capacity_level);
	hwlog_info("mcu report wireless_aligned_status:%d\n",
		g_charge_info->charge_info.wireless_aligned_status);
	hwlog_info("mcu report bat_charge_now:%d mAh\n",
		g_charge_info->charge_info.bat_charge_now);
	hwlog_info("mcu report quick_charge_status:%d\n",
	    g_charge_info->charge_info.quick_charge_status);
}
	...
	struct mcu_report_work_struct *report_work)
{
	...
	watch_set_quick_charge_status(count[WIRELESS_CHARGE]);
...
static void watch_mcu_charge_monitor_work(struct work_struct *work)
{
	...
	if ((g_charge_info->charge_info.online != DEFAULT_CHARGER_ONLINE_ST) ||
		...
		if (g_charge_info->charge_info.online == CHARGER_ONLINE) {
	...
	unsigned char command_id, unsigned char *data, int data_len)
{
	...
	case CHG_READ_COMMAND_ID:
		...
		__pm_wakeup_event(wake_lock, WAKELOCK_TIMEOUT_MS);
		break;
	case CHG_WRITE_COMMAND_ID:
		/* free after callback work has done */
...
static void watch_get_charge_info_timer_callback(struct timer_list *unused)
{
...
static void watch_get_mcu_charge_info_timer_init(void)
{
	...
	timer_setup(&g_get_charge_info_timer, watch_get_charge_info_timer_callback, 0);
...
static int watch_charge_probe(struct platform_device *pdev)
{
	...
	wake_lock = wakeup_source_register(NULL, "charger_wake_lock");
...
static int watch_charge_remove(struct platform_device *pdev)
{
	...
	wakeup_source_unregister(wake_lock);
...
MODULE_DEVICE_TABLE(of, watch_charge_match_table);
