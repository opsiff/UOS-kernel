...
extern ssize_t boost_store(struct device *dev, struct device_attribute *attr,
	   const char *buf, size_t count);
extern ssize_t boost_timeout_show(struct device *dev, struct device_attribute *devattr,
			char *buf);
extern ssize_t boost_timeout_store(struct device *dev, struct device_attribute *devattr,
			const char *buf, size_t count);
...
ssize_t cur_enable_store(struct device *dev, struct device_attribute *devattr,
	const char *buf, size_t count);
DEVICE_ATTR(boost, 0644, boost_show, boost_store);
DEVICE_ATTR(boost_timeout, (S_IWUSR | S_IRUGO), boost_timeout_show,
				   boost_timeout_store);
DEVICE_ATTR(cur_power, S_IRUGO, cur_power_show, NULL);
DEVICE_ATTR(cur_enable, S_IWUSR | S_IRUGO, cur_enable_show, cur_enable_store);
