		    ...
		    int cpu_idx)
{
	...
	if (cpumask_weight(cpu_smt_mask(cpu)) == 2) {
		u64 total_busy[3], delta_busy[2];

		lpcpu_time_in_smt_get(cpu, total_busy, 3);
		delta_busy[0] = total_busy[2] - cpufreq_cdev->busy_time[cpu_idx].mt_busy_time;
		...
		delta_idle = delta_time - delta_busy[0] - (delta_busy[1] * 50 / 65);
		cpufreq_cdev->busy_time[cpu_idx].mt_busy_time = total_busy[2];
