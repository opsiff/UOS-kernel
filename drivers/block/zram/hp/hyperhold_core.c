...
static unsigned long memcg_reclaim_size(struct mem_cgroup *memcg)
{
	...
	unsigned long new_size = (zram_size + cur_size) *
		(u64)atomic_read(&memcg->memcg_reclaimed.ub_zram2ufs_ratio) / 100;
...
static void hyperhold_memcg_active_swapout_by_ratio(struct mem_cgroup *mcg)
{
	...
	size = (u64)atomic64_read(&mcg->zram_stored_size);
	ratio = (u32)atomic_read(&mcg->memcg_reclaimed.ub_zram2ufs_ratio);
	size = size * ratio / 100;
	...
	u64 target_sz, u64 *actual_reclaimed_sz, unsigned long *cost_time)
{
	...
	iter_start = jiffies;
	mcg_score = (u64)atomic64_read(&mcg->memcg_reclaimed.app_score);
	reclaim_sz = (u64)atomic64_read(&mcg->zram_stored_size);
...
static bool hyperhold_task_active_swapout_protect(struct mem_cgroup *mcg)
{
	...
	if (unlikely(!mcg) || unlikely(!mcg->zram))
		return true;
	cur_mcg_score = (u64)atomic64_read(&mcg->memcg_reclaimed.app_score);
