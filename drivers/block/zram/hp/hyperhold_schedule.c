...
static void hyperhold_io_entry_proc(struct hyperhold_segment *segment)
{
	...
	if (segment->req->io_para.scenario == HYPERHOLD_RECLAIM_IN)
		mutex_lock(hyperhold_segment_mutex());
	...
	if (segment->req->io_para.scenario == HYPERHOLD_RECLAIM_IN)
		mutex_unlock(hyperhold_segment_mutex());
}
...
static int hyperhold_bio_set_bdev(struct bio *bio, struct block_device *bdev)
{
	...
	if (space_info->userdata_bdev && op_is_read(bio_op(bio))) {
		bio_set_dev(bio, space_info->userdata_bdev);
		return 0;
	}

	hh_print(HHLOG_ERR, "bdev is NULL, op:%u\n", op_is_read(bio_op(bio)));
	return -EIO;
}
...
static int hyperhold_submit_bio(struct hyperhold_segment *segment)
{
	...
	if (unlikely(hyperhold_bio_set_bdev(bio, segment->req->io_para.bdev))) {
		bio_put(bio);
		hh_print(HHLOG_ERR, "hyperhold set bio bdev fail\n");
		return -EIO;
	}
	...
	hh_print(HHLOG_DEBUG, "submit_bio_sector 0x%llx ext_id %d\n",
	...
	bool wait_io_finish_flag)
{
	...
	req->wait_io_finish_flag = wait_io_finish_flag;
