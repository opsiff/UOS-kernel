...
int hyperhold_cache_decomp(struct zram *zram, u32 index, struct page *page)
{
	...
	addr = zram_get_handle(zram, index);
	size = (int)zram_get_obj_size(zram, index);
...
static bool hyperhold_cache_objs_del(struct zram *zram, u32 index,
...
static bool move_cache_to_zram(struct zram *zram, int ext_id)
{
...
static bool extent_can_move(struct hyperhold_area *area, int ext_id)
{
...
static int hyperhold_memcg_move_cache(struct zram *zram, struct mem_cgroup *mcg,
				int nr_exts)
{
	int moved = 0;
				      ...
				      void **private)
{
	...
	return (unsigned long)reclaim_size;
}
...
static bool extent_not_cached(struct hyperhold_area *area, int ext_id)
{
