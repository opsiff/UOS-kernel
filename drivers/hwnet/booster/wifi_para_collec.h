...
#define BEACON_RSSI_LEN 10
#define UL_DELAY_ALLAY_LEN 4
...
struct wifi_res {
	...
	s32 chload;
	s32 ul_delay;
	u16 ul_delay_array[UL_DELAY_ALLAY_LEN];
	u16 tx_time[UL_DELAY_ALLAY_LEN];
	s8 beacon_rssi[BEACON_RSSI_LEN]; // the latest 10 rssi, 1 rssi per 100ms.
	u16 chload_self;
	u8 tx_mcs;
	u8 tx_nss;
	u8 tx_bw;
...
struct wifi_para {
	...
	u16 tx_time[UL_DELAY_ALLAY_LEN];
	s8 beacon_rssi[BEACON_RSSI_LEN]; // the latest 10 rssi, 1 rssi per 100ms.
	u16 chload_self; // Note the four-byte alignment
	u8 tx_mcs;
	u8 tx_nss;
	u8 tx_bw;
	u8 tx_flags;
