...
static inline long long read_atomic(const atomic64_t *v)
{
	return atomic64_read(v);
}

static inline void set_atomic(atomic64_t *v, long long i)
{
	return atomic64_set(v, i);
}
#else
static inline int read_atomic(const atomic_t *v)
{
	return atomic_read(v);
}

static inline void set_atomic(atomic_t *v, int i)
{
	atomic_set(v, i);
}
#endif
...
void chr_update_buf_time(s64 time, u32 protocal)
{
	...
	case SOL_TCP:
		...
		if (dif_jif > FILTER_TIME_LIMIT) {
			set_atomic(&g_chr_key_val.tcp_buf_time, cur_buf);
		} else {
			buff = read_atomic(&g_chr_key_val.tcp_buf_time);
			buff = buff - buff / ALPHA_FILTER_PARA +
				cur_buf / ALPHA_FILTER_PARA;
			set_atomic(&g_chr_key_val.tcp_buf_time, buff);
		}
	...
	case SOL_UDP:
		...
		if (dif_jif > FILTER_TIME_LIMIT) {
			set_atomic(&g_chr_key_val.udp_buf_time, cur_buf);
		} else {
			buff = read_atomic(&g_chr_key_val.udp_buf_time);
			buff = buff - buff / ALPHA_FILTER_PARA +
				cur_buf / ALPHA_FILTER_PARA;
			set_atomic(&g_chr_key_val.udp_buf_time, buff);
		}
...
static u32 report_buf(void)
{
	...
	jif = jiffies;
	buf64 = read_atomic(&g_chr_key_val.udp_buf_time);
	...
	buf_rtn = tmp_buf;
	buf64 = read_atomic(&g_chr_key_val.tcp_buf_time);
	tmp_buf = (u16) (buf64 / NS_CONVERT_TO_MS);
	if (buf64 > MAX_VALID_NS)
		tmp_buf = MAX_VALID_U16;
...
void web_stat_no_ack_report(u8 interface_type, u8 hash_cnt)
{
	...
	if (time_after(jiffies, g_abn_stamp_web_no_ack) &&
		...
		spin_unlock_bh(&g_web_stat_lock);
		chr_notify_event(CHR_WEB_STAT_EVENT, get_user_space_pid(), 0,
			g_rtn_abn);
...
void chr_no_ack_report(u32 hash_cnt, u8 if_type)
{
	...
	if (type == HTTP_GET && time_after(jiffies,
		...
		pr_info("chr: web no ack report uid:%d-port:%u\n", g_stream_list[hash_cnt].uid, htons(g_stream_list[hash_cnt].tcp_port));
...
void wifi_disconnect_report(void)
{
	...
	spin_unlock_bh(&g_web_stat_lock);
	chr_notify_event(CHR_WEB_STAT_EVENT, get_user_space_pid(), 0, g_rtn_stat);
	chr_notify_event(CHR_WEB_STAT_EVENT, get_user_space_pid(), 0,
		&g_rtn_stat[IPV6_INTERFACE]);
	spin_lock_bh(&g_web_stat_lock);
	memset(&g_rtn_stat, 0, sizeof(g_rtn_stat));
...
void abn_stamp_web_stat_report(u8 interface_type, u8 hash_cnt)
{
	...
	case WEB_FAIL:
		...
		pr_info("chr: web fail report uid:%d-port:%u\n", g_stream_list[hash_cnt].uid, htons(g_stream_list[hash_cnt].tcp_port));
...
static void rtt_large_abn_report(int index)
{
	...
	if (time_after(jiffies, g_abn_stamp_rtt_large) &&
		...
		chr_notify_event(CHR_WEB_STAT_EVENT,
			get_user_space_pid(), 0, g_rtn_abn);
...
static bool is_port_need(struct tcphdr *tcph, bool flag)
{
	int port = 0;
	if (flag)
		port = htons(tcph->source);
	else
...
static unsigned int hook_local_out(void *ops, struct sk_buff *skb, const struct nf_hook_state *state)
{
	...
	if (get_http_stream_iface(skb, false) == -1 || !is_port_need(tcph, false) ||
...
static unsigned int hook_local_in(void *ops, struct sk_buff *skb, const struct nf_hook_state *state)
{
	...
	if (get_http_stream_iface(skb, true) == -1 || !is_port_need(tcph, true) ||
...
void timestamp_initialization(void)
{
	...
	set_atomic(&g_chr_key_val.tcp_buf_time, 0);
	set_atomic(&g_chr_key_val.udp_buf_time, 0);
}
...
int set_report_app_uid(int tag, u32 paras)
{
	...
	if (tag == GET_AP_REPORT_TAG) {
		if (paras & 0x01) {
			chr_notify_event(CHR_SPEED_SLOW_EVENT, get_user_space_pid(),
				report_buf(), NULL);
			return 0;
		}
	}
