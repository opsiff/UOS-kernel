...
static bool row_ctr_get_origin_dev(struct dm_target *ti, unsigned int *argc, char **argv, struct dm_row *s, int *err, char *origin_path, fmode_t *origin_mode, dev_t *origin_dev)
{
	if (*argc != 4) {
...
static bool row_ctr_get_cow_dev(struct dm_target *ti, unsigned int *argc, char **argv, struct dm_row *s, int *err, char *cow_path, dev_t *origin_dev, dev_t *cow_dev)
{
	cow_path = argv[0];
	argv++;
	*argc--;
...
static bool row_ctr_get_cow_data(struct dm_target *ti, unsigned int *argc, char **argv, struct dm_row *s, int *err, unsigned int *args_used, unsigned long *bmp_len)
{
...
static int row_ctr(struct dm_target *ti, unsigned int argc, char **argv)
{
	...
	if (!row_ctr_get_origin_dev(ti, &argc, argv, s, &err, origin_path, &origin_mode, &origin_dev))
		return err;
	if (!row_ctr_get_cow_dev(ti, &argc, argv, s, &err, cow_path, &origin_dev, &cow_dev))
		return err;
	if (!row_ctr_get_cow_data(ti, &argc, argv, s, &err, &args_used, &bmp_len))
		return err;
...
static bool row_map_preprocess(struct dm_row *s, int *err, struct bio *bio, sector_t *chunk)
{
...
static int row_map(struct dm_target *ti, struct bio *bio)
{
	...
	if (!row_map_preprocess(s, &err, bio, &chunk))
		return err;
