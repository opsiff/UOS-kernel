...
static int pca953x_gpio_direction_input(struct gpio_chip *gc, unsigned off)
{
	...
	dev_info(&chip->client->dev, "%s: off=%u\n", __func__, off);
		...
		unsigned off, int val)
{
	...
	dev_info(&chip->client->dev, "%s: off=%u,val=%d\n", __func__, off, val);
	mutex_lock(&chip->i2c_lock);
	/* set output level */
...
static int pca953x_irq_setup(struct pca953x_chip *chip, int irq_base)
{
	...
	ret = devm_request_threaded_irq(&client->dev, client->irq,
					...
					IRQF_ONESHOT | IRQF_SHARED | IRQF_NO_SUSPEND,
...
static void pca953x_init_gpio_state(struct pca953x_chip *chip, struct device *dev)
{
	int i;
	int ret;
	...
	for (i = 0; i < PCA953X_PINNUM; i++) {
		if (pin_val[i] == 0) { /* 0: direction input */
		...
		if (pin_val[i] == 1) { /* 1: direction output low */
		...
		if (pin_val[i] == 2) { /* 2: direction output high */
...
static void pca953x_debug_init(struct pca953x_chip *pca953x_chip)
{
	...
	if (pca953x_chip->names) {
		...
		if (snprintf_s(node_num_str, PCA_DEBUG_NODE_LENGTH, PCA_DEBUG_NODE_LENGTH -1,
			"sub_%d", node_num_int) < 0)
...
static void pca953x_reset_chip(struct i2c_client *client)
{
	int rest_gpio;
	int ret;

	rest_gpio = of_get_named_gpio(client->dev.of_node, "reset-chip-gpio", 0);
	if (!gpio_is_valid(rest_gpio)) {
		dev_err(&client->dev, "reset gpio is invalid\n");
		return;
	}
	dev_info(&client->dev, "reset_chip_gpio %d\n", rest_gpio);

	ret = gpio_request(rest_gpio, "reset_chip_gpio");
	if (ret) {
		dev_err(&client->dev, "reset gpio request fail %d\n", ret);
		return;
	}

	ret = gpio_direction_output(rest_gpio, 0);
	mdelay(5);
	ret += gpio_direction_output(rest_gpio, 1);
	if (ret)
		dev_err(&client->dev, "reset gpio set fail %d\n", ret);
			 ...
			 const struct i2c_device_id *i2c_id)
{
	...
	if (ret)
		pr_err("%s: setup irq fail ret=%d", __func__, ret);
	...
	pca953x_init_gpio_state(chip, &client->dev);
#ifdef CONFIG_GPIO_PCA953X_IRQ
