...
#if defined(CONFIG_CPU_ISOLATION_OPT) && defined(CONFIG_DFX_DEBUG_FS)
			      ...
			      char *buf)
{
	...
	if (!smt_privileged_user()) {
		struct cpumask orig_map, new_map;

		cpumask_copy(&orig_map, ca->map);
		cpu_to_core_mask(&orig_map, &new_map);
		return cpumap_print_to_pagebuf(true, buf, &new_map);
	}
...
#ifdef CONFIG_SCHED_SMT_USER_HIDE
	if (!smt_privileged_user())
		return sysfs_emit(buf, "%d\n", num_possible_cpus() - num_secondary_cpus() - 1);
#endif
