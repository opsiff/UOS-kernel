		...
		unsigned long action, void *data)
{
	...
	case USB_BUS_REMOVE:
		...
		{
			struct usb_bus *ubus = (struct usb_bus *)data;
			printk(KERN_INFO "%s:USB_BUS_REMOVE uvc at bus %d, remove bus %d\n", __func__, stream->dev->udev->bus->busnum,
				ubus->busnum);
			if (ubus->busnum != stream->dev->udev->bus->busnum)
				break;
		}
#endif
