...
int i2c_dw_init_master(struct dw_i2c_dev *dev)
{
	...
	CALL_HCK_VH(i2c_dw_dma_fifo_cfg, dev, &err);
	if (!err)
...
void i2c_dw_xfer_init(struct dw_i2c_dev *dev)
{
	...
	CALL_HCK_VH(i2c_dw_xfer_init_irq_mask, dev, &err);
	if (!err)
...
static int i2c_dw_irq_handler_master(struct dw_i2c_dev *dev)
{
	 ...
	 */
	CALL_HCK_VH(i2c_dw_stop_irq_handler, dev, stat);
tx_aborted:
	if ((stat & (DW_IC_INTR_TX_ABRT | DW_IC_INTR_STOP_DET)) || dev->msg_err) {
		CALL_HCK_VH(i2c_dw_irq_mask_all, dev);
