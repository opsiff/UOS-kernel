...
enum smartpa_enumrate_state {
	SMARTPA_ENUMRATE_SUCCED = 0,
	SMARTPA_ALREADY_ENUMRATED_NEED_SKIP,
};
...
/* for system/lib64/x.so(64 bits) */
struct smartpakit_high_power_set_param {
	unsigned int pwm_level;
	unsigned int voltage_mode;
	unsigned int need_add_timer;
	struct smartpakit_set_param config_param;
};
...
/* for system/lib/x.so(32 bits) */
struct smartpakit_high_power_set_param_compat {
	unsigned int pwm_level;
	unsigned int voltage_mode;
	unsigned int need_add_timer;
	struct smartpakit_set_param_compat config_param;
};

struct smartpakit_high_power_info {
...
#define SMARTPAKIT_HIGH_POWER_CONFIG \
	_IOW('M', 0x28, struct smartpakit_high_power_set_param)
...
#define SMARTPAKIT_HIGH_POWER_CONFIG_COMPAT \
	_IOW('M', 0x28, struct smartpakit_high_power_set_param_compat)

// pa check loop
#define SMARTPAKIT_CHECK    _IOW('M', 0x27, int)

#define SMARTPAKIT_HIGH_POWER_R_INFO \
	_IOR('M', 0x29, struct smartpakit_high_power_info)
