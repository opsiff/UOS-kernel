...
#define BOX_CONSISTENT_COMPARE_MIN_NUM 2
...
struct boxs_vendor_test_groups {
	unsigned int num;
	int boxid[BOX_NUM_MAX];
};

struct box_consistent_test {
	bool test_support;
	unsigned int group_num;
	unsigned int total_elem_num;
	unsigned int count_of_elem_each_group[BOX_NUM_MAX];
	...
	int actual_id)
{
	...
	for (i = 0; i < box_group->num; i++) {
		...
		memcpy(copy_info, actual_info, sizeof(struct box_info_st));
	}
}
...
static void box_consistent_group_show_info(struct box_consistent_test *test)
{
	unsigned int i;
	unsigned int j;
	struct boxs_vendor_test_groups *group = NULL;
	...
	for (i = 0; i < test->group_num; i++) {
		group = (struct boxs_vendor_test_groups *)(&test->groups[i]);
		pr_info("%s: group[%d]'s num = %d\n", __func__, i, group->num);
		for (j = 0; j < group->num; j++)
			pr_info("%s: group[%d][%d] boxid = %d\n",
				__func__, i, j, group->boxid[j]);
	}
}

static bool box_consistent_test_check_legality(struct device_node *node,
	struct box_consistent_test *test,
	unsigned int *array, unsigned int arr_len)
{
	int ret;
	unsigned int elem_num_dt_parse;
	const char *test_groups = "consistent_test_groups";

	ret = of_property_count_elems_of_size(node,
		test_groups, (int)sizeof(u32));
	if (ret < 0) {
		pr_err("%s: count %s failed, ret:%d\n",
			__func__, test_groups, ret);
		return false;
	}
	elem_num_dt_parse = (unsigned int)ret;
	if (elem_num_dt_parse != test->total_elem_num) {
		pr_err("%s: total_elem_num:%d and %d is error, is not match\n",
			__func__, test->total_elem_num, elem_num_dt_parse);
		return false;
	}

	if (elem_num_dt_parse > arr_len) {
		pr_err("%s: elem_num_dt_parse:%d is invaild, pls check\n",
			__func__, elem_num_dt_parse);
		return false;
	}
	ret = of_property_read_u32_array(node, test_groups,
		array, (size_t)(long)elem_num_dt_parse);
	if (ret < 0) {
		pr_err("%s: get %s array failed\n", __func__,
			test_groups);
		return false;
...
static void box_consistent_parse_test_groups(struct device_node *node,
	struct box_consistent_test *test)
{
	unsigned int i;
	unsigned int j;
	unsigned int index = 0;
	unsigned int test_group_dt_parse[BOX_NUM_MAX];

	if (!box_consistent_test_check_legality(node, test,
		test_group_dt_parse, BOX_NUM_MAX)) {
		pr_err("%s: test_group check error\n", __func__);
		goto box_parse_test_groups_err;
	}

	for (i = 0; i < test->group_num; i++) {
		test->groups[i].num = test->count_of_elem_each_group[i];
		if (test->groups[i].num < BOX_CONSISTENT_COMPARE_MIN_NUM ||
			test->groups[i].num > BOX_NUM_MAX) {
			pr_err("%s: groups[%u].num: %u, is error\n",
				__func__, i, test->groups[i].num);
			goto box_parse_test_groups_err;
		}

		for (j = 0; j < test->groups[i].num; j++) {
			if (index >= BOX_NUM_MAX) {
				pr_err("%s: i: %d index: %u, is invalid\n",
					__func__, i, index);
				return;
			}
			if (test_group_dt_parse[index] >= BOX_NUM_MAX) {
				pr_err("%s:group_dt_parse[%u]:%u, is error\n",
					__func__, index,
					test_group_dt_parse[index]);
				goto box_parse_test_groups_err;
			}
			test->groups[i].boxid[j] = test_group_dt_parse[index];
			index++;
		}
	}
	box_consistent_group_show_info(test);

	return;
box_parse_test_groups_err:
	test->group_num = 0;
	test->test_support = false;
}

static void box_consistent_parse_elem_num_each_group(struct device_node *node,
	struct box_consistent_test *test)
{
	int ret;
	unsigned int i;
	const char *each_group_elem_num = "each_group_elem_num";
	...
	ret = of_property_count_elems_of_size(node,
		each_group_elem_num, (int)sizeof(u32));
	if (ret < 0) {
		pr_err("%s: count %s failed, ret:%d\n",
			__func__, each_group_elem_num, ret);
		goto parse_elem_num_each_group_err;
	}
	test->group_num = (unsigned int)ret;
	if (test->group_num == 0 || test->group_num > BOX_NUM_MAX) {
		pr_err("%s: test group_num %d is invalid\n", __func__,
			test->group_num);
		goto parse_elem_num_each_group_err;
	}

	ret = of_property_read_u32_array(node, each_group_elem_num,
		test->count_of_elem_each_group,
		(size_t)(long)(test->group_num));
	if (ret < 0) {
		pr_err("%s: get %s array failed\n", __func__,
			each_group_elem_num);
		goto parse_elem_num_each_group_err;
	...
	for (i = 0; i < test->group_num; i++)
		test->total_elem_num += test->count_of_elem_each_group[i];

	pr_info("%s:group_num:%d, total_elem_num:%d\n", __func__,
		test->group_num, test->total_elem_num);
	box_consistent_parse_test_groups(node, test);

	return;
parse_elem_num_each_group_err:
	pr_err("%s: get test_group_num:%d failed\n", __func__, test->group_num);
	test->group_num = 0;
	test->test_support = false;
}
...
static void box_parse_dt_consistent_test_info(struct platform_device *pdev)
{
	...
	box_consistent_parse_elem_num_each_group(node, test);
}
