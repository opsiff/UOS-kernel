...
typedef struct {
	int feature_item;
	int (*func_ptr)(char *cur, void *ctrl, size_t count);
} lcd_kit_dbg_event_handle;
...
static ssize_t lcdkit_dbg_level_set(char *cur, void *ctrl, size_t count)
{
	int cnt = 0;
	int val = 0;
...
static ssize_t lcdkit_dbg_mipi_clk(char *cur, void *ctrl, size_t count)
{
	int cnt = 0;
	int val = 0;
...
static ssize_t lcdkit_dbg_reg_read(char *cur, void *ctrl, size_t count)
{
	lcdkit_release(ctrl);
	return count;
}

static ssize_t lcdkit_dbg_reg_write(char *cur, void *ctrl, size_t count)
{
	lcdkit_release(ctrl);
	return count;
}

static ssize_t lcdkit_dbg_init_code(char *cur, void *ctrl, size_t count)
{
	int cnt = 0;
	int val = 0;
	int ret = 0;
...
static ssize_t lcdkit_dbg_panel_vsp_vsn(char *cur, void *ctrl, size_t count)
{
	int ret = 0;
...
static ssize_t lcdkit_dbg_esd_enable(char *cur, void *ctrl, size_t count)
{
	int val = 0;
	int cnt = 0;
...
static ssize_t lcdkit_dbg_esd_recover_test(char *cur, void *ctrl, size_t count)
{
	int cnt = 0;
	int val = 0;
...
static ssize_t lcdkit_dbg_esd_reset(char *cur, void *ctrl, size_t count)
{
	int cnt = 0;
	int val = 0;
...
static ssize_t lcdkit_dbg_esd_bl_enable(char *cur, void *ctrl, size_t count)
{
	int cnt = 0;
	int val = 0;
...
static ssize_t lcdkit_dbg_esd_bl_set(char *cur, void *ctrl, size_t count)
{
	int cnt = 0;
	int val = 0;
...
static ssize_t lcdkit_dbg_esd_check_reg(char *cur, void *ctrl, size_t count)
{
	int ret = 0;
	char *ptr1, *ptr2;
	unsigned long temp = 0;
...
static ssize_t lcdkit_dbg_esd_check_value(char *cur, void *ctrl, size_t count)
{
	int ret = 0;
	int cnt = 0;
	char *ptr1, *ptr2;
...
static const lcd_kit_dbg_event_handle g_lcd_kit_handle[] = {
	{ LCDKIT_DBG_LEVEL_SET, lcdkit_dbg_level_set },
	{ LCDKIT_DBG_MIPI_CLK, lcdkit_dbg_mipi_clk },
	{ LCDKIT_DBG_REG_READ, lcdkit_dbg_reg_read },
	{ LCDKIT_DBG_REG_WRITE, lcdkit_dbg_reg_write },
	{ LCDKIT_DBG_INIT_CODE, lcdkit_dbg_init_code },
	{ LCDKIT_DBG_PANEL_VSP_VSN, lcdkit_dbg_panel_vsp_vsn },
	{ LCDKIT_DBG_ESD_ENABLE, lcdkit_dbg_esd_enable },
	{ LCDKIT_DBG_ESD_RECOVER_TEST, lcdkit_dbg_esd_recover_test },
	{ LCDKIT_DBG_ESD_RESET, lcdkit_dbg_esd_reset },
	{ LCDKIT_DBG_ESD_BL_ENABLE, lcdkit_dbg_esd_bl_enable },
	{ LCDKIT_DBG_ESD_BL_SET, lcdkit_dbg_esd_bl_set },
	{ LCDKIT_DBG_ESD_CHECK_REG, lcdkit_dbg_esd_check_reg },
	{ LCDKIT_DBG_ESD_CHECK_VALUE, lcdkit_dbg_esd_check_value },
};

static ssize_t get_cmd_type(char **cur, int *cmd_type, size_t count, void *ctrl)
{
	int i = 0;
	int length = sizeof(lcdkit_cmd_list) / sizeof(lcdkit_cmd_list[0]);

	for (i = 0; i < length; i++) {
		if (lcdkit_str_start_with(*cur, lcdkit_cmd_list[i].pstr) == 0) {
...
/* write function */
static ssize_t lcdkit_dbg_write(struct file *file, const char __user *buff,
	size_t count, loff_t *ppos)
{
	...
	int table_num = sizeof(g_lcd_kit_handle) / sizeof(lcd_kit_dbg_event_handle);
	...
	if (ctrl == NULL) {
		...
		lcdkit_release(ctrl);
		return count;
	} else if (copy_from_user(lcd_debug_buf, buff, count)) {
		lcdkit_release(ctrl);
		return -EFAULT;
	} else if (lcdkit_str_to_lower(cur) != 0) {
	...
	ret1 = lcdkit_lock(ctrl);
	if (ret1) {
		LCDKIT_ERR("lock failed or panel off.\n");
		return -EFAULT;
	}
	LCDKIT_DEBUG("cur=%s, count=%d!\n", cur, (int)count);
	if (get_cmd_type(&cur, &cmd_type, count, ctrl) == -EFAULT)
		return -EFAULT;

	for (i = 0; i < table_num; i++) {
		if ((cmd_type == g_lcd_kit_handle[i].feature_item) &&
			(g_lcd_kit_handle[i].func_ptr))
			return g_lcd_kit_handle[i].func_ptr(cur, ctrl, count);
	}
