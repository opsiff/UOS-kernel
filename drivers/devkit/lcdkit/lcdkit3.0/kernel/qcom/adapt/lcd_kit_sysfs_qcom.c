...
static void get_oled_type(uint32_t panel_id, char *oled_type, int len)
{
	...
	if (common_info->oled_type == LTPS)
		...
		strncpy(oled_type, "INVALID", strlen("INVALID"));
}
...
static void dual_panel_fps_scene_show_prepare(void)
{
	u32 panel_state;
	int i;
	...
	for (i = 0; i < MAX_ACTIVE_PANEL; i++) {
		panel_state = lcm_get_panel_state(i);
		if (panel_state == LCD_POWER_STATE_ON)
			return;
	}
	wait_panel_initialized();
	LCD_KIT_INFO("exit");
}
	...
	struct device_attribute *attr, char *buf)
{
	...
	if (lcd_kit_get_product_type() != LCD_SINGLE_PANEL_TYPE)
		dual_panel_fps_scene_show_prepare();
...
static ssize_t lcd_real_fps_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	int ret;
	u32 panel_type = LTPS;
	...
	if (!buf || !attr) {
		LCD_KIT_ERR("lcd_real_fps_show buf NULL\n");
		return LCD_KIT_FAIL;
	}
	panel_type = lcd_get_active_oled_type(panel_id);
	if (panel_type != LTPO1)
		return LCD_KIT_FAIL;
...
static int lcd_check_checksum(uint32_t panel_id)
{
	...
	if (FACT_INFO->checksum.value_ext.arry_data) {
		...
		if (!err_cnt_ext) {
			...
			if (!err_cnt)
				LCD_KIT_INFO("primary checksum pass, ret is LCD_KIT_OK\n");
		}
	}
	return ret;
}
...
static void lcd_get_parse_data_info_data(char *lcd_parse_data, char *buf)
{
	int i;
	...
	struct device_attribute* attr, char *buf)
{
	...
	lcd_get_parse_data_info_data(str_oem, oem_info_data);
	...
	struct device_attribute *attr, char *buf)
{
	...
	ret = snprintf(buf, PAGE_SIZE, "%s\n", str_oem);
	LCD_KIT_INFO("lcd_panel_sncode_show str_oem = %s\n", str_oem);
	if (ret < 0) {
		LCD_KIT_ERR("snprintf fail\n");
		return LCD_KIT_FAIL;
	}
	...
	struct device_attribute *attr, char *buf)
{
	...
	value = lcd_kit_get_product_type();
	if (value == LCD_DUAL_PANEL_NOT_SIM_TYPE)
		value = LCD_DUAL_PANEL_ASYNC_DISPLAY_TYPE;
	if (value == LCD_SINGLE_DSI_MULTIPLEX_TYPE)
		value = LCD_DUAL_PANEL_SIM_DISPLAY_TYPE;
...
static int lcd_mipi_detect_validate_reg(uint32_t panel_id, unsigned char *read_value,
	unsigned int pic_num, int loop_num)
{
	int i;
	int ret = LCD_KIT_OK;
	unsigned int *detect_value = NULL;
	...
	for (i = 0; i < config_data.cnt; i++) {
		...
		LCD_KIT_INFO("pic: %u, read_value%d = 0x%x, except_value = 0x%x",
			pic_num, i, read_value[i], expect_value);
		if ((i < VAL_NUM) && (loop_num < MIPI_DETECT_LOOP))
			FACT_INFO->mipi_detect.reg_val[loop_num][pic_num][i] = read_value[i];
		if (read_value[i] >= expect_value) {
...
static int lcd_judge_mipi_detect(uint32_t panel_id, unsigned int pic_num, int loop_num)
{
	...
	ret = lcd_mipi_detect_validate_reg(panel_id, read_value, pic_num, loop_num);
	if (ret) {
		LCD_KIT_ERR("mipi detect compare error\n");
		return ret;
	}
...
static void lcd_mipi_detect_compare(uint32_t panel_id, unsigned int *err_flag)
{
	int i;
	int err_record = 0;
...
static int lcd_mipi_detect(uint32_t panel_id)
{
	...
	LCD_KIT_INFO("count = %d, loop_num = %d\n", count, loop_num);
	if (loop_num >= MIPI_DETECT_LOOP) {
		LCD_KIT_INFO("loop num max\n");
		lcd_mipi_detect_compare(panel_id, err_flag);
		loop_num = 0;
	}
	LCD_KIT_INFO("exit\n");
	return ret;
...
static int lcd_mipi_detect_set(uint32_t panel_id, unsigned int pic_num)
{
	...
	ret = adapt_ops->mipi_tx(panel_info->display, &enter_cmds[pic_num]);
	if (ret) {
...
static int fps_cmd_send(struct dsi_display *display, struct lcd_kit_dsi_panel_cmds *cmds)
{
	struct lcd_kit_adapt_ops *adapt_ops = NULL;
	adapt_ops = lcd_kit_get_adapt_ops();
	if (!adapt_ops) {
	...
	return adapt_ops->mipi_tx(display, cmds);
}


static void send_fps_lock_cmd(unsigned int panel_id, unsigned int val)
{
	int ret = LCD_KIT_OK;
	struct qcom_panel_info *panel_info = NULL;
	int table_num;
	...
	for (i = 0; i < table_num; i++) {
		if (val == fps_lock_cmd[i].val)
			ret = fps_cmd_send(panel_info->display, fps_lock_cmd[i].cmds);
	}
...
static int lcd_check_support(int index)
{
	...
	case PANEL_DIEID_INDEX:
		return pinfo->panel_dieid_info.support;
...
struct lcd_kit_sysfs_ops g_lcd_sysfs_ops = {
	...
	.real_fps_show = lcd_real_fps_show,
