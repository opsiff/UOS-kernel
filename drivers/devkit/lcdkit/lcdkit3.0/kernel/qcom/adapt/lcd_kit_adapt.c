...
void dsi_display_set_cmd_tx_ctrl_flags(struct dsi_display *display, struct dsi_cmd_desc *cmd);
#endif
	...
	struct lcd_kit_dsi_cmd_desc *cmds, int link_state)
{
	...
	if ((display->ctrl_count > 1) && (!(cmd.msg.flags & MIPI_DSI_MSG_UNICAST)) &&
		...
		LCD_KIT_INFO("dsi display broadcast cmd\n");
		ret = dsi_display_broadcast_cmd(display, &cmd);
	} else {
		ret = dsi_ctrl_transfer_prepare(dsi_ctrl, cmd.ctrl_flags);
		if (ret) {
			LCD_KIT_ERR("failed to prepare for command transfer: %d\n", ret);
			return ret;
		}

		ret = dsi_ctrl_cmd_transfer(dsi_ctrl, &cmd);
		common_info->close_dual_dsi_sn = 0;

		dsi_ctrl_transfer_unprepare(dsi_ctrl, cmd.ctrl_flags);
	}
...
int qcom_mipi_dsi_cmd_rx(struct dsi_display *display, struct dsi_ctrl *dsi_ctrl, unsigned char *buffer,
	unsigned int *len, struct lcd_kit_dsi_cmd_desc *cmds, int link_state)
{
	...
	ret = dsi_ctrl_transfer_prepare(dsi_ctrl, cmd.ctrl_flags);
	if (ret) {
		LCD_KIT_ERR("prepare for rx cmd transfer failed rc = %d\n", ret);
		return LCD_KIT_FAIL;
	}
...
static int lcd_kit_gpio_disable_plugin(uint32_t panel_id, u32 type)
{
	lcd_kit_gpio_tx_plugin(panel_id, type, GPIO_LOW);
	return LCD_KIT_OK;
}
...
static void lcd_regulator_disable(struct lcd_kit_array_data *pvreg_info, struct lcd_kit_regulate *pvreg)
{
	(void)regulator_disable(pvreg->vreg);
	(void)regulator_set_load(pvreg->vreg, pvreg_info->buf[POWER_DISABLE_LOAD]);
	if (regulator_count_voltages(pvreg->vreg) > 0) {
		LCD_KIT_INFO("pvreg->vreg is not 0\n");
		regulator_set_voltage(pvreg->vreg, 0, pvreg_info->buf[POWER_MAX_VOL]);
	}
}
	...
	struct lcd_kit_regulate *pvreg, bool enable)
{
	...
	if (enable) {
		...
		lcd_regulator_disable(pvreg_info, pvreg);
	}
...
static int lcd_kit_regulator_disable_plugin(uint32_t panel_id, u32 type)
{
	int ret = LCD_KIT_OK;

	switch (type) {
	case LCD_KIT_VCI:
		ret = lcd_kit_regulate_ctrl(g_pwr_off_optimize_cfg.lcd_vci, g_pwr_off_optimize_cfg.vci_vreg, false);
		break;
	case LCD_KIT_IOVCC:
		ret = lcd_kit_regulate_ctrl(g_pwr_off_optimize_cfg.lcd_iovcc, g_pwr_off_optimize_cfg.iovcc_vreg, false);
		break;
	case LCD_KIT_VDD:
		ret = lcd_kit_regulate_ctrl(g_pwr_off_optimize_cfg.lcd_vdd, g_pwr_off_optimize_cfg.vdd_vreg, false);
		break;
	case LCD_KIT_VSN:
		ret = lcd_kit_regulate_ctrl(g_pwr_off_optimize_cfg.lcd_vsn, g_pwr_off_optimize_cfg.vsn_vreg, false);
		break;
	default:
		LCD_KIT_ERR("regulator type:%d not support\n", type);
		break;
