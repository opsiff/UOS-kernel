...
static void lcd_kit_panel_parse_local_hbm_circle_cmds(uint32_t panel_id, struct device_node *np);
...
static void lcd_kit_nve_handle(char *read_value, char *expect_ptr, uint32_t panel_id)
{
	char write_nve_value[RW_NV_ARR_MAX] = {0};
	uint32_t primpanel_checkreg_state = 0;
	uint32_t secpanel_checkreg_state = 0;
	struct qcom_panel_info *pinfo = lcm_get_panel_info(panel_id);

	if ((char)read_value[0] == expect_ptr[0]) {
		lcd_kit_set_panel_checkreg_state(panel_id, PANEL_CHECKREG_SUCC_STATE);
		primpanel_checkreg_state = lcd_kit_get_panel_checkreg_state(PRIMARY_PANEL);
		secpanel_checkreg_state = lcd_kit_get_panel_checkreg_state(SECONDARY_PANEL);
	}
	if (primpanel_checkreg_state == PANEL_CHECKREG_SUCC_STATE && secpanel_checkreg_state == PANEL_CHECKREG_SUCC_STATE) {
		if (lcd_kit_write_nv_info(pinfo, PANEL_NV_NUM, sizeof(write_nve_value), "LCD_STA", write_nve_value))
	...
	struct lcd_kit_check_reg_dsm *check_reg_dsm)
{
	...
	if (ret == LCD_KIT_OK) {
		if (lcd_kit_get_product_type() == LCD_DUAL_PANEL_NOT_SIM_TYPE)
			lcd_kit_nve_handle((char *)read_value, expect_ptr, panel_id);
...
{
	if (lcd_kit_is_power_event(event) && (enable == 0) &&
		common_info->pwr_off_optimize_info.support) {
...
static void lcd_kit_panel_parse_local_hbm_circle_cmds(uint32_t panel_id, struct device_node *np)
{
	if (common_info->hbm.support) {
		...
		lcd_kit_parse_u32(np, "lcd-kit,local-hbm-threshold",
			&common_info->hbm.local_hbm_v1_hbm_dbv_threshold, 0);
		lcd_kit_parse_u32(np, "lcd-kit,local-hbm-version",
			&common_info->hbm.local_hbm_version, 0);
		if (common_info->hbm.local_hbm_version) {
			lcd_kit_parse_dcs_cmds(np,
...
static void parse_power_off_time_optimize(uint32_t panel_id, const struct device_node *np);
...
static void lcd_kit_panel_parse_util(uint32_t panel_id, const struct device_node *np)
{
	...
	parse_power_off_time_optimize(panel_id, np);
}
...
static void lcd_kit_panel_parse_ddic_cmd(uint32_t panel_id, const struct device_node *np)
{
	...
	lcd_kit_parse_u32(np, "lcd-kit,od-support",
		&common_info->od_support, 0);
	lcd_kit_parse_u32(np, "lcd-kit,irc-support",
		&common_info->irc_support, 0);
}
...
static void lcd_kit_panel_parse_backlight(uint32_t panel_id, const struct device_node *np)
{
	...
	lcd_kit_parse_u32(np, "lcd-kit,panel-bl-trig-frame",
		&common_info->backlight.bl_trig_frame, 0);
...
static void lcd_kit_panel_parse_sn_code(uint32_t panel_id, const struct device_node *np)
{
	...
	lcd_kit_parse_u32(np, "lcd-kit,read-sn-not-close-dual",
		&common_info->sn_code.read_sn_not_close_dual, 0);
...
static int lcd_kit_panel_parse_dt(uint32_t panel_id, struct device_node *np)
{
	...
	/* parse local_hbm_circle info */
	lcd_kit_panel_parse_local_hbm_circle_cmds(panel_id, np);
	/* parse normal info */
