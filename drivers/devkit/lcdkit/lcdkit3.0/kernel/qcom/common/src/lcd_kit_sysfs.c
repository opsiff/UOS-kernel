...
static ssize_t lcd_kit_real_fps_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	ssize_t ret = LCD_KIT_OK;
	struct lcd_kit_sysfs_ops *sysfs_ops = NULL;
	...
	if (sysfs_ops->real_fps_show)
		ret = sysfs_ops->real_fps_show(dev, attr, buf);
	return ret;
}
	...
	struct device_attribute *attr, char *buf)
{
	...
	if (sysfs_ops->pre_camera_position_show)
		ret = sysfs_ops->pre_camera_position_show(dev, attr, buf);
	...
	struct device_attribute *attr, char *buf)
{
	...
	mutex_lock(&COMMON_LOCK->sysfs_lock.panel_dieid_lock);
	if (sysfs_ops->panel_dieid_info_show)
		ret = sysfs_ops->panel_dieid_info_show(dev, attr, buf);
	mutex_unlock(&COMMON_LOCK->sysfs_lock.panel_dieid_lock);
	return ret;
}
	...
	struct device_attribute *attr, char *buf)
{
	...
	if (sysfs_ops->product_type_show)
		ret = sysfs_ops->product_type_show(dev, attr, buf);
	return ret;
}
...
static DEVICE_ATTR(lcd_fps_order, 0644, lcd_kit_fps_order_show,
	NULL);
static DEVICE_ATTR(lcd_real_fps, 0644, lcd_kit_real_fps_show,
	NULL);
