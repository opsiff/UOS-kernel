...
#define MAX_DBV 4095
#define MAX_INIT_TIME 1000
#define MAX_WAKE_TIME 500
#define  PANEL_NV_NUM 460
#define  PANEL_CHECKREG_SUCC_STATE 1
#define  PANEL_CHECKREG_FAIL_STATE 0
#define  PANEL_NV_SUCC_STATE 1
#define  PANEL_NV_INIT_STATE 0
#define NVE_BUF0_EQUAL_MAX_NUM '2'
#define NVE_BUF1_EQUAL_TWO '2'
#define NVE_BUF1_EQUAL_ONE '1'
#define NVE_BUF1_EQUAL_ZERO '0'
...
static uint32_t panel_nve_state;
static uint32_t primary_panel_checkreg_state;
static uint32_t secondary_panel_checkreg_state;
struct completion aod_lp1_done;
struct completion lcd_panel_init_done[MAX_ACTIVE_PANEL];
struct completion lcd_panel_initialized;
struct completion first_frame_done;
struct completion power_off_done;
struct lcd_pwr_off_optimize_config g_pwr_off_optimize_cfg = {0};
...
uint32_t lcd_get_active_oled_type(uint32_t panel_id)
{
	return common_info->oled_type;
}
...
static void lcd_kit_btb_check(uint32_t panel_id)
{
	int ret;
	uint8_t read_value = 0;
	...
	ret = lcd_kit_dsi_cmds_rx(panel_info->display, &read_value,
		1, &disp_info->btb_check.cmds);
	if (ret) {
		LCD_KIT_ERR("btb check read error\n");
		return;
	}

	expect_value = disp_info->btb_check.value.buf[0];
	if (read_value > expect_value) {
#if defined CONFIG_HUAWEI_DSM
		ret_dsm = snprintf(record_buf, DMD_RECORD_BUF_LEN,
			"btb check reg: read_val = 0x%x, expe_val = 0x%x\n",
			read_value, expect_value);
		if (ret_dsm < 0)
			LCD_KIT_ERR("snprintf happened error!\n");
		(void)lcd_dsm_client_record(lcd_dclient, record_buf,
			DSM_LCD_BTB_CHECK_ERROR_NO,
			REC_DMD_NO_LIMIT,
			&recordtime);
#endif
		LCD_KIT_ERR("btb check error: read_val = 0x%x, expe_val = 0x%x\n", read_value, expect_value);
	} else {
		LCD_KIT_INFO("btb check normal: read_val = 0x%x, expe_val = 0x%x\n", read_value, expect_value);
	}
}
...
static void bl_notify(struct dsi_panel *panel)
{
	...
	notify_backlight_event(priv);
}

static int bl_cmd_tx(struct dsi_panel *panel, struct mipi_dsi_device *dsi)
{
...
static void bl_order_handle(int tuned_level, struct dsi_panel *panel)
{
	uint32_t panel_id = lcd_kit_get_current_panel_id(panel);
	...
	unsigned int level)
{
	...
	LCD_KIT_INFO("%d+, in_level [%u]", panel_id, level);
	...
	bl_order_handle(tuned_level, panel);
	if (!common_info->backlight.bl_cmd.link_state) {
		mode_flags = dsi->mode_flags;
		dsi->mode_flags |= MIPI_DSI_MODE_LPM;
	}
	ret = bl_cmd_tx(panel, dsi);
	if (common_info->backlight.bl_trig_frame)
		bl_notify(panel);
	...
	LCD_KIT_INFO("%d-, tuned_level [%u]", panel_id, tuned_level);
	return ret;
}
...
static int lcd_kit_dsi_panel_power_on(struct dsi_panel *panel)
{
	...
	complete_all(&lcd_panel_initialized);
...
static void wait_power_off_done(struct dsi_panel *panel)
{
	uint32_t panel_id = lcd_kit_get_current_panel_id(panel);
	int working_flag;

	working_flag = g_pwr_off_optimize_cfg.working_flag;
...
int lcd_kit_panel_pre_prepare(struct dsi_panel *panel)
{
	...
	wait_power_off_done(panel);
	mutex_lock(&panel->panel_lock);

	if (lcd_kit_get_product_type() == LCD_DUAL_PANEL_NOT_SIM_TYPE &&
		!strcmp(panel->type, "secondary")) {
		lcm_set_panel_state(PRIMARY_PANEL, LCD_POWER_STATE_OFF);
		LCD_KIT_ERR("set another panel state on\n");
	}

	if (lcd_kit_get_product_type() == LCD_DUAL_PANEL_NOT_SIM_TYPE &&
		!strcmp(panel->type, "primary")) {
		lcm_set_panel_state(SECONDARY_PANEL, LCD_POWER_STATE_OFF);
		LCD_KIT_ERR("set secondary panel state off\n");
	}
...
static void lcd_kit_power_off_optimize_work(struct work_struct *work)
{
	...
	if (!g_pwr_off_optimize_cfg.working_flag) {
		...
		__pm_relax(g_pwr_off_optimize_cfg.pwr_off_optimize_wlock);
		return;
	}
	...
	for (i = 0; i < g_pwr_off_optimize_cfg.panel_power_off_seq_cnt; i++) {
		...
		ret = lcd_kit_event_handler_plugin(panel_id, NULL,
...
void lcd_kit_power_off_optimize_handle(void)
{
	uint32_t panel_id = lcd_get_active_panel_id();

	if (!common_info->pwr_off_optimize_info.support)
		return;
	if (g_pwr_off_optimize_cfg.working_flag) {
	...
	g_pwr_off_optimize_cfg.lcd_vsn = &power_hdl->lcd_vsn;
	g_pwr_off_optimize_cfg.vci_vreg = &common_info->vci_vreg;
	g_pwr_off_optimize_cfg.iovcc_vreg = &common_info->iovcc_vreg;
	g_pwr_off_optimize_cfg.vdd_vreg = &common_info->vdd_vreg;
	g_pwr_off_optimize_cfg.vsn_vreg = &common_info->vsn_vreg;
	__pm_wakeup_event(g_pwr_off_optimize_cfg.pwr_off_optimize_wlock, MAX_WAKE_TIME);
	schedule_delayed_work(&g_pwr_off_optimize_cfg.pwr_off_optimize_work,
		msecs_to_jiffies(common_info->pwr_off_optimize_info.delay_time));
}
...
static void lcd_kit_register_power_off_optimize(void)
{
	...
	if (g_pwr_off_optimize_cfg.init_count) {
		LCD_KIT_INFO("power off work already init!\n");
		return;
	}
	...
	g_pwr_off_optimize_cfg.pwr_off_optimize_wlock = wakeup_source_register(NULL, "power_off_lock");
	g_pwr_off_optimize_cfg.init_count++;
...
int lcd_kit_panel_post_unprepare(struct dsi_panel *panel)
{
	...
	mutex_lock(&panel->panel_lock);
	lcd_kit_power_off_optimize_handle();
...
void wait_panel_initialized(void)
{
	reinit_completion(&lcd_panel_initialized);
	if (!wait_for_completion_timeout(&lcd_panel_initialized, msecs_to_jiffies(MAX_INIT_TIME)))
		LCD_KIT_ERR("wait panel init time out!\n");
}
...
static void lcd_kit_completion_init(void)
{
	...
	init_completion(&lcd_panel_initialized);
	completion_init_flag = true;
}

static void lcd_kit_init_panel_state(uint32_t panel_id)
{
	if ((panel_id == SECONDARY_PANEL) &&
		(lcd_kit_get_product_type() == LCD_DUAL_PANEL_NOT_SIM_TYPE))
		lcm_set_panel_state(panel_id, LCD_POWER_STATE_OFF);
	else
		lcm_set_panel_state(panel_id, LCD_POWER_STATE_ON);
}

static void lcd_dbg_init(void)
{
...
static void lcd_display_engine_on(uint32_t panel_id)
{
	display_engine_panel_on(panel_id);
}

void lcd_kit_probe_product_type(void)
{
...
uint32_t lcd_kit_get_panel_checkreg_state(uint32_t panel_id)
{
	if (panel_id == SECONDARY_PANEL) {
		return primary_panel_checkreg_state;
	} else {
		return secondary_panel_checkreg_state;
	}
	return primary_panel_checkreg_state;
}

void lcd_kit_set_panel_checkreg_state(uint32_t panel_id, uint32_t checkreg_state)
{
	if (panel_id == SECONDARY_PANEL) {
		primary_panel_checkreg_state = checkreg_state;
	} else {
		secondary_panel_checkreg_state = checkreg_state;
	}
	return;
}

static void lcd_kit_init_panel_checkreg_state(uint32_t panel_id)
{
	if (panel_id == SECONDARY_PANEL) {
		lcd_kit_set_panel_checkreg_state(SECONDARY_PANEL, PANEL_CHECKREG_FAIL_STATE);
	} else {
		lcd_kit_set_panel_checkreg_state(PRIMARY_PANEL, PANEL_CHECKREG_FAIL_STATE);
	}
	return;
}

uint32_t lcd_kit_get_panel_nve_state(void)
{
	return panel_nve_state;
}

void lcd_kit_set_panel_nve_state(uint32_t nve_state)
{
	panel_nve_state = nve_state;
	return;
}

static void lcd_kit_init_panel_nve_state(uint32_t panel_id)
{
	lcd_kit_set_panel_nve_state(PANEL_NV_INIT_STATE);
	return;
}

int lcd_kit_read_nv_info(struct qcom_panel_info *pinfo, int nv_num, int valid_size, char *nv_name, char *info)
{
	int ret;

	LCD_KIT_INFO("+\n");
	if (!nv_name) {
		LCD_KIT_ERR("lcd_kit_read_nv_info invalid arg \n");
		return -EINVAL;
	}
	memset(&pinfo->nv_info, 0, sizeof(pinfo->nv_info));
	pinfo->nv_info.nv_operation = NV_READ;
	pinfo->nv_info.nv_number = nv_num;
	pinfo->nv_info.valid_size = valid_size;
	strncpy(pinfo->nv_info.nv_name, nv_name, sizeof(pinfo->nv_info.nv_name));
	pinfo->nv_info.nv_name[sizeof(pinfo->nv_info.nv_name) - 1] = '\0';
	ret = hw_nve_direct_access(&pinfo->nv_info);
	if (ret != 0) {
		LCD_KIT_ERR("lcd_kit_read_nv_info : read lcd_kit info from nv %d fail\n", nv_num);
		return -1;
	}
	strncpy(info, pinfo->nv_info.nv_data, valid_size - 1);
	LCD_KIT_INFO("info is %s\n", info);
	LCD_KIT_INFO("-\n");
	return 0;
}

int lcd_kit_write_nv_info(struct qcom_panel_info *pinfo, int nv_num, int valid_size, char *nv_name, char *info)
{
	int ret;
	unsigned int len;
	...
	LCD_KIT_INFO("+\n");
	if (!nv_name || !info) {
		LCD_KIT_ERR("lcd_kit_write_nv_info invalid arg \n");
		return -EINVAL;
	}
	memset(&pinfo->nv_info, 0, sizeof(pinfo->nv_info));
	pinfo->nv_info.nv_operation = NV_WRITE;
	pinfo->nv_info.nv_number = nv_num;
	pinfo->nv_info.valid_size = valid_size;
	strncpy(pinfo->nv_info.nv_name, nv_name, sizeof(pinfo->nv_info.nv_name));
	pinfo->nv_info.nv_name[sizeof(pinfo->nv_info.nv_name) - 1] = '\0';

	len = sizeof(pinfo->nv_info.nv_data) < valid_size ? sizeof(pinfo->nv_info.nv_data) : valid_size;
	memcpy(pinfo->nv_info.nv_data, info, len);
	ret = hw_nve_direct_access(&pinfo->nv_info);
	if (ret != 0) {
		LCD_KIT_ERR("lcd_kit_write_nv_info : write lcd_kit info to nv %d fail\n", nv_num);
		return -1;
	}
	LCD_KIT_INFO("-\n");
	return 0;
...
void lcd_kit_modify_read_nv_data(char *data_nv0, char *data_nv1)
{
	uint32_t panel_id = lcd_get_active_panel_id();
	struct qcom_panel_info *pinfo = lcm_get_panel_info(panel_id);
	char read_nve_value[RW_NV_ARR_MAX] = {0};

	LCD_KIT_INFO("+\n");
	lcd_kit_set_panel_nve_state(PANEL_NV_SUCC_STATE);
	if (lcd_kit_read_nv_info(pinfo, PANEL_NV_NUM, sizeof(read_nve_value), "LCD_STA", read_nve_value))
		LCD_KIT_ERR("read nv info fail\n");
	LCD_KIT_INFO("read_nve_value = %s\n", read_nve_value);

	if (read_nve_value[0] < NVE_BUF1_EQUAL_ZERO)
		read_nve_value[0] = NVE_BUF1_EQUAL_ZERO;
	if (read_nve_value[0] < NVE_BUF0_EQUAL_MAX_NUM) {
		read_nve_value[0]++;
	} else {
		read_nve_value[0] = NVE_BUF0_EQUAL_MAX_NUM;
	}
	if (read_nve_value[0] == NVE_BUF0_EQUAL_MAX_NUM && panel_id == PRIMARY_PANEL)
		read_nve_value[1] = data_nv0[0];

	if (read_nve_value[0] == NVE_BUF0_EQUAL_MAX_NUM && panel_id == SECONDARY_PANEL)
		read_nve_value[1] = data_nv1[0];

	if (lcd_kit_write_nv_info(pinfo, PANEL_NV_NUM, sizeof(read_nve_value), "LCD_STA", read_nve_value))
...
void lcd_kit_daul_panel_display_handle(void)
{
	char buf_bit0 = NVE_BUF1_EQUAL_TWO;
	char buf_bit1 = NVE_BUF1_EQUAL_ONE;
	lcd_kit_modify_read_nv_data(&buf_bit0, &buf_bit1);
}

void lcd_kit_panel_ppdone_handle(void)
{
	char buf_bit0 = NVE_BUF1_EQUAL_ONE;
	char buf_bit1 = NVE_BUF1_EQUAL_TWO;
	lcd_kit_modify_read_nv_data(&buf_bit0, &buf_bit1);
}
...
int lcd_kit_init(struct dsi_panel *panel)
{
	...
	/* init panel state */
	lcd_kit_init_panel_state(panel_id);
	lcd_kit_set_thp_proximity_state(panel_id, POWER_ON);
	lcd_display_engine_on(panel_id);
	lcd_dbg_init();
	if (lcd_kit_get_product_type() == LCD_DUAL_PANEL_NOT_SIM_TYPE) {
		lcd_kit_init_panel_nve_state(panel_id);
		lcd_kit_init_panel_checkreg_state(panel_id);
	}
	LCD_KIT_INFO("exit\n");
	return ret;
}
	...
	struct drm_file *file_priv)
{
	...
	if (mmi_level == 0) {
		...
		display_engine_brightness_set_alpha_bypass(false, lcd_get_active_panel_id());
	} else {
		backlight_type = BACKLIGHT_HIGH_LEVEL;
		display_engine_brightness_set_alpha_bypass(true, lcd_get_active_panel_id());
	}
