...
#define lcdb_setting_func(_id, _sec_access, _valid)	\
	[_id] = {				\
		.address = _id##_REG,		\
...
static struct settings lcdb_settings_pm660l[] = {
	lcdb_setting_func(LCDB_BST_PD_CTL, false, true),
	lcdb_setting_func(LCDB_RDSON_MGMNT, false, true),
	lcdb_setting_func(LCDB_MISC_CTL, false, true),
	lcdb_setting_func(LCDB_SOFT_START_CTL, false, true),
	lcdb_setting_func(LCDB_PFM_CTL, false, true),
	lcdb_setting_func(LCDB_PWRUP_PWRDN_CTL, true, true),
	lcdb_setting_func(LCDB_LDO_PD_CTL, false, true),
	lcdb_setting_func(LCDB_LDO_SOFT_START_CTL, false, true),
	lcdb_setting_func(LCDB_NCP_PD_CTL, false, true),
	lcdb_setting_func(LCDB_NCP_SOFT_START_CTL, false, true),
	lcdb_setting_func(LCDB_BST_SS_CTL, false, false),
	lcdb_setting_func(LCDB_LDO_VREG_OK_CTL, false, false),
};

/* For PMICs like pmi632/pm8150L */
static struct settings lcdb_settings[] = {
	lcdb_setting_func(LCDB_BST_PD_CTL, false, true),
	lcdb_setting_func(LCDB_RDSON_MGMNT, false, false),
	lcdb_setting_func(LCDB_MISC_CTL, false, false),
	lcdb_setting_func(LCDB_SOFT_START_CTL, false, false),
	lcdb_setting_func(LCDB_PFM_CTL, false, false),
	lcdb_setting_func(LCDB_PWRUP_PWRDN_CTL, false, true),
	lcdb_setting_func(LCDB_LDO_PD_CTL, false, true),
	lcdb_setting_func(LCDB_LDO_SOFT_START_CTL, false, true),
	lcdb_setting_func(LCDB_NCP_PD_CTL, false, true),
	lcdb_setting_func(LCDB_NCP_SOFT_START_CTL, false, true),
	lcdb_setting_func(LCDB_BST_SS_CTL, false, true),
	lcdb_setting_func(LCDB_LDO_VREG_OK_CTL, false, true),
};
...
static struct settings lcdb_settings_pm7325b[] = {
	...
	lcdb_setting_func(LCDB_SOFT_START_CTL, false, false),
	lcdb_setting_func(LCDB_PFM_CTL, false, false),
	lcdb_setting_func(LCDB_PWRUP_PWRDN_CTL, false, false),
	lcdb_setting_func(LCDB_LDO_PD_CTL, false, true),
	lcdb_setting_func(LCDB_LDO_SOFT_START_CTL, false, true),
	lcdb_setting_func(LCDB_NCP_PD_CTL, false, true),
	lcdb_setting_func(LCDB_NCP_SOFT_START_CTL, false, true),
	lcdb_setting_func(LCDB_BST_SS_CTL, false, true),
	lcdb_setting_func(LCDB_LDO_VREG_OK_CTL, false, false),
	lcdb_setting_func(PM7325B_LCDB_BST_VREG_OK_CTL, false, true),
	lcdb_setting_func(LCDB_STEPPER_VOUT_CTL, false, true),
	lcdb_setting_func(LCDB_CONFIG_SEL, false, true),
	lcdb_setting_func(PM7325B_LCDB_WARMUP_DLY_SEL_1, false, true),
	lcdb_setting_func(PM7325B_LCDB_WARMUP_DLY_SEL_2, false, true),
	lcdb_setting_func(PM7325B_LCDB_PRECHARGE_CTL, false, true),
};
...
{
	int rc = 0;
	int len = (lcdb->subtype == PM7325B) ? 5 : 6;
	u8 sts[6] = {0};
...
static int qpnp_lcdb_ttw_enter(struct qpnp_lcdb *lcdb)
{
	...
	val = 0;
	rc = qpnp_lcdb_write(lcdb, lcdb->base + LCDB_NCP_SOFT_START_CTL_REG, &val, 1);
	if (rc < 0)
		return rc;

	val = 0;
	rc = qpnp_lcdb_write(lcdb, lcdb->base + LCDB_LDO_SOFT_START_CTL_REG, &val, 1);
	if (rc < 0)
		return rc;
	...
	val = 0;
	rc = qpnp_lcdb_write(lcdb, lcdb->base + LCDB_BST_VREG_OK_CTL_REG, &val, 1);
	if (rc < 0)
		return rc;

	val = BOOST_DIS_PULLDOWN_BIT;
	rc = qpnp_lcdb_write(lcdb, lcdb->base + LCDB_BST_PD_CTL_REG, &val, 1);
	if (rc < 0)
		return rc;

	val = LDO_DIS_PULLDOWN_BIT;
	rc = qpnp_lcdb_write(lcdb, lcdb->base + LCDB_LDO_PD_CTL_REG, &val, 1);
	if (rc < 0)
		return rc;

	val = NCP_DIS_PULLDOWN_BIT;
	rc = qpnp_lcdb_write(lcdb, lcdb->base + LCDB_NCP_PD_CTL_REG, &val, 1);
	if (rc < 0)
		return rc;

	val = HWEN_RDY_BIT;
	rc = qpnp_lcdb_write(lcdb, lcdb->base + LCDB_ENABLE_CTL1_REG, &val, 1);

	return rc;
}

static int qpnp_lcdb_ttw_enter_pm660l_sub1(struct qpnp_lcdb *lcdb)
{
	int rc = 0;
	u8 val = 0;
	...
	rc = qpnp_lcdb_write(lcdb, lcdb->base + LCDB_RDSON_MGMNT_REG, &val, 1);
	if (rc < 0) {
	...
	rc = qpnp_lcdb_write(lcdb, lcdb->base + LCDB_MISC_CTL_REG, &val, 1);
	if (rc < 0) {
	...
	val = 0;
	rc = qpnp_lcdb_write(lcdb, lcdb->base + LCDB_SOFT_START_CTL_REG, &val, 1);
	if (rc < 0) {
	...
	rc = qpnp_lcdb_write(lcdb, lcdb->base + LCDB_PFM_CTL_REG, &val, 1);
	if (rc < 0) {
...
static int qpnp_lcdb_ttw_enter_pm660l_sub2(struct qpnp_lcdb *lcdb)
{
	int rc = 0;
	u8 val = 0;

	val = 0;
	rc = qpnp_lcdb_secure_write(lcdb, lcdb->base + LCDB_PWRUP_PWRDN_CTL_REG, val);
	if (rc < 0) {
...
static int qpnp_lcdb_ttw_enter_pm660l(struct qpnp_lcdb *lcdb)
{
	...
	rc = qpnp_lcdb_ttw_enter_pm660l_sub1(lcdb);
	if (rc < 0)
		return rc;

	rc = qpnp_lcdb_ttw_enter_pm660l_sub2(lcdb);
	if (rc < 0)
		return rc;
...
static int qpnp_lcdb_ttw_enter_pm7325b_sub1(struct qpnp_lcdb *lcdb)
{
	int rc = 0;
	u8 val = 0;
	...
	val = 0;
	rc = qpnp_lcdb_write(lcdb, lcdb->base + PM7325B_LCDB_BST_VREG_OK_CTL_REG, &val, 1);
	if (rc < 0)
		return rc;

	val = 0;
	rc = qpnp_lcdb_write(lcdb, lcdb->base + PM7325B_LCDB_WARMUP_DLY_SEL_1_REG, &val, 1);
	if (rc < 0)
		return rc;

	val = 0;
	rc = qpnp_lcdb_write(lcdb, lcdb->base + PM7325B_LCDB_WARMUP_DLY_SEL_2_REG, &val, 1);
	if (rc < 0)
		return rc;

	val = 0;
	rc = qpnp_lcdb_write(lcdb, lcdb->base + PM7325B_LCDB_PRECHARGE_CTL_REG, &val, 1);
	if (rc < 0)
		return rc;
...
static int qpnp_lcdb_ttw_enter_pm7325b_sub2(struct qpnp_lcdb *lcdb)
{
	int rc = 0;
	u8 val = 0;
	...
	rc = qpnp_lcdb_write(lcdb, lcdb->base + LCDB_STEPPER_VOUT_CTL_REG, &val, 1);
	if (rc < 0)
		return rc;

	val = EN_FAST_STARTUP_BIT;
	rc = qpnp_lcdb_write(lcdb, lcdb->base + LCDB_CONFIG_SEL_REG, &val, 1);
	if (rc < 0)
		return rc;
...
static int qpnp_lcdb_ttw_enter_pm7325b(struct qpnp_lcdb *lcdb)
{
	...
	rc = qpnp_lcdb_ttw_enter_pm7325b_sub1(lcdb);
	if (rc < 0)
		return rc;

	rc = qpnp_lcdb_ttw_enter_pm7325b_sub2(lcdb);
	if (rc < 0)
		return rc;
...
static int qpnp_lcdb_enable_sub(struct qpnp_lcdb *lcdb)
{
	int rc = 0;
	int timeout = 0;
	int delay = 0;
	...
	/* poll for vreg_ok */
	timeout = 10;
	delay = lcdb->bst.soft_start_us + lcdb->ldo.soft_start_us + lcdb->ncp.soft_start_us;
	delay += lcdb->bst.vreg_ok_dbc_us + lcdb->ldo.vreg_ok_dbc_us + lcdb->ncp.vreg_ok_dbc_us;
	while (timeout--) {
	...
	if (lcdb->voltage_step_ramp) {
		...
		rc = qpnp_lcdb_set_voltage_step(lcdb, voltage_mv + VOLTAGE_STEP_MV, LDO_NCP);
		if (rc < 0) {
...
static irqreturn_t qpnp_lcdb_sc_irq_handler_sm660l(int irq, void *data)
{
...
static irqreturn_t qpnp_lcdb_sc_irq_handler_other(int irq, void *data)
{
	struct qpnp_lcdb *lcdb = data;
	int rc = 0;
	u8 val = 0;
...
static irqreturn_t qpnp_lcdb_sc_irq_handler(int irq, void *data)
{
	...
	if (lcdb->subtype == PM660L &&
		...
		qpnp_lcdb_sc_irq_handler_sm660l(irq, data);
	else
		qpnp_lcdb_sc_irq_handler_other(irq, data);

irq_handled:
	return IRQ_HANDLED;
}

static void qpnp_lcdb_set_bst_voltage_sub1(struct qpnp_lcdb *lcdb, int *bst_voltage_mv)
{
	switch (lcdb->subtype) {
	case PM660L:
...
static void qpnp_lcdb_set_bst_voltage_sub2(struct qpnp_lcdb *lcdb,
	u8 *mask, u8 *voltage_step, int *min_bst_voltage)
{
	switch (lcdb->subtype) {
	case PM660L:
					...
					int voltage_mv, u8 type)
{
	...
	qpnp_lcdb_set_bst_voltage_sub1(lcdb, &bst_voltage_mv);

	if (bst_voltage_mv != bst->voltage_mv) {
		qpnp_lcdb_set_bst_voltage_sub2(lcdb, &mask, &voltage_step, &min_bst_voltage);
		val = DIV_ROUND_UP(bst_voltage_mv - min_bst_voltage, voltage_step);
		rc = qpnp_lcdb_masked_write(lcdb, lcdb->base + LCDB_BST_OUTPUT_VOLTAGE_REG, mask, val);
		if (rc < 0) {
...
static int qpnp_lcdb_regulator_register_sub(struct qpnp_lcdb *lcdb, u8 type,
	struct device_node *node, struct regulator_desc *rdesc, struct regulator_dev *rdev)
{
	int rc = 0;
...
{
	int off_on_delay = 0;
	int voltage_step = VOLTAGE_STEP_50_MV;
	struct regulator_desc *rdesc;
	struct regulator_dev *rdev;
	struct device_node *node;
	...
	if (type == LDO) {
		...
		rdesc->n_voltages = ((lcdb->max_voltage_mv - lcdb->min_voltage_mv) / voltage_step) + 1;
		rdev			= lcdb->ldo.rdev;
	} else if (type == NCP) {
		node			= lcdb->ncp.node;
	...
	return qpnp_lcdb_regulator_register_sub(lcdb, type, node, rdesc, rdev);
}
...
static int _qpnp_lcdb_init_ldo_sub1(struct qpnp_lcdb *lcdb, u8 pd_mask, u8 pd_enable)
{
	int rc = 0;
...
static int _qpnp_lcdb_init_ldo_sub2(struct qpnp_lcdb *lcdb, u8 ilim_sd_shift, u8 ilim_ctl_reg, u8 ilim_mask)
{
	int rc = 0;
	int ilim_ma = 0;
	int i = 0;
	...
	if (lcdb->ldo.ilim_ma != -EINVAL) {
		if (lcdb->subtype == PM7325B) {
			...
			*/
			for (i = 0; i < ARRAY_SIZE(pm7325b_ldo_ilim_ma) && ilim_ma >= pm7325b_ldo_ilim_ma[i]; i++) {}
			val = (i == 0) ? 0 : i - 1;
		} else {
...
static int _qpnp_lcdb_init_ldo(struct qpnp_lcdb *lcdb)
{
	int rc = 0;
	u8 pd_mask;
	u8 pd_enable;
	...
	rc = _qpnp_lcdb_init_ldo_sub1(lcdb, pd_mask, pd_enable);
	if (rc < 0) {
		pr_err("_qpnp_lcdb_init_ldo_sub1 failed\n");
		return rc;
	}

	rc = _qpnp_lcdb_init_ldo_sub2(lcdb, ilim_sd_shift, ilim_ctl_reg, ilim_mask);
	if (rc < 0) {
		pr_err("_qpnp_lcdb_init_ldo_sub2 failed\n");
		return rc;
	}
...
static int qpnp_lcdb_init_ldo(struct qpnp_lcdb *lcdb)
{
	...
	rc = _qpnp_lcdb_init_ldo(lcdb);
	if (rc < 0) {
		pr_err("Failed _qpnp_lcdb_init_ldo rc=%d\n", rc);
		return rc;
	}
...
static int qpnp_lcdb_init_ncp_sub1(struct qpnp_lcdb *lcdb, u8 pd_enable)
{
	int rc = 0;
...
static int qpnp_lcdb_init_ncp_sub2(struct qpnp_lcdb *lcdb, u8 ilim_sd_shift, u8 ilim_ctl_reg, u8 ilim_mask, const u32 *ncp_ilim)
{
	int rc = 0;
	u8 val = 0;
	int i = 0;
...
static int qpnp_lcdb_init_ncp_sub3(struct qpnp_lcdb *lcdb, const u32 *dbc_ncp)
{
	int rc = 0;
	u8 val = 0;
...
static int qpnp_lcdb_init_ncp(struct qpnp_lcdb *lcdb)
{
	...
	/* configure parameters only if LCDB is disabled */
	if (!is_lcdb_enabled(lcdb)) {
		rc = qpnp_lcdb_init_ncp_sub1(lcdb, pd_enable);
		if (rc < 0) {
			pr_err("qpnp_lcdb_init_ncp_sub1 failed\n");
			return rc;
		}

		rc = qpnp_lcdb_init_ncp_sub2(lcdb, ilim_sd_shift, ilim_ctl_reg, ilim_mask, ncp_ilim);
		if (rc < 0) {
			pr_err("qpnp_lcdb_init_ncp_sub2 failed\n");
			return rc;
		}
	}

	rc = qpnp_lcdb_init_ncp_sub3(lcdb, dbc_ncp);
	if (rc < 0) {
		pr_err("qpnp_lcdb_init_ncp_sub3 failed\n");
		return rc;
	}
...
static int qpnp_lcdb_init_bst_lcdb_sub(struct qpnp_lcdb *lcdb, u8 bst_ilim_en)
{
	int rc = 0;
	u8 val = 0;

	if (lcdb->bst.pd_strength != -EINVAL) {
		rc = qpnp_lcdb_masked_write(lcdb, lcdb->base + LCDB_BST_PD_CTL_REG, BOOST_PD_STRENGTH_BIT, lcdb->bst.pd_strength ? BOOST_PD_STRENGTH_BIT : 0);
		if (rc < 0) {
			pr_err("Failed to configure NCP PD strength %s rc=%d\n", lcdb->bst.pd_strength ? "(strong)" : "(weak)", rc);
			return rc;
		}
	}
	...
	if (lcdb->bst.ilim_ma != -EINVAL) {
		...
		rc = qpnp_lcdb_masked_write(lcdb, lcdb->base + LCDB_BST_ILIM_CTL_REG, SET_BST_ILIM_MASK | bst_ilim_en, val);
		if (rc < 0) {
	...
	if (lcdb->bst.ps != -EINVAL) {
		rc = qpnp_lcdb_masked_write(lcdb, lcdb->base + LCDB_PS_CTL_REG, EN_PS_BIT, lcdb->bst.ps ? EN_PS_BIT : 0);
		if (rc < 0) {
...
static int qpnp_lcdb_init_bst_lcdb(struct qpnp_lcdb *lcdb, u8 *bst_vreg_ok_reg)
{
	int rc = 0;
	int bst_ps_min;
	int bst_ps_step;
	...
	if (lcdb->bst.pd != -EINVAL) {
		rc = qpnp_lcdb_masked_write(lcdb, lcdb->base + LCDB_BST_PD_CTL_REG, pd_mask, lcdb->bst.pd ? pd_enable : ~pd_enable);
		if (rc < 0) {
	...
	qpnp_lcdb_init_bst_lcdb_sub(lcdb, bst_ilim_en);

	if (lcdb->bst.ps_threshold != -EINVAL) {
		mask = (lcdb->subtype == PM660L) ? PM660_PS_THRESH_MASK : PS_THRESH_MASK;
		val = (lcdb->bst.ps_threshold - bst_ps_min) / bst_ps_step;
		val = (val & mask) | EN_PS_BIT;
		rc = qpnp_lcdb_masked_write(lcdb, lcdb->base + LCDB_PS_CTL_REG, mask | EN_PS_BIT, val);
		if (rc < 0) {
...
static int qpnp_lcdb_init_bst(struct qpnp_lcdb *lcdb)
{
	...
	rc = qpnp_lcdb_init_bst_lcdb(lcdb, &bst_vreg_ok_reg);
	if (rc < 0)
		return rc;
	...
	if (lcdb->subtype == PM660L) {
		rc = qpnp_lcdb_read(lcdb, lcdb->base + LCDB_SOFT_START_CTL_REG, &val, 1);
		if (rc < 0) {
		...
		rc = qpnp_lcdb_read(lcdb, lcdb->base + LCDB_BST_SS_CTL_REG, &val, 1);
		if (rc < 0) {
...
static int qpnp_lcdb_hw_init_sub1(struct qpnp_lcdb *lcdb)
{
	int rc = 0;
...
static int qpnp_lcdb_hw_init_sub2(struct qpnp_lcdb *lcdb)
{
	int rc = 0;
...
static int qpnp_lcdb_hw_init_regulator(struct qpnp_lcdb *lcdb)
{
	int rc = 0;
...
static int qpnp_lcdb_hw_init(struct qpnp_lcdb *lcdb)
{
	...
	rc = qpnp_lcdb_hw_init_sub1(lcdb);
	if (rc < 0)
		return rc;

	rc = qpnp_lcdb_hw_init_sub2(lcdb);
	if (rc < 0)
		return rc;

	rc = qpnp_lcdb_hw_init_regulator(lcdb);
	if (rc < 0)
		return rc;
...
static int qpnp_lcdb_parse_dt_sub1(struct qpnp_lcdb *lcdb, struct device_node *node)
{
	int rc = 0;
	const char *label;
...
static int qpnp_lcdb_parse_dt_sub2(struct qpnp_lcdb *lcdb, struct device_node *node)
{
	int rc = 0;
	u32 tmp;
	...
	if (!rc) {
		...
		if (rc < 0) {
			pr_err("qcom,pwrdn-delay-ms failed\n");
			return rc;
		}
	}
	...
	if (!rc) {
		...
		if (rc < 0) {
			pr_err("qcom,pwrup-delay-ms failed\n");
			return rc;
		}
	}
	...
	if (!rc) {
		...
		if (rc < 0) {
			pr_err("qcom,high-p2-blank-time-ns failed\n");
			return rc;
		}
	}
	...
	if (!rc) {
		...
		if (rc < 0) {
			pr_err("qcom,low-p2-blank-time-ns failed\n");
			return rc;
		}
	}
	...
	if (!rc) {
		...
		if (rc < 0) {
			pr_err("qcom,mpc-current-thr-ma failed\n");
			return rc;
		}
	}
	return 0;
...
static int qpnp_lcdb_parse_dt(struct qpnp_lcdb *lcdb)
{
	...
	rc = qpnp_lcdb_parse_dt_sub1(lcdb, node);
	if (rc < 0) {
		pr_err("qpnp_lcdb_parse_dt_sub1 failed\n");
		return rc;
	}
	...
	rc = qpnp_lcdb_parse_dt_sub2(lcdb, node);
	if (rc < 0) {
		pr_err("qpnp_lcdb_parse_dt_sub2 failed\n");
		return rc;
	}
...
#define LCDB_START_VOLTAGE_UV 6000000
#define LCDB_LDO "lcdb_ldo"
#define LCDB_NCP "lcdb_ncp"

static int lcdb_inner_set_bias_power_down(int vpos, int vneg)
{
	struct regulator *bias_reg = NULL;

	pr_info("vpos=%d,vneg=%d\n", vpos, vneg);

	bias_reg = regulator_get(NULL, LCDB_LDO);
	if (bias_reg != NULL) {
		regulator_disable(bias_reg);
	} else {
		pr_err("failed to set ldo vreg down\n");
	}

	bias_reg = regulator_get(NULL, LCDB_NCP);
	if (bias_reg != NULL) {
		regulator_disable(bias_reg);
	} else {
		pr_err("failed to set ncp vreg down\n");
	}

	return LCD_KIT_OK;
}

static int lcdb_inner_set_bias(int vpos, int vneg)
{
	struct regulator *bias_reg = NULL;
	...
	bias_reg = regulator_get(NULL, LCDB_LDO);
	if (bias_reg != NULL) {
		regulator_set_voltage(bias_reg, vpos, vpos);
		regulator_enable(bias_reg);
	} else {
		pr_err("failed to set ldo vreg on\n");
	}

	bias_reg = regulator_get(NULL, LCDB_NCP);
	if (bias_reg != NULL) {
		regulator_set_voltage(bias_reg, vneg, vneg);
		regulator_enable(bias_reg);
	} else {
		pr_err("failed to set ncp vreg on\n");
	}

	return LCD_KIT_OK;
}

static struct lcd_kit_bias_ops lcdb_inner_bias_ops = {
	.set_bias_voltage = lcdb_inner_set_bias,
	.set_bias_power_down = lcdb_inner_set_bias_power_down,
};
#endif

static int qpnp_lcdb_regulator_probe_sub(struct platform_device *pdev, struct qpnp_lcdb *lcdb, struct device_node *node)
{
	int rc = 0;
	...
	lcdb->subtype = (enum pmic_type)of_device_get_match_data(&pdev->dev);
	lcdb->dev = &pdev->dev;
	lcdb->pdev = pdev;
...
static int qpnp_lcdb_regulator_probe(struct platform_device *pdev)
{
	...
	rc = qpnp_lcdb_regulator_probe_sub(pdev, lcdb, node);
	if (rc < 0) {
		pr_err("probe init failed\n");
		return rc;
	}

	dev_id = of_match_device(lcdb->dev->driver->of_match_table, lcdb->dev);

	lcdb->min_voltage_mv = (lcdb->subtype == PM7325B) ? PM7325B_MIN_VOLTAGE_MV : MIN_VOLTAGE_MV;
	lcdb->max_voltage_mv = (lcdb->subtype == PM7325B) ? PM7325B_MAX_VOLTAGE_MV : MAX_VOLTAGE_MV;
	...
	if (rc < 0) {
		...
		pr_info("LCDB module: %s successfully registered!\n", dev_id->compatible);
	}

#ifdef CONFIG_LCD_KIT_DRIVER
	lcdb_inner_set_bias(LCDB_START_VOLTAGE_UV, LCDB_START_VOLTAGE_UV);
	lcd_kit_bias_register(&lcdb_inner_bias_ops);
#endif
	return rc;
}
...
int __init qpnp_lcdb_regulator_init(void)
{
	return platform_driver_register(&qpnp_lcdb_regulator_driver);
}

void __exit qpnp_lcdb_regulator_exit(void)
{
