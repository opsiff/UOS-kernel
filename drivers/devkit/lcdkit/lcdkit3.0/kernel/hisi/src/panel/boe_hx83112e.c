	...
	struct lcd_kit_dsi_panel_cmds *rgbw_cmds)
{
	...
	static uint32_t sat_ctl_old, frm_gain_old, clr_old;
	static uint32_t pxl_gain_old, pwm_duty_old, rgbw_total_old;
	int ret = LCD_KIT_OK;
	...
	if ((sat_ctl_old != sat_ctl) || (frm_gain_old != frm_gain) ||
		...
		LCD_KIT_DEBUG("[RGBW] sat_ctl=%d, sat_ctl_old=%d!\n",
			sat_ctl, sat_ctl_old);
		ret = lcd_kit_dsi_cmds_tx(LCD_ACTIVE_PANEL, hisifd, rgbw_cmds);
	}
	sat_ctl_old = sat_ctl;
