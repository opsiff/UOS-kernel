...
#else
static void lcd_kit_timerout_function(struct timer_list *timer)
{
#endif
...
#else
		timer_setup(backlight_timer, lcd_kit_timerout_function, 0);
#endif
...
static int boe_r66451_set_vss_by_thermal(void *hld)
{
	...
	bl_level = (int)common_info->set_vss.new_backlight;
