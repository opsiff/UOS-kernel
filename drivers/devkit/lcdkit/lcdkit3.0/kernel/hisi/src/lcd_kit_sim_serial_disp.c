	...
	struct callback_data *data)
{
	...
	if (hisifd->index == LCD_EXT_PANEL && common_ops->panel_power_on &&
		...
		common_ops->panel_power_on(LCD_ACTIVE_PANEL, (void *)hisifd);
	...
	struct callback_data *data)
{
	...
	if (hisifd->index == LCD_EXT_PANEL && common_ops->panel_power_off &&
		hisifd->aod_function == 0) {
		lcd_kit_activate_panel_mutex(LCD_MAIN_PANEL);
		common_ops->panel_power_off(LCD_ACTIVE_PANEL, (void *)hisifd);
		lcd_kit_deactivate_panel_mutex();
	}
	g_pwr_ref_count = 0;
	return LCD_KIT_OK;
	...
	uint32_t func_id, struct callback_data *data)
{
	...
	if (index >= MAX_SIM_SERIAL_PANEL) {
		LCD_KIT_ERR("g_sim_serial_panel has out-of-bounds access risks\n");
		return LCD_KIT_FAIL;
	}
...
static int lcd_kit_sim_serial_set_fastboot(struct platform_device* pdev)
{
	uint32_t index = FB_INDEX_MAX;
	int ret = LCD_KIT_OK;
	struct callback_data cb_data = { LCD_KIT_FUNC_SET_FASTBOOT, NULL, NULL };
	index = lcd_kit_get_hisifd_index(pdev);
...
static int lcd_kit_sim_serial_on(struct platform_device* pdev)
{
	uint32_t index = FB_INDEX_MAX;
	int ret = LCD_KIT_OK;
	struct callback_data cb_data = { LCD_KIT_FUNC_ON, NULL, NULL };
	index = lcd_kit_get_hisifd_index(pdev);
	if (index >= FB_INDEX_MAX) {
...
static int lcd_kit_sim_serial_off(struct platform_device* pdev)
{
	uint32_t index = FB_INDEX_MAX;
	int ret = LCD_KIT_OK;
	struct callback_data cb_data = { LCD_KIT_FUNC_OFF, NULL, NULL };
	index = lcd_kit_get_hisifd_index(pdev);
	if (index >= FB_INDEX_MAX) {
	...
	uint32_t bl_level)
{
	uint32_t index = FB_INDEX_MAX;
	int ret = LCD_KIT_OK;
	struct callback_data cb_data = { LCD_KIT_FUNC_SET_BACKLIGHT, NULL, &bl_level };
	uint32_t cali_bl;
	index = lcd_kit_get_hisifd_index(pdev);
	if (index >= FB_INDEX_MAX) {
	...
	LCD_KIT_INFO("fb%d set original backlight %d\n", index, bl_level);
	cali_bl = lcd_kit_sim_serial_cali_backlight(PRIMARY_PANEL_IDX, bl_level);
	cb_data.spi_bl_cali = &cali_bl;
	...
	uint32_t mode)
{
	...
	LCD_KIT_INFO("fb%d aod alpm setting %u\n", hisifd->index, mode);
	lcd_kit_deactivate_panel_mutex();
	return LCD_KIT_OK;
}
