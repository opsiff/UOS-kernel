	...
	struct platform_device *pdev, uint32_t panel_id)
{
	if ((PUBLIC_CFG->product_type != LCD_FOLDER_TYPE &&
		PUBLIC_CFG->product_type != LCD_DUAL_PENEL_TYPE) ||
...
void lcd_kit_power_off_optimize_handle(struct hisi_fb_data_type *hisifd)
{
	...
	if ((pinfo->product_type != LCD_FOLDER_TYPE) && (pinfo->product_type != LCD_DUAL_PENEL_TYPE))
		return;
	if ((pinfo->product_type == LCD_FOLDER_TYPE) && (pinfo->skip_power_on_off != FOLD_POWER_ON_OFF))
		return;
	if ((pinfo->product_type == LCD_DUAL_PENEL_TYPE) && !lcd_kit_get_skip_power_on_off()) {
	...
	g_pwr_off_optimize_cfg.panel_id = panel_id;
...
static int lcd_kit_ext_panel_probe(struct platform_device *pdev)
{
	...
	if ((PUBLIC_CFG->product_type == LCD_DUAL_PENEL_TYPE) &&
		hisi_fb_device_probe_defer(PANEL_MEDIACOMMON, 0)) {
		LCD_KIT_INFO("hisi_fb_device_probe_defer!\n");
	...
	/* probe driver */
	if ((PUBLIC_CFG->product_type != LCD_DUAL_PENEL_TYPE) &&
		hisi_fb_device_probe_defer(pinfo->type, pinfo->bl_set_type))
		goto err_probe_defer;
	if (lcd_kit_need_add_device(LCD_EXT_PANEL)) {
		pdev->id = 1; /* dev id should be 1 */
		if (PUBLIC_CFG->product_type == LCD_DUAL_PENEL_TYPE)
			pdev->id = 2; /* dev id should be 2 */
