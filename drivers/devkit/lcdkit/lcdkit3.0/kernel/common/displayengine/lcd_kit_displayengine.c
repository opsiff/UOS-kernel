...
static int display_engine_local_hbm_v1_set_circle(uint32_t panel_id, void *hld, bool is_on);
static int display_engine_local_hbm_v1_set_circle_color(uint32_t panel_id, int grayscale);
static bool display_engine_local_hbm_version_valid(uint32_t panel_id);
static int display_engine_local_hbm_set_color_by_grayscale_default(uint32_t panel_id, void *hld,
	uint32_t fps, int grayscale);
static void display_engine_local_hbm_set_circle_alpha_dbv_default(uint32_t panel_id, void *hld);
...
static struct display_engine_context g_de_context[DISPLAY_ENGINE_PANEL_COUNT] = {
	{ /* Inner panel or primay panel */
		...
		.brightness.power_mode = SDE_MODE_DPMS_ON,
...
static int display_engine_panel_id_to_display_count(uint32_t de_id)
{
	switch (de_id) {
	case DISPLAY_ENGINE_PANEL_INNER:
		return 0;
...
static void display_engine_brightness_update_backlight(void)
{
	...
	int display_count = display_engine_panel_id_to_display_count(panel_id);

	/* 0 refers to the first display */
	bd = backlight_device_get_by_type_and_display_count(BACKLIGHT_RAW, display_count);
	if (!bd) {
...
void display_engine_brightness_set_alpha_bypass(bool is_bypass, uint32_t panel_id)
{
	...
	LCD_KIT_INFO("panel_id=%u: brightness.is_alpha_bypass [%u] -> [%u]\n",
		panel_id, de_context->brightness.is_alpha_bypass, is_bypass);
...
static int get_mapped_alpha_inner(int bl_level, uint32_t panel_id)
{
	struct display_engine_context *de_context = get_de_context(panel_id);
...
static int get_mapped_level_handle_mode(
	int in_level, struct display_engine_context *de_context, uint32_t panel_id)
{
	...
	if (mode_in_use >= DISPLAY_ENGINE_MODE_MAX) {
		LCD_KIT_ERR("panel_id[%u], mode_in_use out of range, mode_in_use = %d\n",
		    panel_id, mode_in_use);
		goto error_out;
	}
	if (in_level == 0) {
		de_context->brightness.mode_in_use = DISPLAY_ENGINE_BRIGHTNESS_MODE_DEFAULT;
		LCD_KIT_DEBUG("panel_id[%u], in_level is 0, need sync later, mdoe = %d\n",
		    panel_id, mode_in_use);
		goto error_out;
	}

	LCD_KIT_DEBUG("panel_id[%u], in_level[%d], mode_in_use[%u]\n", panel_id, in_level, mode_in_use);
	switch (mode_in_use) {
	case DISPLAY_ENGINE_BRIGHTNESS_MODE_DEFAULT:
		...
		if (is_brightness_need_sync(
			...
			ALPHA_DEFAULT, get_mapped_alpha_inner(in_level, panel_id))) {
			/*
...
int display_engine_brightness_get_mapped_level(int in_level, uint32_t panel_id)
{
	...
	return get_mapped_level_handle_mode(in_level, de_context, panel_id);
...
int display_engine_brightness_get_mapped_alpha(uint32_t panel_id)
{
	...
	alpha = get_mapped_alpha_inner(de_context->brightness.last_level_in, panel_id);
	de_context->brightness.last_alpha = de_context->brightness.current_alpha;
...
static int display_engine_brightness_set_mode(uint32_t panel_id, uint32_t mode)
{
	...
	LCD_KIT_INFO("panel_id[%d] brightness.mode_set_by_ioctl [%d] -> [%d]\n",
		panel_id, de_context->brightness.mode_set_by_ioctl, mode);
...
static void brightness_handle_sync(struct qcom_panel_info *panel_info,
	struct display_engine_context *de_context, uint32_t lcd_panel_id)
{
	...
	sync_delay_time_us = get_backlight_sync_delay_time_us(
		panel_info, de_context, lcd_panel_id);
...
void display_engine_brightness_handle_mode_change(uint32_t panel_id)
{
	...
	/* need sync */
	brightness_handle_sync(panel_info, de_context, lcd_panel_id);
}
...
#endif
	info->oled_type = lcd_common_info->oled_type;
	LCD_KIT_INFO("panel_id=%u res(w=%u,h=%u) lum(min=%u,max=%u) bl(min=%u,max=%u) name=%s oled_type=%d\n",
		panel_id, info->width, info->height, info->max_luminance, info->min_luminance,
		info->max_backlight, info->min_backlight, info->panel_name, info->oled_type);
	return LCD_KIT_OK;
}
	...
	struct display_engine_roi_param roi_param)
{
	...
	if (!common_info->ddic_bic.roi_cmds.cmds) {
		LCD_KIT_WARNING("DE_FC ddic_bic.roi_cmds is NULL\n");
		return LCD_KIT_FAIL;
	}
	...
	uint16_t dbi_weight[DE_COLOR_INDEX_COUNT][DDIC_COMPENSATION_LUT_COUNT])
{
	...
	if (!common_info->ddic_bic.weight_cmds.cmds) {
		LCD_KIT_WARNING("DE_FC ddic_bic.weight_cmds is NULL\n");
		return LCD_KIT_FAIL;
	}
...
void display_engine_init(uint32_t panel_id)
{
	struct display_engine_context *de_context =
		get_de_context(display_engine_panel_id_to_de(panel_id));
	struct ud_fp_display_ops finger_print_callback = {NULL, NULL};
...
static bool display_engine_local_hbm_version_valid(uint32_t panel_id)
{
	if (common_info->hbm.local_hbm_version < VERSION_DEFAULT ||
		common_info->hbm.local_hbm_version >= VERSION_NUM) {
		LCD_KIT_ERR("local hbm version error\n");
		return false;
	}
	return true;
}
	...
	uint32_t fps, int grayscale)
{
	...
	if (!display_engine_local_hbm_version_valid(panel_id))
		return LCD_KIT_FAIL;
	switch (common_info->hbm.local_hbm_version) {
	case VERSION_DEFAULT:
		ret = display_engine_local_hbm_set_color_by_grayscale_default(panel_id, hld,
			fps, grayscale);
		if (ret != LCD_KIT_OK) {
			LCD_KIT_ERR("set circle color version default error\n");
			return LCD_KIT_FAIL;
		}
		break;
	case VERSION_V1:
		ret = display_engine_local_hbm_v1_set_circle_color(panel_id, grayscale);
		if (ret != LCD_KIT_OK) {
			LCD_KIT_ERR("set circle color version1 error\n");
			return LCD_KIT_FAIL;
		}
		break;
	default:
		LCD_KIT_INFO("this local hbm version cannot support\n");
		return LCD_KIT_FAIL;
	}
	...
	struct display_engine_param *param)
{
	...
	if (!param->local_hbm.test.is_local_hbm_on) {
		...
		display_engine_local_hbm_set_dbv(panel_id, hld,
			display_engine_brightness_get_mipi_level(panel_id));
	} else {
		if (grayscale < 0) {
...
static void display_engine_set_amoled_param(struct display_engine_param *param, bool *is_fail)
{
	...
	if (param->modules & DISPLAY_ENGINE_DDIC_OD) {
		LCD_KIT_INFO("DISPLAY_ENGINE_DDIC_OD %d for panel %d! +\n",
			param->ddic_od_enable, param->panel_id);
		lcd_kit_get_common_ops()->set_ddic_cmd(param->panel_id,
			param->ddic_od_enable ? DSI_CMD_SET_OD_ON : DSI_CMD_SET_OD_OFF,
			LCD_KIT_DDIC_CMD_ASYNC);
		LCD_KIT_INFO("DISPLAY_ENGINE_DDIC_OD %d for panel %d! -\n",
			param->ddic_od_enable, param->panel_id);
	}
	if (param->modules & DISPLAY_ENGINE_DDIC_IRC) {
		LCD_KIT_INFO("DISPLAY_ENGINE_DDIC_IRC %d for panel %d! +\n",
			param->ddic_irc_enable, param->panel_id);
		lcd_kit_get_common_ops()->set_ddic_cmd(param->panel_id,
			param->ddic_irc_enable ? DSI_CMD_SET_IRC_ON : DSI_CMD_SET_IRC_OFF,
			LCD_KIT_DDIC_CMD_ASYNC);
		LCD_KIT_INFO("DISPLAY_ENGINE_DDIC_IRC %d for panel %d! -\n",
			param->ddic_irc_enable, param->panel_id);
	}
...
int display_engine_set_param(struct drm_device *dev, void *data, struct drm_file *file_priv)
{
	...
	display_engine_set_amoled_param(param, &is_fail);
...
static void display_engine_local_hbm_set_circle_alpha_dbv(uint32_t panel_id, void *hld)
{
	...
	switch (common_info->hbm.local_hbm_version) {
	case VERSION_DEFAULT:
		display_engine_local_hbm_set_circle_alpha_dbv_default(panel_id, hld);
		break;
	case VERSION_V1:
		display_engine_local_hbm_v1_set_circle(panel_id, hld, true);
		break;
	default:
		LCD_KIT_INFO("this local hbm version cannot support\n");
		break;
...
static void display_engine_local_hbm_set_circle_alpha_dbv_default(uint32_t panel_id, void *hld)
{
	...
	dbv = display_engine_brightness_get_mipi_level(panel_id);
...
static void display_engine_local_hbm_exit_circle_alpha_dbv(uint32_t panel_id, void *hld)
{
	...
	switch (common_info->hbm.local_hbm_version) {
	case VERSION_DEFAULT:
		display_engine_local_hbm_set_circle(panel_id,
			hld, false, LCD_KIT_ALPHA_DEFAULT);
		display_engine_alpha_set_inner(panel_id,
			hld, LCD_KIT_ALPHA_DEFAULT, FORCE_DISABLE);
		display_engine_local_hbm_set_dbv(panel_id, hld,
			display_engine_brightness_get_mipi_level(panel_id));
		break;
	case VERSION_V1:
		display_engine_local_hbm_v1_set_circle(panel_id, hld, false);
		break;
	default:
		LCD_KIT_INFO("this local hbm version cannot support\n");
		return;
...
static int display_engine_local_hbm_v1_set_circle(uint32_t panel_id, void *hld, bool is_on)
{
	...
	dbv = display_engine_brightness_get_mipi_level(panel_id);
	threshold = common_info->hbm.local_hbm_v1_hbm_dbv_threshold;
	LCD_KIT_INFO("dbv:%d, threshold:%d\n", dbv, threshold);
	if (common_info->hbm.circle_exit_first_cmds.cmds == NULL) {
		LCD_KIT_ERR("circle_exit_first_cmds.cmds is NULL\n");
		return LCD_KIT_FAIL;
	}
	if (common_info->hbm.circle_exit_second_cmds.cmds == NULL) {
	...
	if (common_info->hbm.circle_enter_second_cmds.cmds == NULL) {
		LCD_KIT_ERR("circle_enter_second_cmds.cmds is NULL\n");
		return LCD_KIT_FAIL;
	}
	if (!is_on) {
		ret = adapt_ops->mipi_tx(hld, &common_info->hbm.circle_exit_second_cmds);
	} else {
		ret = adapt_ops->mipi_tx(hld, &common_info->hbm.circle_enter_second_cmds);
	}
	return ret;
}
...
static int display_engine_local_hbm_v1_set_circle_color(uint32_t panel_id, int grayscale)
{
	/* circle_color_cmds.cmd_cnt's maximum value is 4 */
	if (common_info->hbm.circle_color_cmds.cmd_cnt != 4) {
		LCD_KIT_ERR("circle_color_cmds.cmd_cnt does not match\n");
		return LCD_KIT_FAIL;
	}

	/* circle_color_cmds.cmds[r|g|b].dlen maximum value is 2 */
	if ((int)common_info->hbm.circle_color_cmds.cmds[R_INDEX].dlen < 2 ||
		(int)common_info->hbm.circle_color_cmds.cmds[G_INDEX].dlen < 2 ||
		(int)common_info->hbm.circle_color_cmds.cmds[B_INDEX].dlen < 2) {
		LCD_KIT_ERR("array index out of range, cmds.dlen\n");
		return LCD_KIT_FAIL;
	}

	/* circle_color_cmds.cmds[r|g|b].payload value index is 1 */
	common_info->hbm.circle_color_cmds.cmds[R_INDEX].payload[1] = grayscale;
	common_info->hbm.circle_color_cmds.cmds[G_INDEX].payload[1] = grayscale;
	common_info->hbm.circle_color_cmds.cmds[B_INDEX].payload[1] = grayscale;
	return LCD_KIT_OK;
}
...
static int display_engine_local_hbm_set_inner(int grayscale)
{
	...
	if (panel_info == NULL || panel_info->display == NULL) {
		LCD_KIT_ERR("panel_info is NULL\n");
		return LCD_KIT_FAIL;
	}
	...
	if (!display_engine_local_hbm_version_valid(panel_id)) {
		LCD_KIT_ERR("this local hbm version cannot support\n");
		return LCD_KIT_FAIL;
	}
	if (display_engine_get_alpha_support(panel_id) == 0 &&
		common_info->hbm.local_hbm_version == VERSION_DEFAULT) {
		LCD_KIT_ERR("local_hbm_support is 0\n");
		return LCD_KIT_FAIL;
	}
	if (grayscale > 0) {
		notify_local_hbm_event(panel_info->display->drm_dev->dev_private, 1);
		...
		notify_local_hbm_event(panel_info->display->drm_dev->dev_private, 0);
	}
	return LCD_KIT_OK;
}
...
void display_engine_handle_power_mode_enter(int mode)
{
	...
	if (display_engine_get_power_mode() == SDE_MODE_DPMS_ON && mode != SDE_MODE_DPMS_ON) {
		de_context->brightness.local_hbm.current_grayscale = 0;
		ud_fp_notify_lhbm_interrupted_by_power();
