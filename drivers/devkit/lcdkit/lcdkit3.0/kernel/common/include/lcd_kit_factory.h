...
#define TEST_LOOP_0	0
#define TEST_LOOP_1	1
#define TEST_LOOP_2	2
#define TEST_LOOP_3	3
...
#define LCD_KIT_CHECKSUM_SIZE_DKMD 32
...
struct lcd_kit_checksum {
	...
	struct lcd_kit_arrays_data sub_value;
...
struct check_sum2_detect_cmd {
	...
	u32 threshold;
	u32 delay;
	u32 time[CHECK_SUM2_NUM];
	u32 model_switch;
	u8 reg_val[CHECK_SUM2_NUM][CHECKSUM2_VAL_NUM];
	struct lcd_kit_array_data value[CHECK_SUM2_NUM];
	struct lcd_kit_dsi_panel_cmds rd_cmds[CHECK_SUM2_NUM];
	struct lcd_kit_dsi_panel_cmds enter_cmds[CHECK_SUM2_NUM];
};
...
struct lcd_fact_ops {
	...
	ssize_t (*mbist_detect_store)(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count);
	ssize_t (*dynamic_volt_show)(struct device *dev,
		struct device_attribute *attr, char *buf);
	ssize_t (*dynamic_volt_store)(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count);
	ssize_t (*pmic_detect_show)(struct device *dev,
		struct device_attribute *attr, char *buf);
};
...
#define VOLT_MODE_NUM 3 // VOLT_HIGHT, VOLT_LOW, VOLT_NOMAL
...
struct lcd_dynamic_volt {
	u32 support;
	int vdd_vol[VOLT_MODE_NUM];
	int iovcc_vol[VOLT_MODE_NUM];
	int vci_vol[VOLT_MODE_NUM];
};

struct lcd_pmic_detect {
	u32 support;
};
...
struct lcd_fact_desc {
	...
	/* dynamic modify voltage */
	struct lcd_dynamic_volt dynamic_volt;
	/* pmic 2.0 detect */
	struct lcd_pmic_detect pmic_detect;
};
