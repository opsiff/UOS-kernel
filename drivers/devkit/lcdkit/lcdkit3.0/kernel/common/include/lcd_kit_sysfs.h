...
enum lcd_kit_sysfs_index {
	...
	LCD_DIMMING_INDEX,
	SEC_LCD_MODEL_INDEX,
	LCD_AVG_ON_TIME_INDEX,
	TCON_UPDATE_INDEX,
	TCON_UPDATE_INFO_INDEX,
};
/* sysfs support enum */
...
struct lcd_kit_sysfs_ops {
	...
	ssize_t (*panel_dimming_switch_store)(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count);
	ssize_t (*avg_on_time_show)(struct device *dev,
		struct device_attribute *attr, char *buf);
	ssize_t (*tcon_update_store)(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count);
	ssize_t (*tcon_update_show)(struct device *dev,
		struct device_attribute *attr, char *buf);
	ssize_t (*tcon_update_info_show)(struct device *dev,
		struct device_attribute *attr, char *buf);
};
#endif
