...
int lcd_kit_parse_array_data(const struct device_node *np, const char *name,
...
int lcd_kit_parse_dcs_cmds(const struct device_node *np, char *cmd_key,
...
unsigned int lcd_kit_parse_u32_val(const struct device_node *np, const char *prop_name,
	unsigned int def)
{
	unsigned int temp = 0;
	...
	LCD_KIT_INFO("of_property_read: %s value:%u \n", prop_name, temp);
	return temp;
}
