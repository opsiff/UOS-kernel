	...
	struct device_attribute *attr, char *buf)
{
	...
	mutex_lock(&COMMON_LOCK->sysfs_lock.avg_on_lock);
	if (sysfs_ops->avg_on_time_show)
		ret = sysfs_ops->avg_on_time_show(dev, attr, buf);
	mutex_unlock(&COMMON_LOCK->sysfs_lock.avg_on_lock);
	return ret;
}
...
static ssize_t lcd_kit_tcon_update_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	struct lcd_kit_sysfs_ops *sysfs_ops = NULL;
	...
	if (!sysfs_ops->tcon_update_show) {
		LCD_KIT_ERR("tcon_update_show is NULL!\n");
		return LCD_KIT_FAIL;
	}

	return sysfs_ops->tcon_update_show(dev, attr, buf);
}

static ssize_t lcd_kit_tcon_update_store(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
	struct lcd_kit_sysfs_ops *sysfs_ops = NULL;
	...
	if (!sysfs_ops->tcon_update_store) {
		LCD_KIT_ERR("tcon_update_store is NULL!\n");
		return LCD_KIT_FAIL;
	}

	if (!sysfs_ops->tcon_update_store(dev, attr, buf, count)) {
		LCD_KIT_ERR("tcon_update_store error!\n");
		return LCD_KIT_FAIL;
	}
	return count;
}

static ssize_t lcd_kit_tcon_update_info_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	struct lcd_kit_sysfs_ops *sysfs_ops = NULL;
	...
	if (!sysfs_ops->tcon_update_info_show) {
		LCD_KIT_ERR("tcon_update_info_show is NULL!\n");
		return LCD_KIT_FAIL;
	}

	return sysfs_ops->tcon_update_info_show(dev, attr, buf);
}
...
static DEVICE_ATTR(lcd_avg_on_time, 0644, lcd_avg_on_time_show, NULL);
static DEVICE_ATTR(lcd_tcon_update, 0664, lcd_kit_tcon_update_show,
	lcd_kit_tcon_update_store);
static DEVICE_ATTR(lcd_tcon_update_info, 0644, lcd_kit_tcon_update_info_show, NULL);
...
struct attribute *lcd_kit_conf[] = {
	...
	&dev_attr_lcd_avg_on_time.attr,
	&dev_attr_lcd_tcon_update.attr,
	&dev_attr_lcd_tcon_update_info.attr,
};
