...
static int lcd_kit_dbg_vsync_area(char *par);
...
static int lcd_kit_dbg_initial_code_cmd(char *par);
static int lcd_kit_dbg_read_initial_code(char *par);
...
static int lcd_kit_dbg_dual_dsi_cmd(char *par);
static int lcd_kit_dbg_cmdstate(char *par);
static int lcd_kit_dbg_ddic_bin(char *par);
static int lcd_kit_dbg_hsync_area(char *par);
static int lcd_kit_dbg_hline_time(char *par);
static int lcd_kit_dbg_dpi_hsize(char *par);
static int lcd_kit_dbg_panel_xres(char *par);
static int lcd_kit_dbg_panel_yres(char *par);
static int lcd_kit_dbg_panel_width(char *par);
static int lcd_kit_dbg_panel_height(char *par);
...
struct lcd_kit_dbg_func item_func[] = {
	...
	{ "VSyncArea", lcd_kit_dbg_vsync_area},
	{ "PanelBlMaxnit", lcd_kit_dbg_bl_max_nit },
	{ "HsyncArea", lcd_kit_dbg_hsync_area },
	{ "HLineTime", lcd_kit_dbg_hline_time },
	{ "DpiHSize", lcd_kit_dbg_dpi_hsize },
	...
	{ "PanelInitialCodeCommand", lcd_kit_dbg_initial_code_cmd },
	{ "ReadInitialCodeCommand", lcd_kit_dbg_read_initial_code },
	...
	{ "PanelDualDsiCommand", lcd_kit_dbg_dual_dsi_cmd },
	{ "PanelXres", lcd_kit_dbg_panel_xres},
	{ "PanelYres", lcd_kit_dbg_panel_yres},
	{ "PanelWidth", lcd_kit_dbg_panel_width},
	{ "PanelHeight", lcd_kit_dbg_panel_height},
};
...
static struct lcd_kit_debug lcd_kit_dbg;
...
static struct lcd_kit_dsi_panel_cmds slave_dsi_cmds;
...
#define LCD_KIT_PANEL_INI_CODE_PATH             "/data/panel_ini_code.txt"
#define DOUBLE_DSI_XML_FILE_MIN_LEN             2
#define DSI_CTRL_HEAD_SIZE                      7
#define DSI_SINGLE_CHAR_WIDTH                   6
...
static int lcd_kit_dbg_hback_porch(char *par)
{
	...
	LCD_KIT_INFO("hback_porch value = %d\n", value);
	return LCD_KIT_OK;
}
...
static int lcd_kit_dbg_hsync_area(char *par)
{
	...
	lcd_kit_parse_u32_digit(par, &value, 1);
	if (dbg_ops->hsync_area)
		dbg_ops->hsync_area(value);
	LCD_KIT_INFO("hsync_area value = %d\n", value);
	return LCD_KIT_OK;
}
...
static int lcd_kit_dbg_hline_time(char *par)
{
	...
	lcd_kit_parse_u32_digit(par, &value, 1);
	if (dbg_ops->hline_time)
		dbg_ops->hline_time(value);
	LCD_KIT_INFO("hline_time value = %d\n", value);
	return LCD_KIT_OK;
}
...
static int lcd_kit_dbg_dpi_hsize(char *par)
{
	...
	lcd_kit_parse_u32_digit(par, &value, 1);
	if (dbg_ops->dpi_hsize)
		dbg_ops->dpi_hsize(value);
	LCD_KIT_INFO("dpi_hsize value = %d\n", value);
	return LCD_KIT_OK;
}
...
static int lcd_kit_dbg_vsync_area(char *par)
{
	...
	lcd_kit_parse_u32_digit(par, &value, 1);
	if (dbg_ops->vsync_area)
		dbg_ops->vsync_area(value);
	LCD_KIT_INFO("vsync_area value = %d\n", value);
	return LCD_KIT_OK;
}
...
static int lcd_kit_dbg_hfront_porch(char *par)
{
	...
	LCD_KIT_INFO("hfront_porch value = %d\n", value);
	return LCD_KIT_OK;
}
...
static int lcd_kit_dbg_hpulse_width(char *par)
{
	...
	LCD_KIT_INFO("hpulse_width value = %d\n", value);
	return LCD_KIT_OK;
}
...
static int lcd_kit_dbg_vback_porch(char *par)
{
	...
	LCD_KIT_INFO("vback_porch value = %d\n", value);
	return LCD_KIT_OK;
}
...
static int lcd_kit_dbg_panel_xres(char *par)
{
	...
	lcd_kit_parse_u32_digit(par, &value, 1);
	if (dbg_ops->panel_xres)
		dbg_ops->panel_xres(value);
	LCD_KIT_INFO("panel_xres value = %d\n", value);
	return LCD_KIT_OK;
}
...
static int lcd_kit_dbg_panel_yres(char *par)
{
	...
	lcd_kit_parse_u32_digit(par, &value, 1);
	if (dbg_ops->panel_yres)
		dbg_ops->panel_yres(value);
	LCD_KIT_INFO("panel_yres value = %d\n", value);
	return LCD_KIT_OK;
}
...
static int lcd_kit_dbg_panel_height(char *par)
{
	...
	lcd_kit_parse_u32_digit(par, &value, 1);
	if (dbg_ops->height)
		dbg_ops->height(value);
	LCD_KIT_INFO("height value = %d\n", value);
	return LCD_KIT_OK;
}
...
static int lcd_kit_dbg_read_xml_bin(char *xml_bin, int xml_len)
{
	struct file *fp;
	mm_segment_t old_fs;
	...
	if (fp == NULL) {
		LCD_KIT_ERR("open ddic.bin fail\n");
		return LCD_KIT_FAIL;
	}
	old_fs = get_fs();
	set_fs(KERNEL_DS);

	vfs_read(fp, xml_bin, xml_len, &(fp->f_pos));
	set_fs(old_fs);
	filp_close(fp, NULL);
...
static int lcd_kit_dbg_initial_code_cmd(char *par)
{
	...
	LCD_KIT_INFO("initial code cmd par:%s\n", par);

	// Read debug_flag.bin's first byte to check whether the read/write is OK
	lcd_kit_debug_clear_file(MIPI_FLAG_PATH);
	lcd_kit_parse_u32_digit(par, &xml_len, 1); // 1:single 32-bit
	LCD_KIT_INFO("initial code xml length:0x%x \n", xml_len);
	if (xml_len > LCD_KIT_INITIAL_CODE_MAX_BLEN || xml_len <= 0) {
		LCD_KIT_ERR("xml_len overflow\n");
		return LCD_KIT_FAIL;
	}

	// dbg_on_cmds length is smaller than dbg_xml_bin
	lcd_kit_dbg.dbg_xml_bin = kzalloc(xml_len, 0);
	if (!lcd_kit_dbg.dbg_xml_bin) {
		LCD_KIT_ERR("dbg_xml_bin kzalloc fail\n");
		return LCD_KIT_FAIL;
	}

	// dbg_initial_code_cmds old memory needs to be freed
	if (lcd_kit_dbg.dbg_initial_code_cmds != NULL) {
		kfree(lcd_kit_dbg.dbg_initial_code_cmds);
		lcd_kit_dbg.dbg_initial_code_cmds = NULL;
	}

	// malloc new memory, memory need to stay the same until next time
	lcd_kit_dbg.dbg_initial_code_cmds = kzalloc(xml_len, 0);
	if (!lcd_kit_dbg.dbg_initial_code_cmds) {
		LCD_KIT_ERR("dbg_initial_code_cmds kzalloc fail\n");
		return LCD_KIT_FAIL;
	}

	ret = lcd_kit_dbg_read_xml_bin(lcd_kit_dbg.dbg_xml_bin, xml_len);
	if (ret == LCD_KIT_FAIL)
		return LCD_KIT_FAIL;

	// dbg_on_cmds length is smaller than dbg_xml_bin, so max out length set as dbg_xml_bin length
	len = lcd_kit_parse_u8_digit(lcd_kit_dbg.dbg_xml_bin, lcd_kit_dbg.dbg_initial_code_cmds, xml_len);

	LCD_KIT_INFO("len = 0x%x\n", len);
	if (len > 0) {
		ret = lcd_kit_dbg_parse_cmd(&common_info->panel_on_cmds,
			lcd_kit_dbg.dbg_initial_code_cmds, len);
		if (ret == LCD_KIT_FAIL)
			return LCD_KIT_FAIL;
	}

	mipi_flag = DEBUG_OK_FLAG;
	ret = lcd_ddic_debug_fs_write(MIPI_FLAG_PATH, &mipi_flag, sizeof(uint8_t));
	return ret;
}

static int lcd_kit_dbg_write_initial_code_file(struct lcd_kit_dsi_panel_cmds *pcmds)
{
	int i;
	int j;
	struct lcd_kit_dsi_cmd_desc *desc;
	struct file *fp;
	mm_segment_t old_fs;
	char dsi_ctrl_head[DSI_CTRL_HEAD_SIZE * DSI_SINGLE_CHAR_WIDTH + 1] = {0};
	char *head;

	fp = filp_open(LCD_KIT_PANEL_INI_CODE_PATH, O_WRONLY | O_CREAT | O_TRUNC, 0644);
	if (fp == NULL) {
		LCD_KIT_ERR("open panel_ini_code.txt fail\n");
		return LCD_KIT_FAIL;
	}
	...
	for (i = 0; i < pcmds->cmd_cnt; i++) {
		desc = &pcmds->cmds[i];
		head = (char *)desc;
		for (j = 0; j < DSI_CTRL_HEAD_SIZE; j++)
			sprintf_s(dsi_ctrl_head + j * DSI_SINGLE_CHAR_WIDTH, DSI_CTRL_HEAD_SIZE * DSI_SINGLE_CHAR_WIDTH + 1,
				"0x%02X, ", head[j]);

		vfs_write(fp, dsi_ctrl_head, DSI_CTRL_HEAD_SIZE * DSI_SINGLE_CHAR_WIDTH, &(fp->f_pos));
		for (j = 0; j < (int)(desc->dlen); j++) {
			if (j < (int)(desc->dlen) - 1)
				sprintf_s(dsi_ctrl_head, DSI_CTRL_HEAD_SIZE * DSI_SINGLE_CHAR_WIDTH + 1,
					"0x%02X, ", desc->payload[j]);
			else
				sprintf_s(dsi_ctrl_head, DSI_CTRL_HEAD_SIZE * DSI_SINGLE_CHAR_WIDTH + 1,
					"0x%02X\r\n", desc->payload[j]);
			vfs_write(fp, dsi_ctrl_head, DSI_SINGLE_CHAR_WIDTH, &(fp->f_pos));
		}
	}
...
static int lcd_kit_dbg_read_initial_code(char *par)
{
	...
	ret = lcd_kit_dbg_write_initial_code_file(&common_info->panel_on_cmds);
	if (ret == LCD_KIT_FAIL)
		return LCD_KIT_FAIL;
...
static void lcd_kit_parse_mipi_dsi_cmds(char *par, char *dbg_cmds, struct lcd_kit_dsi_panel_cmds *mipi_dsi_cmds)
{
	int len;

	mipi_dsi_cmds->buf = NULL;
	mipi_dsi_cmds->blen = 0;
	mipi_dsi_cmds->cmds = NULL;
	mipi_dsi_cmds->cmd_cnt = 0;
	mipi_dsi_cmds->flags = 0;

	len = lcd_kit_parse_u8_digit(par, dbg_cmds, LCD_KIT_CONFIG_TABLE_MAX_NUM);
	if (len > 0)
		lcd_kit_dbg_parse_cmd(mipi_dsi_cmds, dbg_cmds, len);
}
...
static int lcd_kit_dbg_mipi_rx(char *par, uint8_t *read_buf, char *dbg_cmds)
{
	...
	lcd_kit_parse_mipi_dsi_cmds(par, dbg_cmds, &dsi_cmds);
...
static int lcd_kit_dbg_get_read_bytes(struct lcd_kit_dsi_panel_cmds *dsi_cmds)
{
	int read_byte = 0;
	int dcs_type;
	int i = 0;
...
static int lcd_kit_dbg_write_master_slave_data(char *master_buf, char * slave_buf)
{
	int ret;
	uint8_t mipi_flag;
	uint8_t master_len = 0;
	uint8_t slave_len = 0;
	int i;

	master_len = lcd_kit_dbg_get_read_bytes(&dsi_cmds);
	slave_len = lcd_kit_dbg_get_read_bytes(&slave_dsi_cmds);
	if (master_len != slave_len) {
		LCD_KIT_ERR("master rx len not equal to slave\n");
		return LCD_KIT_FAIL;
	}

	if (master_len + slave_len >= READ_DDIC_BUF_LEN) {
		LCD_KIT_ERR("read sum len overflow\n");
		return LCD_KIT_FAIL;
	}

	for (i = 0; i < slave_len; i++)
		master_buf[master_len + i] = slave_buf[i];

	master_buf[master_len + slave_len] = '\0';
	ret = lcd_ddic_debug_fs_write(LCD_KIT_READBIN_FILE_PATH, master_buf, READ_DDIC_BUF_LEN);
	if (ret == LCD_KIT_FAIL)
		return LCD_KIT_FAIL;
...
static int lcd_kit_dbg_dual_mipi_rx(char *par, int len)
{
	int ret;
	uint8_t master_buf[READ_DDIC_BUF_LEN] = {0};
	uint8_t slave_buf[READ_DDIC_BUF_LEN] = {0};
	char *master_cmds;
	char *slave_cmds;

	struct lcd_kit_dbg_ops *dbg_ops = lcd_kit_get_debug_ops();

	if (dbg_ops->dbg_dual_mipi_diff_rx == NULL) {
		LCD_KIT_ERR("dbg_dual_mipi_rx is NULL!\n");
		return LCD_KIT_FAIL;
	}

	if (len < DOUBLE_DSI_XML_FILE_MIN_LEN) {
		LCD_KIT_ERR("xml len error\n");
		return LCD_KIT_FAIL;
	}

	if (is_valid_char(par[len / 2 - 1])) {
		LCD_KIT_ERR("xml mirror symmetry is not satisfied\n");
		return LCD_KIT_FAIL;
	}

	master_cmds = kzalloc(LCD_KIT_CONFIG_TABLE_MAX_NUM, 0);
	slave_cmds = kzalloc(LCD_KIT_CONFIG_TABLE_MAX_NUM, 0);
	if (!master_cmds || !slave_cmds) {
		LCD_KIT_ERR("kzalloc fail\n");
		return LCD_KIT_FAIL;
	}

	// the first half is masetr xml, the second half  is slave xml
	par[len / 2 - 1] = '\0';
	lcd_kit_parse_mipi_dsi_cmds(par, master_cmds, &dsi_cmds);
	lcd_kit_parse_mipi_dsi_cmds(par + len / 2, slave_cmds, &slave_dsi_cmds);

	// slave link state consistent with master link state
	slave_dsi_cmds.link_state = dsi_cmds.link_state;
	ret = dbg_ops->dbg_dual_mipi_diff_rx(master_buf, slave_buf,
		READ_DDIC_BUF_LEN, &dsi_cmds, &slave_dsi_cmds);
	kfree(master_cmds);
	master_cmds = NULL;
	kfree(slave_cmds);
	slave_cmds = NULL;
	...
	master_buf[READ_DDIC_BUF_LEN - 1] = '\0';
	slave_buf[READ_DDIC_BUF_LEN - 1] = '\0';

	return lcd_kit_dbg_write_master_slave_data(master_buf, slave_buf);
}
...
static int lcd_kit_dbg_cmd(char *par)
{
	...
	read_byte = lcd_kit_dbg_get_read_bytes(&dsi_cmds);
	if (read_byte > 0) {
...
static int lcd_kit_dbg_dual_dsi_cmd(char *par)
{
	...
	if (dbg_ops->is_dual_mipi_panel(panel_id) == false) {
		LCD_KIT_ERR("dual dsi not support\n");
		return LCD_KIT_FAIL;
	}

	len = strlen(par);
	return lcd_kit_dbg_dual_mipi_rx(par, len);
}
