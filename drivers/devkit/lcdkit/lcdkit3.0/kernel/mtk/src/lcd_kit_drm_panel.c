...
static int lcm_disable(struct drm_panel *panel)
{
	...
	if (common_ops->panel_power_off)
		common_ops->panel_power_off(LCD_ACTIVE_PANEL, dsi);
	LCD_KIT_INFO("exit\n");
	return LCD_KIT_OK;
}
...
static int lcm_early_prepare(struct drm_panel *panel)
{
	...
	if (common_ops->panel_power_on_without_mipi)
		common_ops->panel_power_on_without_mipi(LCD_ACTIVE_PANEL, dsi);
...
static int lcm_prepare(struct drm_panel *panel)
{
	...
	if (plcd_kit_info == NULL || plcd_kit_info->mipi.mipi_ulpm_after_vci == 0) {
		if (common_ops->panel_power_on)
			common_ops->panel_power_on(LCD_ACTIVE_PANEL, dsi);
	} else {
		if (common_ops->panel_power_on_with_mipi)
			common_ops->panel_power_on_with_mipi(LCD_ACTIVE_PANEL, dsi);
	}
	...
	enum MTK_PANEL_MODE_SWITCH_STAGE stage, int lcd_fps_scence)
{
	...
	if (stage == AFTER_DSI_POWERON) {
		if (ops->mipi_tx) {
			lcm_set_panel_state(LCD_POWER_STATE_OFF);
			ret = ops->mipi_tx(LCD_ACTIVE_PANEL, dsi,
	...
	void *dsi_drv, dcs_write_gce cb, void *handle)
{
	...
	if (disp_info->alpm.need_reset) {
		/* reset pull low */
		lcd_kit_reset_power_ctrl(LCD_ACTIVE_PANEL, 0);
		msleep(1);
		/* reset pull high */
		lcd_kit_reset_power_ctrl(LCD_ACTIVE_PANEL, 1);
		msleep(6);
	}
...
static void lcm_init_ext_params(void)
{
	...
	if (disp_info->fps.support) {
		...
		/* if MTK Fp Delay low fps is enable, MTK hbm delay param used */
		if (disp_info->mtk_fp_delay_enable) {
			ext_params.te_low_cycle = disp_info->mtk_low_te_inerval_time_us;
			ext_params.vsync_low_time = disp_info->mtk_low_left_time_th_us;
			ext_params.hbm_low_deadline = disp_info->mtk_low_right_time_th_us;
		}
	...
	/* MTK DC delay enable */
	if (disp_info->mtk_dc_delay_enable) {
		ext_params.dc_delay_enable = disp_info->mtk_dc_delay_enable;
		ext_params.te_dc_cycle = disp_info->mtk_dc_te_inerval_time_us;
		ext_params.vsync_dc_time = disp_info->mtk_dc_left_time_th_us;
		ext_params.dc_deadline = disp_info->mtk_dc_right_time_th_us;
		ext_params.dc_delay_time = disp_info->mtk_dc_delay_time_us;
	}
}
...
int lcd_kit_init(void)
{
	...
	if (common_ops->btb_init) {
		common_ops->btb_init(LCD_ACTIVE_PANEL);
