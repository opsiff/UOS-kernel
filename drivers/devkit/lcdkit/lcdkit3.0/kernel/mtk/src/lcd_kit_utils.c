...
void lcd_kit_parse_fps(struct device_node *np)
{
	...
	/* hopping */
	lcd_kit_parse_u32(np, "lcd-kit,mipi-hop-support", &disp_info->fps.hop_support, 0);
...
void lcd_kit_parse_util(struct device_node *np)
{
	...
	/* HBM delay set */
	lcd_kit_parse_u32(np, "lcd-kit,mtk-fp-delay-enable",
		&disp_info->mtk_fp_delay_enable, 0);
	...
	/* DC delay set */
	lcd_kit_parse_u32(np, "lcd-kit,mtk-dc-delay-enable",
		&disp_info->mtk_dc_delay_enable, 0);
	lcd_kit_parse_u32(np, "lcd-kit,mtk-dc-left-time-th-us",
		&disp_info->mtk_dc_left_time_th_us, 0);
	lcd_kit_parse_u32(np, "lcd-kit,mtk-dc-right-time-th-us",
		&disp_info->mtk_dc_right_time_th_us, 0);
	lcd_kit_parse_u32(np, "lcd-kit,mtk-dc-te-inerval-time-us",
		&disp_info->mtk_dc_te_inerval_time_us, 0);
	lcd_kit_parse_u32(np, "lcd-kit,mtk-dc-delay-time-us",
		&disp_info->mtk_dc_delay_time_us, 0);
	/* idle mode support */
...
int lcd_kit_check_pcd_errflag_check(void *hld)
{
	...
	if (disp_info->pcd_errflag.pcd_support) {
		...
		if (adapt_ops->mipi_rx) {
			ret = adapt_ops->mipi_rx(LCD_ACTIVE_PANEL, hld, read_pcd,
	...
	if (disp_info->pcd_errflag.errflag_support) {
		...
		if (adapt_ops->mipi_rx)
			ret = adapt_ops->mipi_rx(LCD_ACTIVE_PANEL, hld, read_errflag,
				LCD_KIT_ERRFLAG_SIZE - 1, &disp_info->pcd_errflag.read_errflag_cmds);
		if (ret) {
			LCD_KIT_INFO("mipi_rx fail\n");
			return ret;
		}
...
static int lcd_kit_proximity_power_off(void)
{
	...
	if (common_ops->panel_only_power_off)
		common_ops->panel_only_power_off(LCD_ACTIVE_PANEL, NULL);
	up(&disp_info->thp_second_poweroff_sem);
