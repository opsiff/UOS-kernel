...
static int touch_driver_power_init(int panel_id)
{
	int ret;
...
static int touch_driver_power_on_for_3662(struct thp_device *tdev)
{
	...
	ret = thp_power_supply_ctrl(THP_VCC, THP_POWER_ON, 0, tdev->thp_core->panel_id); /* 0ms */
	if (ret)
		thp_log_err("%s:power on ctrl vcc failed\n", __func__);

	ret = thp_power_supply_ctrl(THP_IOVDD, THP_POWER_ON, 0, tdev->thp_core->panel_id); /* 0ms */
	if (ret)
...
static void touch_driver_power_release(struct thp_device *tdev)
{
...
static int touch_driver_power_on(struct thp_device *tdev)
{
	...
	gpio_direction_output(tdev->gpios->rst_gpio, GPIO_LOW);
	mdelay(1);
	ret = thp_power_supply_ctrl(THP_IOVDD, THP_POWER_ON, 1, tdev->thp_core->panel_id); /* 1ms */
	if (ret)
		thp_log_err("%s:power on ctrl vddio failed\n", __func__);
	ret = thp_power_supply_ctrl(THP_VCC, THP_POWER_ON, 1, tdev->thp_core->panel_id); /* 1ms */
	if (ret)
		thp_log_err("%s:power on ctrl vcc failed\n", __func__);
	thp_log_info("%s pull up tp ic reset\n", __func__);
	gpio_set_value(tdev->gpios->rst_gpio, GPIO_HIGH);
...
static int touch_driver_power_off(struct thp_device *tdev)
{
	...
	ret = thp_power_supply_ctrl(THP_IOVDD, THP_POWER_OFF, 0, tdev->thp_core->panel_id);
	if (ret)
		thp_log_err("%s:power off ctrl vddio failed\n", __func__);
	ret = thp_power_supply_ctrl(THP_VCC, THP_POWER_OFF, 1, tdev->thp_core->panel_id); /* 1ms */
	if (ret)
	...
	struct mxt_info *mxtinfo)
{
	...
	touch_driver_power_release(tdev);
...
static int touch_driver_chip_detect(struct thp_device *tdev)
{
	...
	gpio_set_value(tdev->gpios->rst_gpio, GPIO_LOW);
	thp_log_info("%s: called\n", __func__);
	ret_val = touch_driver_power_init(tdev->thp_core->panel_id);
	if (ret_val)
