...
#define THP_GESTURE_SINGLE_CLICK (1 << 0)
...
/* thp in sensorhub, sensorhub push-down */
static void need_work_in_suspend_for_shb_thp_2(struct thp_device *tdev)
{
	int ret;
...
#ifdef CONFIG_HUAWEI_SHB_THP
static int touch_driver_set_ap_state_thp_1(struct thp_device *tdev)
{
	unsigned char report_to_ap_cmd[SYNA_CMD_LEN] = {
		0xC7, 0x02, 0x00,
		0x36, 0x01
	...
	thp_log_info("%s get flag retry 0x%02x\n",
		__func__, report_to_ap_cmd_gesture[TOUCH_GESTURE_CMD]);
	touch_driver_set_cmd(tdev, report_to_ap_cmd_gesture,
		sizeof(report_to_ap_cmd_gesture));
	return 0;
}
	...
	unsigned int finger_status)
{
	...
	if (tdev->thp_core->tsa_event_to_udfp == TAS_EVENT_TO_UDFP_MODE_ONE && finger_status) {
...
static void need_work_in_suspend_for_shb_thp_1(struct thp_device *tdev)
{
	int ret;
	char report_to_ap_cmd[SYNA_CMD_LEN] = { 0xC7, 0x02, 0x00, 0x2E, 0x01 };

	thp_log_info("%s: called\n", __func__);
#if (defined(CONFIG_HUAWEI_SHB_THP))
	if (tdev->thp_core->tsa_event_to_udfp == TAS_EVENT_TO_UDFP_MODE_TWO) {
		ret = touch_driver_set_ap_state_thp_1(tdev);
		if (ret)
			thp_log_err("failed to set ap state thp 1.0\n");
	}
#endif
	thp_log_info("%s:thp need work in suspend\n", __func__);
	ret = thp_bus_lock();
...
static int touch_driver_suspend_self_control_power(struct thp_device *tdev)
{
	...
	if (is_pt_test_mode(tdev)) {
		...
		if (cd->support_shb_thp)
			need_work_in_suspend_for_shb_thp_2(tdev); // use thp in sensorhub, sensorhub push-down
		else if (cd->tsa_event_to_udfp == TAS_EVENT_TO_UDFP_MODE_TWO)
			need_work_in_suspend_for_shb_thp_1(tdev); // use kit in sensorhub, sensorhub no push-down
		else
