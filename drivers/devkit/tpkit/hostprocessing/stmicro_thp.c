	...
	struct thp_udfp_data *udfp_data, unsigned int len)
{
	 ...
	 */
	udfp_data->tpud_data.tp_event = (read_buff[3] & KEEP_LOW_BIT);
	...
	thp_log_info("%s:version:%x, tp_event:%d, x:%d, y:%d, aod:%d, key:%d\n",
		__func__, udfp_data->tpud_data.version,
		udfp_data->tpud_data.tp_event, udfp_data->tpud_data.tp_x,
...
static int touch_driver_power_on(struct thp_device *tdev)
{
	...
	if (tdev->thp_core->support_vendor_ic_type == STU56A) {
		...
		ret = thp_power_supply_ctrl(THP_VCC, THP_POWER_ON, 2, tdev->thp_core->panel_id);
		if (ret)
	...
	gpio_set_value(tdev->gpios->cs_gpio, GPIO_LOW);
	ret = thp_power_supply_ctrl(THP_VCC, THP_POWER_ON, 1, tdev->thp_core->panel_id); /* delay 1ms */
	if (ret)
		thp_log_err("%s:power ctrl vcc fail\n", __func__);
	ret = thp_power_supply_ctrl(THP_IOVDD, THP_POWER_ON, 5, tdev->thp_core->panel_id); /* delay 5ms */
	if (ret)
...
static int touch_driver_power_off(struct thp_device *tdev)
{
	...
	if (tdev->thp_core->support_vendor_ic_type == ST_BA80Y) {
		...
		ret = thp_power_supply_ctrl(THP_VCC, THP_POWER_OFF, 1, tdev->thp_core->panel_id);
		if (ret)
			thp_log_err("%s:power ctrl vcc fail\n", __func__);
		/* iovdd power off need delay 1 ms */
		ret = thp_power_supply_ctrl(THP_IOVDD, THP_POWER_OFF, 1, tdev->thp_core->panel_id);
		if (ret)
			thp_log_err("%s:power ctrl iovdd fail\n", __func__);
		return ret;
	}
	...
	gpio_direction_output(tdev->gpios->rst_gpio, GPIO_LOW);
	mdelay(tdev->timing_config.suspend_reset_after_delay_ms);
	ret = thp_power_supply_ctrl(THP_VCC, THP_POWER_OFF, 1, tdev->thp_core->panel_id); /* delay1ms */
	if (ret)
		thp_log_err("%s:power ctrl vcc fail\n", __func__);
	ret = thp_power_supply_ctrl(THP_IOVDD, THP_POWER_OFF, 1, tdev->thp_core->panel_id); /* delay1ms */
	if (ret)
		thp_log_err("%s:power ctrl iovdd fail\n", __func__);
	gpio_set_value(tdev->gpios->cs_gpio, GPIO_LOW);
#else
...
static int touch_driver_suspend(struct thp_device *tdev)
{
	...
	g_st_thp_udfp_status = thp_get_status(THP_STATUS_UDFP, tdev->thp_core->panel_id);
