...
volatile int g_tskit_ic_type = HYBRID;
...
static void ts_watchdog_timer(struct timer_list *data)
{
	struct ts_kit_platform_data *cd = from_timer(cd, data, watchdog_timer);
...
static void ts_kit_status_check_init(void)
{
	...
	if (data->chip_data->need_wd_check_status) {
		...
		timer_setup(&(data->watchdog_timer), ts_watchdog_timer,
...
static int ts_kit_init(void *data)
{
	...
	cd->ts_wake_lock = wakeup_source_register(&cd->ts_dev->dev, "ts_wake_lock");
#else
	cd->ts_wake_lock = wakeup_source_register("ts_wake_lock");
#endif
	if (!cd->ts_wake_lock) {
...
err_out:
	...
	wakeup_source_unregister(cd->ts_wake_lock);
	cd->ts_wake_lock = NULL;
out:
