...
int set_support_number(struct zinitix_platform_data *zinitix_pdata)
{
	...
	zinitix_pdata->multi_fingers = g_zinitix_pdata->support_finger_number;
...
int get_xy_node_info(struct zinitix_platform_data *zinitix_pdata)
{
	...
	if (zinitix_pdata->channel_x_num == ZINITIX_TOTAL_NUMBER_OF_X)
		zinitix_pdata->channel_x_num = g_zinitix_pdata->zinitix_device_data->channel_x_num;
	...
	if (zinitix_pdata->channel_y_num == ZINITIX_TOTAL_NUMBER_OF_Y)
		zinitix_pdata->channel_y_num = g_zinitix_pdata->zinitix_device_data->channel_y_num;
		...
		struct ts_event *event_data)
{
	...
	u16 config_x_max = g_zinitix_pdata->zinitix_device_data->config_x_max;
	u16 config_y_max = g_zinitix_pdata->zinitix_device_data->config_y_max;
	...
	for (i = 0; i < g_zinitix_pdata->multi_fingers; i++) {
		...
		if (zinitix_bit_test((*touch_data).coord[i].sub_status,
			...
			event_data->position_x[i] = abs(x-config_x_max);
			event_data->position_y[i] = abs(y-config_y_max);
		} else {
...
static int touch_driver_read_gesture(struct ts_event *event_data)
{
	...
	if ((gesture_flag == DOUBLE_TAP_VALUE) &&
		(g_zinitix_dev_data->easy_wakeup_info.off_motion_on == true)) {
		g_zinitix_dev_data->easy_wakeup_info.off_motion_on = false;
...
static int touch_driver_read_touch_data(struct ts_event *event_data)
{
	...
	if (zinitix_bit_test(touch_data.status, ZINITIX_BIT_PALM)) {
		...
		if (ret)
			TS_LOG_INFO("%s:zinitix swreset cmd in fail\n", __func__);
		goto read_data_exit;
	}
...
int wait_for_irq(int read_num, u32 pkg_size, int *raw_data)
{
	...
	while (gpio_get_value(g_zinitix_dev_data->ts_platform_data->irq_gpio)) {
		wait_times++;
		if (wait_times >= ZINITIX_WAIT_IRQ_TIMES_PSD) {
	...
	struct ts_rawdata_info_new *info)
{
	int ret = 0;
	int raw_data_size;
	int raw_data[ZINITIX_TX_NUM_MAX][ZINITIX_RX_NUM_MAX] = {0};
	int rx_delta[ZINITIX_TX_NUM_MAX - 1][ZINITIX_RX_NUM_MAX] = {0};
	int tx_delta[ZINITIX_TX_NUM_MAX][ZINITIX_RX_NUM_MAX - 1] = {0};
	unsigned int idx = 0;
	unsigned int itx = 0;
	unsigned int row;
	unsigned int col;
	...
	idx += g_zinitix_pdata->channel_x_num;
	...
	struct ts_rawdata_info_new *info)
{
	...
	int rawdata_buff_index = 2;
	int times = 0;
	int max_noise[ZINITIX_TX_NUM_MAX][ZINITIX_RX_NUM_MAX] = {0};
	int tmp_noise[ZINITIX_TX_NUM_MAX][ZINITIX_RX_NUM_MAX] = {0};
	unsigned int idx = 0;
	signed short data;
	unsigned int row;
	unsigned int col;
	...
	struct ts_rawdata_info_new *info)
{
	...
	if (retval < 0) {
		TS_LOG_ERR("%s:set pdnd mode failed, retval=%d\n",
				__func__, retval);
	...
	if (retval < 0) {
		TS_LOG_ERR("%s:test raw data failed, retval=%d\n",
				__func__, retval);
		goto restore_point_mode;
	}
	info->listnodenum++;
	...
	if (retval < 0) {
		TS_LOG_ERR("%s:test diff data failed, retval=%d\n",
			__func__, retval);
		goto restore_point_mode;
	}
	info->listnodenum++;
	...
	if (retval < 0) {
		TS_LOG_ERR("%s:set delta mode failed, retval=%d\n",
			__func__, retval);
		goto restore_point_mode;
	}
	retval = ztx_noise_max_node_test(g_zinitix_params, info);
	if (retval < 0) {
		TS_LOG_ERR("%s:test noise max node failed, retval=%d\n",
			__func__, retval);
		goto restore_point_mode;
	}
	info->listnodenum++;
	...
	if (retval < 0) {
		TS_LOG_ERR("%s:short test failed, retval = %d\n",
			__func__, retval);
		goto restore_point_mode;
	}
	info->listnodenum++;
	...
	if (retval < 0) {
		TS_LOG_ERR("%s:set self mode failed, retval=%d\n",
			__func__, retval);
		goto restore_point_mode;
	}
	...
	if (retval < 0) {
		TS_LOG_ERR("%s:test self data failed, retval=%d\n",
			__func__, retval);
		goto restore_point_mode;
	}
	info->listnodenum++;
...
restore_point_mode:
	...
	if (retval < 0) {
		TS_LOG_ERR("%s:set point mode failed, retval=%d\n",
			__func__, retval);
	}
	/* power reset */
	touch_driver_power_off();
	msleep(ZINITIX_CHIP_OFF_DELAY);
	retval = touch_driver_power_resume();
	if (retval < 0) {
	...
	for (index = 0; index < ZINITIX_GENERAL_RETRY_TIMES; index++) {
		...
		if (retval < 0)
			TS_LOG_INFO("%s:send clear int cmd failed,retval=%d\n",
					__func__, retval);
	}
	return 0;
}
...
int write_firmware_to_flash(const struct firmware *fw)
{
	...
	for (flash_addr = 0; flash_addr < need_write_bytes_tmp;) {
		...
		TS_LOG_INFO("%s:gpio_get_value is %d\n", __func__ ,
			gpio_get_value(g_zinitix_dev_data->ts_platform_data->irq_gpio));
		mdelay(ZINITIX_GENERAL_DELAY_1MS);
		while (!gpio_get_value(g_zinitix_dev_data->ts_platform_data->irq_gpio));
...
int read_vendor_id(u8 *vendorid_check_cnt)
{
	...
	if (ret < 0) {
		TS_LOG_ERR("%s:[ESD]:read vendor id failed, ret=%d\n",
				__func__, ret);
		i2c_err++;
		if (i2c_err >= g_zinitix_pdata->need_change_esd_check_times) {
...
/* if vendor id incorrect in ZINITIX_ESD_CHECK_TIMES times,reset it */
	if (buffer != ZINITIX_VENDOR_ID_DEFAULT) {
		(*vendorid_check_cnt)++;
		if (*vendorid_check_cnt >= g_zinitix_pdata->need_change_esd_check_times) {
...
esd_check_reset:
	...
	reset_cnt++;
	if (reset_cnt <= g_zinitix_pdata->need_change_esd_check_times)
		return ret;
	reset_cnt = 0;
	touch_driver_power_off();
	ret = touch_driver_power_resume();
...
static int touch_driver_suspend(void)
{
	...
	if (((g_zinitix_dev_data->easy_wakeup_info.sleep_mode ==
		TS_GESTURE_MODE) || (g_zinitix_dev_data->easy_wakeup_info.aod_mode ==
		TS_TOUCH_AOD_OPEN)) && (g_zinitix_pdata->ts_pinctrl)) {
		ret = pinctrl_select_state(g_zinitix_pdata->ts_pinctrl,
			g_zinitix_pdata->pinctrl_state_idle);
		if (ret < 0)
			TS_LOG_ERR("%s:set pinctrl idle fail\n", __func__);
		TS_LOG_INFO("%s: pinctrl_select_state idle\n", __func__);

#ifdef CONFIG_HUAWEI_SHB_THP
		ret = send_thp_driver_status_cmd(POWER_OFF,
			g_zinitix_dev_data->ts_platform_data->multi_panel_index,
...
static int touch_driver_resume(void)
{
	...
	if (((g_zinitix_dev_data->easy_wakeup_info.sleep_mode == TS_GESTURE_MODE) ||
		(g_zinitix_dev_data->easy_wakeup_info.aod_mode == TS_TOUCH_AOD_OPEN)) &&
		(g_zinitix_pdata->ts_pinctrl)) {
		ret = pinctrl_select_state(g_zinitix_pdata->ts_pinctrl,
			g_zinitix_pdata->pinctrl_state_default);
		if (ret < 0)
			TS_LOG_ERR("%s: set pinctrl default fail\n", __func__);
		TS_LOG_INFO("%s: pinctrl_select_state default\n", __func__);
#ifdef CONFIG_HUAWEI_SHB_THP
...
static int touch_driver_init_chip(void)
{
	...
	g_zinitix_pdata->multi_fingers = g_zinitix_pdata->support_finger_number;
...
static int touch_driver_pinctrl_init(void)
{
	...
	pinctrl_select_state(g_zinitix_pdata->ts_pinctrl,
		g_zinitix_pdata->pinctrl_state_default);
	TS_LOG_INFO("set int gpio default");
	return 0;
err_pinctrl_put:
...
static int touch_driver_chip_detect(struct ts_kit_platform_data *pdata)
{
	...
	if (g_zinitix_pdata->support_pinctrl) {
		ret = touch_driver_pinctrl_init();
		if (ret < 0) {
			TS_LOG_ERR("%s: zts pinctrl init failed\n", __func__);
