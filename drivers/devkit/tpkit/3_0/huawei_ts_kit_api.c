...
void ts_set_irq_status(struct ts_kit_platform_data *cd, int status)
{
	if (!cd) {
		TS_LOG_ERR("%s: cd is nullptr\n", __func__);
		return;
	}

	mutex_lock(&cd->irq_mutex);
	if (cd->irq_enabled != (!!status)) {
		status ? enable_irq(cd->irq_id) : disable_irq(cd->irq_id);
...
EXPORT_SYMBOL(ts_set_irq_status);
