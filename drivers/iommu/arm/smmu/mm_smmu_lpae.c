...
static struct device *smmu_dev[SMMUID_MAX];

struct device *get_smmu_dev(int smmuid)
{
	if (smmuid < 0 || smmuid >= SMMUID_MAX) {
		pr_err("smmuid is invalid\n");
		return NULL;
	}

	return smmu_dev[smmuid];
}

static int vmm_attach_dev_lpae(struct device *dev,
		struct device_node *np, struct mm_dom_cookie *cookie)
{
	int ret;
	int smmuid;

	ret = of_get_iova_info(np, "vm_iova_info",
				&cookie->vm_iova.iova_start,
				&cookie->vm_iova.iova_size,
				&cookie->vm_iova.iova_align);
	if (ret) {
		pr_err("get dev(%s) vm iova info fail\n", dev_name(dev));
		return ret;
	}

	cookie->vm_iova_pool =
		iova_pool_setup(cookie->vm_iova.iova_start, cookie->vm_iova.iova_size,
				cookie->vm_iova.iova_align);
	if (!cookie->vm_iova_pool) {
		pr_err("setup dev(%s) vm iova pool fail\n", dev_name(dev));
		return -ENOMEM;
	}

	ret = of_property_read_u32(np, "smmuid", &smmuid);
	if (ret || (smmuid < 0 || smmuid >= SMMUID_MAX)) {
		pr_err("get dev(%s) smmuid info fail\n", dev_name(dev));
		iova_pool_destroy(cookie->vm_iova_pool);
		return -EINVAL;
	}
...
int of_get_iova_info(struct device_node *np, const char *name,
...
static int mm_smmu_handle_mapping_lpae(struct iommu_domain *domain,
				  ...
				  size_t size, int prot)
{
	...
	pgd = (smmu_pgd_t *)mm_domain->va_pgtable_addr;
	if (!pgd) {
...
out_unlock:
#ifdef CONFIG_DFX_MEM_TRACE
	smmu_trace_hook(DFX_MEM_ALLOC, iova, paddr, (unsigned int)size);
#endif
...
#ifdef CONFIG_DFX_MEM_TRACE
	smmu_trace_hook(DFX_MEM_FREE, iova, 0, unmap_size);
#endif
...
static int mm_attach_dev_lpae(struct iommu_domain *domain, struct device *dev)
{
...
#else
	if (!dev->iommu || dev->iommu->fwspec->ops != &mm_smmu_ops)
#endif
	...
	ret = of_get_iova_info(np, "iova_info",
	...
	ret = vmm_attach_dev_lpae(dev, np, cookie);
	if (ret) {
		pr_err("dev(%s): vmm attach dev fail\n", dev_name(dev));
...
#else
static int mm_iommu_map_sg_lpae(struct iommu_domain *domain,
	unsigned long iova, struct scatterlist *sg, unsigned int nents, int prot,
...
#endif
{
	...
	*mapped = 0;
	...
	for_each_sg(sg, s, nents, i) {
		...
		ret = mm_smmu_map_lpae(domain, iova + mapped_size, phys,
					 (size_t)s->length, prot, gfp);
		if (ret)
			goto out_err;
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(4, 19, 0))
	mm_smmu_unmap_lpae(domain, iova, mapped_size, NULL);
#else
	mm_smmu_unmap_lpae(domain, iova, mapped_size);
#endif
	return 0;
}
...
#else
static struct iommu_device *mm_smmu_probe_device(struct device *dev)
{
	struct mm_smmu_device_lpae *mm_smmu = NULL;
	...
	if (!dev->iommu->fwspec || dev->iommu->fwspec->ops != &mm_smmu_ops) {
		return ERR_PTR(-ENODEV); /* Not a iommu client device */
	}

	pr_info("enter %s, dev %s\n", __func__, dev_name(dev));

	mm_smmu = dev_iommu_priv_get(dev);
	if (!mm_smmu) {
		pr_err("Device (%pK %s) have no mm smmu for add to\n", dev,
		       dev_name(dev));
		return ERR_PTR(-ENODEV);
	...
	return &mm_smmu->iommu;
}

static void mm_smmu_release_device(struct device *dev) { }
#endif
...
static struct iommu_group *mm_smmu_device_group(struct device *dev)
{
	...
	struct mm_smmu_device_lpae *mm_smmu = dev->iommu_fwspec->iommu_priv;
#else
	struct mm_smmu_device_lpae *mm_smmu = dev_iommu_priv_get(dev);
#endif
...
#else
	.probe_device = mm_smmu_probe_device,
	.release_device = mm_smmu_release_device,
#endif
...
#if (LINUX_VERSION_CODE < KERNEL_VERSION(4, 19, 0))
IOMMU_OF_DECLARE(mm_smmu, "mm,mm-smmu-lpae", NULL);
#endif
