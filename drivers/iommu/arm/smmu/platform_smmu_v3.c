...
#if defined(CONFIG_LIBLINUX) && defined(CONFIG_MM_LB)
...
static int cd_force_linear = 1;
static struct arm_smmu_device *smmu_device[SMMU_MAX_ID] = {0};
...
static inline bool is_npu_smmuid(int smmuid)
{
	return (smmuid == SMMU_NPU);
}
...
static void arm_smmu_tcu_read_cmdq(struct arm_smmu_device *smmu, u32 cons_index)
{
	...
	dev_err(smmu->dev, "1:0x%llx\n", cmd[0]);
	dev_err(smmu->dev, "2:0x%llx\n", cmd[1]);
				...
				struct arm_smmu_s1_cfg *cfg)
{
	...
	cdptr = arm_smmu_get_cd_ptr(smmu_domain, cfg->cd.ssid);
	if (!cdptr)
		return -ENOMEM;
	...
	for (i = 0; i < cfg->cd.ssid_nums; i++) {
		...
		cdptr_offset = i * CTXDESC_CD_DWORDS;
		cdptr[cdptr_offset] = cpu_to_le64(val);

		val = (cfg->cd.ttbr & CTXDESC_CD_1_TTB0_MASK <<
			CTXDESC_CD_1_TTB0_SHIFT) | CTXDESC_CD_1_HAD0 | CTXDESC_CD_1_PBHA;
		cdptr[cdptr_offset + CTXDESC_CD_1_TTB0_OFFSET] =
			cpu_to_le64(val);

		cdptr[cdptr_offset + CTXDESC_CD_3_MAIR_OFFSET] =
				...
				struct arm_smmu_strtab_ent *ste)
{
	...
	if (ste->s1_cfg) {
		...
		val |= (ste->s1_cfg->cdcfg.cdtab_dma & STRTAB_STE_0_S1CTXPTR_MASK
			<< STRTAB_STE_0_S1CTXPTR_SHIFT)
			| FIELD_PREP(STRTAB_STE_0_S1CDMAX, ste->s1_cfg->s1cdmax)
			| FIELD_PREP(STRTAB_STE_0_S1FMT, ste->s1_cfg->s1fmt)
			| STRTAB_STE_0_CFG_S1_TRANS;
		/* TLB flush by VMID(SID) + ASID/SSID */
		dst[STRTAB_STE_2_S2VMID_OFFSET] = cpu_to_le64(
			ste->s1_cfg->cdcfg.vmid << STRTAB_STE_2_S2VMID_SHIFT);
	}
				...
				u32 sid, u32 ssid)
{
	...
	const u64 cdl1_mask = 0xffffffffff000;
	struct arm_smmu_strtab_cfg *cfg = &smmu->strtab_cfg;
	...
	if (((step[0] & STRTAB_STE_0_S1FMT) >> STRTAB_STE_0_S1FMT_SHIFT)
					== STRTAB_STE_0_S1FMT_LINEAR) {
		cd = (step[0] & cd_mask); /* cd phys */
		if (!cd) {
			pr_err("%s, linear cd entry is null!\n", __func__);
			return NULL;
		}
		cdtabp = phys_to_virt(cd); /* cd virt */
		cde = cdtabp + ssid * CTXDESC_CD_DWORDS; /* cd entry */
	} else {
		u64 cdtab;
		__le64 *cdp = NULL;
		unsigned int idx;
		...
		cd = (step[0] & cd_mask);
		if (!cd) {
			pr_err("%s, 2l cd l1desc is null!\n", __func__);
			return NULL;
		}
		cdp = phys_to_virt(cd);

		idx = ssid >> CTXDESC_SPLIT; /* l1 entry inxde */
		cdtab = cdp[idx * CTXDESC_L1_DESC_DWORDS] & cdl1_mask;
		if (!cdtab) {
			pr_err("%s, 2l cd l2desc is null!\n", __func__);
			return NULL;
		}
		cdtabp = phys_to_virt(cdtab);

		idx = ssid & (CTXDESC_L2_ENTRIES - 1); /* l2 cd entry index */
		cde = cdtabp + idx * CTXDESC_CD_DWORDS;
	}
...
{
	struct arm_smmu_device *smmu_media1 = smmu_device[SMMU_MEDIA1];

	if (!smmu_media1) {
		pr_err("%s smmu_media1 not exist!\n", __func__);
		return;
	}
...
static void arm_smmu_tlb_inv_context_dev(void *cookie, u32 ssid)
{
	...
	if (smmu_domain->stage == ARM_SMMU_DOMAIN_S1) {
		...
		cmd.tlbi.vmid	= smmu_domain->s1_cfg.cdcfg.vmid;
	} else {
...
static void arm_smmu_flush_tlb(struct device *dev, struct iommu_domain *domain)
{
	...
	u32 i;
	struct arm_smmu_domain *smmu_domain = NULL;
	struct arm_smmu_domain smmu_domain_temp = { 0 };
	...
	smmu_domain_temp.s1_cfg.cd.ssid = ssid_base;
	smmu_domain_temp.s1_cfg.cd.asid = master->asid_base;
	smmu_domain_temp.s1_cfg.cd.ssid_nums = master->ssid_nums;
	smmu_domain_temp.s1_cfg.cdcfg.vmid = smmu_domain->s1_cfg.cdcfg.vmid;
	smmu_domain_temp.s2_cfg.vmid = smmu_domain->s2_cfg.vmid;
	smmu_domain_temp.smmu = smmu_domain->smmu;
	smmu_domain_temp.stage = smmu_domain->stage;

	for (i = 0; i < master->ssid_nums; i++)
		arm_smmu_tlb_inv_context_dev(&smmu_domain_temp, ssid_base + i);
}
...
static int mm_smmu_alloc_asid_range(struct arm_smmu_device *smmu, int span, int nr)
{
	u32 size;
	u32 asid;
	int i;
...
static int arm_smmu_alloc_cd_tables(struct arm_smmu_domain *smmu_domain)
{
	...
	if (!(smmu->features & ARM_SMMU_FEAT_2_LVL_CDTAB) ||
		...
		l1size = max_contexts * (CTXDESC_CD_DWORDS << DWORD_BYTES_NUM);
	} else {
		cfg->s1fmt = STRTAB_STE_0_S1FMT_4K_L2;
		...
		l1size = cdcfg->num_l1_ents * (CTXDESC_L1_DESC_DWORDS << DWORD_BYTES_NUM);
	}

	cdcfg->cdtab = dmam_alloc_coherent(smmu->dev, l1size,
				&cdcfg->cdtab_dma, GFP_KERNEL | __GFP_ZERO);
	if (!cdcfg->cdtab) {
...
static void arm_smmu_free_cd_tables(struct arm_smmu_domain *smmu_domain)
{
	...
	if (cdcfg->l1_desc) {
		size = CTXDESC_L2_ENTRIES * (CTXDESC_CD_DWORDS <<
					DWORD_BYTES_NUM);
...
static void *arm_smmu_search_cdptr_by_sid(struct arm_smmu_device *smmu,
				struct arm_smmu_domain *smmu_domain, dma_addr_t *cdptr_dma)
{
	struct arm_smmu_iommu_group *arm_smmu_group = NULL;
	struct arm_smmu_domain *first_smmu_domain = NULL;
	struct iommu_domain *first_domain = NULL;
	struct device *master_dev = NULL;
	u32 sid;
	u32 ssid = 0;

#if (LINUX_VERSION_CODE < KERNEL_VERSION(5, 10, 0))
	sid = arm_smmu_get_sid(smmu_domain->dev->iommu_fwspec);
#else
	sid = arm_smmu_get_sid(smmu_domain->dev->iommu->fwspec);
#endif
#ifdef CONFIG_HIPCIE_CORE
	ssid = hi_pcie_get_ssid_by_dev(smmu_domain->dev);
#endif
	pr_err("into %s, %s: ssid is %d\n", __func__, smmu_domain->dev->init_name, ssid);

	list_for_each_entry(arm_smmu_group, &(smmu->iommu_groups), list)
		if (arm_smmu_group->sid == sid && arm_smmu_group->ssid != ssid) {
			master_dev = arm_smmu_group->dev;
			if (!master_dev) {
				pr_err("into %s, search master dev is NULL\n", __func__);
				continue;
			}

			first_domain = iommu_get_domain_for_dev(master_dev);
			first_smmu_domain = to_smmu_domain(first_domain);
			*cdptr_dma = first_smmu_domain->s1_cfg.cdcfg.cdtab_dma;
			return first_smmu_domain->s1_cfg.cdcfg.cdtab;
		}
				...
				struct io_pgtable_cfg *pgtbl_cfg)
{
	...
	if (smmu_domain->ssid_type == ISOLATE_SSID)
		cfg->cdcfg.cdtab = arm_smmu_search_cdptr_by_sid(smmu,
				smmu_domain, &cfg->cdcfg.cdtab_dma);
	else
		cfg->cdcfg.cdtab = NULL;
				...
				void *task_para)
{
	...
	task = (struct task_struct *)task_para;
	mm = get_task_mm(task);
	if (!mm) {
...
void arm_smmu_svm_ssid_asign(struct iommu_domain *dom, u16 ssid)
{
	struct arm_smmu_domain *smmu_domain;
	struct arm_smmu_s1_cfg *cfg;

	if (!dom) {
		pr_err("%s, dom is null\n", __func__);
		return;
	}
				...
				struct io_pgtable_cfg *pgtbl_cfg)
{
	...
	cfg->s1fmt = dev_smmu_domain->s1_cfg.s1fmt;
	cfg->s1cdmax = dev_smmu_domain->s1_cfg.s1cdmax;
	cfg->cdcfg.cdtab = dev_smmu_domain->s1_cfg.cdcfg.cdtab;
	cfg->cdcfg.cdtab_dma = dev_smmu_domain->s1_cfg.cdcfg.cdtab_dma;
	cfg->cdcfg.l1_desc = dev_smmu_domain->s1_cfg.cdcfg.l1_desc;
	cfg->cdcfg.num_l1_ents = dev_smmu_domain->s1_cfg.cdcfg.num_l1_ents;
...
static int arm_smmu_svm_alloc_pgtable_s1(struct io_pgtable_cfg *cfg)
{
	...
	reg = (ARM_LPAE_MAIR_ATTR_NC
	       << arm_lpae_mair_attr_shift(MT_NORMAL_NC)) |
	      (ARM_LPAE_MAIR_ATTR_WBRWA
	       << arm_lpae_mair_attr_shift(MT_NORMAL)) |
	      (ARM_LPAE_MAIR_ATTR_DEVICE
	       << arm_lpae_mair_attr_shift(MT_DEVICE_nGnRE));
				    ...
				    struct arm_smmu_master_data *master)
{
	...
	if (is_svm_bind_task_domain(domain)) {
		...
		pgtbl_ops = alloc_io_pgtable_ops((enum io_pgtable_fmt)fmt, &pgtbl_cfg, smmu_domain);
		if (!pgtbl_ops)
			return -ENOMEM;
	}
	...
	ret = finalise_stage_ops.finalise_stage_fn(smmu_domain,
				master, &pgtbl_cfg);
	if (ret < 0) {
...
#ifdef CONFIG_LDK_SVM
static struct mm_svm *get_svm_by_pid(u32 pid)
{
...
static u16 svm_get_domain_asid(u32 pid)
{
	struct mm_svm *svm = NULL;
	struct arm_smmu_domain *smmu_domain = NULL;
	u16 asid;
	struct mutex *mm_svm_mutex = NULL;
	struct arm_smmu_device *svm_smmu = smmu_device[SMMU_NPU];

	mm_svm_mutex = get_svm_mutex();
	mutex_lock(mm_svm_mutex);

	svm = get_svm_by_pid(pid);
	if (!svm) {
		pr_err_ratelimited("%s: invalid pid %u, no svm found!\n", __func__, pid);
		mutex_unlock(mm_svm_mutex);
		if (!svm_smmu)
			asid = (u16)(ARM_SMMU_ID_SIZE_8 - 1);
		else
			asid = (u16)((1UL << svm_smmu->asid_bits) - 1);
...
void arm_smmu_svm_tlb_inv_context(u32 pid)
{
	struct arm_smmu_cmdq_ent cmd;
	struct arm_smmu_device *svm_smmu = smmu_device[SMMU_NPU];
	...
	cmd.opcode	= CMDQ_OP_TLBI_NH_ASID;
	cmd.tlbi.asid	= svm_get_domain_asid(pid);
...
static u16 __svm_get_mm_domain_asid(struct mm_struct *mm)
{
	...
	if (!svm) {
		pr_err_ratelimited("%s: invalid mm_struct, no svm found!\n", __func__);
		mutex_unlock(mm_svm_mutex);
		if (!svm_smmu)
			asid = (u16)(ARM_SMMU_ID_SIZE_8 - 1);
		else
				...
				struct device *dev)
{
	...
	/* clear cd decs */
	cde = arm_smmu_get_cd_ptr(smmu_domain, cfg->cd.ssid);
	memset(cde, 0x0, CTXDESC_CD_DWORDS << DWORD_BYTES_NUM); /* unsafe_function_ignore: memset */
...
static int arm_smmu_set_ste(struct arm_smmu_domain *smmu_domain,
				struct device *dev)
{
	struct arm_smmu_device *smmu = NULL;
	struct arm_smmu_master_data *master = NULL;
	...
	if (smmu_domain->stage == ARM_SMMU_DOMAIN_BYPASS) {
		...
		if (smmu_domain->ssid_type == ISOLATE_SSID) {
#ifdef CONFIG_HIPCIE_CORE
			ste->s1_cfg->cd.ssid = hi_pcie_get_ssid_by_dev(dev);
#endif
			ste->s1_cfg->cd.ssid_nums = 1;
		} else {
			ste->s1_cfg->cd.ssid = arm_smmu_get_ssid(dev);
			ste->s1_cfg->cd.ssid_nums = master->ssid_nums;
		}
		ste->s1_cfg->cdcfg.tlb_flush = master->tlb_flush;
		ste->s1_cfg->cdcfg.vmid = arm_smmu_get_s1_cfg_vmid(dev, smmu_domain);
		ste->s2_cfg = NULL;
		if (arm_smmu_write_ctx_desc(smmu_domain, ste->s1_cfg)) {
			pr_err("%s write_ctx_desc fail\n", __func__);
			arm_smmu_free_cd_tables(smmu_domain);
			return -EINVAL;
		}
	} else {
				...
				struct arm_smmu_domain *smmu_domain)
{
	...
	if (master->tlb_flush != ASID_FLUSH) {
		...
		smmu_domain->s1_cfg.cd.asid = (u16)asid;
		pr_err("into %s, SSID_FLUSH mode need to allocate new asid:%d for this ssid:%d\n",
			__func__, asid,
			arm_smmu_get_ssid(dev)
			);
	}

	if (arm_smmu_set_ste(smmu_domain, dev)) {
		dev_err(dev, "%s set ste err!\n", __func__);
		return -EINVAL;
	}
...
static int arm_smmu_attach_dev(struct iommu_domain *domain, struct device *dev)
{
	...
	if (!smmu_domain->smmu) {
		...
		smmu_domain->tlb_flush = master->tlb_flush;
		smmu_domain->ssid_type = master->ssid_type;
		smmu_domain->dev = dev;
...
out_unlock:
	...
	dev_err(dev, "%s: [sid:%u] attach to SMMU, ret[%d]\n",
				__func__, master->sids[0], ret);
	return ret;
}
			 ...
			 gfp_t gfp, size_t *mapped)
{
	...
	for_each_sg(sg, s, nents, i) {
		...
		/*
		 * On HiSCv550, gid is represented by 6 bits.
		 * When PBHA is enabled, gid is splited into two parts.
		 * PBHA[3:0] is GID[3:0], paddr[41:40] is GID[5:4].
		 */
		if (PageLB(pg)) {
			/* PTE_LB here is PBHA, and it only contains the gid's bit[3:0] */
			phys |= PTE_LB(lb_page_to_gid(pg) & PGID_LOW_4_BITS_MASK);
			/* pa[41:40] contains gid bit[5:4] */
			phys |= PBHA_GID_HIGH_BIT(lb_page_to_gid(pg) & PGID_HIGH_2_BITS_MASK);
		}
#else
		if (PageLB(pg))
...
static u32 of_get_ssid_type(struct device_node *np)
{
	u32 ssid_type = SHARE_SSID;
	int ret;

	if (!np)
		return SHARE_SSID;

	ret = of_property_read_u32(np, "ssid_type", &ssid_type);
	if (ret)
		return SHARE_SSID;

	pr_err("%s:ssid_type %u\n", __func__, ssid_type);
	return ssid_type;
}
...
static int arm_smmu_add_device(struct device *dev)
{
	...
	master->ssid_bits = smmu->ssid_bits;
...
static struct iommu_device *arm_smmu_probe_device(struct device *dev)
{
	...
	if (WARN_ON_ONCE(dev_iommu_priv_get(dev))) {
		...
		master->num_sids = fwspec->num_ids;
		master->tlb_flush = (u8)of_get_tlb_flush(dev->of_node);
		master->ssid_type = (u8)of_get_ssid_type(dev->of_node);
		master->ssid_nums = of_get_ssid_nums(dev->of_node);
		ret = of_get_iova_info_smmu(dev->of_node, &master->iova);
		if (ret)
			pr_err("get dev(%s) iova info fail\n", dev_name(dev));
...
static struct iommu_group *arm_smmu_get_iommu_group_by_ssid(struct device *dev)
{
	u32 sid;
	u32 ssid = 0;
	struct arm_smmu_device *smmu = NULL;
...
#endif
#ifdef CONFIG_HIPCIE_CORE
	ssid = hi_pcie_get_ssid_by_dev(dev);
#endif
	pr_err("into %s, %s: ssid is %d\n", __func__, dev->init_name, ssid);
...
static void arm_smmu_group_set_ssid(struct device *dev,
				struct arm_smmu_iommu_group *group)
{
#if (LINUX_VERSION_CODE < KERNEL_VERSION(5, 10, 0))
	struct arm_smmu_master_data *master = dev->iommu_fwspec->iommu_priv;
#else
	...
	if (!master || (master->ssid_type != ISOLATE_SSID))
		return;

#ifdef CONFIG_HIPCIE_CORE
	group->ssid = hi_pcie_get_ssid_by_dev(dev);
#endif
	pr_err("into %s, %s: ssid is %d\n", __func__, dev->init_name, group->ssid);
	return;
}
...
static struct iommu_group *arm_smmu_device_group(struct device *dev)
{
	...
	if (master && (master->ssid_type == ISOLATE_SSID)) {
		group = arm_smmu_get_iommu_group_by_ssid(dev);
		if (group)
			return group;
...
static int arm_smmu_device_hw_probe_idr0(struct arm_smmu_device *smmu)
{
	...
	dev_err(smmu->dev, "IDR0:0x%x\n", reg);

	/* 2-level structures */
	if (IS_ENABLED(CONFIG_SMMU_SUPPORT_2_LVL_STETAB) &&
		(reg & IDR0_ST_LVL_MASK << IDR0_ST_LVL_SHIFT)
		 == IDR0_ST_LVL_2LVL) {
		dev_err(smmu->dev, "smmuid[%d] init 2 level strtab\n",
					smmu->smmuid);
	...
	/* 2-level cd */
	if (IS_ENABLED(CONFIG_SMMU_SUPPORT_2_LVL_CDTAB) &&
		(reg & IDR0_CD2L)) {
		dev_err(smmu->dev, "smmuid[%d] init 2 level cdtab\n",
					smmu->smmuid);
...
static int arm_smmu_device_hw_probe_idr1(struct arm_smmu_device *smmu)
{
	...
	/* only npu tcu can enable ste/cd l2 */
	if (!is_npu_smmuid(smmu->smmuid)) {
		ste_force_linear = 1;
		cd_force_linear = 1;
		dev_err(smmu->dev, "smmuid[%d] force to linear ste/cd tab\n",
						smmu->smmuid);
	}
	...
	if (smmu->sid_bits <= STRTAB_SPLIT || ste_force_linear) {
		...
		smmu->features &= ~ARM_SMMU_FEAT_2_LVL_STRTAB;
		dev_err(smmu->dev, "smmuid[%d] init linear level strtab\n",
						smmu->smmuid);
	}

	if (smmu->ssid_bits <= CTXDESC_SPLIT || cd_force_linear) {
		smmu->ssid_bits = CTXDESC_SURPPOT_SPLIT;
		smmu->features &= ~ARM_SMMU_FEAT_2_LVL_CDTAB;
		dev_err(smmu->dev, "smmuid[%d] init linear level cdtab\n",
						smmu->smmuid);
	}
				...
				struct arm_smmu_device *smmu)
{
	...
	if (of_property_read_bool(dev->of_node, "hypervisor_smmu"))
		smmu->hypervisor_smmu = true;
	else
		smmu->hypervisor_smmu = false;

	if (of_find_property(dev->of_node, "himatrix_bypass_tcu", NULL))
		smmu->himatrix_bypass_tcu = true;
	else
		smmu->himatrix_bypass_tcu = false;

	if (of_find_property(dev->of_node, "ecc_enable", NULL))
		smmu->ecc_enable = true;
	else
		smmu->ecc_enable = false;
	...
	if (smmuid < SMMU_MAX_ID)
		smmu_device[smmuid] = smmu;
...
#ifdef CONFIG_SMMU_SUPPORT_ECC
static void arm_smmu_ecc_detect_multpl_handler(void *data)
{
	...
	ecc_type = readl_relaxed(smmu->base + ARM_SMMU_IRPT_STAT_NS);
	ecc_bit = TCU_MEM_ERR_MULTI | TCU_MEM_ERR_DETECT;
	writel_relaxed(ecc_bit, smmu->base + ARM_SMMU_IRPT_CLR_NS);
	spin_unlock(&smmu->tcu_lock);

	if (ecc_type & TCU_MEM_ERR_MULTI) {
		bbox_diaginfo_record(ECC_SMMU, NULL, "tcu %u ecc error: 2bit\n",
			smmu->smmuid);
		dev_err(smmu->dev, "ecc_multpl_handler, ecc_type(0x78) = %d\n", ecc_type);
	}

	if (ecc_type & TCU_MEM_ERR_DETECT) {
		bbox_diaginfo_record(ECC_SMMU, NULL, "tcu %u ecc error: 1bit\n",
			smmu->smmuid);
		dev_err(smmu->dev, "ecc_detect_handler, ecc_type(0x78) = %d\n", ecc_type);
	}

	return;
}

static irqreturn_t arm_smmu_ecc_irq_handler(int irq, void *dev)
{
	unused(irq);

	arm_smmu_ecc_detect_multpl_handler(dev);
	return IRQ_HANDLED;
}

static void arm_smmu_get_ecc_irq(struct platform_device *pdev,
					int smmuid, int *irq1bit, int *irqmbit)
{
	switch (smmuid) {
	case SMMU_MEDIA1:
	case SMMU_MEDIA2:
	case SMMU_GPU:
	case SMMU_CFGDMA:
		*irq1bit = platform_get_irq_byname(pdev, "ecc-1bit");
		*irqmbit = platform_get_irq_byname(pdev, "ecc-mulbit");
		break;
	case SMMU_NPU:
		*irq1bit = platform_get_irq(pdev, 0);
		*irqmbit = platform_get_irq(pdev, 1);
		break;
	default:
		break;
	}
...
static void arm_smmu_set_common_ecc_irq(struct platform_device *pdev,
					struct arm_smmu_device *smmu)
{
	int irq_detect = 0;
	int irq_multi = 0;
	...
	spin_lock_irqsave(&smmu->tcu_lock, flags);
	if (smmu->ecc_irq_init) {
		spin_unlock_irqrestore(&smmu->tcu_lock, flags);
		return;
	}

	smmu->ecc_irq_init = true;
	spin_unlock_irqrestore(&smmu->tcu_lock, flags);

	arm_smmu_get_ecc_irq(pdev, smmu->smmuid, &irq_detect, &irq_multi);
	if (irq_detect > 0 && smmu->ecc_1bit_irq == 0) {
		smmu->ecc_1bit_irq = irq_detect;
		ret = devm_request_threaded_irq(smmu->dev, irq_detect,
					arm_smmu_ecc_irq_handler,
					NULL,
					IRQF_ONESHOT,
					"arm-smmu-v3-ecc-1bit-irq", smmu);
		if (ret < 0)
			dev_warn(smmu->dev, "failed to enable ecc_1bit irq\n");
	}

	if (irq_multi > 0 && smmu->ecc_mulbit_irq == 0) {
		smmu->ecc_mulbit_irq = irq_multi;
		ret = devm_request_threaded_irq(smmu->dev, irq_multi,
					arm_smmu_ecc_irq_handler,
					NULL,
					IRQF_ONESHOT,
					"arm-smmu-v3-ecc-mulbit-irq", smmu);
		if (ret < 0)
			dev_warn(smmu->dev, "failed to enable ecc_mulbit irq\n");
	}

	return;
}

static void arm_smmu_set_hsdt_ecc_irq(struct platform_device *pdev,
					struct arm_smmu_device *smmu)
{
	struct device *dev = &pdev->dev;
	int bit;
	int hsdt_bus_index = HSDT_BUS_0;
	unsigned long flags;
	...
	if (of_property_read_u32(dev->of_node, "ecc-l2-bit", &bit)) {
		dev_err(smmu->dev, "missing ecc-l2-bit\n");
		return;
	}

	if (smmu->smmuid == SMMU_HSDT1)
		hsdt_bus_index = HSDT_BUS_1;

	if (smmu->ecc_1bit_irq == 0) {
		smmu->ecc_1bit_irq = BIT(bit);
		hsdt_bus_ecc_trigger_register(hsdt_bus_index, DETECT_ECC_IRQ, BIT(bit),
									arm_smmu_ecc_detect_multpl_handler, (void *)smmu);
	}

	if (smmu->ecc_mulbit_irq == 0) {
		smmu->ecc_mulbit_irq = BIT(bit);
		hsdt_bus_ecc_trigger_register(hsdt_bus_index, MULTPL_ECC_IRQ, BIT(bit),
									arm_smmu_ecc_detect_multpl_handler, (void *)smmu);
	}
}

static void arm_smmu_set_ecc_irq(struct arm_smmu_device *smmu)
{
	int smmuid = smmu->smmuid;
	struct device *dev = smmu->dev;
	...
	case SMMU_CFGDMA:
	case SMMU_NPU:
		arm_smmu_set_common_ecc_irq(pdev, smmu);
		break;
	case SMMU_PCIE0:
	case SMMU_PCIE1:
	case SMMU_HSDT0:
	case SMMU_HSDT1:
		arm_smmu_set_hsdt_ecc_irq(pdev, smmu);
		break;
	default:
		break;
	}
...
bool arm_smmu_tcu_is_disable(int smmuid)
{
	struct arm_smmu_device *smmu = NULL;
	bool is_disable = true;

	if (smmuid >= SMMU_MAX_ID) {
		pr_err("%s, invalid smmuid %d\n", __func__, smmuid);
		return is_disable;
	}

	smmu = smmu_device[smmuid];
	if (smmu && smmu->status == ARM_SMMU_ENABLE)
		is_disable = false;
...
int arm_smmu_tcu_poweron(struct arm_smmu_device *smmu)
{
	...
	if (smmu->status == ARM_SMMU_ENABLE) {
		smmu->cnt++;
		dev_info(smmu->dev, "%s, status is enable, cnt is %u\n", __func__, smmu->cnt);
		spin_unlock_irqrestore(&smmu->tcu_lock, flags);
		return 0;
	}
...
#ifdef CONFIG_SMMU_SUPPORT_ECC
	if (!ret && smmu->ecc_enable == true)
		arm_smmu_set_ecc_irq(smmu);
#endif
...
int arm_smmu_tcu_poweroff(struct arm_smmu_device *smmu)
{
	...
	if (--smmu->cnt) {
		dev_info(smmu->dev, "%s, status is still enable, cnt is %lu\n", __func__, smmu->cnt);
		goto out_unlock;
	}
...
static int rda_fake_smmu_device_probe(struct platform_device *pdev)
{
	/* init Media2 TCU bypass STE for avoid poweron failed */
	pr_info("into %s\n", __func__);
	return 0;
}

static const struct of_device_id rda_fake_smmu_of_match[] = {
	{ .compatible = "fake,smmu-v3", },
	{ },
};
MODULE_DEVICE_TABLE(of, rda_fake_smmu_of_match);

static struct platform_driver rda_fake_smmu_driver = {
	.driver	= {
		.name		= "fake-smmu-v3",
		.of_match_table	= of_match_ptr(rda_fake_smmu_of_match),
	},
	.probe	= rda_fake_smmu_device_probe,
};

static int __init rda_fake_smmu_init(void)
{
	int ret = 0;

	pr_info("into %s\n", __func__);
	ret = platform_driver_register(&rda_fake_smmu_driver);
	if (ret) {
		pr_err("%s failed, ret:%d\n", __func__, ret);
		return -EINVAL;
	}
...
late_initcall_sync(rda_fake_smmu_init);
#endif
