...
void mm_iova_dom_info(struct device *dev)
{
	...
	for (n = rb_first(&cookie->iova_root); n; n = rb_next(n)) {
		...
		pr_err_ratelimited("%s: iova:0x%lx size:0x%lx, pid:%d\n",
	...
	int entry_nums, size_t size, int prot, unsigned long *out_size)
{
	...
	if (!cookie->vm_iova_pool) {
		dev_warn(dev, "%s, vm_iova_pool is null, use iova_pool!\n", __func__);
		cookie->vm_iova_pool = cookie->iova_pool;
	}
...
{
#else
static inline int mm_idle_display_lb_attach(struct mm_heap_helper_buffer *buffer,
		u32 plc_id)
{
#endif
	int ret = 0;
...
{
#else
static void mm_idle_display_lb_detach(struct mm_heap_helper_buffer *buffer, u32 plc_id)
{
#endif
	struct sg_table *table = buffer->sg_table;
				...
				size_t l3size, size_t lbsize)
{
	...
	if (mm_idle_display_lb_attach(dmabuf->priv, policy_id)) {
		pr_err("idle display lb attach failed!\n");
		goto free_iova;
	}
