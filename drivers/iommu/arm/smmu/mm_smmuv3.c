...
#ifdef CONFIG_LDK_SVM
static DEFINE_MUTEX(svm_pid_list_lock);
LIST_HEAD(svm_pid_list);
struct svm_pid {
	u32 pid; // task->tgid;
...
bool is_svm_task(u32 pid)
{
	...
	mutex_lock(&svm_pid_list_lock);
	list_for_each(pos, &svm_pid_list) {
		tmp = list_entry(pos, struct svm_pid, list);
		if (tmp->pid == pid) {
			mutex_unlock(&svm_pid_list_lock);
			return true;
		}
	}
	mutex_unlock(&svm_pid_list_lock);
...
int mm_svm_flag_set(u32 pid, u32 flag)
{
	struct list_head *pos = NULL;
	...
	if (flag) {
		...
		arm_smmu_svm_tlb_inv_context(pid);
		svm_pid_ent->pid = pid;
		list_add_tail(&svm_pid_ent->list, &svm_pid_list);
	} else {
		list_for_each_safe(pos, tmp, &svm_pid_list) {
			svm_pid_ent = list_entry(pos, struct svm_pid, list);
			if (svm_pid_ent->pid == pid) {
...
static void mm_svm_flush_tlb_by_pid(u32 pid)
{
	if (!pid) {
		mm_svm_print(SVM_ERROR, "%s pid NULL!\n", __func__);
		return;
	}
				...
				u32 pid)
{
	...
	(void)mm_svm_flush_tlb_by_pid(pid);
...
int mm_svm_flush_cache(u32 pid, unsigned long addr, size_t size)
{
	...
	arm_smmu_svm_tlb_inv_context(pid); /* flush tlb when mmap */
