...
#ifdef CONFIG_MM_RDA_SMMU
	struct gen_pool *vm_iova_pool;
#endif
	struct iommu_domain *domain;
	struct iommu_domain_data iova;
#ifdef CONFIG_MM_RDA_SMMU
	struct iommu_domain_data vm_iova;
#endif
...
#else
static inline unsigned long mm_viommu_map_entry(struct device *dev, struct map_entry *m_entry,
	int entry_nums, size_t size, int prot, unsigned long *out_size)
{
	return 0;
}
static inline int mm_viommu_unmap(struct device *dev, unsigned long iova, size_t size)
{
	return 0;
}
static inline struct device *get_smmu_dev(int smmuid)
{
	return NULL;
}
#endif
