						  ...
						  int use_sched_clock)
{
	...
	CALL_HCK_VH(lpcpu_timer_sp804_clock_init, clkevt, &vendor_flag);
	if (vendor_flag == 0) {
...
static inline void timer_shutdown(struct clock_event_device *evt)
{
	...
	CALL_HCK_VH(lpcpu_timer_sp804_shutdown, common_clkevt, &vendor_flag);
	if (vendor_flag == 0)
				  ...
				  struct clk *clk, const char *name)
{
	...
	CALL_HCK_VH(lpcpu_timer_sp804_clockevents_init, common_clkevt, &vendor_flag);
	if (vendor_flag == 0)
		writel(0, common_clkevt->ctrl);
...
static void __init sp804_clkevt_init(struct sp804_timer *timer, void __iomem *base)
{
	...
	for (i = 0; i < NR_TIMERS; i++) {
		...
		CALL_HCK_VH(lpcpu_timer_sp804_clkevt_init, clkevt, timer_base, timer);
...
#ifdef CONFIG_LPCPU_HITIMER
	hck_lpcpu_timer_register();
#endif
	/* Ensure timers are disabled */
	CALL_HCK_VH(lpcpu_timer_sp804_of_timers_disable, timer1_base, timer2_base,
		    timer->ctrl, &vendor_flag);
	if (vendor_flag == 0) {
...
static int __init integrator_cp_of_init(struct device_node *np)
{
	...
	CALL_HCK_VH(lpcpu_timer_sp804_timers_disable, base, arm_sp804_timer.ctrl,
		    &vendor_flag);
	if (vendor_flag == 0)
