...
{
	u64 iowait;
	u64 io_wait_time = -1ULL;

	if (cpu_online(cpu))
		io_wait_time = get_cpu_iowait_time_us(cpu, NULL);

	if (io_wait_time == -1ULL)
		/* !NO_HZ or cpu offline so we can rely on cpustat.iowait */
		iowait = kcpustat_cpu(cpu).cpustat[CPUTIME_IOWAIT];
	else
#if (KERNEL_VERSION(4, 14, 0) <= LINUX_VERSION_CODE)
		iowait = io_wait_time * NSEC_PER_USEC;

#else
		iowait = usecs_to_cputime64(io_wait_time);
#endif
	return iowait;
}
...
#ifdef CONFIG_CHIP_HONGHU
		high_load_tick_mask((unsigned char)(0xff << (CONFIG_NR_CPUS / 2)), CPUSET_ROOT,
...
static void get_current_task_mask(u32 type)
{
	...
	for (cpu_index = 0; cpu_index < CONFIG_NR_CPUS; cpu_index++) {
		...
		if (((1 << cpu_index) & cpumask) == 0)
			continue;

		rq_cur = cpu_rq(cpu_index);
		if (!rq_cur) {
			pr_info("cpuload: cpu:%u rq_cur is NULL!", cpu_index);
			continue;
		}

		p = rq_cur->curr;
		if (!p) {
			pr_info("cpuload: cpu:%u p is NULL!", cpu_index);
			continue;
		}
