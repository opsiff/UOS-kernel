...
{
	if (flow->sk != NULL) {
		if ((flow->flow_ctx.flow_msg.dev_if != flow->sk->sk_bound_dev_if) ||
		flow->sk->sk_err == ECONNABORTED || sock_flag(flow->sk, SOCK_DEAD)) {
			...
			flow->flow_ctx.flow_stat.sock_state = flow->sk->sk_state;
		}

		if (flow->sk->sk_protocol == IPPROTO_TCP) {
		...
		flow->flow_ctx.flow_msg.sk_mark = flow->sk->sk_bound_dev_if;
	} else {
		flow->flow_ctx.flow_stat.sock_state = 0;
		flow->flow_ctx.flow_msg.sk_mark = 0;
	}
}
...
static void fi_flow_stat_log(const struct fi_flow_node *node)
{
	if (node->flow_ctx.flow_msg.l3proto == ETH_P_IP) {
		fi_logi("dev: %s, sk_mark:%u, SrcIP: "IPV4_FMT", SrcPort: %hu, DstIP: "IPV4_FMT", DstPort: %hu, "
			"Proto: %u, SockState: %u, sk: %pK, UpPktNum: %d, DownPktNum: %d, up_byte: %d, down_byte: %d, "
			"raw_up_pkt_byte: %d, raw_down_pkt_byte: %d, rtt: %u, totalRetrans: %u",
		...
		fi_logi("dev: %s, sk_mark:%u, SrcIP: "IPV6_FMT", SrcPort: %hu, DstIP: "IPV6_FMT", DstPort: %hu, "
			"Proto: %u, SockState: %u, sk: %pK, UpPktNum: %d, DownPktNum: %d, up_byte: %d, down_byte: %d, "
			"raw_up_pkt_byte: %d, raw_down_pkt_byte: %d, rtt: %u, totalRetrans: %u",
			node->flow_ctx.flow_msg.dev, node->flow_ctx.flow_msg.sk_mark,
