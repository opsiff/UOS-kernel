...
static bool emcom_mproute_need_reset(struct sock *sk, const uint32_t rst_to_ifindex, const uint32_t rst_from_ifindex)
{
	if (sk->sk_bound_dev_if == rst_to_ifindex)
		return false;

	if (!rst_from_ifindex && (sk->sk_bound_dev_if != rst_from_ifindex))
		return false;
...
static void emcom_mproute_path_handover(struct sock *sk, uint32_t ifindex)
{
	struct inet_sock *inet = NULL;

	if (!emcom_is_valid_dev_if(sock_net(sk), ifindex)) {
		emcom_loge("ifindex: %d is not valid", ifindex);
		return;
	}
	...
	emcom_logi("path_handover sk:%pK sport[%u] dev_if[%d] bind[%d] family[%u] ifindex[%u]",
		sk, sk->sk_num, sk->sk_bound_dev_if,
		sk->is_mp_flow_bind, sk->sk_family, ifindex);
