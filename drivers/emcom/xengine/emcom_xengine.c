...
static const struct emcom_xengine_event_ext g_emcom_xengine_handle_event_list_ext[] = {
	...
	{NETLINK_EMCOM_DK_DSCP_START, emcom_xengine_dscp_config_start},
	{NETLINK_EMCOM_DK_DSCP_UPDATE_APP_INFO, emcom_xengine_dscp_update_app_info}
};
...
void emcom_xengine_evt_proc(int32_t event, const uint8_t *data, uint16_t len)
{
	...
	case NETLINK_EMCOM_DK_MPF_VLAN_CLEAR:
		mpflow_vlan_clear();
		return;
	case NETLINK_EMCOM_DK_DSCP_CLEAR:
		emcom_xengine_dscp_clear();
		return;
	}
