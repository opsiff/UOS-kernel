...
#define DECOMPOSE_WORK_DELAY_SHORT		(HZ / 2)
#define DECOMPOSE_WORK_DELAY_LONG		(3 * HZ)
...
static void __dubai_schedule_decompose_work_locked(unsigned long delay)
{
...
static void dubai_delay_schedule_decompose_work_locked(unsigned long delay)
{
	__dubai_schedule_decompose_work_locked(delay);
}
...
static void dubai_schedule_decompose_work_locked(void)
{
	...
	delay = ((decompose_check_retry % DECOMPOSE_RETRY_PERIOD) != 0) ?
		DECOMPOSE_WORK_DELAY_SHORT : DECOMPOSE_WORK_DELAY_LONG;
	__dubai_schedule_decompose_work_locked(delay);
}
...
void dubai_remove_proc_decompose(pid_t tgid)
{
	...
	if (entry != NULL) {
		...
		entry->tgid = -1;
		decompose_count--;
		dubai_delay_schedule_decompose_work_locked(DECOMPOSE_WORK_DELAY_LONG);
	}
	mutex_unlock(&dubai_proc_decompose_lock);
}
