...
enum {
	...
	DUBAI_BATTERY_PROP_CHARGE_FULL_DESIGN = 13,
};
...
static int dubai_get_design_capacity(void)
{
	int amp;
	int val;

	amp = g_batt_ops ? g_batt_ops->charge_full_amp : DEFAULT_CHARGE_FULL_AMP;
	val = dubai_get_psy_intprop(g_batt_psy, POWER_SUPPLY_PROP_CHARGE_FULL_DESIGN, -1) / amp;
	dubai_info("Get battery design capacity: %d", val);
	return val;
}
...
static int dubai_get_battery_prop(void __user *argp) {
	...
	case DUBAI_BATTERY_PROP_CHARGE_STATUS:
		...
		break;
	case DUBAI_BATTERY_PROP_CHARGE_FULL_DESIGN:
		propval.value = dubai_get_design_capacity();
		break;
	default:
