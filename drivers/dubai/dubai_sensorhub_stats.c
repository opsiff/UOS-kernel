...
{
	int ret;
	struct aod_info info_aod;
	struct aod_dd_info dd_info;
	struct aod_working_state_monitor stat;

	ret = dubai_fetch_sensorhub_data(DUBAI_EVENT_AOD_INFO, &info_aod, sizeof(struct aod_info));
	if (ret < 0)
		return ret;

	ret = dubai_fetch_sensorhub_data(DUBAI_EVENT_AOD_BL_INFO, &dd_info, sizeof(struct aod_dd_info));
	if (ret < 0)
		return ret;

	stat.info_data = info_aod;
	stat.dd_info = dd_info;
