...
#define HIVIEW_BIN_NAME			"/bin/hiview"
...
static bool check_cmdline()
{
	char buffer[CMDLINE_LEN];
	int ret = dubai_get_cmdline(current, buffer, CMDLINE_LEN);
	if (ret != 0) {
		dubai_err("failed to get_cmdline");
		return false;
	}
	if (!strstr(buffer, HIVIEW_BIN_NAME))
		return false;
...
static long dubai_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	if (!strstr(comm, DUBAID_NAME) && !strstr(comm, HIVIEW_NAME) && !check_cmdline()) {
		dubai_err("failed to ioctl comm: %s", comm);
		return -EPERM;
	}
#endif
	rc = 0;
