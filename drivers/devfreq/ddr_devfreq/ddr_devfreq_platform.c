...
#include <platform_include/cee/linux/tele_mntn.h>
#endif
...
#endif
struct ddr_devfreq_device *g_ddev = NULL;
...
unsigned long ddr_devfreq_get_cur_freq(void)
{
	unsigned long freq = 0;
	if (g_ddev == NULL)
		return 0;

	freq = clk_get_rate(g_ddev->get);

	return freq;
}
EXPORT_SYMBOL(ddr_devfreq_get_cur_freq);
