...
sd_read_write_protect_flag(struct scsi_disk *sdkp, unsigned char *buffer)
{
	...
	if (!scsi_status_is_good(res)) {
		...
		if (storage_rochk_filter_sd(sdp)) {
			unsigned int bootopt = get_storage_rofa_bootopt();
			storage_rochk_record_disk_wp_status(sdkp->disk->disk_name, sdkp->write_prot);
			if (sdkp->write_prot && bootopt == STORAGE_ROFA_BOOTOPT_BYPASS) {
				sd_printk(KERN_NOTICE, sdkp,"Reset Write Protect\n");
				sdkp->write_prot = 0;
			}
		}
#endif
...
#ifdef CONFIG_MAS_BLK
		if(sdp->host->queue_quirk_flag & SHOST_QUIRK(SHOST_QUIRK_HUFS_MQ)) {
			blk_queue_flag_set(QUEUE_FLAG_NONROT, q);
			blk_queue_flag_clear(QUEUE_FLAG_ADD_RANDOM, q);
		}
#endif
...
#ifdef CONFIG_SCSI_DISK_PARTITION_DRIFT
	if (sdp->host->hostt->name && strcmp(sdp->host->hostt->name, "ufshcd") == 0 && index < 6)
		index = sdp->lun;
#endif
...
#ifdef CONFIG_HUAWEI_STORAGE_ROFA_EXTEND
	if (storage_rochk_filter_sd(sdp)) {
		storage_rochk_record_disk(gd->disk_name, gd->major, gd->first_minor);
