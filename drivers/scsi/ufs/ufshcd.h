...
#include <platform_include/basicplatform/linux/ufs/dfa_inf.h>
#endif
...
struct debugfs_files {
	...
	struct dentry *mcq_enable;
	struct dentry *esi_enable;
#endif
...
void ufshcd_mcq_send_command(struct ufs_hba *hba, unsigned int task_tag, struct ufshcd_lrb *lrbp);
void ufshcd_mcq_transfer_req_compl_all(struct ufs_hba *hba, bool io_abort);
void ufshcd_mcq_dump(struct ufs_hba *hba);
int ufshcd_mcq_abort(struct scsi_cmnd *cmd);
int ufshcd_mcq_sq_cleanup(struct ufs_hba *hba, int task_tag);
void ufshcd_init_mcq_hwq_slot(struct ufs_hba *hba);
#else
static inline int ufshcd_config_mcq(struct ufs_hba *hba, bool async)
{
	return 0;
}
static inline irqreturn_t ufshcd_handle_mcq_cq_events(struct ufs_hba *hba)
{
	return IRQ_HANDLED;
}
static inline void ufshcd_mcq_send_command(struct ufs_hba *hba, unsigned int task_tag, struct ufshcd_lrb *lrbp)
{
}
static inline void ufshcd_mcq_transfer_req_compl_all(struct ufs_hba *hba, bool io_abort)
{
}
static inline  void ufshcd_mcq_dump(struct ufs_hba *hba)
{
}
static inline int ufshcd_mcq_abort(struct scsi_cmnd *cmd)
{
	return 0;
}
static inline int ufshcd_mcq_sq_cleanup(struct ufs_hba *hba, int task_tag)
{
	return 0;
}
static inline void ufshcd_init_mcq_hwq_slot(struct ufs_hba *hba)
{
}
#endif

#ifdef CONFIG_DFA_MCQ
static inline bool is_dfa_enabled(struct ufs_hba * hba)
{
	return hba->dhba.dfa_enabled;
}
#else
static inline bool is_dfa_enabled(struct ufs_hba * hba)
{
