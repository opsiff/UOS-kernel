...
static int scsi_dev_data_remap(struct request_queue *q,
				struct stor_dev_data_remap_info *data_remap_info)
{
	struct scsi_device *sdev = q->queuedata;

	if (!sdev || sdev->type != TYPE_DISK ||
		!sdev->host->hostt->dev_data_remap)
		return -EINVAL;

	if (sdev->host->host_self_blocked || sdev->sdev_state != SDEV_RUNNING)
		return -EINVAL;

	return sdev->host->hostt->dev_data_remap(sdev, data_remap_info);
}
...
static int scsi_dev_slc_mode_error_inject(struct request_queue *q,
		unsigned char error_status)
{
	struct scsi_device *sdev = q->queuedata;

	if (!sdev || sdev->type != TYPE_DISK ||
		!sdev->host->hostt->dev_slc_mode_error_inject)
		return -EINVAL;

	if (sdev->host->host_self_blocked || sdev->sdev_state != SDEV_RUNNING)
		return -EINVAL;

	return sdev->host->hostt->dev_slc_mode_error_inject(sdev,
		error_status);
}
#endif
...
void scsi_dev_unistore_register(struct Scsi_Host *shost)
{
	...
	mas_blk_mq_tagset_data_remap_register(
		&shost->tag_set, shost->hostt->dev_data_remap ?
		scsi_dev_data_remap : NULL);
	...
	mas_blk_mq_tagset_slc_mode_error_inject_register(
		&shost->tag_set, shost->hostt->dev_slc_mode_error_inject ?
		scsi_dev_slc_mode_error_inject : NULL);
#endif
}
