...
#define SPE_PMU_FEAT_FILT_NEVT			(1UL << 6)
#define SPE_PMU_FEAT_DISCARD			(1UL << 7)
#define SPE_PMU_FEAT_PMFZ				(1UL << 8)
...
#define ATTR_CFG_FLD_discard_CFG		config2
#define ATTR_CFG_FLD_discard_LO		12
#define ATTR_CFG_FLD_discard_HI		12

#define ATTR_CFG_FLD_freeze_CFG		config2
#define ATTR_CFG_FLD_freeze_LO		13
#define ATTR_CFG_FLD_freeze_HI		13

#define ATTR_CFG_FLD_nevent0_filter_CFG		config2
#define ATTR_CFG_FLD_nevent0_filter_LO		16
#define ATTR_CFG_FLD_nevent0_filter_HI		31

#define ATTR_CFG_FLD_nevent1_filter_CFG		config2
#define ATTR_CFG_FLD_nevent1_filter_LO		32
#define ATTR_CFG_FLD_nevent1_filter_HI		63
...
GEN_PMU_FORMAT_ATTR(nevent0_filter);
GEN_PMU_FORMAT_ATTR(nevent1_filter);
GEN_PMU_FORMAT_ATTR(discard);
GEN_PMU_FORMAT_ATTR(freeze);
...
static struct attribute *arm_spe_pmu_formats_attr[] = {
	...
	&format_attr_nevent0_filter.attr,
	&format_attr_nevent1_filter.attr,
	&format_attr_discard.attr,
	&format_attr_freeze.attr,
	NULL,
};
...
static u64 arm_spe_event_to_pmsfcr(struct perf_event *event)
{
	...
	if ((spe_pmu->features & SPE_PMU_FEAT_FILT_NEVT) && !(reg & BIT(SYS_PMSFCR_EL1_FE_SHIFT)))
		if (ATTR_CFG_GET_FLD(attr, nevent0_filter) || ATTR_CFG_GET_FLD(attr, nevent1_filter))
...
static u64 arm_spe_event_to_pmsnevfr0(struct perf_event *event)
{
	struct perf_event_attr *attr = &event->attr;
	return ATTR_CFG_GET_FLD(attr, nevent0_filter);
}

static u64 arm_spe_event_to_pmsnevfr1(struct perf_event *event)
{
	struct perf_event_attr *attr = &event->attr;
	return ATTR_CFG_GET_FLD(attr, nevent1_filter);
}

#define PMSNEVFR_H_OFFSET 48
static u64 arm_spe_event_to_pmsnevfr(struct perf_event *event)
{
	struct arm_spe_pmu *spe_pmu = to_spe_pmu(event->pmu);

	if (spe_pmu->features & SPE_PMU_FEAT_FILT_NEVT) {
		u64 pmsnevfr0 = arm_spe_event_to_pmsnevfr0(event);
		u64 pmsnevfr1 = arm_spe_event_to_pmsnevfr1(event);
		return pmsnevfr0 | (pmsnevfr1 << PMSNEVFR_H_OFFSET);
	}
	return 0;
}
...
static u64 __arm_spe_pmu_next_off(struct perf_output_handle *handle)
{
	...
	if (!IS_ALIGNED(head, max_comp_align)) {
		unsigned long delta = roundup(head, max_comp_align) - head;
					  ...
					  struct perf_event *event)
{
	...
	if ((spe_pmu->features & SPE_PMU_FEAT_PMFZ) && ATTR_CFG_GET_FLD(attr, freeze))
		limit |= BIT(SYS_PMBLIMITR_EL1_PMFZ_SHIFT);

	if ((spe_pmu->features & SPE_PMU_FEAT_DISCARD) && ATTR_CFG_GET_FLD(attr, discard))
		limit |= SYS_PMBLIMITR_EL1_FM_DISCARD;
...
static int arm_spe_pmu_event_init(struct perf_event *event)
{
	...
	if (arm_spe_event_to_pmsnevfr(event) & SYS_PMSNEVFR_EL1_RES0)
		return -EOPNOTSUPP;
...
static void arm_spe_pmu_start(struct perf_event *event, int flags)
{
	...
	if (!reg && (spe_pmu->features & SPE_PMU_FEAT_FILT_NEVT)) {
...
static void __arm_spe_pmu_dev_probe(void *info)
{
	...
	if (fld >= ID_AA64DFR0_PMSVER_V1P2) {
		spe_pmu->features |= SPE_PMU_FEAT_DISCARD;
		spe_pmu->features |= SPE_PMU_FEAT_PMFZ;
	}
	...
	if (max_comp_align < spe_pmu->align)
		max_comp_align = spe_pmu->align;
	...
	if (reg & BIT(SYS_PMSIDR_EL1_FNE_SHIFT))
		spe_pmu->features |= SPE_PMU_FEAT_FILT_NEVT;
