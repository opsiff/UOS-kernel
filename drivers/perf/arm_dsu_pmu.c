...
#if defined(CONFIG_ARCH_PLATFORM) && defined(CONFIG_CPU_PM)
...
#if defined(CONFIG_ARCH_PLATFORM) && defined(CONFIG_CPU_PM)
EXPORT_SYMBOL_GPL(dsu_pmu_start);
#endif
...
#if defined(CONFIG_ARCH_PLATFORM) && defined(CONFIG_CPU_PM)
EXPORT_SYMBOL_GPL(dsu_pmu_stop);
#endif
...
#if defined(CONFIG_ARCH_PLATFORM) && defined(CONFIG_CPU_PM)
EXPORT_SYMBOL_GPL(dsu_pmu_enable);
#endif
...
#if defined(CONFIG_ARCH_PLATFORM) && defined(CONFIG_CPU_PM)
EXPORT_SYMBOL_GPL(dsu_pmu_disable);
#endif
...
static struct dsu_pmu *dsu_pmu_alloc(struct platform_device *pdev)
{
	...
	CALL_HCK_VH(lpcpu_pmu_dsu_pmu_init, dsu_pmu);
	return dsu_pmu;
}
...
static int dsu_pmu_dt_get_cpus(struct device *dev, cpumask_t *mask)
{
	...
	for (; i < n; i++) {
		...
		if (!cpu_node) {
			CALL_HCK_VH(lpcpu_pmu_hook_flag, &flag);
			if (flag != 0)
				continue;
			break;
		}
...
static int dsu_pmu_device_probe(struct platform_device *pdev)
{
	...
	CALL_HCK_VH(lpcpu_pmu_cpu_pm_dsu_pmu_register, dsu_pmu, name, &rc, &vendor_flag);
	if (vendor_flag == 0)
...
static int dsu_pmu_device_remove(struct platform_device *pdev)
{
	...
	CALL_HCK_VH(lpcpu_pmu_cpu_pm_dsu_pmu_unregister, dsu_pmu);
