...
static int hiview_hisysevent_sendmsg(struct msghdr *msg, struct kvec *vec, size_t num, size_t size);

static int hiview_hisysevent_sock_reset(void);
...
int hiview_hisysevent_sendmsg(struct msghdr *msg, struct kvec *vec, size_t num, size_t size)
{
	mutex_lock(&hisysevent_init_lock);
	if (hisysevent_sock == NULL) {
		hwlog_err("hisysevent sock is null");
		mutex_unlock(&hisysevent_init_lock);
		return -EINVAL;
	}
	int ret = kernel_sendmsg(hisysevent_sock, msg, vec, num, size);
	mutex_unlock(&hisysevent_init_lock);
	return ret;
}

int hiview_hisysevent_sock_reset(void)
{
	hwlog_info("reset hisysevent sock");
	...
	int ret = hiview_send_hisysevent_init();
	mutex_unlock(&hisysevent_init_lock);
	return ret;
}
...
int hiview_hisysevent_report(struct hiview_hisysevent *event)
{
	...
	ret = hiview_hisysevent_sendmsg(&msg, &vec, SYS_SIZE_OFFSET_1, data_size);
	/* reinitialize hisysevent sender socket */
	if (ret == -ENOTCONN) {
		hwlog_err("kernel_sendmsg failed, socket is not connected");
		ret = hiview_hisysevent_sock_reset();
		if (ret < 0) {
