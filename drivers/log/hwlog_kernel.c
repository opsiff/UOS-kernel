...
#ifdef CONFIG_POWER_DUBAI_HISYSEVENT
#include <linux/workqueue.h>
#include <dfx/hiview_hisysevent.h>
#define HW_DOAMIN_DUBAI		"KERNEL_XPOWER"
#endif

#define MAX_WORK_COUNT		20
#define MAX_TAG_SIZE		64
#define MAX_MSG_SIZE		256
#define DUBAI_TAG_LEN		10
...
#ifdef CONFIG_POWER_DUBAI_HISYSEVENT
static int hwlog_send_hisysevent(struct hwlog_work *wk);
#endif
...
#ifdef CONFIG_POWER_DUBAI_HISYSEVENT
	if (!IS_ERR_OR_NULL(work) && (work->bufid == HW_LOG_ID_DUBAI)) {
		hwlog_send_hisysevent(work);
		work->waiting = 0;
		return;
	}
#endif
...
int hwdubai_send_event(const char *domain, const char *event_name, enum hisysevent_type type, const char *message)
{
	...
	const char * cut_event_name = event_name;
	if (strstr(event_name, "DUBAI_TAG"))
		cut_event_name = event_name + DUBAI_TAG_LEN;

	event = hisysevent_create(domain, cut_event_name, type);
	if (!event) {
...
int hwlog_send_hisysevent(struct hwlog_work *wk)
{
	wk->tagid[wk->taglen - 1] = '\0';
	wk->msg[wk->msglen - 1] = '\0';
	int rt = hwdubai_send_event(HW_DOAMIN_DUBAI, wk->tagid, BEHAVIOR, wk->msg);
	return rt;
}
#endif
