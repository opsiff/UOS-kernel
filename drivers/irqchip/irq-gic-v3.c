...
static asmlinkage void __exception_irq_entry gic_handle_irq(struct pt_regs *regs)
{
	...
	CALL_HCK_VH(lpcpu_cpuidle_irqinfo_update, irqnr);
...
static void __init gic_smp_init(void)
{
	...
	CALL_HCK_VH(lpcpu_gic_smp_register_sgis, gic_data.domain, -1, 16,
		    NUMA_NO_NODE, &sgi_fwspec, false, NULL, &init, &base_sgi);
	...
	CALL_HCK_VH(lpcpu_gic_smp_ipi_range, base_sgi, 16, &init);
	if (init == 0)
				 ...
				 struct fwnode_handle *handle)
{
	...
	hck_lpcpu_gic_register();
	hck_lpcpu_gic_its_register();
#endif
