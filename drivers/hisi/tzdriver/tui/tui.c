...
static phys_addr_t get_frame_addr(void)
{
	...
	if (dpu_tui_get_active_display_rect(&active_rect) != 0) {
		tloge("get active display rect failed\n");
		return 0;
	}

	check_params = (active_rect.w > MAX_SCREEN_RESOLUTION) ||  (active_rect.h > MAX_SCREEN_RESOLUTION);
	if (check_params) {
		tloge("Horizontal resolution or Vertical resolution is too large\n");
		return 0;
	}
	screen_r = active_rect.w* active_rect.h * COLOR_TYPE * BUFFER_NUM;
...
#ifdef CONFIG_TEE_TUI_DISPLAY_3_0
		while (!dpu_tui_is_power_on() && (*count) < TIME_OUT_FOWER_ON) {
#else
	...
	const struct teec_tui_parameter *tui_param)
{
	...
	mb_pack->operation.params[0].value.b = make32(active_rect.w, active_rect.h);
#else
...
#ifdef CONFIG_TEE_TUI_DISPLAY_3_0
	mb_pack->operation.params[3].value.b = make32(dpu_tui_get_rotation(), tui_param->display_state);
#else
...
#ifdef CONFIG_TEE_TUI_DISPLAY_3_0
		check_value = (status_temp != TUI_STATE_UNUSED && dpu_tui_is_power_on()) || event == TUI_POLL_FOLD;
#else
...
#ifdef CONFIG_TEE_TUI_DISPLAY_3_0
	if (atomic_read(&g_tui_state) != TUI_STATE_UNUSED && dpu_tui_is_power_on()) {
#else
