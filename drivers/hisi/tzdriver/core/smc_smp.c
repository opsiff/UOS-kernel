...
{
	return g_reserved_cmd_buffer;
}
#endif

#ifdef CONFIG_TEE_RESET
static bool g_vm_first_start = true;
static int __init tz_check_vm_first_start_func(char *str)
{
	if (str != NULL && *str == '0')
		g_vm_first_start = false;
	return 0;
}
early_param("vm_first_start_flag", tz_check_vm_first_start_func);
#endif
...
void smc_set_cmd_buffer(void)
{
	 ...
	 */
#ifdef CONFIG_TEE_RESET
	if (g_reserved_cmd_buffer && g_vm_first_start)
		return;
#else
