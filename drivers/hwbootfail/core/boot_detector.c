	...
	struct bootfail_log_header *phdr)
{
	...
	run_param.adp.sec_funcs.memcpy_s(phdr->version,
		(unsigned long)sizeof(phdr->version),
		pparam->version,
		(unsigned long)sizeof(phdr->version));
#endif
}

#if defined(CONFIG_LIBLINUX) || defined(CONFIG_OHOS_HM)
static int need_save_logs(struct bootfail_proc_param *pparam)
{
	int is_bootloader = run_param.adp.is_bootloader();
	/*
	 * For the upper layer bootfail, OHOS_HM call save_logs in liblinux
	 * and set log_saved_in_upper_layer to 1, UEFI need to call save_logs
	 * only if log_saved_in_upper_layer is 0.
	 */
	if (is_bootloader == 0)
		pparam->binfo.log_saved_in_upper_layer = 1;

	return (is_bootloader == 0) || (pparam->binfo.log_saved_in_upper_layer == 0);
}
#else
static int need_save_logs(struct bootfail_proc_param *pparam)
{
	...
	return run_param.adp.is_bootloader();
}
#endif
