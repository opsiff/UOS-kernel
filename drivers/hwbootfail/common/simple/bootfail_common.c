...
static struct work_struct bbox_work;
...
static void add_bbox_notify(struct work_struct *work)
{
	send_bbox_notify();
}
	...
	unsigned long arg)
{
	...
	case KERNEL_BBOX_NOTIFY:
		...
		INIT_WORK(&bbox_work, add_bbox_notify);
		schedule_work(&bbox_work);
		return 0;
	default:
		print_err("Invalid CMD: 0x%x\n", cmd);
		ret = -EFAULT;
...
{
	return (cmd == PROCESS_SHUTFAIL || cmd == GET_SFI_DEV_PATH || cmd == SET_REBOOT || cmd == GET_SFI_DEV_OFFSET);
}

static void process_shut_start(enum shut_stage stage)
{
	if (stage != SHUT_STAGE_NATIVE_START)
	return;
...
static int process_upper_shutfail(unsigned long arg)
{
	...
	memset_s((void *)&param, sizeof(param), 0, sizeof(param));
	if (copy_from_user(&param, ((struct shutfail_proc_param *)(uintptr_t)arg), sizeof(param)) != 0) {
		print_err("copy_from_user failed!\n");
		return -EFAULT;
	}
	...
	tsk = kthread_run(__process_upper_shutfail, (void *)&param, "proc upper shutfail");
	if (IS_ERR(tsk)) {
		print_err("kthread_run failed!\n");
		return -1;
	}
...
static long ioctl_shut_detector(struct file *file, unsigned int cmd, unsigned long arg)
{
	long ret = 0;
	int is_reboot = -1;
	struct bfi_dev_path *path = (struct bfi_dev_path *)(uintptr_t)arg;
	...
	case GET_SFI_DEV_PATH:
		if (get_sfi_dev_path() == NULL)
			return -ENODEV;
		/*lint -e666*/
		if (copy_to_user(path->path, get_sfi_dev_path(), min(strlen(get_sfi_dev_path()), sizeof(path->path) - 1)) !=
			0) {
			print_err("Failed to copy bfi dev path to user!\n");
			return -EFAULT;
		}
	...
	case GET_SFI_DEV_OFFSET:
		...
		if (copy_to_user((int *)(uintptr_t)arg, &offset, sizeof(offset)) != 0) {
			print_err("copy_to_user failed!\n");
			ret = -EFAULT;
			break;
		}
	...
	case SET_REBOOT:
		if (copy_from_user(&is_reboot, (int *)(uintptr_t)arg, sizeof(is_reboot)) != 0) {
			print_err("copy_from_user failed!\n");
			ret = -EFAULT;
			break;
		}
...
static long ioctl_shut_stage(struct file *file, unsigned int cmd, unsigned long arg)
{
	long ret = 0;
	...
	case GET_SHUT_STAGE:
		...
		if (copy_to_user((enum shut_stage *)(uintptr_t)arg, &stage, sizeof(stage)) != 0) {
			print_err("copy_to_user failed!\n");
			ret = -EFAULT;
			break;
		}
		break;
	case SET_SHUT_STAGE:
		if (copy_from_user(&stage, (int *)(uintptr_t)arg, sizeof(stage)) != 0) {
			print_err("copy_from_user failed!\n");
			ret = -EFAULT;
			break;
		}
	...
	unsigned long arg, unsigned int *is_used)
{
	*is_used = 1;
	...
	if (is_cmd_of_dynamic_brd(cmd))
		...
		return ioctl_shut_stage(file, cmd, arg);
	else if (is_cmd_of_shut_detector(cmd))
		return ioctl_shut_detector(file, cmd, arg);
#endif
