...
static int power_cali_sysfs_get_dc_data(enum power_cali_sysfs_type type, char *buf)
{
	int value = -1;
	char temp_buf[POWER_CALI_WR_BUF_SIZE] = { 0 };

	if (type == POWER_CALI_SYSFS_DC_SHOW) {
		if (power_cali_show_data("dc", temp_buf, POWER_CALI_WR_BUF_SIZE - 1))
			return -EINVAL;
		return scnprintf(buf, POWER_CALI_WR_BUF_SIZE, "%s\n", temp_buf);
	}

	power_cali_get_data("dc", type - POWER_CALI_SYSFS_DC_CUR_A, &value);
	return scnprintf(buf, POWER_CALI_WR_BUF_SIZE, "%d\n", value);
}

static int power_cali_sysfs_get_coul_data(enum power_cali_sysfs_type type, char *buf)
{
	int value = -1;

	if (type == POWER_CALI_SYSFS_COUL_MODE)
		power_cali_get_mode("coul", &value);
	else
		power_cali_get_data("coul", type - POWER_CALI_SYSFS_COUL_CUR_A, &value);

	return scnprintf(buf, POWER_CALI_WR_BUF_SIZE, "%d\n", value);
}

static int power_cali_sysfs_get_wltrx_curr_data(enum power_cali_sysfs_type type, char *buf)
{
	int value = -1;
	int ret = 0;
	int a = 0;
	...
	if (type == POWER_CALI_SYSFS_WLTRX_CURR_SUPPORT) {
		if (power_cali_get_data("wltrx_curr", type - POWER_CALI_SYSFS_WLTRX_CURR_SUPPORT, &value))
			return -EINVAL;
		return scnprintf(buf, POWER_CALI_WR_BUF_SIZE, "%d\n", value);
	}

	if (type == POWER_CALI_SYSFS_WLTRX_CURR_GET_PARA) {
		ret = power_cali_get_data("wltrx_curr", type - POWER_CALI_SYSFS_WLTRX_CURR_SUPPORT, &a);
		ret += power_cali_get_data("wltrx_curr", type - POWER_CALI_SYSFS_WLTRX_CURR_SUPPORT + 1, &b);
		if (ret)
			return -EINVAL;
...
static int power_cali_sysfs_set_dc_data(enum power_cali_sysfs_type type, int value)
{
	int ret = 0;

	if (type == POWER_CALI_SYSFS_DC_SAVE)
		ret = power_cali_save_data("dc");
	else
		power_cali_set_data("dc", type - POWER_CALI_SYSFS_DC_CUR_A, value);

	return ret;
}

static int power_cali_sysfs_set_coul_data(enum power_cali_sysfs_type type, int value)
{
	int ret = 0;

	if (type == POWER_CALI_SYSFS_COUL_SAVE)
		ret = power_cali_save_data("coul");
	else if (type == POWER_CALI_SYSFS_COUL_MODE)
		power_cali_set_mode("coul", value);
	else
		power_cali_set_data("coul", type - POWER_CALI_SYSFS_COUL_CUR_A, value);

	return ret;
}

static int power_cali_sysfs_set_wltrx_curr_data(enum power_cali_sysfs_type type, int value)
{
	return power_cali_set_data("wltrx_curr", type - POWER_CALI_SYSFS_WLTRX_CURR_IC_TYPE, value);
}

static struct power_cali_sysfs_handler {
	enum power_cali_sysfs_type type;
	int (*store_handler)(enum power_cali_sysfs_type type, int value);
	int (*show_handler)(enum power_cali_sysfs_type type, char *buf);
} const g_sysfs_handler[] = {
	{ POWER_CALI_SYSFS_DC_CUR_A, power_cali_sysfs_set_dc_data, power_cali_sysfs_get_dc_data },
	{ POWER_CALI_SYSFS_DC_CUR_B, power_cali_sysfs_set_dc_data, power_cali_sysfs_get_dc_data },
	{ POWER_CALI_SYSFS_AUX_SC_CUR_A, power_cali_sysfs_set_dc_data, power_cali_sysfs_get_dc_data },
	{ POWER_CALI_SYSFS_AUX_SC_CUR_B, power_cali_sysfs_set_dc_data, power_cali_sysfs_get_dc_data },
	{ POWER_CALI_SYSFS_DC_SAVE, power_cali_sysfs_set_dc_data, NULL },
	{ POWER_CALI_SYSFS_DC_SHOW, NULL, power_cali_sysfs_get_dc_data },
	{ POWER_CALI_SYSFS_COUL_CUR_A, power_cali_sysfs_set_coul_data, power_cali_sysfs_get_coul_data },
	{ POWER_CALI_SYSFS_COUL_CUR_B, power_cali_sysfs_set_coul_data, power_cali_sysfs_get_coul_data },
	{ POWER_CALI_SYSFS_COUL_VOL_A, power_cali_sysfs_set_coul_data, power_cali_sysfs_get_coul_data },
	{ POWER_CALI_SYSFS_COUL_VOL_B, power_cali_sysfs_set_coul_data, power_cali_sysfs_get_coul_data },
	{ POWER_CALI_SYSFS_COUL_CUR, NULL, power_cali_sysfs_get_coul_data },
	{ POWER_CALI_SYSFS_COUL_VOL, NULL, power_cali_sysfs_get_coul_data },
	{ POWER_CALI_SYSFS_COUL_MODE, power_cali_sysfs_set_coul_data, power_cali_sysfs_get_coul_data },
	{ POWER_CALI_SYSFS_COUL_SAVE, power_cali_sysfs_set_coul_data, NULL },
	{ POWER_CALI_SYSFS_WLTRX_CURR_SUPPORT, NULL, power_cali_sysfs_get_wltrx_curr_data },
	{ POWER_CALI_SYSFS_WLTRX_CURR_GET_PARA, NULL, power_cali_sysfs_get_wltrx_curr_data },
	{ POWER_CALI_SYSFS_WLTRX_CURR_IC_TYPE, power_cali_sysfs_set_wltrx_curr_data, NULL },
	{ POWER_CALI_SYSFS_WLTRX_CURR_INIT, power_cali_sysfs_set_wltrx_curr_data, NULL },
	{ POWER_CALI_SYSFS_WLTRX_CURR_Y1, power_cali_sysfs_set_wltrx_curr_data, NULL },
	{ POWER_CALI_SYSFS_WLTRX_CURR_Y2, power_cali_sysfs_set_wltrx_curr_data, NULL },
	{ POWER_CALI_SYSFS_WLTRX_CURR_CAL, power_cali_sysfs_set_wltrx_curr_data, NULL },
};
	...
	struct device_attribute *attr, char *buf)
{
	...
	for (i = 0; i < ARRAY_SIZE(g_sysfs_handler); i++) {
		if ((info->name == g_sysfs_handler[i].type) &&
			g_sysfs_handler[i].show_handler)
			ret = g_sysfs_handler[i].show_handler(info->name, buf);
	}
	return ret;
}
	...
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	for (i = 0; i < ARRAY_SIZE(g_sysfs_handler); i++) {
		if ((info->name == g_sysfs_handler[i].type) &&
			g_sysfs_handler[i].store_handler)
			ret = g_sysfs_handler[i].store_handler(info->name, value);
	}
	if (ret)
		return -EINVAL;
	return count;
