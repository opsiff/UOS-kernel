...
static int power_supply_get_batid_property(int type)
{
	struct power_supply_ops *l_ops = power_supply_get_ops(type);
...
static struct power_supply_battery_get_property_handler {
	unsigned long prop;
	int (*handler)(int);
} const g_battery_get_property[] = {
	{ POWER_SUPPLY_PROP_STATUS, power_supply_get_status_property },
	{ POWER_SUPPLY_PROP_HEALTH, power_supply_get_health_property },
	{ POWER_SUPPLY_PROP_PRESENT, power_supply_get_present_property },
	{ POWER_SUPPLY_PROP_ONLINE, power_supply_get_online_property },
	{ POWER_SUPPLY_PROP_TECHNOLOGY, power_supply_get_technology_property },
	{ POWER_SUPPLY_PROP_TEMP, power_supply_get_temp_property },
	{ POWER_SUPPLY_PROP_CYCLE_COUNT, power_supply_get_cycle_count_property },
	{ POWER_SUPPLY_PROP_LIMIT_FCC, power_supply_get_limit_fcc_property },
	{ POWER_SUPPLY_PROP_VOLTAGE_NOW, power_supply_get_voltage_now_property },
	{ POWER_SUPPLY_PROP_VOLTAGE_MAX, power_supply_get_voltage_max_property },
	{ POWER_SUPPLY_PROP_VOLTAGE_MAX_DESIGN, power_supply_get_voltage_max_design_property },
	{ POWER_SUPPLY_PROP_CURRENT_NOW, power_supply_get_current_now_property },
	{ POWER_SUPPLY_PROP_CURRENT_AVG, power_supply_get_current_avg_property },
	{ POWER_SUPPLY_PROP_CAPACITY, power_supply_get_capacity_property },
	{ POWER_SUPPLY_PROP_CAPACITY_LEVEL, power_supply_get_capacity_level_property },
	{ POWER_SUPPLY_PROP_CAPACITY_RM, power_supply_get_capacity_rm_property },
	{ POWER_SUPPLY_PROP_CAPACITY_FCC, power_supply_get_capacity_fcc_property },
	{ POWER_SUPPLY_PROP_CHARGE_FULL, power_supply_get_charge_full_property },
	{ POWER_SUPPLY_PROP_CHARGE_COUNTER, power_supply_get_charge_counter_property },
	{ POWER_SUPPLY_PROP_CHARGE_FULL_DESIGN, power_supply_get_charge_full_design_property },
	{ POWER_SUPPLY_PROP_CHARGE_NOW, power_supply_get_charge_now_property },
	{ POWER_SUPPLY_PROP_ID_VOLTAGE, power_supply_get_batid_property },
};
	...
	enum power_supply_property psp, union power_supply_propval *val)
{
	...
	if (psp == POWER_SUPPLY_PROP_BRAND) {
		val->strval = power_supply_get_brand_property(type);
		return 0;
	}

	for (i = 0; i < ARRAY_SIZE(g_battery_get_property); i++) {
		if (psp == g_battery_get_property[i].prop) {
			val->intval = g_battery_get_property[i].handler(type);
			return 0;
		}
	}
...
static int power_supply_register_bat(struct power_supply_dev *l_dev)
{
...
static int power_supply_register_raw_bat(struct power_supply_dev *l_dev)
{
...
static int power_supply_register_assist_bat(struct power_supply_dev *l_dev)
{
...
static int power_supply_register_mains(struct power_supply_dev *l_dev)
{
...
static int power_supply_register_usb(struct power_supply_dev *l_dev)
{
...
static int power_supply_register_wireless(struct power_supply_dev *l_dev)
{
...
{
	if (power_supply_register_bat(l_dev))
		goto fail_register_battery;

	if (power_supply_register_raw_bat(l_dev))
		goto fail_register_raw_battery;

	if (power_supply_register_assist_bat(l_dev))
		goto fail_register_assist_battery;

	if (power_supply_register_mains(l_dev))
		goto fail_register_mains;

	if (power_supply_register_usb(l_dev))
		goto fail_register_usb;

	if (power_supply_register_wireless(l_dev))
