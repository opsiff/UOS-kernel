...
static bool power_vote_set_is_similar(struct power_vote_object *obj,
	const char *client_name, int client_id, bool enabled, int value)
{
...
static int power_vote_set_result(struct power_vote_object *obj,
	int client_id, int *eff_result, int *eff_id)
{
	switch (obj->type) {
...
static int power_vote_callback(struct power_vote_object *obj,
	int eff_result, int eff_id)
{
	...
	const char *client_name, bool enabled, int value)
{
	...
	if (power_vote_set_is_similar(obj, client_name, client_id, enabled, value)) {
		ret = 0;
		goto out;
	}

	if (power_vote_set_result(obj, client_id, &eff_result, &eff_id) < 0)
		goto out;
	...
	ret = power_vote_callback(obj, eff_result, eff_id);
