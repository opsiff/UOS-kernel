...
static void power_dbg_create_file(void)
{
	...
	list_for_each(pos, &g_power_dbg_list) {
		...
		file = debugfs_lookup(pattr->file, dir);
...
static ssize_t power_dbg_post_create_store(void *dev_data, const char *buf, size_t size)
{
	hwlog_info("post create\n");
	power_dbg_create_file();
	return size;
}
...
static int __init power_dbg_init(void)
{
	...
	power_dbg_ops_register("post_create", "create", NULL, NULL,
		power_dbg_post_create_store);
