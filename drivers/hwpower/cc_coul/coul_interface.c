...
int coul_interface_get_battery_charge_counter(int type)
{
	...
	if (l_ops && l_ops->get_battery_charge_counter)
		return l_ops->get_battery_charge_counter(l_ops->dev_data);
...
int coul_interface_get_desired_charging_current(int type)
{
	...
	if (l_ops && l_ops->get_desired_charging_current)
		return l_ops->get_desired_charging_current(l_ops->dev_data);
...
int coul_interface_get_desired_charging_voltage(int type)
{
	...
	if (l_ops && l_ops->get_desired_charging_voltage)
		return l_ops->get_desired_charging_voltage(l_ops->dev_data);
...
bool coul_interface_is_smart_battery(int type)
{
	...
	if (l_ops && l_ops->is_smart_battery)
		return l_ops->is_smart_battery(l_ops->dev_data);
...
EXPORT_SYMBOL_GPL(coul_interface_ops_register);
