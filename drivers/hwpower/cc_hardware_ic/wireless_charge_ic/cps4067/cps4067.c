...
static int cps4067_dev_check(struct cps4067_dev_info *di, const char *chip_name)
{
	int ret;
	u16 chip_id = 0;
	...
	if (!strncmp(chip_name, CPS4067_CHIP_NAME, sizeof(CPS4067_CHIP_NAME)))
		di->chip_id = CPS4067_CHIP_ID;
	else
		di->chip_id = CPS4061_CHIP_ID;
	hwlog_info("[dev_check] chip_id=0x%04x\n", di->chip_id);
	return 0;
}
	...
	const struct i2c_device_id *id)
{
	...
	ret = cps4067_dev_check(di, id->name);
	if (ret) {
		ret = -EPROBE_DEFER;
		goto dev_ck_fail;
	}
...
static const struct i2c_device_id cps4067_i2c_id[] = {
	{ CPS4067_CHIP_NAME, WLTRX_IC_MAIN },
	{ CPS4061_CHIP_NAME, WLTRX_IC_MAIN },
	{},
};
