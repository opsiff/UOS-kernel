...
static void cps4067_fw_print_vrect(struct cps4067_dev_info *di)
{
	...
	hwlog_info("[print_vrect] gpio_good_val=%d\n", gpio_get_value(di->gpio_pwr_good));
...
static int cps4067_fw_status_check(struct cps4067_dev_info *di, u32 cmd)
{
	...
	if (di->chip_id == CPS4067_CHIP_ID)
		...
		ret = cps4067_write_dword(di, CPS4061_SRAM_CHK_CMD_ADDR, 0);
	if (ret) {
	...
	if (di->chip_id == CPS4067_CHIP_ID)
		...
		ret = cps4067_write_dword(di, CPS4061_SRAM_STRAT_CMD_ADDR, cmd);
	if (ret) {
	...
	for (i = 0; i < 20; i++) {
		...
		if (di->chip_id == CPS4067_CHIP_ID)
			...
			ret = cps4067_read_dword(di, CPS4061_SRAM_CHK_CMD_ADDR, (u32 *)&status);
		if (ret) {
		...
		if (status == CPS4067_CHK_FAIL) {
			hwlog_err("status_check: failed, status=0x%x, cmd=0x%x\n", status, cmd);
			return -ENXIO;
		}
	}
...
static int cps4061_fw_write_sram_data(struct cps4067_dev_info *di, u16 cur_addr,
	const u8 *data, int len)
{
	int ret;
	int size_to_wr;
	...
	while (remaining > 0) {
		size_to_wr = remaining > CPS4067_FW_PAGE_SIZE ? CPS4067_FW_PAGE_SIZE : remaining;
		(void)memcpy_s(wr_buff, CPS4067_FW_PAGE_SIZE, data + wr_already, size_to_wr);
		ret = cps4067_write_block(di, addr, wr_buff, size_to_wr);
		if (ret) {
			hwlog_err("write_sram_data: fail, addr=0x%x\n", addr);
			return ret;
		}
...
static int cps4061_fw_write_mtp_data(struct cps4067_dev_info *di, const u8 *mtp_data, u16 mtp_size)
{
	int ret;
	int offset = 0;
	int remaining = mtp_size;
	int wr_size;
	int use_buf_index = CPS4061_WRITE_MTP_USE_BUF0;
	u16 buf_addr;
	u32 bf2mtp_val;

	while (remaining > 0) {
		wr_size = remaining > CPS4061_HW_MTP_BUFF_SIZE ?
			CPS4061_HW_MTP_BUFF_SIZE : remaining;
		if (use_buf_index == 0) {
			buf_addr = CPS4061_SRAM_MTP_BUFF0;
			/* Write Data Buffer0 data to MTP: Write 0x00000010 to register 0x17F0 */
			bf2mtp_val = CPS4067_STRAT_CARRY_BUF0;
			use_buf_index = CPS4061_WRITE_MTP_USE_BUF1;
		} else {
			buf_addr = CPS4061_SRAM_MTP_BUFF1;
			/* Write the Data Buffer1 data to MTP: Write 0x00000020 to register 0x17F0 */
			bf2mtp_val = CPS4067_STRAT_CARRY_BUF1;
			use_buf_index = CPS4061_WRITE_MTP_USE_BUF0;
		}

		ret = cps4061_fw_write_sram_data(di, buf_addr, mtp_data + offset, wr_size);
		if (ret) {
			hwlog_err("write_mtp_data: write mtp failed\n");
			return ret;
		}

		ret = cps4067_fw_status_check(di, bf2mtp_val);
		if (ret) {
			hwlog_err("write_mtp_data: verify integrity failed\n");
			return ret;
		}
...
static int __cps4061_fw_load_mtp(struct cps4067_dev_info *di, const u8 *mtp_data, u16 mtp_size)
{
	int ret;

	/* Configuring the Data Buffer size 1024 byte */
	ret = cps4067_write_word(di, 0x17F8, 0x0100);
	if (ret) {
		hwlog_err("load_mtp: config data buffer failed\n");
		return ret;
	}

	ret = cps4067_fw_status_check(di, CPS4061_STRAT_ERASE_MTP);
	if (ret) {
		hwlog_err("load_mtp: verify integrity failed\n");
		return ret;
	}

	ret = cps4061_fw_write_mtp_data(di, mtp_data, mtp_size);
	if (ret) {
...
static int __cps4067_fw_load_mtp(struct cps4067_dev_info *di, const u8 *mtp_data, u16 mtp_size)
{
	int ret;
...
static int cps4067_fw_load_mtp(struct cps4067_dev_info *di, const u8 *mtp_data, u16 mtp_size)
{
	...
	if (di->chip_id == CPS4067_CHIP_ID)
		ret = __cps4067_fw_load_mtp(di, mtp_data, mtp_size);
	else
		ret = __cps4061_fw_load_mtp(di, mtp_data, mtp_size);
	if (ret)
		return ret;
...
static int __cps4061_fw_load_bootloader(struct cps4067_dev_info *di)
{
	int ret;

	ret = cps4061_fw_write_sram_data(di, CPS4061_SRAM_BTL_ADDR,
		g_cps4061_bootloader, sizeof(g_cps4061_bootloader));
	if (ret) {
		hwlog_err("load_bootloader: load bootloader data failed\n");
		return ret;
	}

	ret = cps4067_write_word(di, 0xff81, 0x3f); /* Enable remap function */
	ret += cps4067_write_word(di, 0xff80, 0x50); /* Restart the MCU */
	if (ret) {
		hwlog_err("load_bootloader: restart mcu failed\n");
		return ret;
	}

	power_usleep(DT_USLEEP_10MS); /* typically 10ms */
	/* Disable remap function: Write 0x00 to register 0xFF81 */
	ret = cps4067_write_word(di, 0xFF81, 0x00);
	if (ret) {
		hwlog_err("check_bootloader: disable remap failed\n");
		return ret;
	}
...
static int __cps4067_fw_load_bootloader(struct cps4067_dev_info *di)
{
	int ret;
...
static int cps4067_fw_load_bootloader(struct cps4067_dev_info *di)
{
	...
	if (di->chip_id == CPS4067_CHIP_ID)
		ret = __cps4067_fw_load_bootloader(di);
	else
		ret = __cps4061_fw_load_bootloader(di);
	if (ret) {
		hwlog_err("load_bootloader: load failed\n");
		return ret;
	}

	ret = cps4067_fw_status_check(di, CPS4067_START_CHK_BTL);
	if (ret) {
		hwlog_err("load_bootloader: check failed\n");
		return ret;
	}
...
static int cps4067_fw_program_post_process(struct cps4067_dev_info *di)
{
	...
	if (di->chip_id == CPS4067_CHIP_ID) {
		...
		ret = cps4067_write_byte(di, 0xff80, 0x08); /* Reset the MCU */
		ret += cps4067_write_byte(di, 0xff80, 0x40); /* Run the MCU */
	}
	if (ret) {
...
static int cps4067_fw_program_prev_process(struct cps4067_dev_info *di)
{
	...
	if (di->chip_id == CPS4067_CHIP_ID) {
		...
		ret = cps4067_write_word(di, 0xff80, 0x58); /* MCU reset and unmask all address */
		if (ret) {
			hwlog_err("load_bootloader: reset mcu failed\n");
			return ret;
		}
	}
...
static int cps4067_fw_rx_program_mtp(unsigned int proc_type, void *dev_data)
{
	...
	mtp_dta = (di->chip_id == CPS4067_CHIP_ID) ? g_cps4067_mtp : g_cps4061_mtp;
	mtp_len = (di->chip_id == CPS4067_CHIP_ID) ? CPS4067_FW_MTP_SIZE : CPS4061_FW_MTP_SIZE;
	hwlog_info("[mtp_check_work] mtp_data[0]=0x%x\n", mtp_dta[0]);
	di->g_val.mtp_chk_complete = false;
	ret = cps4067_fw_program_mtp(di, mtp_dta, mtp_len);
	if (!ret)
...
static bool cps4067_fw_is_mtp_chk_complete(void *dev_data)
{
	struct cps4067_dev_info *di = dev_data;
...
static struct wireless_fw_ops g_cps4067_fw_ops = {
	...
	.is_mtp_chk_complete    = cps4067_fw_is_mtp_chk_complete,
};
...
int cps4067_fw_ops_register(struct wltrx_ic_ops *ops, struct cps4067_dev_info *di)
{
	...
	if (di->chip_id == CPS4067_CHIP_ID) {
		...
		di->fw_mtp.mtp_ver = CPS4061_MTP_VER;
		di->fw_mtp.mtp_crc = CPS4061_MTP_CRC;
	}
