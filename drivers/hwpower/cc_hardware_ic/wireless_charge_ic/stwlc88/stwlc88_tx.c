...
static int stwlc88_sw2tx(struct stwlc88_dev_info *di)
{
	int ret;
	int i;
	u8 mode = 0;
...
static void stwlc88_tx_select_init_para(struct stwlc88_dev_info *di,
	unsigned int client)
{
	di->tx_init_para.ocp_th = ST88_TX_OCP_TH;
	...
	case WLTX_CLIENT_BAT_HEATING:
		di->tx_init_para.ping_freq = ST88_BAT_HEATING_PING_FREQ;
		di->tx_init_para.ping_interval = ST88_BAT_HEATING_PING_INTERVAL;
		di->tx_init_para.ocp_th = ST88_TX_BAT_HEATING_OCP_TH;
		break;
	default:
		di->tx_init_para.ping_freq = di->tx_ping_freq;
		di->tx_init_para.ping_interval = ST88_TX_PING_INTERVAL;
...
static int stwlc88_tx_set_init_para(struct stwlc88_dev_info *di)
{
	...
	ret += stwlc88_write_byte(ST88_TX_OCP_TH_ADDR,
		di->tx_init_para.ocp_th / ST88_TX_OCP_TH_STEP);
...
static void stwlc88_tx_mode_irq_recheck(struct stwlc88_dev_info *di)
{
	...
	if (irq_val & ST88_TX_IRQ_EPT_PKT_RCVD) {
		hwlog_info("[tx_mode_irq_recheck] handle tx_ept event\n");
