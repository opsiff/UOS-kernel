...
static void cps4057_rx_set_turbo_chg_flag(bool flag, void *dev_data)
{
	struct cps4057_dev_info *di = dev_data;
...
static void cps4057_rx_set_cur_pmode_name(const char *cur_pmode_name, void *dev_data)
{
	struct cps4057_dev_info *di = dev_data;
...
static struct wlrx_ic_ops g_cps4057_rx_ic_ops = {
	...
	.set_turbo_chg_flag     = cps4057_rx_set_turbo_chg_flag,
	.set_cur_pmode_name     = cps4057_rx_set_cur_pmode_name,
	.need_chk_pu_shell      = NULL,
	.set_pu_shell_flag      = NULL,
};

static ssize_t cps4057_rx_cm_store(void *dev_data, const char *buf, size_t size)
{
	int ret;
	int tmp[CPS4057_RX_CM_LEN] = { 0 };
	struct cps4057_dev_info *di = NULL;

	di = (struct cps4057_dev_info *)dev_data;
	if (!di)
		return -ENODEV;

	/* 0~2 index */
	if (sscanf_s(buf, "%x %x %x", &tmp[0], &tmp[1], &tmp[2]) != CPS4057_RX_CM_LEN) {
		hwlog_err("cm_store: unable to parse input %s\n", buf);
		return -EINVAL;
	}
	hwlog_info("[rx_cm_store] input=0x%x 0x%x 0x%x\n", tmp[0], tmp[1], tmp[2]);
	di->mod_cfg.cm.l_volt = (u8)tmp[0];
	di->mod_cfg.cm.h_volt = (u8)tmp[1];
	di->mod_cfg.cm.fac_h_volt = (u8)tmp[2];
	return (ssize_t)size;
}

static ssize_t cps4057_rx_cm_show(void *dev_data, char *buf, size_t size)
{
	struct cps4057_dev_info *di = NULL;
...
static void cps4057_rx_debug_register(struct cps4057_dev_info *di)
{
	...
	power_dbg_ops_register(dir_name, "cm_cfg", di, cps4057_rx_cm_show, cps4057_rx_cm_store);
}
