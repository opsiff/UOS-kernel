...
static int mt5785_tx_set_bridge(unsigned int v_ask, unsigned int type, void *dev_data)
{
	...
	if ((v_ask > MT5785_TX_SWITCH_BRI_CTL_VTH) || (type == WLTX_PT_FULL_BRIDGE))
		(void)mt5785_write_byte(di, MT5785_TX_FUNC_EN_ADDR, MT5785_TX_FUNC_EN);

	switch (type) {
	case WLTX_PING_HALF_BRIDGE:
		return mt5785_write_byte_mask(di, MT5785_TX_BRG_MODE_ADDR,
			MT5785_TX_PING_HALF_BRG_MASK | MT5785_TX_PING_FULL_BRG_MASK, 
			0, MT5785_TX_PING_HALF_BRG_MASK);
	case WLTX_PING_FULL_BRIDGE:
		return mt5785_write_byte_mask(di, MT5785_TX_BRG_MODE_ADDR,
			MT5785_TX_PING_HALF_BRG_MASK | MT5785_TX_PING_FULL_BRG_MASK, 
			0, MT5785_TX_PING_FULL_BRG_MASK);
	case WLTX_PT_HALF_BRIDGE:
		return mt5785_write_byte_mask(di, MT5785_TX_BRG_MODE_ADDR,
			MT5785_TX_PT_HALF_BRG_MASK | MT5785_TX_PT_FULL_BRG_MASK, 
			0, MT5785_TX_PT_HALF_BRG_MASK);
	case WLTX_PT_FULL_BRIDGE:
		return mt5785_write_byte_mask(di, MT5785_TX_BRG_MODE_ADDR,
			MT5785_TX_PT_HALF_BRG_MASK | MT5785_TX_PT_FULL_BRG_MASK, 
			0, MT5785_TX_PT_FULL_BRG_MASK);
	default:
		return -EINVAL;
	}
}
...
static int mt5785_tx_set_min_duty(int duty, void *dev_data)
{
	...
	(void)mt5785_read_word(dev_data, MT5785_TX_VIN_ADDR, &tx_vin);
	if (tx_vin >= WLRX_IC_HIGH_VOUT2)
		duty = MT5785_TX_PT_DUTY_MIN_HP_DFLT;
...
{
	return mt5785_read_byte(dev_data, MT5785_TX_FOD_TH_ID_ADDR, id);
}
...
static int mt5785_tx_set_ping_ocp(int value, struct mt5785_dev_info *di)
{
	int dbg;
	...
	return mt5785_write_word(di, MT5785_TX_PING_OCP_TH_ADDR, (u16)value);
}

static int mt5785_tx_set_pt_ocp(int value, struct mt5785_dev_info *di)
{
	int dbg;
	...
	return mt5785_write_word(di, MT5785_TX_OCP1_ADDR, (u16)value);
}
...
static int mt5785_tx_set_init_para(struct mt5785_dev_info *di)
{
	...
	ret = mt5785_tx_set_pt_ocp(di->tx_init_para.ocp_th, di);
	...
	ret += mt5785_tx_set_ping_ocp(di->tx_init_para.ping_ocp_th, di);
	...
	ret += mt5785_write_byte(di, MT5785_TX_FUNC_EN_ADDR, MT5785_TX_FUNC_DIS);
	if (di->tx_ilim_proc_cycle != MT5785_TX_ILIM_PROC_CYCLE_DFLT) {
		ret += mt5785_write_byte(di, MT5785_TX_NATIONAL_STD_CER_ADDR, MT5785_TX_NATIONAL_STD_EN);
		ret += mt5785_write_byte(di, MT5785_TX_ILIM_PROC_CYCLE_ADDR, (u8)di->tx_ilim_proc_cycle);
	}
	if (ret) {
...
static void mt5785_tx_debug_register(struct mt5785_dev_info *di)
{
