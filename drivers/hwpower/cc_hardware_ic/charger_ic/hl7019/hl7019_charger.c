...
static int hl7019_chip_init(struct charge_init_data *init_crit)
{
	int ret = 0;
	struct hl7019_device_info *di = g_hl7019_dev;
...
static int hl7019_set_charger_hiz(int enable)
{
	...
	if (enable > 0) {
		...
		if ((di->hiz_iin_limit == 1) &&
			...
			ret = hl7019_set_watchdog_timer(CHAGRE_WDT_DISABLE);
			if (ret < 0)
	...
	const struct i2c_device_id *id)
{
	...
	ret = charge_ops_register(&g_hl7019_ops, BUCK_IC_TYPE_THIRDPARTY);
	if (ret) {
