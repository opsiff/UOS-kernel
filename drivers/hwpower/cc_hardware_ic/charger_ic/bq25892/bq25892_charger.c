...
static int bq25892_get_vilim_sample(void)
{
	...
	for (i = 0; i < retry_times; ++i) {
		v_sample = lpm_adc_get_value(adc_channel_iin);
		if (v_sample < 0)
...
static int bq25892_set_watchdog_timer(int value)
{
	...
	if (dog_time >= CHAGRE_WDT_160S)
		val = BQ25892_REG_07_WATCHDOG_160;
	else if (dog_time >= CHAGRE_WDT_80S)
		val = BQ25892_REG_07_WATCHDOG_80;
	else if (dog_time >= CHAGRE_WDT_40S)
...
static int bq25892_chip_init(struct charge_init_data *init_crit)
{
	int ret = -1;
	struct bq25892_device_info *di = g_bq25892_dev;
	...
	const struct i2c_device_id *id)
{
	...
	ret = charge_ops_register(&bq25892_ops, BUCK_IC_TYPE_THIRDPARTY);
	if (ret) {
