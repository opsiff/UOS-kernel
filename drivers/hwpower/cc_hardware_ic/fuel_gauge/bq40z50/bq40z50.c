...
static int bq40z50_read_word_raw(u8 reg, u16 *data)
{
	int ret;
...
static int bq40z50_get_battery_temp(void)
{
	...
	ret = bq40z50_read_word_raw(BQ40Z50_REG_TEMP, &temp_k);
	if (ret) {
...
static bool bq40z50_is_abnormal_low_capacity(s16 cur, u16 vol)
{
	...
	ret = bq40z50_read_word_raw(BQ40Z50_REG_CELL1_VOLT, &cell1_vol);
	if (!ret) {
	...
	ret = bq40z50_read_word_raw(BQ40Z50_REG_CELL2_VOLT, &cell2_vol);
	if (!ret) {
	...
	ret = bq40z50_read_word_raw(BQ40Z50_REG_CELL3_VOLT, &cell3_vol);
	if (!ret) {
...
static int bq40z50_get_battery_soc(void)
{
	...
	ret = bq40z50_read_word_raw(BQ40Z50_REG_SOC, &soc);
	if (ret)
		soc = di->cache.soc;
	else
		di->cache.soc = soc;
...
static int bq40z50_get_battery_vol(void)
{
	...
	ret = bq40z50_read_word_raw(BQ40Z50_REG_VOLT, &vol);
	if (ret)
		vol = di->cache.vol;
	else
		di->cache.vol = vol;
...
static int bq40z50_get_battery_curr(void)
{
	...
	ret = bq40z50_read_word_raw(BQ40Z50_REG_CURR, &curr);
	if (ret)
		curr = di->cache.curr;
	else
		di->cache.curr = curr;
...
static int bq40z50_get_battery_avgcurr(void)
{
	...
	ret = bq40z50_read_word_raw(BQ40Z50_REG_AVRGCURR, &avg_curr);
	if (ret)
...
static int bq40z50_get_battery_rm(void)
{
	...
	ret = bq40z50_read_word_raw(BQ40Z50_REG_RM, &rm);
	if (ret)
		rm = di->cache.rm;
	else
		di->cache.rm = rm;
...
static int bq40z50_get_battery_dc(void)
{
	...
	ret = bq40z50_read_word_raw(BQ40Z50_REG_DC, &dc);
	if (ret)
		dc = di->cache.dc;
	else
		di->cache.dc = dc;
...
static int bq40z50_get_battery_fcc(void)
{
	...
	ret = bq40z50_read_word_raw(BQ40Z50_REG_FCC, &fcc);
	if (ret)
		fcc = di->cache.fcc;
	else
		di->cache.fcc = fcc;
...
static int bq40z50_get_battery_soh(void)
{
	...
	ret = bq40z50_read_word_raw(BQ40Z50_REG_SOH, &soh);
	if (ret)
		soh = di->cache.soh;
	else
		di->cache.soh = soh;
...
static int bq40z50_get_battery_tte(void)
{
	...
	ret = bq40z50_read_word_raw(BQ40Z50_REG_TTE, &tte);
	if (ret)
		tte = di->cache.tte;
	else
		di->cache.tte = tte;
...
static int bq40z50_get_battery_ttf(void)
{
	...
	ret = bq40z50_read_word_raw(BQ40Z50_REG_TTF, &ttf);
	if (ret)
		ttf = di->cache.ttf;
	else
		di->cache.ttf = ttf;
...
static int bq40z50_get_battery_cycle(void)
{
	...
	ret = bq40z50_read_word_raw(BQ40Z50_REG_CYCLE, &cycle);
	if (ret)
...
static int bq40z50_get_battery_status(void)
{
	...
	ret = bq40z50_read_word_raw(BQ40Z50_REG_BATTERY_STATUS, &status);
	if (ret)
...
static int bq40z50_get_desired_charging_current(void)
{
	...
	ret = bq40z50_read_word_raw(BQ40Z50_REG_CHARGING_CURRENT, &curr);
	if (ret)
...
static int bq40z50_get_desired_charging_voltage(void)
{
	...
	ret = bq40z50_read_word_raw(BQ40Z50_REG_CHARGING_VOLTAGE, &vol);
	if (ret)
