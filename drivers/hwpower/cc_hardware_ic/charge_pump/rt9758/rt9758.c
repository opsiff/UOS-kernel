...
static bool rt9758_is_chg_en_enable(void *dev_data)
{
	int ret;
	u8 val = 0;

	ret = rt9758_read_byte(dev_data, RT9758_CTRL6_REG, &val);
	hwlog_info("[is_chg_en_enable] reg[0x%x]=0x%x\n", RT9758_CTRL6_REG, val);
	if (!ret && ((val & RT9758_CTRL6_MOD_CHG_EN_MASK) ==
		(RT9758_CTRL6_MOD_CHG_EN_EN << RT9758_CTRL6_MOD_CHG_EN_SHIFT)))
		return true;
...
static int rt9758_set_bp_mode(void *dev_data)
{
	...
	do {
		...
		if (!rt9758_is_chg_en_enable(dev_data))
			(void)rt9758_write_mask(dev_data, RT9758_CTRL6_REG, RT9758_CTRL6_MOD_CHG_EN_MASK,
