...
static int sc8545_get_ibus(int *ibus, void *dev_data)
{
	int delta_ibus;
	int ibus_temp;
	struct sc8545_device_info *di = dev_data;
	...
	if (sc8545_get_ibus_ma(&ibus_temp, dev_data))
		return -EPERM;
	 ...
	 */
	delta_ibus = abs(ibus_temp - di->last_ibus);
	if (ibus_temp && di->last_ibus && (delta_ibus > SC8545_ERROR_DELTA_IBUS)) {
		power_usleep(DT_USLEEP_1MS);
		hwlog_info("ic_%u ibus=%d last_ibus=%d, read again\n", di->ic_role, ibus_temp, di->last_ibus);
		if (sc8545_get_ibus_ma(&ibus_temp, dev_data))
			return -EPERM;
	}
	di->last_ibus = ibus_temp;
	*ibus = ibus_temp;
	return 0;
}
