...
#define COUL_FAKE_RETRY_TIMES       30
...
static inline bool bat_1s2p_condition_match(struct bat_1s2p_device *di)
{
	return di->retry_times >= COUL_FAKE_RETRY_TIMES;
}

static bool bat_1s2p_waiting_ready(struct bat_1s2p_device *di)
{
	if (!di->notify_flag && bat_1s2p_condition_match(di)) {
		hwlog_info("send fake event.main_ready = %d,aux_ready = %d.\n",
			di->bat_info[BAT_MAIN].is_ready, di->bat_info[BAT_AUX].is_ready);
		power_event_bnc_notify(POWER_BNT_COUL, POWER_NE_COUL_READY_STATUS, NULL);
		di->notify_flag = true;
	}
...
static int bat_1s2p_parse_bat_info_dts(struct bat_1s2p_device *di)
{
...
static int bat_1s2p_parse_dts(struct bat_1s2p_device *di)
{
	...
	ret = bat_1s2p_parse_bat_info_dts(di);
	if (ret)
		return ret;
