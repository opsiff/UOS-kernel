...
LIST_HEAD(batt_type_ic_head);

void bat_type_ic_register(struct batt_type_entry *entry)
{
	if (!entry) {
		hwlog_err("%s batt_type_entry is null\n", __func__);
		return;
	}
	list_add_tail(&entry->node, &batt_type_ic_head);
}
...
static void bat_type_set_dev_status(enum bat_type_dev_status status)
{
	struct platform_device *pdev = NULL;
	struct batt_type_entry *pos = NULL;
...
/**
 * bat_type_apply_mode - control pmos for ic or id battery identify
 * @mode: indicates different identify pattern.
 *
 * This ops will apply a lock, must in pairs with fuction
 * bat_type_release_mode. If mode is input as BAT_ID_VOLTAGE,
 * pmos will be closed. if mode is input as  BAT_ID_SN, pmos will be open.
 */
void bat_type_apply_mode(int mode)
{
	...
	bat_type_set_dev_status(BAT_TYPE_DEV_IN);
...
/**
 * bat_type_release_mode - control pmos for ic or id battery identify
 * @flag: indicates whether the PMOS needs to be disabled
 *
 * This ops will release lock, must in pairs with bat_type_apply_mode.
 * If flag is true, pmos will be closed. If flag is false, pmos
...
void bat_type_release_mode(bool flag)
{
	...
	bat_type_set_dev_status(BAT_TYPE_DEV_LOW);
