...
static void bat_fault_cutoff_vol_event_handle(struct bat_fault_device *di)
{
	...
	int temp = coul_interface_get_battery_temperature(bat_core_get_coul_type());
...
static void bat_fault_parse_dts(struct bat_fault_device *di)
{
	...
	case BAT_MODEL_BAT_CATHODE_TYPE_SILICON:
		(void)power_dts_read_u32(power_dts_tag(HWLOG_TAG), np,
			"vol_cutoff_normal_si", &di->config.vol_cutoff_normal,
			BAT_FAULT_NORMAL_CUTOFF_VOL);
		(void)power_dts_read_u32(power_dts_tag(HWLOG_TAG), np,
			"vol_cutoff_sleep_si", &di->config.vol_cutoff_sleep,
			BAT_FAULT_SLEEP_CUTOFF_VOL);
		(void)power_dts_read_u32(power_dts_tag(HWLOG_TAG), np,
			"vol_cutoff_low_temp_si", &di->config.vol_cutoff_low_temp,
			BAT_FAULT_NORMAL_CUTOFF_VOL);
		(void)power_dts_read_u32(power_dts_tag(HWLOG_TAG), np,
			"vol_cutoff_filter_cnt_si", &di->config.vol_cutoff_filter_cnt,
