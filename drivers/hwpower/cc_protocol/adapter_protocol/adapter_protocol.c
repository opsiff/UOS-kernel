...
int adapter_get_max_power(int prot, int *max_pwr)
{
	...
	if (!l_ops->get_max_power) {
		hwlog_err("get_max_power is null\n");
		return -EPERM;
	}

	return l_ops->get_max_power(max_pwr);
}
...
int adapter_uvdm_send_data(int prot, u32 *data, u8 cnt, bool wait_rsp, int type)
{
	...
	if (!l_ops || !data) {
		hwlog_err("adapter_uvdm_send_data is null\n");
		return -EPERM;
	}

	if (!l_ops->set_send_data) {
		hwlog_err("set_send_data is err\n");
		return -EPERM;
	}

	return l_ops->set_send_data(data, cnt, wait_rsp, type);
}
