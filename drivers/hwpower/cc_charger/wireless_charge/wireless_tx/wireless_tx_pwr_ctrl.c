...
static struct {
	...
	{ PWR_SW_BY_PREVFOD,  "PWR_SW_BY_PREVFOD" }
};
...
/* pwr_type: 5VBST_RVSSC4_CP */
const struct wltx_pwr_attr g_attr10[] = {
	{ PWR_SW_BY_VBUS_ON,  true,  PWR_SRC_5VBST },
	{ PWR_SW_BY_VBUS_OFF, true,  PWR_SRC_5VBST },
	{ PWR_SW_BY_OTG_ON,   false, PWR_SRC_5VBST },
	{ PWR_SW_BY_OTG_OFF,  true,  PWR_SRC_5VBST },
	{ PWR_SW_BY_SOC_HIGH, true,  PWR_SRC_RVS_SC4_CP },
	{ PWR_SW_BY_PREVFOD,  true,  PWR_SRC_5VBST }
};
...
static struct {
	...
	{ WL_TX_PWR_5VBST_RVSSC4_CP, "5VBST_RVSSC4_CP", g_attr10, ARRAY_SIZE(g_attr10) }
};
...
int wltx_enable_pwr_supply(void)
{
	...
	hwlog_info("[%s] after, pwr_sw_scn: %s, pwr_src: %s\n",
		__func__, wltx_get_pwr_sw_scn_name(di->cur_pwr_sw_scn),
		wltx_get_pwr_src_name(di->cur_pwr_src));
	if ((src == PWR_SRC_RVS_SC4_CP) && (di->cur_pwr_src == PWR_SRC_NULL))
...
static void wltx_vbus_rvssc4_cp_extra_pwr(struct wltx_dev_info *di)
{
	if (di->cur_pwr_src == PWR_SRC_RVS_SC4_CP) {
		di->cur_pwr_src = wltx_set_pwr_src_output(false, PWR_SRC_RVS_SC4_CP);
		msleep(20); /* delay 20ms for rvssc4 pwr_off */
		di->cur_pwr_src = wltx_set_pwr_src_output(true, PWR_SRC_5VBST);
	}
}
...
void wltx_enable_extra_pwr_supply(void)
{
	...
	case WL_TX_PWR_5VBST_VBUS_OTG_CP:
		...
		break;
	case WL_TX_PWR_5VBST_RVSSC4_CP:
		wltx_vbus_rvssc4_cp_extra_pwr(di);
		break;
	default:
		break;
	}
...
int wltx_pwr_prevfod_limit_handler(void)
{
	...
	if (!wltx_need_switch_power(dts->pwr_type, PWR_SW_BY_PREVFOD, di->cur_pwr_src))
		return -ENOTSUPP;

	di->cur_pwr_sw_scn = PWR_SW_BY_PREVFOD;
	wltx_cancel_work_handler();
	wltx_restart_work_handler();
