...
static int wlrx_auth_wait_service_ready(void)
{
	int i;
	int delay = WLRX_AUTH_RETRY_INTERVAL_BASE;

	for (i = 1; i <= WLRX_AUTH_RETRY_COUNT; i++) {
		power_msleep(delay, 0, NULL);
		if (wlrx_auth_srv_ready())
			return 0;

		if (i % WLRX_AUTH_RETRY_INTERVAL_CYCLE == 0) {
			delay = WLRX_AUTH_RETRY_INTERVAL_BASE +
				i / WLRX_AUTH_RETRY_INTERVAL_CYCLE * WLRX_AUTH_RETRY_INTERVAL_INC;
			power_event_bnc_notify(POWER_BNT_BMS_AUTH, POWER_NE_BMS_AUTH_START_FORCE, "wireless_rx");
			power_msleep(delay, 0, NULL);
		}
	}
...
{
	int ret = WLRX_ACC_AUTH_SRV_ERR;

	di->auth_res = 0;
	reinit_completion(&di->completion);
	power_event_bnc_notify(POWER_BNT_BMS_AUTH, POWER_NE_BMS_AUTH_START, "wireless_rx");

	if (wlrx_auth_wait_service_ready()) {
		hwlog_err("service not ready\n");
		ret = WLRX_ACC_AUTH_SRV_NOT_READY;
		goto end;
	}

	if (power_genl_easy_send(POWER_GENL_TP_AF, POWER_GENL_CMD_WLRX_AUTH_HASH, 0,
		di->auth_hash, WLRX_AUTH_HASH_LEN)) {
		hwlog_err("power genl send wlrx auth failed\n");
		goto end;
	}
	...
	ret = WLRX_ACC_AUTH_SUCC;
end:
	power_event_bnc_notify(POWER_BNT_BMS_AUTH, POWER_NE_BMS_AUTH_STOP, "wireless_rx");
	return ret;
}
...
static int wlrx_auth_srv_state_callback(bool state)
{
	struct wlrx_auth_dev *di = g_rx_auth_di;
...
static struct power_genl_node wlrx_auth_genl_node = {
	...
	.srv_state_cb = wlrx_auth_srv_state_callback,
};
