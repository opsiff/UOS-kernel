...
static bool can_vbatt_do_wldc_charge(struct wldc_dev_info *di, int dc_mode)
{
	...
	ls_vbatt = wldc_get_bat_voltage(di, dc_mode);
...
static bool can_batchk_support_mode(struct wldc_dev_info *di, int dc_mode)
{
	if (!wldc_is_mode_para_valid(di, dc_mode))
		return false;

	return multi_batchk_match_curr_dc_type(di->mode_para[dc_mode].init_para.dc_type,
		di->dc_charging && (dc_mode == di->cur_dc_mode));
}
...
bool wldc_is_tmp_forbidden(struct wldc_dev_info *di, int mode)
{
	...
	if (!can_tbatt_do_wldc_charge(di))
		return true;
	if (!can_batchk_support_mode(di, mode))
		return true;
...
static int wldc_prev_check(struct wldc_dev_info *di, const char *mode_name)
{
	...
	for (i = 0; i < di->total_dc_mode; i++) {
		if (power_strsame(mode_name, di->mode_para[i].init_para.dc_name))
			break;
	}
	...
	path_ratio = di->mode_para[i].init_para.cp_ratio * di->mode_para[i].init_para.dc_ratio;
	if (wldc_get_mode_by_name(mode_name) == WLDC_MODE_SCX) {
		if (!wlrx_can_enter_scx(WLTRX_DRV_MAIN, path_ratio))
			return -EPERM;
		di->support_scx_mode = true;
	}
...
static int wldc_open_dc_path_regulate_vrx(struct wldc_dev_info *di)
{
	...
	ls_vbatt = wldc_get_bat_voltage(di, di->cur_dc_mode);
...
{
	int vbatt = 0;
	int ls_vbatt = wldc_get_bat_voltage(di, di->cur_dc_mode);
	char tmp_buf[ERR_NO_STRING_SIZE] = {0};
...
static int wldc_set_rx_init_vout(struct wldc_dev_info *di)
{
	...
	if (wldc_get_mode_by_name(di->mode_para[di->cur_dc_mode].init_para.dc_name) == WLDC_MODE_SCX)
		is_scx_mode = true;
	wlrx_ic_set_scx_mode(di->drv_type, is_scx_mode);
...
static int wireless_dc_charge_check(struct wldc_dev_info *di)
{
...
static int wldc_formal_check(struct wldc_dev_info *di, const char *mode_name)
{
	...
	for (i = 0; i < di->total_dc_mode; i++) {
		...
		ret = wireless_dc_charge_check(di);
		if (!ret) {
