...
static void wlrx_ui_send_max_pwr_evt(struct wlrx_ui_dev *di)
{
	int cur_pwr = 0;
	int tx_max_pwr_mw;

	if ((di->ui_para->ui_max_pwr <= 0) || (di->ui_para->product_max_pwr <= 0) ||
		(di->ui_para->tx_max_pwr < di->ui_para->ui_max_pwr)) {
		power_ui_event_notify(POWER_UI_NE_MAX_POWER, &cur_pwr);
		dc_turbo_send_max_power(cur_pwr);
		return;
	}

	tx_max_pwr_mw = di->ui_para->tx_max_pwr;
	...
	hwlog_info("[send_max_pwr] revised_tx_pwr=%d ui_pwr=%d product_pwr=%d\n",
		tx_max_pwr_mw, di->ui_para->ui_max_pwr, di->ui_para->product_max_pwr);

	cur_pwr = (tx_max_pwr_mw < di->ui_para->product_max_pwr) ?
		tx_max_pwr_mw : di->ui_para->product_max_pwr;
	power_ui_event_notify(POWER_UI_NE_MAX_POWER, &cur_pwr);
	if (wlrx_support_turbo_charge(di->drv_type))
		dc_turbo_send_max_power(cur_pwr);
}

static void wlrx_ui_send_soc_decimal_evt(struct wlrx_ui_dev *di)
{
	int cur_pwr;

	cur_pwr = (di->ui_para->tx_max_pwr < di->ui_para->product_max_pwr) ?
		di->ui_para->tx_max_pwr : di->ui_para->product_max_pwr;
...
static void wlrx_ui_prepare_ui_para(struct wlrx_ui_dev *di)
{
	...
	di->ui_para->product_max_pwr = wlrx_pctrl_get_product_pmax(di->drv_type);
	di->ui_para->tx_max_pwr = acc_cap->vmax / POWER_MV_PER_V *
		acc_cap->imax * acc_cap->pwr_ratio / POWER_PERCENT;
}
...
void wlrx_ui_send_charge_uevent(unsigned int drv_type, unsigned int icon_type)
{
	...
	if (di->curr_icon_type ^ icon_type) {
		...
		if (di->icon_flag.super_charge) {
			wlrx_ui_prepare_ui_para(di);
			wlrx_ui_send_soc_decimal_evt(di);
			wlrx_ui_send_max_pwr_evt(di);
		}
	}
