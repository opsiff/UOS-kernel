...
void wlrx_ic_set_scx_mode(unsigned int type, bool scx_mode)
{
	...
	if (ops && ops->set_scx_mode)
		ops->set_scx_mode(scx_mode, ops->dev_data);
}
...
int wlrx_ic_get_vrx_by_vfc_map(unsigned int type, int vtx)
{
	...
	if (ops && ops->get_vrx_by_vfc_map)
		return ops->get_vrx_by_vfc_map(vtx, ops->dev_data);
...
void wlrx_ic_set_turbo_charge_flag(unsigned int type, bool flag)
{
	...
	if (ops && ops->set_turbo_chg_flag)
		ops->set_turbo_chg_flag(flag, ops->dev_data);
}
...
void wlrx_ic_set_cur_pmode_name(unsigned int type, const char *cur_pmode_name)
{
	...
	if (ops && ops->set_cur_pmode_name)
		ops->set_cur_pmode_name(cur_pmode_name, ops->dev_data);
}
...
void wlrx_ic_set_fod_offset(unsigned int type, u8 offset)
{
	...
	if (ops && ops->set_fod_offset)
		ops->set_fod_offset(offset, ops->dev_data);
}
