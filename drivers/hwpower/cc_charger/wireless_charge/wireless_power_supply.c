...
struct wlps_dev_info {
	...
	int proc_otp_pwr;
	int recover_otp_pwr;
	unsigned long gpio_requested;
	unsigned long gpio_need_request; /* configured in dts */
};
...
static void wlps_gpio_val_check(unsigned int gpio_type, int gpio_no, int set_val)
{
	...
	case WLPS_GPIO_TXSW:
		snprintf(dsm_buf, sizeof(dsm_buf), "gpio_txsw: set=%d actual=%d\n",
			set_val, gpio_val);
		err_no = POWER_DSM_WLPS_GPIO_TXSW;
		break;
	default:
		return;
	}
...
static void wlps_rx_sw_ctrl_by_pwrsw(unsigned int ic_type, bool flag)
{
	struct wlps_dev_info *di = wlps_get_dev_info(ic_type);

	if (!di || (di->rxsw_index < 0))
		return;

	hwlog_info("[rx_sw_ctrl] %s\n", flag ? "on" : "off");
	if (flag)
...
static void wlps_rxsw_ctrl(unsigned int ic_type, bool flag)
{
	...
	if (!di || (di->gpio_rxsw <= 0))
		return wlps_rx_sw_ctrl_by_pwrsw(ic_type, flag);
...
static void wlps_sc_sw_ctrl(unsigned int ic_type, bool flag)
{
	struct wlps_dev_info *di = wlps_get_dev_info(ic_type);

	if (!di || (di->scsw_index < 0))
		return;

	hwlog_info("[sc_sw_ctrl] %s\n", flag ? "on" : "off");
	if (flag)
...
{
	if (di->gpio_need_request && !test_bit(WLPS_GPIO_TXSW, &di->gpio_need_request))
		return 0;
	...
	set_bit(WLPS_GPIO_TXSW, &di->gpio_requested);
	return 0;
}

static int wlps_request_tx_sppwr_gpio(struct device_node *np, struct wlps_dev_info *di)
{
	if (di->gpio_need_request && !test_bit(WLPS_GPIO_TX_SPPWR_EN, &di->gpio_need_request))
		return 0;
	...
	set_bit(WLPS_GPIO_TX_SPPWR_EN, &di->gpio_requested);
	return 0;
}

static int wlps_request_rx_sw_by_pwrsw(struct device_node *np, struct wlps_dev_info *di)
{
	if ((di->gpio_need_request <= 0) ||
		!test_bit(WLPS_GPIO_RXSW, &di->gpio_need_request)) {
		di->rxsw_index = -1;
		return 0;
	}

	if (power_dts_read_u32(power_dts_tag(HWLOG_TAG), np,
		"rxsw_index", (u32 *)&di->rxsw_index, 0))
		return -EINVAL;

	set_bit(WLPS_GPIO_RXSW, &di->gpio_requested);
	return 0;
}

static int wlps_request_rx_sw_gpio(struct device_node *np, struct wlps_dev_info *di)
{
	if (di->gpio_need_request && !test_bit(WLPS_GPIO_RXSW, &di->gpio_need_request)) {
		di->rxsw_index = -1;
		return 0;
	}
	...
	if (di->gpio_rxsw <= 0)
		return wlps_request_rx_sw_by_pwrsw(np, di);
	...
	if (di->gpio_need_request && !test_bit(WLPS_GPIO_RXSW_AUX, &di->gpio_need_request))
		return 0;
	...
	set_bit(WLPS_GPIO_RXSW_AUX, &di->gpio_requested);
	return 0;
}

static int wlps_request_ext_pwr_sw_gpio(struct device_node *np, struct wlps_dev_info *di)
{
	if (di->gpio_need_request && !test_bit(WLPS_GPIO_EXT_PWR_SW, &di->gpio_need_request))
		return 0;
	...
	set_bit(WLPS_GPIO_EXT_PWR_SW, &di->gpio_requested);
	return 0;
}

static int wlps_request_sc_sw2_gpio(struct device_node *np, struct wlps_dev_info *di)
{
	if (di->gpio_need_request && !test_bit(WLPS_GPIO_SC_SW2, &di->gpio_need_request))
		return 0;
	...
	set_bit(WLPS_GPIO_SC_SW2, &di->gpio_requested);
	return 0;
}

static int wlps_request_sc_sw(struct device_node *np, struct wlps_dev_info *di)
{
	if ((di->gpio_need_request <= 0) ||
		!test_bit(WLPS_GPIO_SC_SW, &di->gpio_need_request)) {
		di->scsw_index = -1;
		return 0;
	}

	if (power_dts_read_u32(power_dts_tag(HWLOG_TAG), np,
		"scsw_index", (u32 *)&di->scsw_index, 0))
		return -EINVAL;

	set_bit(WLPS_GPIO_SC_SW, &di->gpio_requested);
	return 0;
}
...
static int wlps_request_resources(struct device_node *np, struct wlps_dev_info *di)
{
	...
	if (wlps_request_sc_sw(np, di))
		goto sc_sw_fail;
	if (di->gpio_need_request > 0) {
		hwlog_info("[request_resources] need_request=0x%x requested=0x%x\n",
			di->gpio_need_request, di->gpio_requested);
		if (di->gpio_need_request != di->gpio_requested)
			goto request_resources_fail;
	}
...
static void wlps_parse_dts(struct device_node *np, struct wlps_dev_info *di)
{
	...
	(void)power_dts_read_u32(power_dts_tag(HWLOG_TAG), np,
		"gpio_need_request", (u32 *)&di->gpio_need_request, 0);
}
