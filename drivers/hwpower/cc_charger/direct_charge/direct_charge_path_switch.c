...
void dc_check_recharge_path(void)
{
	bool rechg_en;
	int rechg_ibat = 0;
	int pre_ibat = 0;
	...
	if (!l_di || !dc_rechg_is_support())
		return;

	if (dc_rechg_is_enforce_en() && dc_rechg_is_ibus_support(l_di->ls_ibus)) {
		dc_rechg_enforce_en();
		return;
	}

	/* ibat_th >= open_ith for limit scene */
	rechg_en = dc_rechg_is_ibat_th_support(l_di->cur_ibat_th_high);
	/* ibus >= open_ith */
	rechg_en = rechg_en && dc_rechg_is_ibus_support(l_di->ls_ibus);
	/* battery connect_mode is serial mode */
	rechg_en = rechg_en && (l_di->battery_connect_mode == BAT_SERIAL_MODE);
	...
	if (rechg_en) {
		...
		dc_rechg_regulation(adp_max_cur - pre_ibat, rechg_ibat);
		hwlog_info("rechg_target_ibat=%d, pre_ibat=%d", rechg_ibat, pre_ibat);
	}
}
