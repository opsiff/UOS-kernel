	...
	struct mipi_dsi_phy_ctrl *phy_ctrl, char __iomem *mipi_dsi_base)
{
	...
	if (mipi->phy_mode == CPHY_MODE) {
		...
		mipi_config_phy_test_code(mipi_dsi_base, (CDPHY_TEST_CTRL_TESTEN_BIT + 0x58),
			(0x4 + mipi->lane_nums) << 4 | 0);
		mipi_config_phy_test_code(mipi_dsi_base, (CDPHY_TEST_CTRL_TESTEN_BIT + 0x5B), 0x19);
		/* PLL update control */
		...
		mipi_config_phy_test_code(mipi_dsi_base, (CDPHY_TEST_CTRL_TESTEN_BIT + 0x5B), phy_ctrl->rg_vrefsel_vcm);

		/* config dpy mipi drive voltage EQ */
		if (phy_ctrl->rg_vrefsel_eq != 0)
			mipi_config_phy_test_code(mipi_dsi_base, (CDPHY_TEST_CTRL_TESTEN_BIT + 0x57), phy_ctrl->rg_vrefsel_eq);
