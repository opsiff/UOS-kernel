...
static uint32_t get_hsize_after_spr_dsc(struct dsc_calc_info *dsc, uint32_t input_width, uint32_t panel_xres)
{
	...
	// only dsc should directly return
	if (dsc->spr_en == 0) {
		dpu_pr_info("[dsc]no spr, output_width=%d\n", hsize);
		return hsize;
	}
	...
	dpu_pr_info("[dsc]has spr, output_width=%d\n", hsize);
	return hsize;
}
...
uint32_t get_dsc_out_width(struct dsc_calc_info *dsc, uint32_t ifbc_type, uint32_t input_width, uint32_t panel_xres)
{
	...
	/*
	 * bpc = 10 && bpp = 10, xres_div = 3
	 * after dsc, data stream map to 8 bit dsi out: input_width * 10 / 8 / 3
