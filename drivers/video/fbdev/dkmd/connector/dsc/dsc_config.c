...
static void dss_dsc_ctrl_cfg(char __iomem * dsc_base, struct dsc_calc_info *dsc)
{
	...
	uint32_t flatness_det_thresh = 0;
	...
	if (dsc->dsc_info.dsc_version_minor == DSC_V_1_2) {
		if (dsc->dsc_info.dsc_bpc < 8) {
			flatness_det_thresh = 2;
			dpu_pr_err("dsc error, dsc_bpc < 8 !!!\n");
		} else {
			flatness_det_thresh = 2 << (dsc->dsc_info.dsc_bpc - 8);
		}
		set_reg(DPU_DSC_12_ADDR(dsc_base), flatness_det_thresh, 10, 0);
	}
}
...
static void dss_dsc_rc_range_cfg(char __iomem * dsc_base, struct dsc_calc_info *dsc)
{
	...
	for (i = 0; i <= 12; i += 2) {
		...
		dpu_pr_info("dsc rc_range[%d]=%d", (DPU_DSC_RC_RANGE_PARAM0_ADDR(dsc_base) + 2 * i), rc_range0.value);
	}
...
{
	if (!dsc || !dsc_base) {
		dpu_pr_warn("dsc or dsc_base is null!\n");
		return;
	}
