...
/* work_mode define */
#define HDMITX_WORK_MODE_TMDS    0
#define HDMITX_WORK_MODE_FRL     1
...
/* see HDMI2.1 7.6.4 */
struct hdmitx_vrr_info {
	struct hdmitx_vrr_mode vrr_mode;
	u32 m_vrr;
	bool vrr_en;
};
...
struct hdmitx_ctrl {
	...
	struct hdmitx_hdr_metadata hdr_metadata;
	struct hdmitx_hdr_infoframe hdr_infoframe;
	...
	struct hdmitx_vrr_info hdmitx_vrr;
	struct hrtimer hdmitx_hrtimer;
	struct workqueue_struct *hdmitx_wq;
	struct work_struct hdcp_random_work;
};
...
#define hdmi_set(bs, offset, val) hdmi_writel((bs), (offset), hdmi_readl((bs), (offset)) | (val))
#define hdmi_clrset(bs, offset, mask, val) hdmi_writel((bs), (offset), (val) | (hdmi_readl((bs), (offset)) & ~(mask)))

extern struct hdmitx_ctrl *g_hdmitx;
#endif
