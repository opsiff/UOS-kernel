	...
	struct device_node *parent_node)
{
	...
	if (IS_ERR_OR_NULL(conn_mgr->dsssubsys_regulator)) {
		...
		return (int32_t)PTR_ERR((void *)conn_mgr->dsssubsys_regulator);
	}
	...
	if (IS_ERR(conn_mgr->vivobus_regulator)) {
		...
		return (int32_t)PTR_ERR((void *)conn_mgr->vivobus_regulator);
	}
	...
	if (IS_ERR(conn_mgr->media1_subsys_regulator)) {
		...
		return (int32_t)PTR_ERR((void *)conn_mgr->media1_subsys_regulator);
	}
	...
	struct device_node *parent_node)
{
	...
	for_each_child_of_node(parent_node, node) {
		...
		if (of_property_read_u32(node, "connector_id", &connector_id) != 0)
