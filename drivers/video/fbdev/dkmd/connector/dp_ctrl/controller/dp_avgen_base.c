...
int dptx_update_panel_info(struct dp_ctrl *dptx)
{
	...
	dpu_pr_info("[DP] dptx_update_panel_info xres:%d, yres:%d, pixel_clock:%d",
		mdtd->h_active, mdtd->v_active, mdtd->pixel_clock);
	...
	if (((mdtd->h_active + mdtd->h_blanking) * (mdtd->v_active + mdtd->v_blanking)) > 0) {
		...
		if (pixels != 0)
			vparams->m_fps = (uint8_t)(dptx->connector->ldi.pxl_clk_rate / pixels);
		else
			vparams->m_fps = 60;
	} else {
		return -1;
...
int dptx_change_video_mode_user(struct dp_ctrl *dptx)
{
	...
	if (dptx->user_mode != 0) {
		...
		vparams->mode = (uint8_t)dptx->user_mode; /* switch to user setting */
		needchanged = true;
