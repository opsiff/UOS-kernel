...
static int hdmirx_get_emp_packet_content(struct hdmirx_ctrl_st *hdmirx,
...
static bool hdmirx_is_emp_packet(hdmirx_packet_type type)
{
	if (type == HDMIRX_PACKET_FVAVRR)
		return true;
												...
												uint32_t max_len)
{
	...
	if (hdmirx_is_emp_packet(type))
		return hdmirx_get_emp_packet_content(hdmirx, type, data, max_len);
...
void hdmirx_packet_proc_allm(struct hdmirx_ctrl_st *hdmirx)
{
	uint32_t allm;
	allm = inp32(hdmirx->hdmirx_pwd_base + 0x1128) & 0x2;
	if (allm)
		disp_pr_info("allm enable\n");
}
...
void hdmirx_packet_proc_hfvsi(struct hdmirx_ctrl_st *hdmirx, uint32_t interrupts)
{
	...
	hdmirx_packet_depack_hfvsi(hdmirx);
	hdmirx_packet_proc_allm(hdmirx);
}
...
{
	hdmirx_set_reg(hdmirx->hdmirx_sysctrl_base + 0x040, 0x3, 2, 0);
}

void hdmirx_packet_depack_enable(struct hdmirx_ctrl_st *hdmirx)
{
	// PACKET TYPE irq MASK
	hdmirx_set_reg(hdmirx->hdmirx_pwd_base + 0x13D4, 0x7FFFF, 19, 0);
}

void hdmirx_packet_vrr_mask(struct hdmirx_ctrl_st *hdmirx)
{
	hdmirx_packet_context *packet_ctx = NULL;
	packet_ctx = hdmirx_pakcet_get_ctx();
	packet_ctx->m_const = 0;
	packet_ctx->vrr_en = 0;
	packet_ctx->base_refresh_rate = 0;
	hdmirx_set_reg(hdmirx->hdmirx_pwd_base + 0x12C4, 0x101, 32, 0);
	hdmirx_set_reg(hdmirx->hdmirx_pwd_base + 0x12D4, 0x1, 1, 5);
	hdmirx_set_reg(hdmirx->hdmirx_pwd_base + 0x13FC, 0x7, 3, 27);
}

static uint32_t hdmirx_packet_get_fvavrr_reg(struct hdmirx_ctrl_st *hdmirx)
{
	uint32_t reg;
	reg = inp32(hdmirx->hdmirx_pwd_base + REG_FVAVRRRX_WORD0); // must read this register
	mdelay(1);
	reg = inp32(hdmirx->hdmirx_pwd_base + REG_FVAVRRRX_WORD0 + 0x8);
	return reg;
}

void hdmirx_packet_vrr_get_packet(struct hdmirx_ctrl_st *hdmirx)
{
	hdmirx_packet_context *packet_ctx = NULL;
	...
	uint32_t m_const;
	uint32_t vrr_en;
	uint32_t base_refresh_rate;
	uint32_t reg;
	reg = hdmirx_packet_get_fvavrr_reg(hdmirx);
	m_const = (reg >> 25) & BIT0;
	vrr_en = (reg >> 24) & BIT0;
	base_refresh_rate = reg & BIT9_8;
	base_refresh_rate |= (reg >> 16) & BIT7_0;
	if ((packet_ctx->m_const == m_const) && (packet_ctx->vrr_en == vrr_en) &&
		(packet_ctx->base_refresh_rate == base_refresh_rate))
		return;
	packet_ctx->m_const = m_const;
	packet_ctx->vrr_en = vrr_en;
	packet_ctx->base_refresh_rate = base_refresh_rate;
	disp_pr_info("m_const:0x%x", m_const);
	disp_pr_info("vrr_en:0x%x", vrr_en);
	disp_pr_info("base_refresh_rate:0x%x", base_refresh_rate);
}

int hdmirx_packet_vrr_irq_proc(struct hdmirx_ctrl_st *hdmirx)
{
	uint32_t temp;

	temp = inp32(hdmirx->hdmirx_pwd_base + REG_EMP_INTR2);
	temp = temp & BIT29_27;

	if (!temp)
		return 0;

	hdmirx_ctrl_irq_clear(hdmirx->hdmirx_pwd_base + REG_EMP_INTR2);
	hdmirx_packet_vrr_get_packet(hdmirx);
	return 0;
}