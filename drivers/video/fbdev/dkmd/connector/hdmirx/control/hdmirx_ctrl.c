...
void hdmirx_ctrl_yuv422_set(struct hdmirx_ctrl_st *hdmirx, uint32_t enable)
{
	...
	hdmirx_set_reg(hdmirx->hdmirx_pwd_base + 0x526C, enable, 1, 0);
}

static int hdmirx_ctrl_burrs_filter_set(struct hdmirx_ctrl_st *hdmirx)
{
	// video data period filter enable
	hdmirx_set_reg(hdmirx->hdmirx_pwd_base + 0x1A100, 0, 1, 19);

	// data island period filter enable
	hdmirx_set_reg(hdmirx->hdmirx_pwd_base + 0x1A100, 0, 1, 15);

	// control period filter enable
	hdmirx_set_reg(hdmirx->hdmirx_pwd_base + 0x1A100, 0, 1, 11);

	// hsync filter enable
	hdmirx_set_reg(hdmirx->hdmirx_pwd_base + 0x1A100, 0, 1, 7);

	// vsync filter enable
	hdmirx_set_reg(hdmirx->hdmirx_pwd_base + 0x1A100, 0, 1, 3);
...
int hdmirx_ctrl_init(struct hdmirx_ctrl_st *hdmirx)
{
	...
	// TEST
	hdmirx_set_reg(hdmirx->hdmirx_aon_base + 0x10B0, 0, 32, 0);
...
void hdmirx_ctrl_source_select_hdmi(struct hdmirx_ctrl_st *hdmirx)
{
	...
	hdmirx_set_reg(hdmirx->hsdt1_sysctrl_base + 0x30, 0x4, 3, 3);
	hdmirx_set_reg(hdmirx->hsdt1_sysctrl_base + 0x30, 0x4, 3, 9);
}

void hdmirx_ctrl_set_sys_mute(struct hdmirx_ctrl_st *hdmirx, bool enable)
{
	if (enable == true)
		hdmirx_set_reg(hdmirx->hdmirx_pwd_base + 0x2c, 0x1, 1, 1);
	else
		hdmirx_set_reg(hdmirx->hdmirx_pwd_base + 0x2c, 0x1, 1, 0);
}
...
{
	hdmirx_set_reg(hdmirx->hdmirx_sysctrl_base + 0x40, 0x1, 1, 7);
}
...
void hdmirx_ctrl_mute_proc(struct hdmirx_ctrl_st *hdmirx)
{
	...
	if (temp & 0x80) {
		...
		hdmirx_set_reg(hdmirx->hdmirx_sysctrl_base + 0x44, 0x1, 1, 7);
	}
...
int hdmirx_ctrl_get_timinginfo(struct rx_interface *rx, void __user *argp)
{
	...
	if ((rx->get_timing_info == NULL) ||
		...
		disp_pr_err("get timing fail\n");
		return -1;
	}

	if (copy_to_user(argp, &timing, sizeof(timing)) != 0) {
		disp_pr_err("copy timing to user fail");
		return -1;
	}

	disp_pr_info("get timing success\n");
...
{
	impl->get_timing_info = hdmirx_video_get_timinginfo;
...
void hdmirx_hpd_retry(struct hdmirx_ctrl_st *hdmirx)
{
	...
	if (!hdmirx_is_frl_mode())
		return;

	hdmirx->phy_cnt++;
	if(hdmirx->phy_cnt <= 200)
		return;

	hdmirx_link_set_hpd_level(hdmirx, false);
	mdelay(2000);
	hdmirx_link_set_hpd_level(hdmirx, true);
	disp_pr_err("phy restart hpd");
