...
int hdmirx_link_frl_init(struct hdmirx_ctrl_st *hdmirx)
{
	...
	hdmirx_set_reg(hdmirx->hdmirx_aon_base + 0x1094, 0x21ff, 32, 0);

	hdmirx_set_reg(hdmirx->hdmirx_pwd_base + 0x0028, 0x1, 1, 0);
	hdmirx_set_reg(hdmirx->hdmirx_pwd_base + 0x0028, 0x1, 1, 3);
	hdmirx_set_reg(hdmirx->hdmirx_pwd_base + 0x0028, 0x0, 1, 2);

	if (hdmirx->fpga_flag == 1) {
		hdmirx_set_reg(hdmirx->hdmirx_pwd_base + 0x0028, 0x0, 2, 16);
	} else {
		if (g_pll_reset_1) {
			hdmirx_set_reg(hdmirx->hdmirx_pwd_base + 0x0028, 0x3, 2, 16);
		} else {
			hdmirx_set_reg(hdmirx->hdmirx_pwd_base + 0x0028, 0x0, 2, 16);
		}
	}
	...
	/* set pattern cheak timeout, set 125ms per lane */
	hdmirx_set_reg(hdmirx->hdmirx_pwd_base + 0xb06c, 0x1, 8, 0);
	hdmirx_set_reg(hdmirx->hdmirx_pwd_base + 0xb07c, 0x1, 8, 0);
	hdmirx_set_reg(hdmirx->hdmirx_pwd_base + 0xb08c, 0x1, 8, 0);
	hdmirx_set_reg(hdmirx->hdmirx_pwd_base + 0xb09c, 0x1, 8, 0);
...
int hdmirx_link_frl_lut_data(struct hdmirx_ctrl_st *hdmirx)
{
	...
	hdmirx_set_reg(hdmirx->hdmirx_pwd_base + 0xBB00, 0xa, 4, 0);
	hdmirx_set_reg(hdmirx->hdmirx_pwd_base + 0xBB00, 0x1, 1, 4);
	hdmirx_set_reg(hdmirx->hdmirx_pwd_base + 0xBB00, 0x0, 2, 5);
	...
	for (num_10b9b = 0; num_10b9b < lut_10b9b_size; num_10b9b++){
		...
		hdmirx_set_reg(hdmirx->hdmirx_pwd_base + offset_10b9b, 0, 32, 0);
	}
	...
	for (num_8b7b = 0; num_8b7b < lut_8b7b_size; num_8b7b++){
		...
		hdmirx_set_reg(hdmirx->hdmirx_pwd_base + offset_8b7b, 0, 32, 0);
	}
	hdmirx_set_reg(hdmirx->hdmirx_pwd_base + 0xBB00, 0x15, 32, 0);
	hdmirx_set_reg(hdmirx->hdmirx_pwd_base + 0xBB00, 0xa, 4, 0);
	hdmirx_set_reg(hdmirx->hdmirx_pwd_base + 0xBB00, 0x1, 1, 4);
	hdmirx_set_reg(hdmirx->hdmirx_pwd_base + 0xBB00, 0x0, 2, 5);
	num_10b9b = 0;
	...
	hdmirx_set_reg(hdmirx->hdmirx_pwd_base + 0xBB14, 0x0, 32, 0);
	hdmirx_set_reg(hdmirx->hdmirx_pwd_base + 0xBB1C, 0x0, 32, 0);
	...
	hdmirx_set_reg(hdmirx->hdmirx_pwd_base + 0xBB14, 0x1, 32, 0);
	hdmirx_set_reg(hdmirx->hdmirx_pwd_base + 0xBB1C, 0x1, 32, 0);

	lut_10b9b_size = sizeof(g_frl_10b9b_lut) / sizeof(g_frl_10b9b_lut[0]);

	for (num_10b9b = 0; num_10b9b < lut_10b9b_size; num_10b9b++){
		offset_10b9b = 0xB100 + 0x4 * num_10b9b;
		hdmirx_set_reg(hdmirx->hdmirx_pwd_base + offset_10b9b, g_frl_10b9b_lut[num_10b9b], 32, 0);
	}
	...
	for (num_8b7b = 0; num_8b7b < lut_8b7b_size; num_8b7b++){
		offset_8b7b = 0xB900 + 0x4 * num_8b7b;
		hdmirx_set_reg(hdmirx->hdmirx_pwd_base + offset_8b7b, g_frl_8b7b_lut[num_8b7b], 32, 0);
	}
	...
	hdmirx_set_reg(hdmirx->hdmirx_pwd_base + 0xBC08, 0x0, 32, 0);

	hdmirx_set_reg(hdmirx->hdmirx_pwd_base + 0xBC0C, 0x10, 32, 0);
...
int hdmirx_link_frl_ltp_gap_checkcycle(struct hdmirx_ctrl_st *hdmirx)
{
	...
	hdmirx_set_reg(hdmirx->hdmirx_pwd_base + 0xBC20, 0x0, 32, 0);
	hdmirx_set_reg(hdmirx->hdmirx_pwd_base + 0xBC24, 0x0, 32, 0);
	hdmirx_set_reg(hdmirx->hdmirx_pwd_base + 0xBC28, 0x10, 32, 0);
	return 0;
}
...
int hdmirx_link_frl_ltp_irq_set(struct hdmirx_ctrl_st *hdmirx)
{
	...
	hdmirx_set_reg(hdmirx->hdmirx_pwd_base + 0xB014, 0x1, 1, 0);
	hdmirx_set_reg(hdmirx->hdmirx_pwd_base + 0xB018, 0xf, 4, 3);
	hdmirx_set_reg(hdmirx->hdmirx_pwd_base + 0xB01C, 0x3, 2, 0);
	return 0;
}
...
{
	uint32_t frl_ready;
	hdmirx_frl_clear_interrupt(hdmirx);
	if (g_hw_init_en)
		hdmirx_set_reg(hdmirx->hdmirx_pwd_base + 0xB028, 0x1, 1, 7); // hardware set
	hdmirx_set_reg(hdmirx->hdmirx_pwd_base + 0x8500, 0x1, 1, 6);

	frl_ready = inp32(hdmirx->hdmirx_pwd_base + 0x8500);
	hdmirx_set_reg(hdmirx->hdmirx_pwd_base + 0x8440, 0x1, 1, 0);  // status update

	disp_pr_info("frl ready:0x%x\n", frl_ready);
	return 0;
}

void hdmirx_frl_clear_interrupt(struct hdmirx_ctrl_st *hdmirx)
{
	uint32_t b000;
	uint32_t b004;
	uint32_t b008;
	...
	if (b008 != 0)
		hdmirx_set_reg(hdmirx->hdmirx_pwd_base + 0xB008, b008, 8, 0);

	if (b004 != 0)
		hdmirx_set_reg(hdmirx->hdmirx_pwd_base + 0xB004, b004, 8, 0);

	if (b000 != 0)
		hdmirx_set_reg(hdmirx->hdmirx_pwd_base + 0xB000, b000, 8, 0);

	if (b00c != 0)
		hdmirx_set_reg(hdmirx->hdmirx_pwd_base + 0xB00c, b004, 8, 0);

	if (b010 != 0)
		hdmirx_set_reg(hdmirx->hdmirx_pwd_base + 0xB010, b000, 8, 0);
}

static void frl_trans_proc(struct hdmirx_ctrl_st *hdmirx)
{
	if (((inp32(hdmirx->hdmirx_pwd_base + 0x8440) & 0x10) >> 4) == 0) {
		hdmirx_set_reg(hdmirx->hdmirx_pwd_base + 0xB028, 1, 1, 2);
		hdmirx_set_reg(hdmirx->hdmirx_pwd_base + 0xB028, 1, 1, 5);
		frl_set_state(hdmirx, FRL_STATE_DONE);
	}
}

hdmirx_frl_rate hdmirx_frl_get_rate(struct hdmirx_ctrl_st *hdmirx)
{
	uint32_t rate;
	hdmirx_frl_rate frl_rate;
	rate = hdmirx_reg_read(hdmirx->hdmirx_pwd_base, REG_SINK_CONFIGURATION_B1, FRL_RATE);
	switch (rate) {
	case 0: /*  0:TMDS */
		frl_rate = HDMIRX_FRL_RATE_TMDS;
		break;
	case 1: /*  1:3L3G */
		frl_rate = HDMIRX_FRL_RATE_FRL3L3G;
		break;
	case 2: /*  2:3L6G */
		frl_rate = HDMIRX_FRL_RATE_FRL3L6G;
		break;
	case 3: /*  3:4L3G */
		frl_rate = HDMIRX_FRL_RATE_FRL4L6G;
		break;
	case 4: /*  4:4L8G */
		frl_rate = HDMIRX_FRL_RATE_FRL4L8G;
		break;
	case 5: /*  5:4L10G */
		frl_rate = HDMIRX_FRL_RATE_FRL4L10G;
		break;
	case 6: /*  6:4L12G */
		frl_rate = HDMIRX_FRL_RATE_FRL4L12G;
		break;
	default:
		frl_rate = HDMIRX_FRL_RATE_TMDS;
		break;
...
uint8_t hdmirx_scdc_get_max_ffe(struct hdmirx_ctrl_st *hdmirx)
{
	uint8_t ffe;
	ffe = (uint8_t)hdmirx_reg_read(hdmirx->hdmirx_pwd_base, REG_SINK_CONFIGURATION_B1, FFE_LEVELS);
	return ffe;
}

uint8_t hdmirx_scdc_get_ltp_req(struct hdmirx_ctrl_st *hdmirx, uint8_t lane)
{
	uint8_t req;
	switch (lane) {
	case 0: /*  lane 0 */
		req = hdmirx_reg_read(hdmirx->hdmirx_pwd_base, REG_STATUS_FLAGS_B1, LN0_LTP_REQ);
		break;
	case 1: /*  lane 1 */
		req = hdmirx_reg_read(hdmirx->hdmirx_pwd_base, REG_STATUS_FLAGS_B1, LN1_LTP_REQ);
		break;
	case 2: /*  lane 2 */
		req = hdmirx_reg_read(hdmirx->hdmirx_pwd_base, REG_STATUS_FLAGS_B2, LN2_LTP_REQ);
		break;
	case 3: /*  lane 3 */
		req = hdmirx_reg_read(hdmirx->hdmirx_pwd_base, REG_STATUS_FLAGS_B2, LN3_LTP_REQ);
		break;
	default:
		req = 0xff; /*  0xff:invalid value */
		break;
	}
	return req;
}

uint32_t hdmirx_get_lane_count(struct hdmirx_ctrl_st *hdmirx)
{
	hdmirx_frl_ctx *frl_ctx = NULL;
	frl_ctx = hdmirx_frl_get_ctx();
	if (frl_ctx->rate >= HDMIRX_FRL_RATE_FRL4L6G)
		return FRL_4_LANE;

	return FRL_3_LANE;
}

void hdmirx_frl_set_tmds_ok(struct hdmirx_ctrl_st *hdmirx, bool en)
{
	hdmirx_reg_write(hdmirx->hdmirx_pwd_base, REG_FRL_CTRL, TMDS_OK, en);
}

static void frl_set_phy_power(struct hdmirx_ctrl_st *hdmirx, hdmirx_frl_rate rate)
{
	...
	hdmirx_ctrl_set_phy_power(hdmirx, type);
}

bool hdmirx_is_frl_mode(void)
{
	hdmirx_frl_ctx *frl_ctx = NULL;
	frl_ctx = hdmirx_frl_get_ctx();
	if ((frl_ctx->rate > HDMIRX_FRL_RATE_TMDS) && (frl_ctx->rate <= HDMIRX_FRL_RATE_FRL4L12G))
		return true;

	return false;
}

static void frl_source_int_proc(struct hdmirx_ctrl_st *hdmirx)
{
	hdmirx_frl_ctx *frl_ctx = NULL;
	hdmirx_frl_rate rate;
	uint8_t ffe;
	uint8_t req[4]; /* 4: 4 lane */

	frl_ctx = hdmirx_frl_get_ctx();
	rate = hdmirx_frl_get_rate(hdmirx);
	frl_ctx->rate = rate;
	ffe = hdmirx_scdc_get_max_ffe(hdmirx);
	if (rate >= HDMIRX_FRL_RATE_MAX)
		return;

	req[0] = hdmirx_scdc_get_ltp_req(hdmirx, FRL_LANE_0);
	req[1] = hdmirx_scdc_get_ltp_req(hdmirx, FRL_LANE_1); /* 1: lane 1 */
	req[2] = hdmirx_scdc_get_ltp_req(hdmirx, FRL_LANE_2); /* 2: lane 2 */
	req[3] = hdmirx_scdc_get_ltp_req(hdmirx, FRL_LANE_3); /* 3: lane 3 */
	disp_pr_info("source start train,rate :%s, req:%d, %d, %d, %d\n",
		g_frl_rate_string[rate], req[FRL_LANE_0], req[FRL_LANE_1], req[FRL_LANE_2], req[FRL_LANE_3]);
	if (rate == HDMIRX_FRL_RATE_TMDS) {
		frl_clear_init(hdmirx);
		hdmirx_frl_set_tmds_ok(hdmirx, TRUE);
		frl_set_state(hdmirx, FRL_STATE_IDEL);
	} else if (rate < HDMIRX_FRL_RATE_MAX) {
		frl_ctx->rate = rate;
		frl_ctx->max_ffe = ffe;
		if (hdmirx->fpga_flag == 0)
			frl_set_phy_power(hdmirx, rate);
		frl_set_state(hdmirx, FRL_STATE_LTP_REQ);
	} else {
		frl_clear_init(hdmirx);
	}
}
bool hdmirx_scdc_get_flt_update(struct hdmirx_ctrl_st *hdmirx)
{
	uint32_t update;
	update = inp32(hdmirx->hdmirx_pwd_base + 0x8440) & 0x20;
	if (update)
		return true;
...
static int hdmirx_frl_start_reset(struct hdmirx_ctrl_st *hdmirx)
{
	hdmirx_set_reg(hdmirx->hdmirx_pwd_base + 0x8440, 0x0, 1, 4);  // frl start flase
	return 0;
}

static void frl_ltp_req_proc(struct hdmirx_ctrl_st *hdmirx)
{
	hdmirx_frl_ctx *frl_ctx = NULL;
	if (g_hw_init_en) {
	...
	hdmirx_frl_start_reset(hdmirx);
	if (hdmirx_scdc_get_flt_update(hdmirx))
		return;

	hdmirx_frl_req_set(hdmirx);  // soft
	frl_set_state(hdmirx, FRL_STATE_PATTEN_CHECK);
}
...
int hdmirx_frl_process(struct hdmirx_ctrl_st *hdmirx)
{
	...
	if ((frl_get_interrupt(hdmirx, HDMIRX_FRL_INT_SOURCE_FLT) == true) &&
		(frl_get_interrupt(hdmirx, HDMIRX_FRL_INT_FRL_RATE) == true)) {
		frl_clear_interrupt(hdmirx, HDMIRX_FRL_INT_SOURCE_FLT);
		frl_clear_interrupt(hdmirx, HDMIRX_FRL_INT_FRL_RATE);
		frl_source_int_proc(hdmirx);
	}
	if ((frl_ctx->rate != HDMIRX_FRL_RATE_TMDS) &&
		(hdmirx_frl_get_rate(hdmirx) == HDMIRX_FRL_RATE_TMDS)) {
		disp_pr_debug("frl rate change to TMDS\n");
		frl_clear_init(hdmirx);
	}
	switch (frl_ctx->state) {
	case FRL_STATE_LTP_REQ:
		frl_ltp_req_proc(hdmirx);
	...
	case FRL_STATE_GAP_CHECK:
		...
		break;
	case FRL_STATE_FRL_TRANS:
		frl_trans_proc(hdmirx);
		break;
	default:
		break;
	}
...
int hdmirx_frl_req_set(struct hdmirx_ctrl_st *hdmirx)
{
	...
	hdmirx_set_reg(hdmirx->hdmirx_pwd_base + 0x8504, frl_ctx->ltp_req[FRL_LANE_0], 4, 0);
	hdmirx_set_reg(hdmirx->hdmirx_pwd_base + 0x8504, frl_ctx->ltp_req[FRL_LANE_1], 4, 4);
	hdmirx_set_reg(hdmirx->hdmirx_pwd_base + 0x8508, frl_ctx->ltp_req[FRL_LANE_2], 4, 0);

	if (frl_ctx->rate >= HDMIRX_FRL_RATE_FRL4L6G)
		hdmirx_set_reg(hdmirx->hdmirx_pwd_base + 0x8508, frl_ctx->ltp_req[FRL_LANE_3], 4, 4);
...
int hdmirx_frl_flt_update(struct hdmirx_ctrl_st *hdmirx)
{
	...
	hdmirx_set_reg(hdmirx->hdmirx_pwd_base + 0x8440, 0x1, 1, 5);
...
static void frl_set_ltp_fixreq_bylane(struct hdmirx_ctrl_st *hdmirx, frl_lane lane)
{
	...
	frl_ctx = hdmirx_frl_get_ctx();
	lane_count =  hdmirx_get_lane_count(hdmirx);
	if (lane >= lane_count) {
	...
	switch (lane) {
	case 0: /*  lane 0 */
		hdmirx_set_reg(hdmirx->hdmirx_pwd_base + 0x8504, 0xE, 4, 0);
		break;
	case 1: /*  lane 1 */
		hdmirx_set_reg(hdmirx->hdmirx_pwd_base + 0x8504, 0xE, 4, 4);
		break;
	case 2: /*  lane 2 */
		hdmirx_set_reg(hdmirx->hdmirx_pwd_base + 0x8508, 0xE, 4, 0);
		break;
	case 3: /*  lane 2 */
		hdmirx_set_reg(hdmirx->hdmirx_pwd_base + 0x8508, 0xE, 4, 4);
		break;
	default:
		break;
	}
...
static void frl_pattern_timeout_proc(struct hdmirx_ctrl_st *hdmirx, uint8_t lane)
{
	...
	if (frl_ctx->ffe >= 3) {
		frl_ctx->ffe = 0;
		hdmirx_set_reg(hdmirx->hdmirx_pwd_base + 0x8504, 0xFF, 32, 0);
		hdmirx_set_reg(hdmirx->hdmirx_pwd_base + 0x8508, 0xFF, 32, 0);
		disp_pr_err("ffe : max\n");
		hdmirx_frl_flt_update(hdmirx);
		frl_set_state(hdmirx, FRL_STATE_IDEL);
		return;
...
static bool frl_is_eq_done(struct hdmirx_ctrl_st *hdmirx)
{
	phy_eq_state state;
	state = hdmirx_phy_get_eq_state(hdmirx);
	if ((state == PHY_EQ_STATE_HANDLE_DONE) || (state == PHY_EQ_STATE_HANDLE_TIMEOUT) ||
		(state == PHY_EQ_STATE_AUTO_DONE) || (state == PHY_EQ_STATE_AUTO_TIMEOUT)) {
		return TRUE;
	} else {
		return FALSE;
	}
...
int hdmirx_frl_ltp_pattern_check(struct hdmirx_ctrl_st *hdmirx)
{
	...
	while(flag){
		...
		for (lane = FRL_LANE_0; lane < hdmirx_get_lane_count(hdmirx); lane++) {
		...
		if (lane_done == hdmirx_get_lane_count(hdmirx) && (frl_is_eq_done(hdmirx) == TRUE)) {
			frl_ctx->ltp_state = HDMIRX_FRL_LTP_DONE;
			frl_set_state(hdmirx, FRL_STATE_GAP_CHECK);
			hdmirx_set_reg(hdmirx->hdmirx_pwd_base + 0x8440, 0x0, 1, 4);  // frl start flase
			hdmirx_set_reg(hdmirx->hdmirx_pwd_base + 0x8504, 0x0, 32, 0);
			hdmirx_set_reg(hdmirx->hdmirx_pwd_base + 0x8508, 0x0, 32, 0);  /* 4 lane */
...
int hdmirx_frl_start(struct hdmirx_ctrl_st *hdmirx)
{
	...
	hdmirx_set_reg(hdmirx->hdmirx_pwd_base + 0x8440, 0x1, 1, 4);
	return 0;
}
...
int hdmirx_frl_status_update(struct hdmirx_ctrl_st *hdmirx)
{
	...
	hdmirx_set_reg(hdmirx->hdmirx_pwd_base + 0x8440, 0x1, 1, 0);
	return 0;
}
...
int hdmirx_link_training_done(struct hdmirx_ctrl_st *hdmirx)
{
	...
	hdmirx_set_reg(hdmirx->hdmirx_pwd_base + 0xB028, 0x1, 1, 5);
	hdmirx_set_reg(hdmirx->hdmirx_pwd_base + 0xB028, 0x0, 1, 5);
	return 0;
}
...
void frl_init_ltp_req(struct hdmirx_ctrl_st *hdmirx)
{
	...
	hdmirx_set_reg(hdmirx->hdmirx_pwd_base + 0xB0B0, frl_ctx->ltp_req[FRL_LANE_0], 4, 0);
	hdmirx_set_reg(hdmirx->hdmirx_pwd_base + 0xB0B0, frl_ctx->ltp_req[FRL_LANE_1], 4, 4);
	hdmirx_set_reg(hdmirx->hdmirx_pwd_base + 0xB0B4, frl_ctx->ltp_req[FRL_LANE_2], 4, 0);
	hdmirx_set_reg(hdmirx->hdmirx_pwd_base + 0xB0B4, frl_ctx->ltp_req[FRL_LANE_3], 4, 4);
}
...
void frl_clear_interrupt(struct hdmirx_ctrl_st *hdmirx, hdmirx_frl_intr_type type)
{
	...
	if ((type >= HDMIRX_FRL_INT_SOURCE_FLT) && (type <= HDMIRX_FRL_INT_SOURCE_CLR_TIMEOUT)) {
		hdmirx_set_reg(hdmirx->hdmirx_pwd_base + 0xB000, reg_value, 8, 0);
	} else if ((type >= HDMIRX_FRL_INT_RSCC) && (type <= HDMIRX_FRL_INT_FRL_START)) {
		hdmirx_set_reg(hdmirx->hdmirx_pwd_base + 0xB004, reg_value, 8, 0);
	} else if ((type >= HDMIRX_FRL_INT_STATUS_UPDATE) && (type <= HDMIRX_FRL_INT_RS_UNCORR_OVER)) {
		hdmirx_set_reg(hdmirx->hdmirx_pwd_base + 0xB008, reg_value, 8, 0);
	} else if ((type >= HDMIRX_FRL_INT_LN3_NORM_DATA_ERR_EXC) && (type <= HDMIRX_FRL_INT_LN0_LTP_DATA_ERR_EXC)) {
		hdmirx_set_reg(hdmirx->hdmirx_pwd_base + 0xB00C, reg_value, 8, 0);
	} else if ((type >= HDMIRX_FRL_INT_BK_PREAMBLE_ERR) && (type <= HDMIRX_FRL_INT_RC_ERR)) {
		hdmirx_set_reg(hdmirx->hdmirx_pwd_base + 0xB010, reg_value, 8, 0);
	}
}

bool hdmirx_frl_is_ltpdone(struct hdmirx_ctrl_st *hdmirx)
{
	hdmirx_frl_ctx *frl_ctx = NULL;
	bool done;

	frl_ctx = hdmirx_frl_get_ctx();
	if (frl_ctx->state == FRL_STATE_DONE) {
		done = TRUE;
	} else {
		done = FALSE;
	}
