...
#include "hdmirx_video_dsc_reg.h"
...
#define S_YES "yes"
#define S_NO "no"
#define PACKET0_LENGTH 8
#define PACKET1TO5_LENGTH 34

#define BITS_PER_PPS 8

typedef struct {
	uint8_t range_min_qp;
	uint8_t range_max_qp;
	uint8_t range_bpg_offset;
}hdmirx_rc_range_parameters;
...
typedef struct {
	...
	uint8_t rc_tgt_offset_hi;
	uint8_t rc_tgt_offset_lo;
	uint8_t rc_buf_thresh[DSC_NUM_BUF_RANGES - 1];
	hdmirx_rc_range_parameters rc_range_parameters[DSC_NUM_BUF_RANGES];
}hdmirx_rc_parameter_set;
...
typedef struct {
	...
	hdmirx_rc_parameter_set rc_parameter_set; /* 400 bit pps38-87 */
...
typedef struct {
	...
	uint32_t packet_0[8];
	uint32_t packet_1to5[34];
	hdmirx_dsc_pps_info pps;
} hdmirx_dsc_ctx;
