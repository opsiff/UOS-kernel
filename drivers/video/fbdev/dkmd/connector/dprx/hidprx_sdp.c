...
void rx_sdp_parse_pps(struct pps_sdp *pps_table)
{
	...
	dprx_pr_info("dsc vesion: %u.%u\n", dsc.dsc_version_major, dsc.dsc_version_minor);
	dprx_pr_info("dsc.pps_identifier: %u\n", dsc.pps_identifier);
	dprx_pr_info("dsc bpc: %u\n", dsc.dsc_bpp);

	dprx_pr_info("dsc linebuf_depth: %u\n", dsc.linebuf_depth);
	dprx_pr_info("dsc block_pred_enable: %u\n", dsc.block_pred_enable);
	dprx_pr_info("dsc convert_rgb: %u\n", dsc.convert_rgb);
	dprx_pr_info("dsc simple_422: %u\n", dsc.simple_422);
	dprx_pr_info("dsc vbr_enable: %u\n", dsc.vbr_enable);
	dprx_pr_info("dsc dsc_bpp: %u\n", dsc.dsc_bpp);

	dprx_pr_info("dsc pic_height: %u\n", dsc.pic_height);
	dprx_pr_info("dsc pic_width: %u\n", dsc.pic_width);
	dprx_pr_info("dsc slice_height: %u\n", dsc.slice_height);
	dprx_pr_info("dsc slice_width: %u\n", dsc.slice_width);
	dprx_pr_info("dsc chunk_size: %u\n", dsc.chunk_size);

	dprx_pr_info("dsc initial_xmit_delay: %u\n", dsc.initial_xmit_delay);
	dprx_pr_info("dsc initial_dec_delay: %u\n", dsc.initial_dec_delay);
	dprx_pr_info("dsc initial_scale_value: %u\n", dsc.initial_scale_value);
	dprx_pr_info("dsc scale_increment_interval: %u\n", dsc.scale_increment_interval);
	dprx_pr_info("dsc scale_decrement_interval: %u\n", dsc.scale_decrement_interval);

	dprx_pr_info("dsc first_line_bpg_offset: %u\n", dsc.first_line_bpg_offset);
	dprx_pr_info("dsc nfl_bpg_offset: %u\n", dsc.nfl_bpg_offset);
	dprx_pr_info("dsc slice_bpg_offset: %u\n", dsc.slice_bpg_offset);
	dprx_pr_info("dsc initial_offset: %u\n", dsc.initial_offset);
	dprx_pr_info("dsc final_offset: %u\n", dsc.final_offset);

	dprx_pr_info("dsc flatness_min_qp: %u\n", dsc.flatness_min_qp);
	dprx_pr_info("dsc flatness_max_qp: %u\n", dsc.flatness_max_qp);

	dprx_pr_info("dsc rc_model_size: %u\n", dsc.rc_model_size);
	dprx_pr_info("dsc rc_edge_factor: %u\n", dsc.rc_edge_factor);
	dprx_pr_info("dsc rc_quant_incr_limit0: %u\n", dsc.rc_quant_incr_limit0);
	dprx_pr_info("dsc rc_quant_incr_limit1: %u\n", dsc.rc_quant_incr_limit1);
	dprx_pr_info("dsc rc_tgt_offset_hi: %u\n", dsc.rc_tgt_offset_hi);
	dprx_pr_info("dsc rc_tgt_offset_lo: %u\n", dsc.rc_tgt_offset_lo);

	for (i = 0; i < DSC_THRESH_LENGTH; i++)
		dprx_pr_info("dsc rc_buf_thresh[%u]: %u\n", i, dsc.rc_buf_thresh[i]);

	for (i = 0; i < DSC_RC_RANGE_LENGTH; i++)
		dprx_pr_info("dsc rc_range[%u]: min_qp = %u, max_qp = %u, offset = %u\n",
			i, dsc.rc_range[i].min_qp, dsc.rc_range[i].max_qp, dsc.rc_range[i].offset);

	dprx_pr_info("dsc native_420: %u\n", dsc.native_420);
	dprx_pr_info("dsc native_422: %u\n", dsc.native_422);
	dprx_pr_info("dsc second_line_bpg_offset: %u\n", dsc.second_line_bpg_offset);
	dprx_pr_info("dsc nsl_bpg_offset: %u\n", dsc.nsl_bpg_offset);
	dprx_pr_info("dsc second_line_offset_adj: %u\n", dsc.second_line_offset_adj);
}
...
void rx_sdp_init(struct pps_sdp *pps_table)
{
	...
	if (err_ret != EOK) {
		dprx_pr_err("memcpy pps_table error\n");
		return;
	}
	...
	if (err_ret != EOK) {
		dprx_pr_err("memcpy pps_payload error\n");
		return;
	}
	rx_sdp_parse_pps2(&pps_payload);
}

static void dprx_sdp_parse_rgb_colorimetry_format(uint8_t colorimetry_format)
{
	switch (colorimetry_format) {
	case 0:
		dprx_pr_info("[DPRX] sRGB (IEC 61966-2-1)\n");
		break;
	case 1:
		dprx_pr_info("[DPRX] RGB wide gamut fixed point\n");
		break;
	case 2:
		dprx_pr_info("[DPRX] RGB wide gamut floating point\n");
		break;
	case 3:
		dprx_pr_info("[DPRX] Adobe RGB\n");
		break;
	case 4:
		dprx_pr_info("[DPRX] DCI-P3\n");
		break;
	case 5:
		dprx_pr_info("[DPRX] Custom Color Profile\n");
		break;
	case 6:
		dprx_pr_info("[DPRX] ITU-R BT.2020 R'G'B'\n");
		break;
	default:
		break;
	}
...
static void dprx_sdp_parse_yuv_colorimetry_format(uint8_t colorimetry_format)
{
	switch (colorimetry_format) {
	case 0:
		dprx_pr_info("[DPRX] ITU-R BT.601\n");
		break;
	case 1:
		dprx_pr_info("[DPRX] ITU-R BT.709\n");
		break;
	case 2:
		dprx_pr_info("[DPRX] xvYCC601\n");
		break;
	case 3:
		dprx_pr_info("[DPRX] xvYCC709\n");
		break;
	case 4:
		dprx_pr_info("[DPRX] sYCC601\n");
		break;
	case 5:
		dprx_pr_info("[DPRX] opYCC601\n");
		break;
	case 6:
		dprx_pr_info("[DPRX] ITU-R BT.2020 Y'C_C'BC_C'RC\n");
		break;
	case 7:
		dprx_pr_info("[DPRX] ITU-R BT.2020 Y'C'BC'R\n");
		break;
	default:
		break;
	}
...
static void dprx_sdp_parse_color_depth(uint8_t color_depth_indication)
{
	...
	case 0:
		dprx_pr_info("[DPRX] bpc = 6\n");
		break;
	case 1:
		dprx_pr_info("[DPRX] bpc = 8\n");
		break;
	case 2:
		dprx_pr_info("[DPRX] bpc = 10\n");
		break;
	case 3:
		dprx_pr_info("[DPRX] bpc = 12\n");
		break;
	case 4:
		dprx_pr_info("[DPRX] bpc = 16\n");
		break;
	default:
		break;
	}
...
static int dprx_sdp_parse_vsc_sdp(uint8_t *db_data)
{
	...
	dprx_pr_info("[DPRX] db16: 0x%u, db17: 0x%u\n", db_data[16], db_data[17]);
	...
	case SDP_RGB:
		dprx_pr_info("[DPRX] pixel_encoding: RGB\n");
		dprx_sdp_parse_rgb_colorimetry_format(colorimetry_format);
		dprx_sdp_parse_color_depth(color_depth_indication);
		break;
	case SDP_YCBCR444:
		dprx_pr_info("[DPRX] pixel_encoding: YCbCr 444\n");
		dprx_sdp_parse_yuv_colorimetry_format(colorimetry_format);
		dprx_sdp_parse_color_depth(color_depth_indication);
	case SDP_YCBCR422:
		dprx_pr_info("[DPRX] pixel_encoding: YCbCr 422\n");
		dprx_sdp_parse_yuv_colorimetry_format(colorimetry_format);
		dprx_sdp_parse_color_depth(color_depth_indication);
	case SDP_YCBCR420:
		dprx_pr_info("[DPRX] pixel_encoding: YCbCr 420\n");
		dprx_sdp_parse_yuv_colorimetry_format(colorimetry_format);
		dprx_sdp_parse_color_depth(color_depth_indication);
		break;
	case SDP_YONLY:
		dprx_pr_info("[DPRX] pixel_encoding: YONLY\n");
		break;
	case SDP_RAW:
		dprx_pr_info("[DPRX] pixel_encoding: RAW\n");
		break;
	default:
		break;
	}
...
static void dprx_parse_pps_sdp(uint32_t db_data_len, uint8_t *db_data, struct dprx_ctrl *dprx)
{
	...
	if (db_data_len != PPS_SIZE) {
		dprx_pr_err("[DPRX] PPS_SDP len error\n");
		return;
	}
	rx_sdp_parse_pps2((struct drm_dsc_picture_parameter_set *)db_data);
	reg = dprx_readl(dprx, 0x160);
	if (unlikely(dsc_cfg->slice_width == 0)) {
		dprx_pr_err("dsc_cfg->slice_width is error\n");
		return;
	}
	if (reg & BIT(0)) {
		dprx->dsc_en = 1;
		reg = dprx_readl(dprx, DPRX_SCTRL_OFFSET + DPRX_DSC_CTRL1);
		reg &= ~BIT(5);
		dprx_writel(dprx, DPRX_SCTRL_OFFSET + DPRX_DSC_CTRL1, reg);
		reg = 0x0;
		reg |= (0xFFFF & (dsc_cfg->slice_chunk_size));
		reg |= ((0xFFFF & (dsc_cfg->slice_width)) << 16);
		dprx_writel(dprx, DPRX_SCTRL_OFFSET + DPRX_DSC_CTRL0, reg);
		reg = dprx_readl(dprx, DPRX_SCTRL_OFFSET + DPRX_DSC_CTRL1);
		reg = 0x1F & ((dsc_cfg->pic_width / dsc_cfg->slice_width) - 1);
		reg |= BIT(5);
		dprx_writel(dprx, DPRX_SCTRL_OFFSET + DPRX_DSC_CTRL1, reg);
...
static int dprx_parse_vsc_sdp(struct sdp_hb_fifo_data *hb_data, uint8_t *db_data, uint32_t db_data_len)
{
	...
	if (hb_data->header.headers.hb2 != 0x5 || hb_data->header.headers.hb3 != 0x13) {
		dprx_pr_info("[DPRX] VSC_SDP dont contain Pixel Encoding/Colorimetry Format indication\n");
		return ret;
	}
	if (db_data_len != SDP_COMMON_PAYLOAD_SIZE) {
		dprx_pr_err("[DPRX] VSC_SDP len error\n");
		return ret;
	}
									...
									uint32_t db_data_len)
{
	...
	case DISPLAYPORT_RESERVED2_SDP:
		dprx_pr_info("[DPRX] SDP_Type: DisplayPort Reserved\n");
		break;
	case AUDIO_TIMESTAMP_SDP:
		dprx_pr_info("[DPRX] SDP_Type: AUDIO_TIMESTAMP_SDP\n");
		break;
	case AUDIO_STREAM_SDP:
		dprx_pr_info("[DPRX] SDP_Type: AUDIO_STREAM_SDP\n");
		break;
	case EXTENSION_SDP:
		dprx_pr_info("[DPRX] SDP_Type: EXTENSION_SDP\n");
		break;
	case AUDIO_COPY_MANAGEMENT_SDP:
		dprx_pr_info("[DPRX] SDP_Type: AUDIO_COPY_MANAGEMENT_SDP\n");
		break;
	case ISRC_SDP:
		dprx_pr_info("[DPRX] SDP_Type: ISRC_SDP\n");
		break;
	case VSC_SDP:
		dprx_pr_info("[DPRX] SDP_Type: VSC_SDP\n");
		/* for test, todo */
		ret = dprx_parse_vsc_sdp(hb_data, db_data, db_data_len);
		break;
	case PPS_SDP:
		dprx_pr_info("[DPRX] SDP_Type: PPS_SDP\n");
		/* for test, todo */
		dprx_parse_pps_sdp(db_data_len, db_data, dprx);
		dprx->dsc_en = 1;
		break;
	case VSC_EXT_VESA_SDP:
		dprx_pr_info("[DPRX] SDP_Type: VSC_EXT_VESA_SDP\n");
		break;
	case VSC_EXT_CTA_SDP:
		dprx_pr_info("[DPRX] SDP_Type: VSC_EXT_CTA_SDP\n");
		break;
	case ADAPTIVE_SYNC_SDP:
		dprx_pr_info("[DPRX] SDP_Type: ADAPTIVE_SYNC_SDP\n");
		break;
	case VENDOR_SPECIFIC_INFOFRAME_SDP:
		dprx_pr_info("[DPRX] SDP_Type: VENDOR_SPECIFIC_INFOFRAME_SDP\n");
		break;
	case AVI_INFOFRAME_SDP:
		dprx_pr_info("[DPRX] SDP_Type: AVI_INFOFRAME_SDP\n");
		break;
	case SPD_INFOFRAME_SDP:
		dprx_pr_info("[DPRX] SDP_Type: SPD_INFOFRAME_SDP\n");
		break;
	case AUDIO_INFOFRAME_SDP:
		dprx_pr_info("[DPRX] SDP_Type: AUDIO_INFOFRAME_SDP\n");
		break;
	case MPEG_SOURCE_INFOFRAME_SDP:
		dprx_pr_info("[DPRX] SDP_Type: MPEG_SOURCE_INFOFRAME_SDP\n");
		break;
	case NTSC_VBI_INFOFRAME_SDP:
		dprx_pr_info("[DPRX] SDP_Type: NTSC_VBI_INFOFRAME_SDP\n");
		break;
	case DYNAMIC_RANGE_AND_MASTERING_INFOFRAME_SDP:
		dprx_pr_info("[DPRX] SDP_Type: DYNAMIC_RANGE_AND_MASTERING_INFOFRAME_SDP\n");
		/* todo */
		break;
	default:
		break;
...
static int dprx_sdp_parse(struct dprx_ctrl *dprx)
{
	...
	for (i = 0; i < sdp_raw_data->hb_data_len; i++) {
		...
		if (data_len_sum > sdp_raw_data->db_data_len) {
			dprx_pr_err("[DPRX] SDP parse error\n");
			return -1;
		}
		dprx_pr_info("[DPRX] SDP %u len: %u\n", i, data_len);
...
{
	if (reg & EXT_SDP_TYPE)
		dprx_pr_info("[DPRX] HW_PARSE_SDP_Type: EXTENSION_SDP\n");
	if (reg & COPY_MANAG_SDP_TYPE)
		dprx_pr_info("[DPRX] HW_PARSE_SDP_Type: AUDIO_COPY_MANAGEMENT_SDP\n");
	if (reg & ISRC_SDP_TYPE)
		dprx_pr_info("[DPRX] HW_PARSE_SDP_Type: ISRC_SDP\n");
	if (reg & VSC_SDP_TYPE)
		dprx_pr_info("[DPRX] HW_PARSE_SDP_Type: VSC_SDP\n");
	if (reg & PPS_SDP_TYPE)
		dprx_pr_info("[DPRX] HW_PARSE_SDP_Type: PPS_SDP\n");
	if (reg & VEN_SPEC_INFOFRAME_SDP_TYPE)
		dprx_pr_info("[DPRX] HW_PARSE_SDP_Type: VENDOR_SPECIFIC_INFOFRAME_SDP\n");
	if (reg & AVI_INFOFRAME_SDP_TYPE)
		dprx_pr_info("[DPRX] HW_PARSE_SDP_Type: AVI_INFOFRAME_SDP\n");
	if (reg & SPD_INFOFRAME_SDP_TYPE)
		dprx_pr_info("[DPRX] HW_PARSE_SDP_Type: SPD_INFOFRAME_SDP\n");
	if (reg & AUDIO_INFOFRAME_SDP_TYPE)
		dprx_pr_info("[DPRX] HW_PARSE_SDP_Type: AUDIO_INFOFRAME_SDP\n");
	if (reg & MPEG_INFOFRAME_SDP_TYPE)
		dprx_pr_info("[DPRX] HW_PARSE_SDP_Type: MPEG_SOURCE_INFOFRAME_SDP\n");
	if (reg & VBI_INFOFRAME_SDP_TYPE)
		dprx_pr_info("[DPRX] HW_PARSE_SDP_Type: NTSC_VBI_INFOFRAME_SDP\n");
	if (reg & DRM_INFOFRAME_SDP_TYPE)
		dprx_pr_info("[DPRX] HW_PARSE_SDP_Type: DYNAMIC_RANGE_AND_MASTERING_INFOFRAME_SDP\n");
}
