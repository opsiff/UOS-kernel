...
static int check_aux_status(struct dp_ctrl *dptx, bool *try_again, bool rw)
{
	...
	case DPTX_CFG_AUX_STATUS_I2C_NACK:
		dpu_pr_warn("[DP] AUX Nack, aux_reply_err_detected is 0x%x", aux_reply_err_detected);
		*try_again = true;
		break;
	case DPTX_CFG_AUX_STATUS_AUX_DEFER:
	case DPTX_CFG_AUX_STATUS_I2C_DEFER:
		dpu_pr_info("[DP] AUX Defer, aux_reply_err_detected is 0x%x", aux_reply_err_detected);
		*try_again = true;
		break;
	default:
...
again:
	tries++;
	dpu_check_and_return((tries > MAX_AUX_RETRY_COUNT), -EAGAIN, warn, "[DP] AUX exceeded retries");
	...
	while (true) {
		...
		aux_req_count++;
		if (aux_req_count > CHECK_AUX_REQ_COUNT) {
			// if dp is plug out, aux is not connnected, this is normal.
			if (dptx->dptx_enable) {
				dpu_pr_err("[DP] wait aux_req=0 exceeded retries");
			} else {
				dpu_pr_warn("[DP] wait aux_req=0 exceeded retries");
			}
			return -ETIMEDOUT;
		}
	...
	if (g_dp_set_auxfailed == DPTX_AUXFAILED_SEC &&
			dptx->connector->conn_info->base.mode != EDP_MODE) {
		mdelay(1);
		goto again;
	} else if (g_dp_set_auxfailed == DPTX_AUXFAILED_ALL) {
		mdelay(1);
		goto again;
	}
