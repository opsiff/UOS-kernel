...
{
	uint32_t divide_val;
	uint32_t slice_height_limit;
	struct dkmd_connector_info *pinfo = NULL;

	dpu_check_and_return(!dptx, -EINVAL, err, "[DP] null pointer\n"); 
	pinfo = dptx->connector->conn_info;
	dpu_check_and_return(!pinfo, -EINVAL, err, "[DP] pinfo is null pointer");

	divide_val = PIC_HEIGHT_DIV_INIT;
	slice_height_limit = DSS_SLICE_HEIGHT_LIMIT;

	dpu_pr_info("[DP] %s pic_height=%x\n", __func__, pic_height);

	while (!((pic_height / (uint32_t)divide_val) < slice_height_limit))
		divide_val++; /* divide to 2 */

	if (pinfo->dsc_height_div != 0)
		divide_val = pinfo->dsc_height_div;
...
static void dptx_dsc_adjust_htotal(struct dp_ctrl *dptx, uint32_t div_max)
{
	uint32_t h_total;
	...
	if (h_total % div_max != 0)
		mdtd->h_blanking = mdtd->h_blanking - (h_total % div_max) + div_max;
}
...
int dptx_dsc_get_clock_div(struct dp_ctrl *dptx)
{
	...
	if (dptx->dp_panel_mgr && (dptx->dp_panel_mgr->adjust_htotal != 0)) {
		dptx_dsc_adjust_htotal(dptx, div_max);
		h_total = mdtd->h_active + mdtd->h_blanking;
		dpu_pr_info("[DP] new h_total=%u h_active=%u h_blanking=%u div_max=%u",
				h_total, mdtd->h_active, mdtd->h_blanking, div_max);
	}
...
static void ldi_clock_delay_calculation(struct dp_ctrl *dptx)
{
	...
	dpu_check_and_no_retval(!connector, err, "[DP] null pointer\n");
	pinfo = dptx->connector->conn_info;
	...
	if (connector->active_idx != 0 && connector->post_info[1] == NULL) {
		dpu_pr_err("[DP] [HiDP] active_idx %d not support\n", connector->active_idx);
		connector->active_idx = 0;
	}
	connector->ldi.h_front_porch = h_front_porch_new * 2;
	connector->ldi.h_pulse_width = h_sync_width_new * 2;
	connector->ldi.h_back_porch = h_back_porch_new * 2;
	dptx->vparams.dp_dsc_info.h_active_new = h_active_new * 2;
	connector->post_info[connector->active_idx]->dsc.dsc_info.output_width = dptx->vparams.dp_dsc_info.h_active_new;
	connector->post_info[connector->active_idx]->dsc.dsc_info.output_height = mdtd->v_active;
	pinfo->base.comp_obj_info->dsc_out_width =
		pinfo->base.dsc_out_width = connector->post_info[connector->active_idx]->dsc.dsc_info.output_width;
	pinfo->base.comp_obj_info->dsc_out_height =
		pinfo->base.dsc_out_height = connector->post_info[connector->active_idx]->dsc.dsc_info.output_height;
	pinfo->base.comp_obj_info->dsc_en =
		pinfo->base.dsc_en = connector->post_info[connector->active_idx]->dsc.dsc_en = 1;
	dpu_pr_info("[DP]dsc_info output_width*output_height %u*%u\n",
		connector->post_info[connector->active_idx]->dsc.dsc_info.output_width,
		connector->post_info[connector->active_idx]->dsc.dsc_info.output_height);
	dpu_pr_info("[DP]comp_obj_info dsc_out_width*dsc_out_height: %d*%d\n",
		pinfo->base.comp_obj_info->dsc_out_width, pinfo->base.comp_obj_info->dsc_out_height);
	dsc_calculation(&(connector->post_info[connector->active_idx]->dsc), pinfo->ifbc_type);
}
...
void dptx_dsc_dss_config(struct dp_ctrl *dptx)
{
	...
	if (dptx->connector->active_idx != 0 && dptx->connector->post_info[1] == NULL) {
		dpu_pr_err("[DP] active_idx %d not support\n", dptx->connector->active_idx);
		dptx->connector->active_idx = 0;
	}
	p_dsc->vesa_dsc_info_calc(NULL, &(dptx->connector->post_info[dptx->connector->active_idx]->dsc.dsc_info),
...
void dptx_dsc_sdp_manul_send(struct dp_ctrl *dptx, bool enable)
{
	...
	if (enable) {
		/* manual dsc sdp send */
		dpu_pr_info("[DP] start dptx_dsc_sdp_manul_send");
...
static void dptx_dsc_calc_timestamp(struct dp_ctrl *dptx)
{
	...
	dpu_pr_debug("[DP] [HIDP dsc] 0x25c write reg is %x\n", reg);
