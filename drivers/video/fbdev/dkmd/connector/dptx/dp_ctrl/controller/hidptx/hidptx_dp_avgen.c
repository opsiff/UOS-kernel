...
#define EDP_BACKLIGHT_MODE	0x2
#define EDP_BACKLIGHT_ENABLE	0x1
#define CLOSE_FEC_WAIT_TIME 10 /* Wait for 10 ms after the FEC is disabled */
#define DP_MISC_DCI_P3 7 /* DP1.4 MISC 1110 */
...
void dptx_hdr_infoframe_set_reg(struct dp_ctrl *dptx, uint8_t enable)
{
	...
	dpu_check_and_no_retval(!dptx, err, "[DP] null pointer");
	dpu_check_and_no_retval(!dptx->dptx_enable || dptx->power_saving_mode, err, "[DP] dptx has already off");
	...
	for (i = 0; i < DPTX_SDP_LEN; i++) {
		...
		dpu_pr_debug("[DP] hdr_sdp_data.payload[%d]: %x, reg: 0x%x", i, hdr_sdp_data.payload[i], reg);
	}
...
int dptx_psr_sdp_te_config(struct dp_ctrl *dptx, uint32_t payload)
{
	uint32_t reg = 0;

	dpu_check_and_return(!dptx, IRQ_NONE, err, "[DP] null pointer");

	mutex_lock(&dptx->dptx_mutex);
	if (!dptx->dptx_enable || dptx->power_saving_mode || dptx->is_power_offing) {
		dpu_pr_warn("[DP] dptx is not ready, dptx_enable:%d, power_saving_mode:%d, is_power_offing:%d\n",
			dptx->dptx_enable, dptx->power_saving_mode, dptx->is_power_offing);
		mutex_unlock(&dptx->dptx_mutex);
		return -1;
	}

	reg = dptx_readl(dptx, DPTX_VSC_SDP_EXTENSION_FOR_COLORIMETRY);
	reg &= ~DPTX_VSC_SDP_EXTENSION_TE_CONFIG_MASK;
	reg |= (payload << 19);
	dpu_pr_debug("[DP] dptx vsc sdp config db18~db16 is: 0x%x", reg);
	dptx_writel(dptx, DPTX_VSC_SDP_EXTENSION_FOR_COLORIMETRY, reg);
...
void dptx_sdp_config(struct dp_ctrl *dptx, int stream, enum dptx_sdp_config_type sdp_type)
{
	...
	dpu_pr_info("[DP] dptx sdp config");
	if (dptx->mst)
...
int dptx_fec_enable(struct dp_ctrl *dptx, bool fec_enable)
{
	...
	if (fec_enable) {
		...
		msleep(CLOSE_FEC_WAIT_TIME); // wait 10ms to close fec
	}
...
static void dptx_video_mst_ts_calculate(struct dp_ctrl *dptx, int link_rate, uint8_t *tu, uint8_t *tu_frac)
{
	...
	if (!dptx->dptx_link_params.compress_params.fec) {
	...
	*tu = (uint8_t)(mst_tu_temp / FLOAT_ACCURACY_CONVERT);

	mst_tu_frac = mst_tu_temp - (*tu) * FLOAT_ACCURACY_CONVERT;

	mst_tu_frac_mod = (mst_tu_frac * 64) % 100;

	*tu_frac = (uint8_t)((mst_tu_frac * 64) / FLOAT_ACCURACY_CONVERT);
	...
	int bpc, int encoding, uint64_t pixel_clock)
{
	...
	dptx->video_tu[NORMAL_SCENE].aver_bytes_per_tu = (uint8_t)tu;
	dptx->video_tu[NORMAL_SCENE].aver_bytes_per_tu_frac = (uint8_t)tu_frac;

	dpu_pr_info("NORMAL_SCENE tu=%u, tu_frac=%u", dptx->video_tu[NORMAL_SCENE].aver_bytes_per_tu, 
		dptx->video_tu[NORMAL_SCENE].aver_bytes_per_tu_frac);

	if (dptx->dsc) {
		dsc_tu_temp = (uint32_t)(((64 * 6 * (pixel_clock / 1000)) *
		...
		dptx->video_tu[DSC_SCENE].aver_bytes_per_tu = (uint8_t)tu;
		dptx->video_tu[DSC_SCENE].aver_bytes_per_tu_frac = (uint8_t)tu_frac;

		dpu_pr_info("DSC_SCENE tu=%u, tu_frac=%u", dptx->video_tu[DSC_SCENE].aver_bytes_per_tu, 
			dptx->video_tu[DSC_SCENE].aver_bytes_per_tu_frac);
	}
	/* mst: recalc tu and tu_frac */
	if (dptx->mst) {
		dptx_video_mst_ts_calculate(dptx, link_rate, &dptx->video_tu[MST_SCENE].aver_bytes_per_tu,
			&dptx->video_tu[MST_SCENE].aver_bytes_per_tu_frac);
	}

	vparams->init_threshold = 0xf;

	if (dptx->dsc) {
		tu = dptx->video_tu[DSC_SCENE].aver_bytes_per_tu;
		tu_frac = dptx->video_tu[DSC_SCENE].aver_bytes_per_tu_frac;
	} else {
		tu = dptx->video_tu[NORMAL_SCENE].aver_bytes_per_tu;
		tu_frac = dptx->video_tu[NORMAL_SCENE].aver_bytes_per_tu_frac;
	}

	if (dptx->mst) {
		tu = dptx->video_tu[MST_SCENE].aver_bytes_per_tu;
		tu_frac = dptx->video_tu[MST_SCENE].aver_bytes_per_tu_frac;
	}

	dpu_pr_info("[DP] tu=%d, ts=%d, tu_frac=%d, col_dep:%d, pix_clock:%llu, lane_num:%d, link_rate:%d",
...
void dptx_video_ts_change(struct dp_ctrl *dptx, int stream)
{
	...
	if (dptx->mst) {
		...
		dpu_pr_debug("[DP] vparams->aver_bytes_per_tu %d", vparams->aver_bytes_per_tu);
		...
		dpu_pr_debug("[DP] stream %d, reg 0x%x", stream, reg);
...
void dptx_video_set_sink_col(struct dp_ctrl *dptx, int stream)
{
	...
	case RGB:
		if (dynamic_range == CEA) {
			...
			col_mapping = 0;
			if (dptx->dp_dci_p3_config == 1)
				col_mapping = DP_MISC_DCI_P3;
		}
		break;
	case YCBCR422:
...
int dptx_link_timing_config(struct dp_ctrl *dptx)
{
	...
	if (dptx->dptx_link_params.compress_params.fec && dptx->dptx_fec_enable) {
		...
		if (retval) {
			...
			dp_maintenance_add_error_log(dptx, "fec enable failed");
			return retval;
		}
	}
...
void dptx_link_close_stream(struct dp_ctrl *dptx)
{
	...
	/* set 0x708 to 0x20000 */
	dptx_writel(dptx, DPTX_GCTL0, 0x20000);
...
void dptx_psr2_config_su_region(struct dp_ctrl *dptx)
{
	uint32_t reg = 0;
	...
	reg |= dptx->su_rect.top_x;
	reg |= (dptx->su_rect.top_y << 16);
	dptx_writel(dptx, DPTX_SU_COO_TL, reg);
	dpu_pr_debug("[DP] set start region: 0x%x, top_x:%d, top_y:%d", reg, dptx->su_rect.top_x, dptx->su_rect.top_y);
 
	reg = dptx_readl(dptx, DPTX_SU_COO_BR);
	reg &= ~DPTX_CFG_SU_COO_BR_X_MASK;
	reg &= ~DPTX_CFG_SU_COO_BR_Y_MASK;
	reg |= dptx->su_rect.bottom_x;
	reg |= (dptx->su_rect.bottom_y << 16);
	dptx_writel(dptx, DPTX_SU_COO_BR, reg);
	dpu_pr_debug("[DP] set end region: 0x%x, bottom_x:%d, bottom_y:%d", reg, dptx->su_rect.bottom_x, dptx->su_rect.bottom_y);
}

void dptx_aux_set_backlight_config(struct dp_ctrl *dptx)
{
	int ret;
	uint8_t reg = 0;
	...
	// config MSB/LSB mode.
	ret = dptx_read_dpcd(dptx, EDP_BACKLIGHT_MODE_SET, &reg);
	if (ret != 0)
		dpu_pr_err("[DP] EDP_BACKLIGHT_MODE_SET read failed. port[%d]", dptx->port_id);
	reg |= EDP_BACKLIGHT_MODE;
	ret = dptx_write_dpcd(dptx, EDP_BACKLIGHT_MODE_SET, reg);
	if (ret != 0)
		dpu_pr_err("[DP] EDP_BACKLIGHT_MODE_SET set failed.  port[%d]", dptx->port_id);

	// enable edp aux set backlight
	ret = dptx_read_dpcd(dptx, EDP_DISPLAY_CONTROL, &reg);
	if (ret != 0)
		dpu_pr_err("[DP] EDP_DISPLAY_CONTROL read failed. port[%d]", dptx->port_id);
	reg |= EDP_BACKLIGHT_ENABLE;
	ret = dptx_write_dpcd(dptx, EDP_DISPLAY_CONTROL, reg);
	if (ret != 0)
		dpu_pr_err("[DP] EDP_DISPLAY_CONTROL set failed. port[%d]", dptx->port_id);

	// read MAX addr value
	ret = dptx_read_dpcd(dptx, EDP_PWMGEN_BIT_COUNT_CAP_MAX, &reg);
	if (ret != 0)
		dpu_pr_err("[DP] get EDP_PWMGEN_BIT_COUNT_CAP_MAX failed. port[%d]", dptx->port_id);

	// config bit count
	ret = dptx_write_dpcd(dptx, EDP_PWMGEN_BIT_COUNT, reg);
	if (ret != 0)
		dpu_pr_err("[DP] EDP_PWMGEN_BIT_COUNT set failed. port[%d]", dptx->port_id);

	// get capabilities
	ret = dptx_read_dpcd(dptx, EDP_BACKLIGHT_ADJUSTMENT_CAPABILITIES, &reg);
	if (ret != 0)
		dpu_pr_err("[DP] get backlight capa failed. port[%d]", dptx->port_id);
	if ((reg & EDP_CAPABILITIES_MSB_LSB_MODE) == EDP_CAPABILITIES_MSB_LSB_MODE) {
		dpu_pr_info("[DP] support LSB/MSB config.\n");
		dptx->is_support_backlight_lmsb_mode = true;
	}
}

void dptx_underflow_pre_handle(struct dp_ctrl *dptx)
{
	...
	if (dptx->psr_params.is_psr_active && is_dpu_pu_support(&pinfo->base)) {
		...
		dpu_pr_info("current su region[%u %u %u %u]", su_val.top_x, su_val.top_y, su_val.bottom_x, su_val.bottom_y);
	}
}

bool dptx_get_enable_status(struct dp_ctrl *dptx)
{
	if (!dptx || !dptx->dptx_enable || dptx->power_saving_mode || dptx->is_power_offing) {
		dpu_pr_warn("[DP] dptx is not exist or has already been off!");
		return false;
	}
...
void dptx_get_tu_and_bpc_value(struct dp_ctrl *dptx)
{
	dpu_check_and_no_retval(!dptx, err, "[DP] null pointer");

	if (dptx->dsc) {
		dptx->vparams.bpc = COLOR_DEPTH_10;
		dptx->vparams.aver_bytes_per_tu = dptx->video_tu[DSC_SCENE].aver_bytes_per_tu;
		dptx->vparams.aver_bytes_per_tu_frac = dptx->video_tu[DSC_SCENE].aver_bytes_per_tu_frac;
		dpu_pr_info("DSC mode, tu=%u, tu_frac=%u, bpc=%d", dptx->vparams.aver_bytes_per_tu,
			dptx->vparams.aver_bytes_per_tu_frac, dptx->vparams.bpc);
	} else {
		dptx->vparams.bpc = dptx->bit_depth;
		dptx->vparams.aver_bytes_per_tu = dptx->video_tu[NORMAL_SCENE].aver_bytes_per_tu;
		dptx->vparams.aver_bytes_per_tu_frac = dptx->video_tu[NORMAL_SCENE].aver_bytes_per_tu_frac;
		if (dptx->dptx_get_sync_calib_param)
			dptx->dptx_get_sync_calib_param(dptx, 0); // get_calib_param for no_dsc
		dpu_pr_info("No DSC mode, tu=%u, tu_frac=%u, bpc=%d", dptx->vparams.aver_bytes_per_tu,
			dptx->vparams.aver_bytes_per_tu_frac, dptx->vparams.bpc);
	}

	if (dptx->mst) {
		dptx->vparams.aver_bytes_per_tu = dptx->video_tu[MST_SCENE].aver_bytes_per_tu;
		dptx->vparams.aver_bytes_per_tu_frac = dptx->video_tu[MST_SCENE].aver_bytes_per_tu_frac;
		dpu_pr_info("Mst mode, tu=%u, tu_frac=%u", dptx->vparams.aver_bytes_per_tu, dptx->vparams.aver_bytes_per_tu_frac);
	}
}
