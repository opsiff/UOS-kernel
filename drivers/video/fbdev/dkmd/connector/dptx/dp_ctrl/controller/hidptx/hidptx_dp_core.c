...
void dptx_global_intr_en(struct dp_ctrl *dptx)
{
	...
	irq_ien = DPTX_IRQ_VSYNC | DPTX_IRQ_STREAM0_UNDERFLOW | DPTX_IRQ_VACTIVE_START;
	...
	if (dptx->connector->conn_info->base.mode == EDP_MODE &&
			dptx->psr_params.psr_version >= DP_PSR_IS_SUPPORTED)
		dptx_psr_intr_en(dptx);

	dpu_pr_info("[DP] DPTX set intr_en\n");
}
...
static int dptx_core_on(struct dp_ctrl *dptx)
{
	...
	switch_notification_event_unchange_state(&dptx->sdev, HOT_PANEL_ON);
	return 0;
}
/* HDCP_STOP_WAIT_TIME_COUNT should not be too small. Timeout might cause system reset.
 * soft_authed_encrypted_normal in hdcp_dptx_drv will delay 600ms according to HDCP protocol
 * So theoretically, the power-on and power-off messages are blocked for a maximum of 600 ms.
 * The preceding scenario is a low probability scenario.
 * Pressure Test: 46/49 < 20ms; 3/49 >= 80ms; 1/49 >= 100ms
 */
#define HDCP_STOP_WAIT_TIME_COUNT 300 /* 600ms */
#define HDCP_STOP_CHECK_INTERVAL 2

static void wait_hdcp_quit(uint32_t port_id)
{
	int counter = 0;
	while (counter <= HDCP_STOP_WAIT_TIME_COUNT) {
		if (is_hdcp_busy(port_id)) {
			if (counter == HDCP_STOP_WAIT_TIME_COUNT) {
				dpu_pr_warn("[DP] wait port[%u] hdcp quit TIMEOUT!", port_id);
				break;
			}
	...
	dpu_pr_info("[DP] wait port[%u] hdcp quit time %d ms", port_id, counter * HDCP_STOP_CHECK_INTERVAL);
}

static void dptx_core_off(struct dp_ctrl *dptx)
{
	dpu_check_and_no_retval(!dptx, err, "[DP] dptx is NULL");

	switch_notification_event_unchange_state(&dptx->sdev, HOT_PANEL_OFF);
	wait_hdcp_quit(dptx->port_id); // wait for hdcp ta exit
...
static void dptx_free_lanes(struct dp_ctrl *dptx)
{
	dpu_check_and_no_retval(!dptx, err, "[DP] null pointer");
...
int dptx_phy_wait_busy(struct dp_ctrl *dptx, uint32_t lanes)
{
	...
	do {
		...
		if ((phyifctrl & mask) == mask) {
			dptx_read_phy_reg(dptx, DPTX_PHYIF_STATUS0, false);
			return 0;
		}
		udelay(1);
	} while (retry_cnt++ < MAX_DP_TIMEOUT_WAIT);

	dptx_read_phy_reg(dptx, DPTX_PHYIF_STATUS0, true);
	dpu_pr_warn("[DP] PHY BUSY timed out, phyifctrl = 0x%x, mask = 0x%x", phyifctrl, mask);
...
static int dptx_phy_wait_changed_lane_busy(struct dp_ctrl *dptx, uint32_t phyifctrl, uint32_t orig_phyifctrl)
{
	...
	dptx_read_phy_reg(dptx, DPTX_PHYIF_STATUS0, true);
	dpu_pr_warn("[DP] PHY BUSY timed out, phyifctrl = 0x%x, val = 0x%x, mask = 0x%x", phyifctrl, val, mask);
...
void dptx_phy_set_lanes_power(struct dp_ctrl *dptx, bool mainlink_open)
{
	uint32_t phyifctrl;
	uint32_t lanes;
	...
	case 2:
	case 1:
		phyifctrl |= ((dptx->lane_low_power_mode << DPTX_LANE3_PHY_STATUS_SHIFT) |
			(dptx->lane_low_power_mode << DPTX_LANE2_PHY_STATUS_SHIFT));
		phyifctrl |= dptx->lane_low_power_mode << DPTX_LANE1_PHY_STATUS_SHIFT;
		phyifctrl |= dptx->lane_low_power_mode;
		break;
	default:
		dpu_pr_err("[DP] Invalid number of lanes %u", lanes);
		return;
	...
	if (mainlink_open) { /* set lanes to P0 mode */
		switch (lanes) {
		case 4:
			phyifctrl &= ~(DPTX_LANE2_PHY_STATUS_MASK | DPTX_LANE3_PHY_STATUS_MASK);
		case 2:
...
static void dptx_phy_set_ssc(struct dp_ctrl *dptx, bool bswitchphy)
{
	uint8_t byte = 0;
...
static void dptx_default_params_from_core(struct dp_ctrl *dptx)
{
	...
	if (dptx->connector->conn_info->base.fpga_flag == 1) {
		...
		dpu_pr_info("[DP] set max_rate HBR");
	} else {
		dptx->max_rate = (uint8_t)dptx->connector->conn_info->max_dptx_rate;
		dpu_pr_info("[DP] set max_rate %d", dptx->max_rate);
	}
...
static bool dptx_dp_enhanced_frame_cap(struct dp_ctrl *dptx)
{
	return dptx->is_extended_dpcd_caps_available ? drm_dp_enhanced_frame_cap(dptx->rx_extended_caps) :
		drm_dp_enhanced_frame_cap(dptx->rx_original_caps);
}
...
int dptx_set_enhanced_framing_mode(struct dp_ctrl *dptx)
{
	...
	/* enhance frame */
	if (dptx->efm_en && dptx_dp_enhanced_frame_cap(dptx)) {
...
static void dptx_link_core_reset(struct dp_ctrl *dptx)
{
	int retval;
	...
	if (dptx->dptx_phy_lane_reset) {
		dptx->dptx_phy_lane_reset(dptx, false);
		mdelay(2);
		dptx->dptx_phy_lane_reset(dptx, true);
	}
...
void dptx_link_set_preemp_vswing(struct dp_ctrl *dptx)
{
	...
	for (i = 0; i < dptx->link.lanes; i++) {
		...
		dpu_pr_info("[DP] lane: %d, rate: %d, vs: %d, pe: %d", i, dptx->link.rate, vs, pe);
...
static int dptx_link_set_lane_status(struct dp_ctrl *dptx)
{
	...
	if (retval != 0)
		dpu_pr_warn("[DP] set power P0 fail");
	...
	if (dptx->connector->conn_info->base.mode == EDP_MODE) {
		byte = (uint8_t)dptx->rx_rate_index;
		dpu_pr_info("[DP] set 115 dpcd %u\n", dptx->rx_rate_index);
		retval = dptx_write_dpcd(dptx, DP_LINK_RATE_SET, byte);
	}
...
void dptx_ctrl_layer_init(struct dp_ctrl *dptx)
{
	...
	dptx->dptx_psr_sdp_te_config = dptx_psr_sdp_te_config;
	...
	dptx->dptx_psr2_config_su_region = dptx_psr2_config_su_region;
	dptx->dptx_aux_set_backlight_config = dptx_aux_set_backlight_config;
