...
static uint8_t dptx_get_rx_line_buffer_depth(struct dp_ctrl *dptx)
{
	int retval;
...
static bool dptx_line_buffer_depth_limit(uint8_t line_buf_depth)
{
	if (line_buf_depth > DSS_DSC_MAX_LINE_BUF_DEP) {
		dpu_pr_warn("[DP] RX line_buffer_depth is over than TX capabilitie");
		return false;
	}
	return true;
}
...
int dptx_dsc_initial(struct dp_ctrl *dptx)
{
	...
	int retval;
	uint32_t cur_mode;
	bool rev_support_dsc;
	bool rev_support_fec;
	uint8_t linebuf_depth;
	bool is_line_buf_dep_fit;

	dpu_check_and_return(!dptx, -EINVAL, err, "[DP] dptx is NULL!");

	cur_mode = dptx->connector->conn_info->base.mode;
	rev_support_dsc =  (dptx->dpcd_rev >= DP_DPCD_REV_14) || ((dptx->dpcd_rev >= DP_DPCD_REV_13) && (cur_mode == EDP_MODE));
	...
	is_line_buf_dep_fit = dptx_line_buffer_depth_limit(linebuf_depth);
	if (!is_line_buf_dep_fit) {
		dpu_pr_warn("[DP] Have to disable DSC");
		dptx->dsc = false;
	}
	dpu_pr_info("[DP] DSC byte=0x%x, rev_support_dsc=%d, rx linebuf_depth=%u", byte, rev_support_dsc, linebuf_depth);
	if (dptx->dsc && ((byte & DP_DSC_DECOMPRESSION_IS_SUPPORTED) != 0) && rev_support_dsc) {
		dpu_pr_info("[DP] DSC enable!");
		dptx->dsc = true;
		dpu_pr_info("[DP] dptx_dsc_initial dp_mode: %d", dptx->connector->conn_info->base.mode);
	} else {
		dpu_pr_info("[DP] DSC disable!");
		dptx->dsc = false;
	}

	byte = 0;
	retval = dptx_read_dpcd(dptx, DP_FEC_CAPABILITY, &byte);
	if (retval != 0)
		return retval;

	rev_support_fec = (dptx->dpcd_rev >= DP_DPCD_REV_14) && (cur_mode == DP_MODE) && dptx->dsc;
	dpu_pr_info("[DP] FEC byte=0x%x, rev_support_fec=%d", byte, rev_support_fec);

	if ((dptx->dptx_link_params.compress_params.fec != 0) && ((byte & DP_FEC_CAPABLE) != 0) && rev_support_fec) {
		dpu_pr_info("[DP] FEC enable!");
		dptx->dptx_link_params.compress_params.fec = true;
	} else {
		dpu_pr_info("[DP] FEC disable!");
