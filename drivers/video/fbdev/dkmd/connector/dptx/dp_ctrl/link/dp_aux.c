...
static int dptx_aux_rw_bytes(struct dp_ctrl *dptx,
	bool rw,
	bool i2c,
	bool mot,
	uint32_t addr,
	...
	uint32_t len)
{
	...
	for (i = 0; i < len; ) {
		...
		if (!i2c) {
			if (dptx->aux_rw) {
				retval = dptx->aux_rw(dptx, rw, i2c, mot, false,
	...
	uint32_t len)
{
	...
	return dptx_aux_rw_bytes(dptx, true, true, false, device_addr, bytes, len);
}
	...
	uint32_t len)
{
	...
	return dptx_aux_rw_bytes(dptx, false, true, true, device_addr, bytes, len);
}
...
int dptx_read_dpcd(struct dp_ctrl *dptx, uint32_t addr, uint8_t *byte)
{
	...
	return dptx_aux_rw_bytes(dptx, true, false, true, addr, byte, 1);
}
...
int dptx_write_dpcd(struct dp_ctrl *dptx, uint32_t addr, uint8_t byte)
{
	...
	return dptx_aux_rw_bytes(dptx, false, false, true, addr, &byte, 1);
}
	...
	uint32_t len)
{
	...
	return dptx_aux_rw_bytes(dptx, true, false, true, reg_addr, bytes, len);
}
	...
	uint32_t len)
{
	...
	return dptx_aux_rw_bytes(dptx, false, false, true, reg_addr, bytes, len);
}
