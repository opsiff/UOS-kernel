...
#define DTS_GFX_DP3_NAME   "gfx_dp3"
#define DTS_GFX_DP4_NAME   "gfx_dp4"
#define DTS_GFX_DP5_NAME   "gfx_dp5"
...
#define DTS_COMP_DP3       "dkmd,"DTS_GFX_DP3_NAME
#define DTS_COMP_DP4       "dkmd,"DTS_GFX_DP4_NAME
#define DTS_COMP_DP5       "dkmd,"DTS_GFX_DP5_NAME
...
#define DP_BOARD_PRODUCT 0
#define DP_BOARD_UDP 1
#define DP_UNPLUGGABLE 0
#define DP_PLUGGABLE 1

#define MAX_DPTX_DEV_INDEX    6 // support six dptx max
#define EDP0_PORT_ID    4
...
struct dp_private {
	...
	uint32_t dp_relink_overtime;
	uint32_t dp_wait_before_read_edid;
	uint32_t hpd_gpio;
	uint32_t te_from_tcon_gpio;
	uint32_t hpd_irq_no;
	uint32_t te_irq_no;
	uint32_t dp_hpd_plug;
	uint32_t dp_hpd_jump_plugout;
	uint32_t dp_aux_phy_config;
	uint32_t show_phy_reg;
	uint32_t support_aux_backlight;
	uint32_t link_device_address_mode;
	uint32_t dp_jump_wait_dss;
};
