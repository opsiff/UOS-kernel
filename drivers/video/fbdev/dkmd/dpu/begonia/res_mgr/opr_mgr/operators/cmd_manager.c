...
int32_t generate_network_cmd(struct dkmd_base_frame *frame, struct dkmd_network *network)
{
	...
	if (network->is_first_block && unlikely(request_scene_client(frame) != 0))
		return -1;

	if (unlikely(request_dm_client(frame, network) != 0))
		return -1;

	if (unlikely(request_reg_client(frame, network) != 0))
		return -1;
