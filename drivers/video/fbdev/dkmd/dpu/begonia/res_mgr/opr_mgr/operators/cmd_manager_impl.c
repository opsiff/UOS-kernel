	...
	const union dkmd_opr_id *next_opr_ids, uint32_t next_opr_ids_num)
{
	...
	if (unlikely(request_opr_cfg_client(frame, cmd_data) != 0)) {
	...
	if (cmd_data->data->cfg_addr_info) {
		...
		cmd_data->data->cfg_addr_info = NULL;
	}
	...
	const struct dkmd_pipeline *pipeline, union dkmd_opr_id *pre_opr_id)
{
	...
	dpu_pr_debug("popr_num=%u", pipeline->opr_num);
	...
	for (i = 0; i < pipeline->opr_num - 1; ++i) {
		...
		if (unlikely(gen_opr_cmd(cmd_data, frame, pipeline->block_layer,
			*pre_opr_id, &pipeline->opr_ids[i + 1], 1) != 0)) {
	...
	const struct dkmd_pipeline *pre_pipeline, const struct dkmd_pipeline *post_pipelines, uint32_t post_pipelines_num)
{
	...
	if (unlikely(gen_pipeline_cmd(frame, network, pre_pipeline, &pre_opr_id) != 0)) {
		dpu_pr_err("generate pre pipeline cmd fail");
		return -1;
	}

	if (unlikely(set_head_oprs_info(post_head_opr_ids, post_pipelines, post_pipelines_num) != 0)) {
	...
	if (unlikely(gen_opr_cmd(cmd_data, frame, pre_pipeline->block_layer, pre_opr_id,
			post_head_opr_ids, post_pipelines_num) != 0)) {
	...
	union dkmd_opr_id pre_pipeline_last_opr_id, const struct dkmd_pipeline *post_pipeline)
{
	...
	if (unlikely(gen_pipeline_cmd(frame, network, post_pipeline, &pre_pipeline_last_opr_id) != 0)) {
	...
	if (unlikely(gen_opr_cmd(cmd_data, frame, post_pipeline->block_layer, pre_pipeline_last_opr_id, NULL, 0) != 0)) {
