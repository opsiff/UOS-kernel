...
static void dpu_dacc_access_timeout_config(char __iomem *dpu_base)
{
	/*
	* riscv access respone timeout
	* defult: 0x2 -> 31*1.5ns
	* set value: 0x3 -> 63*1.5ns
	*/
	uint32_t glb_cfg_ctrl_set_value = 0x3;
 	outp32(DPU_GLB_DBG_CFG_CTRL_ADDR(dpu_base + DPU_GLB0_OFFSET), glb_cfg_ctrl_set_value);
	dpu_pr_info("glb_cfg_ctrl_value = %#x", inp32(DPU_GLB_DBG_CFG_CTRL_ADDR(dpu_base + DPU_GLB0_OFFSET)));
}
...
void dpu_dacc_load(char __iomem *dpu_base)
{
	...
	configure_dss_service_security(DSS_CH_DEFAULT_SEC_CONFIG, 0, 0, BIG_DPU);
#endif
	dpu_dacc_access_timeout_config(dpu_base);
}
...
void dpu_dacc_config_scene(char __iomem *dpu_base, uint32_t scene_id, bool enable_cmdlist)
{
	...
	if (is_offline_scene(scene_id))
		set_reg(SOC_DACC_NEW_FRAME_RDY_CFG_ADDR(dmc_base, scene_id), 1, 1, 0);
	else
...
uint32_t dpu_dacc_handle_clear(char __iomem *dpu_base, uint32_t scene_id)
{
	...
	dpu_pr_info("dacc clear for scene_id=%d, CLR_START_REG=%#x!",
		scene_id, inp32(SOC_DACC_CLR_RISCV_START_ADDR(module_base, scene_id)));
	...
	if (check_addr_status_is_valid(SOC_DACC_CLR_TIMEOUT_STATUS_REG_ADDR(module_base, scene_id),
		0b01, 1000, DACC_CRG_CTL_CLEAR_TIMEOUT_THR)) {
		status = inp32(SOC_DACC_CLR_TIMEOUT_STATUS_REG_ADDR(module_base, scene_id));
		dpu_pr_info("status: 0x%x", status);
		if (status == 0b11) {
			dpu_pr_info("dacc clear timeout, need reset dpu! "
				...
				inp32(SOC_DACC_CLR_RISCV_START_ADDR(module_base, scene_id)));
			ret = BIT(scene_id);
		}
		dpu_pr_info("dpu_dacc_handle_clear: 0x%x", ret);
		outp32(SOC_DACC_CLR_RISCV_START_ADDR(module_base, scene_id), 0);
		return ret;
	}
	dpu_pr_info("Invalid timeout status: 0x%x", ret);
	outp32(SOC_DACC_CLR_RISCV_START_ADDR(module_base, scene_id), 0);
	ret = BIT(scene_id);
	return ret;
}
