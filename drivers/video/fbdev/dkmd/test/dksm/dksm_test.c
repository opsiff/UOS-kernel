...
static void test_timeline_with_release(void)
{
	...
	listener = dkmd_timeline_alloc_listener(&g_test_timeline_listener_ops,
		&g_test_timeline_data, dkmd_timeline_get_pt_value(timeline) + 1);
	...
	CHECK_EQUAL(dkmd_timeline_get_pt_value(timeline), 2);

	raw_notifier_call_chain(&nofitier_header, 0x8, &g_test_timeline_data);
	CHECK_EQUAL(dkmd_timeline_get_pt_value(timeline), 4);
...
static void test_timeline_without_release(void)
{
	...
	raw_notifier_call_chain(&nofitier_header, 0x8, &g_test_timeline_data);
	CHECK_EQUAL(dkmd_timeline_get_pt_value(timeline), 8);
...
static void test_timeline_dma_buf(void)
{
	...
	raw_notifier_call_chain(&nofitier_header, 0x8, &g_test_timeline_data);
	CHECK_EQUAL(dkmd_timeline_get_pt_value(timeline), 12);
}
...
int32_t dksm_testcase_suite_init(void)
{
	...
	dkmd_timeline_init(g_test_timeline, "test_timeline", &g_test_timeline_data);
	CHECK_EQUAL(dkmd_timeline_get_pt_value(g_test_timeline), 1);
	CHECK_EQUAL(g_test_timeline_data, 0);
