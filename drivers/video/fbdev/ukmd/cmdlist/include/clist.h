/**
 * @file clist.h
 * @brief Interface for cmdlist list function
 *
 * Copyright (c) Huawei Technologies Co., Ltd. 2020-2021. All rights reserved.
 *
 * This program is free software; you can redistribute it and/or modify
 * it under the terms of the GNU General Public License version 2 and
 * only version 2 as published by the Free Software Foundation.
 *
 * This program is distributed in the hope that it will be useful,
 * but WITHOUT ANY WARRANTY; without even the implied warranty of
 * MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
 * GNU General Public License for more details.
 *
 */

#ifndef __CLIST_H__
#define __CLIST_H__

#include <stdbool.h>
#include <stddef.h>

struct cnode {
	struct cnode *next, *prev;
};

typedef void (*cnode_func)(struct cnode *node);
typedef void (*cnode_funcx)(struct cnode *node, void *data);

/**
 * cnode_entry - get the struct for this entry
 * @node:	the &struct cnode pointer.
 * @type:	the type of the struct this is embedded in.
 * @member:	the name of the cnode within the struct.
 */
#define cnode_entry(node, type, member) \
	((type*)((char*)(node) - offsetof(type, member)))

static inline void clist_init(struct cnode *list)
{
	list->next = list;
	list->prev = list;
}

static inline bool clist_is_empty(const struct cnode *list)
{
	return list->next == list;
}

static inline bool clist_is_head(const struct cnode *list, const struct cnode *node)
{
	return list->next == node;
}

static inline bool clist_is_tail(const struct cnode *list, const struct cnode *node)
{
	return list->prev == node;
}

static inline void clist_insert(struct cnode *pos, struct cnode *node)
{
	node->prev = pos->prev;
	node->next = pos;
	node->prev->next = node;
	node->next->prev = node;
}

static inline void clist_add_tail(struct cnode *list, struct cnode *node)
{
	clist_insert(list, node);
}

static inline void clist_add_head(struct cnode *list, struct cnode *node)
{
	clist_insert(list->next, node);
}

static inline void clist_remove(struct cnode *node)
{
	node->prev->next = node->next;
	node->next->prev = node->prev;
}

static inline void clist_remove_tail(struct cnode *list)
{
	clist_remove(list->prev);
}

static inline void clist_remove_head(struct cnode *list)
{
	clist_remove(list->next);
}

static inline void clist_replace(struct cnode *old, struct cnode *node)
{
	node->next = old->next;
	node->next->prev = node;
	node->prev = old->prev;
	node->prev->next = node;
}

#define clist_for_each(node, list) \
	for (node = (list)->next; \
		 node != (list); \
		 node = (node)->next)

#define clist_for_each_saft(node, tmp, list) \
	for (node = (list)->next, tmp = (node)->next; \
		 node != (list); \
		 node = tmp, tmp = (node)->next)

/**
 * @brief release all cnode in clist
 *
 * @param list header list
 * @param node_release release function
 */
static inline void clist_deinit(struct cnode *list, cnode_funcx node_release, void *data)
{
	struct cnode *node = NULL, *tmp = NULL;

	if (unlikely(!node_release))
		return;

	clist_for_each_saft(node, tmp, list)
		node_release(node, data);
}

/**
 * @brief dump all cnode in clist
 *
 * @param list header list
 * @param node_release dump function
 */
static inline void clist_dump(struct cnode *list, cnode_func node_dump)
{
	struct cnode *node = NULL, *tmp = NULL;

	if (unlikely(!node_dump))
		return;

	clist_for_each_saft(node, tmp, list)
		node_dump(node);
}

/**
 * clist_first_entry - get the first element from a list
 * @ptr:	the list head to take the element from.
 * @type:	the type of the struct this is embedded in.
 * @member:	the name of the cnode within the struct.
 */
#define clist_first_entry(ptr, type, member) \
	(clist_is_empty(ptr) ? NULL : cnode_entry((ptr)->next, type, member))

/**
 * clist_last_entry - get the last element from a list
 * @ptr:	the list head to take the element from.
 * @type:	the type of the struct this is embedded in.
 * @member:	the name of the cnode within the struct.
 */
#define clist_last_entry(ptr, type, member) \
	(clist_is_empty(ptr) ? NULL : cnode_entry((ptr)->prev, type, member))

/**
 * clist_next_entry - get the next element of @entry from a list
 * @ptr:	the list head to take the element from.
 * @type:	the type of the struct this is embedded in.
 * @member:	the name of the cnode within the struct.
 */
#define clist_next_entry(entry, ptr, type, member) \
	(clist_is_tail(ptr, &(entry)->member) ? \
		NULL : \
		cnode_entry((entry)->member.next, type, member))

/**
 * clist_prev_entry - get the prev element of @entry from a list
 * @ptr:	the list head to take the element from.
 * @type:	the type of the struct this is embedded in.
 * @member:	the name of the cnode within the struct.
 */
#define clist_prev_entry(entry, ptr, type, member) \
	(clist_is_head(ptr, &(entry)->member) ? \
		NULL : \
		cnode_entry((entry)->member.prev, type, member))

/**
 * clist_for_each_entry_safe - iterate over list of given type safe against removal of list entry
 * @pos:	the type * to use as a loop cursor.
 * @tmp:	another type * to use as temporary storage
 * @list:	the list head to take the element from.
 * @type:	the type for your list.
 * @member:	the name of the cnode within the struct.
 */
#define clist_for_each_entry_safe(pos, tmp, list, type, member)	\
	for (pos = cnode_entry((list)->next, type, member), \
		 tmp = cnode_entry((pos)->member.next, type, member); \
		 &(pos)->member != (list); \
		 pos = tmp, tmp = cnode_entry((pos)->member.next, type, member))

/**
 * clist_for_each_entry_safe_reverse - iterate backwards over list safe against removal
 * @pos:	the type * to use as a loop cursor.
 * @tmp:	another type * to use as temporary storage
 * @list:	the list head to take the element from.
 * @type:	the type for your list.
 * @member:	the name of the cnode within the struct.
 *
 * Iterate backwards over list of given type, safe against removal
 * of list entry.
 */
#define clist_for_each_entry_safe_reverse(pos, tmp, list, type, member)	\
	for (pos = cnode_entry((list)->prev, type, member), \
		tmp = cnode_entry((pos)->member.prev, type, member); \
		&(pos)->member != (list); \
		pos = tmp, tmp = cnode_entry((pos)->member.prev, type, member))

#endif