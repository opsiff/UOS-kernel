...
static int cpa_prepare_session(struct ion_cpa_heap *cpa_heap)
{
	int ret = 0;

	if (!cpa_heap->ta_init) {
		ret = sec_tee_init(&cpa_heap->tee);
		if (ret) {
			    ...
			    unsigned long size, unsigned long flags)
{
	...
	ret = __cpa_heap_input_check(cpa_heap, size, flags);
	if (ret) {
...
struct ion_heap *ion_cpa_heap_create(struct ion_platform_heap *data)
{
	...
	ret = sec_tee_alloc(&cpa_heap->tee);
	if (ret) {
