...
static void set_platform_heaps_value(struct ion_platform_heap *p_data, int index)
{
...
static ssize_t sys_pool_watermark_show(struct device *dev,
				       struct device_attribute *attr,
				       char *buf)
{
	return sprintf_s(buf, "%lu\n", get_sys_pool_watermark());
}

static DEVICE_ATTR_RW(sys_pool_watermark);
