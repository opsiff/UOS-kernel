...
#if defined(CONFIG_MM_AMA)
#define SMEM_MINFREE_MIN  (600 * 256)
#define SMEM_MINFREE_LOW  (1000 * 256)
#define SMEM_MINFREE_HIGH (1600 * 256)
#else
...
#if defined(CONFIG_MM_AMA) && defined(CONFIG_DFX_STATS)
static ssize_t smem_minfree_watermark_store(struct device *dev,
				struct device_attribute *attr,
				const char *buf, size_t size)
{
	unsigned long val;
	...
	i = 0;
	smem_minfree[i++] = val;
	smem_minfree[i++] = val / 3 * 5; /* minfree low is 5/3 min */
	smem_minfree[i] = val / 3 * 8; /* minfree high is 8/3 min */
	pr_err("smem minfree store, min[%lu] low[%lu] high[%lu]\n",
		smem_minfree[0], smem_minfree[1], smem_minfree[2]);

	return size;
}

static ssize_t smem_minfree_watermark_show(struct device *dev,
				struct device_attribute *attr, char *buf)
{
	int i;

	/* vltmm shrink max count is 8, loop(32) is pool size 256M  / 8 */
	for (i = 0; i < 32; i++)
		smem_send_reclaim_msg();
...
static DEVICE_ATTR_RW(smem_minfree_watermark);
static struct attribute *smem_minfree_attrs[] = {
	&dev_attr_smem_minfree_watermark.attr,
	NULL,
};
static const struct attribute_group smem_minfree_attr_group = {
	.name = NULL,
	.attrs = smem_minfree_attrs,
};
#endif
...
#if defined(CONFIG_MM_AMA) && defined(CONFIG_DFX_STATS)
	ret = sysfs_create_group(&mm_ion_dev->dev.kobj, &smem_minfree_attr_group);
	if (ret)
		pr_err("create smem_minfree attr fail!\n");
#endif
}
