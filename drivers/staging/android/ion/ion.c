...
static int ion_buffer_attach_lb(struct ion_buffer *buffer, unsigned long flags)
{
	...
	osh = !!(flags & ION_FLAG_MM_LB_OSH_MASK);
	/*
	 * OSH flag is attached to page at the time of ion_alloc.
	 * GID0 may also carry OSH flag for NPU's svm.
	 */
	if (osh)
		lb_sg_set_osh(buffer->sg_table->sgl, buffer->sg_table->nents, osh);
#endif

	plc_id = ION_FLAG_2_PLC_ID(flags);
	if (plc_id && !(flags & ION_FLAG_SECURE_BUFFER)) {
		pr_info("MM ION LB %u\n", plc_id);
	...
	unsigned int pid, unsigned long offset, size_t size)
{
	...
	for_each_sg(table->sgl, sg, table->nents, i) {
		...
		/* stub: currently do not use OSH prot for npu attach_lb buffer */
		if (offset < len) {
			if (lb_pages_attach(pid, sg_page(sg), len >> PAGE_SHIFT))
				goto err;
