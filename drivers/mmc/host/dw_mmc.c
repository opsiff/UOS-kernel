				    ...
				    unsigned int sg_len)
{
	...
	if(SDMMC_32_BIT_DMA == host->dma_64bit_address) {
		...
		for (i = 0; i < sg_len; i++, desc++) {
			...
			/* Prevent memory from not initializing */
...
{
#ifdef CONFIG_SD_SDIO_CRC_RETUNING
	if ((host->retuning_flag == SD_RETUNING_ON) &&
			(MMC_TIMING_UHS_SDR104 == timing) &&
			(0 == host->need_clk_change)) {
...
{
	if (sd_test_reset_flag && mrq && mrq->cmd &&
			(host->hw_mmc_id == DWMMC_SD_ID))
...
static int dw_mci_init_slot(struct dw_mci *host, unsigned int id)
{
	...
	if (host->sd_vmmcmosen_switch == 1) {
		...
		if (IS_ERR(host->vmmcmosen)) {
			pr_warn("%s: no vmmcmosen regulator found\n", mmc_hostname(mmc));
			host->vmmcmosen = NULL;
		}
	} else {
		host->vmmcmosen = NULL;
...
static struct dw_mci_board *dw_mci_parse_dt(struct dw_mci *host)
{
	...
	if (of_find_property(np, "caps2-wifi-support-cmd11", NULL)
		&& of_property_count_u8_elems(np, "caps3-wifi-support-cmd11") <= 0) {
		dev_info(dev, "dw_mci:%d %s find wifi support cmd11 in dts\n",
									host->hw_mmc_id,  __func__);
		pdata->caps3 |= MMC_CAP3_SUPPORT_WIFI_CMD11;
	}

	if (of_find_property(np, "caps2-wifi-no-lowpwr", NULL)
		&& of_property_count_u8_elems(np, "caps-wifi-no-lowpwr") <= 0) {
		dev_info(dev, "dw_mci:%d %s find wifi support no_lowpwr in dts\n",
									host->hw_mmc_id,  __func__);
		pdata->caps3 |= MMC_CAP3_WIFI_NO_LOWPWR;
	}

	if (of_find_property(np, "caps2-support-wifi", NULL)) {
		pr_info("dw_mci:%d %s find caps3-support-wifi in dts\n", host->hw_mmc_id,  __func__);
		pdata->caps3 |= MMC_CAP3_SUPPORT_WIFI;
	}
...
int dw_mci_probe(struct dw_mci *host)
{
	...
	if (!host->pdata->select_slot && host->pdata->num_slots < 1) {
		dev_err(host->dev, "Platform data must supply select_slot function\n");
		return -ENODEV;
	}
...
err:
	if (host->cur_slot && (mmc_can_gpio_cd(host->cur_slot->mmc) ||
		!mmc_card_is_removable(host->cur_slot->mmc)))
		clk_disable_unprepare(host->biu_clk);
