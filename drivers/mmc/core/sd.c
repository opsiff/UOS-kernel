	...
	bool reinit)
{
	...
	if (!reinit) {
		...
		if (err) {
			pr_info(KERN_ERR "%s:send acmd51 to get scr fail when init sd first time,err=%d,check data0!!!!\n",mmc_hostname(host),err);
			return err;
		}
		...
		if (err) {
			pr_info(KERN_ERR "%s:send cmd13 to get ssr fail when init sd first time,err=%d\n",mmc_hostname(host),err);
			return err;
		}
	...
	if (status & R1_CARD_IS_LOCKED) {
		if (card->auto_unlock) {
			if (card->unlock_pwd[0] > 0 ) {
				/* unlock sd card */
				err = mmc_lock_unlock_by_buf(card, card->unlock_pwd+1, (int)card->unlock_pwd[0],
				...
				err = mmc_send_status(card, &status);
				if (err) {
					printk("[SDLOCK] %s resume sd card exception\n", __func__);
					goto free_card;
				}
			} else {
				printk("[SDLOCK] %s unlock pwd is null\n", __func__);
			}
		}
