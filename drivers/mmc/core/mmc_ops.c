	...
	if (err) {
		...
		if (sdcard_dclient && !dsm_client_ocuppy(sdcard_dclient)) {
			dsm_client_record(sdcard_dclient,
				"cmd[MMC_SET_RELATIVE_ADDR] fail, err = %d, resp[0] = %d", err, cmd.resp[0]);
...
int sd_wait_lock_unlock_cmd(struct mmc_card *card, unsigned int mode)
{
	int err;
	struct mmc_command cmd = {0};
...
int mmc_lock_unlock_by_buf(struct mmc_card *card, u8 *key_buf, int key_len, unsigned int mode)
{
	int err = -EINVAL;
	int data_size;
	int max_buf_size ;
...
sdlock_retry:
	if (retry > 1) {
		...
		if (retry++ > 0) {
			...
			mmc_lock_sd_init_card(card, true);
		}
	}
