	...
	if (rwlog_enable_flag == 1) {
		if (mrq->cmd->opcode == MMC_WRITE_MULTIPLE_BLOCK
			...
			if (rwlog_index == host->index) {
				pr_info("%s:cmd=%d, mrq->data.blocks=%d, index=%d, arg=%x\n", __func__,
...
int mmc_set_uhs_voltage(struct mmc_host *host, u32 ocr)
{
	...
	cmd.flags = MMC_RSP_PRESENT | MMC_RSP_OPCODE | MMC_CMD_AC | MMC_RSP_BUSY;
#else
	...
	mmc_delay(1);
	/*
	 * zodiac wifi chip(1102) sets a 1.7ms timer when getting a response of
...
void mmc_power_off(struct mmc_host *host)
{
	if (host->ios.power_mode == MMC_POWER_OFF)
		return;
	/* zodiac do not use power sequence */
...
static int mmc_rescan_detect_sd_or_mmc(struct mmc_host *host, unsigned freq)
{
	...
	unsigned char card_mode = host->ios.power_mode;
#endif
	...
	if (init_success) {
		if (host->init_cnt != 0) {
			if (sdcard_dclient && !dsm_client_ocuppy(sdcard_dclient)) {
				dsm_client_record(sdcard_dclient, "%s:send init retry cnt=%u",
					mmc_hostname(host), host->init_cnt);
				dsm_client_notify(sdcard_dclient, DSM_SDCARD_INIT_FAIL_RETRY);
			}
...
void mmc_rescan(struct work_struct *work)
{
	...
	if (host->ops->get_cd && host->ops->get_cd(host) == 0) {
		if (MMC_CAP3_SUPPORT_WIFI & (host->caps3))
