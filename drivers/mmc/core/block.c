...
#include <platform_include/basicplatform/linux/mmc/ffu.h>
...
#include <platform_include/basicplatform/linux/mmc/mmc_zodiac_card.h>
#endif
...
#include <platform_include/basicplatform/linux/mmc/mmc_health_diag.h>
#endif
...
static int __mmc_blk_ioctl_cmd(struct mmc_card *card, struct mmc_blk_data *md,
			       struct mmc_blk_ioc_data *idata, struct request *req)
#else
static int __mmc_blk_ioctl_cmd(struct mmc_card *card, struct mmc_blk_data *md,
			       struct mmc_blk_ioc_data *idata)
#endif
{
	...
	if (idata->rpmb || (cmd.flags & MMC_RSP_R1B) == MMC_RSP_R1B) {
		 ...
		 */
#ifdef CONFIG_HW_SD_HEALTH_DETECT
		err = card_busy_detect(card, MMC_BLK_TIMEOUT_MS, req, NULL);
#else
...
static int mmc_blk_check_blkdev(struct block_device *bdev)
{
	...
	if (bdev_is_partition(bdev))
		return -EPERM;
#else
	if ((!capable(CAP_SYS_RAWIO) && !capable(CAP_SYS_ADMIN)) || bdev_is_partition(bdev))
		return -EPERM;
#endif
	...
	do {
		...
		if (mmc_card_sd(card) || (mmc_card_removable_mmc(card))) {
			...
			if (time_after(jiffies, sd_timeout)) {
				dev_err(mmc_dev(card->host),
						"SD card stuck in wrong state! %s status: %#x\n",
						__func__, status);
				...
				/* We cant use sd card in current mode,try to slow down clock */
...
#ifdef CONFIG_HW_SD_HEALTH_DETECT
			ret = __mmc_blk_ioctl_cmd(card, md, idata[i], req);
#else
				  ...
				  struct request *req)
{
	...
	if (mmc_card_sd(md->queue.card)) {
		...
		time1 = sched_clock();
		rq_byte = mmc_calculate_ioworkload_and_rwspeed(time1, req, md->disk);
	}
#endif
...
#ifdef CONFIG_ZODIAC_MMC
void mmc_blk_zodiac_stub_emmc_for_ufs(struct mmc_card *card)
{
	struct mmc_host *host = card->host;
	...
	if (!strncmp(dev_name(mmc_dev(host)), "hi_mci.1", strlen("hi_mci.1"))) {
		 ...
		 *init
		 */
		if (find_first_zero_bit(name_use, max_devices == 0)) {
...
static void mmc_blk_sd_dump_status(
...
extern int zodiac_mmc_manual_bkops_config(struct request_queue *q);
extern bool zodiac_mmc_is_bkops_needed(struct mmc_card *card);
#endif
#define ZODIAC_MMC_IO_LATENCY_MS 2000
#define ZODIAC_SD_IO_LATENCY_MS 2000
static void mmc_blk_zodiac_cfg_queue_feature(struct mmc_card *card, struct mmc_blk_data *md, int area_type)
{
	if (area_type == MMC_BLK_DATA_AREA_MAIN) {
		if (mmc_card_mmc(card) ) {
			blk_queue_latency_warning_set(md->queue.queue, ZODIAC_MMC_IO_LATENCY_MS);
#ifdef CONFIG_MMC_CQ_HCI
			...
			if (card->ext_csd.man_bkops_en && zodiac_mmc_is_bkops_needed(card))
				zodiac_mmc_manual_bkops_config(md->queue.queue);
#endif

		} else if (mmc_card_sd(card)) {
			blk_queue_latency_warning_set(md->queue.queue, ZODIAC_SD_IO_LATENCY_MS);
					      ...
					      int area_type)
{
	...
	ret = mmc_init_queue(&md->queue, card, area_type);
	if (ret)
		goto err_putdisk;
...
#ifdef CONFIG_MAS_BLK
	mmc_blk_zodiac_cfg_queue_feature(card, md, area_type);
#endif
...
static int mmc_blk_direct_flush(struct request_queue *q)
{
	...
	printk(KERN_EMERG "<%s>\r\n", __func__);
	ret = mmc_get_card_zodiac(card, false);
...
#ifdef CONFIG_ZODIAC_MMC_FLUSH_REDUCE
#ifndef CONFIG_ZODIAC_MMC_FLUSH_REDUCE_WHITE_LIST
