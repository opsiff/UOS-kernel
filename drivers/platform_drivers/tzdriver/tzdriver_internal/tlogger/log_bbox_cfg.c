 ...
 * Description: for bbox log cfg api define
 *
...
static const uint64_t g_current_core_id = BBOX_TEEOS;
...
static void tee_log_get_systime(excep_time *time)
{
	...
	do_gettimeofday(&tv);
	time->tv_sec = (uint64_t)tv.tv_sec;
	time->tv_usec = (uint64_t)tv.tv_usec;
}

static uint32_t get_log_mem_len(void)
{
...
static void init_bbox_except_ctrl(struct bbox_module_exception_ctrl *ctrl, uint32_t excepid)
{
...
static void init_bbox_header(struct bbox_module_ctrl *ctrl_header, uint32_t excepid)
{
...
static void report_log_exception(uint32_t excepid)
{
...
{
	report_log_exception((uint32_t)TEEOS_MODID);
}
...
{
	report_log_exception((uint32_t)TEEOS_TA_MODID);
}
...
void unregister_log_exception(void)
{
	...
	ret = bbox_unregister_exception((uint32_t)TEEOS_MODID);
	if (ret != 0)
...
static int init_bbox_excep_info(struct bbox_exception_info *info, bool is_ta_crash)
{
	...
	if (is_ta_crash) {
		...
		info->e_excepid = (uint32_t)TEEOS_TA_MODID;
		info->e_excepid_end = (uint32_t)TEEOS_TA_MODID_END;
		info->e_process_priority = BBOX_MINOR;
		info->e_reboot_priority = BBOX_REBOOT_NO;
	} else {
		tee_module_desc = "Teeos system crash";

		info->e_excepid = (uint32_t)TEEOS_MODID;
		info->e_excepid_end = (uint32_t)TEEOS_MODID_END;
	...
	info->e_reentrant = (unsigned char)BBOX_REENTRANT_ALLOW;
