...
static void kdatan_bus_dump_gpc_lsc_data(struct bus_dump_resource_info *info, int core_id)
{
	...
	kdatan_bus_dump_state_type(info, core_id, DEST_ID_GPC_LSC, STATE_TYEP_1, DEST_ID_GPC_LSC_STATE_1_SIZE);
...
static void kdatan_bus_dump_bvh_debug_data(struct bus_dump_resource_info *info, int core_id)
{
	...
	kdatan_bus_dump_state_type(info, core_id, DEST_ID_BVH_BCACHE, STATE_TYEP_1, DEST_ID_BVH_BCACHE_STATE_1_SIZE);
...
static void datan_bus_dump_fcm_debug_data(struct bus_dump_resource_info *info, int core_id)
{
	...
	kdatan_bus_dump_state_type(info, core_id, DEST_ID_FCM_CQ_VCQ, STATE_TYEP_0, DEST_ID_FCM_CQ_VCQ_STATE_0_SIZE);
...
static void datan_bus_dump_l2mmu_debug_data(struct bus_dump_resource_info *info, int core_id)
{
	...
	kdatan_bus_dump_state_type(info, core_id, DEST_ID_L2MMU_UTLB3, STATE_TYEP_0, DEST_ID_L2MMU_UTLB3_STATE_0_SIZE);
	kdatan_bus_dump_state_type(info, core_id, DEST_ID_L2MMU_UTLB3, STATE_TYEP_1, DEST_ID_L2MMU_UTLB3_STATE_1_SIZE);
	kdatan_bus_dump_state_type(info, core_id, DEST_ID_L2MMU_UTLB4, STATE_TYEP_0, DEST_ID_L2MMU_UTLB4_STATE_0_SIZE);
	kdatan_bus_dump_state_type(info, core_id, DEST_ID_L2MMU_UTLB4, STATE_TYEP_1, DEST_ID_L2MMU_UTLB4_STATE_1_SIZE);
}
...
static void datan_bus_dump_cq_control_register(struct bus_dump_resource_info *info)
{
	...
	u32 reg_offset = cq_control_reg(gdev, CQ_BC_SEND_MODE);
	char data[DUMP_DATA_MAX_SIZE] = {0};
...
static void datan_bus_dump_mmu_register_v2xx(struct bus_dump_resource_info *info)
{
	...
	struct hvgr_device *gdev = info->gdev;
	u32 end_addr = mmu_control_reg(gdev, MMU_CMD_DONE);
	u32 start_addr = gdev->dm_dev.reg_base.mmu_stage1_reg_base;

	for (; start_addr <= end_addr; start_addr += (u32)sizeof(u32))
		kdatan_debug_bus_record_reg_data(info, start_addr);
	end_addr = AS_FAULTADDRESS_HI;
	...
	for (; addr_space_num < MAX_ADDR_SPACE_NUM; addr_space_num++) {
		...
		for (; start_addr <= end_addr; start_addr += FCP_REG_SIZE) {
			...
			dump_addr = mmu_control_reg(gdev, mmu_as_reg(addr_space_num, start_addr));
...
static void datan_bus_dump_fcp_cq_regs(struct bus_dump_resource_info *info)
{
	...
	u32 addr = fcp_mb_reg(gdev, FCP_MB_MCUVECTOR_ADDR_CQ);

	for (; addr <= fcp_mb_reg(gdev, FCP_MB_MCU_ERROR_CQ); addr += FCP_REG_SIZE)
		kdatan_debug_bus_record_reg_data(info, addr);

	addr = fcp_mb_reg(gdev, FCP_MB_MCU_VAADDR_CQ);

	for (; addr <= fcp_mb_reg(gdev, FCP_MB_MCU_REG_STATUS_CQ); addr += FCP_REG_SIZE)
		kdatan_debug_bus_record_reg_data(info, addr);

	addr = fcp_mb_reg(gdev, FCP_MB_CQ_MCU_IRQ);

	for (; addr <= fcp_mb_reg(gdev, FCP_MB_CQ_MCU_DONE); addr += FCP_REG_SIZE)
		kdatan_debug_bus_record_reg_data(info, addr);

	kdatan_debug_bus_record_reg_data(info, fcp_mb_reg(gdev, FCP_MB_EDMA2MCU_CQ_MASKINT));
}
...
static void datan_bus_dump_fcp_cf_regs(struct bus_dump_resource_info *info)
{
	...
	u32 addr = fcp_mb_reg(gdev, FCP_MB_MCUVECTOR_ADDR);

	for (; addr <= fcp_mb_reg(gdev, FCP_MB_MCU_ERROR); addr += FCP_REG_SIZE)
		kdatan_debug_bus_record_reg_data(info, addr);

	addr = fcp_mb_reg(gdev, FCP_MB_MCU_VAADDR);

	for (; addr <= fcp_mb_reg(gdev, FCP_MB_MCU_REG_STATUS); addr += FCP_REG_SIZE)
		kdatan_debug_bus_record_reg_data(info, addr);

	addr = fcp_mb_reg(gdev, FCP_MB_CF_MCU_IRQ);

	for (; addr <= fcp_mb_reg(gdev, FCP_MB_CF_MCU_DONE); addr += FCP_REG_SIZE)
		kdatan_debug_bus_record_reg_data(info, addr);

	kdatan_debug_bus_record_reg_data(info, fcp_mb_reg(gdev, FCP_MB_EDMA2MCU_CF_MASKINT));
}
...
static void datan_bus_dump_fcp_ddr_map_cq_regs(struct bus_dump_resource_info *info)
{
	...
	for (; req_num < MAX_REQ_NUM; req_num++) {
		addr = fcp_mb_mcu_ddr_map_cq_reg(gdev, req_num);
		kdatan_debug_bus_record_reg_data(info, addr);
	}

	for (; window_num < MAX_WINDOW_NUM; window_num++) {
		addr = fcp_mb_mcu_ddr_map_cq_wind(gdev, window_num);
		kdatan_debug_bus_record_reg_data(info, addr);
	}
}
...
static void datan_bus_dump_fcp_ddr_map_regs(struct bus_dump_resource_info *info)
{
	...
	for (; req_num < MAX_REQ_NUM; req_num++) {
		addr = fcp_mb_mcu_ddr_map_reg(gdev, req_num);
		kdatan_debug_bus_record_reg_data(info, addr);
	}

	for (; window_num < MAX_WINDOW_NUM; window_num++) {
		addr = fcp_mb_mcu_ddr_map_wind(gdev, window_num);
		kdatan_debug_bus_record_reg_data(info, addr);
	}
}
...
{
	struct hvgr_device *gdev = info->gdev;
	u32 addr = fcp_mb_reg(gdev, FCP_MB_EDMA2JM_MASKINT);
...
static void datan_bus_dump_fcp_cmd_msg_regs(struct bus_dump_resource_info *info)
{
	...
	for (; mb_cmd_msg_num < MAX_MB_CMD_MSG_NUM; mb_cmd_msg_num++) {
		addr = fcp_mb_reg(gdev, FCP_MB_HOST2FCP_MSG_0) + mb_cmd_msg_num * FCP_REG_SIZE;
		kdatan_debug_bus_record_reg_data(info, addr);
	}
}
...
static void datan_bus_dump_fcp_status_msg_regs(struct bus_dump_resource_info *info)
{
	...
	for (; mb_status_msg_num < MAX_MB_STATUE_MSG_NUM; mb_status_msg_num++) {
		addr = fcp_mb_reg(gdev, FCP_MB_FCP2HOST_MSG_0) + mb_status_msg_num * FCP_REG_SIZE;
		kdatan_debug_bus_record_reg_data(info, addr);
	}
}
...
{
	struct hvgr_device *gdev = info->gdev;
	u32 addr = fcp_mb_reg(gdev, FCP_MB_HOST2FCP_MSG_VLD);

	for (; addr <= fcp_mb_reg(gdev, FCP_MB_HOST2FCP_MSG_VLD_CLR); addr += FCP_REG_SIZE)
		kdatan_debug_bus_record_reg_data(info, addr);

	addr = fcp_mb_reg(gdev, FCP_MB_FCP2HOST_MSG_VLD);
...
static void datan_bus_dump_fcp_timer_regs(struct bus_dump_resource_info *info)
{
	...
	for (timer_idx = 0; timer_idx < timer_number; timer_idx++) {
		...
		for (; addr <= end_addr; addr += FCP_REG_SIZE) {
			dump_addr = fcp_timer_load_reg(gdev, timer_idx, addr);
			kdatan_debug_bus_record_reg_data(info, dump_addr);
		}
	}
}
...
{
	int ret;
	/* For V300, Not For V100 or V120 or V200 */
	ret = snprintf_s(data, size, size - 1, VERSION_300);
	if (ret < 0)
		hvgr_err(gdev, HVGR_DATAN, "debug bus [%s] snprintf_s failed.", __func__);
}
