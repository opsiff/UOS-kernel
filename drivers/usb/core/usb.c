...
#ifdef CONFIG_USB_HIUSB
		dev_set_name(&dev->dev, "port-%d-usb%d", hiusb_find_portid_by_bus(bus), bus->busnum);
#else
...
#ifdef CONFIG_USB_HIUSB
		dev_set_name(&dev->dev, "port-%d-%d-%s", hiusb_find_portid_by_bus(bus), bus->busnum, dev->devpath);
#else
