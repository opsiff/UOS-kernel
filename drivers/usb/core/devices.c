...
/* T:  Bus=dd Lev=dd Prnt=dd Port=dd Cnt=dd Dev#=ddd Spd=dddd MxCh=dd PortId=d */
"\nT:  Bus=%2.2d Lev=%2.2d Prnt=%2.2d Port=%2.2d Cnt=%2.2d Dev#=%3d Spd=%-4s MxCh=%2d PortId=%d\n";
#else
static const char format_topo[] =
			       ...
			       int level, int index, int count)
{
	...
	data_end = pages_start + sprintf(pages_start, format_topo,
			...
			speed, usbdev->maxchild, hiusb_find_portid_by_udev(usbdev));
#else
	data_end = pages_start + sprintf(pages_start, format_topo,
			bus->busnum, level, parent_devnum,
			index, count, usbdev->devnum,
