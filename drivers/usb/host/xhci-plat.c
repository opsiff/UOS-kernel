...
static int xhci_plat_probe(struct platform_device *pdev)
{
	...
	for (tmpdev = &pdev->dev; tmpdev; tmpdev = tmpdev->parent) {
		...
		if (device_property_read_bool(tmpdev, "host_abort_cmd_quirk"))
			xhci->quirks |= XHCI_ABORT_CMD_QUIRK;

		if (device_property_read_bool(tmpdev, "host_ca_timeout_quirk"))
			xhci->quirks |= XHCI_CACMD_TIMEOUT_QUIRK;
