...
#if (defined(CONFIG_HISI_USB_STORAGE_QUIRK) || defined(CONFIG_CHIP_USB_CONFIGFS)) && !defined(CONFIG_USB_HW_F_ACM_MUSL)
...
#if defined(CONFIG_CHIP_USB_CONFIGFS) && !defined(CONFIG_USB_HW_F_ACM_MUSL)
static ssize_t usb_limit_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	return scnprintf(buf, PAGE_SIZE, "%d", dwc3_gadget_get_reset_cnt());
}

static ssize_t usb_limit_store(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t size)
{
	long val = 0;

	if (kstrtol(buf, 10, &val) < 0) {
		hwlog_err("usb_limit control val invalid\n");
		return -EINVAL;
	}

	if (val)
		usb_gadget_udc_limit_speed(true);
	else
		usb_gadget_udc_limit_speed(false);
...
#if defined(CONFIG_CHIP_USB_CONFIGFS) && !defined(CONFIG_USB_HW_F_ACM_MUSL)
static DEVICE_ATTR(usb_limit, 0664, usb_limit_show, usb_limit_store);
#endif
...
#endif
	hw_usb_parse_dts(di);
#if defined(CONFIG_HISI_USB_STORAGE_QUIRK) && !defined(CONFIG_USB_HW_F_ACM_MUSL)
	usb_stor_set_quirks_flag(1);
#endif
