...
#define HEX_LENGTH     17
static const unsigned char data_bin2ascii[HEX_LENGTH] = "0123456789ABCDEF";
...
/* param time: true: only time, false: date and time */
static int dp_timeval_to_str(struct timespec64 time, const char *comment,
	char *str, int size, bool only_time)
{
	...
	if (ret < 0)
		hwlog_err("%s: %s failed %d\n", __func__, comment, ret);
	else
		hwlog_debug("%s: %s is %s %d\n", __func__, comment, str, ret);

	return 0;
}

#ifdef DP_DEBUG_ENABLE
int dp_timeval_to_time_str(struct timespec64 time, const char *comment,
	char *str, int size)
{
...
int dp_timeval_to_datetime_str(struct timespec64 time, const char *comment,
	char *str, int size)
{
...
void dp_set_hotplug_state(enum dp_link_state state)
{
	...
	hwlog_info("%s: last_state=0x%x, hotplug_state old=0x%x, new=0x%x\n",
		__func__, priv->last_hotplug_state, old_state, priv->hotplug_state);
}
EXPORT_SYMBOL_GPL(dp_set_hotplug_state);

void dp_add_state_to_uevent_list(enum dp_link_state state,
	bool dptx_vr_flag)
{
	...
	if (node) {
		...
		list_add_tail(&(node->list), &(g_imonitor_report.uevent_list_head));
...
static void dp_report_hotplug_fail_state(struct dp_dsm_priv *priv)
{
	...
	if (dp_factory_mode_is_enable()) {
		if (dp_check_link_state(priv, DP_LINK_STATE_EDID_FAILED)) {
			...
			dp_report_link_state(priv, DP_LINK_STATE_INVALID_COMBINATIONS);
			mmie_flag = true;
		} else if (dp_check_link_state(priv,
			DP_LINK_STATE_LINK_REDUCE_RATE)) {
			dp_report_link_state(priv, DP_LINK_STATE_LINK_REDUCE_RATE);
			mmie_flag = true;
		}
	}
...
static void dp_imonitor_report_info_one(struct dp_dsm_priv *priv)
{
	...
	if (count < DP_DSM_REPORT_NUM_IN_ONE_DAY) {
		hwlog_info("%s: imonitor report %d %d/%d\n", __func__,
			priv->type, count + 1, DP_DSM_REPORT_NUM_IN_ONE_DAY);
		...
		if (priv->type == DP_IMONITOR_TIME_INFO) {
			...
			report->last_same_mode_time.tv_sec += priv->same_mode_time.tv_sec;
			report->last_diff_mode_time.tv_sec += priv->diff_mode_time.tv_sec;
			report->last_source_switch_num += priv->source_switch_num;
		}
	}
...
static void dp_add_hotplug_info_to_list(struct dp_dsm_priv *priv,
	bool stop)
{
	struct dp_hotplug_info *hotplug = NULL;
	...
	struct timespec64 sec;

	if (!priv->cur_hotplug_info || !priv->need_add_hotplug_to_list)
		return;
	...
	if (!stop) {
		...
		ktime_get_real_ts64(&(hotplug->time[stop_index]));
	}
	...
	if (stop) {
		...
		sec.tv_nsec = hotplug->time[link_succ_index].tv_nsec -
			hotplug->time[start_index].tv_nsec;
		sec.tv_nsec = (sec.tv_sec * DP_DSM_TIME_MS_OF_SEC) +
			(sec.tv_nsec / DP_DSM_TIME_US_OF_MS);

		dp_dsm_get_link_min_time(priv->link_min_time, sec);
		dp_dsm_get_link_max_time(priv->link_max_time, sec);
	}

	if (hotplug->read_edid_retval)
		return;
...
#endif /* DP_DEBUG_ENABLE */
		dp_dsm_add_to_imonitor_list(priv, rcf_extend_info,
...
static void dp_set_typec_in_out_time(struct dp_dsm_priv *priv, bool in)
{
	...
	if (in) {
		ktime_get_real_ts64(&(priv->typec_in_time));
		g_typec_in_out.in_time[index] = priv->typec_in_time;
	} else {
		ktime_get_real_ts64(&(priv->typec_out_time));
		g_typec_in_out.out_time[index] = priv->typec_out_time;
...
static void dp_typec_cable_in(struct dp_dsm_priv *priv, uint8_t mode_type)
{
	...
	priv->typec_cable_type = mode_type;
...
static void dp_typec_cable_out(struct dp_dsm_priv *priv)
{
	...
	/* dp time info report */
	if (!hpd_report && !priv->hotplug_retval &&
	...
	uint8_t irq_type, uint8_t dev_type)
{
	...
	if (dev_type == TCA_DP_IN) { /* hotplug */
		...
		if (priv->hpd_repeated_num >= DP_HPD_REPEATED_THRESHOLD) {
			if (!dp_factory_mode_is_enable())
				dp_add_state_to_uevent_list(DP_LINK_STATE_MULTI_HPD,
					priv->is_dptx_vr);
		...
		priv->dp_out_num++;
		hwlog_info("%s: priv->dp_out_num = %d\n", __func__, priv->dp_out_num);
	...
	uint8_t mode_type, uint8_t dev_type, uint8_t typec_orien)
{
	...
	ktime_get_real_ts64(&(event->time));
	...
	uint8_t *edid)
{
	...
	hwlog_debug("edid_version is %x.%x\n", version, revision);
}
	...
	uint8_t *edid)
{
	...
	hotplug->mid[i] = '\0';
	hwlog_debug("manufacturer id is %s\n", hotplug->mid);
}
...
void dp_set_monitor_info(char *monitor, int len, uint8_t *edid)
{
	...
	if (len < DP_DSM_MONTIOR_INFO_SIZE) {
		hwlog_err("%s: len error\n", __func__);
		return;
	}
	...
	for (i = 0; i < EDID_DESC_NUM; i++, block += EDID_DESC_SIZE) {
		...
		for (j = offset; j < EDID_DESC_SIZE; j++) {
			if (block[j] == EDID_DESC_MONITOR_TAIL)
				break;
			monitor[j - offset] = block[j];
		}
		monitor[j - offset] = '\0';
		break;
	}

	hwlog_debug("monitor name is %s\n", monitor);
}
	...
	uint8_t *edid, int index)
{
	...
	if (index == 0) {
		...
		/* Check Extension Tag, Header Tag stored in the first uint8_t */
	...
	hwlog_debug("%d header_flag=0x%08x, checksum_flag=0x%08x\n",
		index, check->u32.header_flag, check->u32.checksum_flag);
	return 0;
}
	...
	enum dp_imonitor_param param, uint8_t *data)
{
	...
	if (index == 0) {
		...
		dp_clear_timing_info(priv);
#ifdef DP_DEBUG_ENABLE
		if (!dp_debug_get_lanes_rate_force(&priv->lanes_force, &priv->rate_force)) {
			priv->need_lanes_force = true;
			priv->need_rate_force = true;
		}

		if (!dp_debug_get_resolution_force(&priv->user_mode, &priv->user_format))
		...
		dp_set_monitor_info((char *)&hotplug->monitor,
			DP_DSM_MONTIOR_INFO_SIZE, data);
	}
	...
	for (i = 0; i < DP_DSM_EDID_BLOCK_SIZE; i += 16)
		hwlog_debug("EDID%d[%02x]:%02x %02x %02x %02x %02x %02x %02x "
			"%02x %02x %02x %02x %02x %02x %02x %02x %02x\n",
			index, i, edid[i], edid[i + 1], edid[i + 2], edid[i + 3],
...
static void dp_set_dpcd_rx_caps(struct dp_dsm_priv *priv, uint8_t *rx_caps)
{
	...
	hwlog_debug("dpcd_revision is %x.%x\n",
	...
	for (i = 0; i < DP_DSM_DPCD_RX_CAPS_SIZE; i += 16)
		hwlog_debug("DPCD[%02x]:%02x %02x %02x %02x %02x %02x %02x %02x "
	...
	enum dp_imonitor_param param)
{
...
#ifdef DP_DEBUG_ENABLE
		/* memory free in dp_clear_priv_data */
		hotplug = kzalloc(sizeof(*hotplug), GFP_KERNEL);
#else
		if (!priv->cur_hotplug_info) {
			hotplug = kzalloc(sizeof(*hotplug), GFP_KERNEL);
		} else {
			hotplug = priv->cur_hotplug_info;
	...
	if (priv->cur_hotplug_info)
		ktime_get_real_ts64(&(priv->cur_hotplug_info->time[index]));
	/* hotunplug, dp stop */
...
static void dp_set_link_training_failed(struct dp_dsm_priv *priv,
	struct dp_hotplug_info *hotplug)
{
	...
	if (priv->is_hotplug_running) {
		...
		/* 1. link retraining failed, 2. aux rw failed */
		if (!dp_factory_mode_is_enable())
	...
	enum dp_imonitor_param param, void *data)
{
	...
	case DP_PARAM_LINK_LANES:
		...
		dp_factory_set_link_rate_lanes(hotplug->rate, hotplug->lanes,
			hotplug->max_rate, hotplug->max_lanes);
		break;
	case DP_PARAM_TU:
	...
	case DP_PARAM_LINK_TRAINING_FAILED:
		...
		dp_set_link_training_failed(priv, hotplug);
		break;
	default:
		break;
	}
	...
	enum dp_imonitor_param param, const void *data)
{
	...
	case DP_PARAM_HDCP_KEY_S:
		ktime_get_real_ts64(&(priv->hdcp_time));
...
static void dp_set_hotplug(struct dp_dsm_priv *priv, const int *retval)
{
	...
	if (priv->hotplug_retval < 0) {
		...
		if (priv->link_training_retval < 0) { /* link training failed */
#ifndef DP_DEBUG_ENABLE
			dp_dsm_add_to_imonitor_list(priv, rcf_link_training_info,
...
#ifndef DP_DEBUG_ENABLE
			dp_dsm_add_to_imonitor_list(priv, rcf_hotplug_info,
				DP_IMONITOR_HOTPLUG);
#else
		...
		if (priv->cur_source_mode) /* same mode */
			dp_dsm_add_to_imonitor_list(priv, rcf_basic_info_of_same_mode,
				DP_IMONITOR_BASIC_INFO);
		else /* diff mode */
			dp_dsm_add_to_imonitor_list(priv, rcf_basic_info_of_diff_mode,
				DP_IMONITOR_BASIC_INFO);
	}

	dp_report_hotplug_state(priv); /* report link failed event */
}
	...
	if (priv->link_retraining_retval < 0) {
		if (!dp_factory_mode_is_enable())
			dp_add_state_to_uevent_list(DP_LINK_STATE_LINK_FAILED,
				priv->is_dptx_vr);
#ifndef DP_DEBUG_ENABLE
		dp_dsm_add_to_imonitor_list(priv, rcf_link_retraining_info,
			DP_IMONITOR_LINK_TRAINING);
#else
		dp_add_info_to_imonitor_list(DP_IMONITOR_LINK_TRAINING, priv);
#endif /* DP_DEBUG_ENABLE */
	...
	uint32_t addr, uint32_t len, int retval)
{
	...
	ktime_get_real_ts64(&(priv->aux_rw.time));
...
void dp_imonitor_set_param_vs_pe(int index, uint8_t *vs, uint8_t *pe)
{
	...
	if (!priv || !priv->cur_hotplug_info || !vs || !pe) {
...
#ifdef DP_DEBUG_ENABLE
	if (!dp_debug_get_vs_pe_force(&vs_force, &pe_force)) {
...
void dp_imonitor_set_param_resolution(uint8_t *user_mode, uint8_t *user_format)
{
	...
	if (!priv || !user_mode || !user_format) {
		hwlog_err("%s: priv, user_mode or user_formatis NULL\n", __func__);
		return;
	}
...
static int dp_imonitor_basic_info(struct imonitor_eventobj *obj, void *data)
{
	struct dp_dsm_priv *priv = dp_imonitor_peport_priv(g_dp_dsm_priv, data);
	struct dp_hotplug_info *hotplug = priv->cur_hotplug_info;
	int ret = 0;
	...
	(void)priv;
	ret += dp_set_param_i(obj, "CableType", priv->typec_cable_type);
	...
	ret += dp_set_param_i(obj, "HeadNum", hotplug->edid_check.u32.header_num);
	ret += dp_set_param_i(obj, "ChksNum", hotplug->edid_check.u32.checksum_num);
	ret += dp_set_param_i(obj, "HeadFlag", hotplug->edid_check.u32.header_flag);
	ret += dp_set_param_i(obj, "ChksFlag", hotplug->edid_check.u32.checksum_flag);
	ret += dp_set_param_i(obj, "ExtBlkNum", hotplug->edid_check.u32.ext_blocks_num);
	ret += dp_set_param_i(obj, "ExtBlks", hotplug->edid_check.u32.ext_blocks);
...
static int dp_imonitor_time_info(struct imonitor_eventobj *obj, void *data)
{
	...
	/* typec in & out time */
	dp_timeval_to_datetime_str(priv->typec_in_time, "typec_in",
		in_time, DP_DSM_DATETIME_SIZE);
	dp_timeval_to_datetime_str(priv->typec_out_time, "typec_out",
		out_time, DP_DSM_DATETIME_SIZE);
	priv->same_mode_time.tv_sec += DP_DSM_TIMEVAL_WITH_TZ;
	priv->diff_mode_time.tv_sec += DP_DSM_TIMEVAL_WITH_TZ;
	dp_timeval_to_datetime_str(priv->same_mode_time, "same_mode_time",
		same_time, DP_DSM_DATETIME_SIZE);
	dp_timeval_to_datetime_str(priv->diff_mode_time, "diff_mode_time",
		diff_time, DP_DSM_DATETIME_SIZE);
	priv->link_max_time.tv_sec =
		dp_dsm_time_ms_percent(priv->link_max_time.tv_nsec);
	priv->link_max_time.tv_sec += DP_DSM_TIMEVAL_WITH_TZ;
	dp_timeval_to_datetime_str(priv->link_max_time, "link_max_time",
		link_time, DP_DSM_DATETIME_SIZE);
	...
	if (report->report_skip_existed) {
		...
		hwlog_info("%s: last_link_min_time=%ld\n", __func__,
			report->last_link_min_time.tv_nsec);
		hwlog_info("%s: last_link_max_time=%ld\n", __func__,
			report->last_link_max_time.tv_nsec);
...
static void dp_init_report_info(struct dp_imonitor_report_info *report)
{
	...
	report->hpd_jiffies = jiffies + msecs_to_jiffies(DP_DSM_HPD_TIME_INTERVAL);
	report->report_jiffies = jiffies +
		msecs_to_jiffies(DP_DSM_REPORT_TIME_INTERVAL);
}
