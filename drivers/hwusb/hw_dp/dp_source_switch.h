...
int update_external_display_timming_info(
	uint32_t width, uint32_t high, uint32_t fps);
...
static inline int update_external_display_timming_info(
	uint32_t width, uint32_t high, uint32_t fps)
{
	return 0;
}

static inline int save_dp_edid(const uint8_t *edid_buf, uint32_t buf_len)
{
	return 0;
}
...
static inline void register_switch_handler(switch_source_handler handler)
{
}
#endif /* !CONFIG_HUAWEI_DP_SOURCE */
