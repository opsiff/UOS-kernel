...
int dp_hw_debug_init(void)
{
	return 0;
}

void dp_hw_debug_exit(void)
{
}
...
void dp_debug_init_combophy_pree_swing(uint32_t *pv, int count)
{
	...
	if (!priv || !pv) {
		hwlog_err("%s: priv or pv is NULL\n", __func__);
		return;
	}
...
int dp_debug_get_lanes_rate_force(uint8_t *lanes_force,
	uint8_t *rate_force)
{
	struct dp_debug_priv *priv = g_dp_debug_priv;

	if (!priv || !lanes_force || !rate_force) {
...
int dp_debug_get_resolution_force(uint8_t *user_mode,
	uint8_t *user_format)
{
	struct dp_debug_priv *priv = g_dp_debug_priv;
...
{
	int vs_pe_valid[DP_DSM_VS_PE_NUM * DP_DSM_VS_PE_LEN] = {
	...
	for (i = 0; i < DP_DSM_VS_PE_NUM * DP_DSM_VS_PE_LEN; i += DP_DSM_VS_PE_LEN) {
	...
	const struct kernel_param *kp)
{
	...
	unused(kp);
	if (!priv || !priv->combophy_pree_swing) {
	...
	for (i = 0; i < priv->combophy_pree_swing_count; i++) {
		if ((i % DP_COMBOPHY_PREE_SWING_INDENT) == 0)
			...
			dp_debug_append_info(buffer, DP_DEBUG_BUF_SIZE,
			" 0x%08x,", priv->combophy_pree_swing[i]);
	}
