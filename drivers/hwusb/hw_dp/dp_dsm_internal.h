...
struct dp_hotplug_info {
	struct list_head list;
	struct timespec64 time[DP_PARAM_TIME_NUM];
...
struct dp_typec_in_out {
	int in_out_num;
	struct timespec64 in_time[DP_DSM_TYPEC_IN_OUT_NUM];
	struct timespec64 out_time[DP_DSM_TYPEC_IN_OUT_NUM];
};
...
struct dp_dsm_priv {
	...
	struct timespec64 typec_in_time;
	struct timespec64 typec_out_time;
	struct timespec64 link_min_time;
	struct timespec64 link_max_time;
	struct timespec64 same_mode_time;
	struct timespec64 diff_mode_time;
	int event_num;
	enum tcpc_mux_ctrl_type typec_cable_type;
...
#define dp_imonitor_peport_priv(g, d) (((d) == NULL) ? (g) : (d))

#define dp_dsm_get_link_min_time(m, t) do { \
	if (m.tv_nsec == 0) \
		m.tv_nsec = t.tv_nsec; \
	else \
		m.tv_nsec = min(m.tv_nsec, t.tv_nsec); \
} while (0)

#define dp_dsm_get_link_max_time(m, t) \
	m.tv_nsec = max(m.tv_nsec, t.tv_nsec)
...
struct dp_imonitor_report_info {
	...
	struct timespec64 last_link_min_time;
	struct timespec64 last_link_max_time;
	struct timespec64 last_same_mode_time;
	struct timespec64 last_diff_mode_time;
...
#ifdef DP_DSM_SHIELD
#define dp_set_param_i(o, s, v) 0
#define dp_set_param_s(o, s, v) 0
#else
