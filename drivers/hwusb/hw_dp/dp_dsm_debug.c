...
static int dp_get_pd_event_result_in_out(char *buffer, int size)
{
	...
	earlist_point = dp_dsm_earliest_time_point(typec_in_out->in_out_num,
		DP_DSM_TYPEC_IN_OUT_NUM);
	...
	for (i = 0; i < count; i++) {
		dp_timeval_to_datetime_str(typec_in_out->in_time[i], "typec_in",
			datetime, DP_DSM_DATETIME_SIZE);
		ret += dp_debug_append_info(buffer, size, "[%02d] %-19s", i, datetime);

		dp_timeval_to_datetime_str(typec_in_out->out_time[i],
			"typec_out", datetime, DP_DSM_DATETIME_SIZE);
		if (i != earlist_point)
	...
	char *buffer, int size)
{
	...
	list_for_each(pos, &(priv->event_list_head)) {
		...
		dp_timeval_to_datetime_str(event->time, "pd_event", datetime,
			DP_DSM_DATETIME_SIZE);
		ret += dp_debug_append_info(buffer, size,
...
static int dp_get_pd_event_result_irq_vector(
	struct dp_dsm_priv *priv, char *buffer, int size)
{
	int earlist_point;
	int count;
	int i;
	...
	char *buffer, int size)
{
	...
	dp_timeval_to_datetime_str(priv->hdcp_time, "hdcp_auth", datetime,
		DP_DSM_DATETIME_SIZE);
...
int dp_get_pd_event_result(char *buffer, int size)
{
	...
	dp_timeval_to_datetime_str(priv->typec_in_time, "typec_in",
		datetime, DP_DSM_DATETIME_SIZE);
	ret += dp_debug_append_info(buffer, size, "typec_in_time: %s\n", datetime);
	dp_timeval_to_datetime_str(priv->typec_out_time, "typec_out",
		datetime, DP_DSM_DATETIME_SIZE);
	ret += dp_debug_append_info(buffer, size, "typec_out_time: %s\n", datetime);
	...
	char *buffer, int size, char *datetime, int count)
{
	...
	if (priv->aux_rw.retval < 0)
		ret += dp_debug_append_info(buffer, size,
			...
			count, "failed", datetime, priv->aux_rw.rw, priv->aux_rw.i2c,
			priv->aux_rw.addr, priv->aux_rw.len, priv->aux_rw.retval);
	...
	char *buffer, int size)
{
	...
	ret += dp_debug_append_info(buffer, size, "source switch(%d):",
		priv->source_switch_num);
	for (i = 0; i < count; i++) {
		if (i != earlist_point)
			ret += dp_debug_append_info(buffer, size, " %u",
				priv->source_switch[i]);
		else
			ret += dp_debug_append_info(buffer, size, " %u*",
				priv->source_switch[i]);
	}
...
int dp_get_hotplug_result(char *buffer, int size)
{
	...
	if (priv->typec_cable_type == TCPC_DP)
	...
	ret += dp_get_hotplug_result_basic(priv->cur_hotplug_info, buffer, size);
