...
int regulator_enable(struct regulator *regulator)
{
	...
	CALL_HCK_VH(regulator_on_off, rdev);
...
out:
	CALL_HCK_VH(regulator_set_vol, rdev, max_uV, min_uV);
	return ret;
}
...
int regulator_set_mode(struct regulator *regulator, unsigned int mode)
{
	...
	CALL_HCK_VH(regulator_set_mode, rdev, mode);
out:
	regulator_unlock(rdev);
	return ret;
}
...
static int __init regulator_init_complete(void)
{
	 ...
	 *
	 * To solve the problem that the LCD is powered off unexpectedly,
	 * the value of 30000 ms is changed to 300 ms.
	 */
