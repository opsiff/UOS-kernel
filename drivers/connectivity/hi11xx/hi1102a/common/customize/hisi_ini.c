...
#else
int8 g_ini_file_name[INI_FILE_PATH_LEN] = "/app/cfg_hi1102a.ini";
#endif
...
int ini_cfg_init(void)
{
    ...
    if (ini_file_exist(g_ini_file_name)) {
        ini_info("get ini_file_name@%s from default path\n", g_ini_file_name);
        goto success_ini_cfg_init;
    }
    ini_error("can't find ini_file from default path %s!\n", g_ini_file_name);
#endif
