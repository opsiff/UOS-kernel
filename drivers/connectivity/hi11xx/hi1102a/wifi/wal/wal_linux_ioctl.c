...
#ifdef _PRE_WLAN_EXPORT
OAL_STATIC oal_void hwifi_config_export_ini_param(oal_void)
{
    oal_uint32 mac_addr = 0;
    oal_uint32 mac_addr_2byte = 0;

    /* ini值对应mac地址前三字节 */
    mac_addr = hwifi_get_init_value(CUS_TAG_INI, WLAN_CFG_INIT_EFUSE_MAC_ADDR);
    mac_addr_2byte = hwifi_get_init_value(CUS_TAG_INI, WLAN_CFG_INIT_EFUSE_MAC_ADDR_2BYTE);
    if (mac_addr == 0) {
        return;
    }
    if (mac_addr_2byte) {
        device_mac_addr[BYTE_OFFSET_0] = (mac_addr >> BYTE_OFFSET_8) & 0xff;
        device_mac_addr[BYTE_OFFSET_1] = mac_addr & 0xff;
    } else {
        device_mac_addr[BYTE_OFFSET_0] = (mac_addr >> BYTE_OFFSET_16) & 0xff;
        device_mac_addr[BYTE_OFFSET_1] = (mac_addr >> BYTE_OFFSET_8) & 0xff;
        device_mac_addr[BYTE_OFFSET_2] = mac_addr & 0xff;
    }
}
#endif
    ...
    oal_net_device_stru *pst_net_dev, oal_int8 *pc_command, wal_wifi_priv_cmd_stru st_priv_cmd)
{
    ...
    if (oal_unlikely(l_ret != OAL_SUCC)) {
        OAM_WARNING_LOG1(0, OAM_SF_ANY, "{wal_ioctl_priv_cmd_countryd_priv_cmd:return err code [%d]", l_ret);
...
OAL_STATIC oal_uint32 wal_hipriv_mac_addr_read(oal_net_device_stru *net_dev, oal_int8 *param)
{
    ...
    ***************************************************************************/
    ...
    if (leftime == 0) {
        ...
        mac_addr_2byte = hwifi_get_init_value(CUS_TAG_INI, WLAN_CFG_INIT_EFUSE_MAC_ADDR_2BYTE);
        if (mac_addr_2byte) {
            efuse_mac_len = BYTE_OFFSET_4;
        } else {
            efuse_mac_len = BYTE_OFFSET_3;
        }
        if (g_us_efuse_mac_addr_report != 0) {
            for (loop = 0; loop < efuse_mac_len; loop++) {
        ...
        memset_s(&id_param, OAL_SIZEOF(mac_cfg_staion_id_param_stru), 0, OAL_SIZEOF(mac_cfg_staion_id_param_stru));
        for (loop = 0; loop < WLAN_MAC_ADDR_LEN; loop++) {
...
OAL_STATIC oal_uint32 wal_hipriv_autorate_param(oal_net_device_stru *net_dev, oal_int8 *param)
{
    ...
    oal_int32 ret;
    oal_uint32 pow_param;
    oal_uint8 loop;

    for (loop = 0; loop < EXPORT_POW_LEVEL; loop++) {
        pow_param = hwifi_get_init_value(CUS_TAG_INI, WLAN_CFG_INIT_5G_POW_PARA_L0 + loop);
        /* ini中功率参数为6位16进制数,低2位表示dbb值,中2位表示lpf值,高2位表示upc值 */
        autorate_param.autorate_set[loop].dbb = pow_param & 0xff;
        autorate_param.autorate_set[loop].lpf = (pow_param >> EXPORT_POW_LPF_OFFSET) & 0xff;
        autorate_param.autorate_set[loop].upc = (pow_param >> EXPORT_POW_UPC_OFFSET) & 0xff;
    }
