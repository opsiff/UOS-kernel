...
OAL_STATIC oal_void hmac_scan_update_vht_cap(mac_bss_dscr_stru *pst_bss_dscr, oal_uint8 *puc_ie)
{
    ...
    /* : vht cap info非全0或rx mcs非全f，则认为支持11ac */
...
OAL_STATIC oal_void hmac_scan_chip_oui_process(mac_bss_dscr_stru *pst_bss_dscr,
                                               ...
                                               oal_uint16 us_offset)
{
    /* :用于识别TP-LINK 847N，记录芯片厂家OUI，其他芯片厂家记录需要时再增加 */
    ...
    mac_bss_dscr_stru *pst_bss_dscr, oal_uint8 *puc_frame_body, oal_uint16 us_frame_len, mac_device_stru *pst_mac_dev)
{
    ...
    if (puc_ie != OAL_PTR_NULL) {
        ...
        /*  斐讯FIR304商用AP 11g模式，发送的支持速率集个数为12，不符合协议规定,
           为增加兼容性，修改判断分支为12 */
...
{
    /*  对隐藏ssid ap，之前扫描到probe rsp， 携带ssid， 当前扫描到为beacon ，不携带ssid ，则不更新时间戳 */
...
oal_uint32 hmac_scan_proc_scanned_bss(frw_event_mem_stru *pst_event_mem)
{
    ...
    /* begin:  对隐藏ssid AP, 接收到beacon 帧，不更新时间戳，避免删除隐藏ssid 网络后，还显示扫描结果 */
    ...
    /* end:  对隐藏ssid AP, 接收到beacon 帧，不更新时间戳，避免删除隐藏ssid 网络后，还显示扫描结果 */
                                                     ...
                                                     mac_scan_rsp_stru *pst_d2h_scan_rsp_info)
{
    ...
    /* 如果扫描回调函数不为空，则调用回调函数 */
    /* 为了提升去关联后重关联的速度,所以在scan abort后立即上报扫描结果,需要芯片上报上一次扫描结果后才会触发新一轮扫描,从而影响重关联速度 */
    ...
    mac_scan_rsp_stru *pst_d2h_scan_rsp_info)
{
    ...
    if (pst_hmac_vap->en_wait_roc_end == OAL_TRUE) {
        /* 有此标记就通知结束, 防止wal层下发扫描的时候在扫描, 下发abort, 但此时刚好扫描正常结束,
