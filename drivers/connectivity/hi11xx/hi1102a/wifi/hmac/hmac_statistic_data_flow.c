...
OAL_STATIC void hmac_lock_fast_freq(void)
{
    oal_uint8 uc_cpuid;
    /* 所有核都锁定最高频率 */
...
OAL_STATIC oal_void hmac_thread_bind_fast_cpu_affinity(uint8_t thread_bind)
{
#if (_PRE_OS_VERSION_LINUX == _PRE_OS_VERSION)
    ...
    if (thread_bind != 0) {
        ...
        set_cpus_allowed_ptr(hcc_get_110x_handler()->bus_dev->cur_bus->pst_rx_tsk, &fast_cpus);
    }
#endif
#endif
#if defined(_PRE_FEATURE_PLAT_LOCK_CPUFREQ) && !defined(CONFIG_HI110X_KERNEL_MODULES_BUILD_SUPPORT)
...
#if defined(_PRE_FEATURE_PLAT_LOCK_CPUFREQ) && !defined(CONFIG_HI110X_KERNEL_MODULES_BUILD_SUPPORT)
#if (_PRE_OS_VERSION_LINUX == _PRE_OS_VERSION)
OAL_STATIC void hmac_lock_slow_freq(void)
{
    oal_uint8 uc_cpuid;
    for (uc_cpuid = 0; uc_cpuid < OAL_BUS_MAXCPU_NUM; uc_cpuid++) {
        if (g_ast_cpumaxfreq[uc_cpuid].valid != OAL_TRUE) {
...
#if CONFIG_NR_CPUS > OAL_BUS_HPCPU_NUM
    struct cpumask cpus_mask;
    external_get_slow_cpus(&cpus_mask);
...
#if defined(CONFIG_ARCH_HISI) && defined(CONFIG_NR_CPUS)
#if CONFIG_NR_CPUS > OAL_BUS_HPCPU_NUM
OAL_STATIC oal_void hmac_thread_bind_slow_cpu_affinity(uint8_t thread_bind)
{
#if (_PRE_OS_VERSION_LINUX == _PRE_OS_VERSION)
...
#if !defined(WIN32)
    if (thread_bind != 0) {
        set_cpus_allowed_ptr(g_st_rxdata_thread.pst_rxdata_thread, &all_cpus);
        set_cpus_allowed_ptr(hcc_get_110x_handler()->hcc_transer_info.hcc_transfer_thread, &all_cpus);
        set_cpus_allowed_ptr(hcc_get_110x_handler()->bus_dev->cur_bus->pst_rx_tsk, &all_cpus);
    }
#endif
#endif
#if defined(_PRE_FEATURE_PLAT_LOCK_CPUFREQ) && !defined(CONFIG_HI110X_KERNEL_MODULES_BUILD_SUPPORT)
...
OAL_STATIC void hmac_thread_and_hcc_bindcpu(oal_uint8 en_irq_cpu_state,
    uint8_t thread_bind)
{
    if (en_irq_cpu_state == WLAN_IRQ_AFFINITY_BUSY_CPU) {
        hmac_thread_bind_fast_cpu_affinity(thread_bind);
    } else {
        hmac_thread_bind_slow_cpu_affinity(thread_bind);
    }
}


OAL_STATIC uint8_t hmac_wifi_thread_bind_check(uint8_t *thread_bind)
{
...
#endif
#endif
OAL_STATIC void hmac_thread_bindcpu_affinity(oal_uint8 en_irq_cpu_state)
{
#if !defined(_PRE_PRODUCT_HI1620S_KUNPENG) && !defined(_PRE_WINDOWS_SUPPORT)
#if defined(CONFIG_ARCH_HISI) && defined(CONFIG_NR_CPUS)
#if CONFIG_NR_CPUS > OAL_BUS_HPCPU_NUM
    uint8_t thread_bind, ret;
    /* if need to bind */
    ret = hmac_wifi_thread_bind_check(&thread_bind);
    oam_warning_log2(0, OAM_SF_ANY, "{hmac_wifi_thread_bind_check::ret[%d] thread[%d]}",
        ret, thread_bind);
    if (thread_bind == 0) {
        return;
    }
    hmac_thread_and_hcc_bindcpu(en_irq_cpu_state, thread_bind);
#endif
#endif
#endif
}
...
oal_void hmac_thread_bindcpu(oal_uint32 ul_total_pps)
{
    ...
    if (en_irq_cpu_state != g_st_thread_bindcpu.en_irq_cpu_state) {
        ...
        /* 任意裁核方案对短距性能优化需求 */
        if (g_st_thread_bindcpu.en_irq_bindcpu == WLAN_IRQ_AFFINITY_EN) {
            hmac_thread_bindcpu_affinity(en_irq_cpu_state);
        } else {
            /* 流量较大时将收发线程绑定在大核 */
            if (en_irq_cpu_state == WLAN_IRQ_AFFINITY_BUSY_CPU) {
