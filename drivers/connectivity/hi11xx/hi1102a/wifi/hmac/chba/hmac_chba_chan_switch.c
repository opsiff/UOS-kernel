...
uint8_t hmac_chba_is_support_social_chan_cap(void)
{
    return (uint8_t)hwifi_get_chba_social_chan_cap();
}
    ...
    mac_channel_stru *social_channel)
{
    ...
    if (hmac_chba_is_support_social_chan_cap()) {
        ...
        if (chan_temp == 0) {
            ...
            ret = hmac_config_chba_social_channel_params(chan_temp, bandwidth, social_channel);
