...
void hmac_nb_send_action(hmac_vap_stru *hmac_vap, mac_vap_stru *mac_vap, uint8_t *buf, uint32_t len)
{
    ...
    if (hmac_vap == NULL || mac_vap == NULL || g_nb_hmac_user == NULL) {
        oam_warning_log0(0, 0, "hmac_chba_send_sync_action: hmac_vap OR mac_vap OR g_nb_hmac_user is null.");
        return;
    }
    ...
    if (nb_action_frame == NULL) {
        OAM_ERROR_LOG0(mac_vap->uc_vap_id, 0, "hmac_chba_send_sync_action::alloc fail.");
        return;
    }
