    ...
    hmac_external_auth_req_stru *pst_ext_auth)
{
    ...
    if (IS_LEGACY_AP(pst_mac_vap) && !mac_is_chba_mode(pst_mac_vap)) {
        return OAL_SUCC;
    }

    /* 非legacy sta和非chba设备，直接返回 */
    if (!IS_LEGACY_STA(pst_mac_vap) && !mac_is_chba_mode(pst_mac_vap)) {
    ...
    hmac_vap_stru **hmac_vap, hmac_user_stru **hmac_user)
{
    ...
    *hmac_vap = mac_res_get_hmac_vap(mac_vap->uc_vap_id);
    ...
    ret = mac_vap_find_user_by_macaddr(mac_vap, ext_auth->auc_bssid, WLAN_MAC_ADDR_LEN, &user_idx);
    if (ret != OAL_SUCC) {
        oam_warning_log3(mac_vap->uc_vap_id, OAM_SF_SAE,
            "{hmac_config_external_auth::mac_vap_find_user_by_macaddr failed[%d]}",
            ret, ext_auth->auc_bssid[MAC_ADDR_4], ext_auth->auc_bssid[MAC_ADDR_5]);
        return OAL_FAIL;
    }
...
oal_uint32 hmac_config_external_auth(mac_vap_stru *pst_mac_vap, oal_uint16 us_len, oal_uint8 *puc_param)
{
    ...
    ul_ret = hmac_config_external_auth_get_res(pst_mac_vap, pst_ext_auth, &pst_hmac_vap, &pst_hmac_user);
    if (ul_ret != OAL_SUCC) {
        return ul_ret;
    }
    ...
    if (IS_LEGACY_AP(pst_mac_vap) && !mac_is_chba_mode(pst_mac_vap)) {
        ...
        hmac_sae_external_auth_process_ap(pst_hmac_vap, pst_hmac_user, pst_ext_auth);
        return OAL_SUCC;
    }
