...
#define DMAC_HITALK_KEY_LEN                128
...
typedef oal_uint8 wal_autorate_cfg_enum_uint8;
#endif
...
typedef struct {
    ...
    mac_narrow_bw_enum_uint8 en_bw; /* 1M,5M,10M */
    oal_uint8 uc_rsv;
} mac_cfg_distance_stru;
typedef struct {
    uint8_t value;
    mac_power_param_uint8 type; /* UPC, LPF, DBB */
    oal_uint8 uc_rsv[2];
} mac_cfg_power_param_stru;
typedef struct {
    uint8_t vendor_channel; /* 目前只支持0, 1两个vendor 信道 */
    uint8_t uc_rsv[3];
} mac_cfg_vendor_channel_stru;
#endif
...
#ifdef _PRE_WLAN_NARROW_BAND
typedef struct {
    uint8_t nokey_mode_en;
    uint8_t resv[3];
} mac_cfg_set_nokey_mode_stru;

typedef struct {
    uint8_t hitalk_key[DMAC_HITALK_KEY_LEN];
} mac_cfg_key_info_stru;

typedef struct {
    wal_autorate_cfg_enum_uint8 en_type;
    oal_int8 val;
...
typedef struct {
    oal_uint16 user_id;
    oal_uint16 total_mpdu_num;
} mac_cfg_query_tid_queue_stru;
...
#endif
typedef oal_rom_cb_result_enum_uint8 (*p_mac_vap_init_cb)(mac_vap_stru *pst_vap, oal_uint8 uc_chip_id,
    oal_uint8 uc_device_id, oal_uint8 uc_vap_id, mac_cfg_add_vap_param_stru *pst_param, oal_uint32 *pul_cb_ret);
