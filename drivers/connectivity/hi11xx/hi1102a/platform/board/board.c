...
#ifdef _PRE_WLAN_EXPORT
    char type_buff[BOARD_VERSION_LEN] = {0};
    int8_t *pc_key_addr = NULL;

    ret = get_cust_conf_string(INI_MODU_PLAT, "g_param_version.param_version", type_buff, sizeof(type_buff) - 1);
    if (ret == INI_SUCC) {
        pc_key_addr = oal_strstr(type_buff, "Hi1102");
        if (pc_key_addr != NULL) {
            g_board_info.chip_type = BOARD_VERSION_NAME_HI1102A;
            ps_print_info("get board chip_type:%s from ini\n", g_board_info.chip_type);
            return BOARD_SUCC;
        }
    }
#endif
