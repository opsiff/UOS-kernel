                                       ...
                                       oal_gfp_enum_uint8 en_gfp)
{
    ...
    cfg80211_ready_on_channel(pst_netdev, ull_cookie, pst_chan, 1, ul_duration, en_gfp);
#else
#endif
}
                                                ...
                                                oal_gfp_enum_uint8 en_gfp)
{
    ...
    oal_net_device_stru *pst_netdev;
    pst_netdev = pst_wdev->netdev;
    cfg80211_remain_on_channel_expired(pst_netdev,
                                       ull_cookie,
                                       pst_listen_channel,
                                       1,
                                       GFP_ATOMIC);
...
#elif (LINUX_VERSION_CODE >= KERNEL_VERSION(3, 0, 44))
    oal_net_device_stru *pst_netdev;
    pst_netdev = wdev->netdev;
    cfg80211_mgmt_tx_status(pst_netdev, cookie, buf, len, ack, gfp);
#endif
}
...
{
#if defined(_PRE_PRODUCT_ID_HI110X_HOST) && (LINUX_VERSION_CODE >= KERNEL_VERSION(3, 6, 34))
