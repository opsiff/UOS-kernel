...
STATIC int32_t bfgx_recv_device_sw_version(struct ps_core_s *ps_core_d, uint8_t *buf_ptr);
...
STATIC ps_core_msg_handler g_ps_core_msg_handle[MSG_BUTT] = {
    ...
    bfgx_recv_device_sw_version, // BFGX_SW_VERSION = 0x13, 读取BFGX 软件版本号
};
...
STATIC int32_t bfgx_recv_device_sw_version(struct ps_core_s *ps_core_d, uint8_t *buf_ptr)
{
    complete(&bfgx_sw_version_ack);
    ps_print_warning("bfgx_device_software_version::%s\n", buf_ptr);
    return 0;
}
