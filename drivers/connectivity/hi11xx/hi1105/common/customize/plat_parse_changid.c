...
STATIC uint8_t* get_readme_path(void)
{
    ...
    path_len = (uint32_t)OAL_STRLEN(MPXX_CUST_FIRMWARE_PATH) + (uint32_t)OAL_STRLEN(chip_name) +
               (uint32_t)OAL_STRLEN(MPXX_CUST_README_NAME) + 1;
...
STATIC int32_t plat_parse_changid(uint8_t *buf_addr, int32_t buf_len)
{
    ...
    len = (int32_t)OAL_STRLEN(buf_addr) - (int32_t)OAL_STRLEN(ID_KEY_INFO) + 1;
