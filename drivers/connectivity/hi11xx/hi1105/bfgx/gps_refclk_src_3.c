 ...
 *
 * Description: gps refclk_src functional module
...
static ssize_t gps_write_proc_nstandby(struct file *filp, const char __user *buffer, size_t len, loff_t *off)
{
    ...
    if (gps_nstandby == '0') {
        ...
        set_gps_ref_clk_enable_mpxx(false, (gps_modem_id_enum)0, (gps_rat_mode_enum)0);
    } else if (gps_nstandby == '1') {
        printk(KERN_INFO "[GPS] refclk SCPLL0 enable.\n");
        set_gps_ref_clk_enable_mpxx(true, (gps_modem_id_enum)0, (gps_rat_mode_enum)0);
    } else if (gps_nstandby == '2') {
        printk(KERN_INFO "[GPS] refclk SCPLL1 enable.\n");
        set_gps_ref_clk_enable_mpxx(true, (gps_modem_id_enum)0, (gps_rat_mode_enum)4);
    } else if (gps_nstandby == '3') {
        printk(KERN_INFO "[GPS] refclk SCPLL2 enable.\n");
        set_gps_ref_clk_enable_mpxx(true, (gps_modem_id_enum)1, (gps_rat_mode_enum)2);
    } else {
...
static int hi_gps_probe(struct platform_device *pdev)
{
    ...
    register_gps_set_ref_clk_func((void *)set_gps_ref_clk_enable_mpxx);
...
int hi_gps_plat_init(void)
{
    ...
    if (isMyConnectivityChip(CHIP_TYPE_HI110X)) {
        ...
        printk(KERN_ERR "[GPS] chip type not mpxx");
    }
    return ret;
}
...
int set_gps_ref_clk_enable_mpxx(bool enable, gps_modem_id_enum modem_id, gps_rat_mode_enum rat_mode)
{
    int ret = 0;
...
MODULE_DESCRIPTION("GPS MPXX Platfrom driver");
MODULE_LICENSE("GPL");
