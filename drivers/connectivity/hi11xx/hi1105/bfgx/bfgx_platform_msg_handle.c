 ...
 *
 * Description: BFGX Slave Platform Message Processing
...
STATIC platform_msg_handler g_sys_msg_handle[SYS_INF_BUTT] = {
    ...
    bfgx_thread_msg_handle,      // SYS_INF_GNSS_LPPE_INIT = 0x1F,         mp13 GNSS 新增的线程初始化完成
    bfgx_thread_msg_handle,      // SYS_INF_GNSS_LPPE_EXIT = 0x20,         mp13 GNSS 新增的线程退出
    gnss_trickle_request_sleep,  // SYS_INF_GNSS_TRICKLE_SLEEP = 0x21,     mp13 GNSS TRICKLE Sleep
#ifdef _PRE_H2D_GPIO_WKUP
    bfgx_thread_msg_handle,      // SYS_INF_ENABLE_GPIO_WKUP_BFG_RSP = 0x22
#else
    platform_unused_msg,
#endif
    bfgx_thread_msg_handle,      // SYS_INF_BOOT_UP_ACK = 0x23
    bfgx_thread_msg_handle,      // SYS_INF_WAKE_UP_ACK = 0x24
    bfgx_thread_msg_handle,      // SYS_INF_SLE_INIT = 0x25,               SLE软件初始化完成
    bfgx_thread_msg_handle,      // SYS_INF_SLE_EXIT = 0x26,               SLE软件初始化完成
};
...
STATIC void bfgx_thread_msg_handle(struct ps_core_s *ps_core_d,
                                   uint8_t *buf_ptr)
{
    uint8_t msg = *buf_ptr;
    ...
    switch (msg) {
        ...
        case SYS_INF_ENABLE_GPIO_WKUP_BFG_RSP:
            ...
            break;
#endif
        case SYS_INF_SLE_INIT:
            thread_completion_handle(ps_core_d, THREAD_FLAG_INIT, BFGX_SLE);
            break;
        case SYS_INF_SLE_EXIT:
            thread_completion_handle(ps_core_d, THREAD_FLAG_EXIT, BFGX_SLE);
            break;
        default:
            break;
    };
...
STATIC void bfgx_read_mem_complete(struct ps_core_s *ps_core_d, uint8_t *buf_ptr)
{
    oal_reference(buf_ptr);
    bfgx_memdump_finish(ps_core_d);

    ps_print_info("[%s]plat bfgx mem dump complete\n", index2name(ps_core_d->pm_data->index));
    atomic_set(&ps_core_d->is_memdump_runing, 0);
    complete_all(&ps_core_d->wait_read_bfgx_stack);
}
...
STATIC void heartbeat_debug_print(struct ps_core_s *ps_core_d, uint8_t syschar)
{
    ...
    ps_print_info("[%s]pkt_rx:BT=%lu,SLE=%lu,GNSS=%lu,FM=%lu,SYS_MSG=%lu,OML=%lu,timer=%d,mod=%d,sleep_flag=%d,\
                  frc_awake=%d\n",
                  index2name(pm_data->index), ps_core_d->bfgx_info[BFGX_BT].rx_pkt_num,
                  ps_core_d->bfgx_info[BFGX_SLE].rx_pkt_num,
                  ...
                  oal_atomic_read(&pm_data->bfg_timer_mod_cnt),
                  atomic_read(&pm_data->gnss_sleep_flag),
                  pm_data->gnss_frc_awake);
    ps_print_info("[%s]pkt_tx:BT=%lu, SLE=%lu, GNSS=%lu, FM=%lu, IR=%lu\n",
                  index2name(pm_data->index), ps_core_d->bfgx_info[BFGX_BT].tx_pkt_num,
