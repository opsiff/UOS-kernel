...
static int32_t hwal_get_band(int8_t *ifname, oal_ieee80211_supported_band **band)
{
    ...
    if (oal_unlikely(netdev == NULL || netdev->ieee80211_ptr == NULL)) {
        oam_error_log0(0, OAM_SF_ANY, "{hwal_get_band::netdev or ieee80211_ptr null!}");
        return -OAL_EFAIL;
    }
                                   ...
                                   hisi_scan_stru *scan_params, oal_cfg80211_scan_request_stru *request)
{
    ...
    if (oal_unlikely(netdev == NULL)) {
        oam_error_log0(0, OAM_SF_ANY, "{hwal_get_scan_param::netdev null!}");
        return -OAL_EFAUL;
    }
    request->wiphy = wiphy;
    request->dev = netdev;
