...
oal_netbuf_stru *hwal_lwip_skb_alloc(oal_net_device_stru *net_dev, uint16_t lwip_buflen)
{
    ...
    buf = oal_netbuf_alloc(total_size, net_dev->needed_headroom, 32); // 32字节对齐
...
#if (_PRE_OS_VERSION_LITEOS == _PRE_OS_VERSION)
/*
 * 功能描述  : get status of wal_txdata thread
 * 1.日    期: 2013年8月29日
 *   修改内容: 新增函数
 */
...
/*
 * 功能描述  : wake up wal_txdata thread
 * 1.日    期: 2017年1月2日
 *   作    者: wifi
 *   修改内容: 新生成函数
 */
...
{
    if (oal_netbuf_list_len(&g_wal_txdata_thread.txdata_netbuf_head) > 1000) { // 1000: max netbuf cnt
        oal_netbuf_free(netbuf);
...
OAL_STATIC OAL_INLINE oal_netbuf_stru *hwal_lwip_converted_skb(oal_net_device_stru *net_dev, oal_lwip_buf *lwip_buf)
{
    ...
    /* origin liteos */
    /* SDK_B050 */
    /* 32: magic num to be modified by tangshuang */
    ...
    skb_reserve(converted_skb, 32 - (ETHER_HDR_LEN - SNAP_LLC_FRAME_LEN)); // 32: reserve length
#else
...
OAL_STATIC OAL_INLINE oal_netbuf_stru *hwal_lwip_alloc_skb(oal_net_device_stru *net_dev, oal_lwip_buf *lwip_buf)
{
    oal_netbuf_stru *converted_skb = NULL;
#ifdef _PRE_LWIP_ZERO_COPY
    /* 2：HCC发送成功之前,Lwip重传包不做再次下放,直接返回,避免对内存重复操作 */
    if (lwip_buf->ref >= 2) {
        return NULL;
    }
...
void hwal_netif_drv_config(struct netif *netif, uint32_t config_flags, uint8_t set_bit)
{
    return;
}
