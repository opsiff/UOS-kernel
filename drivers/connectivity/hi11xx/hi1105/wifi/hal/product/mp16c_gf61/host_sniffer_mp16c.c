...
#undef THIS_FILE_ID
#define THIS_FILE_ID OAM_FILE_ID_HOST_SNIFFER_MP16C_C

/*
 * 函数描述：rx ppdu描述符 free ring 初始化
 */
uint32_t mp16c_rx_ppdu_dscr_free_ring_init(hal_host_device_stru *hal_dev)
{
    uint16_t entries;
    ...
    if (OAL_FALSE == mp16c_rx_ring_reg_init(hal_dev, rx_ring_ctl, HAL_RING_TYPE_P_F)) {
        oam_error_log0(0, 0, "hal_rx_host_init_ppdu_free_ring alloc pcie_if_hostca_to_devva fail.");
        dma_free_coherent(&pcie_dev->dev, alloc_size, rbase_vaddr, rbase_dma_addr);
...
static uint32_t mp16c_dma_map_rx_ppdu_dscr(
    uint8_t dma_dir, hal_host_rx_ppdu_dscr_stru *ppdu_dscr, dma_addr_t *dma_addr)
{
    ...
    *dma_addr = dma_map_single(&pcie_dev->dev, (uint8_t *)&ppdu_dscr->hw_ppdu_data,
        sizeof(mp16c_rx_ppdu_desc_stru), dma_dir);
 ...
 */
uint32_t mp16c_add_rx_ppdu_dscr(hal_host_device_stru *hal_device)
{
    uint64_t devva = 0;
    ...
    for (loop = 0; loop < need_add; loop++) {
        ...
        if (mp16c_dma_map_rx_ppdu_dscr(DMA_FROM_DEVICE, ppdu_dscr, &pci_dma_addr) != OAL_SUCC) {
            oal_free(ppdu_dscr);
 ...
 */
uint32_t mp16c_sniffer_rx_ppdu_free_ring_init(hal_host_device_stru *hal_device)
{
    /* ring 初始化 */
    if (mp16c_rx_ppdu_dscr_free_ring_init(hal_device) != OAL_SUCC) {
        oam_error_log0(0, 0, "sniffer_rx_ppdu_free_ring_init::init_rx_ppdu_free_ring fail.");
        return OAL_FAIL;
    }
    mp16c_rx_set_ring_regs(&(hal_device->host_rx_ppdu_free_ring));
    /* 向 free ring添加内存 */
    if (mp16c_add_rx_ppdu_dscr(hal_device) != OAL_SUCC) {
 ...
 * 功能描述: 释放rx ppdu ring
 */
void mp16c_rx_ppdu_from_list_free(hal_host_rx_ppdu_dscr_list_head_stru *ppdu_list)
{
    hal_host_rx_ppdu_dscr_stru *ppdu_dscr = NULL;
 ...
 */
void mp16c_sniffer_rx_ppdu_free_ring_deinit(hal_host_device_stru *hal_dev)
{
    ...
    /* 释放alloc list */
    mp16c_rx_ppdu_from_list_free(&hal_dev->host_rx_ppdu_alloc_list);
    /* 释放ring本身 */
 ...
 */
void mp16c_sniffer_set_rx_info(hal_sniffer_extend_info *sniffer_rx_info, mac_rx_ctl_stru *cb,
    mp16c_rx_ppdu_desc_stru *ppdu_dscr)
{
    ...
    sniffer_rx_statistic->c_ant1_rssi = ppdu_dscr->rx_ppdu_vector_info.bit_rpt_rssi_lltf_ant1;
}

/*
 * 功能描述：rx速率信息填充
 */
void mp16c_sniffer_set_rx_rate_info(hal_sniffer_extend_info *sniffer_rx_info,
    mp16c_rx_ppdu_desc_stru *ppdu_dscr)
{
...
static uintptr_t mp16c_get_rx_ppdu_dscr_addr(oal_netbuf_stru *netbuf)
{
...
hal_host_rx_ppdu_dscr_stru *mp16c_sniffer_get_rx_ppdu_dscr(hal_host_device_stru *hal_dev, oal_netbuf_stru *netbuf)
{
    ...
    rx_ppdu_dscr_addr = mp16c_get_rx_ppdu_dscr_addr(netbuf);
...
void mp16c_sniffer_rx_info_fill_host_rx(hal_host_device_stru *hal_device, oal_netbuf_stru *netbuf,
    hal_sniffer_extend_info *sniffer_rx_info, mac_rx_ctl_stru *rx_ctl)
{
    ...
    rx_ppdu = mp16c_sniffer_get_rx_ppdu_dscr(hal_device, netbuf);
    if (rx_ppdu == NULL) {
        return;
    }

    dma_unmap_single(&pcie_dev->dev, rx_ppdu->paddr.addr,
        sizeof(mp16c_rx_ppdu_desc_stru), DMA_FROM_DEVICE);

    mp16c_sniffer_set_rx_info(sniffer_rx_info, rx_ctl, &rx_ppdu->hw_ppdu_data);
    mp16c_sniffer_set_rx_rate_info(sniffer_rx_info, &rx_ppdu->hw_ppdu_data);

    oal_dlist_delete_entry(&rx_ppdu->entry);
    oal_free(rx_ppdu);
}

void mp16c_sniffer_rx_info_fill_device_rx(hal_host_device_stru *hal_device, oal_netbuf_stru *netbuf,
    hal_sniffer_extend_info *sniffer_rx_info, mac_rx_ctl_stru *rx_ctl)
{
...
void mp16c_sniffer_rx_info_fill(hal_host_device_stru *hal_device, oal_netbuf_stru *netbuf,
    hal_sniffer_extend_info *sniffer_rx_info, mac_rx_ctl_stru *rx_ctl)
{
    ...
    if (oal_atomic_read(&hal_device->rx_mode) == HAL_DDR_RX) {
        mp16c_sniffer_rx_info_fill_host_rx(hal_device, netbuf, sniffer_rx_info, rx_ctl);
    } else if (oal_atomic_read(&hal_device->rx_mode) == HAL_RAM_RX) {
        mp16c_sniffer_rx_info_fill_device_rx(hal_device, netbuf, sniffer_rx_info, rx_ctl);
    } else {
