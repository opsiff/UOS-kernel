...
#undef THIS_FILE_ID
#define THIS_FILE_ID OAM_FILE_ID_HOST_HAL_FTM_MP16C_C
 ...
 *   修改内容  : 新生成函数
 */
static uint32_t mp16c_regs_ftm_transfer(hal_host_device_stru *hal_device, uint32_t reg_addr)
{
    uint64_t host_va = 0;
    ...
    if (oal_pcie_devca_to_hostva(HCC_EP_WIFI_DEV, reg_addr + offset, &host_va) != OAL_SUCC) {
        oam_error_log1(0, OAM_SF_ANY, "{regs_ftm_transfer::regaddr[%x] devca2hostva fail.}", reg_addr);
        return OAL_FAIL;
    }
    ...
    switch (reg_addr) {
        ...
        case PHY_GLB_REG_BANK_PHY_BW_MODE_REG:
            ...
            break;
        default:
            oam_warning_log1(0, OAM_SF_FTM, "{regs_ftm_transfer::reg addr[%x] not used.}", reg_addr);
            return OAL_FAIL;
    }
#endif
    return OAL_SUCC;
...
uint32_t mp16c_host_ftm_reg_init(uint8_t hal_dev_id)
{
    uint32_t idx;
    uint32_t hal_regs[] = {
        MAC_PPU_REG_FTM_PROCESS_REG,
        MAC_PPU_REG_FTM_BUF_BASE_ADDR_LSB_REG,
        MAC_PPU_REG_FTM_BUF_BASE_ADDR_MSB_REG,
        MAC_PPU_REG_FTM_WHITELIST_ADDR_LSB_0_REG,
        MAC_PPU_REG_FTM_WHITELIST_ADDR_MSB_0_REG,
        CALI_TEST_REG_BANK_0_FTM_CFG_REG,
        RF_CTRL_REG_BANK_0_CFG_ONLINE_CALI_USE_RF_REG_EN_REG,
        CALI_TEST_REG_BANK_0_FIFO_FORCE_EN_REG,
        MAC_PPU_REG_FTM_INFO_ADDR_LSB_REG,
        MAC_PPU_REG_FTM_INFO_ADDR_MSB_REG,
        PHY_GLB_REG_BANK_PHY_BW_MODE_REG,
    };
    ...
    if (hal_device->device_id >= HAL_DEVICE_ID_BUTT) {
        oam_error_log1(0, OAM_SF_FTM, "{host_ftm_reg_init::device_id[%d].}", hal_device->device_id);
        return OAL_FAIL;
    }
    ...
    if (!hal_device->ftm_regs.inited) {
        for (idx = 0; idx < sizeof(hal_regs) / sizeof(uint32_t); idx++) {
            if (mp16c_regs_ftm_transfer(hal_device, hal_regs[idx]) != OAL_SUCC) {
                hal_pm_try_wakeup_allow_sleep();
                return OAL_FAIL;
            }
        }
 ...
 *   修改内容  : 新生成函数
 */
static void mp16c_ftm_get_dscr_addr(uint64_t value, uint8_t *addr)
{
...
hmac_vap_stru *mp16c_host_vap_get_by_hal_vap_id(uint8_t hal_device_id, uint8_t hal_vap_id)
{
    ...
    if ((hal_vap_id == MP16C_OTHER_BSS_ID) || (hal_vap_id < MP16C_HAL_VAP_OFFSET)) {
        return NULL;
    }

    hal_vap_id -= MP16C_HAL_VAP_OFFSET;
...
uint32_t mp16c_host_ftm_get_info(hal_ftm_info_stru *hal_ftm_info, uint8_t *addr)
{
    ...
    mp16c_ftm_dscr_stru *mp16c_ftm_dscr = NULL;

    /* 获取hal_ftm_info的地址 */
    mp16c_ftm_dscr = (mp16c_ftm_dscr_stru *)addr;
    if (mp16c_ftm_dscr == NULL) {
        return OAL_FAIL;
    }
    /* 唤醒低功耗内存 */
    hal_pm_try_wakeup_forbid_sleep();
    ...
    /* 2. 存储部分结果 */
    hal_ftm_info->ftm_vap_index = mp16c_ftm_dscr->word4.bit_vap_index;
    hal_ftm_info->ftm_init_resp_flag = mp16c_ftm_dscr->word10.bit_init_resp_flag;
    hal_ftm_info->ftm_intp_time = mp16c_ftm_dscr->word10.bit_intp_time;
    hal_ftm_info->ftm_rx_time = mp16c_ftm_dscr->word9.rx_time;
    hal_ftm_info->ftm_tx_time = mp16c_ftm_dscr->word8.tx_time;
    hal_ftm_info->ftm_dialog_token = mp16c_ftm_dscr->word4.bit_dialog_token;
    hal_ftm_info->ftm_timestamp = mp16c_ftm_dscr->word7.timestamp;

    mp16c_ftm_get_dscr_addr(mp16c_ftm_dscr->word5.ra, &hal_ftm_info->ftm_ra[MAC_ADDR_0]);
    mp16c_ftm_get_dscr_addr(mp16c_ftm_dscr->word6.ta, &hal_ftm_info->ftm_ta[MAC_ADDR_0]);

    for (idx = 0; idx < HAL_HOST_MAX_RF_NUM; idx++) {
        hal_ftm_info->ftm_snr_ant[idx] = mp16c_ftm_dscr->word1.snr_ant[idx];
        hal_ftm_info->ftm_rssi_ant[idx] = mp16c_ftm_dscr->word1.rssi_hltf[idx];
    }
    hal_ftm_info->ftm_phase_incr_ant = mp16c_ftm_dscr->word4.bit_phase_incr;
    ftm_power_input = (uint32_t *)((uint8_t *)mp16c_ftm_dscr + sizeof(mp16c_ftm_dscr_stru));
...
void mp16c_host_ftm_set_sample(hal_host_device_stru *hal_device, oal_bool_enum_uint8 ftm_status)
{
    ...
    oal_setl_bits((void *)hal_device->ftm_regs.ftm_pro_reg,
                  MAC_PPU_REG_CFG_FTM_EN_OFFSET, MAC_PPU_REG_CFG_FTM_EN_LEN,
                  ftm_status);
    hal_pm_try_wakeup_allow_sleep();
}
...
void mp16c_host_ftm_set_enable(hal_host_device_stru *hal_device, oal_bool_enum_uint8 ftm_status)
{
    ...
    oal_setl_bits((void *)hal_device->ftm_regs.ftm_cfg_reg,
                  CALI_TEST_REG_BANK_0_CFG_FTM_EN_OFFSET, CALI_TEST_REG_BANK_0_CFG_FTM_EN_LEN,
                  ftm_status);

    /* 置位 PHY FTM cali enable位 mp16c/61 设置为0，见问题单号 */
    oal_setl_bits((void *)hal_device->ftm_regs.ftm_cali_reg,
                  RF_CTRL_REG_BANK_0_CFG_FTM_CALI_WR_RF_REG_EN_OFFSET,
                  RF_CTRL_REG_BANK_0_CFG_FTM_CALI_WR_RF_REG_EN_LEN,
                  0);
    hal_pm_try_wakeup_allow_sleep();
}
    ...
    uint8_t tx_rssi_selection)
{
    ...
    oal_setl_bits((void *)hal_device->ftm_regs.ftm_cfg_reg,
                  CALI_TEST_REG_BANK_0_CFG_FTM_CALI_CH_SEL_OFFSET,
                  CALI_TEST_REG_BANK_0_CFG_FTM_CALI_CH_SEL_LEN,
                  tx_selection);
    oal_setl_bits((void *)hal_device->ftm_regs.fifo_force_reg,
                  CALI_TEST_REG_BANK_0_CFG_REG_TX_FIFO_EN_OFFSET,
                  CALI_TEST_REG_BANK_0_CFG_REG_TX_FIFO_EN_LEN, BIT(tx_selection));
    hal_pm_try_wakeup_allow_sleep();
    oam_warning_log1(0, OAM_SF_ANY,
                     "host_ftm_set_m2s_phy:: tx_selection=[%d]", tx_selection);
}
...
void mp16c_host_ftm_set_buf_size(hal_host_device_stru *hal_device, uint16_t cfg_ftm_buf_size)
{
    ...
    /* 设置FTM采样上报片外循环BUF深度 */
    oal_setl_bits((void *)hal_device->ftm_regs.ftm_pro_reg,
                  MAC_PPU_REG_CFG_FTM_BUF_SIZE_OFFSET,
                  MAC_PPU_REG_CFG_FTM_BUF_SIZE_LEN, cfg_ftm_buf_size);
    hal_pm_try_wakeup_allow_sleep();
}
...
static void mp16c_host_ftm_copy_device_stru_to_host_stru(hal_ftm_info_device_stru *hal_ftm_info_device,
    hal_ftm_info_stru *hal_ftm_info)
{
    ...
    for (i = 0; i < HAL_HOST_MP16C_RF_NUM; i++) {
...
void mp16c_host_ftm_transfer_device_stru_to_host_stru(uint8_t *device_param, uint8_t *host_param)
{
    ...
    mp16c_host_ftm_copy_device_stru_to_host_stru(hal_ftm_info_device, hal_ftm_info);
}
...
void mp16c_host_ftm_get_rssi_selection(uint8_t *hmac_ftm, uint8_t *device_param)
{
    ...
    for (chan_idx = 0; chan_idx < HAL_HOST_MP16C_RF_NUM; ++chan_idx) {
