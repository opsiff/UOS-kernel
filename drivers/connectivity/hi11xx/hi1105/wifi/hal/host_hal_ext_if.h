...
static inline uint32_t hal_host_tx_complete_process(hal_host_device_stru *hal_device)
{
    if (g_hal_common_ops->hmac_tx_complete_process != NULL) {
        g_hal_common_ops->hmac_tx_complete_process(hal_device);
    }
    return OAL_TRUE;
}
