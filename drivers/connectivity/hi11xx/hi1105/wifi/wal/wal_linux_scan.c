...
OAL_STATIC oal_bool_enum_uint8 wal_check_bss_is_include_wfd(mac_bss_dscr_stru *bss_dscr, uint8_t vap_id)
{
    ...
    if ((p2p_ie != NULL) && (p2p_ie[1] > MAC_P2P_MIN_IE_LEN) && (bss_dscr->en_new_scan_bss == OAL_FALSE)) {
        oam_warning_log3(vap_id, OAM_SF_SCAN,
            "wal_inform_all_bss::[%02X:XX:XX:XX:%02X:%02X] include WFD ie, is an old scan result in 5s",
            bss_dscr->auc_bssid[MAC_ADDR_0], bss_dscr->auc_bssid[MAC_ADDR_4], bss_dscr->auc_bssid[MAC_ADDR_5]);
        return OAL_TRUE;
    }
    return OAL_FALSE;
}
