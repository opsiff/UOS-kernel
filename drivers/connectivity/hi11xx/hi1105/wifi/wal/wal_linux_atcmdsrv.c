...
OAL_STATIC void wal_get_mp16_elna_gain_status(oal_net_device_stru *net_dev, int32_t *elna_gain_status)
{
    ...
    /* 判断BT使用的2g c0的ELNA是否异常 */
    if ((*elna_gain_status & 0x1) && (hwifi_get_elna_protect_flag())) {
        cali_data->common_cali_data.rc_r_c_cali_data[WLAN_BAND_2G][WLAN_SINGLE_STREAM_0].elna_abnormal = OAL_TRUE;
    } else {
        cali_data->common_cali_data.rc_r_c_cali_data[WLAN_BAND_2G][WLAN_SINGLE_STREAM_0].elna_abnormal = OAL_FALSE;
    }
#endif
    return;
}
