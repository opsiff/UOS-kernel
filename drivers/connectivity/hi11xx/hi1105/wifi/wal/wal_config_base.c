...
OAL_STATIC int32_t wal_report_data2sdt(mac_vap_stru *mac_vap, wal_msg_request_stru *st_msg_request)
{
    ...
    msg_len = (msg_len > SDT_NETBUF_MAX_PAYLOAD) ? SDT_NETBUF_MAX_PAYLOAD : msg_len;
...
static void wal_msg_req_resp_mem_free(wal_msg_request_stru *msg_request)
{
...
int32_t wal_recv_memory_cmd(uint8_t *puc_buf, uint16_t us_len)
{
    ...
    if (pst_mem_frame->uc_mode == MAC_SDT_MODE_READ) {
        ...
        ret = memcpy_s(oal_netbuf_data(pst_netbuf), us_mem_len, (void *)mem_addr, us_mem_len);
        if (ret != EOK) {
            oam_error_log0(0, OAM_SF_ANY, "wal_recv_memory_cmd:pst_netbuf memcpy fail!");
        ...
        ret = memcpy_s((void *)mem_addr, us_mem_len, pst_mem_frame->auc_data, us_mem_len);
        if (ret != EOK) {
            oam_error_log0(0, OAM_SF_ANY, "wal_recv_memory_cmd:mem_addr memcpy fail!");
            return OAL_FAIL;
        }
    }
...
int32_t wal_parse_global_var_cmd(wal_sdt_global_var_stru *pst_global_frame, unsigned long global_var_addr)
{
    ...
    if (pst_global_frame->uc_mode == MAC_SDT_MODE_WRITE) {
        ...
        us_skb_len = (us_skb_len > SDT_NETBUF_MAX_PAYLOAD) ? SDT_NETBUF_MAX_PAYLOAD : us_skb_len;
    ...
    if (reg_frame->uc_mode == MAC_SDT_MODE_READ || reg_frame->uc_mode == MAC_SDT_MODE_READ16) {
        ...
        if (oal_unlikely(g_oam_sdt_func_hook.p_sdt_report_data_func != NULL)) {
            ...
            if (memcpy_s(oal_netbuf_data(net_buf), (uint16_t)sizeof(wal_sdt_reg_frame_stru), (uint8_t *)reg_frame,
                ...
                oal_mem_sdt_netbuf_free(net_buf, OAL_TRUE);
                return OAL_FAIL;
            }
...
static uint32_t wal_report2sdt(frw_event_mem_stru *event_mem, uint8_t oam_type)
{
    ...
    if (event_mem == NULL) {
        oam_error_log0(0, OAM_SF_ANY, "{wal_report2sdt::event_mem null.}");

        return OAL_ERR_CODE_PTR_NULL;
    }

    event = frw_get_event_stru(event_mem);
    payload_len = event->st_event_hdr.us_length - FRW_EVENT_HDR_LEN;

    netbuf = oam_alloc_data2sdt(payload_len);
    if (netbuf == NULL) {
        return OAL_ERR_CODE_PTR_NULL;
    }

    if (memcpy_s(oal_netbuf_data(netbuf), payload_len, event->auc_event_data, payload_len) != EOK) {
        oam_error_log0(0, OAM_SF_ANY, "wal_sample_report2sdt::memcpy fail!");
        oal_mem_sdt_netbuf_free(netbuf, OAL_TRUE);
        return OAL_FAIL;
    }

    oam_report_data2sdt(netbuf, oam_type, OAM_PRIMID_TYPE_DEV_ACK);
 ...
 *   修改内容  : 新生成函数
 */
uint32_t wal_sample_report2sdt(frw_event_mem_stru *event_mem)
{
    return wal_report2sdt(event_mem, OAM_DATA_TYPE_SAMPLE);
}
#endif
/*
 * 函 数 名  : wal_sample_report2sdt
 ...
 *   修改内容  : 新生成函数
 */
uint32_t wal_dpd_report2sdt(frw_event_mem_stru *event_mem)
{
    return wal_report2sdt(event_mem, OAM_DATA_TYPE_DPD);
}
/*
...
oal_bool_enum_uint8 wal_should_down_protocol_bw(mac_device_stru *mac_dev, mac_vap_stru *mac_vap)
{
    ...
    /* 存在up chba vap时，其他vap入网优先支持降协议。 chba vap入网时，优先支持降协议。 */
    if (mac_vap_is_find_chba_vap(mac_dev) == OAL_TRUE || mac_is_chba_mode(mac_vap) == OAL_TRUE) {
        return OAL_TRUE;
    }
#endif
