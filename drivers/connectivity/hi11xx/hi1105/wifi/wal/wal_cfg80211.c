                                   ...
                                   uint8_t locally_generated)
{
    ...
    /* sta上层发起去关联时locally_generated置1，默认置0表明是被对端踢 */
    cfg80211_disconnected(pst_net_device, us_reason, puc_ie, ul_ie_len, locally_generated, en_gfp);
...
void oal_cfg80211_port_authorized(oal_net_device_stru *net_device, const uint8_t *bssid, oal_gfp_enum_uint8 gfp)
{
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 10, 0))
    cfg80211_port_authorized(net_device, bssid, gfp);
#endif
}
/*
 ...
 * 功能描述  : 上报linux 内核wifi qoe结果
 */
void oal_cfg80211_wifi_event_report(oal_net_device_stru *pst_netdev,
                                    oal_gfp_enum_uint8 en_gfp, uint8_t *puc_buf, uint32_t ul_len)
{
#ifdef CONFIG_HW_WIFI_EVENT
    /* 此接口为终端实现的内核接口，定义处用内核宏CONFIG_HW_WIFI_EVENT包裹 */
    cfg80211_drv_event_result(pst_netdev, en_gfp, puc_buf, ul_len);
#endif
}
