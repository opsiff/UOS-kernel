...
#ifdef WIN32
#define DECLARE_PACKED_ROM
#else
#define DECLARE_PACKED_ROM __attribute__((__packed__))
#endif
...
#define HIMIT_TX_FAIL_FLAG 0xFF /* 发送完成判断：himit发送失败标志 */
#define HIMIT_USER_TX_FAIL_MAX 10 /* user最大发送失败次数 */
#define HIMIT_MSG_TX_FAIL_MAX 3 /* user最大发送失败次数 */

/* Himit desc total size is 36 bytes, keep same with hi1152/mp15 */
typedef struct {
...
} DECLARE_PACKED_ROM hiex_tx_himit_dscr_stru;
...
} DECLARE_PACKED_ROM hiex_tx_himit_v2_dscr_stru;
...
typedef enum mac_hiex_msg_type {
    ...
    HIEX_MSG_TYPE_VAP_STATE = 9,        /* device's up vap info */
    HIEX_MSG_TYPE_4KQAM_CAP = 10,       /* 4KQAM capibility */
...
typedef struct mac_hiex_msg {
    ...
    uint8_t                     data[0];
} DECLARE_PACKED_ROM mac_hiex_msg_stru;
...
} DECLARE_PACKED_ROM mac_hiex_tst_stru;

typedef struct mac_hiex_iperf {
    uint32_t  seq;
    int32_t   tv_sec;
    int32_t   tv_usec;
} DECLARE_PACKED_ROM mac_hiex_iperf_stru;
...
typedef struct mac_hiex_rtt {
    ...
    int32_t           offset;
    int32_t           owd;
} DECLARE_PACKED_ROM mac_hiex_rtt_stru;
...
enum {
    ...
    MAC_HIMIT_VERSION_BUTT,
};

enum {
    MAC_HIEX_CHIP_TYPE_HI1152 = 0,
    MAC_HIEX_CHIP_TYPE_MP15,
};
...
} DECLARE_PACKED_ROM mac_hiex_chs_report_msg_stru;
...
typedef struct mac_hiex_edca_param {
    ...
    uint16_t  cw_max        : 4;
    uint16_t  cw_min        : 4;
}DECLARE_PACKED_ROM mac_hiex_edca_param_stru;

typedef struct mac_hiex_edca_ctrl_msg {
    uint32_t  channel        : 8;
    uint32_t  bandwidth      : 4;
    uint32_t  enabled        : 1; /* mp15 edca ctrled */
    uint32_t  resv0          : 19;
    mac_hiex_edca_param_stru ac[WLAN_WME_AC_BUTT];
} DECLARE_PACKED_ROM mac_hiex_edca_ctrl_msg_stru;

typedef struct mac_hiex_cca_ctrl_msg {
    uint32_t  channel       : 8;
    uint32_t  bandwidth     : 4;
    uint32_t  enabled       : 1; /* mp15 cca ctrled */
    uint32_t  resv0         : 3;
...
} DECLARE_PACKED_ROM mac_hiex_cca_ctrl_msg_stru;
...
} DECLARE_PACKED_ROM mac_hiex_ac_delay_report_msg_stru;

typedef struct {
    uint8_t   ac;
} DECLARE_PACKED_ROM mac_hiex_ac_delay_request_msg_stru;

typedef struct {
    uint16_t  channel       : 8;
    uint16_t  bandwidth     : 4;
    uint16_t  resv0         : 4;
} DECLARE_PACKED_ROM mac_hiex_chs_request_msg_stru;
...
typedef struct {
    ...
    uint16_t                          user_idx;
    uint8_t                           dat[];
} DECLARE_PACKED_ROM mac_hiex_local_msg_stru; /* hd_event */
...
typedef struct {
    uint8_t chan_number;       /* 主20MHz信道号 */

    uint8_t band       : 1;    /* 频段 wlan_channel_band_enum */
    uint8_t ext6g_band : 1;
    uint8_t bandwidth  : 6;    /* 带宽模式 wlan_channel_bandwidth_enum */

    uint8_t vap_mode   : 3;    /* vap模式 wlan_vap_mode_enum_uint8 */
    uint8_t p2p_mode   : 2;    /* 0:非P2P设备; 1:P2P_GO; 2:P2P_Device; 3:P2P_CL wlan_p2p_mode_enum_uint8 */
    uint8_t is_chba    : 1;    /* 是否是chba mode */
    uint8_t resv0      : 2;

    uint8_t chan_idx;
    uint8_t user_num;          /* VAP下已挂接的用户个数 */
} dmac_himit_vap_info_msg_stru;
typedef struct {
    uint8_t up_vap_num;        /* 工作vap数 WLAN_SERVICE_VAP_MAX_NUM_PER_DEVICE = 4 */
    uint8_t bt_on_state : 1;
    uint8_t resv0       : 7;
} dmac_himit_upvap_header_stru;
typedef struct {
    dmac_himit_upvap_header_stru header;
    dmac_himit_vap_info_msg_stru vap_info[WLAN_SERVICE_VAP_MAX_NUM_PER_DEVICE];
} dmac_himit_upvap_rpt_stru;

typedef struct {
    uint8_t enable;
} dmac_himit_4kqam_cap_stru;
#endif
#endif
