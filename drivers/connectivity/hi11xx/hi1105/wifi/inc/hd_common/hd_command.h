 ...
 * 功能说明   : 定义HOST/DEVICE传递命令枚举/结构体 的头文件
 * 作者       :
 * 创建日期   : 2020年10月23日
 */
...
typedef enum {
    ...
    PSD_CFG = 21,           /* PSD初始化配置 */
    PSD_ENABLE = 22,        /* 使能一次PSD周期 */
    PSD_DEBUG = 23,
#endif
#ifdef _PRE_WLAN_FEATURE_MCAST_AMPDU
    MCAST_AMPDU_RETRY = 24, /* 组播聚合模式下配置组播帧重传次数 */
    MCAST_AMPDU_PACK_SWITCH = 25, /* CHBA可靠组播接收端PACK回馈机制开关 */
#endif
...
typedef enum {
    ...
    MAC_VAP_11AX_DEBUG_PRINT_QOSNULL,
...
} mac_vip_data_notify_stru; /* hd_event */
/* 芯片验证，控制帧/管理帧类型 */
typedef enum {
...
typedef struct {
    uint16_t aggr_txtime; /* 控制聚合个数开关 */
    uint16_t aggr_num;        /* 聚合个数 */
} mac_cfg_aggr_stru; /* hd_event */
typedef struct {
...
typedef struct {
    uint8_t ps_max_dtim_multi;              /* dtim周期倍数 */
    uint32_t ps_dtim_switch_time_threshold; /* 调整dtim周期的时间门限，比较收发和tbtt时间 */
} mac_cfg_auto_dtim_period; /* hd_event */

typedef struct {
    uint8_t rssi_type;              /* rssi门限类型 0:extlna bypass off门限 1:extlna bypass on门限 */
    int8_t rssi_val;                /* rssi门限值 */
} mac_cfg_extlna_bypass_rssi; /* hd_event */
