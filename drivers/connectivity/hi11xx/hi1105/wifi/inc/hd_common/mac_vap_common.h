...
typedef struct {
    uint16_t uplink_delay[NUM_4_BYTES]; /* BE/BK/VI/VO上行时延 */
    uint16_t tx_time[NUM_4_BYTES];      /* BE/BK/VI/VO上一帧空口发送时间 */
    int8_t beacon_rssi[NUM_10_BYTES];   /* 最近1s内收到beacon的RSSI */
    uint8_t last_beacon_idx;            /* 最新beacon RSSI */
    uint8_t resv0;
    uint16_t chload_self;               /* 收发业务占空比 */
    uint16_t resv1;
} station_info_extend_2_stru;
