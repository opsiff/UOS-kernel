...
#undef THIS_FILE_ID
#define THIS_FILE_ID OAM_FILE_ID_HMAC_CRYPTO_TKIP_C
...
static uint32_t rotl(uint32_t val, int32_t bits)
{
    return (val << (uint32_t)bits) | (val >> (uint32_t)(NUM_32_BITS - bits));
}

static uint32_t rotr(uint32_t val, int32_t bits)
{
    return (val >> (uint32_t)bits) | (val << (uint32_t)(NUM_32_BITS - bits));
}
...
{
    do {
        (*r) ^= rotl((*l), BIT_OFFSET_17);
        (*l) += (*r);
        (*r) ^= xswap((*l));
        (*l) += (*r);
        (*r) ^= rotl((*l), BIT_OFFSET_3);
        (*l) += (*r);
        (*r) ^= rotr((*l), BIT_OFFSET_2);
        (*l) += (*r);
    } while (0);
}
 ...
 * 作    者  : wifi
 */
OAL_STATIC int32_t hmac_tkip_expand_tailroom_inside_payload(mac_tx_ctl_stru *tx_ctrl, oal_netbuf_stru *netbuf)
{
    int32_t ret;
    ...
    if (netbuf == NULL || oal_netbuf_headroom(netbuf) < MAC_80211_QOS_HTC_4ADDR_FRAME_LEN) {
        return OAL_FAIL;
    }
...
/*
 * 功能描述  : tkip加密流程netbuf扩展headroom空间
 * 作    者  : wifi
 */
OAL_STATIC int32_t hmac_crypto_tkip_enmic_expand_tail(oal_netbuf_stru *netbuf)
{
    ...
    if (mac_get_cb_frame_header_addr(tx_ctrl) == NULL || mac_get_cb_80211_mac_head_type(tx_ctrl) == 0) {
        ...
        return hmac_tkip_expand_tailroom_inside_payload(tx_ctrl, netbuf);
    }
}
/*
...
uint32_t hmac_crypto_tkip_enmic(wlan_priv_key_param_stru *pst_key, oal_netbuf_stru *pst_netbuf)
{
    ...
    if (oal_unlikely(tailroom < IEEE80211_WEP_MICLEN)) {
        ...
        if (hmac_crypto_tkip_enmic_expand_tail(pst_netbuf) != OAL_SUCC) {
    ...
    /* 4.1 拷贝mic到帧尾部 */
    ret = (uint32_t)memcpy_s(puc_mic_tail, tailroom, auc_mic, IEEE80211_WEP_MICLEN);
    if (ret != EOK) {
