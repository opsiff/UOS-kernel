...
uint32_t hmac_config_twt_teardown_req(mac_vap_stru *mac_vap, uint16_t us_len, uint8_t *puc_param)
{
    ...
    if (hmac_vap->st_twt_cfg[0].uc_twt_session_enable == 0) {
        oam_warning_log3(mac_vap->uc_vap_id, OAM_SF_11AX,
            "{hmac_config_twt_teardown_req::the twt session of the ap[%02X:XX:XX:XX:%02X:%02X] hasn't seted up yet!.}",
            twt_teardown_req->auc_mac_addr[MAC_ADDR_0],
