...
static uint32_t hmac_check_md_id(mac_vap_stru *mac_vap, mac_cfg80211_ft_ies_stru *mac_ft_ies)
{
    uint16_t md_id = 0;
    uint32_t ret;
#ifdef _PRE_WLAN_FEATURE_PWL
    ...
    if (mac_vap->pwl_enable == OAL_TRUE) {
        if (hmac_pwl_check_app_11r_cap(mac_ft_ies->auc_ie, mac_ft_ies->us_len) == OAL_TRUE) {
            return OAL_SUCC;
        }
        return OAL_FAIL;
    }
#endif
    ret = mac_mib_get_md_id(mac_vap, &md_id);
    if (ret != OAL_SUCC) {
        oam_error_log1(mac_vap->uc_vap_id, OAM_SF_CFG, "{hmac_config_set_ft_ies::get_md_id fail[%d].}", ret);
        return ret;
    }
    if (md_id != mac_ft_ies->us_mdid) {
        oam_error_log2(mac_vap->uc_vap_id, OAM_SF_CFG, "{hmac_config_set_ft_ies::UNEXPECTED mdid[%d/%d].}",
            mac_ft_ies->us_mdid, md_id);
        return OAL_FAIL;
    }
    return OAL_SUCC;
}
...
uint32_t hmac_config_set_ft_ies(mac_vap_stru *pst_mac_vap, uint16_t us_len, uint8_t *puc_param)
{
    ...
    if (hmac_check_md_id(pst_mac_vap, pst_mac_ft_ies) != OAL_SUCC) {
        return OAL_FAIL;
    }
...
void hmac_config_11r_cap(hmac_vap_stru *hmac_vap, mac_conn_security_stru *conn_sec, mac_bss_dscr_stru *bss_dscr)
{
    ...
    if (hmac_vap->bit_11r_enable == OAL_TRUE) {
        ...
        if (mde != NULL) {
            ret = memcpy_s(conn_sec->auc_mde, sizeof(conn_sec->auc_mde), mde, mde[1] + MAC_IE_HDR_LEN);
            if (ret != EOK) {
                oam_warning_log1(0, OAM_SF_WPA, "hmac_config_11r_cap::mde memcpy fail! ret[%d]", ret);
            }
        }
        mac_mib_init_ft_cfg(&hmac_vap->st_vap_base_info, conn_sec->auc_mde);
    }
#ifdef _PRE_WLAN_FEATURE_PWL
    /* 配置pwl的kvr相关mib和auth_alg */
    hmac_pwl_config_11r_cap(hmac_vap, bss_dscr);
#endif
}
    ...
    hmac_vap_stru *hmac_vap, uint8_t *auth_payload, uint16_t payload_len)
{
    ...
    uint16_t pwl_auth_alg;
    uint16_t auth_ie_offset = MAC_AUTH_ALG_LEN + MAC_AUTH_SEQ_LEN + MAC_AUTH_STATUS_LEN;

    mac_vap = &(hmac_vap->st_vap_base_info);
    if (mac_vap->pwl_enable != OAL_TRUE) {
        return (mac_get_auth_alg(auth_payload, payload_len) == WLAN_WITP_AUTH_FT ? OAL_TRUE : OAL_FALSE);
    }
    if (mac_get_auth_alg(auth_payload, payload_len) != WLAN_PWL_AUTH_FT) {
        return OAL_FALSE;
    }
    if (payload_len <= auth_ie_offset) {
        oam_error_log1(0, 0, "hmac_roam_check_auth_rx_support_11r:payload_len(%u) is too short!", payload_len);
        return OAL_FALSE;
    }
    pwl_auth_alg = hmac_pwl_get_pwl_auth_alg(auth_payload + auth_ie_offset, payload_len - auth_ie_offset);
    return (pwl_auth_alg == PWL_AUTH_ALG_FT);
#else
    return (mac_get_auth_alg(auth_payload, payload_len) == WLAN_WITP_AUTH_FT);
#endif
}
...
/*
 * 函 数 名  : mac_ie_get_rsn_akm_tail_pos
 * 功能描述  : 从rsnie中获取akm suite字段的尾部地址
 */
...
int32_t hmac_akm_suite_to_rsn_ie(uint8_t *rsn_ie, uint8_t *cowork_ie, uint8_t *md_ie,
    oal_sta_ap_cowork_ie_beacon *cowork_ie_tmp,
    uint32_t rsn_ie_len, uint32_t rsn_tail_len, uint16_t *frame_len, uint8_t ext_len)
{
    uint8_t *data_bak = NULL;
    uint16_t add_len = 0;
...
/*
 * 功能描述  : Adaptive 11r支持DFS环境下被动扫描, 对收到的beacon帧进行处理
 */
int32_t hmac_scan_attach_akm_suite_to_rsn_ie(uint8_t *mgmt_frame, uint16_t *frame_len, uint8_t ext_len)
{
    ...
    /* 这里需要先查找md_ie,因为后面会对帧内容做修改,后面再查找会导致存在的md_ie找不到 */
    ...
    return hmac_akm_suite_to_rsn_ie(rsn_ie, cowork_ie, md_ie, &cowork_ie_tmp,
        rsn_ie_len, rsn_tail_len, frame_len, ext_len);
}
...
/* 功能描述: RSN_IE 中akm 是否支持增加快速漫游
 * 如果当前RSN_IE 中akm已经是快速漫游，则不支持增加新的快速漫游选项。
 * 返回值:OAL_TRUE:支持添加新的快速漫游值；OAL_FALSE:不支持添加新的快速漫游
 */
...
oal_bool_enum_uint8 hmac_scan_check_ft_is_support(uint8_t *cowork_ie_param, uint8_t rsn_akm_value)
{
    uint32_t idx;
    ...
    /* cowork ie中携带的akm套件（PSK）和rsn ie中携带akm 套件(SAE)不同，扩展rsn_ie内容后，会导致关联ap 失败。
     * 修改方案：检查cowork ie中携带的akm套件和RSN ie中携带的akm套件，仅相同才扩展rsn ie
     */
    if ((cowork_ie->capa_bitmap.akm_suite_val & 0xff) != rsn_akm_value) {
        return OAL_FALSE;
    }
...
/*
 * 函 数 名  : hmac_scan_extend_mgmt_len_needed
 * 功能描述  : 检查是否需要添加在RSN IE的AKM Suite List之后附加一个对应的支持Fast Transition的AKM Suite
 * 1.日    期  : 2019年4月8日
 *   作    者  : wifi
 *   修改内容  : 新生成函数
 */
uint8_t hmac_scan_extend_mgmt_len_needed(oal_netbuf_stru *netbuf, uint16_t netbuf_len)
{
    ...
    if (hmac_scan_check_ft_is_support(cowork_ie, rsn_ie[rsn_ie_akm_offset - 1]) == OAL_TRUE) {
