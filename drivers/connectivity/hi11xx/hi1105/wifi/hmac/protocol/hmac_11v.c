    ...
    uint8_t *channel, uint8_t *bssid)
{
    ...
    if (check != OAL_SUCC && bss_dscr == NULL) {
        ...
        oam_warning_log4(0, OAM_SF_CFG,
            "{hmac_rx_bsst_req_candidate_info_check::chan[%d] invalid, bssid:%02X:XX:XX:XX:%02X:%02X not in scan list}",
            candidate_channel, bssid[MAC_ADDR_0], bssid[MAC_ADDR_4], bssid[MAC_ADDR_5]);
        return OAL_FAIL;
    }
    /* 有效 */
    if (bss_dscr != NULL && candidate_channel != bss_dscr->st_channel.uc_chan_number) {
        ...
        *channel = bss_dscr->st_channel.uc_chan_number;
        oam_warning_log3(0, OAM_SF_CFG,
            "{hmac_rx_bsst_req_candidate_info_check::bssid:%02X:XX:XX:XX:%02X:%02X]}",
            bssid[MAC_ADDR_0], bssid[MAC_ADDR_4], bssid[MAC_ADDR_5]);
        oam_warning_log2(0, OAM_SF_CFG,
            "{hmac_rx_bsst_req_candidate_info_check::bssinfo channel[%d],not[%d]}",
    ...
    if (mac_is_dual_sta_mode() &&
        ...
        oam_warning_log4(0, OAM_SF_CFG,
            "{hmac_rx_bsst_req_candidate_info_check::dual sta mode, ignore diff band[%d] bss[%02X:XX:XX:XX:%02X:%02X]}",
            channel_band, bssid[MAC_ADDR_0], bssid[MAC_ADDR_4], bssid[MAC_ADDR_5]); /* 打印bssid 4 5两个字节 */
        return OAL_FAIL;
    }
...
uint32_t hmac_rx_bsst_req_action(hmac_vap_stru *hmac_vap, hmac_user_stru *hmac_user, oal_netbuf_stru *netbuf)
{
    ...
    oam_warning_log4(0, OAM_SF_ANY,
        "{hmac_rx_bsst_req_action::associated user mac address=%02X:XX:XX:XX:%02X:%02X bss_list_num=%d}",
