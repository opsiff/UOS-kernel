...
void hmac_scan_mbssid_printf(mac_multi_bssid_frame_info_stru *pst_mbssid)
{
    ...
    oam_error_log3(0, OAM_SF_UM, "{>>>>>>>::Transmitted BSSID:%02X:XX:XX:XX:%02X:%02X.}",
                   pst_mbssid->auc_transmitted_bssid[0],
                   pst_mbssid->auc_transmitted_bssid[MAC_ADDR_4],
                   pst_mbssid->auc_transmitted_bssid[MAC_ADDR_5]);
    oam_error_log3(0, OAM_SF_UM, "{>>>>>>>::Non_Transmitted BSSID:%02X:XX:XX:XX:%02X:%02X.}",
...
{
    /* 2g时，对5g的带宽不进行置位，只根据是否支持2.4g 40M对Bit0进行置位 */
    if (pst_mac_vap->st_channel.en_band == WLAN_BAND_2G) {
