...
OAL_STATIC uint32_t hmac_roam_start_proc(uint8_t vap_id, roam_channel_org_enum scan_type,
    hmac_roam_info_stru *roam_info)
{
    uint32_t ret = OAL_SUCC;
#ifdef CONFIG_HW_WIFI_EVENT
    int32_t event = DRV_EVENT_ROAM_TRIGGER;
#endif
    /* 触发漫游是否搭配扫描0表示不扫描 */
...
#ifdef CONFIG_HW_WIFI_EVENT
    if (roam_info != NULL && roam_info->pst_hmac_vap != NULL && roam_info->pst_hmac_vap->pst_net_device != NULL) {
                         ...
                         roam_trigger_enum roam_trigger)
{
    ...
    return hmac_roam_start_proc(hmac_vap->st_vap_base_info.uc_vap_id, scan_type, roam_info);
}
...
OAL_STATIC uint32_t hmac_roam_connecting_fail(hmac_roam_info_stru *roam_info, void *param)
{
    ...
    /* 给当前old bss发去关联帧，此前回old bss可能出现业务不通的问题；
     * 断连之后亮屏状态下可以立即发起会连，暗屏时pno也可发起回连，pno超时未启动扫描只要亮屏即可发起回连，无感知
     */
    ...
    /* 11r auth失败删去关联无需告知wpa_s，直接发起重新关联 */
    if (roam_info->pst_hmac_vap->bit_11r_auth_fail == OAL_TRUE) {
        roam_info->pst_hmac_vap->bit_11r_auth_fail = OAL_FALSE;
        return ret;
    }
...
OAL_STATIC uint32_t hmac_roam_connect_to_bss(hmac_roam_info_stru *roam_info, void *param)
{
    ...
    hmac_config_close_lp_miracast(&hmac_vap->st_vap_base_info);
...
OAL_STATIC uint32_t hmac_roam_to_old_bss(hmac_roam_info_stru *roam_info, void *param)
{
    ...
    oam_warning_log3(hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_ROAM,
        "{hmac_roam_to_old_bss::now resuming to [%02X:XX:XX:XX:%02X:%02X]}", old_bss->auc_bssid[MAC_ADDR_0],
...
OAL_STATIC uint32_t hmac_roam_to_new_bss(hmac_roam_info_stru *roam_info, void *param)
{
    ...
    oam_warning_log3(0, OAM_SF_ROAM, "{hmac_roam_to_new_bss::roam to [%02X:XX:XX:XX:%02X:%02X] succ}",
        hmac_vap->st_vap_base_info.auc_bssid[MAC_ADDR_0],
...
uint32_t hmac_roam_resume_user(hmac_vap_stru *hmac_vap, void *param)
{
    ...
    hmac_sta_up_ap_wait_queue_wake_up_interrupt(hmac_vap);
...
#ifdef CONFIG_HW_WIFI_EVENT
    oal_cfg80211_wifi_event_report(hmac_vap->pst_net_device, GFP_KERNEL, (uint8_t *)&event, sizeof(int32_t));
#endif
    hmac_chr_roam_info_report(roam_info, HMAC_CHR_ROAM_START);
    return OAL_SUCC;
}
    ...
    for (vap_idx = 0; vap_idx < mac_device->uc_vap_num; vap_idx++) {
        ...
        if (en_vap_state == MAC_VAP_STATE_UP && (en_p2p_mode == WLAN_P2P_CL_MODE)) {
            mac_cfg_kick_user_param_stru kick_user_param = {
                ...
                .send_disassoc_immediately = OAL_TRUE,
            };
            oam_warning_log0(hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_ROAM,
