...
/*
 * 函 数 名  : wal_ioctl_get_nrcoex_stat
 * 功能描述  : 获取NR共存相关信息
 * 1.日    期  : 2019年3月4日
 *   作    者  : wifi
 *   修改内容  : 新生成函数
 */
OAL_STATIC uint32_t wal_ioctl_get_nrcoex_stat(oal_net_device_stru *pst_net_dev, int8_t *data, uint32_t len)
{
    ...
    wal_write_msg_hdr_init(&st_write_msg, WLAN_CFGID_QUERY_NRCOEX_STAT, WAL_MSG_WRITE_MSG_HDR_LENGTH);

    l_ret = wal_send_cfg_event(pst_net_dev, WAL_MSG_TYPE_WRITE, sizeof(wal_msg_write_stru),
...
/*
 * 函 数 名  : wal_vendor_get_nrcoex_info
 * 功能描述  : 获取NR共存相关信息
 * 1.日    期  : 2019年10月26日
 *   作    者  : wifi
 *   修改内容  : 降低圈复杂度
    ...
    oal_net_device_stru *pst_net_dev, wal_wifi_priv_cmd_stru *priv_cmd, int8_t *data, uint32_t len)
{
    ...
    l_ret = (int32_t)wal_ioctl_get_nrcoex_stat(pst_net_dev, data, len);
    if (l_ret != OAL_SUCC) {
        oam_warning_log1(0, OAM_SF_ANY,
            "wal_vendor_get_nrcoex_info:CMD_GET_NRCOEX_INFO Failed to get nrcoex stat ret[%d] !", l_ret);
        return -OAL_EFAIL;
    }
    if (priv_cmd->total_len < sizeof(mac_nrcoex_stat_stru)) {
        oam_error_log1(0, OAM_SF_ANY, "wal_vendor_get_nrcoex_info:total_len=%d", priv_cmd->total_len);
        return -OAL_EFAIL;
    }
    if (oal_copy_to_user(priv_cmd->puc_buf, data, sizeof(mac_nrcoex_stat_stru))) {
        oam_error_log0(0, OAM_SF_ANY,
            "wal_vendor_get_nrcoex_info:CMD_GET_NRCOEX_INFO Failed to copy ioctl_data to user !");
        return -OAL_EFAIL;
    }
    return OAL_SUCC;
}
