...
static uint8_t hmac_tid_update_sync_threshold(hmac_tid_info_stru *tid_info, void *param)
{
    ...
    for (index = 0; index < HMAC_TID_UPDATE_LVL_CNT; index++) {
         ...
         */
        oal_atomic_set(&tid_info->tx_ring.ring_sync_th, last_period_tx_msdu >> tid_update_list->ring_sync_th[index]);
        oal_atomic_set(&tid_info->tid_sche_th, tid_update_list->tid_sche_th[index]);
        oal_atomic_set(&tid_info->tx_ring.ring_tx_opt, tid_update_list->ring_tx_opt[index]);
        break;
    }
...
static uint8_t hmac_tid_update_list_for_each_rcu(hmac_tid_list_func func, void *param)
{
    hmac_tid_info_stru *tid_info = NULL;
    struct list_head *tid_list = &g_tid_update_list.tid_list.list;

    if (oal_unlikely(list_empty(tid_list))) {
        return OAL_FALSE;
    }

    list_for_each_entry_rcu(tid_info, tid_list, tid_update_entry) {
...
void hmac_tid_update_timer_deinit(void)
{
    ...
    frw_timer_immediate_destroy_timer_m(tid_update_timer);
}
