...
#ifndef HMAC_TX_DATA_MACRO_H
#define HMAC_TX_DATA_MACRO_H
...
static inline uint8_t wlan_tos_to_tid(uint8_t tos)
{
    return ((tos == 0) || (tos == 3)) ? WLAN_TIDNO_BEST_EFFORT : // 0/3: BE
           ((tos == 1) || (tos == 2)) ? WLAN_TIDNO_BACKGROUND :  // 1/2: BK
           ((tos == 4) || (tos == 5)) ? WLAN_TIDNO_VIDEO :       // 4/5: VI
           WLAN_TIDNO_VOICE;                                     // 6/7: VO
}
...
#define WLAN_TCP_ACK_BUF_THROUGHPUT_THRESHOLD_LOW       60  /* 30Mb/s */
...
#define WLAN_SMALL_AMSDU_PPS_THRESHOLD_HIGH              23000 /* pps 23000 */
...
static inline void hmac_tx_pkts_stat(uint32_t pkt)
{
    g_host_tx_pkts.snd_pkts += pkt;
}
