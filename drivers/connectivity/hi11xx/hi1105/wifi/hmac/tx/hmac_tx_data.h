...
#ifndef HMAC_TX_DATA_H
#define HMAC_TX_DATA_H
...
typedef struct {
    ...
    uint8_t rx_complete_thread_disable; /* 是否禁止接收内存补充部署在rxcomplete线程, 1:禁止 0:容许 */
    ...
    mac_tx_ctl_stru *pst_tx_ctl, mac_ieee80211_qos_htc_frame_addr4_stru *pst_hdr_addr4)
{
    ...
    if (qos == HMAC_TX_BSS_QOS) {
        ...
        if (OAL_FALSE == mac_get_cb_is_4address(pst_tx_ctl)) {
            ...
            pst_hdr->bit_qc_ack_polocy = mac_get_cb_ack_policy(pst_tx_ctl);
            ...
            pst_hdr_addr4->bit_qc_ack_polocy = mac_get_cb_ack_policy(pst_tx_ctl);
...
/*
 * 函 数 名  : hmac_tx_enabled_himit_htc
 * 功能描述  : check if enable himit htc for this tx_ctl
 * 1.日    期  : 2020年03月31日
 *   修改内容  : 新生成函数
 */
OAL_STATIC OAL_INLINE oal_bool_enum_uint8 hmac_tx_enable_himit_htc(hmac_user_stru *user, mac_tx_ctl_stru *tx_ctl)
{
    return ((mac_user_himit_enabled(&user->st_user_base_info) == OAL_TRUE) &&
            mac_get_cb_is_game_marked_frame(tx_ctl) && ((mac_get_cb_ack_policy(tx_ctl) == WLAN_TX_NORMAL_ACK) ||
            (mac_get_cb_ack_policy(tx_ctl) == WLAN_TX_BLOCK_ACK))) ? OAL_TRUE : OAL_FALSE;
}
#endif
