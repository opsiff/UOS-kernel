...
uint32_t hmac_tx_hostva_to_devva(uint8_t *hostva, uint32_t alloc_size, uint64_t *devva)
{
    ...
    if (pcie_if_hostca_to_devva(HCC_EP_WIFI_DEV, (uint64_t)pcie_dma_addr, devva) != OAL_SUCC) {
        hmac_tx_dma_unmap_addr((uint8_t *)(uintptr_t)pcie_dma_addr, alloc_size);
        return OAL_FAIL;
    }
...
OAL_STATIC void hmac_tx_reg_write_base_addr(hmac_tx_ring_device_info_stru *tx_ring_device_info,
    uintptr_t devva)
{
    hal_host_writel(get_low_32_bits(devva), (uintptr_t)tx_ring_device_info->word_addr[BYTE_OFFSET_0]);
    hal_host_writel(get_high_32_bits(devva), (uintptr_t)tx_ring_device_info->word_addr[BYTE_OFFSET_1]);
}
    ...
    uint16_t wptr)
{
    ...
    hal_host_writel(tx_ring_device_info->msdu_info_word2.word2, tx_ring_device_info->word_addr[BYTE_OFFSET_2]);
}
...
OAL_STATIC OAL_INLINE void hmac_tx_reg_write_rptr(hmac_tx_ring_device_info_stru *tx_ring_device_info, uint16_t rptr)
{
    ...
    hal_host_writel(tx_ring_device_info->msdu_info_word3.word3, tx_ring_device_info->word_addr[BYTE_OFFSET_3]);
}
    ...
    hmac_tx_ring_device_info_stru *tx_ring_device_info, hmac_msdu_info_ring_stru *tx_ring)
{
    ...
    hmac_tx_reg_write_base_addr(tx_ring_device_info, (uintptr_t)devva);
...
{
    hmac_msdu_info_ring_stru *tx_ring = &tx_tid_info->tx_ring;
    hmac_tx_ring_device_info_stru *tx_ring_device_info = tx_tid_info->tx_ring.tx_ring_device_info;
...
OAL_STATIC void hmac_tx_set_msdu_info_from_netbuf(
    mac_vap_stru *mac_vap, msdu_info_stru *msdu_info, oal_netbuf_stru *netbuf, mac_tx_ctl_stru *tx_ctl, uintptr_t devva)
{
...
uint32_t hmac_tx_set_msdu_info(mac_vap_stru *mac_vap, msdu_info_stru *msdu_info, oal_netbuf_stru *netbuf)
{
    ...
    hmac_tx_set_msdu_info_from_netbuf(mac_vap, msdu_info, netbuf, tx_ctl, (uintptr_t)devva);
...
static inline void hmac_tx_ring_push_msdu_post_proc(
    mac_vap_stru *mac_vap, hmac_msdu_info_ring_stru *tx_ring, oal_netbuf_stru *netbuf)
{
...
uint32_t hmac_tx_ring_push_msdu(mac_vap_stru *mac_vap, hmac_msdu_info_ring_stru *tx_ring, oal_netbuf_stru *netbuf)
{
    ...
    hmac_tx_ring_push_msdu_post_proc(mac_vap, tx_ring, netbuf);
...
uint32_t hmac_get_host_ring_state(frw_event_mem_stru *event_mem)
{
    ...
    if ((!hmac_host_ring_tx_enabled()) || (wlan_pm_info == NULL)) {
        return OAL_SUCC;
    }
...
static uint32_t hmac_tx_trig_device_softirq(hmac_vap_stru *hmac_vap)
{
...
{
    /* WCPU软中断调度 */
    if (hmac_tx_ring_soft_irq_sched_enabled() && hmac_tx_trig_device_softirq(hmac_vap) == OAL_SUCC) {
        return;
    }
    /* 下发事件调度 */
