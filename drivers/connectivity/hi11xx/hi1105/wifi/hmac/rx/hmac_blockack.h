...
/* 8 UNION定义 */
/* 9 OTHERS定义 */
extern void hmac_rx_free_netbuf_list(oal_netbuf_head_stru *netbuf_hdr, uint16_t uc_num_buf);
/* 10 inline函数定义 */
/*
...
#define hmac_ba_index(_st, _seq) (((_seq) - (_st)) & 0xFFF)
...
static inline void hmac_tx_buf_bitmap_set(uint32_t *bitmap, uint16_t idx)
{
    if ((idx >> HMAC_TX_BUF_BITMAP_LOG2_WORD_SIZE) >= BYTE_OFFSET_8) {
        return;
    }
    bitmap[idx >> HMAC_TX_BUF_BITMAP_LOG2_WORD_SIZE] |= (1 << (idx & HMAC_TX_BUF_BITMAP_WORD_MASK));
}

static inline void hmac_tx_buf_bitmap_clr(uint32_t *bitmap, uint16_t idx)
{
    if ((idx >> HMAC_TX_BUF_BITMAP_LOG2_WORD_SIZE) >= BYTE_OFFSET_8) {
        return;
    }
    bitmap[idx >> HMAC_TX_BUF_BITMAP_LOG2_WORD_SIZE] &= ~((uint32_t)(1 << (idx & HMAC_TX_BUF_BITMAP_WORD_MASK)));
}
...
static inline oal_bool_enum_uint8 hmac_ba_isset_m(uint32_t *bm, uint16_t n)
{
    return ((n < (HMAC_BA_BMP_SIZE)) && (bm[n >> BIT_OFFSET_5] & (1 << (n & 31)))); // BA BMP MASK 31
}

/* 判断一个seq num是否在发送窗口内 */
static inline oal_bool_enum_uint8 hmac_baw_within(uint16_t start, uint8_t bawsz, uint16_t seqno)
{
    return ((seqno >= start) ? (((seqno - start) & 0xFFF) < bawsz) : (((seqno + 0xFFF - start) & 0xFFF) < bawsz));
}

/* 判断一个seq num是否在发送窗口右边 */
static inline oal_bool_enum_uint8 hmac_baw_rightside(uint16_t start, uint16_t seqno)
{
    return (((seqno >= start) ? (((seqno - start) & 0xFFF) < (0x800)) :
            (((seqno + 0xFFF - start) & 0xFFF) < (0x800))) && (start != seqno));
}
...
OAL_STATIC OAL_INLINE void hmac_ba_clear_rx_bitmap(hmac_ba_rx_stru *pst_ba_rx_hdl)
{
    ...
    us_index = mac_ba_seqno_sub(pst_ba_rx_hdl->us_baw_head, 1) & (HMAC_BA_BMP_SIZE - 1);
                                                    ...
                                                    mac_ieee80211_frame_stru *pst_frame_hdr)
{
    ...
    while (hmac_ba_seqno_lt(pst_ba_rx_hdl->us_baw_head,
                            mac_ba_seqno_sub(pst_ba_rx_hdl->us_baw_start, (HMAC_BA_BMP_SIZE - 1)))) {
        pst_ba_rx_hdl->us_baw_head = mac_ba_seqno_add(pst_ba_rx_hdl->us_baw_head, 1);
...
OAL_STATIC OAL_INLINE uint8_t hmac_mac_get_tid_from_frame_hdr(uint8_t *frame_hdr)
{
    ...
    /* 考虑四地址情况获取报文的tid */
    is_tods = mac_hdr_get_to_ds((uint8_t *)frame_hdr);
    is_from_ds = mac_hdr_get_from_ds((uint8_t *)frame_hdr);
    is_4addr = is_tods && is_from_ds;
    tid = mac_get_tid_value((uint8_t *)frame_hdr, is_4addr);
    return tid;
}
