    ...
    uint8_t *user_mac, uint8_t event_id)
{
    ...
    if (mac_vap->en_protocol == WLAN_HT_MODE) {
        ...
        if (mac_vap->st_channel.en_band == WLAN_BAND_2G && mac_is_smc_ap(user_mac)) {
            ...
            oam_warning_log3(0, OAM_SF_SCAN, "hmac_ht_self_cure_state_init_assoc_event_handle:: \
                add ht user[%02X:XX:XX:XX:%02X:%02X]!",
                user_mac[MAC_ADDR_0], user_mac[MAC_ADDR_4], user_mac[MAC_ADDR_5]);
        }
    } else {
    ...
    switch (event_id) {
        case HMAC_HT_SELF_CURE_EVENT_TX_DHCP_FRAME:
            ...
            if (ht_self_cure->ht_check_info.tx_dhcp_times > HT_SELF_CURE_CLOSE_TX_DHCP_MAX_TIMES) {
                ...
                oam_warning_log3(0, OAM_SF_SCAN, "hmac_ht_self_cure_state_ht_txdhcp_handle:: \
                    add ht user[%02X:XX:XX:XX:%02X:%02X]!",
                    user_mac[MAC_ADDR_0], user_mac[MAC_ADDR_4], user_mac[MAC_ADDR_5]);
            }
            break;
    ...
    uint8_t *user_mac, uint8_t event_id)
{
    ...
    switch (event_id) {
        ...
        case HAMC_HT_SELF_CURE_EVENT_RX_EAPOL:
        case HMAC_HT_SELF_CURE_EVENT_RX_DHCP_FRAME:
        case HMAC_HT_SELF_CURE_EVENT_ASSOC_MISMATCH_HTCAP:
            hmac_ht_self_cure_checkinfo_init();
            break;
        default:
            break;
...
void  hmac_ht_self_cure_event_set(mac_vap_stru *mac_vap, uint8_t *user_mac, uint8_t event_id)
{
    ...
    oam_warning_log2(mac_vap->uc_vap_id, OAM_SF_SCAN, "hmac_ht_self_cure_event_set:Begin:now_state=%d event_id=%d",
        now_state, event_id);
    oam_warning_log3(mac_vap->uc_vap_id, OAM_SF_SCAN, "hmac_ht_self_cure_event_set:user:%02X:XX:XX:XX:%02X:%02X",
        user_mac[MAC_ADDR_0], user_mac[MAC_ADDR_4], user_mac[MAC_ADDR_5]);
