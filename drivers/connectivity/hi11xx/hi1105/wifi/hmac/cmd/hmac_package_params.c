...
uint32_t hmac_package_dtim_params(
    mac_vap_stru *mac_vap, uint32_t *params, uint8_t *new_params, uint16_t *new_params_len)
{
    mac_cfg_auto_dtim_period *dtim_period_ctl = NULL;
    if (hmac_package_check_stru_size(sizeof(mac_cfg_auto_dtim_period)) != OAL_SUCC) {
        return OAL_FAIL;
    }

    dtim_period_ctl = (mac_cfg_auto_dtim_period *)new_params;
    dtim_period_ctl->ps_max_dtim_multi = (uint8_t)params[0];
    dtim_period_ctl->ps_dtim_switch_time_threshold = (uint32_t)params[1];

    *new_params_len = sizeof(mac_cfg_auto_dtim_period);
    return OAL_SUCC;
}

uint32_t hmac_package_p2p_noa_params(
    mac_vap_stru *mac_vap, uint32_t *params, uint8_t *new_params, uint16_t *new_params_len)
{
    mac_cfg_p2p_noa_param_stru *p2p_noa_param = NULL;
    if (hmac_package_check_stru_size(sizeof(mac_cfg_p2p_noa_param_stru)) != OAL_SUCC) {
        return OAL_FAIL;
    }

    p2p_noa_param = (mac_cfg_p2p_noa_param_stru *)new_params;
    p2p_noa_param->start_time = (uint32_t)params[0] * HMAC_MS_TO_US; // 0: cmd param[0] = start_time
    p2p_noa_param->duration = (uint32_t)params[1] * HMAC_MS_TO_US;   // 1: cmd param[1] = duration
    p2p_noa_param->interval = (uint32_t)params[2] * HMAC_MS_TO_US;   // 2: cmd param[2] = interval
    p2p_noa_param->uc_count = (uint8_t)params[3];                    // 3: cmd param[3] = count

    *new_params_len = sizeof(mac_cfg_p2p_noa_param_stru);
    return OAL_SUCC;
}

uint32_t hmac_package_p2p_ops_params(
    mac_vap_stru *mac_vap, uint32_t *params, uint8_t *new_params, uint16_t *new_params_len)
{
    mac_cfg_p2p_ops_param_stru *p2p_ops_param = NULL;
    if (hmac_package_check_stru_size(sizeof(mac_cfg_p2p_ops_param_stru)) != OAL_SUCC) {
        return OAL_FAIL;
    }

    p2p_ops_param = (mac_cfg_p2p_ops_param_stru *)new_params;
    p2p_ops_param->en_ops_ctrl = (uint8_t)params[0];
    p2p_ops_param->uc_ct_window = (uint8_t)params[1];

    *new_params_len = sizeof(mac_cfg_p2p_ops_param_stru);
    return OAL_SUCC;
}
...
#ifdef _PRE_WLAN_CHBA_MGMT
/* 封装CHBA DBAC参数 */
uint32_t hmac_package_chba_channel_seq_params(mac_vap_stru *mac_vap, uint32_t *params,
    uint8_t *new_params, uint16_t *new_params_len)
{
    struct chba_channel_sequence_params *chba_channel_seq = NULL;
    if (hmac_package_check_stru_size(sizeof(struct chba_channel_sequence_params)) != OAL_SUCC) {
        return OAL_FAIL;
    }

    chba_channel_seq = (struct chba_channel_sequence_params *)new_params;
    chba_channel_seq->params_type = CHBA_CHANNEL_SEQ_PARAMS_TYPE_CMD;
    chba_channel_seq->chba_chan_seq_cfg_type = CHBA_CHAN_SEQ_CFG_BUTT; /* 维测命令, 无需配置 */
    chba_channel_seq->data.test_params.channel_seq_level = params[0];  // 0: cmd param[0]
    chba_channel_seq->data.test_params.first_work_channel = params[1]; // 1: cmd param[1]
    chba_channel_seq->data.test_params.sec_work_channel = params[2];   // 2: cmd param[2]

    *new_params_len = sizeof(struct chba_channel_sequence_params);
    return OAL_SUCC;
}
