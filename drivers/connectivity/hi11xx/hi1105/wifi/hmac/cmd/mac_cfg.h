...
typedef struct {
    ...
    /* 为了保证上层删用户时发送disasso帧，在wal层将kick_user事件拆分为 SEND_DISASSOC与KICK_USER 两个事件
     * wal层先下发发送SEND_DISASSOC事件，延迟一段时间，再下发KICK_USER事件
     */
    oal_bool_enum_uint8 send_disassoc_immediately; /* 是否需要在kick_user事件中立即发送disassoc帧 */
