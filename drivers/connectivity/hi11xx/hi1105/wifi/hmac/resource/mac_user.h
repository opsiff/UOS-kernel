...
/* 6G信道偏移定义 */
#define CHAN_OFFSET_8  8
#define CHAN_OFFSET_16 16
...
#define MAC_USER_HE_HDL_STRU(_user)         (&((_user)->st_he_hdl))
...
typedef struct {
    ...
    uint8_t auc_user_mac_addr[WLAN_MAC_ADDR_LEN]; /* user对应的 */
    ...
    uint32_t ampdu_maxsize;     /* 与协议交集计算后的最大AMPDU聚合长度 */
    ...
    mac_hisi_cap_vendor_ie_stru hisi_priv_cap; /* hisi私有能力 */
    ...
    mac_he_6ghz_band_cap_info_stru   he_6ghz_band_cap_ie; /* HE 6 GHz Band Capbilities */
#endif
#else
...
#ifdef _PRE_WLAN_FEATURE_HIEX
static inline mac_hiex_cap_stru *mac_user_get_hiex_cap(mac_user_stru *user)
{
    return &(user->st_hiex_cap);
}
static inline oal_bool_enum_uint8 mac_user_hiex_enabled(mac_user_stru *user)
{
    return (oal_bool_enum_uint8)(user->st_hiex_cap.bit_hiex_enable);
}
static inline oal_bool_enum_uint8 mac_user_himit_enabled(mac_user_stru *user)
{
    return (oal_bool_enum_uint8) (user->st_hiex_cap.bit_himit_enable);
}
#endif
...
static inline oal_bool_enum_uint8 mac_11i_is_ptk(oal_bool_enum_uint8 macaddr_is_zero, oal_bool_enum_uint8 pairwise)
{
    return (oal_bool_enum_uint8)((OAL_TRUE != (macaddr_is_zero)) && (OAL_TRUE == (pairwise)));
}
/*
