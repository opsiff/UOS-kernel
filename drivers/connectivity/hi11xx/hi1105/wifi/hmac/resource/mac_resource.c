...
/* 2 全局变量定义 */
OAL_STATIC wlan_mib_ieee802dot11_stru g_hmac_mib_res[WLAN_SERVICE_VAP_SUPPORT_MAX_NUM_LIMIT];
...
wlan_mib_ieee802dot11_stru *hmac_res_get_mib_info(uint8_t vap_id)
{
    if (oal_unlikely(vap_id == 0 || vap_id >= WLAN_VAP_SUPPORT_MAX_NUM_LIMIT)) {
        return NULL;
    }

    return &g_hmac_mib_res[vap_id - 1];
}
...
mac_chip_stru *mac_res_get_mac_chip(uint32_t chip_idx)
{
    ...
    return &(g_st_mac_board.ast_chip[chip_idx]);
}
