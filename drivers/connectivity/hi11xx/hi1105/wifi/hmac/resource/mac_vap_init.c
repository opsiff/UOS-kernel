 ...
 *   修改内容  : 新生成函数
 */
uint32_t mac_vap_init_privacy(mac_vap_stru *pst_mac_vap, mac_conn_security_stru *conn_sec)
{
    ...
    ret = mac_vap_init_pmf(pst_mac_vap, conn_sec->en_pmf_cap, conn_sec->en_mgmt_proteced);
    if (ret != OAL_SUCC) {
...
#endif
    /* 不加密 */
    if (conn_sec->en_privacy == OAL_FALSE) {
        return OAL_SUCC;
    }

    /* WEP加密 */
    if (conn_sec->uc_wep_key_len != 0) {
        return mac_vap_add_wep_key(pst_mac_vap, conn_sec->uc_wep_key_index,
                                   conn_sec->uc_wep_key_len, conn_sec->auc_wep_key);
    }

    /* WPA/WPA2加密 */
    pst_crypto = &(conn_sec->st_crypto);
