...
/* 初始化Dot11 HT工作站配置条目 */
OAL_STATIC void mac_vap_init_dot11_ht_station_config_entry(mac_vap_stru *pst_mac_vap)
{
...
void mac_vap_init_mib_11n(mac_vap_stru *pst_mac_vap)
{
    ...
    /* 初始化私有能力信息 */
    mac_vap_init_dot11_ht_station_config_entry(pst_mac_vap);
...
void mac_vap_init_mib_11ac(mac_vap_stru *pst_mac_vap)
{
    ...
    mac_mib_set_maxmpdu_length(pst_mac_vap, g_wlan_spec_cfg->vht_max_mpdu_lenth);
    ...
    mac_mib_set_VHTBeamformeeNTxSupport(pst_mac_vap, g_wlan_spec_cfg->vht_bfee_ntx_supports);
#else
...
OAL_STATIC void mac_init_mib_dot11_station_config_entry(mac_vap_stru *pst_mac_vap, mac_device_stru *pst_mac_dev)
{
    /* 公共特性mib值初始化 */
    ...
    /*
     * 1151默认不amsdu ampdu 联合聚合功能不开启 mp12用于小包优化
...
OAL_STATIC void mac_init_mib_dot11_operation_entry(mac_vap_stru *pst_mac_vap)
{
...
OAL_STATIC void mac_init_mib_dot11_phy_dsss_entry(mac_vap_stru *pst_mac_vap)
{
...
void mac_init_mib(mac_vap_stru *pst_mac_vap)
{
    ...
    mac_init_mib_dot11_station_config_entry(pst_mac_vap, pst_mac_dev);
    mac_init_mib_dot11_phy_dsss_entry(pst_mac_vap);

    /* 不同芯片，auth超时时间配置不同值 */
    wlan_chip_mac_mib_set_auth_rsp_time_out(pst_mac_vap);

    mac_init_mib_dot11_operation_entry(pst_mac_vap);
 ...
 *   修改内容  : 新生成函数
 * 2.日    期  : 2016年07月20日
 *   修改内容  : 增加band参数，因为初始设置时，vap尚未设置band信息
 */
