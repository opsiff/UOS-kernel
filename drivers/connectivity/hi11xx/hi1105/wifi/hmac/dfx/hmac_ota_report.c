...
uint32_t hmac_report_80211_get_common_switch(mac_vap_stru *mac_vap, mac_ieee80211_frame_stru *frame_hdr,
    oam_80211_frame_ctx_union *oam_frame_report_un, uint8_t frame_type)
{
    uint16_t                   user_idx = 0xffff;
    uint8_t                   *da_addr = NULL;
    uint32_t                   ret;

    if (frame_hdr == NULL) {
        oam_error_log0(mac_vap->uc_vap_id, OAM_SF_ANY, "{mac_report_80211_get_switch::frame_hdr null.}");

        return OAL_ERR_CODE_PTR_NULL;
    }

    mac_rx_get_da(frame_hdr, &da_addr);

    if (ether_is_multicast(da_addr)) {
        ret = oam_report_80211_mcast_get_switch(OAM_OTA_FRAME_DIRECTION_TYPE_RX, frame_type, oam_frame_report_un);
        if (ret != OAL_SUCC) {
            oam_warning_log2(0, OAM_SF_RX,
                "{hmac_report_80211_get_switch::80211_mcast_get_switch failed! ret[%d], frame_type[%d]}",
                ret, frame_type);
            return ret;
        }
    } else {
        ret = mac_vap_find_user_by_macaddr(mac_vap, frame_hdr->auc_address2, &user_idx);
        if (ret == OAL_ERR_CODE_PTR_NULL) {
            oam_warning_log0(mac_vap->uc_vap_id, OAM_SF_ANY,
                "{hmac_report_80211_get_switch::mac_vap_find_user_by_macaddr failed.}");
            return ret;
        }
        if (ret == OAL_FAIL) {
        ...
        ret = oam_report_80211_ucast_get_switch(OAM_OTA_FRAME_DIRECTION_TYPE_RX, frame_type, oam_frame_report_un,
            user_idx);
        if (ret != OAL_SUCC) {
            oam_warning_log3(mac_vap->uc_vap_id, OAM_SF_RX,
                "{hmac_report_80211_get_switch::ucast_get_switch failed! ret[%d], type[%d], user_idx[%d]}",
                ret, frame_type, user_idx);
            oam_warning_log3(mac_vap->uc_vap_id, OAM_SF_RX,
 ...
 *   修改内容  : 新生成函数
 */
uint32_t mac_report_80211_get_switch(mac_vap_stru *mac_vap,
                                     ...
                                     oam_80211_frame_ctx_union *oam_frame_report_un)
{
    ...
    frame_hdr = (mac_ieee80211_frame_stru *)(mac_get_rx_cb_mac_hdr(rx_ctl));
    if (frame_hdr == NULL) {
        oam_error_log0(mac_vap->uc_vap_id, OAM_SF_ANY, "{mac_report_80211_get_switch::frame_hdr null.}");

        return OAL_ERR_CODE_PTR_NULL;
    }
    if ((frame_hdr->st_frame_control.bit_type == WLAN_CONTROL) ||
        (frame_hdr->st_frame_control.bit_type == WLAN_MANAGEMENT)) {
        frame_type = OAM_USER_TRACK_FRAME_TYPE_MGMT;
    }

    if (frame_hdr->st_frame_control.bit_type == WLAN_DATA_BASICTYPE) {
        frame_type = OAM_USER_TRACK_FRAME_TYPE_DATA;
    }

    /* probe request 和 probe response太多，单独过滤一次 */
    if (frame_hdr->st_frame_control.bit_type == WLAN_MANAGEMENT) {
        if (frame_hdr->st_frame_control.bit_sub_type == WLAN_PROBE_REQ ||
            ...
            return OAL_SUCC;
        } else if (frame_hdr->st_frame_control.bit_sub_type == WLAN_DEAUTH ||
                   frame_hdr->st_frame_control.bit_sub_type == WLAN_DISASOC) {
            ...
            return OAL_SUCC;
        } else if (frame_hdr->st_frame_control.bit_sub_type == WLAN_ACTION ||
                   frame_hdr->st_frame_control.bit_sub_type == WLAN_ACTION_NO_ACK) {
            oam_frame_report_un->frame_ctx.bit_cb = OAL_TRUE;
            oam_frame_report_un->frame_ctx.bit_dscr = OAL_TRUE;
            oam_frame_report_un->frame_ctx.bit_content = OAL_TRUE;
            return OAL_SUCC;
    ...
    return hmac_report_80211_get_common_switch(mac_vap, frame_hdr, oam_frame_report_un, frame_type);
}
 ...
 * 输入参数  ：en_ota_type->oam_ota_type_enum
 * 1.日    期  : 2014年5月22日
 *   作    者  : wifi
 *   修改内容  : 新生成函数
 */
                                ...
                                uint8_t en_ota_type)
{
    ...
    ret = mac_report_80211_get_user_macaddr(pst_rx_cb, &auc_user_macaddr[0], WLAN_MAC_ADDR_LEN);
    if (ret != OAL_SUCC) {
