 ...
 * 功能说明 : hmac_host_time.c 的头文件
 * 作    者 : wifi
 * 创建日期 : 2020年11月9日
 */
#ifndef HMAC_HOST_PERFORMANCE_H
#define HMAC_HOST_PERFORMANCE_H
...
static inline void host_start_record_performance(host_performance_proc_enum_uint8 proc)
{
    host_time_start_record(proc);
}
static inline void host_end_record_performance(uint32_t cnt, host_performance_proc_enum_uint8 proc)
{
    host_time_calc_end(cnt, proc);
}
static inline void host_cnt_init_record_performance(host_performance_proc_enum_uint8 proc)
{
    host_time_cnt_init(proc);
}
static inline uint32_t host_cnt_get_record_performance(host_performance_proc_enum_uint8 proc)
{
    return host_time_cnt_get(proc);
}
static inline void host_cnt_inc_record_performance(host_performance_proc_enum_uint8 proc)
{
    host_time_cnt_inc(proc);
}
