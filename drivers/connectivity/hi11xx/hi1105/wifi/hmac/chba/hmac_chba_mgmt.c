...
void hmac_chba_print_self_conn_info(void)
{
    ...
    oam_warning_log3(0, OAM_SF_CHBA, "hmac_chba_print_self_conn_info::SelfConnInfo. ownerAddr %02X:XX:XX:XX:%02X:%02X",
        self_conn_info->island_owner_addr[MAC_ADDR_0], self_conn_info->island_owner_addr[MAC_ADDR_4],
        self_conn_info->island_owner_addr[MAC_ADDR_5]);
    for (j = 0; j < self_conn_info->island_device_cnt; j++) {
        oam_warning_log3(0, OAM_SF_CHBA, "hmac_chba_print_self_conn_info::device addr %02X:XX:XX:XX:%02X:%02X",
            self_conn_info->island_device_list[j].mac_addr[MAC_ADDR_0],
...
static void hmac_chba_del_one_device_info(uint8_t device_id)
{
    ...
    oam_warning_log4(0, OAM_SF_CHBA,
        "{hmac_chba_del_one_device_info: del chba device %02X:XX:XX:XX:%02X:%02X, device_id[%d]}",
        cur_device_info->mac_addr[MAC_ADDR_0], cur_device_info->mac_addr[MAC_ADDR_4],
        ...
        if (hmac_chba_get_network_topo_ele(self_conn_info->self_device_id, device_id) == FALSE) {
            ...
            if (self_conn_info->self_device_id != device_id) {
                oam_warning_log2(0, OAM_SF_CHBA, "hmac_chba_fill_self_island_device_list:: curr_idx %d, device id %d",
                    curr_idx, device_id);
                oam_warning_log3(0, OAM_SF_CHBA,
                    "hmac_chba_fill_self_island_device_list:mac addr %02X:XX:XX:XX:%02X:%02X",
                    device_id_info[device_id].mac_addr[MAC_ADDR_0], device_id_info[device_id].mac_addr[MAC_ADDR_4],
...
uint8_t hmac_chba_one_dev_update_alive_dev_table(const uint8_t* mac_addr)
{
    ...
    ht->node_cnt++;
    oam_warning_log4(0, OAM_SF_CHBA, "update_alive_dev_table: add new node,mac[%02X:XX:XX:XX:%02X:%02X], deviceid[%d]",
         mac_addr[MAC_ADDR_0], mac_addr[MAC_ADDR_4], mac_addr[MAC_ADDR_5], device_id);
    /* 同步device id到DMAC */
...
uint32_t hmac_chba_del_non_alive_device(mac_vap_stru *mac_vap, uint8_t len, uint8_t *param)
{
    ...
    /* 找不到该设备，将该设备加入哈希表 */
    if (node == NULL) {
        oam_warning_log3(0, OAM_SF_CHBA, "hmac_chba_del_non_alive_device: can't find device %02X:XX:XX:XX:%02X:%02X",
            device_rpt->mac_addr[MAC_ADDR_0], device_rpt->mac_addr[MAC_ADDR_4], device_rpt->mac_addr[MAC_ADDR_5]);
        return OAL_SUCC;
    }
    ...
    for (island_idx = 0; island_idx < island_cnt; island_idx++) {
        ...
        if (hmac_chba_island_device_check(&attr_pos[index]) == TRUE) {
            ...
            if (oal_compare_mac_addr(self_conn_info->island_owner_addr, &attr_pos[index]) != 0) {
                oam_warning_log3(0, 0, "rx_island_owner_attr::old owner %02X:XX:XX:XX:%02X:%02X",
                    self_conn_info->island_owner_addr[MAC_ADDR_0], self_conn_info->island_owner_addr[MAC_ADDR_4],
                    self_conn_info->island_owner_addr[MAC_ADDR_5]);
                oam_warning_log3(0, 0, "rx_island_owner_attr::new owner %02X:XX:XX:XX:%02X:%02X",
                    attr_pos[index + MAC_ADDR_0], attr_pos[index + MAC_ADDR_4], attr_pos[index + MAC_ADDR_5]);
            }
...
static void hmac_chba_init_device_mgmt_info(mac_chba_vap_start_rpt *info_report)
{
    ...
    if (device_id >= MAC_CHBA_MAX_DEVICE_NUM) {
        oam_error_log4(0, OAM_SF_CHBA,
            "hmac_chba_init_device_mgmt_info: addr %02X:XX:XX:XX:%02X:%02X, wrong device_id %d",
