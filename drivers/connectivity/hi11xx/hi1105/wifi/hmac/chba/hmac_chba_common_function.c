...
static void hmac_chba_set_chan_switch_ctrl(uint8_t val)
{
    g_chan_switch_enable = val;
}
...
uint32_t hmac_chba_set_feature_switch(mac_vap_stru *mac_vap, uint16_t len, uint8_t *params)
{
    ...
    if (oal_any_null_ptr2(mac_vap, params)) {
        oam_error_log0(mac_vap->uc_vap_id, OAM_SF_CHBA, "{hmac_chba_set_feature_switch:ptr is NULl!}");
        return OAL_ERR_CODE_PTR_NULL;
    }
