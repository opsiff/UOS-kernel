...
#ifndef HMAC_CHBA_MGMT_H
#define HMAC_CHBA_MGMT_H
...
typedef struct {
    ...
    uint8_t mac_addr[OAL_MAC_ADDR_LEN];             /* 该device id对应的设备 */
...
static inline uint32_t chba_bit(uint32_t idx)
{
    return 0x00000001U << (idx);
}

static inline hmac_chba_device_para_stru *hmac_chba_get_device_info(void)
{
    return &g_chba_device_info;
}
static inline hmac_chba_mgmt_info_stru *hmac_chba_get_mgmt_info(void)
{
    return &g_chba_mgmt_info;
}
static inline uint8_t hmac_chba_get_module_state(void)
{
    return g_chba_mgmt_info.chba_module_state;
}

static inline void hmac_chba_set_module_state(uint8_t val)
{
    g_chba_mgmt_info.chba_module_state = val;
}

static inline hmac_chba_alive_device_table_stru *hmac_chba_get_alive_device_table(void)
{
    return g_chba_mgmt_info.alive_device_table;
}

static inline hmac_chba_island_list_stru *hmac_chba_get_island_list(void)
{
    return &g_chba_mgmt_info.island_list;
}
static inline hmac_chba_per_device_id_info_stru *hmac_chba_get_device_id_info(void)
{
    return &g_chba_mgmt_info.device_id_info[0];
}

static inline mac_chba_self_conn_info_stru *hmac_chba_get_self_conn_info(void)
{
    return &g_chba_mgmt_info.self_conn_info;
}

static inline uint32_t *hmac_chba_get_network_topo_addr(void)
{
    return &g_chba_mgmt_info.network_topo[0][0];
}
static inline uint32_t *hmac_chba_get_network_topo_row(uint8_t row)
{
    return &g_chba_mgmt_info.network_topo[row][0];
}
static inline uint8_t hmac_chba_get_role(void)
{
    return g_chba_mgmt_info.self_conn_info.role;
}
static inline uint8_t hmac_chba_get_own_device_id(void)
{
    return g_chba_mgmt_info.self_conn_info.self_device_id;
}

/* 获取连接拓扑图中[row][col]对应的bit的值 */
static inline uint32_t hmac_chba_get_network_topo_ele(uint8_t row, uint8_t col)
{
    return ((g_chba_mgmt_info.network_topo[(row)][(col) / UINT32_BIT_NUM] \
            >> (UINT32_MAX_BIT_IDX - ((col) % UINT32_BIT_NUM))) & 0x1);
}
/* 将连接拓扑图中[row][col]对应的bit置为0 */
static inline void hmac_chba_set_network_topo_ele_false(uint32_t row, uint8_t col)
{
...
static inline void hmac_chba_set_network_topo_ele_true(uint8_t row, uint8_t col)
{
    g_chba_mgmt_info.network_topo[(row)][(col) / UINT32_BIT_NUM] \
        |= chba_bit(UINT32_MAX_BIT_IDX - ((col) % UINT32_BIT_NUM));
}
