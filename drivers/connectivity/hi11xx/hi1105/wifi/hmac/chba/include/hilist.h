...
#define hi_list_for_each(nd, list) \
    for ((nd) = (list)->base.next; (nd) != &(list)->base; (nd) = (nd)->next)

#define hi_list_for_each_safe(nd, tmp, list) \
    for ((nd) = (list)->base.next, (tmp) = (nd)->next; (nd) != &(list)->base; (nd) = (tmp), (tmp) = (nd)->next)
...
static inline void hi_list_walk(struct hi_list *list,
    hi_node_func_x node_proc, void *arg)
{
