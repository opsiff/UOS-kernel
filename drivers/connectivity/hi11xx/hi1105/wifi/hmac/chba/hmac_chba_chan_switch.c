    ...
    hmac_chba_chan_switch_notify_stru *chan_switch_ntf)
{
    ...
    oam_warning_log4(0, OAM_SF_CHBA,
        "hmac_chba_send_chan_switch_notify_action::CSN action tx, type %d, peer_addr %02X:XX:XX:XX:%02X:%02X",
        chan_switch_ntf->notify_type, peer_addr[MAC_ADDR_0], peer_addr[MAC_ADDR_4], peer_addr[MAC_ADDR_5]);
    ...
    hmac_chba_mgmt_info_stru *chba_mgmt_info, hmac_chba_chan_switch_info_stru *chan_switch_info)
{
    ...
    for (idx = 0; idx < device_cnt; idx++) {
        ...
        oam_warning_log3(0, OAM_SF_CHBA,
            "hmac_chba_lost_device_check_and_save::lost_device_mac[%02X:XX:XX:XX:%02X:%02X]",
            chan_switch_info->csa_lost_device[chan_switch_info->csa_lost_device_num].mac_addr[MAC_ADDR_0],
    ...
    hmac_chba_mgmt_info_stru *chba_mgmt_info, hmac_chba_chan_switch_notify_stru *chan_switch_ntf, uint8_t *peer_addr)
{
    ...
    chan_switch_info = &chba_mgmt_info->chan_switch_info;
    oam_warning_log3(0, OAM_SF_CHBA, "hmac_chba_rx_chan_switch_indication::peer_addr[%02X:XX:XX:XX:%02X:%02X]",
    ...
    hmac_chba_chan_switch_notify_stru *chan_switch_ntf, uint8_t *peer_addr)
{
    ...
    oam_warning_log3(0, OAM_SF_CHBA, "hmac_chba_rx_chan_switch_resp::peer_addr[%02X:XX:XX:XX:%02X:%02X]",
        peer_addr[MAC_ADDR_0], peer_addr[MAC_ADDR_4], peer_addr[MAC_ADDR_5]);
...
/* 功能描述: chba无user状态下立即切信道 */
OAL_STATIC void hmac_chba_chan_switch_without_user(hmac_vap_stru *hmac_vap, mac_channel_stru *target_channel)
{
    int32_t mem_ret;
    uint32_t ret;
    ...
    /* chba无user时不会进行时隙调度, 需直接下发命令切信道 */
    channel_cfg.chan_cfg_type_bitmap = CHBA_CHANNEL_CFG_DIRECT_SET_CHANNEL;
    mem_ret = memcpy_s(&(channel_cfg.channel), sizeof(mac_channel_stru), target_channel, sizeof(mac_channel_stru));
    if (mem_ret != EOK) {
        oam_error_log1(hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_ANY,
            "{hmac_chba_chan_switch_without_user:memcpy fail, ret[%d]}", mem_ret);
        return;
    }
    channel_cfg.channel = *target_channel;
    ...
    if (ret != OAL_SUCC) {
        oam_error_log1(hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_ANY,
            "{hmac_chba_chan_switch_without_user:ret[%d]}", ret);
        return;
    }
    oam_warning_log2(hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_ANY,
        "{hmac_chba_chan_switch_without_user:switch chan[%d][%d]}",
        target_channel->uc_chan_number, target_channel->en_bandwidth);
}
/*
...
void hmac_chba_adjust_channel_proc(hmac_vap_stru *hmac_vap, mac_chba_adjust_chan_info *chan_info)
{
    ...
    /* chba未建链时, 只有极速连接模式支持切信道 */
    if (hmac_vap->st_vap_base_info.us_user_nums == 0) {
        if (chan_info->switch_type == MAC_CHBA_CHAN_SWITCH_FAST_CONNECT) {
            hmac_chba_chan_switch_without_user(hmac_vap, &target_channel);
        }
        return;
    }
    /* 进信道切换流程前停掉扫描 */
    ...
    switch (chan_info->switch_type) {
        ...
        case MAC_CHBA_CHAN_SWITCH_FAST_CONNECT:
