...
#define FTM_WAIT_TIMEOUT        50  /* 等待接收FTM_1的时间50ms，避免有时AP响应慢导致无法开启下一个burst */
...
#endif
    uint32_t ftm_info[HMAC_FTM_BUFF_SIZE];
} hmac_ftm_stru;
...
typedef struct {
    ...
    uint8_t need_ftm_action;
    uint8_t frame_type;
    uint8_t prot_format;
    uint8_t band_cap;
    uint16_t payload_len; /* netbuf payload长度, 若netbuf携带ftm_ctx_action, 则包含ftm_ctx_action长度 */
} hmac_ftm_generate_netbuf_ctx_stru;
