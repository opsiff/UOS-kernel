    ...
    uint32_t payload_len, uint8_t direction)
{
    ...
    if (hdr_type == MAC_NETBUFF_PAYLOAD_ETH) {          /* EthernetⅡ标准帧格式 */
        shift_len = ETHER_HDR_LEN;
    } else if (hdr_type == MAC_NETBUFF_PAYLOAD_SNAP) {  /* IEEE 802.3/802.2 SNAP标准帧格式 */
        shift_len = SNAP_LLC_FRAME_LEN;
    } else {                                            /* EthernetⅠ、RAW802.3、IEEE 802.3/802.2 LLC等其余标准格式 */
        return datatype;
    }
    if (payload_len < shift_len) {
        oam_error_log3(0, OAM_SF_ANY, "mac_get_data_type_from_8023:payload_len=%d, shift_len=%d direction=%d",
