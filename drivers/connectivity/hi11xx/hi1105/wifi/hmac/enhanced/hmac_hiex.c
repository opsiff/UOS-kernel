...
/*
 * 函 数 名   : hmac_hiex_sit_find_oldest
 * 功能描述   : 寻找最不活跃的游戏五元组
 * 1.日    期 : 2019年11月20日
 *   修改内容 : 新生成函数
 */
...
/*
 * 函 数 名   : hmac_hiex_sit_aging_handle
 * 功能描述   : SIT表超时函数
 * 1.日    期 : 2019年11月20日
 *   修改内容 : 新生成函数
 */
...
/*
 * 函 数 名   : hmac_hiex_sit_init
 * 功能描述   : stream info table初始化
 * 1.日    期 : 2019年11月20日
 *   修改内容 : 新生成函数
 */
...
/*
 * 函 数 名   : hmac_hiex_sit_exit
 * 功能描述   : stream info table销毁
 * 1.日    期 : 2019年11月20日
 *   修改内容 : 新生成函数
 */
...
/*
 * 函 数 名   : hmac_hiex_user_exit
 * 功能描述   : hiex hmac user销毁
 * 1.日    期 : 2019年11月20日
 *   修改内容 : 新生成函数
 */
...
/*
 * 函 数 名   : hmac_hiex_sit_add_nolock
 * 功能描述   : stream info table增加条目,无锁模式
 * 1.日    期 : 2019年11月20日
 *   修改内容 : 新生成函数
 */
...
/*
 * 函 数 名   : hmac_hiex_sit_add
 * 功能描述   : stream info table增加条目
 * 1.日    期 : 2019年11月20日
 *   修改内容 : 新生成函数
 */
...
/*
 * 函 数 名   : hmac_hiex_sit_del
 * 功能描述   : stream info table删除条目
 * 1.日    期 : 2019年11月20日
 *   修改内容 : 新生成函数
 */
...
/*
 * 函 数 名   : hmac_hiex_notify_sii
 * 功能描述   : 向对端知会SII
 * 1.日    期 : 2019年11月20日
 *   修改内容 : 新生成函数
 */
OAL_STATIC uint32_t hmac_hiex_notify_sii(mac_vap_stru *vap, uint16_t user_idx,
    hmac_hiex_sii_stru *item, mac_hiex_sii_op_enum op)
{
    ...
    oam_warning_log4(vap->uc_vap_id, OAM_SF_HIEX, "{hmac_hiex_notify_sii:notify peer server ip=0x%x port=0x%x, \
...
/*
 * 函 数 名   : hmac_hiex_game_sit_mark
 * 功能描述   : 打桩，将端口5008-5018的帧标记为游戏
 * 1.日    期 : 2019年12月12日
 *   作    者 : wifi
 *   修改内容 : 新生成函数
 */
...
/*
 * 函 数 名   : hmac_hiex_judge_is_game_marked_enter_to_vo
 * 功能描述   : 判断是否是上层标记的游戏帧，如果是，进入VO队列，并且在tx cb中标记该帧为游戏业务
 * 1.日    期 : 2019年12月12日
 *   作    者 : wifi
 *   修改内容 : 新生成函数
 */
    ...
    oal_netbuf_stru *pst_buf, uint8_t *puc_tid)
{
    ...
    if (hmac_hiex_sit_mark(pst_buf, pst_hmac_vap, pst_user,
...
/*
 * 函 数 名   : hmac_hiex_sit_match_ip_header
 * 功能描述   : 配置支持的包的ip头
 * 1.日    期 : 2020年03月23日
 *   修改内容 : 新生成函数
 */
...
/*
 * 函 数 名   : hmac_hiex_sit_mark
 * 功能描述   : 标记发送流程上的skb
 * 1.日    期 : 2019年11月20日
 *   修改内容 : 新生成函数
 */
oal_bool_enum_uint8 hmac_hiex_sit_mark(oal_netbuf_stru *skb, hmac_vap_stru *hmac_vap, hmac_user_stru *user,
    oal_bool_enum_uint8 to_server)
{
    udp_hdr_stru          *udp      = NULL;
    mac_ip_header_stru    *ip       = NULL;
    hmac_hiex_sit_stru    *table     = NULL;
    mac_vap_stru          *vap = &hmac_vap->st_vap_base_info;
    ...
    if (add && vap->en_vap_mode == WLAN_VAP_MODE_BSS_STA) {
        hmac_hiex_notify_sii(vap, user->st_user_base_info.us_assoc_id, &item, MAC_HIEX_SII_ADD);
    }
...
/*
 * 函 数 名   : hmac_hiex_rx_assoc_req
 * 功能描述   : hiex协商流程处理：rx assoc req
 * 1.日    期 : 2019年11月20日
 *   修改内容 : 新生成函数
 */
...
/*
 * 函 数 名   : hmac_hiex_rx_assoc_rsp
 * 功能描述   : hiex协商流程处理：rx assoc rsp
 * 1.日    期 : 2019年11月20日
 *   修改内容 : 新生成函数
 */
    ...
    uint32_t payload_len)
{
    ...
    hmac_sta->is_hwrouter = OAL_FALSE;
    ...
    /* 解析assoc rsp的huawei vendor IE，记录当前关联AP是否为华为路由器
       用于规避AP 11v和上层5G优选策略冲突 导致的频繁漫游 */
    hmac_sta->is_hwrouter = OAL_TRUE;
...
/*
 * 函 数 名   : hmac_hiex_encap_ie
 * 功能描述   : hiex协商流程处理：组私有IE
 * 1.日    期 : 2019年11月20日
 *   修改内容 : 新生成函数
 */
...
/*
 * 函 数 名   : hmac_hiex_rx_sii_report
 * 功能描述   : hiex游戏五元组协商流程处理
 * 1.日    期 : 2019年11月20日
 *   修改内容 : 新生成函数
 */
...
#ifdef _PRE_WLAN_FEATURE_ERSRU
/*
 * 函 数 名   : hmac_hiex_rx_ersru_ctrl
 * 功能描述   : WiFi6超级窄带远距离小RU私有对通
 * 1.日    期 : 2020年1月2日
 *   修改内容 : 新生成函数
 */
...
#endif
/*
 * 函 数 名   : hmac_hiex_handle_rx_action
 * 功能描述   : hiex rx action处理
 * 1.日    期 : 2019年11月20日
 *   修改内容 : 新生成函数
 */
...
static hmac_user_stru *hmac_hiex_get_user(uint16_t idx)
{
    return mac_res_get_hmac_user(idx);
}
#else
static hmac_user_stru *hmac_hiex_get_user(uint16_t idx)
{
    return hmac_user_get_valid(idx);
}
#endif

/*
 * 函 数 名   : hmac_hiex_rx_local_msg
 * 功能描述   : 处理来自host的hiex消息
 * 1.日    期 : 2019年11月20日
 *   修改内容 : 新生成函数
 */
...
uint32_t hmac_hiex_tx_local_msg(mac_vap_stru *vap, mac_hiex_local_msg_stru *msg, uint32_t size)
{
    ...
    if (memcpy_s(event->auc_event_data, size, msg, msg->size + sizeof(mac_hiex_local_msg_stru)) != EOK) {
        HCC_MSG_FREE(mem);
        return OAL_FAIL;
    }
