...
/* hiex全局管理结构体 */
OAL_STATIC mac_hiex_mgr_stru g_alg_st_hiex_mgr;
...
mac_hiex_cap_stru g_st_default_hiex_cap = {
    ...
    .bit_chip_type          = MAC_HIEX_CHIP_TYPE_MP15,
    .bit_resv               = 0,
#else
    ...
    MAC_HIEX_CHIP_TYPE_MP15,    // bit_chip_type
    0,                            // bit_resv
#endif
};

/*
 * 函 数 名   : mac_hiex_get_mgr
 * 功能描述   : 获取HIEX全局管理结构体
 * 1.日    期 : 2019年11月20日
 *   修改内容 : 新生成函数
 */
...
{
    return &g_alg_st_hiex_mgr;
}

/*
 * 函 数 名   : mac_hiex_cap_compatible
 * 功能描述   : hiex能力检查
 * 1.日    期 : 2019年11月20日
 *   修改内容 : 新生成函数
 */
...
uint8_t mac_hiex_set_vendor_ie(mac_hiex_cap_stru *band_cap, mac_hiex_cap_stru *user_cap, uint8_t *buffer)
{
    ...
    vendor_ie->vender.auc_oui[0] = (uint8_t)(((uint32_t)MAC_WLAN_OUI_HUAWEI >> NUM_16_BITS) & 0xff);
...
/*
 * 函 数 名   : mac_hiex_is_valid_udp
 * 功能描述   : 判断是否为hiex trace UDP
 * 1.日    期 : 2019年11月20日
 *   修改内容 : 新生成函数
 */
