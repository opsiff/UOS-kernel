...
#endif
    { "expand_feature_switch_bitmap", WLAN_CFG_PRIV_EXTPAND_FEATURE_SWITCH_BITMAP },
    { "fem_mode_5g",                  WLAN_CFG_PRIV_5G_FEM_MODE },
    { "v100_product_id",              WLAN_CFG_PRIV_IS_V100_NEW_PRODUCT },
    { NULL,                 0 }
};
    ...
    hmac_to_dmac_cfg_custom_data_stru *pst_syn_msg, uint8_t uc_cfg_id)
{
    switch (uc_cfg_id) {
        ...
        case WLAN_CFG_PRIV_CHBA_DBAC_CHAN_FOLLOW_SWITCH:
            ...
            break;
        case WLAN_CFG_PRIV_EXTPAND_FEATURE_SWITCH_BITMAP:
            pst_syn_msg->en_syn_id = CUSTOM_CFGID_PRIV_EXPAND_FEATURE_SWITCH_BITMAP_SWITCH_ID;
            break;
        case WLAN_CFG_PRIV_IS_V100_NEW_PRODUCT:
            pst_syn_msg->en_syn_id = CUSTOM_CFGID_PRIV_IS_V100_NEW_PRODUCT_ID;
            break;
        default:
            break;
    }
...
OAL_STATIC void hwifi_custom_adapt_device_priv_ini_param_extend(uint8_t *puc_data, uint32_t data_len, uint32_t *pul_len)
{
    ...
    hwifi_custom_adapt_priv_ini_param(WLAN_CFG_PRIV_EXTPAND_FEATURE_SWITCH_BITMAP, puc_data + data_len, &data_len);
    ...
    l_ret = hwifi_custom_adapt_priv_ini_param(WLAN_CFG_PRIV_IS_V100_NEW_PRODUCT, puc_data + data_len, &data_len);
    if (l_ret != OAL_SUCC) {
