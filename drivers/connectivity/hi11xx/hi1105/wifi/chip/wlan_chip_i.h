...
#ifndef WLAN_CHIP_I_H
#define WLAN_CHIP_I_H
...
OAL_STATIC OAL_INLINE uint8_t wlan_chip_get_special_dbdc_cap(void)
{
    if (g_wlan_chip_ops->get_special_dbdc_cap != NULL) {
        return g_wlan_chip_ops->get_special_dbdc_cap();
    }
    return 0;
}

OAL_STATIC OAL_INLINE void wlan_chip_get_rf_band_from_center_freq(uint16_t center_freq,
    uint8_t *rf_band_sel)
{
    if (g_wlan_chip_ops->get_rf_band_from_center_freq != NULL) {
        g_wlan_chip_ops->get_rf_band_from_center_freq(center_freq, rf_band_sel);
    }
}

OAL_STATIC OAL_INLINE uint16_t wlan_chip_get_center_freq_from_chn(const mac_channel_stru *channel_info)
{
    if (g_wlan_chip_ops->get_center_freq_from_chn != NULL) {
        return g_wlan_chip_ops->get_center_freq_from_chn(channel_info);
    }
    return 0;
}

OAL_STATIC OAL_INLINE oal_bool_enum_uint8 wlan_chip_is_support_dual_5g_dbdc_by_radio_cap(void)
{
    if (g_wlan_chip_ops->is_support_dual_5g_dbdc_by_radio_cap != NULL) {
        return g_wlan_chip_ops->is_support_dual_5g_dbdc_by_radio_cap();
    }
    return OAL_FALSE;
}
...
OAL_STATIC OAL_INLINE oal_bool_enum wlan_chip_is_single_dbdc_ini_en(void)
{
    if (g_wlan_chip_ops->is_single_dbdc_ini_en != NULL) {
        return g_wlan_chip_ops->is_single_dbdc_ini_en();
    }
    return OAL_FALSE;
}
...
static inline uint8_t wlan_chip_soft_irq_sch_support(void)
{
    return g_wlan_chip_ops->soft_irq_sch_support ? g_wlan_chip_ops->soft_irq_sch_support() : OAL_FALSE;
}

static inline uint8_t wlan_chip_get_pcie_probe_state(mac_user_stru *mac_user)
{
    return g_wlan_chip_ops->get_pcie_probe_state ? g_wlan_chip_ops->get_pcie_probe_state(mac_user) :
        WLAN_PCIE_PROBE_DEFAULT;
}

#ifdef _PRE_WLAN_CHBA_MGMT
static inline oal_bool_enum_uint8 wlan_chip_is_support_chba_dbdc_dbac(void)
{
    if (g_wlan_chip_ops->is_support_chba_dbdc_dbac != NULL) {
        return g_wlan_chip_ops->is_support_chba_dbdc_dbac();
    }
    return OAL_FALSE;
}
#endif
...
static inline void wlan_chip_lp_miracast_stat_mbps(uint32_t tx_throughput_mbps, uint32_t rx_throughput_mbps)
{
    if (g_wlan_chip_ops->lp_miracast_stat_func != NULL) {
        g_wlan_chip_ops->lp_miracast_stat_func(tx_throughput_mbps, rx_throughput_mbps);
    }
}

static inline uint32_t wal_chip_set_lp_miracast(mac_vap_stru *mac_vap, uint16_t len, uint8_t *param)
{
    if (g_wlan_chip_ops->set_lp_miracast_func != NULL) {
        return g_wlan_chip_ops->set_lp_miracast_func(mac_vap, len, param);
    }
    return OAL_FAIL;
}
