 ...
 *
 * Description: Host spmi driver, which is implemented through the SSI interface
...
OAL_STATIC int32_t ssi_spmi_wait_done(uint32_t channel, uint32_t slave_id)
{
    ...
    addr = spmi_status_addr(channel, slave_id);
    while (timeout--) {
        ret = ssi_read_value32(addr, &data, (((addr >> 16) & 0xffff) + 1)); /* Shifts rightwards by 16 bits. */
        if (ret) {
            continue;
        }
        if (data == 1) {
    ...
    ps_print_err("host spmi trans fail! ssi_read_value32 ret=%d\n", ret);
    return -OAL_EFAIL;
}
...
int32_t ssi_spmi_read(uint32_t addr, uint32_t channel, uint32_t slave_id, uint32_t *value)
{
    ...
    ssi_write_value32(spmi_cmd_addr(channel), cmd);
    if (ssi_spmi_wait_done(channel, slave_id) != OAL_SUCC) {
        ps_print_err("Ssi_spmi_read 0x%x fail\n", spmi_cmd_addr(channel));
        return -OAL_EFAIL;
    }

    ssi_addr = spmi_rdata0_addr(channel, slave_id);
    ret = ssi_read_value32(ssi_addr, &data, (((ssi_addr >> 16) & 0xffff) + 1)); /* Shifts rightwards by 16 bits. */
    if (ret) {
...
int32_t ssi_spmi_write(uint32_t addr, uint32_t channel, uint32_t slave_id, uint32_t data)
{
    ...
    ssi_write_value32(spmi_wdata0_addr(channel), data << WRITE_READ_FIRST_BYTE_OFFSET);
    if (ssi_spmi_wait_done(channel, slave_id) != OAL_SUCC) {
        return -OAL_EFAIL;
    }
