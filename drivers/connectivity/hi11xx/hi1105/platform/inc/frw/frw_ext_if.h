 ...
 *
 * Description: Header file of the external public interface of the FRW
...
/* 事件队列最大个数 */
#define FRW_EVENT_MAX_NUM_QUEUES_WIFI (FRW_EVENT_TYPE_BUTT * WLAN_VAP_SUPPORT_MAX_NUM_LIMIT)
#define FRW_EVENT_MAX_NUM_QUEUES_GT (FRW_EVENT_TYPE_BUTT * GT_VAP_SUPPORT_MAX_NUM_LIMIT)

// 注：确保FRW_EVENT_MAX_NUM_QUEUES使用较大规格值
// 由于FRW_EVENT_TYPE_BUTT是枚举值，无法在预编译时判断两个规格宏的大小
#define FRW_EVENT_MAX_NUM_QUEUES FRW_EVENT_MAX_NUM_QUEUES_WIFI
...
#define frw_event_alloc_m(_us_len) \
    frw_event_alloc_ctx(THIS_FILE_ID, __LINE__, _us_len, CUR_OAL_MEM_CONTEXT)

#define frw_event_free_m(_pst_event_mem) \
    frw_event_free_ctx(THIS_FILE_ID, __LINE__, _pst_event_mem, CUR_OAL_MEM_CONTEXT)

/* mpxx共仓代码适配51，mpxx为了编译时进行检查，后续可能会有调整 */
...
/* 为了mpxx和51共仓代码一致，这里保留该宏定义，使用时注意和frw_get_event_stru进行区分 */
 ...
 *             失败:NULLL
 */
OAL_STATIC OAL_INLINE frw_event_mem_stru *frw_event_alloc_ctx(uint32_t file_id, uint32_t line_num,
    uint16_t payload_length, oal_mem_pool_context_stru *context)
{
    frw_event_mem_stru *pst_event_mem;

    payload_length += OAL_MEM_INFO_SIZE;

    pst_event_mem = oal_mem_alloc_enhanced_ctx(file_id, line_num, OAL_MEM_POOL_ID_EVENT,
                                               (payload_length + FRW_EVENT_HDR_LEN), OAL_TRUE, context);
    if (oal_unlikely(pst_event_mem == NULL)) {
        oal_mem_print_normal_pool_info_ctx(OAL_MEM_POOL_ID_EVENT, context);
...
OAL_STATIC OAL_INLINE frw_event_mem_stru *frw_event_alloc(uint32_t file_id, uint32_t line_num, uint16_t payload_length)
{
    ...
    return frw_event_alloc_ctx(file_id, line_num, payload_length, GT_OAL_MEM_CONTEXT);
#elif defined(_PRE_MULTI_INSTANCE_WIFI)
    return frw_event_alloc_ctx(file_id, line_num, payload_length, WIFI_OAL_MEM_CONTEXT);
#else
    return NULL;
#endif
}
 ...
 * 功能描述  : 释放事件所占用的内存
 */
OAL_STATIC OAL_INLINE uint32_t frw_event_free_ctx(uint32_t file_id,
                                                  uint32_t line_num,
                                                  frw_event_mem_stru *pst_event_mem,
                                                  oal_mem_pool_context_stru *context)
{
    ...
    ret = oal_mem_free_enhanced_ctx(file_id, line_num, pst_event_mem, OAL_TRUE, context);
...
{
#if defined(_PRE_MULTI_INSTANCE_GT)
    return frw_event_free_ctx(file_id, line_num, pst_event_mem, GT_OAL_MEM_CONTEXT);
#elif defined(_PRE_MULTI_INSTANCE_WIFI)
    return frw_event_free_ctx(file_id, line_num, pst_event_mem, WIFI_OAL_MEM_CONTEXT);
#else
    return OAL_FAIL;
#endif
}

typedef struct _frw_subsys_context_ frw_subsys_context;
...
extern void frw_set_init_state_ctx(uint16_t en_init_state, frw_subsys_context *ctx);
extern uint16_t frw_get_init_state_ctx(frw_subsys_context *ctx);
extern uint32_t frw_event_exit_ctx(frw_subsys_context *ctx);
extern uint32_t frw_event_init_ctx(frw_subsys_context *ctx);
extern uint32_t frw_event_dispatch_event_ctx(frw_event_mem_stru *pst_mem, frw_subsys_context *ctx);

extern void frw_event_table_register_ctx(uint8_t en_event_type,
...
extern uint32_t frw_event_flush_event_queue_ctx(uint8_t uc_event_type, frw_subsys_context *ctx);
extern void frw_event_vap_pause_event_ctx(uint8_t uc_vap_id, frw_subsys_context *ctx);
extern void frw_event_vap_resume_event_ctx(uint8_t uc_vap_id, frw_subsys_context *ctx);
extern uint32_t frw_event_vap_flush_event_ctx(uint8_t uc_vap_id,
                                              uint8_t en_event_type,
                                              oal_bool_enum_uint8 en_drop,
                                              frw_subsys_context *ctx);
extern void frw_timer_create_timer_ctx(uint32_t file_id,
                                       uint32_t line_num,
                                       ...
                                       uint32_t core_id,
                                       frw_subsys_context *ctx);
extern void frw_timer_immediate_destroy_timer_ctx(uint32_t file_id,
                                                  uint32_t line_num,
                                                  frw_timeout_stru *pst_timeout,
                                                  frw_subsys_context *ctx);
extern void frw_timer_restart_timer_ctx(frw_timeout_stru *pst_timeout, uint32_t timeout,
                                        oal_bool_enum_uint8 en_is_periodic, frw_subsys_context *ctx);
extern void frw_timer_add_timer_ctx(frw_timeout_stru *pst_timeout, frw_subsys_context *ctx);
extern void frw_timer_stop_timer_ctx(frw_timeout_stru *pst_timeout, frw_subsys_context *ctx);
extern void frw_timer_delete_all_timer_ctx(frw_subsys_context *ctx);
extern void frw_timer_clean_timer_ctx(uint16_t en_module_id, frw_subsys_context *ctx);
extern uint32_t frw_event_queue_info_ctx(frw_subsys_context *ctx);
extern void frw_event_process_all_event_ctx(oal_uint ui_data, frw_subsys_context *ctx);
extern void frw_event_process_all_event_gt(oal_uint ui_data);
extern void frw_event_process_all_event_wifi(oal_uint ui_data);
extern void frw_event_process_all_event_when_wlan_close_ctx(frw_subsys_context *ctx);
extern oal_bool_enum_uint8 frw_is_event_queue_empty_ctx(uint8_t uc_event_type, frw_subsys_context *ctx);
extern oal_bool_enum_uint8 frw_is_vap_event_queue_empty_ctx(uint32_t core_id, uint8_t uc_vap_id,
                                                            uint8_t event_type,
                                                            frw_subsys_context *ctx);
extern uint8_t frw_task_thread_condition_check_ctx(uint32_t core_id, frw_subsys_context *ctx);

extern void hcc_host_update_vi_flowctl_param(uint32_t be_cwmin, uint32_t vi_cwmin, uint8_t dev_id);
