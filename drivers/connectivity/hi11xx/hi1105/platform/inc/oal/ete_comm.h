 ...
 *
 * Description: ete_comm.h header file
 * Author: @CompanyNameTag
 */

#ifndef OAL_ETE_COMM_H
#define OAL_ETE_COMM_H
...
#define PCIE_ETE_TX_CHAN0_ENTRY_NUM 32
#define PCIE_ETE_TX_CHAN1_ENTRY_NUM 32
#define PCIE_ETE_TX_CHAN2_ENTRY_NUM 32

#define PCIE_ETE_RX_CHAN0_ENTRY_NUM 32
#define PCIE_ETE_RX_CHAN1_ENTRY_NUM 32
...
typedef enum _PCIE_HOST_CTL_INTR_ {
    ...
    PHY_2G_INTR_MASK,
    PCIE_SLV_RRESP_INTR,
    HOST_ETE_RX_INTR_MASK = 20,
    HOST_ETE_TX_INTR_MASK,
    SYNC_RX_CH_DR_EMPTY_INTR_MASK,
    SYNC_TX_CH_DR_EMPTY_INTR_MASK,
...
uint32_t pcie_ete_check_rx_ring_is_busy(pcie_ete_ringbuf *pst_ringbuf, uintptr_t base_address, uint32_t chan_idx);
...
OAL_STATIC OAL_INLINE uint32_t pcie_ete_ringbuf_len(pcie_ete_ringbuf *pst_ringbuf)
{
    ...
    int32_t flag = pcie_ete_ringbuf_is_overflow(pst_ringbuf);

    rd = pst_ringbuf->rd;
    wr = pst_ringbuf->wr;

    if (flag == PCIE_ETE_TRANS_OVERFLOW_TAG) { /* 1 means overflow */
