 ...
 *
 * Description: Interface file of the SDIO host functional module
 * Author: @CompanyNameTag
 */

#ifndef OAL_SDIO_HOST_IF_H
#define OAL_SDIO_HOST_IF_H
...
extern int32_t oal_sdio_mpxx_working_check(void);
extern void oal_sdio_exit_module(struct oal_sdio *hi_sdio);

extern struct oal_sdio *g_mpxx_sdio;
OAL_STATIC OAL_INLINE struct oal_sdio *oal_get_sdio_default_handler(void)
{
    return g_mpxx_sdio;
}
#else
extern struct oal_sdio g_mpxx_sdio_ut;
OAL_STATIC OAL_INLINE struct oal_sdio *oal_get_sdio_default_handler(void)
{
#if (_PRE_OS_VERSION_LINUX == _PRE_OS_VERSION)
    return NULL;
#else
    return &g_mpxx_sdio_ut;
#endif
}
