 ...
 *
 * Description: oal_hcc_if.c header file
 * Author: @CompanyNameTag
 */

#ifndef OAL_HCC_HOST_IF_H
#define OAL_HCC_HOST_IF_H
...
#include "external/amax_multilink.h"
#endif
...
struct custom_process_func_handler {
    ...
    custom_cali_func p_custom_cali_func_gt;
};
...
OAL_STATIC OAL_INLINE void hi_wlan_power_set(int32_t on)
{
     ...
     * it depends on the gpio used to power up and down chip
     *
     */
#if (_PRE_OS_VERSION_LINUX == _PRE_OS_VERSION)
    if (on) {
...
#ifdef _PRE_CONFIG_HILINK_AMAX
    external_amax_tx_complete(pst_netbuf);
#endif
