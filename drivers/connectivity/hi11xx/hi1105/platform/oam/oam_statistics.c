...
void oam_stats_report_irq_info_to_sdt(uint8_t *puc_irq_info_addr, uint16_t us_irq_info_len)
{
    ...
    us_skb_len = us_irq_info_len + (uint16_t)sizeof(oam_ota_hdr_stru);
    if (us_skb_len > SDT_NETBUF_MAX_PAYLOAD) {
        us_skb_len = SDT_NETBUF_MAX_PAYLOAD;
        us_irq_info_len = SDT_NETBUF_MAX_PAYLOAD - (uint16_t)sizeof(oam_ota_hdr_stru);
    }
                                              ...
                                              uint16_t us_subpool_free_cnt)
{
    ...
    us_stru_len = (uint16_t)sizeof(oam_stats_mempool_stru);
    /* 为ota消息上报SDT申请空间,头部预留8字节，尾部预留1字节，给sdt_drv用 */
    us_skb_len = us_stru_len + (uint16_t)sizeof(oam_ota_hdr_stru);
    if (us_skb_len > SDT_NETBUF_MAX_PAYLOAD) {
        us_skb_len = SDT_NETBUF_MAX_PAYLOAD;
        us_stru_len = us_skb_len - (uint16_t)sizeof(oam_ota_hdr_stru);
    }
                                               ...
                                               uint32_t ul_alloc_line_num)
{
    ...
    us_memblock_info_len = (uint16_t)sizeof(oam_memblock_info_stru);
    ...
    /* 为ota消息上报SDT申请空间,头部预留8字节，尾部预留1字节，给sdt_drv用 */
    us_skb_len = us_memblock_info_len + us_len + (uint16_t)sizeof(oam_ota_hdr_stru);
    if (us_skb_len > SDT_NETBUF_MAX_PAYLOAD) {
        us_skb_len = SDT_NETBUF_MAX_PAYLOAD;
        if ((us_memblock_info_len + sizeof(oam_ota_hdr_stru)) < us_skb_len) {
            ...
            us_memblock_info_len = us_skb_len - (uint16_t)sizeof(oam_ota_hdr_stru);
            us_len = 0;
        }
    }
                                                  ...
                                                  uint16_t us_event_queue_info_len)
{
    ...
    /* 为上报描述符申请空间,头部预留8字节，尾部预留1字节，给sdt_drv用 */
    us_skb_len = us_event_queue_info_len + (uint16_t)sizeof(oam_ota_hdr_stru);
    if (us_skb_len > SDT_NETBUF_MAX_PAYLOAD) {
        us_skb_len = SDT_NETBUF_MAX_PAYLOAD;
        us_event_queue_info_len = SDT_NETBUF_MAX_PAYLOAD - (uint16_t)sizeof(oam_ota_hdr_stru);
    }
