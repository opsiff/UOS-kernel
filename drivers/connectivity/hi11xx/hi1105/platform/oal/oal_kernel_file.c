 ...
 *
 * Description:Kernel file initialization
 ...
 */
oal_file *oal_kernel_file_open(const uint8_t *file_path, size_t file_name_arry_len, int32_t ul_attribute)
{
    ...
    fs = get_fs();
    set_fs(KERNEL_DS);
    pst_file = filp_open((char *)file_path, ul_attribute, 0777); /* 0777->rwx:rwx:rwx */
    set_fs(fs);
    if (IS_ERR(pst_file)) {
        return 0;
    }
