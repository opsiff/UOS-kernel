...
OAL_STATIC int32_t oal_sdio_hcc_bus_power_patch_lauch(hcc_bus *pst_bus)
{
    ...
    if (oal_wait_for_completion_timeout(&pst_bus->st_device_ready,
        ...
        if (oal_wait_for_completion_timeout(&pst_bus->st_device_ready,
            ...
            oal_print_mpxx_log(MPXX_LOG_WARN, "[E]retry succ, maybe gpio interrupt issue");
...
OAL_STATIC int32_t oal_sdio_mmc_power_reinit(struct oal_sdio *hi_sdio)
{
    ...
    if (ret < OAL_SUCC) {
        ...
        chr_exception_report(CHR_PLATFORM_EXCEPTION_EVENTID, CHR_SYSTEM_PLAT, CHR_LAYER_DRV,
                             CHR_PLT_DRV_EVENT_INIT, CHR_PLAT_DRV_ERROR_SDIO_INIT);
#ifdef CONFIG_ARCH_HI1103_SDIO_DEBUG
                                            ...
                                            void *buf, uint32_t size)
{
    ...
    if (oal_warn_on((uintptr_t)buf & 0x3)) {
        oal_print_mpxx_log(MPXX_LOG_ERR, "%s error: buf :%llx", __FUNCTION__, (uint64_t)(uintptr_t)buf);
        return -OAL_EFAIL;
    }
...
int32_t oal_sdio_transfer_tx(struct oal_sdio *hi_sdio, oal_netbuf_stru *netbuf)
{
    ...
    if (oal_unlikely(oal_any_null_ptr2(hi_sdio, netbuf))) {
        oal_print_mpxx_log(MPXX_LOG_ERR, "pointer hi_sdio or netbuf is null");
        return -OAL_EFAIL;
    }
...
OAL_STATIC int32_t oal_sdio_suspend(struct device *dev)
{
    ...
    if (down_interruptible(&pst_bus->sr_wake_sema)) {
        oal_print_mpxx_log(MPXX_LOG_ERR, "sdio_wake_sema down failed.");
        return -OAL_EFAIL;
    }
    ...
    if (hcc_bus_wakelock_active(pst_bus)) {
         ...
         */
        oal_print_mpxx_log(MPXX_LOG_ERR, "Already wake up");
...
OAL_STATIC void oal_sdio_dev_shutdown(struct device *dev)
{
    ...
    if (g_hisdio_intr_mode) {
        /* gpio interrupt */
        oal_wlan_gpio_intr_enable(hbus_to_dev(hi_sdio->pst_bus), OAL_FALSE);
    } else {
        /* sdio interrupt */
        int32_t ret;
        oal_sdio_claim_host(hi_sdio);
        ...
        oal_sdio_release_host(hi_sdio);
        if (ret) {
            oal_print_mpxx_log(MPXX_LOG_INFO, "wlan shutdown fail, ret=%d!", ret);
            return;
        }
    }
...
{
    if (oal_unlikely(oal_any_null_ptr1(hi_sdio))) {
        oal_print_mpxx_log(MPXX_LOG_ERR, "pointer hi_sdio is null");
        return;
    }
...
OAL_STATIC int32_t oal_sdio_gpio_flowctrl_irq(hcc_bus *pst_hi_bus, int32_t l_irq)
{
    ...
    if (oal_unlikely(pst_hi_bus == NULL)) {
        oal_print_mpxx_log(MPXX_LOG_ERR, "sdio bus is null, irq:%d", l_irq);
        return -OAL_EINVAL;
    }
...
OAL_STATIC int32_t oal_sdio_gpio_irq(hcc_bus *hi_bus, int32_t irq)
{
    ...
    if (oal_unlikely(hi_bus == NULL)) {
        oal_print_mpxx_log(MPXX_LOG_ERR, "sdio bus is null, irq:%d", irq);
        return -OAL_EINVAL;
    }
