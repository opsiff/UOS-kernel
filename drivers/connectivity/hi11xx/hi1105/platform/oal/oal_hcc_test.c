...
OAL_STATIC int32_t hcc_flowctrl_info_print(void *data, char *buf, int32_t buf_len)
{
    ...
    ret = snprintf_s(buf + count, buf_len - count, buf_len - count - 1, "flowctrl on:%u\n",
    ...
    ret = snprintf_s(buf + count, buf_len - count, buf_len - count - 1, "flowctrl off:%u\n",
    ...
    ret = snprintf_s(buf + count, buf_len - count, buf_len - count - 1, "flowctrl reset count:%u\n",
    ...
    ret = snprintf_s(buf + count, buf_len - count, buf_len - count - 1,
                     "flowctrl hi update count:%u,hi credit value:%u\n",
    ...
    for (i = 0; i < HCC_DIR_COUNT; i++) {
        ...
        for (j = 0; j < HCC_QUEUE_COUNT; j++) {
            if (oal_netbuf_list_len(&hcc->hcc_transer_info.hcc_queues[i].queues[j].data_queue)) {
                ret = snprintf_s(buf + count, buf_len - count, buf_len - count - 1, "queue:%d, len:%u\n", j,
...
OAL_STATIC int32_t hcc_test_rx_start(uint16_t start_cmd)
{
    ...
    cmd_len = (uint32_t)(sizeof(hcc_test_cmd_stru) + sizeof(hcc_trans_test_rx_info));
...
OAL_STATIC int32_t hcc_test_pcie_loopback(int32_t is_phy_loopback)
{
    ...
    cmd.cmd_len = (uint16_t)sizeof(hcc_test_cmd_stru);
...
OAL_STATIC ssize_t hcc_test_pcie_loopback_get_para(struct kobject *dev, struct kobj_attribute *attr, char *buf)
{
    ...
    count += ret;
    ret = snprintf_s(buf + count, buf_len - count, buf_len - count - 1, "TimeCost linkup %u us  %ld ms\n",
                     g_pcie_test_request_ack.link_up_cost, g_pcie_test_request_ack.link_up_cost / USEC_PER_MSEC);
    if (ret < 0) {
        return count;
    }
    count += ret;
    ret = snprintf_s(buf + count, buf_len - count, buf_len - count - 1, "TimeCost rw_test_cost %u us  %ld ms\n",
                     g_pcie_test_request_ack.rw_test_cost, g_pcie_test_request_ack.rw_test_cost / USEC_PER_MSEC);
    if (ret < 0) {
        return count;
    }
    count += ret;
    ret = snprintf_s(buf + count, buf_len - count, buf_len - count - 1, "TimeCost total_test_cost %u us  %ld ms\n",
...
OAL_STATIC int32_t hcc_test_normal_start(uint16_t start_cmd)
{
    ...
    cmd.cmd_len = (uint16_t)(sizeof(hcc_test_cmd_stru) + sizeof(hsdio_trans_test_info));
...
OAL_STATIC ssize_t hcc_test_print_thoughput(char *buf, uint32_t buf_len)
{
    ...
    ret = snprintf_s(buf + count, buf_len - count, buf_len - count - 1, "Throughput %d Mbps\n",
                                          ...
                                          const char *buf, size_t count)
{
    ...
    wlan_pm_disable();
    oal_io_print("[dfr_test] trigger wifi panic\n");
...
OAL_STATIC ssize_t hcc_test_get_pad_payload(struct kobject *dev, struct kobj_attribute *attr, char *buf)
{
    ...
    ret += snprintf_s(buf + ret, PAGE_SIZE, PAGE_SIZE - 1, "%u\n", g_hcc_test_event->pad_payload);
    return ret;
}
