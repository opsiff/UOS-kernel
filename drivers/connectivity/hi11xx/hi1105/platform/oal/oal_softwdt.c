 ...
 *
 * Description:softwdt functional module
...
/* 模块参数只读, 不允许修改 */
oal_debug_module_param(g_disable_wdt_flag, int, S_IRUGO);
...
OAL_STATIC void oal_softwdt_timeout_log(void)
{
    ...
    printk(KERN_ERR "Time check: napi time = %lu pcie time = %lu cur time = %lu\n\r",
        g_hisi_softwdt_check.napi_poll_time, g_hisi_softwdt_check.pcie_done_time,
        oal_read_once(OAL_TIME_JIFFY));
}
