...
oal_pcie_linux_res* oal_pcie_res_init(oal_pci_dev_stru *pst_pci_dev, uint32_t dev_id)
{
    ...
    if (atomic_read(&pcie_res->comm_res->refcnt) != 0) {
        ...
        /* 另一路ETE已经初始化完ITAU表配置 */
        pcie_res->ep_res.link_state = PCI_WLAN_LINK_MEM_UP;
        return pcie_res;
    }
    oal_spin_lock_init(&pcie_res->comm_res->st_lock);
    pcie_res->comm_res->pcie_dev = pst_pci_dev;
    pcie_res->comm_res->version = (pst_pci_dev->vendor) | (pst_pci_dev->device << 16); // 左移16位获取pcie版本
...
{
    hcc_bus_dev *hcc_bus_dev = oal_pcie_get_one_alive_bus_dev();

    if (hcc_bus_dev != NULL) {
        return oal_get_pcie_handler(hcc_bus_dev->dev_id);
    }
    return NULL;
}
...
{
    oal_pcie_linux_res *pcie_res = oal_get_pcie_handler(dev_id);
    if (pcie_res == NULL) {
        oal_io_print("pcie get link state error devid %d\n", dev_id);
        return PCI_WLAN_LINK_DOWN;
    }
    return pcie_res->ep_res.link_state;
}
#endif
