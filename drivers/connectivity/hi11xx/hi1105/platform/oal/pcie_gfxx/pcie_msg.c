                                                  ...
                                                  pci_addr_map *pst_ringbuf_base, uint32_t message)
{
    ...
    pst_ringbuf->wr += (uint32_t)sizeof(message);
...
int32_t oal_pcie_ringbuf_read_rd(oal_pcie_linux_res *pst_pci_res, pcie_comm_ringbuf_type type)
{
    ...
    if (oal_unlikely(pst_pci_res->ep_res.link_state < PCI_WLAN_LINK_RES_UP)) {
        pci_print_log(PCI_LOG_WARN, "comm ringbuf %d read rd, link_state:%s", type,
                      oal_pcie_get_link_state_str(pst_pci_res->ep_res.link_state));
        return -OAL_ENODEV;
    }
...
int32_t oal_pcie_send_message_to_dev(oal_pcie_linux_res *pst_pci_res, uint32_t message)
{
    ...
    if (oal_unlikely(pst_pci_res->ep_res.link_state < PCI_WLAN_LINK_RES_UP)) {
        pci_print_log(PCI_LOG_WARN, "send message link invaild, link_state:%s, devid %d",
                      oal_pcie_get_link_state_str(pst_pci_res->ep_res.link_state), pst_pci_res->dev_id);
        return -OAL_EBUSY;
    }
