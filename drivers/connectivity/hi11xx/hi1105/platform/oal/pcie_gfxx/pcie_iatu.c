 ...
 *
 * Description:pcie device iatu configuration
...
int32_t oal_pcie_disable_regions(oal_pcie_linux_res *pcie_res)
{
    ...
    if (oal_atomic_read(&pcie_res->comm_res->refcnt) > 1) {
        pci_print_log(PCI_LOG_DBG, "not disable_regions, other ete using");
        return OAL_SUCC;
    }
                                     ...
                                     oal_pcie_region **region_base, uint32_t *region_num)
{
    if (pcie_res->ep_res.chip_info.cb.pcie_get_bar_region_info == NULL) {
        oal_print_mpxx_log(MPXX_LOG_INFO, "pcie_get_bar_region_info is null");
        return -OAL_ENODEV;
    }
...
int32_t oal_pcie_set_outbound_membar(oal_pcie_linux_res *pcie_res, oal_pcie_iatu_bar *pst_iatu_bar)
{
    if (pcie_res->ep_res.chip_info.cb.pcie_set_outbound_membar == NULL) {
        oal_print_mpxx_log(MPXX_LOG_INFO, "pcie_set_outbound_membar is null");
        return -OAL_ENODEV;
    }
