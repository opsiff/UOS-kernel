 ...
 *
 * Description:PCIE address mapping
...
int32_t oal_pcie_share_mem_res_map(oal_pcie_linux_res *pcie_res)
{
    ...
    ret = oal_pcie_inbound_ca_to_va(pcie_res, ep_res->chip_info.addr_info.sharemem_addr, &addr_map);
    if (ret != OAL_SUCC) {
        /* share mem 地址未映射! */
    ...
    forever_loop() {
        ...
        ret = oal_pcie_inbound_ca_to_va(pcie_res, share_mem_address, &share_mem_map);
        if (ret != OAL_SUCC) {
            return ret;
        }
                                                       ...
                                                       pcie_share_mem_stru *pst_share_mem)
{
    ...
    for (i = 0; i < PCIE_SHARED_ADDR_BUTT; i++) {
        ...
        ret = oal_pcie_inbound_ca_to_va(pcie_res, pst_share_mem->device_addr[i], &st_map);
        if (ret != OAL_SUCC) {
...
OAL_STATIC int32_t oal_pcie_comm_ringbuf_res_map(oal_pcie_ep_res *ep_res)
{
    ...
    for (i = 0; i < PCIE_COMM_RINGBUF_BUTT; i++) {
        ...
        ret = oal_pcie_inbound_ca_to_va(pcie_res, ep_res->st_ringbuf.st_ringbuf[i].base_addr, &st_map);
        if (ret != OAL_SUCC) {
...
int32_t oal_pcie_ringbuf_res_map(oal_pcie_linux_res *pcie_res)
{
    ...
    ret = oal_pcie_inbound_ca_to_va(pcie_res, st_share_mem.ringbuf_res_paddr, &st_map);
    if (ret != OAL_SUCC) {
    ...
    ret = oal_pcie_inbound_ca_to_va(pcie_res, ep_res->st_ringbuf.st_d2h_buf.base_addr, &st_map);
    if (ret != OAL_SUCC) {
    ...
    for (i = 0; i < PCIE_H2D_QTYPE_BUTT; i++) {
        uintptr_t offset;
        ret = oal_pcie_inbound_ca_to_va(pcie_res, ep_res->st_ringbuf.st_h2d_buf[i].base_addr, &st_map);
        if (ret != OAL_SUCC) {
    ...
    /* 初始化消息TX RINGBUFF */
    ret = oal_pcie_inbound_ca_to_va(pcie_res, ep_res->st_ringbuf.st_h2d_msg.base_addr, &st_map);
    if (ret != OAL_SUCC) {
    ...
    /* 初始化消息RX RINGBUFF */
    ret = oal_pcie_inbound_ca_to_va(pcie_res, ep_res->st_ringbuf.st_d2h_msg.base_addr, &st_map);
    if (ret != OAL_SUCC) {
    ...
    ret = oal_pcie_inbound_ca_to_va(pcie_res, st_share_mem.device_stat_paddr, &st_map);
    if (ret != OAL_SUCC) {
 ...
 * 函数返回非0为失败
 */
int32_t oal_pcie_inbound_ca_to_va(oal_pcie_linux_res *pst_pci_lres, uint64_t dev_cpuaddr, pci_addr_map *addr_map)
{
    int32_t index;
    int32_t region_num;
    uint64_t offset;
    ...
    if (pst_pci_lres == NULL) {
        pci_print_log(PCI_LOG_ERR, "pst_pci_lres invalid");
        return -OAL_EINVAL;
    }
    comm_res = pst_pci_lres->comm_res;
    region_num = comm_res->regions.region_nums;
    region_base = comm_res->regions.pst_regions;
    ...
    if (oal_unlikely(pst_pci_lres->ep_res.link_state < PCI_WLAN_LINK_MEM_UP)) {
        pci_print_log(PCI_LOG_ERR, "addr request 0x%llx failed, link_state:%s, dev_id %d",
                      dev_cpuaddr, oal_pcie_get_link_state_str(pst_pci_lres->ep_res.link_state), pst_pci_lres->dev_id);
        return -OAL_EBUSY;
    }
