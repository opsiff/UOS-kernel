/**
 * Copyright (c) @CompanyNameMagicTag 2021-2023. All rights reserved.
 *
 * Description:pcie iatu
 * Author: @CompanyNameTag
 */

#ifdef _PRE_PLAT_FEATURE_GF6X_PCIE
#define MPXX_LOG_MODULE_NAME "[PCIE_H]"
#define HISI_LOG_TAG           "[PCIE]"
#include <linux/module.h>
#include <linux/kernel.h>
#include <linux/device.h>
#include <linux/slab.h>
#include <linux/types.h>
#include <linux/err.h>
#include <linux/workqueue.h>
#include <linux/sched.h>
#include <linux/delay.h>
#include <linux/kthread.h>
#include "oam_ext_if.h"

#include "pcie_reg.h"
#include "oal_hcc_host_if.h"
#include "plat_firmware.h"
#include "plat_pm_wlan.h"
#include "board.h"
#include "plat_pm.h"

#include "chip/gf61/pcie_soc_gf61.h"

OAL_STATIC oal_pcie_region g_pcie_regions[] = {
    {
        .pci_start = 0x00000000,
        .pci_end   = 0x000C7FFF,
        .cpu_start = 0x00000000,
        .cpu_end   = 0x000C7FFF, /* 800KB */
        .flag      = OAL_IORESOURCE_REG,
        .name      = "GF61_WCPU_ITCM0"
    },
    {
        .pci_start = 0x00800000,
        .pci_end   = 0x008C7FFF,
        .cpu_start = 0x00800000,
        .cpu_end   = 0x008C7FFF, /* 800KB */
        .flag      = OAL_IORESOURCE_REG,
        .name      = "GF61_WCPU_ITCM1"
    },
    {
        .pci_start = 0x02000000,
        .pci_end   = 0x0207FFFF,
        .cpu_start = 0x02000000,
        .cpu_end   = 0x0207FFFF, /* 512KB */
        .flag      = OAL_IORESOURCE_REG,
        .name      = "GF61_SHARE_PKTRAM"
    },
    {
        .pci_start = 0x20000000,
        .pci_end   = 0x2007FFFF,
        .cpu_start = 0x20000000,
        .cpu_end   = 0x2007FFFF, /* 512KB */
        .flag      = OAL_IORESOURCE_REG,
        .name      = "GF61_WCPU_DTCM"
    },
    {
        .pci_start = 0x40000000,
        .pci_end   = 0x40123FFF,
        .cpu_start = 0x40000000,
        .cpu_end   = 0x40123FFF, /* 1168k */
        .flag      = OAL_IORESOURCE_REG,
        .name      = "GF61_AON_WAPB"
    },
    {
        .pci_start = 0x41000000,
        .pci_end   = 0x41027FFF,
        .cpu_start = 0x41000000,
        .cpu_end   = 0x41027FFF,
        .flag      = OAL_IORESOURCE_REG,
        .name      = "GF61_B_TRACE_MEM"
    },
    {
        .pci_start = 0x42026000,
        .pci_end   = 0x42027FFF,
        .cpu_start = 0x42026000,
        .cpu_end   = 0x42027FFF, /* 8KB */
        .flag      = OAL_IORESOURCE_REG,
        .name      = "GF61_SLE_TRACE_MEM"
    },
    {
        .pci_start = 0x44400000,
        .pci_end   = 0x4447FFFF,
        .cpu_start = 0x44400000,
        .cpu_end   = 0x4447FFFF,
        .flag      = OAL_IORESOURCE_REG,
        .name      = "GF61_SLERAM0"
    },
    {
        .pci_start = 0x44480000,
        .pci_end   = 0x444F7FFF,
        .cpu_start = 0x44480000,
        .cpu_end   = 0x444F7FFF, /* 480KB */
        .flag      = OAL_IORESOURCE_REG,
        .name      = "GF61_SLERAM1"
    },
    {
        .pci_start = 0x44600000,
        .pci_end   = 0x446C7FFF,
        .cpu_start = 0x44600000,
        .cpu_end   = 0x446C7FFF, /* 800KB */
        .flag      = OAL_IORESOURCE_REG,
        .name      = "GF61_BITCM"
    },
    {
        .pci_start = 0x44700000,
        .pci_end   = 0x44733FFF,
        .cpu_start = 0x44700000,
        .cpu_end   = 0x44733FFF, /* 208KB */
        .flag      = OAL_IORESOURCE_REG,
        .name      = "GF61_BDTCM"
    },
    {
        .pci_start = 0x50000000 + 0x2000000,
        .pci_end   = 0x500A7FFF + 0x2000000,
        .cpu_start = 0x50000000,
        .cpu_end   = 0x500A7FFF, /* 512KB + 128K + 32K */
        .flag      = OAL_IORESOURCE_REG,
        .name      = "GF61_GMEM"
    },
    {
        .pci_start = 0x500A8000,
        .pci_end   = 0x500C7fff,
        .cpu_start = 0x500A8000,
        .cpu_end   = 0x500C7fff, /* 128KB */
        .flag      = OAL_IORESOURCE_REG,
        .name      = "GF61_G_DSP0_ITCM"
    },
    {
        .pci_start = 0x500C8000,
        .pci_end   = 0x500FFFFF,
        .cpu_start = 0x500C8000,
        .cpu_end   = 0x500FFFFF, /* 224KB */
        .flag      = OAL_IORESOURCE_REG,
        .name      = "GF61_G_DSP1_ITCM"
    },
    {
        .pci_start = 0x50100000 + 0x2000000,
        .pci_end   = 0x5017FFFF + 0x2000000,
        .cpu_start = 0x50100000,
        .cpu_end   = 0x5017FFFF, /* 512KB */
        .flag      = OAL_IORESOURCE_REG,
        .name      = "GF61_G_PKTRAM"
    },
    {
        .pci_start = 0x50800000,
        .pci_end   = 0x508BFFFF,
        .cpu_start = 0x50800000,
        .cpu_end   = 0x508BFFFF, /* 768KB */
        .flag      = OAL_IORESOURCE_REG,
        .name      = "GF61_G_DSP0_DTCM"
    },
    {
        .pci_start = 0x50900000,
        .pci_end   = 0x509BFFFF,
        .cpu_start = 0x50900000,
        .cpu_end   = 0x509BFFFF, /* 768KB */
        .flag      = OAL_IORESOURCE_REG,
        .name      = "GF61_G_DSP1_DTCM"
    },
    {
        .pci_start = 0x51C4C000,
        .pci_end   = 0x51C4CFFF,
        .cpu_start = 0x51C4C000,
        .cpu_end   = 0x51C4CFFF,
        .flag      = OAL_IORESOURCE_REG,
        .name      = "GF61_G_PCIE_HOST_CTRL"
    },
    {
        .pci_start = 0x51C42000,
        .pci_end   = 0x51C42FFF,
        .cpu_start = 0x51C42000,
        .cpu_end   = 0x51C42FFF, /* 4KB */
        .flag      = OAL_IORESOURCE_REG,
        .name      = "GF61_G_CTL"
    },
    {
        .pci_start = 0x53107000,
        .pci_end   = 0x5310CFFF,
        .cpu_start = 0x53107000,
        .cpu_end   = 0x5310CFFF,
        .flag      = OAL_IORESOURCE_REG,
        .name      = "GF61_G_PCIE_SUB"
    },
    {
        .pci_start = 0x51E00000,
        .pci_end   = 0x51E0AFFF,
        .cpu_start = 0x51E00000,
        .cpu_end   = 0x51E0AFFF, /* 44KB */
        .flag      = OAL_IORESOURCE_REG,
        .name      = "GF61_DSP0_CTL"
    },
    {
        .pci_start = 0x51F00000,
        .pci_end   = 0x51F0AFFF,
        .cpu_start = 0x51F00000,
        .cpu_end   = 0x51F0AFFF, /* 44KB */
        .flag      = OAL_IORESOURCE_REG,
        .name      = "GF61_DSP1_CTL"
    },
};
int32_t oal_pcie_get_bar_region_info_gf61(oal_pcie_linux_res *pcie_res,
                                          oal_pcie_region **region_base, uint32_t *region_num)
{
    oal_print_mpxx_log(MPXX_LOG_INFO, "gf61 %s region map", mpxx_is_asic() ? "asic" : "fpga");
    *region_num = (uint32_t)oal_array_size(g_pcie_regions);
    *region_base = &g_pcie_regions[0];

    return OAL_SUCC;
}

int32_t oal_pcie_set_outbound_membar_gf61(oal_pcie_linux_res *pcie_res, oal_pcie_iatu_bar *pst_iatu_bar)
{
    int32_t ret;
    ret = oal_pcie_set_outbound_iatu_by_membar(pst_iatu_bar->st_region.vaddr,
                                               0, HISI_PCIE_SLAVE_START_ADDRESS,
                                               HISI_PCIE_MASTER_START_ADDRESS,
                                               HISI_PCIE_IP_REGION_SIZE);
    if (ret) {
        return ret;
    }
    return OAL_SUCC;
}
#endif
