...
OAL_STATIC int32_t oal_pcie_print_pcie_regs(oal_pcie_linux_res *pst_pci_res, uint32_t base, uint32_t size)
{
    ...
    for (i = 0; i < (int32_t)size; i += (int32_t)sizeof(uint32_t)) { /* 每次偏移4字节 */
        value = oal_readl((void *)(uintptr_t)(addr_map.va + i));
        if (value == 0xffffffff && (oal_pcie_device_check_alive(pst_pci_res) != OAL_SUCC)) {
            if (pst_mem != NULL) {
                vfree(pst_mem);
            }
            return -OAL_ENODEV;
...
#ifdef CONFIG_PRINTK
            /* print to kenrel msg, 32B per, groupsize is 4 */
...
void oal_pcie_print_transfer_info(oal_pcie_linux_res *pst_pci_res, uint64_t print_flag)
{
    ...
    if (oal_unlikely(pst_pci_res->ep_res.link_state >= PCI_WLAN_LINK_WORK_UP)) {
        ...
        pci_print_log(PCI_LOG_INFO, "pcie is %s", g_pcie_link_state_str[pst_pci_res->ep_res.link_state]);
    }
}
...
void oal_pcie_regions_info_dump(oal_pcie_comm_res *comm_res)
{
    ...
    for (index = 0; index < region_num; index++, region_base++) {
        oal_io_print("[%15s]va:0x%llx, pa:0x%llx, [pci start:0x%8llx end:0x%8llx],[cpu start:0x%8llx end:0x%8llx],\
                     size:%u, flag:0x%x\n",
                     region_base->name,
                     (uint64_t)(uintptr_t)(region_base->vaddr),
                     region_base->paddr,
