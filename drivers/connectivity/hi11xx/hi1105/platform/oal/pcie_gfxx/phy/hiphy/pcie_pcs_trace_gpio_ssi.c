 ...
 *
 * Description:pcie pcs trace driver, used by gpio-ssi
 * Author: @CompanyNameTag
 */

#ifdef _PRE_PLAT_FEATURE_GF6X_PCIE
#define MPXX_LOG_MODULE_NAME "[PCS_TRACE]"
#define HISI_LOG_TAG           "[PCS_TRACE]"
#define MPXX_LOG_MODULE_NAME_VAR pcs_trace_loglevel
...
static pcs_trace_conf g_pcs_conf_table_dump_all[] = {
    // reg_trace_trigger_port_sel lane num set by caller
    { 0x900, 0,  0,  0x0},          // set reg_trace_start : 0
    { 0x900, 2,  2,  0x0},          // reg_trace_trigger_port_sel : lane0
    { 0x904, 19, 15, 0x0},          // trigger_start_mask[31:0]
    { 0x904, 31, 16, 0x0},          // trigger_start_mask[31:0]
    { 0x90C, 31, 0,  0x0},          // start_trigger_value[31:0]
    { 0x9A8, 31, 16, 0x0},          // start_trigger_value[47:32]
    { 0x908, 31, 0,  0x0},          // trigger_stop_mask[31:0]
    { 0x9A4, 14, 0,  0x0},          // trigger_stop_mask[47:32]
    { 0x910, 31, 0,  0xFFFFFFFF},   // stop_trigger_value[31:0]
    { 0x9A8, 15, 0,  0x7FFF},       // stop_trigger_value[47:32]
    { 0x900, 11, 8,  0x0},          // reg_trace_state_sel : 0
    { 0x900, 1,  1,  0x0},          // reg_cap_mode
    { 0x918, 31, 0,  0xf0020fc0},   // state_mask[31:0]
    { 0x91C, 31, 0,  0x41},         // state_mask[63:32]
    { 0x920, 31, 0,  0xfff80000},   // state_mask[95:64]
    { 0x924, 9,  8,  0x3},          // state_mask[97:96]
    { 0x900, 7,  4,  0x0},          // reg_state_cnt_pulse_sel
    { 0x900, 0,  0,  0x1},          // set reg_trace_start : 1
...
static pcs_trace_conf* pcie_pcs_get_tracer_conf(pcs_tracer_dfx_type type)
{
    switch (type) {
        ...
        case PCS_TRACER_LINK:
            ...
            break;
        default:
            oal_print_mpxx_log(MPXX_LOG_ERR, "unsupport pcs tracer type=%d", type);
    }
    oal_print_mpxx_log(MPXX_LOG_ERR, "mismatch pcs conf=%d", type);
    return NULL;
}
                              ...
                              uint32_t dump_type)
{
    ...
    if (oal_warn_on(base_addr == (uintptr_t)NULL)) {
        oal_print_mpxx_log(MPXX_LOG_ERR, "base_addr is null");
        return -OAL_EINVAL;
    }
...
void pcie_pcs_tracer_dump(uintptr_t base_addr, uint32_t lane, uint32_t dump_type)
{
    ...
    if (buff == NULL) {
        oal_print_mpxx_log(MPXX_LOG_ERR, " mem alloc failed size=%d", PCIE_PCS_TRACER_LOGSIZE);
        return;
    }
    ret_err = memset_s(buff, PCIE_PCS_TRACER_LOGSIZE, 0, PCIE_PCS_TRACER_LOGSIZE);
    if (ret_err != EOK) {
        oal_print_mpxx_log(MPXX_LOG_ERR, " memset_s failed=%d", ret_err);
        oal_free(buff);
        return;
    }
    ...
    pcie_pcs_get_bitops(&set_bits32, &get_bits32, dump_type);
    ret = pcie_pcs_get_traceinfo(base_addr, set_bits32, get_bits32,
                                 buff, PCIE_PCS_TRACER_LOGSIZE);
    pcie_pcs_ref_put(dump_type);
    if (ret != OAL_SUCC) {
        oal_print_mpxx_log(MPXX_LOG_ERR, " pcie_pcs_get_traceinfo failed=%d", ret);
        oal_free(buff);
        return;
    }
