...
struct pcie_cali_ctle_result_str {
    uint32_t sel_c;
    uint32_t rx_addr;
    uint32_t sel_r;
    uint32_t sel_gain;
...
static void pcie_pcs_deye_cali_ctle_impl(pcs_reg_ops *reg_ops, uintptr_t base_addr, uint8_t *buff, int32_t size,
                                         struct pcie_cali_ctle_result_str *result)
{
    uint32_t value;

    value = pcie_pcs_deye_cali_debug_read(reg_ops, base_addr, 0x0);
    /* mode SEL_C get pmac_dbg_muxout bit[8:3] total 5bits */
    result->sel_c = pcie_pcs_get_bits_from_value(value, 3, 5);
    /* mode SEL_C get pmac_dbg_muxout low 3bit */
    result->rx_addr = pcie_pcs_get_bits_from_value(value, 0, 3);

    value = pcie_pcs_deye_cali_debug_read(reg_ops, base_addr, 0x1);
    /* mode SEL_R get pmac_dbg_muxout bit[8:4] total 4bits */
    result->sel_r = pcie_pcs_get_bits_from_value(value, 4, 4);
    /* mode SEL_R get pmac_dbg_muxout low 4bit */
    result->sel_gain = pcie_pcs_get_bits_from_value(value, 0, 4);

    value = pcie_pcs_deye_cali_debug_read(reg_ops, base_addr, 0x2);
    /* mode EN_CTLE_VCM get pmac_dbg_muxout bit[4:3] */
    result->gen = pcie_pcs_get_bits_from_value(value, 3, 1);

    value = pcie_pcs_deye_cali_debug_read(reg_ops, base_addr, 0x5);
    /* mode EN_OS_K,EQ_OS,OS_SWQP,RG_CTLE_OS get pmac_dbg_muxout low 5bit */
    result->ctle_os = pcie_pcs_get_bits_from_value(value, 0, 5);

    value = pcie_pcs_deye_cali_debug_read(reg_ops, base_addr, 0x6);
    /* mode EN_OS_K,OS_SEL,E0B_OS_CAL,E0_OS get pmac_dbg_muxout low 5bit */
    result->e0_os = pcie_pcs_get_bits_from_value(value, 0, 5);

    value = pcie_pcs_deye_cali_debug_read(reg_ops, base_addr, 0x7);
    /* mode EN_GAIN_K,GS_CAL_EN,GAIN_VTH_D1_DIG,GS0_OS get pmac_dbg_muxout low 5bit */
    result->gs_os = pcie_pcs_get_bits_from_value(value, 0, 5);
}

#define pcie_pcs_update_offset(offset, ret, buff, size, string, base_addr)                                         \
    do {                                                                                                \
        ret = snprintf_s((char *)(buff + offset), size - offset, size - offset - 1, string, base_addr); \
        offset += ret;                                                                                  \
    } while (0)

int32_t pcie_cali_ctle_result_display(struct pcie_cali_ctle_result_str *result, uintptr_t base_addr, uint8_t *buff,
                                      int32_t size)
{
    int32_t ret = 0;
    int32_t offset = 0;

    pcie_pcs_update_offset(offset, ret, buff, size,
                           "********  PCIe idx:0x%x ctle eq cali result show  ********" NEWLINE, base_addr);
    if (ret <= 0) {
        return offset;
    }
    pcie_pcs_update_offset(offset, ret, buff, size, "sel_c = 0x%x" NEWLINE, result->sel_c);
    if (ret <= 0) {
        return offset;
    }
    pcie_pcs_update_offset(offset, ret, buff, size, "rx_att = 0x%x" NEWLINE, result->rx_addr);
    if (ret <= 0) {
        return offset;
    }
    pcie_pcs_update_offset(offset, ret, buff, size, "sel_r = 0x%x" NEWLINE, result->sel_r);
    if (ret <= 0) {
        return offset;
    }
    pcie_pcs_update_offset(offset, ret, buff, size, "sel_gain = 0x%x" NEWLINE, result->sel_gain);
    if (ret <= 0) {
        return offset;
    }
    pcie_pcs_update_offset(offset, ret, buff, size, "gen = 0x%x" NEWLINE, result->gen);
    if (ret <= 0) {
        return offset;
    }
    pcie_pcs_update_offset(offset, ret, buff, size, "ctle_os = 0x%x" NEWLINE, result->ctle_os);
    if (ret <= 0) {
        return offset;
    }
    pcie_pcs_update_offset(offset, ret, buff, size, "e0_os = 0x%x" NEWLINE, result->e0_os);
    if (ret <= 0) {
        return offset;
    }
    pcie_pcs_update_offset(offset, ret, buff, size, "gs_os = 0x%x" NEWLINE, result->gs_os);
    if (ret <= 0) {
        return offset;
    }
    return offset;
...
{
    struct pcie_cali_ctle_result_str cali_ctle_result;

    pcie_pcs_deye_cali_ctle_impl(reg_ops, base_addr, buff, size, &cali_ctle_result);
    return pcie_cali_ctle_result_display(&cali_ctle_result, base_addr, buff, size);
}

static uint32_t g_deye_pattern_result[EYE_PATTERN_ARRY_SIZE][EYE_PATTERN_ARRY_SIZE];

static void pcie_pcs_deye_pattern_test_impl(pcs_reg_ops *reg_ops, uintptr_t base_addr, uint32_t lane)
{
    uint32_t pi, vref;
...
static int32_t pcie_pcs_deye_patter_print(int32_t *offset_ret, uint8_t *buff, int32_t size)
{
    int i, j, k;
    int32_t ret;
    int32_t offset = *offset_ret;
                                   ...
                                   int32_t size)
{
    ...
    pcie_pcs_deye_pattern_test_impl(reg_ops, base_addr, lane);
    ...
    offset += ret;
    ret = pcie_pcs_deye_patter_print(&offset, buff, size);
    if (ret <= 0) {
        return -OAL_ENOMEM;
    }
    ret = snprintf_s((char *)(buff + offset), size - offset, size - offset - 1, NEWLINE);
