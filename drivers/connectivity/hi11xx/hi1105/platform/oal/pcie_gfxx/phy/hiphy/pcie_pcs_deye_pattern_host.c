 ...
 *
 * Description:pcie_pcs_deye_pattern_host.c
...
int32_t pcie_pcs_deye_pattern_host_test(uintptr_t base_addr, uint32_t lane, uint32_t dump_type)
{
    ...
    if (buff == NULL) {
        oal_print_mpxx_log(MPXX_LOG_ERR, " mem alloc failed size=%d", PCIE_PCS_DEYE_PARTTERN_SIZE);
        return -OAL_ENOMEM;
    }
    ...
    if (ret != OAL_SUCC) {
        oal_print_mpxx_log(MPXX_LOG_ERR, " pcie_pcs_deye_pattern_draw failed=%d", ret);
        oal_free(buff);
        return ret;
    }
