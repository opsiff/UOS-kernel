 ...
 *
 * Description:pcie pcs host driver
...
static int32_t oal_pcie_device_phy_ffe_check_match_info(oal_pcie_linux_res *pst_pci_res, uint32_t base_addr,
...
static int32_t oal_pcie_device_phy_set_ffe(oal_pcie_linux_res *pst_pci_res, uint32_t base_addr,
                                           uint32_t pre, uint32_t post)
{
...
static int32_t oal_pcie_device_phy_ffe_config(oal_pcie_linux_res *pst_pci_res, uint32_t base_addr)
{
    int32_t ret;
...
void oal_pcie_device_phy_config_fixup(oal_pcie_linux_res *pst_pci_res, uint32_t base_addr)
{
    int32_t i = 0;
    unsigned long flags;
    oal_list_entry_stru *pst_pos = NULL;
...
void oal_pcie_device_phy_disable_l1ss_rekey(oal_pcie_linux_res *pst_pci_res, uint32_t base_addr)
{
    oal_pcie_device_phy_config_reg(pst_pci_res,
                                   (base_addr + PCIE_PHY_UPDTCTRL_OFFSET), 8, 3, 0x0); /* bit[10:8] 3位写0，关闭re_k */
}

#define PCIE_RX_GAIN_START_POS_BIT 8
#define PCIE_RX_GAIN_BITS_NUM 3
static void oal_pcie_device_phy_config_single_lane_from_ini(oal_pcie_linux_res *pst_pci_res, uint32_t base_addr)
{
    /* 修改SEL_R默认值: default sel_r:0xb, 设置为0x9 */
    if (g_pcs_sel_r != PCIE_PHY_DEFALUT_FLAG) {
        /* bit[9] 写1，select SEL_R manual mode */
        oal_pcie_device_phy_config_reg(pst_pci_res, (base_addr + PCIE_PHY_RX_AFE_CFG_REG_OFFSET), 9, 1, 0x1);
        /* bit[19:16]，4 bits Rx Gain Stage SEL_R settings */
    ...
    if (g_pcs_atten != PCIE_PHY_DEFALUT_FLAG) {
        /* bit[4] 写1，select ATT GAIN manual mode */
        oal_pcie_device_phy_config_reg(pst_pci_res, (base_addr + PCIE_PHY_RX_AFE_CFG_REG_OFFSET), 4, 1, 0x1);
        /* bit[19:16]，3bits Rx Gain Stage SEL_R settings */
        oal_pcie_device_phy_config_reg(pst_pci_res, (base_addr + PCIE_PHY_RXGSACTRL_OFFSET), PCIE_RX_GAIN_START_POS_BIT,
                                       PCIE_RX_GAIN_BITS_NUM, g_pcs_atten);
    }
}

#define PCIE_SCALE_BITS_NUM 5
void oal_pcie_device_phy_config_single_lane(oal_pcie_linux_res *pst_pci_res, uint32_t base_addr)
{
    ...
    /* PLL TCLP Iref config Value: bit[1:4] & &
     * Scale bit[5] I = (Value * 20uA + 200uA) * (Scale + 1) I:200uA(default) I:1mA(max) */
    oal_pcie_device_phy_config_reg(pst_pci_res, (base_addr + PCIE_PHY_ICP_CTRL_VAL_OFFSET), 1, PCIE_SCALE_BITS_NUM,
                                   0x1f);
