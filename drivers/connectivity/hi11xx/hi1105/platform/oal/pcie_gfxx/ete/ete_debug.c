...
static int32_t oal_pcie_ete_memtrans_test_wait_done(oal_pcie_linux_res *pst_pcie_res)
{
    ...
    ete_memcpy_test_result *result =
        (ete_memcpy_test_result *)((uintptr_t)g_minfo_dma_vaddr + ETE_MEMCPY_IINFO_RESULT_OFFSET);
    forever_loop() {
        if (oal_unlikely(pst_pcie_res->ep_res.link_state < PCI_WLAN_LINK_WORK_UP)) {
            oal_print_mpxx_log(MPXX_LOG_ERR, "link err, link_state:%s",
                               oal_pcie_get_link_state_str(pst_pcie_res->ep_res.link_state));
            return -OAL_ENODEV;
        }
                                          ...
                                          const char *buf)
{
    ...
    /*
     * 使用GT测试时，直接使用asic_test版本的FW，不会初始化GT业务；使用WIFI测试时，参考mpxx ete_debug.c,
