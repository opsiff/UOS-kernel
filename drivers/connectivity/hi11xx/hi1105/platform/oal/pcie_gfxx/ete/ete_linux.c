...
OAL_STATIC int32_t oal_pcie_ete_poweroff_pre(hcc_bus *pst_bus)
{
    int32_t ret;
    oal_pcie_linux_res *pst_pci_lres;
    ...
    if (pst_pci_lres->ep_res.chip_info.cb.pcie_poweroff_pre == NULL) {
        return -OAL_ENODEV;
    }
    ...
    /* device收到下电消息后会关闭clkpll, 此时若业务mac中断未处理完可能会导致dev总线挂死，故先屏蔽intx中的业务中断 */
    ret = pst_pci_lres->ep_res.chip_info.cb.pcie_poweroff_pre(pst_pci_lres);
    if (ret != OAL_SUCC) {
        return ret;
    }
...
OAL_STATIC hcc_bus_opt_ops g_pcie_ete_opt_ops = {
    ...
    .poweroff_pre = oal_pcie_ete_poweroff_pre,
