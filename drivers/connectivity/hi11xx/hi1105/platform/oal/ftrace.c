 ...
 *
 * Description:function trace functions
...
void notrace cyg_profile_func_enter(void *func, void *caller)
{
    int cpu_id = get_cpu();
...
EXPORT_SYMBOL(cyg_profile_func_enter);
...
void notrace cyg_profile_func_exit(void *func, void *caller)
{
    int cpu_id = get_cpu();
    if (oal_any_null_ptr2(func, caller)) {
        return;
    }
...
EXPORT_SYMBOL(cyg_profile_func_exit);
