                                     ...
                                     oal_netbuf_stru *netbuf, struct hcc_transfer_param *param)
{
    ...
    /* should be 1 byte */
    pad_hdr = (oal_uint)HCC_HDR_RESERVED_MAX_LEN - param->extend_len;

    headroom = (oal_uint)HCC_HDR_LEN + pad_hdr + pad_payload;
    ...
    if (oal_unlikely(!oal_is_aligned((uintptr_t)hdr, sizeof(uint32_t)))) { /* 判断是不是4字节对齐 */
        oal_print_mpxx_log(MPXX_LOG_ERR, "hdr is not aligned len:%d", (int)sizeof(uint32_t));
        return -OAL_EFAIL;
    }
...
OAL_STATIC int32_t hcc_rx(struct hcc_handler *hcc, oal_netbuf_stru *netbuf)
{
    ...
    extend_len = HCC_HDR_TOTAL_LEN - (int32_t)HCC_HDR_LEN - hdr->pad_hdr;
...
{
    uint16_t us_burst_limit = 0;
    uint16_t us_low_waterline = 0;
    uint16_t us_high_waterline = 0;
...
struct hcc_handler *hcc_module_init(hcc_bus_dev *pst_bus_dev)
{
    ...
    hcc->tx_max_buf_len = OAL_ROUND_DOWN((uint32_t)HSDIO_HOST2DEV_PKTS_MAX_LEN,
                                         (uint32_t)HISDIO_H2D_SCATT_BUFFLEN_ALIGN);
    /* one byte,2^8=256 */
    ...
    hcc->hdr_rever_max_len = (oal_uint)HCC_HDR_RESERVED_MAX_LEN;
