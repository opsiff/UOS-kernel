...
#define INI_PCIE_DEBUG_MODE "pcie_debug_mode"
...
OAL_STATIC uint32_t g_pcie_intx_idle_bind_cpu = 0;
oal_debug_module_param(g_pcie_intx_idle_bind_cpu, uint, S_IRUGO | S_IWUSR);

OAL_STATIC uint32_t pcie_scatt_list_assemble = 0;
#endif
...
OAL_STATIC void oal_pcie_intx_bind_cpu(oal_pci_dev_stru *pci_dev, uint32_t cpu_num);
...
uint32_t oal_get_pcie_scatt_list_assemble_flag(void)
{
    return pcie_scatt_list_assemble;
}
...
OAL_STATIC int32_t oal_pcie_register_int(oal_pci_dev_stru *pst_pci_dev, oal_pcie_linux_res *pst_pci_lres)
{
    ...
    if (!pst_pci_lres->st_msi.is_msi_support) {
        ...
        pst_pci_lres->irq_stat = 0;
        /* 注册成功后立刻绑定至设置CPU */
        pci_print_log(PCI_LOG_INFO, "request bind_intx_to_cpu%u", g_pcie_intx_idle_bind_cpu);
        oal_pcie_intx_bind_cpu(pst_pci_dev, g_pcie_intx_idle_bind_cpu);
    }
...
OAL_STATIC int32_t oal_pcie_suspend(oal_pci_dev_stru *pst_pci_dev, oal_pm_message_t state)
{
    ...
    oal_io_print("oal_pcie_suspend, event[%d]\n", state.event);
    if (pst_pci_lres->pst_pci_res->chip_info.edma_support == OAL_TRUE) {
...
OAL_STATIC void oal_pcie_intx_bind_cpu(oal_pci_dev_stru *pci_dev, uint32_t cpu_num)
{
    ...
    int32_t ret;
    /* 绑核时需再次校验 */
    if (cpu_num < OAL_BUS_MAXCPU_NUM) {
        ret = irq_set_affinity_hint(pci_dev->irq, cpumask_of(cpu_num));
        if (ret != OAL_SUCC) {
            pci_print_log(PCI_LOG_ERR, "intx_irq bind fail, ret = %d", ret);
        }
    } else {
        pci_print_log(PCI_LOG_ERR, "bind cpu[%u] invalid", cpu_num);
    }
#else
    pci_print_log(PCI_LOG_ERR, "not allow bind...");
#endif
}
                                         ...
                                         int32_t is_bind)
{
    ...
    if (bindcpu_cfg->irq_cpu && pci_dev->irq != 0) {
        if (bindcpu_cfg->is_bind) {
            oal_pcie_intx_bind_cpu(pci_dev, first_big_cpu);
        } else {
            /* 2.6.35-rc1 */
            oal_pcie_intx_bind_cpu(pci_dev, g_pcie_intx_idle_bind_cpu);
        }
                                         ...
                                         uint8_t thread_cmd)
{
    ...
    oal_pcie_intx_bind_cpu(pci_dev, (irq_cpu == 0 ? g_pcie_intx_idle_bind_cpu : (uint32_t)irq_cpu));
#endif
...
int32_t oal_pcie_reinit(hcc_bus *pst_bus)
{
    ...
    // pcie 中断重新绑核
    oal_pcie_intx_bind_cpu(pst_pci_lres->pst_pcie_dev, g_pcie_intx_idle_bind_cpu);
...
int32_t oal_pcie_patch_read(hcc_bus *pst_bus, uint8_t *buff, int32_t len, uint32_t timeout)
{
    ...
    mutex_lock(&pst_pci_lres->pst_pci_res->st_rx_mem_lock);
    /* 检查链路状态 */
    if (oal_pcie_check_link_state(pst_pci_lres->pst_pci_res) != OAL_TRUE) {
        mutex_unlock(&pst_pci_lres->pst_pci_res->st_rx_mem_lock);
        return -OAL_EFAUL;
    }
...
OAL_STATIC void oal_pcie_scatt_list_assemble_init(void)
{
    int32_t cfg_value = 0;
    int32_t ret;

    /* 获取ini的配置值 */
    ret = get_cust_conf_int32(INI_MODU_PLAT, "pcie_scatt_list_assemble", &cfg_value);
    if (ret == INI_FAILED) {
        return;
    }

    if (cfg_value != 0) {
        pcie_scatt_list_assemble = OAL_TRUE;
    } else {
        pcie_scatt_list_assemble = OAL_FALSE;
    }

    pci_print_log(PCI_LOG_INFO, "pcie_scatt_list_assemble = %u", pcie_scatt_list_assemble);
}

OAL_STATIC void oal_pcie_idle_intx_bind_init(void)
{
    int32_t cfg_value = 0;
    int32_t ret;

    /* 获取ini的配置值 */
    ret = get_cust_conf_int32(INI_MODU_PLAT, "pcie_intx_idle_bindcpu", &cfg_value);
    if (ret == INI_FAILED) {
        pci_print_log(PCI_LOG_INFO, "pcie_intx_idle_bindcpu not set");
        return;
    }

    if ((cfg_value < 0) || (cfg_value >= OAL_BUS_MAXCPU_NUM)) {  /* 0:cpu0 */
        pci_print_log(PCI_LOG_ERR, "invalid cpu[%d]\n", cfg_value);
        return;
    }

    g_pcie_intx_idle_bind_cpu = (uint32_t)cfg_value;
    pci_print_log(PCI_LOG_INFO, " pcie_intx_idle_bindcpu[%u]", g_pcie_intx_idle_bind_cpu);
}
...
static void oal_pcie_ini_parameters_init(void)
{
    ...
    oal_pcie_idle_intx_bind_init();
    oal_pcie_phy_ini_config_init();
    oal_pcie_scatt_list_assemble_init();
}
...
#endif
    if (ret != OAL_SUCC) {
        if (get_cust_conf_int32(INI_MODU_PLAT, INI_PCIE_DEBUG_MODE, &debug_value) != INI_SUCC) {
            oal_print_mpxx_log(MPXX_LOG_INFO, "ini is not config pcie_debug_mode\n");
        }
        if (debug_value != OAL_TRUE) {
            hi_wlan_power_set(0);
        }
    }
