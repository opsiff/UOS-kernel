                                          ...
                                          uint32_t cpu_address, uint32_t length)
{
    ...
    for (index = 0; index < length; index += (uint32_t)sizeof(uint32_t)) { /* 每次偏移4字节 */
        uint32_t reg = 0;
                                         ...
                                         uintptr_t host_address, uint32_t length)
{
    ...
    for (index = 0; index < length; index += (uint32_t)sizeof(uint32_t)) {
        /* 这里有可能保存的是寄存器区域，按4字节对齐访问 */
                                            ...
                                            uint32_t size, uint32_t data)
{
    ...
    for (i = 0, mode = 0; i < (size); i += (unsigned int)sizeof(uint32_t), mode++) { /* 每次偏移4字节，直到超过size */
