...
OAL_STATIC oal_bool_enum oal_pcie_device_phy_addr_range_vaild(uint16_t addr)
{
    ...
    oal_print_mpxx_log(MPXX_LOG_ERR, "invaild device phy addr:0x%x", addr);
                                             ...
                                             uint16_t addr, uint16_t value)
{
    ...
    for (i = 0; i < PCIE_EP_PHY_RW_TIMEOUT_CNT; i++) {
        ...
        if (phy_cfg.bits.phy_config_rdy == 1) { /* write done */
            oal_print_mpxx_log(MPXX_LOG_INFO, "write phy done addr:0x%x value=0x%x loop=%d", addr, value, i);
            return OAL_SUCC;
        }
    }

    oal_print_mpxx_log(MPXX_LOG_ERR, "write phy addr:0x%x value=0x%x timeout=%d", addr, value, i);
    ...
    for (i = 0; i < PCIE_EP_PHY_RW_TIMEOUT_CNT; i++) {
        ...
        if (phy_cfg.bits.phy_config_rdy == 1) { /* read done */
            ...
            *pst_value = (uint16_t)phy_rd_data.bits.phy_cr_rd_data;
            oal_print_mpxx_log(MPXX_LOG_INFO, "read phy done addr:0x%x value=0x%x loop=%d", addr, *pst_value, i);
            return OAL_SUCC;
        }
    }

    oal_print_mpxx_log(MPXX_LOG_ERR, "read phy addr:0x%x timeout=%d", addr, i);
    return -OAL_ETIMEDOUT;
}
...
OAL_STATIC int32_t oal_pcie_device_phy_los_en_ovrd(oal_pcie_res *pst_pci_res)
{
    ...
    ret = oal_pcie_device_phy_read(pst_pci_res, PCIE_PHY_LANEN_DIG_RX_OVRD_IN_LO_OFF, &value);
    if (ret == OAL_SUCC) {
        oal_print_mpxx_log(MPXX_LOG_INFO, "PCIE_PHY_LANEN_DIG_RX_OVRD_IN_LO=0x%x", value);
    }
...
OAL_STATIC int32_t oal_pcie_device_phy_config_bugfix(oal_pcie_res *pst_pci_res)
{
    ...
    if (pst_pci_dev->device == PCIE_DEVICE_ID_MP15) { /* mp15 soc bugfix */
        ret = oal_pcie_device_phy_los_en_ovrd(pst_pci_res);
        if (ret != OAL_SUCC) {
            oal_print_mpxx_log(MPXX_LOG_INFO, "pcie phy los en ovrd failed=%d", ret);
            return ret;
        }
        oal_print_mpxx_log(MPXX_LOG_INFO, "pcie phy los en ovrd succ");
    }

    oal_print_mpxx_log(MPXX_LOG_DBG, "pcie device id:0x%x", pst_pci_dev->device);
