                                                   ...
                                                   uintptr_t start, unsigned long length, uint8_t test_value)
{
    ...
    forever_loop() {
        ...
        /* memcheck */
        for (i = 0; i < copy_size; i += (uint32_t)sizeof(uint32_t)) {
                                                  ...
                                                  uintptr_t start, unsigned long length, uint8_t test_value)
{
    ...
    for (i = 0; i < (uint32_t)length; i += (uint64_t)sizeof(uint32_t)) { /* 每次偏移4字节 */
                                                             ...
                                                             unsigned long length)
{
    ...
    forever_loop() {
        ...
        for (i = 0; i < copy_size; i += (unsigned long)sizeof(uint32_t)) {      /* 每次偏移4字节 */
                                                      ...
                                                      uint32_t *pst_scan_nums)
{
    ...
    *pst_scan_nums = 0;
    if (get_mpxx_subchip_type() == BOARD_VERSION_MP15) {
        ...
        if (mpxx_is_asic()) {
            *pst_scan_nums = (uint32_t)sizeof(g_mp15_asic_wmem_scan_array) /
                             (PCIE_MEM_MSG_SIZE * (uint32_t)sizeof(g_mp15_asic_wmem_scan_array[0][0]));
            *pst_scan_base = g_mp15_asic_wmem_scan_array;
        } else {
            *pst_scan_nums = (uint32_t)sizeof(g_mp15_fpga_wmem_scan_array) /
                             (PCIE_MEM_MSG_SIZE * (uint32_t)sizeof(g_mp15_fpga_wmem_scan_array[0][0]));
            *pst_scan_base = g_mp15_fpga_wmem_scan_array;
        }
    } else if (get_mpxx_subchip_type() == BOARD_VERSION_MP16) {
        if (mpxx_is_asic()) {
            *pst_scan_nums = (uint32_t)sizeof(g_mp16_asic_wmem_scan_array) /
                             (PCIE_MEM_MSG_SIZE * (uint32_t)sizeof(g_mp16_asic_wmem_scan_array[0][0]));
            *pst_scan_base = g_mp16_asic_wmem_scan_array;
        }
    } else if (get_mpxx_subchip_type() == BOARD_VERSION_MP16C) {
        if (mpxx_is_asic()) {
            *pst_scan_nums = (uint32_t)sizeof(g_mp16c_asic_wmem_scan_array) /
                             (PCIE_MEM_MSG_SIZE * (uint32_t)sizeof(g_mp16c_asic_wmem_scan_array[0][0]));
            *pst_scan_base = g_mp16c_asic_wmem_scan_array;
        }
    } else {
        if (pst_pci_res->revision == PCIE_REVISION_4_70A) {
            *pst_scan_nums = (uint32_t)sizeof(g_mpw2_wmem_scan_array) /
                             (PCIE_MEM_MSG_SIZE * (uint32_t)sizeof(g_mpw2_wmem_scan_array[0][0]));
            *pst_scan_base = g_mpw2_wmem_scan_array;
        } else if (pst_pci_res->revision == PCIE_REVISION_5_00A) {
            *pst_scan_nums = (uint32_t)sizeof(g_pilot_wmem_scan_array) /
                             (PCIE_MEM_MSG_SIZE * (uint32_t)sizeof(g_pilot_wmem_scan_array[0][0]));
                                                      ...
                                                      uint32_t *pst_scan_nums)
{
    ...
    *pst_scan_nums = 0;
    if (get_mpxx_subchip_type() == BOARD_VERSION_MP15) {
        ...
        if (mpxx_is_asic()) {
            *pst_scan_nums = (uint32_t)sizeof(g_mp15_asic_bmem_scan_array) /
                             (PCIE_MEM_MSG_SIZE * (uint32_t)sizeof(g_mp15_asic_bmem_scan_array[0][0]));
        ...
        if (mpxx_is_asic()) {
            *pst_scan_nums = (uint32_t)sizeof(g_mp16_asic_bmem_scan_array) /
                             (PCIE_MEM_MSG_SIZE * (uint32_t)sizeof(g_mp16_asic_bmem_scan_array[0][0]));
        ...
        if (mpxx_is_asic()) {
            *pst_scan_nums = (uint32_t)sizeof(g_mp16c_asic_bmem_scan_array) /
                             (PCIE_MEM_MSG_SIZE * (uint32_t)sizeof(g_mp16c_asic_bmem_scan_array[0][0]));
            *pst_scan_base = g_mp16c_asic_bmem_scan_array;
        }
    } else {
        if (pst_pci_res->revision == PCIE_REVISION_4_70A) {
            *pst_scan_nums = (uint32_t)sizeof(g_mpw2_bmem_scan_array) /
                             (PCIE_MEM_MSG_SIZE * (uint32_t)sizeof(g_mpw2_bmem_scan_array[0][0]));
            *pst_scan_base = g_mpw2_bmem_scan_array;
        } else if (pst_pci_res->revision == PCIE_REVISION_5_00A) {
            *pst_scan_nums = (uint32_t)sizeof(g_pilot_bmem_scan_array) /
                             (PCIE_MEM_MSG_SIZE * (uint32_t)sizeof(g_pilot_bmem_scan_array[0][0]));
                                                      ...
                                                      uint32_t *pst_scan_nums)
{
    if (get_mpxx_subchip_type() == BOARD_VERSION_MP16) {
        ...
        if (mpxx_is_asic()) {
            *pst_scan_nums = (uint32_t)sizeof(g_mp16_asic_gmem_scan_array) /
                             (PCIE_MEM_MSG_SIZE * (uint32_t)sizeof(g_mp16_asic_gmem_scan_array[0][0]));
        ...
        if (mpxx_is_asic()) {
            *pst_scan_nums = (uint32_t)sizeof(g_mp16c_asic_gmem_scan_array) /
                             (PCIE_MEM_MSG_SIZE * (uint32_t)sizeof(g_mp16c_asic_gmem_scan_array[0][0]));
                                           ...
                                           uint32_t data_size)
{
    ...
    if (oal_unlikely(((uintptr_t)ddr_address & 0x3) || (data_size & 0x3))) {
        oal_print_mpxx_log(MPXX_LOG_ERR, "ddr address 0x%llx, length 0x%lx is invalid",
                           (uint64_t)(uintptr_t)ddr_address, length);
        return -OAL_EINVAL;
    }
