...
#define MPXX_LOG_MODULE_NAME "[ETE_DBG_H]"
...
static int32_t oal_get_pcie_ete_memtrans_addr(oal_pcie_res *pst_pcie_res, uint32_t reg_low, uint32_t reg_high,
                                              pci_addr_map *reg_low_map, pci_addr_map *reg_high_map)
{
    int32_t ret;

    ret = oal_pcie_inbound_ca_to_va(pst_pcie_res, reg_low, reg_low_map);
    if (ret != OAL_SUCC) {
        oal_print_mpxx_log(MPXX_LOG_ERR, "reg_low 0x%x convert failed!", reg_low);
        return ret;
    }

    ret = oal_pcie_inbound_ca_to_va(pst_pcie_res, reg_high, reg_high_map);
    if (ret != OAL_SUCC) {
        oal_print_mpxx_log(MPXX_LOG_ERR, "reg_high 0x%x convert failed!", reg_high);
        return ret;
    }
...
static int32_t oal_pcie_ete_memtrans_minfo_alloc(oal_pcie_res *pst_pcie_res, oal_pci_dev_stru *pst_pci_dev)
{
    int32_t ret;
    ...
    if (g_minfo_dma_vaddr == NULL) {
        oal_print_mpxx_log(MPXX_LOG_ERR, "dma alloc minfo mem failed, size=%d", ETE_MEMCPY_MINFO_MSIZE);
        return -OAL_ENOMEM;
    }
    ...
    oal_print_mpxx_log(MPXX_LOG_INFO, "ete memcpy test minfo iova:0x%llx va:0x%p , pci slave:0x%llx",
...
static void oal_pcie_ete_memtrans_minfo_free(oal_pcie_res *pst_pcie_res, oal_pci_dev_stru *pst_pci_dev)
{
    if (g_minfo_dma_vaddr == NULL) {
        return;
    }
...
static int32_t oal_pcie_ete_memtrans_txbuff_alloc(oal_pcie_res *pst_pcie_res, oal_pci_dev_stru *pst_pci_dev,
                                                  uint32_t size)
{
    int32_t ret;
    g_tx_buff_dma_vaddr = dma_alloc_coherent(&pst_pci_dev->dev, size, &g_tx_buff_dma_addr, GFP_KERNEL);
    if (g_tx_buff_dma_vaddr == NULL) {
        oal_print_mpxx_log(MPXX_LOG_ERR, "dma alloc minfo mem failed, size=%d", size);
        return -OAL_ENOMEM;
    }
    ...
    oal_print_mpxx_log(MPXX_LOG_INFO, "ete memcpy test txbuf iova:0x%llx va:0x%p , pci slave:0x%llx",
                       (uint64_t)g_tx_buff_dma_addr, g_tx_buff_dma_vaddr, g_tx_buff_pci_slave_addr);

    return OAL_SUCC;
}

static void oal_pcie_ete_memtrans_txbuff_free(oal_pcie_res *pst_pcie_res, oal_pci_dev_stru *pst_pci_dev,
                                              uint32_t size)
{
    if (g_tx_buff_dma_vaddr == NULL) {
        return;
...
static int32_t oal_pcie_ete_memtrans_rxbuff_alloc(oal_pcie_res *pst_pcie_res, oal_pci_dev_stru *pst_pci_dev,
                                                  uint32_t size)
{
    int32_t ret;
    ...
    oal_print_mpxx_log(MPXX_LOG_INFO, "ete memcpy test rxbuf iova:0x%llx va:0x%p , pci slave:0x%llx",
                       (uint64_t)g_rx_buff_dma_addr, g_rx_buff_dma_vaddr, g_rx_buff_pci_slave_addr);

    return OAL_SUCC;
}

static void oal_pcie_ete_memtrans_rxbuff_free(oal_pcie_res *pst_pcie_res, oal_pci_dev_stru *pst_pci_dev,
                                              uint32_t size)
{
    if (g_rx_buff_dma_vaddr == NULL) {
        return;
...
int32_t pcie_ete_memtrans_data_check(ete_memcpy_test_minfo* test_minfo)
{
    ...
    for (i = 0; i < len; i++) {
        ...
        if (c != (uint8_t)test_minfo->data) {
            oal_print_mpxx_log(MPXX_LOG_ERR, "addr offset: 0x%x err data:0x%x expect:0x%x", i, c,
        ...
        if (*last_cnt != result->transfer_count) {
            *timeout_cnt = 0;
            ...
            if (++(*timeout_cnt) > max_timeout) {
                oal_print_mpxx_log(MPXX_LOG_ERR, "wait ete memcpy test done timeout!");
...
/* 等待device测试完成 */
static int32_t oal_pcie_ete_memtrans_test_wait_done(oal_pcie_res *pst_pcie_res)
{
    int32_t ret;
    ...
    forever_loop() {
        if (oal_unlikely(pst_pcie_res->link_state < PCI_WLAN_LINK_WORK_UP)) {
            oal_print_mpxx_log(MPXX_LOG_ERR, "link err, link_state:%s",
        ...
        if (timeout != 0) {
            ...
            oal_print_mpxx_log(MPXX_LOG_ERR, "current thread was interrupted!");
            return -OAL_ETIMEDOUT;
        }
    }

    if (result->ret != OAL_SUCC) {
        oal_print_mpxx_log(MPXX_LOG_ERR, "invalid result ret=%d from device",
                           result->ret);
        oal_print_mpxx_log(MPXX_LOG_ERR, "transfer count=%llu", result->transfer_count);
        return result->ret;
    }
...
static void oal_pcie_ete_memtrans_test_calc(void)
{
    ...
    oal_print_mpxx_log(MPXX_LOG_INFO, "Result:");
    oal_print_mpxx_log(MPXX_LOG_INFO, "ETE Memcpy Test Mode:%s",
                       oal_pcie_get_ete_memtrans_test_modestr(test_minfo->mode));
    oal_print_mpxx_log(MPXX_LOG_INFO, "ETE Transer BurstSize: %u (Bytes), burst count: %llu ",
                       test_minfo->burst_size, result->transfer_count);
    oal_print_mpxx_log(MPXX_LOG_INFO, "ETE Transer Size: %llu (Bytes)",  result->transfer_size);
    oal_print_mpxx_log(MPXX_LOG_INFO, "ETE Transer Time: %llu (ms)",  result->transfer_time);
    oal_print_mpxx_log(MPXX_LOG_INFO, "ETE Transer Throught: %llu %s",  temp >> shift, str);
    ...
    if (ret <= 0) {
        oal_print_mpxx_log(MPXX_LOG_ERR, "snprintf failed, ret=%d", ret);
    }
}
...
void oal_pcie_ete_memtrans_mode_reinit(void)
{
    ...
    if (ret <= 0) {
        oal_print_mpxx_log(MPXX_LOG_ERR, "oal_pcie_ete_memtrans_mode_reinit::snprintf failed, ret=%d", ret);
    }
}
                                                    ...
                                                    pci_addr_map *reg_low_map, pci_addr_map *reg_high_map)
{
    ...
    *high_r = oal_readl(reg_high_map->va);
    if ((*low_r != *low) || (*high_r != *high)) {
        oal_print_mpxx_log(MPXX_LOG_ERR, "low_r(0x%x) != low(0x%x) or high_r(0x%x) != high(0x%x)",
                           *low_r, *low, *high_r, *high);
        return -OAL_EFAIL;
    }
    oal_print_mpxx_log(MPXX_LOG_INFO, "set ete memcpy minfo address done");
    return OAL_SUCC;
}

static int32_t oal_pcie_ete_memtrans_para_check(oal_pcie_res *pst_pcie_res,
                                                ...
                                                pci_addr_map *reg_high_map)
{
    ...
    if ((burst_size == 0) || (burst_size > ETE_MEMCPY_BUFF_MSIZE)) {
        oal_print_mpxx_log(MPXX_LOG_ERR, "invalid burst_size %u", burst_size);
        return -OAL_EINVAL;
    }

    oal_print_mpxx_log(MPXX_LOG_INFO, "mode_int=%u , burst_size=%u, runtime=%u, reg_low=0x%x, reg_high=0x%x",
...
int32_t oal_pcie_ete_memcpy_mode_test(oal_pcie_res *pst_pcie_res, oal_pci_dev_stru *pst_pci_dev, uint32_t mode_int,
                                      uint32_t burst_size, uint32_t runtime, uint32_t reg_low, uint32_t reg_high)
{
    int32_t ret;
