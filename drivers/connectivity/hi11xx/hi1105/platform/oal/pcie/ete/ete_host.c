...
#define ETE_H2D_MAX_BUFF_LEN (HCC_HDR_TOTAL_LEN + PCIE_EDMA_TRANS_MAX_FRAME_LEN)
/* ete tx config info */
...
int32_t pcie_ete_txbuf_reset(pcie_ete_trans_txbuf *tx_buff)
{
    if (oal_get_pcie_scatt_list_assemble_flag() == OAL_FALSE) {
        return OAL_SUCC;
    }
    tx_buff->buff_rd = 0;
    tx_buff->buff_wr = 0;
    tx_buff->buff_rd_flag = 0;
    tx_buff->buff_wr_flag = 0;
    if (tx_buff->buff_cpu_addr != 0x0) {
        memset_s((void*)(uintptr_t)tx_buff->buff_cpu_addr, tx_buff->buff_size,
                 0xff, tx_buff->buff_size);
    }
    oal_print_mpxx_log(MPXX_LOG_INFO, "tx_buff has been reset");
    return OAL_SUCC;
}
...
void oal_ete_tx_res_clean(oal_pcie_res *pst_pci_res, uint32_t force_clean)
{
    ...
    for (i = 0; i < ETE_TX_CHANN_MAX_NUMS; i++) {
        ...
        /* reset tx_buf */
        (void)pcie_ete_txbuf_reset(&pst_h2d_res->tx_buff);
    }
}

void oal_ete_update_txbuff_rd(pcie_ete_trans_txbuf *tx_buff)
{
    if (oal_get_pcie_scatt_list_assemble_flag() == OAL_FALSE) {
        return;
    }
    if (tx_buff->buff_rd < (tx_buff->buff_items - 1)) {
        tx_buff->buff_rd++;
    } else {
        tx_buff->buff_rd = 0;
        tx_buff->buff_rd_flag ^= 0x1;
    }
}

void oal_ete_update_txbuff_wr(pcie_ete_trans_txbuf *tx_buff)
{
    if (tx_buff->buff_wr < (tx_buff->buff_items - 1)) {
        tx_buff->buff_wr++;
    } else {
        tx_buff->buff_wr = 0;
        tx_buff->buff_wr_flag ^= 0x1;
    }
}

int32_t pcie_ete_txbuf_freecount(pcie_ete_trans_txbuf *tx_buff)
{
    int32_t free_cnt;
    if (tx_buff->buff_wr_flag == tx_buff->buff_rd_flag) {
        free_cnt = tx_buff->buff_items - (tx_buff->buff_wr - tx_buff->buff_rd);
    } else {
        free_cnt = tx_buff->buff_rd - tx_buff->buff_wr;
    }
...
int32_t oal_ete_tx_transfer_process(oal_pcie_res *pst_pci_res, uint32_t chan_idx)
{
    ...
    if (oal_likely(trans_cnt != PCIE_ETE_INVALID_RD_VALUE)) {
        ...
        forever_loop() {
            ...
            oal_ete_update_txbuff_rd(&pst_h2d_res->tx_buff);
            oal_pcie_tx_netbuf_free(pst_pci_res, pst_netbuf);
            netbuf_trans_cnt++;
...
OAL_STATIC OAL_INLINE int32_t oal_ete_write_txbuff_by_send_buff(oal_ete_chan_h2d_res *pst_h2d_res,
                                                                oal_netbuf_stru *pst_netbuf,
                                                                uint64_t *cpu_addr, uint64_t *dst_addr,
                                                                uint32_t len)
{
    int32_t free_cnt;
    if (len > ETE_H2D_MAX_BUFF_LEN) {
        oal_print_mpxx_log(MPXX_LOG_ERR, "invalid netbuf lenth: %d", len);
        hcc_tx_netbuf_free(pst_netbuf);
        return -OAL_FAIL;
    }

    free_cnt = pcie_ete_txbuf_freecount(&pst_h2d_res->tx_buff);
    if (free_cnt <= 0) {
        return -OAL_ENOMEM;
    }

    *cpu_addr = pst_h2d_res->tx_buff.buff_cpu_addr + (ETE_H2D_MAX_BUFF_LEN * pst_h2d_res->tx_buff.buff_wr);
    *dst_addr = pst_h2d_res->tx_buff.buff_io_addr + (ETE_H2D_MAX_BUFF_LEN * pst_h2d_res->tx_buff.buff_wr);
    if (memcpy_s((void *)(uintptr_t)(*cpu_addr), len, (void *)(oal_netbuf_data(pst_netbuf)), len) != EOK) {
        oal_print_mpxx_log(MPXX_LOG_ERR, "ete tx buff write failed");
        hcc_tx_netbuf_free(pst_netbuf);
        return -OAL_FAIL;
    }

    oal_ete_update_txbuff_wr(&pst_h2d_res->tx_buff);
                                                             ...
                                                             oal_pci_dev_stru *pst_pci_dev)
{
    ...
    int32_t i;
    uint32_t len;
    uint32_t trans_assemble_flag = oal_get_pcie_scatt_list_assemble_flag();
    ...
    for (i = 0; i < (int32_t)send_cnt; i++) {
        ...
        if (trans_assemble_flag == OAL_TRUE) {
            ret = oal_ete_write_txbuff_by_send_buff(pst_h2d_res, pst_netbuf, &pci_dma_addr, &dst_addr, len);
            if (oal_unlikely(ret != OAL_SUCC)) {
                continue;
            }
            oal_print_mpxx_log(MPXX_LOG_DBG, "ete tx[%d] netbuf 0x%p, dma pa:0x%llx, dst_addr:0x%llx, len=%d",
                               pst_h2d_res->chan_idx, pst_netbuf, (uint64_t)pci_dma_addr, dst_addr, len);
        } else {
...
void oal_ete_ip_exit(oal_pcie_res *pst_pci_res)
{
    ...
    /* ete exit应与init对应，此处原有拉低ete_clk_en
     * 此时已在pcie下电流程，wifi下电或进L3后，pcie、ete模块会直接下电
     * 且此处先关闭时钟再disable中断，易引发异常，故此处删除关闭ete时钟动作 */
}
...
void oal_pcie_ete_buff_free(oal_pcie_res *pst_pci_res, size_t buff_size, void *cpu_addr, dma_addr_t iova)
{
    oal_pci_dev_stru *pst_pci_dev = pcie_res_to_dev(pst_pci_res);
    dma_free_coherent(&pst_pci_dev->dev, buff_size, cpu_addr, iova);
}

int32_t oal_pcie_ete_buff_alloc(oal_pcie_res *pst_pci_res, size_t buff_size,
                                void **cpu_addr, uint64_t *dst_addr, dma_addr_t *iova)
{
    int32_t ret;
    oal_pci_dev_stru *pst_pci_dev = pcie_res_to_dev(pst_pci_res);
    *cpu_addr = dma_alloc_coherent(&pst_pci_dev->dev, buff_size, iova, GFP_KERNEL);
    if (cpu_addr == NULL) {
        oal_print_mpxx_log(MPXX_LOG_ERR, "dma alloc buff failed, buff_size=%u", (uint32_t)buff_size);
        return -OAL_ENOMEM;
    }
    ...
    if (ret != OAL_SUCC) {
        ...
        dma_free_coherent(&pst_pci_dev->dev, buff_size, (void *)(*cpu_addr), (dma_addr_t)(*iova));
        return ret;
    }
    return OAL_SUCC;
}
...
void oal_ete_chan_tx_exit(oal_pcie_res *pst_pci_res)
{
    ...
    for (i = 0; i < ETE_TX_CHANN_MAX_NUMS; i++) {
        pst_h2d_res = &pst_pci_res->ete_info.h2d_res[i];
        if (pst_h2d_res->tx_buff.buff_cpu_addr != 0) {
            ret = oal_pcie_host_slave_address_switch(pst_pci_res,
                                                     (uint64_t)pst_h2d_res->tx_buff.buff_io_addr, &iova, OAL_FALSE);
            if (oal_unlikely(ret != OAL_SUCC)) {
                oal_print_mpxx_log(MPXX_LOG_ERR, "slave address switch failed ret=%d devca=%llx", ret,
                                   (uint64_t)pst_h2d_res->tx_buff.buff_io_addr);
            }
            oal_pcie_ete_buff_free(pst_pci_res, pst_h2d_res->tx_buff.buff_size,
                                   (void*)(uintptr_t)pst_h2d_res->tx_buff.buff_cpu_addr,
                                   (dma_addr_t)(iova));
            pst_h2d_res->tx_buff.buff_cpu_addr = 0;
            pst_h2d_res->tx_buff.buff_io_addr = 0;
        }
        if (pst_h2d_res->ring.cpu_addr != 0) {
            ...
            oal_pcie_ete_buff_free(pst_pci_res, pst_h2d_res->ring.size,
...
int32_t oal_ete_tx_trans_buff_init(oal_pcie_res *pst_pci_res, oal_ete_chan_h2d_res *pst_h2d_res)
{
    int32_t ret;
    size_t buff_size;
    dma_addr_t iova = 0;
    ...
    pst_h2d_res->tx_buff.buff_items = (uint32_t)pst_h2d_res->ring_max_num;
    buff_size = ETE_H2D_MAX_BUFF_LEN * pst_h2d_res->tx_buff.buff_items;

    ret = oal_pcie_ete_buff_alloc(pst_pci_res, buff_size, &cpu_addr, &dst_addr, &iova);
    if (ret != OAL_SUCC) {
        oal_print_mpxx_log(MPXX_LOG_ERR, "ete h2d chan%d txbuff init failed, ring_max_num = %d",
                           pst_h2d_res->chan_idx, pst_h2d_res->ring_max_num);
        return -OAL_FAIL;
    }
    pst_h2d_res->tx_buff.buff_cpu_addr = (uint64_t)cpu_addr;
    pst_h2d_res->tx_buff.buff_io_addr = dst_addr;
    pst_h2d_res->tx_buff.buff_size = buff_size;
    pst_h2d_res->tx_buff.buff_wr = 0;
    pst_h2d_res->tx_buff.buff_wr_flag = 0;
    pst_h2d_res->tx_buff.buff_rd = 0;
    pst_h2d_res->tx_buff.buff_rd_flag = 0;

    oal_print_mpxx_log(MPXX_LOG_DBG, "ete h2d chan%d txbuff iova=%llx  devva=%llx, io_addr=%llx",
                       pst_h2d_res->chan_idx, (uint64_t)iova, dst_addr, pst_h2d_res->tx_buff.buff_io_addr);
    oal_print_mpxx_log(MPXX_LOG_INFO, "ete h2d chan%d txbuff init succ, ring_max_num=%d, buff_size=%u",
                       pst_h2d_res->chan_idx, pst_h2d_res->ring_max_num, (uint32_t)buff_size);
    return OAL_SUCC;
}
...
int32_t oal_ete_chan_tx_init(oal_pcie_res *pst_pci_res)
{
    ...
    for (i = 0; i < ETE_TX_CHANN_MAX_NUMS; i++) {
        ...
        ret = oal_pcie_ete_buff_alloc(pst_pci_res, buff_size, &cpu_addr, &dst_addr, &iova);
        if (ret != OAL_SUCC) {
            oal_print_mpxx_log(MPXX_LOG_ERR, "ete h2d chan%d ring buff init failed, ring_max_num = %d",
                               pst_h2d_res->chan_idx, pst_h2d_res->ring_max_num);
            return -OAL_FAIL;
        }
        ...
        if (ret != OAL_SUCC) {
            oal_pcie_ete_buff_free(pst_pci_res, buff_size, cpu_addr, (dma_addr_t)(iova));
            return ret;
        }
        ...
        ret = oal_ete_tx_trans_buff_init(pst_pci_res, pst_h2d_res);
        if (ret != OAL_SUCC) {
            oal_pcie_ete_buff_free(pst_pci_res, buff_size, cpu_addr, (dma_addr_t)(iova));
            return ret;
        }
...
int32_t oal_ete_chan_rx_init(oal_pcie_res *pst_pci_res)
{
    ...
    for (i = 0; i < ETE_RX_CHANN_MAX_NUMS; i++) {
        ...
        if (ret != OAL_SUCC) {
            oal_print_mpxx_log(MPXX_LOG_ERR, "ete h2d chan%d ring buff init failed, ring_max_num = %d",
                               pst_d2h_res->chan_idx, pst_d2h_res->ring_max_num);
            return -OAL_FAIL;
        }
