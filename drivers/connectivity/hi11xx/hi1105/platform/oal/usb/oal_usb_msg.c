...
static int32_t oal_usb_send_message_to_dev(struct oal_usb_dev *usb_dev, uint32_t message)
{
    ...
    if (buf == NULL) {
        oal_print_mpxx_log(MPXX_LOG_ERR, "alloc buffer failed");
        return ret;
    }
    buf[0] = message;

    param = oal_usb_alloc_xfer_param(usb_dev, (uint8_t *)buf, buf_len, TRANSFER_DEFAULT_TIMEOUT, HOST_USB_DMA_XFER,
...
int32_t oal_usb_send_msg(hcc_bus *bus, uint32_t val)
{
    ...
    if (val >= H2D_MSG_COUNT) {
        oal_print_mpxx_log(MPXX_LOG_INFO, "invaild param[%u]", val);
        return -OAL_EINVAL;
    }
    ...
    oal_print_mpxx_log(MPXX_LOG_INFO, "send msg to device [%u]", val);
...
static int32_t oal_usb_read_d2h_message(struct oal_usb_dev *usb_dev, uint32_t *message)
{
    ...
    if (ret != OAL_SUCC) {
        ...
        *message = *buf;
        oal_print_mpxx_log(MPXX_LOG_DBG, "rx succ msg[%u]", *message);
    }
    oal_free(buf);
    return ret;
}
...
int32_t oal_usb_gpio_rx_data(hcc_bus *bus)
{
    ...
    forever_loop()
    {
        ...
        oal_print_mpxx_log(MPXX_LOG_INFO, "recv message: %u", message);
        ...
        if (message < D2H_MSG_COUNT) {
            ...
            if (bus->msg[message].msg_rx) {
                ...
                oal_print_mpxx_log(MPXX_LOG_INFO, "usb message :%u no callback functions", message);
            }
        }
    }
