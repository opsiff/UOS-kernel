...
#define USB_VENDOR_ID_GF61     0x12d1
#define USB_PRODUCT_ID_GF61_HISI 0x1443
...
#define USB_SUBMIT_TIMEOUT 1000

/* table of devices that work with this driver */
static const struct usb_device_id g_oal_usb_ids[] = { { USB_DEVICE(USB_VENDOR_ID_GF61, USB_PRODUCT_ID_GF61_HISI) },
                                                      {} };
...
static int32_t oal_usb_match_interface(struct usb_interface *interface)
{
    ...
    if (type >= OAL_USB_INTERFACE_NUMS || type == OAL_USB_INTERFACE_CTRL) {
        oal_print_mpxx_log(MPXX_LOG_INFO, "unknow type[%u]", type);
        return -OAL_EINVAL;
    }
    if (chan >= OAL_USB_CHAN_NUMS) {
        oal_print_mpxx_log(MPXX_LOG_INFO, "invaild chan[%u]", chan);
        return -OAL_EINVAL;
    }
...
static int32_t oal_usb_bulk_alloc(struct oal_usb_dev *usb_dev, struct usb_host_interface *iface_desc)
{
    ...
    oal_print_mpxx_log(MPXX_LOG_INFO, "bNumEndpoints[%u]", iface_desc->desc.bNumEndpoints);
    for (i = 0; i < iface_desc->desc.bNumEndpoints; ++i) {
        endpoint = &iface_desc->endpoint[i].desc;

        if (usb_dev->ep_info.in_ep_addr == 0 && usb_endpoint_is_bulk_in(endpoint)) {
            oal_print_mpxx_log(MPXX_LOG_INFO, "discover bulk in endpoint, addr[0x%x]", endpoint->bEndpointAddress);
        ...
        if (usb_dev->ep_info.out_ep_addr == 0 && usb_endpoint_is_bulk_out(endpoint)) {
            oal_print_mpxx_log(MPXX_LOG_INFO, "discover bulk out endpoint addr[0x%x]", endpoint->bEndpointAddress);
...
static int32_t oal_usb_intr_alloc(struct oal_usb_dev *usb_dev, struct usb_host_interface *iface_desc)
{
    ...
    for (i = 0; i < iface_desc->desc.bNumEndpoints; ++i) {
        ...
        if (usb_dev->ep_info.in_ep_addr == 0 && usb_endpoint_is_int_in(endpoint)) {
            oal_print_mpxx_log(MPXX_LOG_INFO, "discover intr in endpoint addr[0x%x]", endpoint->bEndpointAddress);
            usb_dev->ep_info.in_ep_addr = endpoint->bEndpointAddress;
            usb_dev->ep_info.in_interval = endpoint->bInterval;
            usb_dev->ep_info.in_max_packet = usb_endpoint_maxp(endpoint);
        }
        if (usb_dev->ep_info.out_ep_addr == 0 && usb_endpoint_is_int_out(endpoint)) {
            oal_print_mpxx_log(MPXX_LOG_INFO, "discover intr out endpoint addr[0x%x]", endpoint->bEndpointAddress);
            usb_dev->ep_info.out_ep_addr = endpoint->bEndpointAddress;
...
static int32_t oal_usb_isoc_alloc(struct oal_usb_dev *usb_dev, struct usb_host_interface *iface_desc)
{
    ...
    for (i = 0; i < iface_desc->desc.bNumEndpoints; ++i) {
        ...
        if (usb_dev->ep_info.in_ep_addr == 0 && usb_endpoint_is_isoc_in(endpoint)) {
            oal_print_mpxx_log(MPXX_LOG_INFO, "discover isoc in endpoint addr[0x%x]", endpoint->bEndpointAddress);
            usb_dev->ep_info.in_ep_addr = endpoint->bEndpointAddress;
            usb_dev->ep_info.in_interval = endpoint->bInterval;
            usb_dev->ep_info.in_max_packet = usb_endpoint_maxp(endpoint);
        }
        if (usb_dev->ep_info.out_ep_addr == 0 && usb_endpoint_is_isoc_out(endpoint)) {
            oal_print_mpxx_log(MPXX_LOG_INFO, "discover isoc out endpoint addr[0x%x]", endpoint->bEndpointAddress);
            usb_dev->ep_info.out_ep_addr = endpoint->bEndpointAddress;
            usb_dev->ep_info.out_interval = endpoint->bInterval;
            usb_dev->ep_info.out_max_packet = usb_endpoint_maxp(endpoint);
        }
...
static struct oal_usb_dev *oal_usb_alloc(struct usb_interface *interface, enum OAL_USB_INTERFACE type)
{
    ...
    if (usb_dev == NULL) {
        oal_print_mpxx_log(MPXX_LOG_ERR, "alloc oal_usb_dev failed [%d]", (int32_t)sizeof(struct oal_usb_dev));
        return NULL;
    }
...
void oal_usb_create_rx_thread(struct oal_usb_dev *usb_dev)
{
    ...
    if (usb_dev->rx_task != NULL) {
        oal_print_mpxx_log(MPXX_LOG_ERR, "usb rx thread already exist, type [%s] chan [%d]", g_usb_xfer_type[type],
                           chan);
        return;
    }
    if (usb_dev->rx_thread == NULL) {
        oal_print_mpxx_log(MPXX_LOG_ERR, "usb rx thread is NULL, type [%s] chan [%d]", g_usb_xfer_type[type], chan);
        return;
    }
    ...
    if (task_name == NULL) {
        ...
        if (snprintf_s(task_name, TASK_COMM_LEN, TASK_COMM_LEN - 1, "usb_rx_%s_%d", g_usb_xfer_type[type], chan) < 0) {
            oal_print_mpxx_log(MPXX_LOG_ERR, "usb rx task_name init fail, type [%s] chan [%d]",
                               g_usb_xfer_type[type], chan);
        }
    }
    ...
    if (usb_dev->rx_task == NULL) {
        ...
        oal_print_mpxx_log(MPXX_LOG_ERR, "usb rx task create fail, type [%s] chan [%d]",
                           g_usb_xfer_type[type], chan);
    }
...
void oal_usb_distory_rx_thread(struct oal_usb_dev *usb_dev)
{
    ...
    if (usb_dev->rx_task != NULL) {
        oal_print_mpxx_log(MPXX_LOG_INFO, "stop rx thread type [%s] chan [%d]", g_usb_xfer_type[type], chan);
...
static int oal_usb_probe(struct usb_interface *interface, const struct usb_device_id *id)
{
    ...
    if (oal_usb_match_interface(interface) < 0) {
        oal_print_mpxx_log(MPXX_LOG_INFO, "match fail");
        return -EINVAL;
    }
    ...
    oal_print_mpxx_log(MPXX_LOG_INFO, "oal_usb_probe type[%s] chan[%u]" NEWLINE, g_usb_xfer_type[type], chan);

    usb_dev = oal_usb_alloc(interface, type);
    if (usb_dev == NULL) {
        oal_print_mpxx_log(MPXX_LOG_ERR, "failed to alloc usb_dev!");
        return -EINVAL;
    }
...
static void oal_usb_draw_down(struct oal_usb_dev *usb_dev)
{
    ...
    time = usb_wait_anchor_empty_timeout(&usb_dev->submitted, USB_SUBMIT_TIMEOUT);
    if (time == 0) {
        usb_kill_anchored_urbs(&usb_dev->submitted);
    }
}
...
static int32_t oal_usb_suspend(struct usb_interface *interface, pm_message_t message)
{
    ...
    oal_print_mpxx_log(MPXX_LOG_INFO, "usb suspend");
    usb_dev = usb_get_intfdata(interface);
    if (usb_dev == NULL) {
        oal_print_mpxx_log(MPXX_LOG_ERR, "usb_dev is null");
        return OAL_SUCC;
    }
...
static int32_t oal_usb_resume(struct usb_interface *interface)
{
    ...
    oal_print_mpxx_log(MPXX_LOG_INFO, "usb resume");
    usb_dev = usb_get_intfdata(interface);
    if (usb_dev == NULL) {
        oal_print_mpxx_log(MPXX_LOG_ERR, "usb_dev is null");
        return OAL_SUCC;
...
int32_t oal_usb_driver_init(void)
{
    ...
    if (ret) {
        oal_print_mpxx_log(MPXX_LOG_ERR, "register usb driver failed ret=%d", ret);
    }
    return ret;
}
