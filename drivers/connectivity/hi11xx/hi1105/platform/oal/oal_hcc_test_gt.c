...
#define WAIT_TEST_TRACE_TIMEOUT 5000
/* 全局变量定义 */
                                 ...
                                 uint8_t *pst_context)
{
    ...
    } else if (stype == HCC_TEST_SUBTYPE_RW) {
        hcc_test_rcvd_rw(pst_netbuf);
        return OAL_SUCC;
#endif
    } else {
        /* unkown subtype */
...
OAL_STATIC int32_t hcc_test_wait_trans_done(hcc_test_cmd_stru *cmd)
{
    ...
    forever_loop() {
        ret = wait_for_completion_interruptible_timeout(&g_hcc_test_event_gt->test_trans_done,
            oal_msecs_to_jiffies(WAIT_TEST_TRACE_TIMEOUT));
        if (ret < 0) {
            oal_io_print("Test Event  terminated ret=%d\n", ret);
            ret = -OAL_EFAIL;
...
OAL_STATIC int32_t hcc_test_normal_start(uint16_t start_cmd)
{
    ...
    /* wait 50ms for device recv the cmd,change the test mode! */
    oal_msleep(50);
