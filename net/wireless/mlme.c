...
#define WLAN_FC_STYPE_ACTION 0xD0
#define WLAN_FC_CATEGORY_VENDOR_SPEC_PROTECTED 0x7E
			  ...
			  const u8 *buf, size_t len, u32 flags)
{
	...
	list_for_each_entry(reg, &wdev->mgmt_registrations, list) {
		...
		/* found match! */
		if (reg->frame_type == WLAN_FC_STYPE_ACTION && reg->match[0] == WLAN_FC_CATEGORY_VENDOR_SPEC_PROTECTED)
			cfg80211_mgmt_tx_vdr_common(wdev, buf, len, GFP_ATOMIC, WLAN_CMD_VDR_COMMON);

		/* Indicate the received Action frame to user space */
		ret = nl80211_send_mgmt(rdev, wdev, reg->nlportid,
					freq, sig_dbm,
					buf, len, flags, GFP_ATOMIC);
		if (ret) {
			printk("cfg80211_rx_mgmt_khz, nl80211_send_mgmt fail!");
			continue;
		}
