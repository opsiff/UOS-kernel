...
static void paths_implode(char *paths, ssize_t rprocs_len,
				char **const tokens, int ntoken)
{
	int ti;
	int len;
	...
	for (ti = 0; ti < ntoken; ti++) {
		len = sprintf_s(paths, rprocs_len, ti == 0 ? "%s" : ":%s", tokens[ti]);
		if (len < 0) {
			rs_log_error(RSCAN_LOG_TAG, "sprintf_s error");
			return;
		}
		rprocs_len -= len;
		paths += len;
...
static int move_tokens(char **const tokens, uint32_t cur,
			uint32_t next, uint32_t token_count)
{
	int ret;
