...
// CONFIG_JUMP_LABEL:Macros will modify kernel code
// for optimize decision branches
...
static int rs_pause_verify_open(struct inode *inode, struct file *file)
{
	return 0;
}

static ssize_t rs_pause_verify_read(struct file *file, char __user *buf,
				size_t count, loff_t *offp)
{
	int test = 0;
...
static int rs_pause_verify_release(struct inode *inode, struct file *file)
{
	if (file->private_data != NULL) {
		vfree(file->private_data);
		file->private_data = NULL;
...
static int rs_resume_verify_open(struct inode *inode, struct file *file)
{
	return 0;
}

static ssize_t rs_resume_verify_read(struct file *file, char __user *buf,
				size_t count, loff_t *offp)
{
	int test = 0;
...
static int rs_resume_verify_release(struct inode *inode, struct file *file)
{
	if (file->private_data != NULL) {
		vfree(file->private_data);
		file->private_data = NULL;
...
static const struct file_operations rs_pause_verify_fops = {
	.owner = THIS_MODULE,
	.open  = rs_pause_verify_open,
	.read  = rs_pause_verify_read,
	.release = rs_pause_verify_release,
};

static const struct file_operations rs_resume_verify_fops = {
	.owner = THIS_MODULE,
	.open  = rs_resume_verify_open,
	.read  = rs_resume_verify_read,
	.release = rs_resume_verify_release,
};
...
static const struct proc_ops rs_pause_verify_fops = {
	.proc_open  = rs_pause_verify_open,
	.proc_read  = rs_pause_verify_read,
	.proc_release = rs_pause_verify_release,
};

static const struct proc_ops rs_resume_verify_fops = {
	.proc_open  = rs_resume_verify_open,
	.proc_read  = rs_resume_verify_read,
	.proc_release = rs_resume_verify_release,
};
...
int rscan_dynamic_init(void)
{
	...
	proc_create("rs_pause_verify",  0777, NULL, &rs_pause_verify_fops);
	proc_create("rs_resume_verify", 0777, NULL, &rs_resume_verify_fops);
