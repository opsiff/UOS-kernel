...
prmem_pool(selinux_prot_pool, ro_no_recl, sizeof(void *),
	PAGE_SIZE, PAGE_SIZE);

__init void selinux_protection_init(struct selinux_state *state)
{
	state->prot = (struct selinux_prot *)pzalloc(&selinux_prot_pool,
		sizeof(struct selinux_prot *), PRMEM_NO_FLAGS);
	if (!state->prot) {
...
#ifdef CONFIG_HKIP_SELINUX_PROT
	selinux_protection_init(&selinux_state);
#endif
