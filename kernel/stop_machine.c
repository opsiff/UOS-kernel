...
static int multi_cpu_stop(void *data)
{
	...
	do {
		...
		/*
		 * A livelock happened between little-core and mid-core/big-core.
		 * CPU0 always stlxr failed for atomic operation
		 * in function dyntick_save_progress_counter, which holds a spin-lock.
		 * CPU4 ldxr/stlxr the same atomic address all the time in a tight loop,
		 * it can not exit loop as no available cpu for context switch.
		 * Other cpus halt on wfe, waiting for CPU0 release spin-lock.
		 * So add 1us delay to destroy this tight loop of CPU4.
		 */
		udelay(1);
#endif
