...
#include <platform_include/basicplatform/linux/eagle_eye.h>
...
EXPORT_PER_CPU_SYMBOL(hrtimer_interrupts);
DEFINE_PER_CPU(unsigned long, hrtimer_interrupts_saved);
EXPORT_PER_CPU_SYMBOL(hrtimer_interrupts_saved);
static unsigned long soft_lockup_nmi_warn;

/* Global variables, exported for sysctl */
unsigned int __read_mostly softlockup_panic =
            CONFIG_BOOTPARAM_SOFTLOCKUP_PANIC_VALUE;
EXPORT_SYMBOL(softlockup_panic);
...
EXPORT_SYMBOL(set_sample_period);
...
{
	CALL_HCK_VH(hck_dfx_wdt_set_nmi_touch, true);
...
static enum hrtimer_restart watchdog_timer_fn(struct hrtimer *hrtimer)
{
	...
	if (unlikely(duration)) {
		...
		update_touch_ts();
#ifdef CONFIG_DFX_ZEROHUNG_SOFTLOCKUP
		trace_hck_dfx_watchdog_check_hung(0);
#endif
		...
		CALL_HCK_VH(hck_dfx_wdt_cond_dump_stack, (regs != NULL));
		...
		if (softlockup_panic) {
			CALL_HCK_VH(hck_dfx_wdt_set_softlockup_panic_happen, true);
...
void watchdog_enable(unsigned int cpu)
{
	...
	CALL_HCK_VH(hck_dfx_wdt_cpumask_set, cpu);
...
#endif
#ifdef CONFIG_DFX_ZEROHUNG_SOFTLOCKUP
	trace_hck_dfx_watchdog_lockup_init_work(0);
#endif
}
...
void watchdog_disable(unsigned int cpu)
{
	...
	CALL_HCK_VH(hck_dfx_wdt_cpumask_clear, cpu);
