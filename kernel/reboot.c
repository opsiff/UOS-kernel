...
void kernel_restart_prepare(char *cmd)
{
	...
	pr_emerg("shutfail set_shut_stage: SHUT_STAGE_SUCC ret:%d\n", ret);
	stop_boot_timer();
#endif
	device_shutdown();
}
...
#ifdef CONFIG_SHUT_DETECTOR
	int ret = 0;
	ret = set_shut_stage(SHUT_STAGE_KERNEL);
	pr_emerg("shutfail set_shut_stage: SHUT_STAGE_KERNEL ret:%d\n", ret);
#endif
		...
		void __user *, arg)
{
	...
	case LINUX_REBOOT_CMD_RESTART:
		CALL_HCK_VH(hck_dfx_wdt_shutdown_oneshot, 30);
		kernel_restart(NULL);
		break;
