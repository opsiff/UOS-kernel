...
int set_power_on_alarm(long time_sec, bool enable_irq)
{
	...
	pmu_rtc_readtime(&rtc_current_rtc_time);
	...
	pr_info("%s() set_rtc_time is : [%d-%d-%d] [%d:%d:%d] %lu\n",
		...
		alarm_value);
	pmu_rtc_setalarmtime(alarm_value);
	return 0;
}
...
static int __init alarmtimer_init(void)
{
	...
	alarm_bases[ALARM_POWEROFF_REALTIME].get_timespec = ktime_get_real_ts64;
