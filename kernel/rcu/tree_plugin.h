...
/*
 * To be optimized by Joel Fernandes's patch in the future.
 * For now we have to simply hack the rcu_idle_gp_delay and take the risk
 * of its disadvantages.
 * Since slow cpus are unlikely to be idle for long in common cases, we
 * can specify a long delay only for slow cpus which will impact rcu gp
 * delay less and still good enough for fcm idle energy efficiency.
 */
#define RCU_IDLE_GP_DELAY_LONG 25
static int rcu_idle_gp_delay_hack(void)
{
	return test_slow_cpu(smp_processor_id())
		? max(rcu_idle_gp_delay, RCU_IDLE_GP_DELAY_LONG)
		: rcu_idle_gp_delay;
}
#endif
...
#ifdef CONFIG_OPT_TIMER_ENERGY_EFFICIENCY
	dj = roundup(rcu_idle_gp_delay_hack() + jiffies, rcu_idle_gp_delay_hack()) - jiffies;
#else
