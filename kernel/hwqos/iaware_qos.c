...
static int sched_set_task_vip_prio(unsigned long arg)
{
	...
	rcu_read_lock();
	task = find_task_by_vpid(config.pid);
	if (!task) {
		rcu_read_unlock();
		return -ESRCH;
	}
	...
	ret = set_vip_prio(task, (unsigned int)config.value);
...
/* prio:[0,139] */
static int set_task_scheduler(struct task_struct *task, int prio)
{
	int ret;
	struct sched_param sp = {0};
	int policy = SCHED_NORMAL;
	if (!task || prio < 0 || prio > MAX_PRIO - 1)
		return -EINVAL;

	if (prio < MAX_USER_RT_PRIO) {
	...
	if (ret < 0)
		pr_err("%s task:%d setscheduler ret:%d\n",
			__func__, task->pid, ret);

	return ret;
}

static long qos_ctrl_set_task_attr(unsigned int cmd, unsigned long arg)
{
	void __user *uarg = (void __user *)arg;
	struct task_config attr;
	...
	if (copy_from_user(&attr, uarg, sizeof(attr))) {
		pr_err("set_task_attr copy data failed\n");
		return -EINVAL;
	}

	rcu_read_lock();
	p = find_task_by_vpid(attr.pid);
	if (p == NULL) {
		rcu_read_unlock();
		return -EINVAL;
	}
...
#ifdef CONFIG_SCHED_HISI_TASK_MIN_UTIL
	if (cmd == CTRL_SET_TASK_MIN_UCLAMP) {
		set_task_min_util(p, (unsigned int)attr.value);
	} else if (cmd == CTRL_SET_TASK_MAX_UCLAMP) {
		set_task_max_util(p, (unsigned int)attr.value);
	} else if (cmd == CTRL_SET_TASK_PRIORITY) {
#else
	if (cmd == CTRL_SET_TASK_PRIORITY) {
#endif
		ret = set_task_scheduler(p, (unsigned int)attr.value);
	} else {
		put_task_struct(p);
		return -EINVAL;
	}
	...
	unsigned int cmd, unsigned long arg)
{
	...
	case CTRL_SET_TASK_MIN_UTIL:
		...
		break;
	case CTRL_SET_TASK_MIN_UCLAMP:
	case CTRL_SET_TASK_MAX_UCLAMP:
	case CTRL_SET_TASK_PRIORITY:
		ret = qos_ctrl_set_task_attr(cmd, arg);
		break;
	default:
		ret = -EINVAL;
		break;
