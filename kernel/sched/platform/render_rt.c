...
/*
 *  g_wake_queue_rwlock --the global lock for global wake_queue
 */
static DEFINE_RWLOCK(g_wake_queue_rwlock);
#endif
...
/* max number for wake queue, i.e. the buffer */
#define MAX_QUEUE_NUM 4

/* each task own 3 bits to save index for each wake queue */
#define WAKE_QUEUE_IDX_SHIFT 3
#define PER_SHIFT 3
...
#ifdef CONFIG_RENDER_RT_GLOBAL
	/*
	 * @prio: normal_prio for child_thread
	 * @wake_depth: depth for child_thread, 0 for render itself, 1 for direct waker
	 * @ppid: parent pid for child_thread, i.e, the wakee pid
	 * @binder_flag: mark child_thread is a binder thread
	 */
	int prio;
	int wake_depth;
	pid_t ppid;
 ...
 */
#ifndef CONFIG_RENDER_RT_GLOBAL
static struct render_related_thread g_ta_render;
#else
/*
 * range [RENDER_RT_GLO, RENDER_RT_MAX_BUFF] reserve for rrt_global
 * RENDER_RT_GRP reserve for original rrt
 */
static struct render_related_thread g_ta_render[RENDER_RT_MAX_BUFF];
#endif

#ifndef CONFIG_RENDER_RT_GLOBAL
static void init_child_rthread(int index, struct task_struct *p);
#else
static void init_child_rthread(int index, struct task_struct *p,
			       struct render_related_thread *rrt_pos);
#endif

#ifdef CONFIG_RENDER_RT_GLOBAL
static struct render_related_thread *g_ta_render_ptr_glo __read_mostly;
static int g_inited_pid = 0;
static int g_wake_queue_num = 0;
static int g_inited_queue_num = 0;
static int g_cur_queue = 0;

/*
 * set_task_wake_queue_bit() - set wake queue bit for task
 * @task: target task
 * @queue_index: wake queue index
 * @index: task index in wake queue
 *
 * fake_index is saved in task->wake_queue_bit
 */
void set_task_wake_queue_bit(struct task_struct *task, int queue_index, int index)
{
	int fake_index = index + 1;

	if (unlikely(!task))
		return;

	if (unlikely(index < 0 || index >= MAX_QUEUE_LENGTH))
		return;

	task->wake_queue_bit &= ~(0xFF << (queue_index << PER_SHIFT));
	task->wake_queue_bit |= fake_index << (queue_index << PER_SHIFT);
}

/*
 * find_task_index_in_wake_queue() - get task index in specific wake queue
 * @queue_index: index for wake_queue
 * @task: target task
 *
 * Return task index in array of wake queue(e.g., 0 for render, 1 for a render's waker)
 */
static inline int find_task_index_in_wake_queue(int queue_index, struct task_struct *task)
{
	int fake_index;

	if (unlikely((queue_index < 0) || (queue_index >= MAX_QUEUE_NUM)))
		return -1;

	if (unlikely(task == NULL))
		return -1;

	fake_index = (task->wake_queue_bit >> (queue_index << PER_SHIFT)) & 0xFF;

	return fake_index - 1;
}

static void reset_inqueue_tasks(int queue_index)
{
	struct task_struct *task = NULL;
	int i = 0;
	int child_thread_cnt;
	struct child_thread *rthreads = NULL;
	struct render_related_thread *rrt_pos = &g_ta_render[RENDER_RT_GLO];

	rrt_pos += queue_index;
	child_thread_cnt = rrt_pos->total_count;
	rthreads = rrt_pos->ta_child_threads;
	for (i = 0; i < child_thread_cnt; i++, rthreads++) {
		if (rthreads->exited)
			continue;
...
static void reset_single_wake_queue(pid_t pid, struct task_struct *render, int queue_index)
{
	struct render_related_thread *rrt_pos = NULL;

	if (queue_index < 0 || queue_index >= g_wake_queue_num)
		return;

	rrt_pos = &g_ta_render[RENDER_RT_GLO];

	/* reset tasks's wake_queue_bit */
	reset_inqueue_tasks(queue_index);

	/* flush state statistic */
	rrt_pos += queue_index;
	rrt_pos->total_count = 1;
	rrt_pos->pid = pid;
	rrt_pos->render_index = RENDER_IDX_RRTG;
	rrt_pos->stopped = false;

	/* insert render thread to first position */
	init_child_rthread(RENDER_IDX_RRTG, render, rrt_pos);
	set_task_wake_queue_bit(render, queue_index, RENDER_IDX_RRTG);
}

/*
 * reset_wake_queue() - reset wake queue to fresh-new status
 * @pid: pid for key dude, i.e, the render
 * @queue_index: index for wake queue to be reset
 * @render: render thread, the key dude
 *
 * Returns 0 for success, other for failed
 *
 * reset wake queue to fresh-new status
 * i.e., render is the only ready dude in the wake queue
 */
static int reset_wake_queue(pid_t pid, int queue_index, struct task_struct *render)
{
	int i;
	int ret = 0;

	if (unlikely(g_inited_queue_num == 0)) {
		ret = 1;
		goto out;
	}

	/* reset for all wake queues */
	if (queue_index == g_wake_queue_num) {
		for (i = 0; i < g_wake_queue_num; i++)
			reset_single_wake_queue(pid, render, i);

		goto out;
	}

	/* reset for single wake queue */
	reset_single_wake_queue(pid, render, queue_index);
out:
	return ret;
}

/*
 * init_wake_queue() - init all wake queues, i.e, the buffers
 * @pid: pid for key dude, i.e, the render
 * @max_depth: depth for the wake tree, should match wake queues num
 * @render: render task, the key dude
 *
 * Returns 0 for success, other for failed
 *
 * called by init_render_rthread_global()
 * NO NEED to check paras here
 */
static int init_wake_queue(int pid, int max_depth, struct task_struct *render)
{
	g_wake_queue_num = max_depth;
	g_cur_queue = 0;
	g_inited_pid = pid;
	g_inited_queue_num = 1;

	return reset_wake_queue(pid, max_depth, render);
}

static void feed_back_rrt_info(struct render_rt_glo *result,
			     struct render_related_thread *rrt_pos)
{
	struct child_thread *rthreads = NULL;
	struct thread_wake_info *wake_infos = NULL;
	int i;
	int count = min(rrt_pos->total_count, MAX_QUEUE_LENGTH);

	result->render_pid = rrt_pos->pid;
	result->num = rrt_pos->total_count;
	rthreads = rrt_pos->ta_child_threads;
	wake_infos = result->infos;

	for (i = 0; i < count; i++) {
		wake_infos[i].pid = rthreads[i].pid;
		wake_infos[i].prio = rthreads[i].prio;
		wake_infos[i].wake_depth = rthreads[i].wake_depth;
		wake_infos[i].woken_count = rthreads[i].woken_count;
		wake_infos[i].ppid = rthreads[i].ppid;
		wake_infos[i].binder_flag = rthreads[i].binder_flag;
	}
}

/*
 * get_wake_relationship() - feedback wake relation
 * @result: result buffer
 *
 * each get will fetch data from one wake queue
 */
static void get_wake_relationship(struct render_rt_glo *result, struct task_struct *render)
{
	struct render_related_thread *rrt_pos = NULL;

	rrt_pos = &g_ta_render[RENDER_RT_GLO + g_cur_queue];
	if (g_inited_queue_num < g_wake_queue_num) {
		/*
		 * not all wake queues are initied, but all wake queues are reset
		 * NO NEED to reset again
		 */
		feed_back_rrt_info(result, rrt_pos);
		g_inited_queue_num++;
		return;
	}

	/* all wake queues inited, reset cur queue after feedback */
	feed_back_rrt_info(result, rrt_pos);
	reset_wake_queue(rrt_pos->pid, g_cur_queue, render);
	g_cur_queue = (g_cur_queue + 1 ) % g_wake_queue_num;
}

/*
 * insert_task_to_wake_queue() - insert new waker to wake queue
 * @target: the wakee, not the dude to be inserted
 * @queue_index: wake queue index
 * @depth: waker's depth to be inserted
 * @rrt_pos: pointer to wake queue
 *
 * the target is wakee, but we want to insert waker, i.e., the current
 */
static void insert_task_to_wake_queue(struct task_struct *target, int queue_index, int depth,
					     struct render_related_thread *rrt_pos)
{
	int rrt_cnt;
	struct child_thread *thread = NULL;

	rrt_cnt = rrt_pos->total_count;

	if (unlikely(rrt_cnt >= MAX_QUEUE_LENGTH))
		return;

	if (unlikely(queue_index >= MAX_QUEUE_NUM))
		return;

	init_child_rthread(rrt_cnt, current, rrt_pos);
	thread = &rrt_pos->ta_child_threads[rrt_cnt];
	thread->wake_depth = depth;
	thread->ppid = target->pid;

	set_task_wake_queue_bit(current, queue_index, rrt_cnt);
	rrt_pos->total_count++;
}

/*
 * judge_render_task() - target belong to any wake queue?
 * @target: the wakee
 *
 * Returns TRUE if target not appear in any wake queue, i,e, an outsider
 */
static inline bool render_unrelated_task(struct task_struct *target)
{
	return target->wake_queue_bit == 0;
}

static void remove_task_from_rrt_glo(struct task_struct *task)
{
	int index, i;
	unsigned long flags;
	struct render_related_thread *rrt_pos = NULL;
	...
	write_lock_irqsave(&g_wake_queue_rwlock, flags);

	/* not in any wake queue, NO need to bother wake queues */
	if (render_unrelated_task(task))
		goto out_unlock;

	/* if render died, change rrt_glo to un-inited state */
	if (unlikely(task->pid == g_inited_pid))
		g_ta_render_ptr_glo = NULL;

	rrt_pos = &g_ta_render[RENDER_RT_GLO];

	for (i = 0; i < g_wake_queue_num; i++, rrt_pos++) {
		index = find_task_index_in_wake_queue(i, task);
		if (index < 0 || index >= MAX_QUEUE_LENGTH)
			continue;
		rthreads = &rrt_pos->ta_child_threads[index];
		rthreads->exited = true;
	}

	task->wake_queue_bit = 0;

out_unlock:
	write_unlock_irqrestore(&g_wake_queue_rwlock, flags);

	return;
}

/*
 * add_waker_to_global_render_rthread() - update wake relationship when wakee waked
 * @wakee: the wakee
 *
 * core func for RENDER_RT_GLOBAL
 * in ttwu context, DONT WORRY wakee and current
 */
static void add_waker_to_global_render_rthread(struct task_struct *wakee)
{
	int wakee_index, i, waker_index, depth;
	unsigned long flags;
	struct child_thread *thread = NULL;
	struct render_related_thread *rrt_pos = NULL;
	...
	/*
	 * render's refcount to zero, i.e., truly died
	 * just SKIP the wakeup relation dealing
	 * g_ta_render_ptr_glo set to NULL in remove_task_from_rrt_glo
	 */
	if (!g_ta_render_ptr_glo)
		goto out_unlock;

	/*
	 * if wakee not in any wake queue, just SKIP
	 *
	 * actually we should put it into the critical section for identify accuracy
	 * but here is the ttwu context, lock-unlock routine waste cycles
	 */
	if (render_unrelated_task(wakee))
		goto out_unlock;

	/* waker is render_thread itself, just SKIP */
	if (current->pid == g_inited_pid)
		goto out_unlock;

	rrt_pos = &g_ta_render[RENDER_RT_GLO];
	for (i = 0; i < g_inited_queue_num; i++, rrt_pos++) {
		wakee_index = find_task_index_in_wake_queue(i, wakee);
		/* if wakee not in wake queue, SKIP */
		if (wakee_index < 0)
			continue;

		waker_index = find_task_index_in_wake_queue(i, current);

		/* waker and wakee both valid, UPDATE */
		thread = rrt_pos->ta_child_threads;
		depth = thread[wakee_index].wake_depth;

		if (g_inited_queue_num < g_wake_queue_num) {
			/*
			 * not all queue inited, only inited queue could do update
			 * only one depth can update for each wake queue
			 * depth is is determined by queue index
			 *
			 * maybe
			 * depth + 1 = g_inited_queue_num - i
			 * is easy to understand
			 * here depth + 1 is the waker depth we want
			 */
			if (g_inited_queue_num != depth + i + 1)
				continue;
		} else {
			/*
			 * all queue inited, but still only one depth is active for
			 * each wake queue.
			 */
			if ((((depth + i + 1) % g_wake_queue_num) != g_cur_queue))
				continue;
		}

		/* here generates a wake event should be book */
		thread[wakee_index].woken_count++;

		/* if waker not in wake queue, ADD to wake queue */
		if (waker_index < 0)
			insert_task_to_wake_queue(wakee, i, depth + 1, rrt_pos);
	}
out_unlock:
	write_unlock_irqrestore(&g_wake_queue_rwlock, flags);
}
#endif /* CONFIG_RENDER_RT_GLOBAL */
 ...
 */
static int get_child_rthread_index(struct task_struct *task)
{
...
static void fix_child_rthread_waker(int threshold)
{
...
#else
			init_child_rthread(index, task, g_ta_render_ptr);
#endif
			goto unlock;
		} else {
			reason = 2;
...
#else
		init_child_rthread(g_ta_render_ptr->total_count, task, g_ta_render_ptr);
#endif
...
#else
		init_child_rthread(0, task, g_ta_render_ptr);
#endif
...
#else
		init_child_rthread(i, task, g_ta_render_ptr);
#endif
...
unlock:
	trace_perf(add_render_rthread, task);
...
static void update_rthread_util(struct child_thread *thread,
				       u64 window_size);
...
void remove_render_rthread(struct task_struct *task)
{
	...
	trace_perf(remove_render_rthread, task);
...
err:
	...
	if (reason)
		printk_ratelimited(KERN_WARNING
			"[%s] fail to find the taskreason=%d comm=%s pid=%d\n",
			__func__, reason, task->comm, task->pid);
}

static void add_waker_to_in_group_render_rthread(struct task_struct *task)
{
	...
	if (!test_bit(waker_index, thread->waker_bitmap)) {
		if (thread->waker_count < MAX_WAKER_COUNT) {
			...
			trace_perf(add_waker_to_render_rthread, current, waker_index,
							  task, wakee_index,
							  thread->waker_count);
		}
	}
...
#ifdef CONFIG_RENDER_RT_GLOBAL
	add_waker_to_global_render_rthread(task);
#endif

	/* original render rt */
	if (g_ta_render_ptr != NULL)
		add_waker_to_in_group_render_rthread(task);
}
...
#endif
{
...
#else
	struct child_thread *thread = &rrt_pos->ta_child_threads[index];
#endif
	...
	thread->task = p;
#ifdef CONFIG_RENDER_RT_GLOBAL
	thread->prio = p->normal_prio;
	thread->wake_depth = 0;
	thread->ppid = -1;
	thread->binder_flag = p->is_binder;
#endif
	thread->pid = p->pid;
	thread->visted = false;
	thread->exited = false;
				       ...
				       u64 window_size)
{
	...
	trace_perf(update_rthread_util, thread->task, frame_load, frame_util);
 ...
 */
static void update_all_rthread_util(void)
{
	struct child_thread *thread = NULL;
	u64 curr_ktime, window_size;
	int index;
...
unsigned int sysctl_sched_enable_render_rt_trace;

/*lint -e24 -e31 -e563*/
static void dbg_print_waker(int index)
{
	int i, waker_index;
...
static void dbg_dump_ta_threads(void)
{
	int total_count = g_ta_render_ptr->total_count;
	struct child_thread *thread = NULL;
	int index;
...
static void dbg_show_rthread(int *rthread_results)
{
	int i;
...
static void dbg_show_hrthread(struct thread_util *rthread_results,
				     int total_cnt)
{
	int i;
...
static void reset_child_rthread(void)
{
	int total_count = g_ta_render_ptr->total_count;
	struct child_thread *thread = NULL;
	int index;
 ...
 */
static void remove_exited_render_rthread(void)
{
	int total_count = g_ta_render_ptr->total_count;
	struct child_thread *thread = NULL;
...
static void populate_rthread(int *rthread_results,
...
static void hierarchical_bfs_traverse(int *rthread_results)
{
	int i = 0;
	int rthread_index;
...
static void populate_results(struct render_rt *result,
...
static struct task_struct *get_leader_task(pid_t pid)
{
...
#else
	g_ta_render_ptr = &g_ta_render[RENDER_RT_GRP];
#endif
	g_ta_render_ptr->pid = rfps.render_pid;

	/* mark the start timestamp */
	g_ta_render_ptr->window_start = sched_ktime_clock();
...
#else
		init_child_rthread(i, child_tasks[i].task, g_ta_render_ptr);
#endif
	}
...
/*
 * init_render_rthread_global() - init the global wakequeue
 */
int init_render_rthread_global(void __user *uarg)
{
	...
	struct render_init_paras_glo rfps;
	unsigned long flags;

	if (copy_from_user(&rfps, uarg, sizeof(struct render_init_paras_glo))) {
		ret = -EFAULT;
		goto out;
	}

	depth = rfps.max_wake_depth;
	if (depth > MAX_QUEUE_NUM || depth <= 0) {
		depth = MAX_DEPTH_DEFAULT;
		pr_info("[RRT_GLO] invalid depth, use value %d\n", MAX_DEPTH_DEFAULT);
	}

	render = find_get_task_by_vpid(rfps.render_pid);
	if (!render) {
		pr_err("[RRT_GLO] invalid render pid %d\n", rfps.render_pid);
		ret = -ENODEV;
		goto out;
	}
	...
	/* non-force-init with new render pid not allowed */
	if (!rfps.force_init && rfps.render_pid != g_inited_pid) {
		ret = -EBUSY;
		goto out_unlock;
	}

	g_ta_render_ptr_glo = &g_ta_render[RENDER_RT_GLO];

	/* non-force-init with original render, DO NOT reset wake queues */
	if (!rfps.force_init)
		goto out_unlock;

	ret = init_wake_queue(rfps.render_pid, depth, render);
	if (ret) {
		g_ta_render_ptr_glo = NULL;
		ret = -EBUSY;
	}
...
/*
 * get_render_rthread_glo() - return the current wakequeue to usermode
 */
int get_render_rthread_glo(void __user *uarg)
{
	struct render_rt_glo result;
	struct task_struct *render;
	unsigned long flags;
	int ret = 0;

	if (copy_from_user(&result, uarg, sizeof(struct render_rt_glo))) {
		ret = -EFAULT;
		printk_ratelimited(KERN_ERR "[%s %d] ret=%d\n",
				   __func__, __LINE__, ret);
		goto out;
	...
	render = find_get_task_by_vpid(result.render_pid);
	if (!render) {
		ret = -ENOENT;
		pr_err("[RRT_GLO] given render pid %d not found\n", result.render_pid);
		goto out;
	}

	write_lock_irqsave(&g_wake_queue_rwlock, flags);
	if (!g_ta_render_ptr_glo || !g_inited_pid || result.render_pid != g_inited_pid) {
		ret = -ENOENT;
		goto out_unlock;
	}

	get_wake_relationship(&result, render);
	write_unlock_irqrestore(&g_wake_queue_rwlock, flags);
	put_task_struct(render);

	if (copy_to_user(uarg, &result, sizeof(struct render_rt_glo)))
		ret = -EIO;
