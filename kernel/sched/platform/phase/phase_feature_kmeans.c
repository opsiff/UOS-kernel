...
static int make_decision(struct kmeans_lsvm *kmeans_lsvm, u64 *buffer)
{
	...
	for (i = 0; i < kmeans_lsvm->nr_lsvm; i++) {
		...
		trace_perf(km_cluster_make_decision, fdis_min, lsvm_col, i, sum[i], plsvm->lower, plsvm->upper, plsvm->threshold);
				...
				enum smt_mode smt)
{
	...
	trace_perf(km_cluster_predict_buffer, pleft, pright, buffer, smt);
	for (i = 0; i < kmeans_lsvm->dim_lsvm; i++)
		buffer[i] = (int)buffer[i] * kmeans_lsvm->buffer_scaler[i];

	is_match = make_decision(kmeans_lsvm, buffer);
	trace_perf(km_cluster_predict_pmu, pleft, pright, phase_get_counter(pleft->phase_info, smt),
