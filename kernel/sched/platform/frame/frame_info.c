...
static bool frame_task_is_sched_vip_policy(int prio)
{
	if (prio == TOP_TASK_KEY + DEFAULT_RT_PRIO ||
...
static bool frame_task_prio_is_rt_task(int prio)
{
	bool is_rt_task = (prio != NOT_RT_PRIO);

#ifdef CONFIG_HW_RTG_VIP_FOR_MULTI_FRAME
	is_rt_task &= !frame_task_is_sched_vip_policy(prio);
#endif
...
{
	bool is_rt_task = frame_task_prio_is_rt_task(prio);
	bool need_dec_flag = false;
	bool need_inc_flag = false;
	int sched_policy = RTG_SCHED_POLICY_NORMAL;
	int err;

	trace_perf(rtg_frame_sched, frame_info->rtg->id, "rtg_rt_thread_num", read_rtg_rt_thread_num());
	/* change policy to RT */
	if (is_rt_task && (atomic_read(&frame_info->curr_rt_thread_num) <
		...
		/* change RT priority */
		sched_policy = RTG_SCHED_POLICY_RT;
		...
		if (!is_rt_task && is_rtg_rt_task(task))
			need_dec_flag = true;
#ifdef CONFIG_HW_RTG_VIP_FOR_MULTI_FRAME
		/* change policy to vip */
		if (frame_task_is_sched_vip_policy(prio))
			sched_policy = RTG_SCHED_POLICY_VIP;
#endif
	}
out:
	trace_perf(rtg_frame_sched, frame_info->rtg->id, "rtg_rt_thread_num", read_rtg_rt_thread_num());
	trace_perf(rtg_frame_sched, frame_info->rtg->id, "curr_rt_thread_num",
			      atomic_read(&frame_info->curr_rt_thread_num));

	err = sched_rtg_set_task_scheduler(task, sched_policy, prio);
	if (err == 0) {
		if (need_dec_flag)
...
static void update_frame_task_prio(struct frame_info *frame_info, int prio)
{
	...
	if (!atomic_read(&frame_info->frame_sched_state))
		return;
...
int set_frame_rate(struct frame_info *frame_info, int rate)
{
	...
	trace_perf(rtg_frame_sched, id, "FRAME_QOS", rate);
	trace_perf(rtg_frame_sched, id, "FRAME_MAX_TIME", frame_info->max_vload_time);
...
int set_frame_margin(struct frame_info *frame_info, int margin)
{
	...
	frame_info->max_vload_time =
		...
		frame_info->vload_margin;
	id = frame_info->rtg->id;
	trace_perf(rtg_frame_sched, id, "FRAME_MARGIN", margin);
	trace_perf(rtg_frame_sched, id, "FRAME_MAX_TIME", frame_info->max_vload_time);
...
int set_frame_max_util(struct frame_info *frame_info, int max_util)
{
	...
	id = frame_info->rtg->id;
	trace_perf(rtg_frame_sched, id, "FRAME_MAX_UTIL", frame_info->frame_max_util);
...
#ifdef CONFIG_HW_RTG_VIP_FOR_MULTI_FRAME
	if (frame_info != NULL && frame_info->rtg != NULL && is_frame_rtg(frame_info->rtg->id))
		is_rt_task &= frame_task_prio_is_rt_task(prio);
#endif
...
void set_frame_sched_state(struct frame_info *frame_info, bool enable)
{
	...
	if (enable) {
		...
		trace_perf(rtg_frame_sched, id, "FRAME_SCHED_ENABLE", 1);
		...
		trace_perf(rtg_frame_sched, id, "FRAME_SCHED_ENABLE", 0);

		(void)sched_set_group_normalized_util(id, 0, FRAME_NORMAL_UPDATE);
		frame_info->status = FRAME_END;
		trace_perf(rtg_frame_sched, id, "preferred_cluster",
	...
	trace_perf(rtg_frame_sched, id, "FRAME_STATUS", frame_info->status);
	trace_perf(rtg_frame_sched, id, "frame_status", frame_info->status);
}
EXPORT_SYMBOL_GPL(set_frame_sched_state);

static struct task_struct *do_update_thread(struct frame_info *frame_info, int old_prio, int prio,
					    int pid, struct task_struct *old_task)
{
	...
	trace_perf(rtg_frame_sched, frame_info->rtg->id, "FRAME_SCHED_ENABLE",
	...
	if (atomic_read(&frame_info->frame_sched_state) == 1) {
		...
		trace_perf(rtg_frame_sched, frame_info->rtg->id, "curr_rt_thread_num",
				      atomic_read(&frame_info->curr_rt_thread_num));
		trace_perf(rtg_frame_sched, frame_info->rtg->id, "rtg_rt_thread_num",
...
void update_frame_cfs_thread(struct frame_info *frame_info, int tid, int index)
{
	...
	if (index >= 0 && index < MAX_FRAME_CFS_THREADS)
		frame_info->cfs_task[index] = do_update_thread(frame_info, 0, NOT_RT_PRIO, tid,
							       frame_info->cfs_task[index]);
	write_unlock(&frame_info->lock);
}
...
{
	if (frame_info->frame_util < (u64)frame_info->frame_boost_min_util)
...
static void set_frame_start(struct frame_info *frame_info)
{
	...
	trace_perf(rtg_frame_sched, id, "FRAME_STATUS", frame_info->status);
	trace_perf(rtg_frame_sched, id, "frame_last_task_time",
		frame_info->prev_frame_exec);
	trace_perf(rtg_frame_sched, id, "frame_last_time", frame_info->prev_frame_time);
	trace_perf(rtg_frame_sched, id, "frame_last_load", frame_info->prev_frame_load);
	trace_perf(rtg_frame_sched, id, "frame_last_load_util",
		frame_info->prev_frame_load_util);
	...
	trace_perf(rtg_frame_sched, id, "frame_vload", frame_info->frame_vload);
}
...
static void do_frame_end(struct frame_info *frame_info, bool fake)
{
	...
	trace_perf(rtg_frame_sched, id, "frame_last_load_util",
		frame_info->prev_frame_load_util);
	trace_perf(rtg_frame_sched, id, "frame_last_fake_load_util",
		frame_info->prev_fake_load_util);
	trace_perf(rtg_frame_sched, id, "frame_util", frame_info->frame_util);
	trace_perf(rtg_frame_sched, id, "frame_vload", frame_info->frame_vload);
}

static void set_frame_end(struct frame_info *frame_info)
{
	trace_perf(rtg_frame_sched, frame_info->rtg->id, "FRAME_STATUS", FRAME_END);
...
int set_frame_status(struct frame_info *frame_info, unsigned long status)
{
	...
	id = grp->id;
	trace_perf(rtg_frame_sched, id, "FRAME_TIMESTAMP_SKIPPED",
	...
	trace_perf(rtg_frame_sched, id, "FRAME_MARGIN_IMME", frame_info->margin_imme);
	trace_perf(rtg_frame_sched, id, "FRAME_TIMESTAMP", status);
	...
	u64 timeline)
{
	...
	case FRAME_START:
		...
		if (!check_frame_util_invalid(frame_info, timeline)) {
			...
			frame_info->status = FRAME_INVALID;
			trace_perf(rtg_frame_sched, id, "frame_status",
				frame_info->status);
			/*
...
static void update_frame_info_tick(struct related_thread_group *grp)
{
	...
	trace_perf(rtg_frame_sched, id, "update_curr_pid", current->pid);
	trace_perf(rtg_frame_sched, id, "frame_timeline", timeline / NSEC_PER_MSEC);
...
int set_frame_min_util(struct frame_info *frame_info, int min_util, bool is_boost)
{
	...
	if (is_boost) {
		...
		trace_perf(rtg_frame_sched, id, "FRAME_BOOST_MIN_UTIL", min_util);
	} else {
