...
char spread_affinity_fast_cpus_name[SPREAD_AFFINITY_NAME_LENGTH] __wr;
#else
char spread_affinity_fast_cpus_name[SPREAD_AFFINITY_NAME_LENGTH];
#endif

static bool match_spread_affinity_fast_cpus(const char *name)
{
	char *process_name, *process_list;
	char *process_list_copy;
	int ret;
	bool found = false;

	process_list_copy = kmalloc(SPREAD_AFFINITY_NAME_LENGTH, GFP_KERNEL);
	if (!process_list_copy)
		return false;
	
	ret = strcpy_s(process_list_copy, SPREAD_AFFINITY_NAME_LENGTH, spread_affinity_fast_cpus_name);
	if (ret != 0) {
		pr_err("%s strcpy fail\n", __func__);
		goto free;
	}

	process_list = process_list_copy;
	while(process_name = strsep(&process_list, ",")) {
...
free:
	kfree(process_list_copy);
	return found;
}

static bool should_spread_affinity_fast_cpus(struct task_struct *p)
{
	return match_spread_affinity_fast_cpus(p->group_leader->comm);
}

void core_ctl_spread_affinity(cpumask_t *allowed_mask)
{
	struct cluster_data *cluster = NULL;
	struct cpumask fast_cpus_mask;
	...
	if (cluster->enable &&
		...
		get_fast_cpus(&fast_cpus_mask);

		/* Spread affinity for big cluster's subset. */
		if (cpumask_subset(allowed_mask, &cluster->cpu_mask)) {
#ifdef CONFIG_SCHED_SMT
			cpumask_or(allowed_mask, allowed_mask, &fast_cpus_mask);
#else
			/* For forward compatibility purpose only. */
		...
		/*
		 * Spread affinity for specific tasks, to correct App's
		 * strange behaviour.
		 * For example:
		 * 1. Cod game bind to middle cluster.
		 * 2. Douyin app bind to little cluster sometimes.
		 * Since douyin may also bind to little and middle, which we
		 * don't want to reject. So let's spread the mask to fast_cpus
		 * when it is subset of fast_cpus or subset of slow_cpus.
		 */
		if ((cpumask_subset(allowed_mask, &fast_cpus_mask) ||
		     !cpumask_intersects(allowed_mask, &fast_cpus_mask)) &&
		    should_spread_affinity_fast_cpus(current))
			cpumask_or(allowed_mask, allowed_mask, &fast_cpus_mask);
	}
}
...
void core_ctl_check(bool sched_avg_updated)
{
	...
	if (unlikely(g_core_ctrl_enable == false))
		return;
#endif
...
static int __ref cpuhp_core_ctl_online(unsigned int cpu)
{
	...
	/*
	 * To consider active cpus is greater than need cpus
	 * when the cpu is online.
	 */
	cluster->active_cpus = get_active_cpu_count(cluster);
	if (cluster->active_cpus > cluster->need_cpus)
		apply_need(cluster, true);
	else
		apply_need(cluster, false);
