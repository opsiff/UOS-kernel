...
#include "uclamp_sched_tunnables.h"
...
}; /* end of struct task_group */

/*
 * We do not support uclamp.min/max in schedtune since nobody use it
 * and it is very complex. See comment in uclamp_group_def.h.
 */
struct schedtune {
	struct cgroup_subsys_state css;
...
#define UCLAMP_FLAG_IDLE 0x01
	...
	u64 sum_pred_load;
	u32 curr_max_vip_load;
	u32 prev_max_vip_load;
...
enum {
	...
	PRED_LOAD_ENQUEUE		= (1 << 10),
	SET_MIN_UTIL			= (1 << 11),
	ENQUEUE_MIN_UTIL		= (1 << 12),
	FRAME_UPDATE			= (1 << 13),
#ifdef CONFIG_MIPS_LOAD_TRACKING
	MIPS_UPDATE			= (1 << 14),
	MIPS_MIGRATE			= (1 << 15),
#endif
	COLOC_GRP_WINDOW_ROLLOVER	= (1 << 16),
	COLOC_GRP_NEED_UPDATE		= (1 << 17),
};
...
#ifdef CONFIG_HISI_EAS_SCHED
static inline void cpufreq_gov_update(int cpu, unsigned int flags)
{
	sugov_mark_util_change(cpu, flags);
	sugov_check_freq_update(cpu);
#ifdef CONFIG_MIPS_LOAD_TRACKING
	mips_check_freq_update(cpu, flags);
#endif
}
#endif
