 ...
 *
 * Copyright (c) 2018-2023 Huawei Technologies Co., Ltd.
 *
...
int sched_set_group_window_rollover(unsigned int grp_id)
{
	...
	list_for_each_entry(p, &grp->tasks, grp_list) {
		...
		boost = uclamp_task_util_boost(p);
#endif
		if (boost > grp->max_boost)
			grp->max_boost = boost;

		boost = uclamp_task_placement_boost(p);
		if (boost > grp->max_placement_boost)
			grp->max_placement_boost = boost;
	}
			       ...
			       struct task_struct *p, int event)
{
	...
	walt_update_task_ravg(p, rq, TASK_MIGRATE, wallclock, 0);
				    ...
				    unsigned int flag)
{
	...
	if (need_update_prev_freq) {
		...
		mipsgov_mark_freq_change(prev_cpu);
		mips_check_freq_update(prev_cpu, MIPS_UPDATE);
#endif
	...
	if (need_update_next_freq) {
#ifdef CONFIG_MIPS_LOAD_TRACKING
		mipsgov_mark_freq_change(next_cpu);
		mips_check_freq_update(next_cpu, MIPS_UPDATE);
#endif
