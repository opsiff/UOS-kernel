...
static void update_rq_highest_prio(struct rq *rq)
{
	...
	list_for_each_entry(task, &rq->vip_thread_list, vip_entry) {
		highest_priority = max(highest_priority, task->vip_prio);

		if (++task_cnt >= VIP_TASK_LIMIT_ITER_NUM)
			break;
	}

	rq->highest_vip_prio = highest_priority;
}
#endif
...
void enqueue_vip_thread(struct rq *rq, struct task_struct *p)
{
	...
	if (p->static_vip || atomic64_read(&p->dynamic_vip)) {
		list_for_each_safe(pos, num, &rq->vip_thread_list) {
			if (pos == &p->vip_entry) {
				exist = true;
				break;
			}
...
pick_first_vip_thread(struct rq *rq)
{
	...
	struct task_struct *num = NULL;
	unsigned int task_cnt = 0;

	list_for_each_entry_safe(task, num, &rq->vip_thread_list, vip_entry) {
...
static struct task_struct *pick_first_vip_thread(struct rq *rq)
{
	...
	list_for_each_safe(pos, num, vip_thread_list) {
...
void pick_vip_thread(struct rq *rq,
	struct task_struct **task, struct sched_entity **se)
{
			...
			struct cpumask *search_cpus)
{
	int i;
	int target_cpu = -1;
	unsigned int lowest_priority = p->vip_prio;
...
bool task_fits_max(struct task_struct *task, int cpu)
{
	...
	if (task_fits(task, cpu))
		return true;

	/* Now task does not fit. Check if there's a better one. */
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5,4,0))
	cpumask_and(&allowed_cpus, task->cpus_ptr, cpu_online_mask);
#else
	cpumask_and(&allowed_cpus, &task->cpus_allowed, cpu_online_mask);
#endif
			...
			struct task_struct *pre, struct task_struct *next)
{
	...
	if (pre->on_rq) {
		...
		if (schedtune_prefer_idle(pre))
			return VIP_PREEMPT_TOPAPP;
		if (pre->sched_class == &fair_sched_class)
			return VIP_PREEMPT_OTHER;
	} else {
		/*
		 * No preemption.
		 * Check if there are runnable tasks(pre task
...
unsigned int select_allowed_cpu(struct task_struct *task,
	struct cpumask *allowed_cpus)
{
	int cpu;

	if (task->vip_prio) {
		cpu = find_lowest_vip_cpu(task, allowed_cpus);
		if (cpu != -1)
...
bool test_dynamic_vip(struct task_struct *tsk, int type)
{
	...
	dynamic_vip = atomic64_read(&tsk->dynamic_vip);
...
static inline void dynamic_vip_dec(struct task_struct *tsk, int type)
{
	atomic64_sub(dynamic_vip_one((u32)type), &tsk->dynamic_vip);
}

static inline void dynamic_vip_inc(struct task_struct *tsk, int type)
{
	atomic64_add(dynamic_vip_one((u32)type), &tsk->dynamic_vip);
}
...
static void __dynamic_vip_dequeue(struct task_struct *task, int type)
{
	...
	if (type == DYNAMIC_VIP_FUTEX) {
		...
		rq = __task_rq_lock(task, &flag);
	} else {
		rq = task_rq_lock(task, &flag);
	}
#else
	rq = task_rq_lock(task, &flag);
#endif
...
{
	return task->static_vip & WALT_PREDICT_VIP_BOOST_STATE;
}

bool task_enable_rt_pi(struct task_struct *task)
{
	return task->static_vip & INHERIT_RT_PRIORITY;
}
...
static int vip_task_hot(struct task_struct *task,
	struct rq *src_rq, struct rq *dst_rq)
{
	...
	if (unlikely(task->policy == SCHED_IDLE))
		return 0;

	if (sched_feat(CACHE_HOT_BUDDY) && dst_rq->nr_running &&
		(&task->se == src_rq->cfs.next || &task->se == src_rq->cfs.last))
		return 1;
	...
	delta = src_rq->clock_task - task->se.exec_start;
...
static int vip_can_migrate(struct task_struct *task,
	struct rq *src_rq, struct rq *dst_rq)
{
	if (task_running(src_rq, task))
		return 0;
	if (vip_task_hot(task, src_rq, dst_rq))
		return 0;
	if (task_rq(task) != src_rq) /*lint !e58*/
		return 0;
	if (!test_task_exist(task, &src_rq->vip_thread_list))
		return 0;
	return 1;
}
...
static int __do_vip_balance(void *info)
{
	struct rq *src_rq = info;
	struct rq *dst_rq = NULL;
...
#ifdef CONFIG_VIP_SCHED_OPT
void set_thread_vip_prio(struct task_struct *p, unsigned int priority)
{
	struct rq_flags rf;
	struct rq *rq = NULL;
	bool queued = false;

	if (p == NULL || p->vip_prio == priority)
		return;
