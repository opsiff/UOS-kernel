...
#ifdef CONFIG_RWSEM_PRIO_OPT
static void rwsem_init_reader_info(struct rw_semaphore *sem)
{
	struct rwsem_reader_info *info;

	if (sem->reader_info)
		return;
	info = kmalloc(sizeof(struct rwsem_reader_info), GFP_KERNEL);
	if (!info)
		return;

	raw_spin_lock_init(&info->lock);
	info->count = 0;
	for (int i = 0; i < MAX_RD_OWNER; i++) {
		info->owner[i] = NULL;
		__clear_bit(i, info->vip_bitmap);
	}
	sem->reader_info = info;
}

void rwsem_release_reader_info(struct rw_semaphore *sem)
{
	struct rwsem_reader_info *info = sem->reader_info;

	if (!info)
		return;
	kfree(info);
	sem->reader_info = NULL;
}
#endif
...
void rwsem_set_vip(struct rw_semaphore *sem)
{
	...
	// In the MicroBench test, "|" change to "+" is more time-saving
	do {
...
#ifdef CONFIG_RWSEM_PRIO_OPT
static void set_key_reader_owned(struct rw_semaphore *sem,
				 struct task_struct *owner, unsigned long val)
{
	struct task_struct *rd_owner;
	unsigned long flags;
	struct rwsem_reader_info *reader_info = sem->reader_info;

	if (likely(!is_rwsem_vip(sem) || !reader_info))
		return;

	raw_spin_lock_irqsave(&reader_info->lock, flags);
	if (reader_info->count >= MAX_RD_OWNER)
		goto unlock;

	for (int i = 0; i < MAX_RD_OWNER; i++) {
		if (!reader_info->owner[i]) {
			rd_owner = (struct task_struct *)(val & ~RWSEM_OWNER_FLAGS_MASK);
			get_task_struct(rd_owner);
			reader_info->owner[i] = rd_owner;
			reader_info->count++;
			break;
		}
	}
unlock:
	raw_spin_unlock_irqrestore(&reader_info->lock, flags);
}

static void clear_key_reader_owned(struct rw_semaphore *sem)
{
	int count = 0;
	int vip_bit;
	unsigned long flags;
	...
	if (likely(!reader_info))
		return;

	raw_spin_lock_irqsave(&reader_info->lock, flags);
	if (likely(!reader_info->count))
		goto unlock;

	for (int i = 0; count < reader_info->count && i < MAX_RD_OWNER; i++) {
		if (current == reader_info->owner[i]) {
			vip_bit = i;
			reader_info->owner[i] = NULL;
			reader_info->count--;
			rwsem_read_dynamic_vip_dequeue(sem, current, vip_bit);
...
#ifdef CONFIG_RWSEM_PRIO_OPT
	set_key_reader_owned(sem, owner, val);
#endif
}
...
#ifdef CONFIG_RWSEM_PRIO_OPT
	if ((task_flags & RWSEM_READER_OWNED) &&
		is_rwsem_vip(sem) && reader_info && !need_resched()) {
		raw_spin_lock_irqsave(&reader_info->lock, flags);
		for (int i = 0; task_cnt < reader_info->count && i < MAX_RD_OWNER; i++) {
			if (reader_info->owner[i]) {
				vip_bit = i;
				rwsem_read_dynamic_vip_enqueue(waiter.task,
								current,
								reader_info->owner[i],
								sem, vip_bit);
				task_cnt++;
...
#endif
#ifdef CONFIG_HW_VIP_THREAD
	dynamic_vip_dequeue(current, DYNAMIC_VIP_RWSEM);
#endif
...
{
#ifdef CONFIG_DFX_HUNTASK_MMAP_SEM_DBG
...
#ifdef CONFIG_DFX_HUNGTASK_HUAWEI_MMAP_SEM_DBG
	clear_remote_mm();
	trace_hck_mmap_sem_debug(sem);
#endif
	return 0;
