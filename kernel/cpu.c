...
{
	return down_read_trylock(&cpu_hotplug_lock);
}
...
void lockdep_assert_cpus_held(void)
{
	...
	WARN_ON(!rwsem_is_locked(&cpu_hotplug_lock));
}
#else
...
unsigned int sysctl_offline_last_compat_core_enabled = 0;

bool is_cpu_down_allowed(unsigned int cpu)
{
	struct cpumask compat_cpumask;
	struct cpumask compat_down_cpumask;

	if (sysctl_offline_last_compat_core_enabled)
		return true;

	cpumask_copy(&compat_cpumask, system_32bit_el0_cpumask());
	if (!cpumask_test_cpu(cpu, &compat_cpumask))
		return true;

	cpumask_andnot(&compat_down_cpumask, &compat_cpumask, cpu_online_mask);
#ifdef CONFIG_CPU_ISOLATION_OPT
	cpumask_or(&compat_down_cpumask, &compat_down_cpumask, cpu_isolated_mask);
#endif
	cpumask_set_cpu(cpu, &compat_down_cpumask);
	if (cpumask_subset(&compat_cpumask, &compat_down_cpumask))
		return false;
...
static int cpu_up(unsigned int cpu, enum cpuhp_state target)
{
	...
	CALL_HCK_VH(hck_dfx_bbox_cpu_up_diaginfo_record, cpu, err);
...
struct cpumask __cpu_primary_mask __read_mostly;
EXPORT_SYMBOL(__cpu_primary_mask);

struct cpumask __cpu_secondary_mask __read_mostly;
EXPORT_SYMBOL(__cpu_secondary_mask);
#endif
...
#ifdef CONFIG_SCHED_SMT_USER_HIDE
void init_cpu_primary(const struct cpumask *src)
{
	cpumask_copy(&__cpu_primary_mask, src);
}

void init_cpu_secondary(const struct cpumask *src)
{
	cpumask_copy(&__cpu_secondary_mask, src);
}
#endif
