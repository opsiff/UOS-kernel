...
void raise_softirq(unsigned int nr)
{
	...
	local_irq_save(flags);
	CALL_HCK_VH(lpcpu_cpuidle_irqinfo_update, nr);
		...
		if (tasklet_trylock(t)) {
			if (!atomic_read(&t->count)) {
				...
				if (t->use_callback) {
					...
					CALL_HCK_VH(hck_dfx_bbox_tasklet_hook, (u64)(t->callback), 0);
					t->callback(t);
					CALL_HCK_VH(hck_dfx_bbox_tasklet_hook, (u64)(t->callback), 1);
					trace_tasklet_exit(t->callback);
				} else {
					trace_tasklet_entry(t->func);
					CALL_HCK_VH(hck_dfx_bbox_tasklet_hook, (u64)(t->func), 0);
					t->func(t->data);
					CALL_HCK_VH(hck_dfx_bbox_tasklet_hook, (u64)(t->func), 1);
