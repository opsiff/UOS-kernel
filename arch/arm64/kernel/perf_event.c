...
static inline u64 armv8pmu_event_mtpme(struct perf_event *event)
{
	struct arm_pmu *cpu_pmu = to_arm_pmu(event->pmu);
	return event->attr.mtpme && (cpu_pmu->features & ARM_PMU_MTPMU);
}
...
static inline void armv8pmu_write_event_type(struct perf_event *event)
{
	...
	if (armv8pmu_event_mtpme(event) && (idx != ARMV8_IDX_CYCLE_COUNTER))
		hwc->config_base |= ARMV8_PMU_EVTYPE_MTPME;
	/*
...
static void __armv8pmu_probe_pmu(void *info)
{
	...
	ptpmuver = cpuid_feature_extract_unsigned_field(dfr0,
			ID_AA64DFR0_MTPMU_SHIFT);

	if (ptpmuver == 1)
		cpu_pmu->features |= ARM_PMU_MTPMU;
