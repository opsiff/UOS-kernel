...
#include <platform_include/see/hhee_hibernate.h>
...
#ifdef CONFIG_HHEE_HIBERNATE
		ret = hhee_hibernate_freeze();
		if (ret != 0) {
			pr_err("Can't hibernate: hhee hibernate freeze fail.\n");
...
#ifdef CONFIG_HHEE_HIBERNATE
		ret = hhee_hibernate_restore();
		if (ret != 0)
			pr_err("hhee hibernate restore fail.\n");
#endif
...
#ifdef CONFIG_HHEE_HIBERNATE
	rc = hkip_hvc4(HHEE_HIBERNATE_CONFIG_DISABLE, 0, 0, HHEE_HIBERNATE_CONFIG_DISABLE);
	if (rc) {
		pr_err("failed to config hhee disable\n");
		return rc;
	}
#endif
