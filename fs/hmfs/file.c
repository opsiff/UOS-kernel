						...
						int datasync, bool atomic)
{
	...
	/*
	 * DATA must be flushed into device before NODE, or NODE may record
	 * some lbas that OOB not flushed. Then in recover_node(), the lba
	 * after curseg->offset will be recoveried, causing f2fs_bug_on in
...
static loff_t f2fs_llseek(struct file *file, loff_t offset, int whence)
{
	...
	if (f2fs_compressed_file(inode))
		maxbytes = hmfs_max_file_blocks(inode) << F2FS_BLKSIZE_BITS;
...
{
	hmfs_truncate_data_blocks_range(dn, ADDRS_PER_BLOCK(dn->inode));
}
...
int hmfs_do_truncate_blocks(struct inode *inode, u64 from, bool lock)
{
	...
	if (free_from >= hmfs_max_file_blocks(inode))
		goto free_partial;
...
int hmfs_truncate_blocks(struct inode *inode, u64 from, bool lock)
{
	...
	err = hmfs_do_truncate_blocks(inode, free_from, lock);
	if (err)
		return err;
...
static int __f2fs_ioc_setflags(struct inode *inode, unsigned int flags)
{
	...
	if ((flags ^ oldflags) & (F2FS_COMPR_FL | F2FS_NOCOMPR_FL)) {
		if (!f2fs_sb_has_compression(F2FS_I_SB(inode)))
			return -EOPNOTSUPP;
		if ((flags & F2FS_COMPR_FL) && (flags & F2FS_NOCOMPR_FL))
			return -EINVAL;
	}

	if ((flags ^ oldflags) & F2FS_COMPR_FL) {
		if (S_ISREG(inode->i_mode) &&
			(fi->i_flags & F2FS_COMPR_FL || i_size_read(inode) ||
						F2FS_HAS_BLOCKS(inode)))
			return -EINVAL;
		if (flags & F2FS_NOCOMPR_FL)
			return -EINVAL;
		if (flags & F2FS_COMPR_FL) {
			int err = hmfs_convert_inline_inode(inode);
	...
	if ((flags ^ oldflags) & F2FS_NOCOMPR_FL) {
...
int hmfs_precache_extents(struct inode *inode)
{
	...
	end = hmfs_max_file_blocks(inode);
...
static int release_compress_blocks(struct dnode_of_data *dn, pgoff_t count)
{
	...
	for (i = 0; i < count; i++) {
		...
		if (!__is_valid_data_blkaddr(blkaddr))
			continue;
		if (unlikely(!hmfs_is_valid_blkaddr(sbi, blkaddr, DATA_GENERIC)))
			return -EFSCORRUPTED;
	}
...
static int f2fs_release_compress_blocks(struct file *filp, unsigned long arg)
{
	...
	while (page_idx < last_idx) {
		...
		set_new_dnode(&dn, inode, NULL, NULL, 0);
		ret = hmfs_get_dnode_of_data(&dn, page_idx, LOOKUP_NODE);
		if (ret) {
			if (ret == -ENOENT) {
				page_idx = hmfs_get_next_page_offset(&dn,
								page_idx);
				ret = 0;
				continue;
			}
...
out:
	...
	if (ret >= 0) {
		...
		set_sbi_flag(sbi, SBI_NEED_FSCK);
		hmfs_msg(sbi->sb, KERN_WARNING, "%s: partial blocks were released i_ino=%lx "
...
static int f2fs_ioc_set_compress_option(struct file *filp, unsigned long arg)
{
	...
	if (!f2fs_is_compress_backend_ready(inode))
		hmfs_msg(sbi->sb, KERN_WARNING, "compression algorithm is successfully set, "
...
static int f2fs_ioc_decompress_file(struct file *filp, unsigned long arg)
{
	...
	if (ret)
		hmfs_msg(sbi->sb, KERN_WARNING, "%s: The file might be partially decompressed "
...
static int f2fs_ioc_compress_file(struct file *filp, unsigned long arg)
{
	...
	if (ret)
		hmfs_msg(sbi->sb, KERN_WARNING, "%s: The file might be partially compressed "
				"(errno=%d). Please delete the file.\n",
				__func__, ret);
out:
	inode_unlock(inode);
