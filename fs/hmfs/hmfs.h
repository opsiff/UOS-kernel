...
#define hmfs_printk_ratelimited(fmt, ...)	\
({											\
	static DEFINE_RATELIMIT_STATE(hmfs_rs,	\
		DEFAULT_RATELIMIT_INTERVAL, 50);	\
											\
	if (__ratelimit(&hmfs_rs))				\
...
#define F2FS_GOING_DOWN_NEED_FSCK	0x5	/* going down to trigger fsck */
...
struct hmfs_dm_manager {
	struct radix_tree_root root;	 /* two streams need two trees */
...
#define HMFS_COMPRESSED_PAGE_MAGIC	0xF5F2C000
...
static inline void set_extra_flag(struct f2fs_sb_info *sbi, int flag)
{
	...
	set_buffer_dirty(bh);
	unlock_buffer(bh);
	__sync_dirty_buffer(bh, REQ_PREFLUSH | REQ_FUA);
...
static inline void f2fs_lock_op(struct f2fs_sb_info *sbi)
{
	...
	if (sbi->cp_rwsem_owner &&
		...
		show_stack(sbi->cp_rwsem_owner, NULL, KERN_DEFAULT);
	}
...
int hmfs_do_truncate_blocks(struct inode *inode, u64 from, bool lock);
...
int hmfs_init_casefolded_name(const struct inode *dir,
			      struct f2fs_filename *fname);
int hmfs_setup_filename(struct inode *dir, const struct qstr *iname,
			int lookup, struct f2fs_filename *fname);
int hmfs_prepare_lookup(struct inode *dir, struct dentry *dentry,
			struct f2fs_filename *fname);
void hmfs_free_filename(struct f2fs_filename *fname);
struct f2fs_dir_entry *hmfs_find_target_dentry(const struct f2fs_dentry_ptr *d,
...
int hmfs_add_regular_entry(struct inode *dir, const struct f2fs_filename *fname,
			struct dentry *dentry, struct inode *inode,
			nid_t ino, umode_t mode);
int hmfs_add_dentry(struct inode *dir, const struct f2fs_filename *fname,
			struct dentry *dentry, struct inode *inode,
			nid_t ino, umode_t mode);
...
loff_t hmfs_max_file_blocks(struct inode *inode);
 ...
 * hash.c
 */
void hmfs_hash_filename(const struct inode *dir, struct f2fs_filename *fname);
...
int hmfs_flush_inline_data(struct f2fs_sb_info *sbi);
...
int hmfs_recover_inline_xattr(struct inode *inode, struct page *page);
...
void hmfs_balance_fs_bg(struct f2fs_sb_info *sbi, bool from_bg);
...
bool hmfs_segment_has_free_slot(struct f2fs_sb_info *sbi, int segno);
 ...
 * data.c
 */
void hmfs_submit_bio(struct f2fs_sb_info *sbi,
...
int hmfs_fiemap(struct inode *inode, struct fiemap_extent_info *fieinfo,
			u64 start, u64 len);
int hmfs_encrypt_one_page(struct f2fs_io_info *fio);
...
int hmfs_write_single_data_page(struct page *page, int *submitted,
				struct writeback_control *wbc,
				enum iostat_type io_type,
...
int hmfs_init_post_read_wq(struct f2fs_sb_info *sbi);
void hmfs_destroy_post_read_wq(struct f2fs_sb_info *sbi);
...
int hmfs_recover_inline_data(struct inode *inode, struct page *npage);
...
int hmfs_make_empty_inline_dir(struct inode *inode, struct inode *parent,
			struct page *ipage);
int hmfs_add_inline_entry(struct inode *dir, const struct f2fs_filename *fname,
...
void hmfs_init_extent_tree(struct inode *inode, struct page *ipage);
...
/* verity.c */
extern const struct fsverity_operations hmfs_verityops;
 ...
 * compress.c
 */
#ifdef CONFIG_HMFS_FS_COMPRESSION
bool hmfs_is_compressed_page(struct page *page);
...
int hmfs_init_compress_mempool(void);
void hmfs_destroy_compress_mempool(void);
...
#else
static inline bool hmfs_is_compressed_page(struct page *page) { return false; }
...
static inline int hmfs_init_compress_mempool(void) { return 0; }
static inline void hmfs_destroy_compress_mempool(void) { }
...
static inline void part_in_flight_rw(struct hd_struct *part,
