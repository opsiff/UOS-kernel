	...
	while (1) {
		...
		if (unlikely(is_sbi_flag_set(sbi, SBI_CP_DISABLED))) {
			if (p.alloc_mode == LFS) {
				 ...
				 */
				if (!hmfs_segment_has_free_slot(sbi, segno))
					goto next;
			}
		}
...
gc_more:
	...
	if (!(total_rounds % GC_PRINT_ROUND) ||
		...
		hmfs_msg(sbi->sb, KERN_ERR, "too many gc rounds=%u, "
			"free_sec=%u, free_seg=%u, segno=%u, prefree_sec=%u, "
			"prefree_seg=%u, gc lock time=%llu, cp_ver=%llx",
			total_rounds, free_sections(sbi), free_segments(sbi),
			segno, prefree_sections(sbi), prefree_segments(sbi),
			local_clock() - gc_time, cur_cp_version(F2FS_CKPT(sbi)));

			hmfs_datamove_tree_get_info(sbi, NULL, NULL);
			gc_time = local_clock();
	}
...
/*
 * The function needs lock gc_mutex by caller, and will move
 * the whole section where segno in.
 */
static void do_section_garbage_collect(struct f2fs_sb_info *sbi,
		unsigned int segno, int gc_type)
{
	unsigned int i;
	unsigned int start_segno = (segno / sbi->segs_per_sec) *
							sbi->segs_per_sec;
	...
	for (i = start_segno; i < end_segno; i++) {
		...
		sbi->next_victim_seg = i;
		/* do_garbage_collect moves one segment per time in unistore */
		do_garbage_collect(sbi, i, &gc_list, gc_type);
		put_gc_inode(&gc_list);
	}
}
		...
		unsigned int end)
{
	...
	for (segno = start; segno <= end; segno += sbi->segs_per_sec) {
		...
		mutex_lock(&sbi->gc_mutex);
		do_section_garbage_collect(sbi, segno, FG_GC);
		mutex_unlock(&sbi->gc_mutex);
	...
	/*
	 * After move the segments, we need clear all datamove cache to make
	 * the space free.
	 */
	mutex_lock(&sbi->gc_mutex);
	hmfs_wait_all_dm_complete(sbi);
	hmfs_datamove_force_flush(sbi, true);
	hmfs_wait_all_dm_complete(sbi);
	mutex_unlock(&sbi->gc_mutex);
...
static bool hmfs_datamove_need_force_flush(struct f2fs_sb_info *sbi)
{
	...
	/* Every dm stream needs individual cache tree. */
	while (1) {
		...
		bool need_clear_all)
{
	...
	down_write(&dm->rw_sem);
	force_flush = need_clear_all || hmfs_datamove_need_force_flush(sbi);
	for (i = 0; i < NR_CURSEG_DM_TYPE; i++) {
		if (force_flush)
					...
					unsigned int *unverify_free_sec)
{
	...
	while (1) {
		...
		for (i = 0; i < ret; i++) {
			...
			if (set_secno != tmp_secno) {
				...
				if (!valid_blocks) {
					free_sec++;
					hmfs_printk_ratelimited("%sF2FS-fs: unverify free"
						"section:%u, is prefree section:%u",
						KERN_ALERT, set_secno,
						test_bit(set_secno,
						DIRTY_I(sbi)->dirty_segmap[PRE_SEC]));
				}
				tmp_secno = set_secno;
			}
		}
	...
	for (i = 0; i < NR_CURSEG_DM_TYPE; i++)
		hmfs_msg(sbi->sb, KERN_ERR, "DM stream%u  dm_verify:%u, "
			"dm_next:%u, dm_last:%u", i, (dm->dm_info[i]).verified_blkaddr,
			(dm->dm_info[i]).next_blkaddr, (dm->dm_info[i]).cached_last_blkaddr);
