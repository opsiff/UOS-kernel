...
#ifdef CONFIG_FS_ENCRYPT_CASEFOLD
/*
 * Adapt the casefold feature to v1 policy.
 */
static void fscrypt_setup_v1_dirhash_key(struct fscrypt_info *ci, const u8 *src_dirhash)
{
	/*
	 * v1 policy does not support the casefold feature, so adapt the casefold feature to v1,
	 * Use nonce plaintext to perform sha256 algorithm, and then use it as dirhash.
	 */
	u8 digest[SHA256_DIGEST_SIZE] = { 0 };

	sha256(src_dirhash, FS_KEY_DERIVATION_CIPHER_SIZE, digest);

#ifdef CONFIG_FS_INLINE_ENCRYPT_DEBUG
	char *digest_str = kasprintf(
		GFP_KERNEL, "%*phN", SHA256_DIGEST_SIZE, digest);
	char *src_hash_str = kasprintf(
		GFP_KERNEL, "%*phN", FS_KEY_DERIVATION_CIPHER_SIZE, src_dirhash);
	fscrypt_debug(ci->ci_inode, "[FBE]%s src_dirhash: %s,digest:%s",
		     __func__, src_hash_str, digest_str);
	kfree(digest_str);
	kfree(src_hash_str);
#endif
	/*
	 * After we use the plaintext nonce to make SHA256,
	 * the hashkey of the file name SIPHASH only needs to use 16bytes, and we take the first 16bytes.
	 */
	memcpy(&ci->ci_dirhash_key, digest, sizeof(ci->ci_dirhash_key));
...
int fscrypt_setup_v1_file_key_via_subscribed_keyrings(
	...
	bool enc, bool need_dirhash_key)
{
	struct key *key;
...
#ifdef CONFIG_FS_ENCRYPT_CASEFOLD
	if (need_dirhash_key)
		fscrypt_setup_v1_dirhash_key(ci, enc ? ci->ci_nonce : derived_key);
#endif
