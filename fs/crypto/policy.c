					...
					const struct inode *inode)
{
	...
	if ((policy->flags & FSCRYPT_POLICY_FLAG_DIRECT_KEY) &&
	    !supported_direct_key_modes(inode, policy->contents_encryption_mode,
					policy->filenames_encryption_mode))
		return false;
#ifndef CONFIG_FS_ENCRYPT_CASEFOLD
 ...
 * encryption policy.  @nonce must be a new random nonce. iv is NULL when kernel
 * does not support inline encrypt.
 *
 * Return: the size of the new context in bytes.
 */
static int fscrypt_new_context(union fscrypt_context *ctx_u,
			       const union fscrypt_policy *policy_u,
			       const u8 *nonce, int nonce_len, const u8 *iv,
			       int iv_len)
{
				...
				int ctx_size)
{
	...
	/*
         * For earlier kernel version, we have extended fscrypt version to v2,
         * which diverges from 5.10 kernel, because 5.10 has its own v2. Here,
         * we only reserve old fscrypt policy version(v2) and convert it to v1.
         */
	if (ctx_u->version != FSCRYPT_CONTEXT_V2) {
		fscrypt_err(NULL,
			    "[FBE]%s ctx version not compatible with old data",
			    __func__);
		return -EINVAL;
	}
...
static int
encrypt_nonce_by_gcm(struct inode *inode, const u8 *descriptor,
		     const u8 source_nonce[FS_KEY_DERIVATION_CIPHER_SIZE],
		     u8 derived_nonce[FS_KEY_DERIVATION_CIPHER_SIZE],
		     u8 iv[FS_KEY_DERIVATION_IV_SIZE])
{
	...
	description = kasprintf(GFP_KERNEL, "%s%*phN", FSCRYPT_KEY_DESC_PREFIX,
				FSCRYPT_KEY_DESCRIPTOR_SIZE, descriptor);
	if (!description)
		return ERR_PTR(-ENOMEM);

#ifdef CONFIG_FS_INLINE_ENCRYPT_DEBUG
	fscrypt_debug(inode, "[FBE]%s master key descriptor = %s", __func__,
		     description);
#endif
	...
	if (IS_ERR(keyring_key)) {
		if (inode->i_sb->s_cop->key_prefix) {
			...
			keyring_key =
				request_key(&key_type_logon, description, NULL);
			if (!IS_ERR(keyring_key)) {
				kfree(description);
				goto got_key;
			}
...
got_key:
	...
	res = fscrypt_derive_gcm_key(tfm, source_nonce, derived_nonce, iv, 1);
	if (res)
		goto out;
out:
	if (tfm)
...
{
	u8 nonce[FS_KEY_DERIVATION_CIPHER_SIZE] = { 0 };
	...
	if (policy->version == FSCRYPT_POLICY_V1) {
		err = encrypt_nonce_by_gcm(inode,
					   policy->v1.master_key_descriptor,
					   plain_text, nonce, iv);
		if (err)
			return err;
		nonce_str = kasprintf(GFP_KERNEL, "%*phN",
				      FS_KEY_DERIVATION_NONCE_SIZE, nonce);
		iv_str = kasprintf(GFP_KERNEL, "%*phN",
				   FS_KEY_DERIVATION_IV_SIZE, iv);
	}

	ctxsize = fscrypt_new_context(&ctx, policy, nonce, sizeof(nonce), iv,
				      sizeof(iv));

	int ret = inode->i_sb->s_cop->set_context(inode, &ctx, ctxsize, NULL);
	kfree(nonce_str);
	kfree(iv_str);
	return ret;
}

static bool fscrypt_sdp_context_equal(struct inode *parent, struct inode *child)
{
	int res;
	union fscrypt_context parent_ctx, child_ctx;
	...
	return memcmp(parent_ctx.v1.master_key_descriptor,
		      child_ctx.v1.master_key_descriptor,
		      FS_KEY_DESCRIPTOR_SIZE) == 0 &&
		(parent_ctx.v1.contents_encryption_mode ==
		 child_ctx.v1.contents_encryption_mode) &&
		(parent_ctx.v1.filenames_encryption_mode ==
		 child_ctx.v1.filenames_encryption_mode) &&
		(parent_ctx.v1.flags == child_ctx.v1.flags);
}
...
sdp_perm:
	return fscrypt_sdp_context_equal(parent, child);
/* TicketNo:AR0009DF3P -- End */
}
...
int fscrypt_set_context(struct inode *inode, void *fs_data)
{
	...
	char *nonce = kasprintf(GFP_KERNEL, "%*phN",
				FS_KEY_DERIVATION_NONCE_SIZE, ci->ci_nonce);
	char *iv = kasprintf(GFP_KERNEL, "%*phN", FS_KEY_DERIVATION_IV_SIZE,
			     ci->ci_iv);
	fscrypt_debug(inode,
		     "[FBE]%s encrypted nonce = %s, nonce + 64 = %s, iv = %s",
		     __func__, nonce, nonce + FS_KEY_DERIVATION_NONCE_SIZE, iv);
	kfree(nonce);
	kfree(iv);
#endif

	ctxsize = fscrypt_new_context(&ctx, &ci->ci_policy, ci->ci_nonce,
				      sizeof(ci->ci_nonce), ci->ci_iv,
				      sizeof(ci->ci_iv));
