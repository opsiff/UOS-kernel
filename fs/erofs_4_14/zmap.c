...
static void print_mem(struct z_erofs_maprecorder *m)
{
	if (m->kaddr != NULL) {
		print_hex_dump(KERN_ERR, "[in]: ", DUMP_PREFIX_OFFSET, 16, 1, m->kaddr, PAGE_SIZE, true);
	} else {
		errln("m->kaddr = NULL");
	}
}
				  ...
				  unsigned int eofs)
{
	...
	if (1 << amortizedshift == 4) {
		...
		errln("error in unpack_compacted_index, amortizedshift = %d, lclusterbits = %d", amortizedshift, lclusterbits);
		print_mem(m);
		return -ENOTSUPP;
	}
						...
						unsigned long lcn)
{
	...
	if (lcn >= totalidx) {
		errln("lcn = %lu, totalidx = %u, inode->i_size %llu", lcn, totalidx, inode->i_size);
		return -EINVAL;
	}
				      ...
				      unsigned int lcn)
{
	...
	errln("error in vle_load_cluster_from_disk because datamode = %u", datamode);
	return -EINVAL;
}
			    ...
			    int flags)
{
	...
	if (err) {
		errln("fill_inode_lazy ERROR, err: %d", err);
		goto out;
	}
	...
	if (err) {
		errln("vle_load_cluster_from_disk ERROR, err: %d", err);
		goto unmap_out;
	}
...
out:
	if (err < 0 && err != -ENOMEM) {
		errln("fill_inode_lazy, nid: %lld, EROFS_V_Z_INITED_BIT: %lu", vi->nid, vi->flags);
		errln("vle_load_cluster_from_disk, datamode: %d", vi->datamode);
		errln("inode->i_size %llu, lclusterbits %u", inode->i_size, lclusterbits);
		errln("offset %llu of nid %llu, err: %d", ofs, vi->nid, err);
		errln("%s, m_la %llu m_pa %llu m_llen %llu m_plen %llu m_flags 0%o",
