...
/*
 * this interface called by VIP readahead of hisi.
 * the target page locate in mid of prefetch window
 *
 * The continuity of read addresses should guaranteed by the caller
 *
 * This patch add z_erofs_mas_readahead to support mas_readahead,
 * which is the interface of readhead added by hisi.
 * It will allow be triggered from below path:
 *
 * 1. z_erofs_mas_readahead enable
 * trace
 * - z_erofs_vle_normalaccess_readpages
 * - z_erofs_mas_readahead
 * - pch_read_pages
 * - pch_read_around
 * - filemap_fault
 *
 * 2. z_erofs_mas_readahead disable
 * trace
 ...
 * - filemap_fault
 *
 * as erofs is designed to wait for IO compelete in each read op,
 * with z_erofs_mas_readahead disabled,patch reads will
 * be forced to break down into serial read. One by one.
 *
 * For example, android need read some essential resources(apk)
 * during start of camera. It will occurs a lot of sequential read(resources be dropped).
 * With z_erofs_mas_readahead enabled,load HwCamera2/base-master.apk will
 * save about 50% time cost(25ms->13.7ms).
 */
#ifdef CONFIG_MM_PAGECACHE_HELPER
static int z_erofs_mas_readahead(struct file *file,
				 struct address_space *mapping,
				 struct list_head *pages,
				 unsigned nr_pages,
				 struct readahead_control *rac)
{
	rac->_nr_pages -= nr_pages;
	rac->_index += nr_pages;
	return z_erofs_vle_normalaccess_readpages(file, mapping, pages,
			nr_pages);
}
#endif
...
#ifdef CONFIG_MM_PAGECACHE_HELPER
	.mas_readahead = z_erofs_mas_readahead,
#endif
};
