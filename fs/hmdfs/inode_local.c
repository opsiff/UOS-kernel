...
out:
	...
	if (local_res) {
		...
		if (adapter_identify_dentry_flag(dir, dentry, lower_dentry) ==
			...
			hmdfs_adapter_create(sbi, dentry, relative_dir_path, NULL);
		}
#endif
	}
	if (error) {
...
out_created:
	...
	if (adapter_identify_dentry_flag(dir, dentry, lower_dentry) ==
	    ADAPTER_PHOTOKIT_DENTRY_FLAG)
		hmdfs_adapter_create(sbi, dentry, relative_dir_path, symname);
	kfree(relative_dir_path);
#endif
out_err:
	hmdfs_put_lower_path(&lower_path);
...
int hmdfs_permission(struct inode *inode, int mask)
{
	...
	uid_t user_system_uid = multiuser_get_uid(sbi->user_id, AID_SYSTEM);
	gid_t user_media_rw_gid = multiuser_get_uid(sbi->user_id, AID_MEDIA_RW);

	if (uid_eq(cur_uid, ROOT_UID) || uid_eq(cur_uid, KUIDT_INIT(user_system_uid)))
		return 0;
	...
	if (uid_eq(cur_uid, inode->i_uid)) {
		...
		if (in_group_p(KGIDT_INIT(user_media_rw_gid)))
			return 0;
	}
