...
struct connection {
	...
	int fd;
	/*
	 * To achieve send atomicity between different codepaths in hmdfs.
	 *
	 * HMDFS operations may be requested from many applications on the same
	 * connection simulateneosly on the client side, and on the server side
	 * some F_* operations are executed in async mode. We have to serialize
	 * sending activity here because:
	 *
	 * 1. Some transports (such as TCP) can result in intermixed messages
	 * 2. A sending operation may request rekey which leads to data race
	 *    on the key material without synchronization
	 *
	 * Use a single mutex to avoid these problems in a general way.
	 */
