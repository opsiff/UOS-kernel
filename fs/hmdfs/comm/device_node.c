				       ...
				       struct socket *sock)
{
	...
	conn_info->d2dp_version  = cmd->version;
	conn_info->device_type   = cmd->device_type;
	conn_info->binded_fd     = cmd->binded_fd;
#endif
}

static const char *hmdfs_get_str_link_type(int link_type)
{
	switch (link_type) {
	case LINK_TYPE_P2P:
		return "P2P";
					   ...
					   struct hmdfs_sb_info *sbi)
{
	...
	if (unlikely(cmd.status > CONNECT_STAT_WAIT_RESPONSE)) {
		hmdfs_err("cmd status error");
		goto out;
	}
...
static const size_t cmd_to_len[NOTIFY_CNT] = {
	[NOTIFY_HS_DONE] = sizeof(struct notify_hs_done_param),
	[NOTIFY_OFFLINE] = sizeof(struct notify_offline_param),
	[NOTIFY_OFFLINE_IID] = sizeof(struct notify_offline_iid_param),
	[NOTIFY_GET_SESSION] = sizeof(struct notify_get_session_param),
	[NOTIFY_REGET_SESSION] = sizeof(struct notify_reget_session_param),
	[NOTIFY_DISCONNECT] = sizeof(struct notify_disconnect_param),
	[NOTIFY_D2DP_REMOTE] = sizeof(struct notify_d2dp_remote_param),
	[NOTIFY_D2DP_FAILED] = sizeof(struct notify_d2dp_failed_param),
};

static ssize_t
sbi_cmd_show(struct kobject *kobj, struct sbi_attribute *attr, char *buf)
{
	...
	out_len = kfifo_out_peek(&sbi->notify_fifo, &cmd, sizeof(uint32_t));
	if (out_len < sizeof(uint32_t)) {
		err = -EINVAL;
		goto unlock;
	}

	if (WARN_ON(cmd > sizeof(cmd_to_len))) {
		hmdfs_err("Unsupported cmd: cmd = %d", cmd);
		err = -EINVAL;
		goto unlock;
	}

	param_len = cmd_to_len[cmd];
	out_len = kfifo_out(&sbi->notify_fifo, buf, param_len);

unlock:
	spin_unlock(&sbi->notify_fifo_lock);

	if (WARN_ON(out_len != param_len))
		err = -EINVAL;

	return err ? err : param_len;
}
...
static const char *hmdfs_get_str_con_type(const struct connection *conn_impl)
{
	switch (conn_impl->type) {
	case CONNECT_TYPE_TCP:
		return "TCP";
#ifdef CONFIG_HMDFS_D2DP_TRANSPORT
	case CONNECT_TYPE_D2DP: {
		struct d2dp_handle *d2dp_handle = conn_impl->connect_handle;

		switch (d2d_get_current_version(d2dp_handle->proto)) {
		case 0:
			return "D2DPv1";
		case 1:
			return "D2DPv2";
	...
	list_for_each_entry(peer, &sbi->connections.node_list, list) {
		...
		list_for_each_entry(conn_impl, &peer->conn_impl_list, list) {
			...
			size += sprintf(buf + size, "\t%d \t%d \t%d \t%p \t%ld"
					...
					hmdfs_get_str_link_type(conn_impl->link_type),
					hmdfs_get_str_con_type(conn_impl),
...
static void notify(struct hmdfs_peer *node, const void *param, size_t len)
{
	...
	spin_lock(&sbi->notify_fifo_lock);
	in_len = kfifo_in(&sbi->notify_fifo, param, len);
	spin_unlock(&sbi->notify_fifo_lock);

	if (WARN_ON(in_len != len))
		return;
...
{
	struct notify_hs_done_param param = {
		.notify = NOTIFY_HS_DONE,
		.remote_iid = node->iid,
	};

	memcpy(param.remote_cid, node->cid, HMDFS_CID_SIZE);
	notify(node, &param, sizeof(param));
}

void notify_offline(struct hmdfs_peer *node)
{
	struct notify_offline_param param = {
		.notify = NOTIFY_OFFLINE,
		.remote_iid = node->iid,
	};
...
{
	struct notify_offline_iid_param param = {
		.notify = NOTIFY_OFFLINE_IID,
		.remote_iid = node->iid,
	};
...
void notify_reget_session(struct connection *conn)
{
	...
	struct notify_reget_session_param param = {
		.notify = NOTIFY_REGET_SESSION,
		.fd = conn->fd,
		.remote_iid = peer->iid,
	};

	memcpy(param.remote_cid, peer->cid, HMDFS_CID_SIZE);
	notify(peer, &param, sizeof(param));
}

void notify_get_connection(struct hmdfs_peer *peer, int flag)
{
	struct notify_get_session_param param = {
...
{
	struct hmdfs_peer *peer = conn->node;
	struct notify_disconnect_param param = {
...
{
	struct hmdfs_peer *peer = conn->node;
	struct notify_d2dp_failed_param param = {
...
void notify_d2dp_remote(struct connection *conn_impl)
{
	struct notify_d2dp_remote_param param = {0};
	...
	hmdfs_info("notify userspace about remote D2DP: port=%hu, "
		   "local dtype=%d, remote dtype=%d, remote version=%d",
		   tcp->d2dp_remote_udp_port, tcp->local_device_type,
		   tcp->remote_device_type, tcp->d2dp_remote_version);

	param.notify = NOTIFY_D2DP_REMOTE;
	param.fd = conn_impl->fd;
	param.remote_udp_port = tcp->d2dp_remote_udp_port;
	param.remote_version = tcp->d2dp_remote_version;
	param.remote_device_type = tcp->remote_device_type;
	memcpy(param.remote_cid, conn_impl->node->cid, HMDFS_CID_SIZE);
	notify(conn_impl->node, &param, sizeof(param));
}
#endif
