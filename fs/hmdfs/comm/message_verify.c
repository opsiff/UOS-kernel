...
static void mkdir_message_verify_init(void)
{
	...
	message_length[C_REQUEST][F_MKDIR][HMDFS_MESSAGE_LEN_JUDGE_INDEX] =
		MESSAGE_LEN_JUDGE_RANGE;
	message_length[C_RESPONSE][F_MKDIR][HMDFS_MESSAGE_MIN_INDEX] = 0;
...
static void create_message_verify_init(void)
{
	...
	message_length[C_REQUEST][F_CREATE][HMDFS_MESSAGE_LEN_JUDGE_INDEX] =
		MESSAGE_LEN_JUDGE_RANGE;
	message_length[C_RESPONSE][F_CREATE][HMDFS_MESSAGE_MIN_INDEX] = 0;
...
static int hmdfs_readpage_verify(int flag, size_t len, void *data)
{
	...
	req = data;
	return le32_to_cpu(req->size) > HMDFS_PAGE_SIZE ? -EINVAL : 0;
}
