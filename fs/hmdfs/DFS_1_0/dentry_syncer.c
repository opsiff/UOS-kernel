...
static void release_task_adaptor(struct kref *ref)
{
...
static void timer_call_back(unsigned long id)
{
	...
	if (task->task_req->status != START) {
		kref_put(&task->ref, release_task_adaptor);
		return;
	}
			 ...
			 struct metabase *result, int len)
{
	...
	if (!add_dentry_sync_task(tmp_task, &syncer_prior_list)) {
		kref_put(&tmp_task->ref, release_task_adaptor);
		err = -ENOSPC;
		return err;
	}
