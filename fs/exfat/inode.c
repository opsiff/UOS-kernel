	...
	if (*clu == EXFAT_EOF_CLUSTER) {
		...
		if (new_clu.dir == EXFAT_EOF_CLUSTER ||
			...
			if (sdcard_dclient && !dsm_client_ocuppy(sdcard_dclient)) {
				dsm_client_record(sdcard_dclient, "bogus cluster new allocated (last_clu : %u, new_clu : %u)",
						last_clu, new_clu.dir);
...
#ifdef CONFIG_HUAWEI_SDCARD_DSM
			if (sdcard_dclient && !dsm_client_ocuppy(sdcard_dclient)) {
				dsm_client_record(sdcard_dclient, "requested for bmap out of range(pos : (%llu) > i_size_aligned(%llu)\n)",
						pos, ei->i_size_aligned);
				dsm_client_notify(sdcard_dclient, DSM_SDCARD_FILESYSTEM_ERR);
			}
#endif
...
#ifdef CONFIG_HUAWEI_SDCARD_DSM
			if (sdcard_dclient && !dsm_client_ocuppy(sdcard_dclient)) {
				dsm_client_record(sdcard_dclient, "invalid size(size(%llu) > aligned(%llu)\n)",
						i_size_read(inode), EXFAT_I(inode)->i_size_aligned);
				dsm_client_notify(sdcard_dclient, DSM_SDCARD_FILESYSTEM_ERR);
			}
#endif
