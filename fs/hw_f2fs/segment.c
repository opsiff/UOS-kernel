		...
		bool *time_after, bool *balance_node, bool *balance_data)
{
	...
	struct timespec64 ts = {DEF_DIRTY_STAT_INTERVAL, 0};
...
void f2fs_balance_fs(struct f2fs_sb_info *sbi, bool need)
{
	...
	if (has_not_enough_free_secs(sbi, 0, 0)) {
		...
		if (!!(gc_th->f2fs_gc_task != NULL)) {
			...
			current->flags |= PF_MUTEX_GC;
#ifdef CONFIG_F2FS_TURBO_ZONE
			f2fs_gc(sbi, true, false, false, false, NULL_SEGNO);
#else
...
static int f2fs_blkdev_issue_flush(struct f2fs_sb_info *sbi,
				struct block_device *bdev)
{
	struct bio *bio;
	int ret;

	bio = f2fs_bio_alloc(sbi, 0, true);
	if (!bio)
		return -ENOMEM;
...
static int __submit_flush_wait(struct f2fs_sb_info *sbi,
				struct block_device *bdev)
{
	int ret = f2fs_blkdev_issue_flush(sbi, bdev);
...
#ifdef CONFIG_F2FS_TURBO_ZONE
	f2fs_bug_on(sbi, seg_type == CURSEG_TURBO_DATA);
#endif
...
#ifdef CONFIG_F2FS_TURBO_ZONE
	if (is_tz_existed(sbi))
		__f2fs_save_inmem_curseg(sbi, CURSEG_TURBO_DATA);
#endif
}
...
#ifdef CONFIG_F2FS_TURBO_ZONE
	if (is_tz_existed(sbi))
		__f2fs_restore_inmem_curseg(sbi, CURSEG_TURBO_DATA);
#endif
}
				 ...
				 unsigned long long age)
{
	...
	if (!v_ops->get_victim(sbi, &segno, BG_GC, type, SSR, age)) {
		curseg->next_segno = segno;
		return 0;
	}
...
static int allocate_segment_in_turbozone(struct f2fs_sb_info *sbi,
					struct curseg_info *curseg,
					int type, unsigned long long age)
{
	unsigned int segno = curseg->segno;
	int ret = -ENOSPC;
	...
	if (sbi->tz_info.free_segs > F2FS_MIN_SEGS_IN_TZ ||
		...
		ret = get_ssr_segment_in_tz(sbi, curseg, &segno, type, age);
		if (ret == 0)
			change_curseg(sbi, type, true);
	}

	if (ret && type == CURSEG_TURBO_DATA) {
...
static void allocate_segment_multizone(struct f2fs_sb_info *sbi,
				struct curseg_info *curseg, int type,
				bool force, unsigned long long age)
{
	int ret = -1;
	if (is_allocated_in_tz(sbi, type)) {
		/* May fail to get new segment */
		ret = allocate_segment_in_turbozone(sbi, curseg, type, age);
		if (ret == -ENOSPC)
			f2fs_warn(sbi, "allocate_segment_in_turbozone failed");
	}

	if (type != CURSEG_TURBO_DATA && ret) {
		/* must succeed */
					...
					int type, bool force, int contig_level)
{
	...
	if (force)
		...
		new_curseg(sbi, type, false);
#ifdef CONFIG_F2FS_GRADING_SSR
	else if (need_ssr_by_type(sbi, type, contig_level) &&
		...
		int contig_level)
{
	...
	if (is_gc_test_set(sbi, GC_TEST_ENABLE_GC_STAT)) {
		if (page == NULL || (page && page_private_gcing(fio->page))) {
			if (curseg->alloc_type == SSR)
...
void init_turbo_curseg(struct f2fs_sb_info *sbi)
{
	...
	allocate_segment_in_turbozone(sbi, curseg, CURSEG_TURBO_DATA, 0);
...
static int build_curseg(struct f2fs_sb_info *sbi)
{
	...
	for (i = 0; i < NO_CHECK_TYPE; i++) {
		...
		if (i < NR_PERSISTENT_LOG)
			...
			array[i].seg_type = CURSEG_COLD_DATA;
#ifdef CONFIG_F2FS_TURBO_ZONE
		else if (i == CURSEG_TURBO_DATA)
			array[i].seg_type = CURSEG_WARM_DATA;
#endif
