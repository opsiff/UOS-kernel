...
ssize_t kernel_read(struct file *file, void *buf, size_t count, loff_t *pos)
{
	...
	if (ret)
		return ret;
#endif
	ret = __kernel_read(file, buf, count, pos);
#ifdef CONFIG_SCSI_UFS_ENHANCED_INLINE_CRYPTO_V3
	rw_finish(READ, file);
#endif
	return ret;
...
#ifdef CONFIG_HW_STAT_IOSTAT_VFS
	__hw_iostat_acct_vfs(file, HW_OP_READ_VFS, ret,
...
ssize_t vfs_write(struct file *file, const char __user *buf, size_t count, loff_t *pos)
{
	...
	inc_syscw(current);
#ifdef CONFIG_HW_STAT_IOSTAT_VFS
	__hw_iostat_acct_vfs(file, HW_OP_WRITE_VFS, ret,
			     ktime_get_ns() - start_time_ns);
#endif
	file_end_write(file);
#ifdef CONFIG_SCSI_UFS_ENHANCED_INLINE_CRYPTO_V3
...
ssize_t ksys_read(unsigned int fd, char __user *buf, size_t count)
{
	...
	if (f.file) {
		...
		if (fd == current->cpuinfo_fd)
			current->getting_max_cap = true;
#endif
