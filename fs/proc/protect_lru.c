...
#if	(KERNEL_VERSION(5, 10, 0) <= LINUX_VERSION_CODE)
			memcg_prot_file = memcg_page_state(memcg, NR_FILE_PAGES);
#else
			memcg_prot_file = memcg_page_state(memcg, MEMCG_CACHE);
#endif
			if (memcg_prot_file == 0)
				continue;

			reclaim += shrink_prot_memcg(memcg);
		}

		/*
		 * In the worst case, file lru list is extremely low while protect lru list
		 * have some pages that cannot be reclaimed and is_prot_memcg_overratio() is
		 * always return true. As a result, the loop will stuck for a long time (> 2s).
		 * Return if no page can be reclaimed is a wise choice.
		 * Meanwhile, some protect lru list may not have any page, just skip it.
		 */
		if (reclaim == 0) {
...
int rejudge_protect_page(struct page *page, struct address_space *mapping, gfp_t gfp_mask, void *shadow)
{
	...
	if (mapping && mapping->host && mapping->host->i_protect) {
		if (distance > protect_refault_distance) {
			...
			ret = mem_cgroup_charge_nonprotect(page, current->mm, gfp_mask);
			to_vec = page_lruvec(page);
			file_page_move_memcg_account(page, from_vec, to_vec, ret);
		}
		trace_mm_filemap_refault_distance(distance);
	}
	return ret;
}
#endif
...
#ifdef CONFIG_PROTECT_LRU_DEBUG
	proc_create_single("protect_file_count", 0, NULL, protect_file_show);
#endif
#ifdef CONFIG_MEMCG_FINE_GRANULARITY_PROTECT_LRU
