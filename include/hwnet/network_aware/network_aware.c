...
#define AWARE_FS_BG_PIDS "bg_pids"
...
static bool is_bg_pid(int pid)
{
	int i;
	bool ret = false;

	spin_lock(&(netinfo.bg_lock));
	for (i = 0; i < netinfo.bg_pid_num; i++) {
		if (pid == netinfo.bg_pids[i]) {
			ret = true;
			break;
		}
	}
...
void tcp_network_aware(bool is_recving)
{
	...
	if (is_bg(cur_uid))
		limit_bg();
	if (cur_uid == UID_SYSTEM && is_bg_pid(cur_pid))
		limit_bg();
}
...
void stat_bg_network_flow(bool is_recving, int len)
{
	...
	if (!is_bg(cur_uid)) {
		if (cur_uid != UID_SYSTEM)
			return;
		if (!is_bg_pid(cur_pid))
			return;
	}
...
static int bg_pids_show(struct seq_file *m, void *v)
{
	int i;

	for (i = 0; i < netinfo.bg_pid_num; i++)
		seq_printf(m, "bg_pids[%d]: %d\n", i, netinfo.bg_pids[i]);

	seq_printf(m, "bg_is_limit:%d\n", atomic_read(&(netinfo.bg_limit)));
	return 0;
}

static int bg_pids_open(struct inode *inode, struct file *filp)
{
	return single_open(filp, bg_pids_show, inode);
}
...
static ssize_t bg_pids_write(struct file *file, const char __user *buf,
	size_t count, loff_t *ppos)
{
	char buffer[MAX_ARRAY_LENGTH];
	char *cur = NULL;
	...
	for (i = 0; i < MAX_UID_NUM; i++) {
		if (pos == NULL)
			break;
		netinfo.bg_pids[i] = simple_strtol(pos, NULL, 0);
		pos = strsep(&cur, ";");
	}
	netinfo.bg_pid_num = i;
	memset_s(&netinfo.bg_net_stat, sizeof(netinfo.bg_net_stat), 0, sizeof(netinfo.bg_net_stat));
	reinit_bg_stats();
	spin_unlock(&(netinfo.bg_lock));
...
static const struct file_operations proc_bg_pids_operations = {
	.open = bg_pids_open,
	.read = seq_read,
	.write = bg_pids_write,
	.llseek = seq_lseek,
...
static const struct proc_ops proc_bg_pids_operations = {
	.proc_open = bg_pids_open,
	.proc_read = seq_read,
	.proc_write = bg_pids_write,
	.proc_lseek = seq_lseek,
	.proc_release = single_release,
};
...
static void __net_init aware_uids_proc_fs_init(struct proc_dir_entry *p_parent)
{
	...
	struct proc_dir_entry *p_temp4 = NULL;
	...
	p_temp4 = proc_create(AWARE_FS_BG_PIDS, S_IRUSR | S_IWUSR, p_parent,
		&proc_bg_pids_operations);
	if (p_temp4 == NULL)
		goto out_p_temp3;
...
static void aware_net_exit(struct net *net)
{
	...
	remove_proc_entry(AWARE_FS_BG_PIDS, net->proc_net);
