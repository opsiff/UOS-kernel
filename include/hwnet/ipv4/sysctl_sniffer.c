...
{
	g_configs.curpos[g_configs.cur_file] += (unsigned int)len;
}
...
#endif
	st_pcap_pkthdr.time.gmt_time = (unsigned int)tv.tv_sec;
#if LINUX_VERSION_CODE < KERNEL_VERSION(5,10,0)
		st_pcap_pkthdr.time.micro_time = tv.tv_usec;
#else
		st_pcap_pkthdr.time.micro_time = (unsigned int)tv.tv_nsec / 1000;
#endif
	st_pcap_pkthdr.caplen = (unsigned int)packet_len;
	st_pcap_pkthdr.len = (unsigned int)orig_len;
	...
	size_t *lenp, loff_t *ppos)
{
	...
	file_pos = (int)sniffer_file_pos(file_index);
...
{
	pCmd2Parse->i_paras[0] = (int)sniffer_file_pos(file_index);
	// return the time of the last packet in the file
	pCmd2Parse->i_paras[1] = (int)sniffer_file_gmttime(file_index);
	pCmd2Parse->i_paras[2] = (int)sniffer_file_microtime(file_index);
	// return packet count
	pCmd2Parse->i_paras[3] = (int)sniffer_file_packetcount(file_index);
...
static int proc_handle_command_file_spec(ctl_cmd_stru *pCmd2Parse, char __user *buffer)
{
	...
	if (pCmd2Parse->i_paras[0] == -1 || pCmd2Parse->i_paras[0] == 0xff ||
		...
		pCmd2Parse->i_paras[0] = (int)SNIFFER_MAX_FILE_NUM;
		pCmd2Parse->i_paras[1] = (int)SNIFFER_MAX_FILE_LEN / (1024 * 1024);
		copy_to_user((char __user *)&(((ctl_cmd_stru *)buffer)->i_paras[0]),
	...
	printk(KERN_INFO "FILE_NUME=%d SNIFFER_MAX_FILE_LEN=%d\n",
		pCmd2Parse->i_paras[0], pCmd2Parse->i_paras[1]);
	old_file_num = (int)SNIFFER_MAX_FILE_NUM;
	old_file_len = (int)SNIFFER_MAX_FILE_LEN;
	SNIFFER_MAX_FILE_NUM = (unsigned int)pCmd2Parse->i_paras[0];
	SNIFFER_MAX_FILE_LEN = (unsigned int)pCmd2Parse->i_paras[1];
...
static int proc_handle_command_data_payload(ctl_cmd_stru *pCmd2Parse, char __user *buffer)
{
	...
	if (pCmd2Parse->i_paras[0] == -1) { // it is a read command
		pCmd2Parse->i_paras[0] = (int)SNIFFER_MAX_PACKET_LEN;
		copy_to_user((char __user *)&(((ctl_cmd_stru *)buffer)->i_paras[0]),
	...
	if (pCmd2Parse->i_paras[0] < 0 || pCmd2Parse->i_paras[0] > MAX_PARAM_LEN) {
		...
		SNIFFER_MAX_PACKET_LEN = (unsigned int)pCmd2Parse->i_paras[0];
	}
