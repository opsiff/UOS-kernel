...
static void get_wifi_qoe_state(struct station_info *sinfo, struct wifi_stat *wifi_stat)
{
#ifdef CONFIG_HW_GET_EXT_2_SIG
	if (sinfo->filled & BIT(NL80211_STA_INFO_UL_DELAY_ARRAY))
		(void)memcpy_s(wifi_stat->info.ul_delay_array,
				sizeof(wifi_stat->info.ul_delay_array), sinfo->ul_delay_array,
				sizeof(sinfo->ul_delay_array));
	if (sinfo->filled & BIT(NL80211_STA_INFO_TX_TIME))
		(void)memcpy_s(wifi_stat->info.tx_time, sizeof(wifi_stat->info.tx_time),
				sinfo->tx_time, sizeof(sinfo->tx_time));
	if (sinfo->filled & BIT(NL80211_STA_INFO_BEACON_RSSI))
		(void)memcpy_s(wifi_stat->info.beacon_rssi, sizeof(wifi_stat->info.beacon_rssi),
				sinfo->beacon_rssi, sizeof(sinfo->beacon_rssi));
	if (sinfo->filled & BIT(NL80211_STA_INFO_CHLOAD_SELF))
		wifi_stat->info.chload_self = sinfo->chload_self;
	if (sinfo->filled & BIT(NL80211_STA_INFO_TX_BITRATE))
		wifi_stat->info.tx_mcs = sinfo->txrate.mcs;
		wifi_stat->info.tx_nss = sinfo->txrate.nss;
		wifi_stat->info.tx_bw = sinfo->txrate.bw;
		wifi_stat->info.tx_flags = sinfo->txrate.flags;
#endif
}
...
#endif
	get_wifi_qoe_state(&sinfo, wifi_stat);
	return 0;
}
	...
	struct wifi_stat *cur, struct wifi_stat *res, int cnt)
{
	...
	for (i = 0; i < cnt; i++) {
		...
		res[i].info.rx_bytes =
			turn_over(cur[i].info.rx_bytes, past[i].info.rx_bytes);
		res[i].info.tx_bytes =
			turn_over(cur[i].info.tx_bytes, past[i].info.tx_bytes);
		res[i].info.rssi = cur[i].info.rssi;
		res[i].info.phy_tx_rate = cur[i].info.phy_tx_rate;
		res[i].info.phy_rx_rate = cur[i].info.phy_rx_rate;
		res[i].info.rx_packets =
			turn_over(cur[i].info.rx_packets, past[i].info.rx_packets);
		res[i].info.tx_packets =
			turn_over(cur[i].info.tx_packets, past[i].info.tx_packets);
		res[i].info.tx_failed =
			turn_over(cur[i].info.tx_failed, past[i].info.tx_failed);
...
#ifdef CONFIG_HW_GET_EXT_2_SIG
		(void)memcpy_s(res[i].info.ul_delay_array, sizeof(res[i].info.ul_delay_array),
				cur[i].info.ul_delay_array, sizeof(cur[i].info.ul_delay_array));
		(void)memcpy_s(res[i].info.tx_time, sizeof(res[i].info.tx_time),
				cur[i].info.tx_time, sizeof(cur[i].info.tx_time));
		(void)memcpy_s(res[i].info.beacon_rssi, sizeof(res[i].info.beacon_rssi),
				cur[i].info.beacon_rssi, sizeof(cur[i].info.beacon_rssi));
		res[i].info.chload_self = cur[i].info.chload_self;
		res[i].info.tx_mcs = cur[i].info.tx_mcs;
		res[i].info.tx_nss = cur[i].info.tx_nss;
		res[i].info.tx_bw = cur[i].info.tx_bw;
		res[i].info.tx_flags = cur[i].info.tx_flags;
#endif
