...
static void update_uid_list(const char *msg, int len)
{
	...
	uid_list.nums = 0;
	if (num > 0)
		uid_list.uids = kmalloc(sizeof(int) * num, GFP_ATOMIC);
...
static struct stats_node *find_new_stats_node(struct sk_buff *skb, struct tcphdr *th, u32 index, bool *new_flag)
{
	struct stats_node *node = NULL;
...
static void tcp_send_stats(struct sk_buff *skb, int tcp_len)
{
	...
	read_lock_bh(&table[index].lock);
	node = find_new_stats_node(skb, th, index, &new_flag);
	if (node == NULL) {
		read_unlock_bh(&table[index].lock);
		return;
	}
