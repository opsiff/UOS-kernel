...
static int ptp_start_timesync(const struct netlink_data *msg_rev)
{
	...
	if (msg_rev->msg_len < sizeof(struct netlink_msg_ptp_start_info)) {
		wifi_audio_log_err("input len is wrong, error");
		return  ERROR;
	}
...
static int ptp_set_config_infor(const struct netlink_data *msg_rev)
{
	...
	if (msg_rev->msg_len < sizeof(struct netlink_msg_config_info)) {
		wifi_audio_log_err("input len is wrong, error");
		return  ERROR;
	}
...
static int ptp_stop_timesync(const struct netlink_data *msg_rev)
{
	...
	if (msg_rev->msg_len < sizeof(struct netlink_msg_ptp_stop_info))
		return  ERROR;
