...
struct coul_interface_ops {
	...
	int (*get_battery_charge_counter)(void *dev_data);
	int (*get_desired_charging_current)(void *dev_data);
	int (*get_desired_charging_voltage)(void *dev_data);
	bool (*is_smart_battery)(void *dev_data);
...
int coul_interface_get_battery_charge_counter(int type);
int coul_interface_set_vterm_dec(int type, int vterm_dec);
int coul_interface_set_battery_low_voltage(int type, int val);
int coul_interface_get_desired_charging_current(int type);
int coul_interface_get_desired_charging_voltage(int type);
bool coul_interface_is_smart_battery(int type);
...
static inline int coul_interface_get_battery_charge_counter(int type)
{
	return -EPERM;
}
...
static int coul_interface_get_desired_charging_current(int type)
{
	return -EPERM;
}

static int coul_interface_get_desired_charging_voltage(int type)
{
	return -EPERM;
}

static bool coul_interface_is_smart_battery(int type)
{
	return false;
}
