...
static inline void power_platform_pmic_vbus_disconnect_process(void)
{
	if (pmic_vbus_is_connected())
		pmic_vbus_disconnect_process();
}

static inline void power_platform_set_usb_ldo(bool state)
{
	if (state)
...
static inline void power_platform_pmic_set_buckboost_mode(int mode)
{
	pmic_set_buckboost_mode(mode);
}
...
{
	return pmic_vbus_irq_is_enabled();
}
#else
static inline int power_platform_support_pmic_detection(void)
{
...
static inline void power_platform_direct_charger_disconnect_event(void)
{
...
static inline void power_platform_charge_set_hiz_enable(int enable)
{
	charge_set_hiz_enable_by_direct_charge(enable);
}

#ifdef CONFIG_HUAWEI_HW_DEV_DCT
static inline void power_platform_set_dev_flag(int dev_id)
{
	set_hw_dev_flag(dev_id);
}
#else
static inline void power_platform_set_dev_flag(int dev_id)
{
