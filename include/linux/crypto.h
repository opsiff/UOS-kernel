...
struct compress_alg {
	...
	int (*coa_decompress)(struct crypto_tfm *tfm, const u8 *src,
			      unsigned int slen, u8 *dst, unsigned int *dlen);
#ifdef CONFIG_CRYPTO_SDCU
	int (*coa_movedata)(struct crypto_tfm *tfm, u8 *dst, const u8 *src, unsigned int len);
#endif
#ifdef CONFIG_CRYPTO_DELTA
...
#ifdef CONFIG_CRYPTO_SDCU
int crypto_comp_movedata(struct crypto_comp *comp,
			 u8 *dst, const u8 *src, unsigned int len);
#endif
#ifdef CONFIG_CRYPTO_DELTA
