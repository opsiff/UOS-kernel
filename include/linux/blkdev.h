...
#ifdef CONFIG_SCSI_UFS_GEAR_CTRL
enum ufs_dss_latency_enum {
	UFS_DSS_LATENCY_SCENARIO_REQUEST,
...
typedef int (*lld_switch_latency_check_fn)(struct request_queue *q,
					   enum ufs_dss_latency_enum scenario,
					   int set_gear, int switch_reason);
struct lld_gear_ctrl_ops {
	...
	lld_switch_latency_check_fn switch_latency_check;
};
#endif
...
#ifdef CONFIG_MAS_VIRTIO_BLK_FE
	atomic64_t virtio_blk_last_ktime;
	atomic64_t virtio_blk_notify_ktime;
	atomic64_t virtio_blk_commit_rqs_ktime;
#endif
	/*
	 * Flush Optimise
	 */
...
#ifdef CONFIG_MAS_VIRTIO_BLK_FE
	atomic64_t virtio_blk_kick_ktime[VIRTIO_BLK_MAX_QID];
	atomic64_t virtio_last_ktime[VIRTIO_BLK_MAX_QID];
#endif
...
#ifdef CONFIG_MAS_VIRTIO_BLK_FE
void mas_virtio_blk_queue_rq_done(struct blk_mq_hw_ctx *hctx,
	struct request *rq, bool last, bool notify);
void mas_virtio_blk_tag_set_init(struct blk_mq_tag_set *tag_set);
void mas_virtio_blk_latency_req_notsupp(struct request *rq);
void mas_virtio_blk_latency_req_zeroes(struct request *rq);
void mas_virtio_blk_latency_req_enospc(struct request *rq);
void mas_virtio_blk_latency_req_enomem(struct request *rq);
void mas_virtio_blk_latency_req_error(struct request *rq);
#else
static inline void mas_virtio_blk_tag_set_init(struct blk_mq_tag_set *tag_set)
{
	return;
}
static inline void mas_virtio_blk_latency_req_notsupp(struct request *rq)
{
	return;
}
static inline void mas_virtio_blk_latency_req_zeroes(struct request *rq)
{
	return;
}
static inline void mas_virtio_blk_latency_req_enospc(struct request *rq)
{
	return;
}
static inline void mas_virtio_blk_latency_req_enomem(struct request *rq)
{
	return;
}
static inline void mas_virtio_blk_latency_req_error(struct request *rq)
{
	return;
}
static inline void mas_virtio_blk_queue_rq_done(struct blk_mq_hw_ctx *hctx,
	struct request *rq, bool last, bool notify)
{
	return;
}
