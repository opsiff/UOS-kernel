...
static int ana_hs_core_init_data(struct platform_device *pdev)
{
	...
	g_data->wake_lock = wakeup_source_register(NULL, "ana_hs_core");
#else
	g_data->wake_lock = wakeup_source_register("ana_hs_core");
#endif
	if (!g_data->wake_lock) {
...
static int ana_hs_core_setup_data(struct platform_device *pdev)
{
	struct device *dev = &pdev->dev;
	struct device_node *node = dev->of_node;
	int ret;
	...
	/* create workqueue */
	return ana_hs_create_workqueue();
}
...
static int ana_hs_core_probe(struct platform_device *pdev)
{
	...
	ret = ana_hs_core_init_data(pdev);
	if (ret < 0)
		return ret;

	ret = ana_hs_core_setup_data(pdev);
	if (ret < 0)
		goto ana_hs_err_out;
