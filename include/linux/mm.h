...
extern unsigned long calc_media_zone_cmafree_node(int nid);
extern unsigned long calc_media_zone_cmafree(void);
#endif
...
extern void change_secpage_range(phys_addr_t phys, unsigned long addr, unsigned long size, pgprot_t prot);
...
#if defined (CONFIG_ION_MM_CPA) || defined (CONFIG_BG_DMABUF_RECLAIM)
void ion_drop_pagecache(void);
#endif
...
void force_page_cache_readahead_abs(struct address_space *mapping,
...
#else
static inline int all_active_reclaim_by_task(struct task_struct *)
{
	return 0;
}
#endif
