...
extern void dfx_irq_disable_stats(void);
extern void dfx_irq_enable_stats(void);

#define raw_local_irq_disable()			\
	do {						\
		bool was_disabled = arch_irqs_disabled(); \
		arch_local_irq_disable();		\
		if (dfx_stats_on && !was_disabled)		\
			dfx_irq_disable_stats();	\
	} while (0)
#define raw_local_irq_enable()				\
	do {						\
		if (dfx_stats_on) \
			dfx_irq_enable_stats();		\
...
#define raw_local_irq_save(flags)			\
	do {						\
		...
		if (dfx_stats_on && !arch_irqs_disabled_flags(flags)) \
			dfx_irq_disable_stats();	\
	} while (0)
