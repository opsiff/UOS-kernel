...
// total_objects
TRACE_EVENT(procfs_super_cache_count,
	TP_PROTO(long total_objects),

	TP_ARGS(total_objects),

	TP_STRUCT__entry(
		__field(long,	total_objects	)
	),

	TP_fast_assign(
		__entry->total_objects		= total_objects;
	),

	TP_printk("procfs total_cache=%ld", __entry->total_objects)
);
#endif /* CONFIG_KSWAPD_PROTECT_DCACHE */
