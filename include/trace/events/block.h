...
TRACE_EVENT(block_rq_complete,
	...
	TP_STRUCT__entry(
		...
		__array(  char,		rwbs,	RWBS_LEN	)
#ifdef CONFIG_MAS_UFS_GEAR_CTRL
		__array(  char,		gear_info, GEAR_INFO_LEN)
#endif
		__dynamic_array( char,	cmd,	1		)
	),
	...
	TP_fast_assign(
		...
		__get_str(cmd)[0] = '\0';
#ifdef CONFIG_MAS_UFS_GEAR_CTRL
		CALL_HCK_VH(mas_blk_fill_gear_info, __entry->gear_info, GEAR_INFO_LEN);
#endif
	),

#ifndef CONFIG_MAS_UFS_GEAR_CTRL
	TP_printk("%d,%d %s (%s) %llu + %u [%d]",
		  MAJOR(__entry->dev), MINOR(__entry->dev),
		  __entry->rwbs, __get_str(cmd),
		  (unsigned long long)__entry->sector,
...
#else
	TP_printk("%d,%d %s (%s) %llu + %u [%d] {%s}",
		  MAJOR(__entry->dev), MINOR(__entry->dev),
		  __entry->rwbs, __get_str(cmd),
		  (unsigned long long)__entry->sector,
		  __entry->nr_sector, __entry->error, __entry->gear_info)
#endif
);
...
#else
	TP_printk("%d,%d %s %u (%s) %llu + %u [%s] {%s}",
		  MAJOR(__entry->dev), MINOR(__entry->dev),
		  __entry->rwbs, __entry->bytes, __get_str(cmd),
		  (unsigned long long)__entry->sector,
		  __entry->nr_sector, __entry->comm, __entry->gear_info)
#endif
);
