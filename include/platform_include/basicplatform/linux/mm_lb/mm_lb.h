...
extern unsigned int lb_get_page_osh_flag(struct page *page);
extern void lb_sg_set_osh(struct scatterlist *sgl, unsigned int nents, unsigned int osh);
void lb_pages_set_osh(struct page *pg, size_t count, unsigned int osh);
#ifndef declare_sc_property
#define declare_sc_property(type_name, property) \
	u32 get_##type_name##_##property(const u32 gid); \
	void set_##type_name##_##property(const u32 gid, const u32 val, const direct);
#endif
