...
u32 uapp_get_bindfile_pos(struct uapp_bindfile_pos *bindfile)
{
	...
	if (ret != 0) {
		uapp_err("error 0x%x, get node path %s\n", ret, UAPP_BINDFILE_PATH);
		return ret;
	}
...
u32 uapp_get_empower_cert_pos(struct uapp_file_pos *empower_cert)
{
	...
	if (!empower_cert) {
		uapp_err("error, pos is NULL\n");
		return UAPP_NULL_PTR_ERR;
	}

	ret = uapp_get_file_pos(empower_cert, EMPOWER_CERT_PATH);
	if (ret != UAPP_OK) {
		uapp_err("error 0x%x, get uapp empower cert pos\n", ret);
		return ret;
	}
