...
#ifdef CONFIG_CRYPTO_CORE_FFA_SUPPORT
static noinline int32_t mspc_send_tzsp_smc(uint64_t _function_id, uint64_t _arg0,
			     ...
			     const char *msg, uint32_t msg_len)
{
	...
	/* when msg is not null, only cos id needs be delivered */
	if (msg)
		param1 = *(uint32_t *)(msg + sizeof(uint32_t));

	ret = mspc_send_tzsp_smc(fid, smc_cmd, (u64)param1, 0);
	return ret;
#else
	char *buff_virt = NULL;
...
#ifdef CONFIG_CRYPTO_CORE_FFA_SUPPORT
	ret = mspc_send_tzsp_smc(func_id, (uint64_t)smc_cmd, phy_addr, size);
#else
