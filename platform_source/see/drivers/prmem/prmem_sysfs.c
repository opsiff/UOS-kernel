...
{									\
	...
	data = container_of(attr, struct prmem_##obj##_sysfs_data,	\
			    attr_##entry);				\
	return snprintf_s(buf, PAGE_SIZE, PAGE_SIZE - 1, format "\n", value);		\
}
					   ...
					   struct kobject *parent)
{
	...
	prmem_sysfs_attr_init(data, phys_db, chunks_nr);
	{
		...
		(void)sysfs_create_files(prmem_phys_db_kobj, attrs);
	}
					       ...
					       char *buf)
{
	...
	ret = (ssize_t)snprintf_s(buf, PAGE_SIZE, PAGE_SIZE - 1, "%s\n",
					       ...
					       struct kobject *parent)
{
	...
	prmem_sysfs_attr_init(data, pool, allocation_type);
	{
		...
		(void)sysfs_create_files(root, attrs);
	}
...
static __init void create_prmem_sysfs_pools(struct kobject *parent)
{
	...
	start = (uintptr_t)__start_data_prmem_pools;
	end = (uintptr_t)__end_data_prmem_pools;
	size = (unsigned long)roundup(sizeof(struct prmem_pool), sizeof(void *));
...
static void create_prmem_sysfs_caches(struct kobject *parent)
{
	...
	size = (unsigned long)roundup(sizeof(struct prmem_cache), sizeof(void *));
	for (p = start; p < end; p += size) {
...
static __init int create_prmem_sysfs_entries(void)
{
	...
	if (!status)
		goto err2;
	err = sysfs_create_file(prmem_kobj, (struct attribute *)status);
	if (err != 0)
		goto err2;
