...
static int32_t cmdlist_alloc_items(uint32_t dev_id,
	struct cmdlist_node *node, uint32_t size, uint32_t dst_addr)
{
	struct cmdlist_private *cmd_priv = cmdlist_get_priv_by_id(dev_id);
	if (unlikely(!cmd_priv)) {
		ukmd_pr_err("cmd_priv is nullptr!");
		return -1;
	}

	node->list_cmd_header = (struct cmd_header *)(uintptr_t)cmd_priv->mem_mgr.mem_alloc(&cmd_priv->mem_mgr,
		size, &node->cmd_header_addr, (uint32_t *)&node->node_size);
	if (!node->list_cmd_header) {
		ukmd_pr_err("cmd header alloc failed, size=%#x!", size);
		return -1;
	}

	if ((node->node_size != 0) && (size > node->node_size)) {
		ukmd_pr_err("return invalid size: %#x, node_size: %#x!", size, node->node_size);
		cmd_priv->mem_mgr.mem_free(&cmd_priv->mem_mgr, node->list_cmd_header, (uint32_t)node->node_size);
	...
	ukmd_pr_debug("alloc node list_cmd_header[0x%llx].node_size[%zu] cmd_flag=%#x",
		node->list_cmd_header, size, node->list_cmd_header->cmd_flag.ul32);
	...
	if (node->cmd_mode == TRANSPORT_MODE) {
		...
		node->list_cmd_header->total_items.bits.items_count = (uint32_t)node->payload_size - 1;
	}
	ukmd_pr_debug("alloc node list_cmd_header[0x%llx].node_size[%zu] total_items=%#x",
		node->list_cmd_header, size, node->list_cmd_header->total_items.ul32);
 ...
 */
struct cmdlist_node *cmdlist_create_node(uint32_t dev_id,
	uint32_t scene_id, uint32_t mode, uint32_t size, uint32_t dst_addr)
{
	...
	if (node->cmd_mode == TRANSPORT_MODE) {
		if (((size % CMDLIST_ITEMS_ALIGN_SIZE) != 0) || (size > CMDLIST_ITEMS_COUNT_MAX * CMDLIST_ITEMS_ALIGN_SIZE)) {
			ukmd_pr_err("size=%#x is not 128bit aligned or out of range!", size);
			kfree(node);
			return NULL;
		}

		node->fence = ukmd_alloc_acquire_fence(&node->lock, (int32_t)scene_id);
		if (unlikely(node->fence == NULL)) {
			ukmd_pr_err("cmdlist node fence alloc failed!");
			kfree(node);
			return NULL;
		}
	}

	if (cmdlist_alloc_items(dev_id, node, size, dst_addr) != 0) {
		ukmd_pr_err("cmdlist alloc items failed!");
		if (node->fence != NULL)
			ukmd_acquire_fence_signal_release(node->fence);
		kfree(node);
		node = NULL;
		return NULL;
	}
 ...
 */
int32_t cmdlist_release_node(uint32_t dev_id, struct cmdlist_node *node)
{
	struct cmdlist_private *cmd_priv = cmdlist_get_priv_by_id(dev_id);
	if (unlikely(!cmd_priv)) {
		ukmd_pr_err("cmd_priv is nullptr!");
	...
	if (unlikely(!node)) {
		ukmd_pr_err("cmdlist node is null!");
		return -1;
	}
...
static bool is_valid_items_count(uint32_t items_count, struct cmdlist_node *node) {
	return items_count < (node->node_size / CMDLIST_ITEMS_ALIGN_SIZE);
}

void cmdlist_clear_node(struct cmdlist_node *node)
{
	uint32_t i = 0;
	...
	if (!is_valid_items_count(items_count, node)) {
		ukmd_pr_err("items cout is invalid, items cout = %u", items_count);
		return;
	}

	for (i = 0; i < items_count; i++) {
		temp_item = &(node->list_cmd_item[i]);
		(void)memset_s(temp_item, sizeof(*temp_item), 0, sizeof(*temp_item));
	}

	if (node->cmd_mode == CONFIG_MODE) {
		node->payload_size = 0;
		node->list_cmd_header->total_items.bits.items_count = 0;
	}
}
...
void cmdlist_dump_node(struct cmdlist_node *node)
{
	...
	ukmd_pr_info("cmdlist_node: %pK fence: %pK", node, node->fence);
	ukmd_pr_info("   cmd_flag    |   next_list   |   total_items   ");
	ukmd_pr_info(" --------------+---------------+-----------------");
	ukmd_pr_info("   0x%8x  |   0x%8x  |   0x%8x ",
	...
	ukmd_pr_info("items_count: %d ", items_count);
	for (i = 0; i < items_count; i++) {
		temp_item = &(node->list_cmd_item[i]);
		if (node->cmd_mode == TRANSPORT_MODE) {
			ukmd_pr_info("addr_offset:%#x value:0x%08x 0x%08x 0x%08x 0x%08x ",
			...
			ukmd_pr_info("set addr:0x%05x value:0x%08x addr1:0x%02x value:0x%08x addr2:0x%02x value:0x%08x",
