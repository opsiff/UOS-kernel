...
static inline void chash_deinit(struct chash_table *ht, cnode_funcx node_release,
	void *data)
{
	size_t i;
	...
	if (likely(node_release)) {
		for (i = 0; i < ht->bkt_size; i++)
			clist_deinit(&ht->bkts[i], node_release, data);
	}
...
static inline void chash_clear(struct chash_table *ht, cnode_funcx node_release,
	void *data)
{
	size_t i;
...
static inline void chash_bkt_clear(struct chash_table *ht,
	size_t bkt_idx, cnode_funcx node_release, void *data)
{
	struct cnode *list = &ht->bkts[bkt_idx % ht->bkt_size];

	clist_deinit(list, node_release, data);
}
