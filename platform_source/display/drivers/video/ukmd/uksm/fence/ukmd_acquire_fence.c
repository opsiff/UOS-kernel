...
{
	ukmd_pr_debug("release fence %s seqno:%llu\n",
...
{
	ukmd_pr_debug("drv:%s timeline:%s seqno:%llu signaled",
		fence->ops->get_driver_name(fence),
...
int32_t ukmd_acquire_fence_create_fd(struct dma_fence *fence, spinlock_t *lock, int32_t value)
{
	int32_t fence_fd = -1;
	...
	fence_fd = ukmd_fence_get_fence_fd(fence);
	if (fence_fd < 0) {
		ukmd_pr_err("get_unused_fd_flags failed error:%d\n", fence_fd);
		dma_fence_put(fence);
		return -1;
	}
...
struct dma_fence *ukmd_alloc_acquire_fence(spinlock_t *lock, int32_t value)
{
...
int32_t ukmd_acquire_fence_wait(struct dma_fence *fence, long timeout)
{
	int32_t rc = 0;
	...
	rc = dma_fence_wait_timeout(fence, false, (int64_t)msecs_to_jiffies((int32_t)timeout));
	if (rc > 0) {
		ukmd_pr_debug("signaled drv:%s timeline:%s seqno:%llu\n",
			fence->ops->get_driver_name(fence),
			fence->ops->get_timeline_name(fence),
			fence->seqno);
		rc = 0;
	} else if (rc == 0) {
		ukmd_pr_err("timeout drv:%s timeline:%s seqno:%llu.\n",
			fence->ops->get_driver_name(fence),
			fence->ops->get_timeline_name(fence),
			fence->seqno);
		rc = -ETIMEDOUT;
 ...
 * @return int32_t
 */
int32_t ukmd_acquire_fence_wait_fd(int32_t fence_fd, long timeout)
{
	int32_t rc = 0;
	...
	if (!fence) {
		ukmd_pr_err("fence_fd=%d sync_file_get_fence failed!\n", fence_fd);
		return -EINVAL;
	}

	ukmd_pr_debug("wait for fence_fd=%d", fence_fd);
	rc = ukmd_acquire_fence_wait(fence, timeout);
	if (rc != 0)
		ukmd_pr_err("wait for fence_fd=%d fail", fence_fd);

	if (g_debug_ukmd_fence) {
		if (fence->ops && fence->ops->get_driver_name)
			ukmd_pr_info("wait gpu acquire fence(fence_fd=%d) driver_name: %s", fence_fd, fence->ops->get_driver_name(fence));
	}
...
void ukmd_acquire_fence_signal_release(struct dma_fence *fence)
{
	if (!fence)
		return;

	if (!dma_fence_is_signaled_locked(fence))
		ukmd_pr_info("no signal drv:%s timeline:%s seqno:%llu, but release!\n",
			fence->ops->get_driver_name(fence),
			fence->ops->get_timeline_name(fence),
			fence->seqno);
...
int32_t ukmd_acquire_fence_signal(int32_t fence_fd)
{
	int32_t rc = 0;
	struct dma_fence *fence = NULL;
	...
	if (dma_fence_is_signaled_locked(fence)) {
		...
		ukmd_pr_info("signal err drv:%s timeline:%s seqno:%llu\n",
			fence->ops->get_driver_name(fence),
			fence->ops->get_timeline_name(fence),
			fence->seqno);
		rc = -1;
