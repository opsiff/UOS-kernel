...
int ukmd_create_chrdev(struct ukmd_chrdev *chrdev)
{
	...
	if (alloc_chrdev_region(&chrdev->devno, 0, 1, chrdev->name)) {
		ukmd_pr_err("alloc_chrdev_region fail!, name=%s", chrdev->name);
		return -1;
	}
	...
	if (cdev_add(&chrdev->cdev, chrdev->devno, 1)) {
		ukmd_pr_err("cdev_add fail!, name=%s", chrdev->name);
		goto err_cdev_add;
	}
	...
	if (chrdev->chr_class == NULL) {
		...
		if (IS_ERR_OR_NULL(chrdev->chr_class)) {
			ukmd_pr_err("class_create fail!, name=%s", chrdev->name);
	...
	if (IS_ERR_OR_NULL(chrdev->chr_dev)) {
		ukmd_pr_err("device_create fail!, name=%s", chrdev->name);
		goto err_create_device;
	}

	dev_set_drvdata(chrdev->chr_dev, chrdev->drv_data);

	ukmd_pr_info("ukmd_create_chrdev succ! major=%d, minor=%d, name=%s",
...
void ukmd_create_attrs(struct device *dev, struct device_attribute *device_attrs, uint32_t len)
{
	...
	if (unlikely(!dev || !device_attrs)) {
		ukmd_pr_err("dev or device_attrs is null\n");
		return;
	}
