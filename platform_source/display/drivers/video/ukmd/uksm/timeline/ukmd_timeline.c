...
static int32_t _timeline_step_pt_value(struct ukmd_timeline *timeline)
{
...
static void ukmd_timeline_sync_workqueue(struct kthread_work *work)
{
	...
	ukmd_check_and_no_retval(!work, err, "work is NULL!!!");

	timeline = container_of(work, typeof(*timeline), timeline_sync_work);
	ukmd_check_and_no_retval(!timeline, err, "timeline is NULL!!!");
	...
	list_for_each_entry_safe(listener_node, _node_, &timeline->listener_list, list_node) {
		if (!listener_node->ops) {
			ukmd_pr_err("listener_node ops is NULL!!!");
			continue;
		}
		listener_node->notify_timestamp = timeline->notify_timestamp;

		if (g_debug_ukmd_fence)
			ukmd_pr_info("timeline:%s timeline_value=<%llu,%llu> listener_name:%s value:%llu notify_timestamp=%lld",
		...
		if (listener_node->ops->handle_signal) {
			...
			if (ret != 0)
				ukmd_pr_warn("listener = 0x%pK handle fail", listener_node);
		}
...
static int32_t _timeline_isr_notify(struct notifier_block *self, unsigned long action, void *data)
{
	...
	struct ukmd_timeline *timeline = (struct ukmd_timeline *)(listener_data->data);

	if (!timeline->routine_enabled)
		return 0;

	if (action != timeline->listening_isr_bit) {
		ukmd_pr_info("action %#x is not equal to listening_isr_bit %#x", action, timeline->listening_isr_bit);
		return 0;
	}
...
void ukmd_timeline_resync(struct ukmd_timeline *timeline)
{
	...
	timeline->notify_timestamp = ktime_get();
	kthread_queue_work(timeline->present_handle_worker, &timeline->timeline_sync_work);
}
...
void ukmd_timeline_dump(struct ukmd_timeline *timeline)
{
	...
	mutex_lock(&timeline->list_lock);
	ukmd_pr_warn("timeline status: pt_value:%llu, next_value:%u, inc_step:%u", timeline->pt_value, 
	...
	list_for_each_entry_safe(listener_node, _node_, &timeline->listener_list, list_node) {
		...
		ukmd_pr_info("timeline:%s listener_name:%s pt_value:%llu",
			timeline->name, listener_node->ops->get_listener_name(listener_node), listener_node->pt_value);
	}
	mutex_unlock(&timeline->list_lock);
}
...
void ukmd_timeline_init(struct ukmd_timeline *timeline, const char *name, void *parent)
{
	...
	kthread_init_work(&timeline->timeline_sync_work, ukmd_timeline_sync_workqueue);

	ukmd_pr_info("init timeline:%s listening_isr_bit=%#x", timeline->name, timeline->listening_isr_bit);
}
	...
	void *listener_data, uint64_t value)
{
	...
	if (!listener) {
		ukmd_pr_err("listener kzalloc failed");
		return NULL;
	}
	...
	ukmd_pr_debug("create listener %pK at pt_value %llu", listener, value);
