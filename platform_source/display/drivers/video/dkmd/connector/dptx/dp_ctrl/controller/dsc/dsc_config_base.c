...
static void dptx_program_dsc_bits_perpixel(struct dp_ctrl *dptx)
{
	...
	bpp_high = (uint8_t)((dptx->vparams.dp_dsc_info.dsc_info.dsc_bpp & DPTX_DSC_BPP_HIGH_MASK) >> 8);
	bpp_low = (uint8_t)((dptx->vparams.dp_dsc_info.dsc_info.dsc_bpp*16) & DPTX_DSC_BPP_LOW_MASK);

	pps = dptx_readl(dptx, dptx_dsc_pps(DPTX_SST_MODE, 1));
	pps |= (uint32_t)bpp_high;
	pps |= (uint32_t)(bpp_low << 8);
	dptx_writel(dptx, dptx_dsc_pps(DPTX_SST_MODE, 1), pps);
...
void dptx_dsc_enabled_by_tu(struct dp_ctrl *dptx)
{
	dpu_check_and_no_retval(!dptx, err, "[DP] null pointer");
	...
	if (dptx->dsc) {
		if (dptx->connector->conn_info->base.mode == EDP_MODE) {
        	...
        	dpu_pr_info("[DP] EDP mode, enable DSC:%d, Disable fec:%d", dptx->dsc, dptx->dptx_link_params.compress_params.fec);
		} else {
			dptx->dsc = dptx->video_tu[NORMAL_SCENE].aver_bytes_per_tu > dptx->max_tu ? true : false;
			dptx->dptx_link_params.compress_params.fec = dptx->dsc; // fec need disable when disable dsc
			dpu_pr_info("[DP] DP mode, DSC enable value:%d, tu:%u", dptx->dsc, dptx->video_tu[NORMAL_SCENE].aver_bytes_per_tu);
		}
	}

	dpu_pr_info("[DP] DSC disable value:%d, tu:%u", dptx->dsc, dptx->video_tu[NORMAL_SCENE].aver_bytes_per_tu);
}
