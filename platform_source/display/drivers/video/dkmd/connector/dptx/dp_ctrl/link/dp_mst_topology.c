...
static void dptx_dpcd_clear_vcpid_table(struct dp_ctrl *dptx)
{
	...
	status = 0;
	while ((status & DP_PAYLOAD_TABLE_UPDATED) == 0) {
		retval = dptx_read_dpcd(dptx, DP_PAYLOAD_TABLE_UPDATE_STATUS, &status);
		if (retval)
			dpu_pr_err("[DP] Read DPCD error");
...
static void dptx_dpcd_set_vcpid_table(struct dp_ctrl *dptx, uint32_t start, uint32_t count, uint32_t stream)
{
	...
	bytes[0] = (uint8_t)stream;
	bytes[1] = (uint8_t)start;
...
static uint8_t drm_dp_msg_header_crc4(const uint8_t *data, size_t num_nibbles)
{
	...
	int number_of_bits = (int)num_nibbles * 4;
	uint8_t remainder = 0;
...
static void drm_dp_encode_sideband_msg_hdr(struct drm_dp_sideband_msg_hdr *hdr, uint8_t *buf, int *len)
{
	...
	for (i = 0; i < (hdr->lct / 2); i++)
		buf[idx++] = hdr->rad[i];
	buf[idx++] = (uint8_t)(((uint32_t)hdr->broadcast << 7) | ((uint32_t)hdr->path_msg << 6) |
		(hdr->msg_len & 0x3f));
	buf[idx++] = (uint8_t)(((uint32_t)hdr->somt << 7) | ((uint32_t)hdr->eomt << 6) | ((uint32_t)hdr->seqno << 4));

	crc4 = drm_dp_msg_header_crc4(buf, (size_t)(idx * 2) - 1);
	buf[idx - 1] |= (crc4 & 0xf);
...
static void dptx_print_sideband_header(struct dp_ctrl *dptx, struct drm_dp_sideband_msg_hdr *header)
{
	...
	dpu_pr_info(
		"[DP] SIDEBAND_MSG_HEADER: lct=%u, lcr=%u, rad=%s, bcast=%d, path=%d, msglen=%u, somt=%d, eomt=%d, seqno=%d",
...
static int dptx_remote_i2c_read(struct dp_ctrl *dptx, uint8_t port, int port1)
{
	...
	if (len < (MST_MSG_BUF_LENGTH - 9)) {
		...
		dpu_pr_info("[DP] +");
		dptx_write_bytes_to_dpcd(dptx, DP_SIDEBAND_MSG_DOWN_REQ_BASE, buf, (uint32_t)len);

		retval = dptx_sideband_get_down_rep(dptx, DP_REMOTE_I2C_READ);
		if (retval)
			return retval;

		dpu_pr_info("[DP] msg length = %hhu", g_mst_msg.msg_len);
		...
		if (retval) {
			...
			dp_maintenance_add_error_log(dptx, "EDID Parserfail");
		}
	} else {
		return -EINVAL;
	}
...
static int dptx_enum_path_resources(struct dp_ctrl *dptx, uint8_t port, int port1)
{
	...
	if (len < (MST_MSG_BUF_LENGTH - 3)) {
		...
		retval = dptx_write_bytes_to_dpcd(dptx, DP_SIDEBAND_MSG_DOWN_REQ_BASE, buf, (uint32_t)len);
		if (retval)
...
again:
	...
	while (1) {
		...
		if (sideband_msg->header.msg_len >= (sizeof(sideband_msg->msg) - msg_len + 1)) {
			dpu_pr_err("[DP] header.msg_len %u exceeds msg remaining size!\n", sideband_msg->header.msg_len);
			retval = -EINVAL;
			goto out;
		}
		...
		if ((first != 0) && !sideband_msg->header.somt) {
	...
	if ((sideband_msg->msg[0] & 0x7f) != DP_CONNECTION_STATUS_NOTIFY) {
		if (retries < 3) {
			dpu_pr_err("[DP] request_id %u does not match expected %d, retrying",
				sideband_msg->msg[0] & 0x7f, DP_UP_REQ_MSG_RDY);
			retries++;
			goto again;
		} else {
			dpu_pr_err("[DP] request_id %hhu does not match expected %d, giving up",
				sideband_msg->msg[0] & 0x7f, DP_UP_REQ_MSG_RDY);
			retval = -EINVAL;
			goto out;
		}
	...
	if (msg_len >= sizeof(sideband_msg->raw.msg)) {
		dpu_pr_err("[DP] msg_len %u exceeds raw msg array size!\n", msg_len);
		retval = -EINVAL;
		goto out;
	}
	...
	if (sideband_msg->req_msg.u.conn_stat.displayport_device_plug_status) {
		...
		dptx_aux_msg_allocate_payload(dptx,
			sideband_msg->req_msg.u.conn_stat.port_number, (uint8_t)dptx->vcp_id, dptx->pbn, -1);
	} else {
		dptx->vcp_id--;
	}
...
static int dptx_wait_down_rep(struct dp_ctrl *dptx)
{
	...
	while (1) {
		...
		if (((vector & DP_DOWN_REP_MSG_RDY) != 0) || ((bytes[0] & DP_DOWN_REP_MSG_RDY) != 0)) {
...
again:
	...
	if ((g_mst_msg.msg[0] & 0x7f) != request_id) {
		if (retries < 3) {
			...
			dpu_pr_err("[DP] request_id %u does not match expected %d, giving up",
				g_mst_msg.msg[0] & 0x7f, request_id);
			return -EINVAL;
		}
	}
	...
	struct drm_dp_sideband_msg_reply_body *repmsg)
{
	...
	return true;
fail_len:
	dpu_pr_err("[DP] link addr reply parse length fail %d %u", idx, raw->curlen);
	return false;
}
...
static int dptx_calc_num_slots(struct dp_ctrl *dptx, int pbn, uint32_t *slots)
{
	...
	*slots = (uint32_t)DIV_ROUND_UP(pbn, div);
...
int dptx_get_topology(struct dp_ctrl *dptx)
{
	...
	dpu_pr_info("[DP] NPORTS = %u", rep.u.link_addr.nports);
...
/**
 * dptx_dp_calc_pbn_mode() - Calculate the PBN for a mode.
 ...
 */
static int dptx_dp_calc_pbn_mode(int clock, int bpp, bool dsc)
{
	/*
...
int dptx_mst_payload_setting(struct dp_ctrl *dptx)
{
	...
	dptx->pbn = (uint16_t)dptx_dp_calc_pbn_mode((int)dptx->vparams.mdtd.pixel_clock, 24, dptx->dsc);
	...
	while ((status & DP_PAYLOAD_ACT_HANDLED) == 0) {
		retval = dptx_read_dpcd(dptx, DP_PAYLOAD_TABLE_UPDATE_STATUS, &status);
		if (retval)
	...
	if (dptx->logic_port)
		dptx_aux_msg_allocate_payload(dptx, dptx->port[0], (uint8_t)dptx->vcp_id, pbn, -1);
	else
		dptx_aux_msg_allocate_payload(dptx, dptx->port[0], (uint8_t)dptx->vcp_id, pbn, 1);
	dptx->vcp_id++;

	dptx_aux_msg_allocate_payload(dptx, dptx->port[1], (uint8_t)dptx->vcp_id, pbn, 1);
