...
static void mipi_cdphy_config_reset(char __iomem *mipi_dsi_base)
{
	/* RST_CTRL reset */
...
static void mipi_cdphy_config_dereset(char __iomem *mipi_dsi_base)
{
	/* RST_CTRL Dereset */
...
static void mipi_cdphy_pll_config_para(struct mipi_dsi_phy_ctrl *phy_ctrl,
	char __iomem *mipi_dsi_base)
{
	...
	if (dpu_config_get_version() == DPU_ACCEL_DPUV9200) {
		mipi_config_phy_test_code(mipi_dsi_base, (CDPHY_TEST_CTRL_TESTEN_BIT + 0x106), 0);
		mipi_config_phy_test_code(mipi_dsi_base, (CDPHY_TEST_CTRL_TESTEN_BIT + 0x107), 0);
		mipi_config_phy_test_code(mipi_dsi_base, (CDPHY_TEST_CTRL_TESTEN_BIT + 0x108), 0);
		mipi_config_phy_test_code(mipi_dsi_base, (CDPHY_TEST_CTRL_TESTEN_BIT + 0x109), phy_ctrl->rg_pll_fbkdiv);
		mipi_config_phy_test_code(mipi_dsi_base, (CDPHY_TEST_CTRL_TESTEN_BIT + 0x10A), 0);
	}
}

static void mipi_cdphy_config_vref_sel(char __iomem *mipi_dsi_base, struct mipi_dsi_phy_ctrl *phy_ctrl,
								uint32_t phy_mode)
{
	dpu_pr_info("config swing=%d \n", phy_ctrl->rg_vrefsel_vcm);
	if (phy_mode == CPHY_MODE) {
		mipi_config_phy_test_code(mipi_dsi_base, (CDPHY_TEST_CTRL_TESTEN_BIT + 0x14f), (phy_ctrl->rg_vrefsel_vcm << 4) + 0x5);
		mipi_config_phy_test_code(mipi_dsi_base, (CDPHY_TEST_CTRL_TESTEN_BIT + 0x154), MIPIDSI_CPHY_TST_RG_MAIN_RT);
	} else {
		mipi_config_phy_test_code(mipi_dsi_base, (CDPHY_TEST_CTRL_TESTEN_BIT + 0x14f), (phy_ctrl->rg_vrefsel_vcm << 4) + 0x5);
		mipi_config_phy_test_code(mipi_dsi_base, (CDPHY_TEST_CTRL_TESTEN_BIT + 0x154), MIPIDSI_DPHY_TST_RG_MAIN_RT);
	}
}
...
uint64_t get_default_lane_byte_clk(struct mipi_panel_info *mipi)
{
	...
	uint32_t rg_pll_prediv = RG_PLL_PREDIV; /* for 38.4M or 19.2M */
...
void mipi_dsi_convert_pxl2cycle(struct dpu_panel_info *pinfo)
{
	...
	pinfo->mipi.hsa = (uint32_t)(round1(h_pulse_width * lane_byte_clk, pixel_clk));
	pinfo->mipi.hbp = (uint32_t)(round1(h_back_porch * lane_byte_clk, pixel_clk));
	pinfo->mipi.hline_time = (uint32_t)(round1(hline_time * lane_byte_clk, pixel_clk));
	pinfo->mipi.dpi_hsize = (uint32_t)(round1(dpi_hsize * lane_byte_clk, pixel_clk));
	...
	uint64_t *lane_clock, int32_t fpga_flag)
{
	...
	if (fpga_flag) {
		...
		*lane_clock = m_pll * (DEFAULT_MIPI_CLK_RATE / n_pll) / vco_div;
		...
		phy_ctrl->rg_pll_prediv = RG_PLL_PREDIV; /* for 38.4M or 19.2M */
...
void get_dsi_dphy_ctrl(struct dpu_connector *connector, struct mipi_dsi_phy_ctrl *phy_ctrl)
{
	...
	phy_ctrl->rg_vrefsel_vcm = MIPIDSI_PHY_TST_RG_VREFSEL_VCM; /* dphy test suggest value */
	...
	struct mipi_panel_info *mipi, char __iomem *mipi_dsi_base, int32_t fpga_flag)
{
	...
	mipi_config_phy_test_code(mipi_dsi_base, CDPHY_TEST_CTRL_TESTEN_BIT + 0x15C, 0x3C);
	mipi_config_phy_test_code(mipi_dsi_base, CDPHY_TEST_CTRL_TESTEN_BIT + 0x15D, 0xFF);
	mipi_config_phy_test_code(mipi_dsi_base, CDPHY_TEST_CTRL_TESTEN_BIT + 0x15E, 0xFF);
}
	...
	struct mipi_panel_info *mipi, char __iomem *mipi_dsi_base)
{
	...
	for (i = 0; i <= MAX_LANE_NUMS; i++) {
		if (i == 2)
			continue;  /* addr: lane0:0x60; lane1:0x80; lane2:0xC0; lane3:0xE0 */
		...
		mipi_config_phy_test_code(mipi_dsi_base, addr, phy_reduce(data_post_delay));
...
static void mipi_config_phy_spec_parameter(char __iomem *mipi_dsi_base, uint32_t phy_mode)
{
	if (dpu_config_get_version() != DPU_ACCEL_DPUV9200)
		return;

	if (phy_mode == DPHY_MODE) {
		mipi_config_phy_test_code(mipi_dsi_base, CDPHY_TEST_CTRL_TESTEN_BIT + 0x15C, 0x3C);
		mipi_config_phy_test_code(mipi_dsi_base, CDPHY_TEST_CTRL_TESTEN_BIT + 0x15D, 0xFF);
		mipi_config_phy_test_code(mipi_dsi_base, CDPHY_TEST_CTRL_TESTEN_BIT + 0x15E, 0xFF);
	}

	mipi_config_phy_test_code(mipi_dsi_base, CDPHY_TEST_CTRL_TESTEN_BIT + 0x167, 0x6);
}
	...
	struct mipi_dsi_phy_ctrl *phy_ctrl, char __iomem *mipi_dsi_base)
{
	...
	if (mipi->phy_mode == CPHY_MODE) {
		...
		if (fpga_flag == 0)
			mipi_config_phy_spec_parameter(mipi_dsi_base, mipi->phy_mode);
	} else {
		...
		if (phy_ctrl->rg_vrefsel_eq != 0) {
			dpu_pr_info("config eq from paneldrv %d\n", phy_ctrl->rg_vrefsel_eq);
			mipi_config_phy_test_code(mipi_dsi_base, (CDPHY_TEST_CTRL_TESTEN_BIT + 0x152), 0x4);
			mipi_config_phy_test_code(mipi_dsi_base, (CDPHY_TEST_CTRL_TESTEN_BIT + 0x155), phy_ctrl->rg_vrefsel_eq);
		}
		/* PLL update control */
		mipi_config_phy_test_code(mipi_dsi_base, (CDPHY_TEST_CTRL_TESTEN_BIT + 0x4B), 0x1);
		if (fpga_flag == 0)
...
bool mipi_phy_status_check(const char __iomem *mipi_dsi_base, uint32_t expected_value)
{
	...
	} while (retry_cnt++ < MAX_TIMEOUT_WAIT);
...
{
	struct mipi_panel_info *mipi = &connector->post_info[connector->active_idx]->mipi;
	struct dsc_calc_info *dsc = &connector->post_info[connector->active_idx]->dsc;
	struct dkmd_connector_info *pinfo = connector->conn_info;
	uint32_t connector_num = 0;
	uint32_t slice_num = 0;
	uint32_t video_chunk_num = 0;
	...
	/* check for dsc: video mode panel_xres is not division 3 and 1dsi support 2slice */
	if (mipi->color_mode == DSI_DSC24_COMPRESSED_DATA && mipi->burst_mode == DSI_NON_BURST_SYNC_EVENTS &&
		is_mipi_video_panel(&pinfo->base)) {
		set_reg(DPU_DSI_VIDEO_PKT_LEN_ADDR(mipi_dsi_base), dsc->dsc_info.chunk_size, 14, 0);
		connector_num = pinfo->sw_post_chn_num;
		slice_num = dsc->dsc_info.pic_width / dsc->dsc_info.slice_width;
		video_chunk_num = slice_num / connector_num;
		set_reg(DPU_DSI_VIDEO_CHUNK_NUM_ADDR(mipi_dsi_base), video_chunk_num, 14, 0);
	} 
...
{
	uint32_t delay_lanebyte_cycle = 0;
	...
	if (!is_ppu_support(&(pinfo->base))) {
		set_reg(DPU_DSI_TE_VS_WD_ADDR(mipi_dsi_base),
			(uint32_t)(((2 * lane_byte_clk / 1000000) << 12) | (2 * lane_byte_clk / 1000000)), 32, 0);
		return;
	}

	delay_lanebyte_cycle = (uint32_t)(lane_byte_clk * TE_DELAY_TIMES_US / 1000000);
	if (delay_lanebyte_cycle > TE_MAX_DELAY_CYCLE) {
		delay_lanebyte_cycle = TE_MAX_DELAY_CYCLE;
	}

	set_reg(DPU_DSI_TE_VS_WD_ADDR(mipi_dsi_base),
		((delay_lanebyte_cycle << 12) | delay_lanebyte_cycle), 32, 0);
	dpu_pr_info("te_delay_us is %d", (uint32_t)(delay_lanebyte_cycle * 1000000 / lane_byte_clk));
}
	...
	struct mipi_dsi_phy_ctrl *phy_ctrl, char __iomem *mipi_dsi_base)
{
	...
	if (is_mipi_cmd_panel(&pinfo->base)) {
		...
		mipi_dsi_te_delay_config(connector, lane_byte_clk);
...
void mipi_init(struct dpu_connector *connector)
{
	...
	struct dkmd_connector_info *pinfo = connector->conn_info;
	struct mipi_panel_info *mipi = NULL;
	struct dsc_calc_info *dsc = NULL;

	if (connector->active_idx == 1 && connector->post_info[1] == NULL) {
		dpu_pr_err("active_idx %u error!\n", connector->active_idx);
		return;
	}

	mipi = &(connector->post_info[connector->active_idx]->mipi);
	dsc = &connector->post_info[connector->active_idx]->dsc;
	...
	set_phy_ref_clk_ctl_en(connector);
...
void mipi_pll_cfg_for_clk_upt(struct dpu_connector *connector,
	struct mipi_dsi_phy_ctrl *phy_ctrl, char __iomem *mipi_dsi_base)
{
	dpu_pr_info("PLL configuration and update control \n");
