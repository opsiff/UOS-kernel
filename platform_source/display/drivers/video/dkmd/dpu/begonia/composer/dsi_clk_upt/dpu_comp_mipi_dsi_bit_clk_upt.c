	...
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	dpu_check_and_return(!buf, -EINVAL, err, "buf is NULL\n");
	dpu_comp = to_dpu_composer(get_comp_from_device(dev));
	dpu_check_and_return(!dpu_comp, -EINVAL, err, "dpu_comp is NULL\n");
	connector = get_primary_connector(dpu_comp->conn_info);
	dpu_check_and_return(!connector, -EINVAL, err, "connector is NULL\n");
	...
	if (mipi->dsi_bit_clk_upt_support == 0 && g_debug_mipi_dsi_clk_upt == 0) {
	...
	if (g_debug_mipi_dsi_clk_upt == 1) {
		if (sscanf_s(buf, "%u", &dsi_bit_clk_upt) == 0) {
			dpu_pr_err("external get dsi_bit_clk_upt err");
			return (ssize_t)count;
		}
		dpu_pr_info("comp index = %u, external dsi_bit_clk = %u\n", dpu_comp->comp.index, dsi_bit_clk_upt);
	} else {
	...
	while ((atomic_read(&connector->post_info[connector->active_idx]->mipi.dsi_bit_clk_upt_flag) == 1) &&
...
{
	ukmd_sysfs_attrs_append(&dpu_comp->attrs, &dev_attr_mipi_dsi_bit_clk_upt.attr);
}
