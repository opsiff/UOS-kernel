...
static bool jpu_device_need_bypass(void)
{
	int32_t ret;
	const char *soc_spec = NULL;
	struct device_node *np = of_find_compatible_node(NULL, NULL, "hisilicon, soc_spec");
	if (!np) {
		jpu_err("of_find_compatible_node fail or normal type chip\n");
		return false;
	}

	ret = of_property_read_string(np, "soc_spec_set", &soc_spec);
	if (ret) {
		jpu_err("of_property_read_string soc_spec_set fail\n");
		return false;
	}

	ret = strncmp(soc_spec, "pc", strlen("pc"));
	if (!ret) {
		jpu_err("pg chip : pc, need bypass jpu\n");
		return true;
	}

	ret = strncmp(soc_spec, "lite_pc", strlen("lite_pc"));
	if (!ret) {
		jpu_err("pg chip : lite_pc, need bypass jpu\n");
		return true;
	}
