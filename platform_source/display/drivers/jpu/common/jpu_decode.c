...
#include "jpu_platform.h"
#include "jpu_osal.h"
#include "jpu_timer.h"
#include "jpu_irq.h"

#ifdef CONFIG_JPEG_VOTE_OPEN
static uint32_t jpegd_vote(enum VOTE_VALUE vote, struct jpu_data_type *jpu_device)
{
	uint32_t ret;
	char __iomem * jpegd_vote_addr = SOC_SCTRL_SCBAKDATA3_MSK_ADDR(jpu_device->sctrl_base);
	uint32_t value = inp32(jpegd_vote_addr);

	if (vote == VOTE_OFF) {
		value = inp32(jpegd_vote_addr);
		value &= (~JPEGD_VOTE_BIT_MASK);
		value |= (JPEGD_VOTE_BIT_MASK << VOTE_BIT_MASK);
		outp32(jpegd_vote_addr, value);
		return SUCCESS;
	}
	...
	value = inp32(jpegd_vote_addr);
	ret = value & JPEGD_VOTE_OTHER_BIT_MASK;
...
static uint32_t jpegd_start_vote(struct jpu_data_type *jpu_device)
{
#ifdef CONFIG_JPEG_VOTE_OPEN
	uint32_t ret = ERROR;
	uint32_t value;
	uint32_t wait_time = VOTE_WAIT_TIMES;

	while (wait_time) {
		wait_time--;
		ret = jpegd_vote(VOTE_ON, jpu_device);
		if (ret == SUCCESS)
			break;
		os_wait_ms(VOTE_SLEEP_TIME);
	}
	return ret;
#else
	return SUCCESS;
...
static void jpegd_end_vote(struct jpu_data_type *jpu_device)
{
#ifdef CONFIG_JPEG_VOTE_OPEN
	jpegd_vote(VOTE_OFF, jpu_device);
#endif
}

static uint32_t jpu_set_bits32(uint32_t old_val, uint32_t val, uint8_t bw, uint8_t bs)
{
...
static int32_t jpu_dec_wait_done(struct jpu_data_type *jpu_device,
	const struct jpu_data_t *jpu_req)
{
	int32_t ret;
	ret = jpu_wait_interrupt((struct jpu_interrupt_t *)(jpu_device->jpu_irq_num));
	if (ret <= 0) {
		jpu_err("jpu decode timeout ret = %d, jpu_dec_done_flag = %u\n", 
...
static int32_t jpu_dec_done(struct jpu_data_type *jpu_device,
	struct jpu_data_t *jpu_req)
{
	...
	if (g_debug_jpu_dec_job_timediff != 0)
		os_get_timestamp(&tv0);

	ret = jpu_dec_wait_done(jpu_device, jpu_req);

	if (g_debug_jpu_dec_job_timediff != 0) {
		os_get_timestamp(&tv1);
		timediff = os_timestamp_diff(&tv0, &tv1);
...
static int32_t jpu_set_crop(struct jpu_data_type *jpu_device)
{
...
static int32_t jpu_set_dht(const struct jpu_data_t *jpu_req,
	char __iomem *jpu_dec_base)
{
	int32_t i;
...
static int32_t sample_size_hal2jpu(int32_t val)
{
	int32_t ret;
...
static int32_t jpu_set_paramters(struct jpu_data_type *jpu_device,
	struct jpu_data_t *jpu_req, jpu_dec_reg_t *pjpu_dec_reg)
{
	int32_t out_format;
	int32_t jpu_freq_scale;
	int32_t ret;
	uint8_t yfac;
	uint8_t ufac;
	uint8_t vfac;
...
static int32_t jpu_check_outbuffer_par(const struct jpu_data_t *jpu_req)
{
...
static int32_t jpu_check_inbuff_par(const struct jpu_data_t *jpu_req)
{
	...
	cond = ((jpu_req->inwidth / (uint32_t)jpu_req->sample_rate) < MIN_INPUT_WIDTH) ||
		(jpu_req->inwidth > MAX_INPUT_WIDTH) ||
		((jpu_req->inheight / (uint32_t)jpu_req->sample_rate) < MIN_INPUT_HEIGHT) ||
	...
	cond = ((jpu_req->pix_width / (uint32_t)jpu_req->sample_rate) < MIN_INPUT_WIDTH) ||
		(jpu_req->pix_width > MAX_INPUT_WIDTH) ||
		((jpu_req->pix_height / (uint32_t)jpu_req->sample_rate) < MIN_INPUT_HEIGHT) ||
...
static int32_t jpu_check_buffers(struct jpu_data_type *jpu_device,
	struct jpu_data_t *jpu_req)
{
...
static int32_t jpu_check_full_decode_info(const struct jpu_data_t *jpu_req)
{
	bool cond = false;
	...
	cond = (jpu_req->out_color_format == JPEG_DECODE_OUT_YUV422_H2V1) &&
		((jpu_req->pix_width / (uint32_t)jpu_req->sample_rate) % JPGDEC_EVEN_NUM);
	if (cond) {
	...
	cond = (jpu_req->out_color_format == JPEG_DECODE_OUT_YUV420) &&
		(((jpu_req->pix_width / (uint32_t)jpu_req->sample_rate) % JPGDEC_EVEN_NUM) ||
		((jpu_req->pix_height / (uint32_t)jpu_req->sample_rate) % JPGDEC_EVEN_NUM));
	if (cond) {
		jpu_err("out_color_format %d, jpu_req->pix_width %u invalid\n",
			jpu_req->out_color_format, jpu_req->pix_width);
		return -EINVAL;
...
static int32_t jpu_check_region_decode_info(const struct jpu_data_t *jpu_req)
{
	...
	if (((jpu_req->out_color_format == JPEG_DECODE_OUT_YUV420) ||
		...
		(((((jpu_req->region_info.right - jpu_req->region_info.left) + 1) /
			(uint32_t)jpu_req->sample_rate) % JPGDEC_EVEN_NUM) ||
		((((jpu_req->region_info.bottom - jpu_req->region_info.top) + 1) /
			(uint32_t)jpu_req->sample_rate) % JPGDEC_EVEN_NUM))) {
...
static int32_t jpu_check_format(const struct jpu_data_t *jpu_req)
{
...
static void jpu_dump_info(const struct jpu_data_t *jpu_info)
{
	...
	for (i = 0; i < HAC_SYMBOL_TABLE_NUM; i++)
		jpu_debug("hac_symbol_tab %u\n", jpu_info->hac_symbol_tab[i]);

	for (i = 0; i < MAX_DCT_SIZE; i++)
		jpu_debug("qvalue %d\n", jpu_info->qvalue[i]);
}

static int32_t jpu_check_userdata(struct jpu_data_type *jpu_device,
	struct jpu_data_t *jpu_req)
{
...
static int32_t jpu_dec_set_unreset(struct jpu_data_type *jpu_device)
{
...
static int32_t jpu_dec_set_cvdr(const struct jpu_data_type *jpu_device)
{
...
static int32_t jpu_dec_reg_init(struct jpu_data_type *jpu_device)
{
	errno_t ret;
	/* cppcheck-suppress */
...
static int32_t jpu_dec_reg_default(struct jpu_data_type *jpu_device)
{
	...
	jpu_dec_reg_t *pjpu_dec_reg, const struct jpu_data_t *jpu_req)
{
	...
	if ((jpu_device->jpu_support_platform == DSS_V600) ||
		...
		/* jpu_req->smmu_enable */
		pjpu_dec_reg->preftch_ctrl = jpu_set_bits32(pjpu_dec_reg->preftch_ctrl,
			jpu_req->smmu_enable ? 0x0 : 0x1, REG_SET_32_BIT, 0);
	}
...
static int32_t jpu_userdata_and_reg_init(struct jpu_data_type *jpu_device,
...
static int32_t jpu_lock_usecase(const struct jpu_data_type *jpu_device)
{
#if defined(CONFIG_DPU_FB_V501) || defined(CONFIG_DPU_FB_V510)
	int32_t ret;
...
static int32_t jpu_dec_set_reg(struct jpu_data_type *jpu_device,
	jpu_dec_reg_t *jpu_dec_reg)
{
	char __iomem *jpu_dec_base = NULL;
...
int32_t jpu_job_decode(struct jpu_data_type *jpu_device, const void __user *argp)
{
	int32_t ret = 0;
	struct jpu_data_t *jpu_req = NULL;
	jpu_dec_reg_t *pjpu_dec_reg = NULL;
	...
	update_time(jpu_device, DELTA_TIME_WORK);
	...
	if (jpegd_start_vote(jpu_device)) {
		jpu_err("jpegd vote failed\n");
		ret = -EINVAL;
		goto err_vote;
	}

	if (jpu_aft_on(jpu_device) != 0) {
		jpu_err("jpu after powr on failed\n");
		ret = -EINVAL;
		goto err_put_hard_lock;
	}

	if (os_copy_from_user(jpu_req, sizeof(struct jpu_data_t), argp, sizeof(struct jpu_data_t)) != 0) {
	...
	if (jpu_handle_iommu_map(jpu_device, jpu_req) != 0) {
			jpu_err("jpu_handle_iommu_map failed\n");
			goto err_put_dma;
	}
...
err_put_unmap:
	(void)jpu_handle_iommu_unmap(jpu_device, jpu_req);
err_put_dma:
	jpu_put_dmabuf(&buf);
err_put_hard_lock:
	jpu_pre_off(jpu_device);
	jpegd_end_vote(jpu_device);
err_vote:
	jpu_unlock_usecase(jpu_device);
err_usecase:
	if (ret != 0) {
		trigger_power_off(jpu_device, DELTA_TIME_OFF, ERR_COND);
	} else {
		trigger_power_off(jpu_device, DELTA_TIME_SLEEP, OFF_COND);
	}
	jpu_info("-\n");
	return ret;
}
