...
static int npu_intr_hub_probe(struct platform_device *pdev)
{
	...
	irq_set_chained_handler_and_data((u32)irqc->parent_irq, npu_intr_hub_intc_irq_handler, irqc);
	npu_drv_info("npu_intr_hub init:success\n");
	return 0;
}

static int npu_intr_hub_remove(struct platform_device *pdev)
{
	struct npu_intr_hub_irq_chip *irqc = platform_get_drvdata(pdev);

	if (irqc->parent_irq >= 0)
		irq_set_chained_handler_and_data((u32)irqc->parent_irq, NULL, NULL);
