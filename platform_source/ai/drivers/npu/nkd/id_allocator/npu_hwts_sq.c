...
u32 npu_alloc_hwts_sq(u8 dev_id, u8 is_long)
{
	...
	if (hwts_sq_info == NULL) {
		npu_drv_err("calc hwts sq info fail, index = %u\n",
			entity_info->index);
		npu_free_entity(dev_id, (is_long != 0) ? NPU_MEM_POOL_TYPE_LONG_HWTS_SQ :
