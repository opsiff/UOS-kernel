	...
	struct npu_mailbox_message *message)
{
	...
	ret = plat_info->adapter.res_ops.npu_mailbox_send(mailbox->send_sram,
		...
		message->message_payload, (unsigned long)message->message_length);
	if (ret != 0) {
...
static irqreturn_t npu_mailbox_ack_irq(int irq, void *data)
{
	struct npu_mailbox_message *message = NULL;
...
int npu_mailbox_init(u8 dev_id)
{
	...
	ret = request_irq((unsigned int)(plat_info->dts_info.irq_mailbox_ack),
	...
	struct npu_mailbox_message *message, int *result)
{
	...
	ret = down_timeout(&message->wait, (long)jiffy);
	if (ret == 0) {
...
void npu_mailbox_destroy(u8 dev_id)
{
	struct npu_dev_ctx *dev_ctx = NULL;
	struct npu_mailbox *mailbox = NULL;
	struct npu_platform_info *plat_info = NULL;
	...
	// register irq handler
	free_irq((unsigned int)(plat_info->dts_info.irq_mailbox_ack), mailbox);
