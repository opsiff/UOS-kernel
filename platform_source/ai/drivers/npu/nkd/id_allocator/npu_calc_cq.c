...
int npu_cq_list_init(u8 dev_id)
{
	...
	for (i = 0; i < NPU_MAX_CQ_NUM; i++) {
		...
		cq_info->tail = 0;
		cq_info->index = i;
		atomic_set(&cq_info->count_report, 0);
...
u32 npu_get_cq_ref_by_stream(u8 dev_id, u32 cq_id)
{
	struct npu_ts_cq_info *cq_info = NULL;
	struct npu_dev_ctx *cur_dev_ctx = NULL;
	u32 cq_stream_num;
...
u32 npu_get_cq_ref_by_communication_stream(u8 dev_id, u32 cq_id)
{
	struct npu_ts_cq_info *cq_info = NULL;
	struct npu_dev_ctx *cur_dev_ctx = NULL;
	u32 cq_stream_num;
...
static u32 npu_alloc_cq_id(u8 dev_id)
{
...
static int npu_free_cq_id(u8 dev_id, u32 cq_id)
{
	struct npu_ts_cq_info *cq_info = NULL;
	struct npu_cq_sub_info *cq_sub = NULL;
	struct npu_dev_ctx *cur_dev_ctx = NULL;
...
static int npu_alloc_cq_mem(u8 dev_id, u32 cq_id, int pid)
{
	struct npu_ts_cq_info *cq_info = NULL;
	struct npu_cq_sub_info *cq_sub = NULL;
	struct npu_dev_ctx *cur_dev_ctx = NULL;
	...
	sq_desc = get_sq_desc();
	cond_return_error(sq_desc == NULL, -1, "sq_desc is null ptr\n");
	phy_addr = (unsigned long long)(sq_desc->base +
...
static int npu_free_cq_mem(u8 dev_id, u32 cq_id)
{
	struct npu_cq_sub_info *cq_sub = NULL;
	struct npu_ts_cq_info *cq_info = NULL;
	struct npu_dev_ctx *cur_dev_ctx = NULL;
...
int npu_cq_list_destroy(u8 dev_id)
{
	...
	if (list_empty_careful(&cur_dev_ctx->cq_available_list) == 0) {
