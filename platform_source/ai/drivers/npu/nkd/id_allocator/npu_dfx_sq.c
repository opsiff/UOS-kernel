...
static int npu_get_dfx_sq_memory(const struct npu_dfx_sq_info *sq_info,
...
static int npu_dfx_sq_para_check(const struct npu_dfx_create_sq_para *sq_para)
{
	...
	struct npu_dfx_create_sq_para *sq_para)
{
	...
	*sq_para->addr = (unsigned long)sq_info[sq_index].phy_addr;
	...
	npu_drv_debug("dev[%d] dfx sq is created, sq id: %d, sq addr: 0x%lx\n",
		...
		(unsigned long)sq_info[sq_index].phy_addr);

	return 0;
}

void npu_destroy_dfx_sq(struct npu_dev_ctx *cur_dev_ctx, u16 sq_index)
{
	struct npu_dfx_sq_info *sq_info = NULL;
	struct npu_dfx_cqsq *cqsq = NULL;
...
int npu_dfx_send_sq(u8 devid, u16 sq_index, const u8 *buffer, u32 buf_len)
{
	struct npu_dfx_sq_info *sq_info = NULL;
