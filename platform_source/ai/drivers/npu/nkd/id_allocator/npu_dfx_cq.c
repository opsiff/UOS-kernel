...
static void npu_heart_beat_work(struct work_struct *work)
{
	...
	u32 exception_num = 0;
	u32 exception_idx;
	u8 cq_data_tmp[NPU_DFX_EXCEP_CQ_LEN][NPU_DFX_MAX_CQ_SLOT_LEN];
	...
	while (cq_slot[0] == cq_info->phase) {
		...
		if (npu_heart_beat_is_exception(cq_data) == 1) {
			npu_drv_warn("it is a exception heart, exception_num = %d!\n", exception_num);
			if (exception_num >= NPU_DFX_EXCEP_CQ_LEN) {
				npu_drv_err("exception collection is full!\n");
				break;
			}
			if (memcpy_s(cq_data_tmp[exception_num], NPU_DFX_MAX_CQ_SLOT_LEN, cq_data,
				cq_info->slot_len - NPU_DFX_DETAILED_CQ_OFFSET) != 0) {
				npu_drv_err("memcpy_s fail\n");
				goto fail;
			}
			exception_num++;
		} else if (cq_info->callback != NULL) {
...
fail:
	...
	for (exception_idx = 0; exception_idx < exception_num; exception_idx++)
		npu_heart_beat_exception_process(cq_data_tmp[exception_idx], NPU_DFX_MAX_CQ_SLOT_LEN);
}
	...
	struct npu_dfx_cq_info *cq_info)
{
	...
	cur_dev_ctx = get_dev_ctx_by_id((u8)cq_info->devid);
	if (cur_dev_ctx == NULL) {
	...
	u32 size)
{
	...
	struct npu_mem_desc *sq_desc = NULL;
	unsigned int cnt;
	...
	sq_desc = get_sq_desc();
	cond_return_error(sq_desc == NULL, -EINVAL, "sq_desc is null ptr\n");
	// phy_addr:sq_calc_size + cq_calc_size + sq_dfx_size + off_size
	cq_info->phy_addr = (unsigned long long)(sq_desc->base +
		NPU_DFX_SQ_OFFSET +
	...
	npu_drv_debug("cur sq %d phy_addr = %pK base = %pK\n", cq_info->index,
		...
		(void *)(uintptr_t)(long)sq_desc->base);

	return 0;
}

static int npu_dfx_cq_para_check(const struct npu_dfx_create_cq_para *cq_para)
{
...
static irqreturn_t npu_dfx_cq_irq_handler(int irq, void *data)
{
	struct npu_dfx_int_context *int_context = NULL;
	struct npu_dfx_cq_info *cq_info = NULL;
	...
	cond_return_error(data == NULL, IRQ_HANDLED, "data is null\n");
	npu_counter_inc(NPU_COUNTER_RECV_DFX_CQ_INT);
...
void npu_destroy_dfx_cq(struct npu_dev_ctx *cur_dev_ctx, u16 cq_index)
{
	struct npu_dfx_int_context *int_context = NULL;
	struct npu_dfx_cq_info *cq_info = NULL;
	struct npu_dfx_cqsq *cqsq = NULL;
...
int npu_dfx_cqsq_init(struct npu_dev_ctx *cur_dev_ctx)
{
	...
	ret = request_irq((unsigned int)cqsq->int_context.irq_num, npu_dfx_cq_irq_handler,
...
void npu_destroy_dfx_cqsq(struct npu_dev_ctx *cur_dev_ctx)
{
	...
	free_irq((unsigned int)cqsq->int_context.irq_num, &cqsq->int_context);
