...
static u32 npu_alloc_sq_id(u8 dev_id)
{
...
static int npu_alloc_sq_mem(u8 dev_id, u32 sq_id)
{
	struct npu_ts_sq_info *sq_info = NULL;
	struct npu_sq_sub_info *sq_sub = NULL;
	struct npu_dev_ctx *cur_dev_ctx = NULL;
	...
	phy_addr = (unsigned long long)(sq_desc->base + (sq_id *
...
static int npu_free_sq_id(u8 dev_id, u32 sq_id)
{
	struct npu_ts_sq_info *sq_info = NULL;
	struct npu_sq_sub_info *sq_sub = NULL;
	struct npu_dev_ctx *cur_dev_ctx = NULL;
...
static int npu_free_sq_mem(u8 dev_id, u32 sq_id)
{
	struct npu_sq_sub_info *sq_sub = NULL;
	struct npu_ts_sq_info *sq_info = NULL;
	struct npu_dev_ctx *cur_dev_ctx = NULL;
...
int npu_sq_list_destroy(u8 dev_id)
{
	...
	if (list_empty_careful(&cur_dev_ctx->sq_available_list) == 0) {
...
{
	int ret = 0;
	u32 sq_id = npu_alloc_sq_id(dev_id);
	if (sq_id >= NPU_MAX_SQ_NUM) {
