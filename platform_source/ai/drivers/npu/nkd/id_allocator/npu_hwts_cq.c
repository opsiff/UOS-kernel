...
#else
	*virt_addr = get_shm_virt_addr(0, NPU_SQ_MEM) + NPU_NS_HWTS_SQ_SIZE;
	*iova = get_shm_phy_addr(0, NPU_SQ_MEM) + NPU_NS_HWTS_SQ_SIZE;
#endif
	return ret;
}
...
int npu_hwts_cq_init(u8 dev_id)
{
	...
	if (ret != 0) {
		npu_drv_err("pool regist error! ret = %d,\n",
			ret);
		return -1;
	}
...
u32 npu_alloc_hwts_cq(u8 dev_id)
{
	...
	if (entity_info == NULL) {
		npu_drv_err("alloc entity fail\n");
		return NPU_MAX_HWTS_CQ_INFO_NUM;
	}

	hwts_cq_info = npu_calc_hwts_cq_info(dev_id, entity_info->index);
	if (hwts_cq_info == NULL) {
		npu_drv_err("calc hwts sq info fail, index=%d\n",
			entity_info->index);
...
int npu_free_hwts_cq(u8 dev_id, u32 hwts_cq_id)
{
	...
	if (ret != 0) {
		npu_drv_err("free entity error! hwts_cq_id=%d, ret=%d",
			hwts_cq_id, ret);
		return ret;
	}
