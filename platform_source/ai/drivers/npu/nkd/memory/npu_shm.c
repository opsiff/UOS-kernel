...
struct npu_mem_desc *get_sq_desc(void)
{
	return g_sq_desc;
}
EXPORT_SYMBOL(get_sq_desc);
...
size_t get_shm_size(u8 devid, u32 index)
{
	if (devid >= NPU_DEV_NUM || index >= NPU_MAX_MEM) {
		npu_drv_err("invalid id or mem_index %u", index);
		return 0;
	}
	return g_shm_desc[devid][index].size;
}
...
struct npu_ts_cq_info *npu_calc_cq_info(u8 devid, u32 index)
{
	...
	cond_return_error(devid >= NPU_DEV_NUM, NULL,
...
static int npu_shm_v200_init_sq_mem(u8 dev_id)
{
...
int npu_shm_v200_init_ffrt_buff(u8 dev_id)
{
	vir_addr_t virt_addr;
	struct npu_platform_info *plat_info = NULL;
	...
	// mdc init
	g_shm_desc[dev_id][NPU_FFRT_MDC_MEM].phy_addr = plat_info->resmem_info.ffrt_mdc_buf->base;
	g_shm_desc[dev_id][NPU_FFRT_MDC_MEM].size = plat_info->resmem_info.ffrt_mdc_buf->len;
	virt_addr = (vir_addr_t)(uintptr_t)ioremap_wc(
		g_shm_desc[dev_id][NPU_FFRT_MDC_MEM].phy_addr,
		g_shm_desc[dev_id][NPU_FFRT_MDC_MEM].size);
	if (virt_addr == 0) {
		npu_drv_err("ioremap_wc failed!");
		goto init_mdc_mem_failed;
	}

	g_shm_desc[dev_id][NPU_FFRT_MDC_MEM].virt_addr = virt_addr;
	memset_s(virt_addr, g_shm_desc[dev_id][NPU_FFRT_MDC_MEM].size, 0, g_shm_desc[dev_id][NPU_FFRT_MDC_MEM].size);
	npu_drv_debug("mdc buff mem: phy_addr = 0x%llx, size = %lu, virt_addr = 0x%llx\n",
		g_shm_desc[dev_id][NPU_FFRT_MDC_MEM].phy_addr,
		g_shm_desc[dev_id][NPU_FFRT_MDC_MEM].size,
		g_shm_desc[dev_id][NPU_FFRT_MDC_MEM].virt_addr);
	// swap buff init
	g_shm_desc[dev_id][NPU_FFRT_SWAP_MEM].phy_addr = plat_info->resmem_info.ffrt_swap_buf->base;
	g_shm_desc[dev_id][NPU_FFRT_SWAP_MEM].size = plat_info->resmem_info.ffrt_swap_buf->len;
	virt_addr = (vir_addr_t)(uintptr_t)ioremap_wc(
		g_shm_desc[dev_id][NPU_FFRT_SWAP_MEM].phy_addr,
		g_shm_desc[dev_id][NPU_FFRT_SWAP_MEM].size);
	if (virt_addr == 0) {
		npu_drv_err("ioremap_wc failed!");
		goto init_swap_mem_failed;
	}

	g_shm_desc[dev_id][NPU_FFRT_SWAP_MEM].virt_addr = virt_addr;
	memset_s(virt_addr, g_shm_desc[dev_id][NPU_FFRT_SWAP_MEM].size, 0, g_shm_desc[dev_id][NPU_FFRT_SWAP_MEM].size);
	npu_drv_debug("ffrt swap buff mem: phy_addr = 0x%llx, size = %lu, virt_addr = 0x%llx\n",
		g_shm_desc[dev_id][NPU_FFRT_SWAP_MEM].phy_addr,
		g_shm_desc[dev_id][NPU_FFRT_SWAP_MEM].size,
		g_shm_desc[dev_id][NPU_FFRT_SWAP_MEM].virt_addr);
	return 0;
init_swap_mem_failed:
	iounmap((void *)(
			uintptr_t)g_shm_desc[dev_id][NPU_FFRT_SWAP_MEM].virt_addr);
	g_shm_desc[dev_id][NPU_FFRT_SWAP_MEM].virt_addr = 0;
init_mdc_mem_failed:
	iounmap((void *)(
			uintptr_t)g_shm_desc[dev_id][NPU_FFRT_MDC_MEM].virt_addr);
	g_shm_desc[dev_id][NPU_FFRT_MDC_MEM].virt_addr = 0;
	return -1;
}
#endif
...
int npu_shm_v200_init_ispnn_info_buff(u8 dev_id)
{
	vir_addr_t virt_addr;
	struct npu_platform_info *plat_info = NULL;
	...
	cond_return_error(plat_info == NULL, NULL, "npu_plat_get_info error");
	// ispnn info init
	g_shm_desc[dev_id][NPU_ISPNN_INFO_MEM].phy_addr = plat_info->resmem_info.ispnn_info_buf->base;
	g_shm_desc[dev_id][NPU_ISPNN_INFO_MEM].size = plat_info->resmem_info.ispnn_info_buf->len;
	virt_addr = (vir_addr_t)(uintptr_t)ioremap_wc(
		g_shm_desc[dev_id][NPU_ISPNN_INFO_MEM].phy_addr,
		g_shm_desc[dev_id][NPU_ISPNN_INFO_MEM].size);
	if (virt_addr == 0) {
		npu_drv_err("ioremap_wc failed!");
		goto init_ispnn_info_mem_failed;
	}

	g_shm_desc[dev_id][NPU_ISPNN_INFO_MEM].virt_addr = virt_addr;
	memset_s(virt_addr, g_shm_desc[dev_id][NPU_ISPNN_INFO_MEM].size, 0, g_shm_desc[dev_id][NPU_ISPNN_INFO_MEM].size);
	npu_drv_warn("ispnn info mem: phy_addr = 0x%llx, size = %lu, virt_addr = 0x%llx\n",
		g_shm_desc[dev_id][NPU_ISPNN_INFO_MEM].phy_addr,
		g_shm_desc[dev_id][NPU_ISPNN_INFO_MEM].size,
		g_shm_desc[dev_id][NPU_ISPNN_INFO_MEM].virt_addr);

	return 0;
init_ispnn_info_mem_failed:
	iounmap((void *)(
			uintptr_t)g_shm_desc[dev_id][NPU_ISPNN_INFO_MEM].virt_addr);
	g_shm_desc[dev_id][NPU_ISPNN_INFO_MEM].virt_addr = 0;
	return -1;
}
#endif

static int npu_shm_v200_init_info_mem(struct npu_platform_info *plat_info,
	u8 dev_id)
{
	vir_addr_t virt_addr;
...
static int npu_shm_v200_init_pad_mem(u8 dev_id)
{
	gfp_t gfp_flags;
	char *tmp = NULL;
...
#endif
#ifdef CONFIG_NPU_FFRT
	ret = npu_shm_v200_init_ffrt_buff(dev_id);
	if (ret != 0)
		return ret;
#endif
#ifdef CONFIG_NPU_ISPNN_INFO
	ret = npu_shm_v200_init_ispnn_info_buff(dev_id);
	if (ret != 0)
		return ret;
#endif
...
#endif
#ifdef CONFIG_NPU_FFRT
	if (g_shm_desc[dev_id][NPU_FFRT_MDC_MEM].virt_addr != 0) {
		iounmap((void *)(
			uintptr_t)g_shm_desc[dev_id][NPU_FFRT_MDC_MEM].virt_addr);
		g_shm_desc[dev_id][NPU_FFRT_MDC_MEM].virt_addr = 0;
	}
	if (g_shm_desc[dev_id][NPU_FFRT_SWAP_MEM].virt_addr != 0) {
		iounmap((void *)(
			uintptr_t)g_shm_desc[dev_id][NPU_FFRT_SWAP_MEM].virt_addr);
		g_shm_desc[dev_id][NPU_FFRT_SWAP_MEM].virt_addr = 0;
	}
#endif
#ifdef CONFIG_NPU_ISPNN_INFO
	if (g_shm_desc[dev_id][NPU_ISPNN_INFO_MEM].virt_addr != 0) {
		iounmap((void *)(
			uintptr_t)g_shm_desc[dev_id][NPU_ISPNN_INFO_MEM].virt_addr);
		g_shm_desc[dev_id][NPU_ISPNN_INFO_MEM].virt_addr = 0;
	}
...
pid_struct_get_fail:
	return ret;
#else
	npu_drv_warn("no need to flush cache in LDK");
	return 0;
#endif
}
...
int l2_mem_swapout(struct vm_area_struct *vma, u8 dev_id, pid_t process_id)
{
	...
	npu_drv_debug(" enter");
	cond_return_error(dev_id >= NPU_DEV_NUM, -EFAULT,
		"illegal npu dev id = %u\n", dev_id);
