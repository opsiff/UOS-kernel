...
static int sg_table_va_map_insert(struct sg_table *table, unsigned int nents,
	unsigned long long va, unsigned long size)
{
	...
	if (g_sg_table_va_map_manager.is_init == 0)
...
static int sg_table_va_map_find(struct sg_table **table, unsigned int *nents,
	...
	unsigned long size)
{
	...
	nents = (unsigned int)(PAGE_ALIGN(size) / PAGE_SIZE);
