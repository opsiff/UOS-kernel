...
static int verify_nonsink_task_comm_field(struct npu_proc_ctx *proc_ctx,
	npu_rt_task_t *ts_task)
{
...
static int verify_evt_rec_comm_task(struct npu_proc_ctx *proc_ctx,
	npu_rt_task_t *ts_task)
{
...
static int verify_wait_evt_comm_task(struct npu_proc_ctx *proc_ctx,
	npu_rt_task_t *ts_task)
{
	if (npu_proc_check_event_id(proc_ctx, ts_task->u.event_task.event_id) != 0) {
		npu_drv_err("invalid event_id:%u, task_id:%u\n",
...
static int verify_memcpy_comm_task(struct npu_proc_ctx *proc_ctx,
...
static int verify_maintenance_comm_task(struct npu_proc_ctx *proc_ctx,
	npu_rt_task_t *ts_task)
{
	unused(proc_ctx);
...
static int verify_create_stream_comm_task(struct npu_proc_ctx *proc_ctx,
	npu_rt_task_t *ts_task)
{
	unused(proc_ctx);
...
static int verify_model_maintenance_comm_task(struct npu_proc_ctx *proc_ctx,
	npu_rt_task_t *ts_task)
{
...
static int verify_model_execute_comm_task(struct npu_proc_ctx *proc_ctx,
	npu_rt_task_t *ts_task)
{
...
static int verify_model_loop_execute_task(struct npu_proc_ctx *proc_ctx,
	npu_rt_task_t *ts_task)
{
	unused(ts_task);
	return proc_ctx->dev_type == DIRECT ? 0 : -1;
}

static int verify_model_stop_task(struct npu_proc_ctx *proc_ctx,
	npu_rt_task_t *ts_task)
{
	unused(ts_task);
...
static int verify_bypass_comm_task(struct npu_proc_ctx *proc_ctx,
	npu_rt_task_t *ts_task)
{
	unused(proc_ctx);
	unused(ts_task);
...
static const verify_ts_task_func verify_ts_sqe_map[] = {
	...
	NULL,                         NULL,                     NULL, NULL,
	/* 12                                         13 */
	verify_model_maintenance_comm_task, verify_model_execute_comm_task,
	/* 14                                 15 */
	verify_model_loop_execute_task, verify_model_stop_task,
	/* 16                         17                        18    19 */
	NULL,                         NULL,                     NULL, NULL,
	/* 20                         21          22    23 */
