...
int npu_send_alloc_stream_mailbox(u8 cur_dev_id, u32 stream_id, u32 cq_id)
{
	...
	ret = npu_mailbox_message_send_for_res(cur_dev_id,
		(u8 *)alloc_stream_msg, (u32)msg_len, &mbx_send_result);
	if (ret != 0) {
...
static struct npu_stream_info *npu_alloc_sink_stream(u8 dev_id, u8 is_long,
	u32 sq_id, u32 cq_id)
{
	...
	stream_id = (u32)npu_alloc_sink_stream_id(dev_id, is_long);
...
static struct npu_stream_info *npu_alloc_non_sink_stream(u8 cur_dev_id,
	u32 sq_id, u32 cq_id)
{
	int ret = 0;
	struct npu_stream_info *stream_info = NULL;
	...
	u32 stream_id = (u32)npu_alloc_stream_id(cur_dev_id);
	...
	inform_ts = (cur_dev_ctx->pm.ts_work_status == NPU_TS_WORK) ? NPU_NEED_TO_INFORM : NPU_NO_NEED_TO_INFORM;
...
static int npu_free_sink_stream(u8 dev_id, u32 stream_id)
{
	struct npu_stream_info *stream_info = NULL;
	int ret;
	struct npu_platform_info *plat_info = npu_plat_get_info();
...
static int npu_add_dev_rubbish_stream(struct npu_dev_ctx *cur_dev_ctx, u32 stream_id)
{
...
static int npu_free_non_sink_stream(u8 dev_id, u32 stream_id)
{
