...
int npu_sink_stream_list_init(struct npu_dev_ctx *dev_ctx)
{
	...
	for (i = 0; i < sink_stream_num; i++) {
		...
		if (sink_stream_info == NULL) {
			npu_id_allocator_destroy(id_allocator_short);
			if (id_allocator_long != NULL)
				npu_id_allocator_destroy(id_allocator_long);
			npu_drv_err("npu get sink_stream_info is null\n");
			return -EINVAL;
		}

		sink_stream_info->id = i + (u32)NPU_MAX_NON_SINK_STREAM_ID;
...
int npu_alloc_sink_stream_id(u8 dev_id, u8 is_long)
{
	...
	cond_return_error(NPU_MAX_SINK_LONG_STREAM_ID == 0 && (is_long != 0),
		-1, "invalid param\n");
	cond_return_error(dev_id >= NPU_DEV_NUM, -1,
		"invalid id\n");
	cur_dev_ctx = get_dev_ctx_by_id(dev_id);
	cond_return_error(cur_dev_ctx == NULL, -1, "cur_dev_ctx is null\n");
	id_allocator = &(cur_dev_ctx->id_allocator[(is_long != 0) ?
