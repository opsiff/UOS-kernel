...
static int npu_recycle_event_id(struct npu_proc_ctx *proc_ctx)
{
...
static int npu_recycle_notify_id(struct npu_proc_ctx *proc_ctx)
{
	...
	list_for_each_safe(pos, n, &proc_ctx->notify_list) {
		notify_info = list_entry(pos, struct npu_id_entity, list);
		if (notify_info != NULL)
			(void)npu_proc_free_notify(proc_ctx, notify_info->id);
	}
	mutex_unlock(&proc_ctx->notify_mutex);

	npu_drv_debug("recycle %u notify resource success\n",
		proc_ctx->notify_num);
	return 0;
}

static int npu_recycle_hwts_event_id(struct npu_proc_ctx *proc_ctx)
{
	struct npu_id_entity *event_info = NULL;
	struct list_head *pos = NULL;
	struct list_head *n = NULL;
...
static void npu_recycle_model_id(struct npu_proc_ctx *proc_ctx)
{
...
static void npu_recycle_task_id(struct npu_proc_ctx *proc_ctx)
{
...
bool npu_is_proc_resource_leaks(const struct npu_proc_ctx *proc_ctx)
{
	...
	if (list_empty_careful(&proc_ctx->sink_stream_list) == 0 ||
		list_empty_careful(&proc_ctx->stream_list) == 0 ||
		list_empty_careful(&proc_ctx->event_list) == 0 ||
		list_empty_careful(&proc_ctx->hwts_event_list) == 0 ||
		list_empty_careful(&proc_ctx->notify_list) == 0 ||
		list_empty_careful(&proc_ctx->model_list) == 0 ||
		list_empty_careful(&proc_ctx->task_list) == 0 ||
		list_empty_careful(&proc_ctx->task_set) == 0 ||
		list_empty_careful(&proc_ctx->excpt_task_set) == 0)
		result = true;
...
void npu_resource_leak_print(const struct npu_proc_ctx *proc_ctx)
{
	...
	if (list_empty_careful(&proc_ctx->event_list) == 0)
		npu_drv_warn("some event id are not released !! event num = %d\n",
			proc_ctx->event_num);

	if (list_empty_careful(&proc_ctx->hwts_event_list) == 0)
		npu_drv_warn(
			"some hwts event id are not released !! hwts event num = %u\n",
			proc_ctx->hwts_event_num);

	if (list_empty_careful(&proc_ctx->notify_list) == 0)
		npu_drv_warn(
			"some notify id are not released !! notify num = %u\n",
			proc_ctx->notify_num);

	if (list_empty_careful(&proc_ctx->model_list) == 0)
		npu_drv_warn("some model id are not released !! model num = %d\n",
			proc_ctx->model_num);

	if (list_empty_careful(&proc_ctx->task_list) == 0)
...
void npu_recycle_npu_resources(struct npu_proc_ctx *proc_ctx)
{
	...
	/* recycle notify */
	(void)npu_recycle_notify_id(proc_ctx);
	...
	if (proc_ctx->dev_type == DIRECT) {
		if (proc_ctx->task_cnt[NPU_NONSEC_EFF] != 0 ||
			proc_ctx->task_cnt[NPU_NONSEC_GE] != 0 || proc_ctx->task_cnt[NPU_NONSEC] != 0) {
			npu_drv_warn("wait for ts to recycle resources\n");
			mdelay(300);
		}
	}
