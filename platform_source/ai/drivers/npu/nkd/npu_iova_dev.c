...
static int npu_iova_probe(struct platform_device *pdev)
{
	struct npu_platform_info *platform_info = NULL;

	npu_drv_debug("npu smmu probe start\n");

	platform_info = npu_plat_get_info();
	if (platform_info == NULL) {
		npu_drv_err("get platform info failed.\n");
		return -ENODEV;
	}
...
static int npu_iova_remove(struct platform_device *pdev)
{
	unused(pdev);
	return 0;
}
...
int npu_iova_init(void)
{
	...
	if (ret != 0) {
		npu_drv_err("insmod devdrv npu iova driver fail\n");
		return ret;
	}
