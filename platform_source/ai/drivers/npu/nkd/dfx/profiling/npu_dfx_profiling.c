...
static int npu_prof_move_ring_buff_to_user(struct profiling_ring_buff_manager *ring_buff,
	u32 circle_len, uint64_t src_addr, uint64_t dest)
{
	...
	if (src_addr == 0 || ring_buff == NULL || dest == 0 ||
		...
		npu_drv_err("addr is null. src_addr:0x%pK, ring_buff:0x%pK, dest:0x%pK, circle_len:%u\n",
			(uintptr_t)src_addr, (uintptr_t)ring_buff, (uintptr_t)dest, circle_len);
		return PROF_ERROR;
	}
	...
	npu_drv_debug("copy_to_user, src_addr:0x%pK, dest:0x%pK, buff_len:0x%x, write:0x%x, read:0x%x\n",
		(uintptr_t)src_addr, (uintptr_t)dest, circle_len, ring_buff->write, ring_buff->read);
	...
	if (ring_buff->read < ring_buff->write) {
		...
		if (ret > 0) {
			npu_drv_err("copy_to_user fail, src_addr:0x%pK, dest:0x%pK, write:0x%x, read:0x%x\n",
				(uintptr_t)src_addr, (uintptr_t)dest, ring_buff->write, ring_buff->read);
			return PROF_ERROR;
		}
		return PROF_OK;
...
static int npu_prof_setting(unsigned long arg)
{
...
static int npu_prof_start(void)
{
	struct prof_buff_desc *prof_buff = npu_prof_get_mem();
	int ret;
...
static int npu_prof_read(unsigned long arg)
{
	void __user *parg = NULL;
...
static long npu_prof_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	int ret = PROF_OK;
...
static int npu_prof_open(struct inode *inode, struct file *filp)
{
	unused(inode);
	unused(filp);
...
static int npu_prof_release(struct inode *inode, struct file *filp)
{
	struct prof_buff_desc *prof_buff = NULL;
	unused(inode);
	unused(filp);
...
static int npu_prof_register_cdev(void)
{
	int ret;
	unsigned int major;
	dev_t devno;
...
static void npu_prof_free_cdev(void)
{
	struct profiling_device *priv = &g_prof_char_dev;
...
static int npu_prof_buff_init(void)
{
	int ret;
...
static int npu_prof_buff_release(void)
{
	int ret;
