...
typedef enum npu_task_type {
	...
	NPU_TASK_KERNEL_AIC_EFF = 1,	    /* AI core task */
	...
	NPU_TASK_MODEL_LOOP_EXECUTE = 14,
	NPU_TASK_MODEL_STOP = 15,
...
typedef struct npu_rt_model_stop_task {
	u16 model_id;
	u8 reserved[46];
} npu_rt_model_stop_task_t;
...
typedef struct npu_rt_notify_record_task {
	u32 notify_id;
	u8 reserved[44];
} npu_rt_notify_record_task_t;

typedef struct npu_rt_notify_wait_task {
	u32 notify_id;
	u8 reserved[44];
} npu_rt_notify_wait_task_t;

typedef struct npu_rt_hts_event_record {
	uint32_t event_id;
	uint32_t dst_id;
	uint32_t pid;
	u8 reserved[36];
} npu_rt_hts_event_record_t;

typedef struct npu_rt_hts_event_wait {
	uint32_t event_id;
	u8 reserved[44];
} npu_rt_hts_event_wait_t;

typedef struct npu_rt_write_value {
	uint32_t type;
	uint32_t value;
	u8 reserved[40];
} npu_rt_write_value_t;
...
typedef struct npu_rt_task {
	...
	union {
		...
		npu_rt_model_stop_task_t model_stop_task;
		...
		npu_rt_hts_event_record_t hts_event_record_task;
		npu_rt_hts_event_wait_t hts_event_wait_task;
		npu_rt_write_value_t write_value_task;
		npu_rt_notify_record_task_t notify_record_task;
		npu_rt_notify_wait_task_t notify_wait_task;
	} u;
} npu_rt_task_t;
