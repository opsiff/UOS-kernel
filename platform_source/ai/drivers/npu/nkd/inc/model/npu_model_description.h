...
typedef enum npu_model_param_type {
	MODEL_PARAM_PRIORITY = 0,
	MODEL_PARAM_EXECUTE_PID,
	MODEL_PARAM_PROFILING,
	MODEL_PARAM_EXECUTE_TIMES,
	MODEL_PARAM_REBOOT_TIMES,
	MODEL_PARAM_EXECUTE_ENABLE,
	MODEL_PARAM_UID,
	MODEL_PARAM_WORKMODE,
	MODEL_PARAM_SCHED_PATH,
	MODEL_PARAM_TYPE_MAX
} npu_model_param_type_t;
...
typedef struct npu_model_param_info {
	u32 model_id;
	u32 type;
	u32 value;
} npu_model_param_info_t;
...
typedef struct npu_model_desc_info {
	...
	uint8_t reserve1[6];
	uint64_t notify_info[NPU_MODEL_NOTIFY_NUM];
	uint8_t reserve2[32]; // 64 bytes cache line align
