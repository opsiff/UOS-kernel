...
int32_t vcodec_strncmp(const char *str_name, const char *dst_name, int32_t size)
{
	if (str_name && dst_name)
		return strncmp(str_name, dst_name, (size_t)size);
...
void *osal_fopen(const char *file_name, int32_t flags, int32_t mode)
{
	...
	filp = filp_open(file_name, flags, (umode_t)mode);
...
int32_t osal_fread(void *fp, char *buf, int32_t len)
{
	...
	oldfs = get_fs();
	set_fs(KERNEL_DS);
	readlen = (int32_t)vfs_read(filp, buf, (size_t)len, &filp->f_pos);
	set_fs(oldfs);
...
int32_t osal_fwrite(void *fp, const char *buf, int32_t len)
{
	...
	oldfs = get_fs();
	set_fs(KERNEL_DS);
	writelen = (int32_t)vfs_write(filp, buf, (size_t)len, &filp->f_pos);
	set_fs(oldfs);
