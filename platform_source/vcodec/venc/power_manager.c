...
bool pm_if_need_power_off(struct power_manager *pm)
{
	...
	do {
		...
		next_seq = 1;
	} while (need_seqretry(&pm->seq_lock, (int)seq));
	done_seqretry_irqrestore(&pm->seq_lock, (int)seq, flags);
...
int32_t pm_show_period_account(struct power_manager *pm, char *buf, int32_t size)
{
	...
	return sprintf_s(buf, (size_t)size,
...
int pm_show_total_account(struct power_manager *pm, char *buf, int32_t size)
{
	...
	return sprintf_s(buf, (size_t)size, "total busy time %llu ms, total power on time %llu ms, total time %llu ms, "
