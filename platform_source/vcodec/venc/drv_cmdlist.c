...
static void cmdlist_free_node(struct cmdlist_node *node)
{
	...
	offset = (uint32_t)((uint64_t)(uintptr_t)node->virt_addr - (uint64_t)(uintptr_t)node);
...
static struct cmdlist_head *cmdlist_get_head(uint32_t type)
{
	...
	for (i = 0; i < CMDLIST_MAX_HEAD_NUM; i++) {
		if (!test_bit((int32_t)i, &g_cmdlist_manager.bit_map)) {
...
static void cmdlist_cfg_smmu(int32_t core_id, uint32_t type)
{
...
static int32_t cmdlist_restore_channel_id(struct cmdlist_head *head)
{
	uint32_t i;
	...
	list_for_each_entry_safe(cmdlist_node, tmp_cmdlist_node, &head->list, list) {
		for (i = 0; i < MAX_INSTANCES_COUNT; i++) {
			if (venc_file_private_info[i].encode_done_info == cmdlist_node->buffer) {
				cmdlist_node->channel_info.id = venc_file_private_info[i].user_channel_id;
				VCODEC_DBG_VENC("User channel ID = %d", cmdlist_node->channel_info.id);
				return 0;
			}
		}
	}
...
static int32_t cmdlist_prepare_encode(struct encode_info *info, struct venc_fifo_buffer *buffer)
{
	...
	type = (info->is_protected == 1 ? CMDLIST_PROTECT : CMDLIST_NORMAL);
...
static void cmdlist_encode_done(struct venc_context *ctx)
{
	...
	if (cmdlist_restore_channel_id(head)) {
		spin_unlock_irqrestore(&ctx->lock, flag);
		return;
	}
