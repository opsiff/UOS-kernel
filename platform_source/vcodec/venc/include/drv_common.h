...
#define MAX_INSTANCES_COUNT 16

#define MAX_RING_BUFFER_SIZE (MAX_INSTANCES_COUNT * MAX_SUPPORT_CORE_NUM)
...
struct venc_file_private {
	uint32_t user_channel_id; // store channel id of use space
	bool venc_low_power_enable_flag;
	uint64_t venc_perf_loads;
	struct venc_fifo_buffer *encode_done_info;
	struct venc_fd_list fdlist;
};
