...
{
	uint32_t data0;
	data0 = readl(SOC_IPC_VRCE_MBX_DATA0_ADDR(VEDU_IPC_BASE, mbx_id));
...
void venc_ipc_recive_ack(int32_t mbx_id)
{
	...
	writel(IPC_KEY_UNLOCK, SOC_IPC_VRCE_IPC_LOCK_ADDR(VEDU_IPC_BASE));
	writel(BIT(ACPU_IPC_ID), SOC_IPC_VRCE_MBX_ICLR_ADDR(VEDU_IPC_BASE, mbx_id));
	VCODEC_INFO_VENC("ipc mbx%d reviced ack:0x%x", mbx_id, data0);
}

void venc_ipc_send_msg(int32_t mbx_id, struct ipc_message *msg)
{
	uint32_t value;
	// step 1
	writel(IPC_KEY_UNLOCK, SOC_IPC_VRCE_IPC_LOCK_ADDR(VEDU_IPC_BASE));
	value = readl(SOC_IPC_VRCE_MBX_SOURCE_ADDR(VEDU_IPC_BASE, mbx_id));
	if (value != 0)
...
irqreturn_t venc_drv_mcore_irq(int32_t irq, void *dev_id)
{
	...
	for (i = 0; i < MAX_SUPPORT_CORE_NUM; i++) {
		if (venc->ctx[i].irq_num.mcore_normal == (uint32_t)irq)
			break;
	}
	...
	status = readl(SOC_INTR_HUB_L2_INTR_STATUS_S_n_ADDR(venc->ctx[i].intr_hub_reg_base, n));
