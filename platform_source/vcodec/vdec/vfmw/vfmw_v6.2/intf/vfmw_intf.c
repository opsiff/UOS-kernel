...
#define WAIT_TASK_EXIT_TIMEOUT  80 // ms
#define WAIT_TVP_TASK_MIN_TIME  20
...
irqreturn_t vfmw_isr(int irq, void *id)
{
	...
	if (dec_dev_isr_state((uint16_t)dev_id) == DEC_OK)
		dec_dev_isr((uint16_t)dev_id);
exit:
...
static int vdec_tvp_task(void *data)
{
	...
	while (!kthread_should_stop()) {
		...
		ret = (int)wait_event_interruptible(g_vdec_tvp.waitq,
...
#ifdef VCODEC_TVP_SUPPORT
static inline void notify_and_wait_tvp_process(volatile bool *notify_flag)
{
	int32_t sleep_count = 0;
	*notify_flag = true;
	wake_up_interruptible(&g_vdec_tvp.waitq);
	OS_MSLEEP(WAIT_TVP_TASK_MIN_TIME);
	while (*notify_flag == true && sleep_count < (WAIT_TASK_EXIT_TIMEOUT / WAIT_TASK_EXIT_INER)) {
