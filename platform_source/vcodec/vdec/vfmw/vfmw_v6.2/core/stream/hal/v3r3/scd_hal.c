...
static void scd_hal_print_reg(const scd_reg *reg, uint8_t isr_before)
{
...
void scd_hal_read_reg(stm_dev *dev)
{
	...
	reg->roll_addr = (UADDR)((uint64_t)data << 32);
	...
	reg->next_addr = (UADDR)((uint64_t)data << 32);
...
void scd_hal_write_reg(scd_reg_ioctl *reg_cfg_info)
{
	...
	data = (dev->poll_irq_enable != 0) ? 1 : reg->int_mask;
