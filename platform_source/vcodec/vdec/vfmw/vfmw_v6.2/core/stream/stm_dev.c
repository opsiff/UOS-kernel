...
int32_t stm_dev_isr(void)
{
	...
	scd_hal_clear_int();
	OS_EVENT_GIVE((uint32_t)dev->event);
...
void stm_dev_deinit(void)
{
	...
	OS_EVENT_EXIT((uint32_t)dev->event);
...
static int32_t stm_dev_block_wait(stm_dev *dev)
{
	...
	return OS_EVENT_WAIT((uint32_t)dev->event, (uint64_t)scd_time_out);
}
