					...
					struct hisp_gpio_s *gpio)
{
	...
	ret = gpio_direction_output(bi->gpios[gpio->type].gpio, (int)gpio->level);
	if (ret == 0) {
				...
				struct hisp_sensor_info **ret_hsi)
{
	...
	list_for_each_entry(hsi, &g_sinfo, link) {
		...
		if ((strncmp(bi->name, name, strlen(name)) == 0) &&
...
void rpmsg_sensor_unregister(void *ptr_sensor)
{
	...
	hsi = container_of(ptr_sensor, struct hisp_sensor_info, sensor);
	if (hsi == NULL) {
