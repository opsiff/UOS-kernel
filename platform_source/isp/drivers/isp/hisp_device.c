...
#define ISP_DYN_RANGE_LOW         0x10000
#define ISP_DYN_RANGE_HIGH        0x1FFFF
...
extern u32 get_kernel_chip_version(void);
...
static int hisp_reg_space_unmap_for_isp(struct rproc *rproc);
#endif
...
int hisp_share_set_para(void)
{
	...
	pr_info("%s: rdr_enable = %d, rdr_enable_type = %u\n",
...
static int hisp_power_on_work(void *data)
{
	...
	ret = hisp_load_and_verify_image();
	if (ret != 0) {
		pr_err("%s failed: hisp_load_and_verify_image.%d\n", ret);
		return ret;
	}
...
int hisp_wait_rpmsg_completion(struct rproc *rproc)
{
	...
	type = hw_is_fpga_board();
	timeout = (type != 0) ? TIMEOUT_IS_FPGA_BOARD : TIMEOUT_IS_NOT_FPGA_BOARD;
...
#ifdef ISP_LINX_CPU_SUPPORT
	boot_pa = ioremap(BOOT_BASE, ISP_MEM_SIZE);
	ret = memcpy_s(boot_pa, ISP_MEM_SIZE, imgva, PAGE_SIZE);
	if (ret != 0) {
		hisp_err("%s: copy text to boot image fail\n", __func__);
		return ret;
	}
	__flush_dcache_area(boot_pa, ISP_MEM_SIZE);
#endif
	if (hisp_smmuv3_mode()) {
...
int hisp_rproc_disable(void)
{
...
#if (defined(DEBUG_HISP) && (!defined(HISP_FUZZ)))
...
#ifdef ISP_LINX_CPU_SUPPORT
	err = hisp_reg_space_unmap_for_isp(rproc);
	if (err < 0) {
		hisp_err("Failed : hisp_reg_space_unmap_for_isp.0x%x\n", err);
		return err;
	}
#endif
	...
	/* fix kernel power param excepiton deal */
...
unsigned int hisp_get_dyn_bootaddr(void)
{
	...
	if (hisp_dev->dyn_bootaddr < ISP_DYN_RANGE_LOW ||
		hisp_dev->dyn_bootaddr > ISP_DYN_RANGE_HIGH) {
		hisp_err("[%s] dyn_addr out of limit 0x%x\n", __func__, hisp_dev->dyn_bootaddr);
		return 0;
	}
...
#ifdef ISP_LINX_CPU_SUPPORT
static int hisp_reg_space_memmap_for_isp(struct rproc *rproc)
{
	struct hisp_boot_device *hisp_dev = &hisp_boot_dev;
	int ret = 0;
	...
	ret = hisp_reg_space_memmap(rproc);
	if (ret) {
		pr_err("%s, hisp_reg_space_memmap failed: %d\n", __func__, ret);
		return ret;
	}
	hisp_info("-\n");
...
static int hisp_reg_space_unmap_for_isp(struct rproc *rproc)
{
	...
	ret = hisp_reg_space_unmap(rproc);
	if (ret) {
		pr_err("%s, hisp_reg_space_unmap failed: %d\n", __func__, ret);
		return ret;
	}
	hisp_info("-\n");
...
#ifdef ISP_LINX_CPU_SUPPORT
	err = hisp_reg_space_memmap_for_isp(rproc);
	if (err < 0) {
		hisp_err("Failed : hisp_reg_space_memmap_for_isp.0x%x\n", err);
		atomic_set(&hisp_dev->rproc_enable_status, 0);
		return err;
	}
#endif
...
#ifdef HISP_FUZZ
	err = hisp_fuzz_kcov_shadow_mem_init();
	if (err != 0)
		hisp_err("Failed : kcov shadow mem init\n");
#endif
			...
			struct hisp_boot_device *dev)
{
	...
	dev->chip_flag = get_kernel_chip_version();
	pr_info("[%s]: dev->chip_flag = %d\n", __func__, dev->chip_flag);
#else
		...
		struct device_attribute *attr, char *buf)
{
	...
	s += snprintf_s(s, NONSEC_MAX_SIZE, NONSEC_MAX_SIZE-1,
			...
			((hisp_bypass_power_updn() != 0) ? "enable" : "disable"));
