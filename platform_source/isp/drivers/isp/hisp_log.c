...
static void hisplog_config_user_para(struct log_user_para *tmp)
{
	...
	if ((hisp_get_last_boot_state() == 0) && (hisp_ispcpu_is_powerup() != 0))
		...
		size_t count)
{
	...
	for (i = 0; i < ARRAY_SIZE(loglevel); i++) {
		...
		if (strncmp(buf, loglevel[i].info, (unsigned long)len) == 0) {
			flag = 1;
			p += 1;
			if (strncmp(p, loglevel[i].enable_cmd,
					(unsigned long)strlen(loglevel[i].enable_cmd)) == 0)
				dev->local_loglevel |= loglevel[i].level;
			else if (strncmp(p, loglevel[i].disable_cmd,
					(unsigned long)strlen(loglevel[i].disable_cmd)) == 0)
				dev->local_loglevel &= ~(loglevel[i].level);
			else
				flag = 0;
			break;
