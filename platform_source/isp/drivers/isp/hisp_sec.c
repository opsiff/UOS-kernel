					...
					unsigned int vaddr, unsigned int offset)
{
	...
	case A7VRING0:
	case A7VRING1:
		addr = hisp_rproc_get_vringmem_pa(rproc, (int)(HISP_VRING0 + etype - A7VRING0));
		break;
	default:
...
int hisp_secboot_prepare(void)
{
	...
	for (index = HISP_SEC_TEXT; index < HISP_SEC_BOOT_MAX_TYPE; index++) {
		...
		if (is_valid == SEC_TYPE_INVALID) {
			pr_info("[%s] ignore type[%u] sfd check, just continue\n", __func__, index);
			continue;
		}
...
int hisp_sec_device_enable(void)
{
	...
	if((hisp_ca_boot_mode() != 0) && (dev->boot_mem_ready_flag != 1)) {
		pr_err("[%s] Failed : boot_mem_ready_flag.%d\n",
			__func__, dev->boot_mem_ready_flag);
		return -EINVAL;
	}
...
int hisp_sec_probe(struct platform_device *pdev)
{
	...
	ret = hisp_mempool_init((unsigned int)dev->isp_iova_start, (unsigned int)dev->isp_iova_size);
	if (ret < 0) {
