...
static void hisp_rdr_system_err_work(struct work_struct *work)
{
	...
	list_for_each_entry_safe(entry, tmp, &dev->err_list, node) {
		if (entry->type == ISP_WDT_TIMEOUT)
			rdr_system_error((u32)entry->type, (u32)dev->wdt_irq, 0);
		else
...
static int hisp_rdr_wdt_init(struct hisp_rdr_device *dev)
{
	...
	ret = request_irq((unsigned int)dev->wdt_irq, hisp_rdr_wdt_irq_handler,
...
{
	pr_info("%s: enter.\n", __func__);
	free_irq((unsigned int)dev->wdt_irq, NULL);
	pr_info("%s: exit.\n", __func__);
}
		...
		char *p, int len)
{
	...
	for (i = 0; i < ARRAY_SIZE(rdrlevel); i++) {
		...
		if (strncmp(buf, rdrlevel[i].info, (size_t)len) == 0) {
			flag = 1;
			p += 1;
			if (strncmp(p, rdrlevel[i].enable_cmd,
					(size_t)strlen(rdrlevel[i].enable_cmd)) == 0)
				dev->offline_rdrlevel |= rdrlevel[i].level;
			else if (strncmp(p, (const char*)rdrlevel[i].disable_cmd,
					(size_t)strlen(rdrlevel[i].disable_cmd)) == 0)
				dev->offline_rdrlevel &= ~(rdrlevel[i].level);
			else
				flag = 0;
			break;
		...
		int len, struct hisp_shared_para *param)
{
	...
	if (strncmp(buf, "sirq", (int)strlen("sirq")) == 0) {
		p += 1;
		q = kzalloc((size_t)size, GFP_KERNEL);
		if (q == NULL)
			return flag;
		...
		if (strncmp(p, "enable", (int)strlen("enable")) == 0) {
			flag = 1;
			param->irq[irq_num] = 1;
			dev->irq[irq_num] = 1;
		} else if (strncmp(p, "disable", (int)strlen("disable")) == 0) {
			flag = 1;
			param->irq[irq_num] = 0;
			dev->irq[irq_num] = 0;
		} else {
