...
struct hippcomdev_s {
	...
	struct regulator *media1_supply;
...
static int hippsmmu_enable(struct hipp_common_s *drv)
{
	...
	if (dev->refs_smmu[drv->tbu_mode] == 0) {
		...
		ret = hipp_ffa_init_smmu_tbu();
#else
		ret = atfhipp_smmu_enable(drv->tbu_mode);
#endif
		if (ret != 0) {
			pr_err("[%s] Failed : ipp smmu enable, ret.%d\n",
				__func__, ret);
...
static int hippsmmu_disable(struct hipp_common_s *drv)
{
	...
	if (dev->refs_smmu[drv->tbu_mode] == 0) {
#ifdef CONFIG_HISP_FFA_SUPPORT
		ret = hipp_ffa_deinit_smmu_tbu();
#else
		ret = atfhipp_smmu_disable(drv->tbu_mode);
#endif
		if (ret != 0) {
			pr_err("[%s] Failed : ipp smmu disable.%d, mode.%d\n",
...
#ifdef CONFIG_HISP_FFA_SUPPORT
	ret = hipp_ffa_cfg_smmu_ssidv(swid, len, sid, ssid);
#else
	ret = atfhipp_smmu_smrx(swid, len, sid, ssid, drv->tbu_mode);
#endif
	if (ret != 0) {
		pr_err("[%s] Failed : ipp cfg smmu smrx.%s, ret.%d\n",
...
#ifdef CONFIG_HISP_FFA_SUPPORT
	ret = hipp_ffa_cfg_smmu_perf(swid, len);
#else
	ret = atfhipp_smmu_pref(swid, len);
#endif
	if (ret != 0) {
		pr_err("[%s] Failed : ipp cfg smmu pref.%s, ret.%d\n",
			__func__, drv->name, ret);
		drv->dump(drv);
		return ret;
	}
...
int __attribute__((weak)) hispcpe_pcie_powerup(void)
{
	return 0;
}

int __attribute__((weak)) hispcpe_pcie_powerdn(void)
{
	return 0;
}

int hipp_core_powerup(struct hippcomdev_s *dev, unsigned int type)
{
	return hispcpe_pcie_powerup();
}

int hipp_core_powerdn(struct hippcomdev_s *dev, unsigned int type)
{
	return hispcpe_pcie_powerdn();
}
#else
...
static int hipp_core_powerup(struct hippcomdev_s *dev, unsigned int type)
{
	...
	ret = regulator_enable(dev->media1_supply);
	if (ret != 0) {
		pr_err("[%s] Failed: media1.%d\n", __func__, ret);
		return -ENODEV;
	}
...
fail_tcu_poweron:
	ret = regulator_disable(dev->media1_supply);
	if (ret != 0)
		pr_err("[%s] Failed: media1 disable.%d\n", __func__, ret);
...
static int hipp_core_powerdn(struct hippcomdev_s *dev, unsigned int type)
{
	...
	ret = regulator_disable(dev->media1_supply);
	if (ret != 0)
		pr_err("[%s] Failed: vivobus regulator_enable.%d\n",
			   __func__, ret);
...
{
	int i = 0;
	const struct transition_info jpeg_trans_rate[HIPP_TRAN_NUM] = {
		{640000000, 214000000},
		{480000000, 160000000},
		{400000000, 200000000},
	};
...
int hippcore_pu(struct hipp_common_s *drv)
{
	int ret = 0;
	struct hippcomdev_s *dev = NULL;
...
int hippcore_pd(struct hipp_common_s *drv)
{
	struct hippcomdev_s *dev = NULL;
	int ret = 0;
	...
	struct dma_buf *modulebuf, int prot, unsigned long *out_size)
{
	...
	if ((drv == NULL) || out_size == NULL) {
		pr_err("[%s] Failed : drv or out_size is NULL\n", __func__);
		return 0;
	}
	...
	struct device *dev)
{
	...
	drv->dump = hipp_dump;
	drv->power_up = hippcore_pu;
	drv->power_dn = hippcore_pd;
	...
	struct hippcomdev_s *comdev)
{
	...
	comdev->media1_supply = devm_regulator_get(dev, "ipp-media1");
	if (IS_ERR_OR_NULL(comdev->media1_supply)) {
		pr_err("[%s] Failed : MEDIA1 devm_regulator_get.%pK\n",
			__func__, comdev->media1_supply);
		return -EINVAL;
	}
	...
	struct hippcomdev_s *comdev)
{
	...
	if (comdev->clock_num > IPP_CLK_MAX) {
		pr_err("[%s] Failed: clock-num invalid.%d\n",
				__func__, comdev->clock_num);
		comdev->clock_num = 0;
	}
