				...
				unsigned int check_type, unsigned int cmd)
{
	if (flp_port->channel_type & check_type)
		flp_generate_netlink_packet(flp_port, msg, len, (unsigned char)cmd);
}
...
/*
 * oper cellfence & wififence
 */
#if (defined CONFIG_SMART_FLP_CELLFENCE) || (defined CONFIG_SMART_FLP_WIFIFENCE)
/*lint -e603*/
static int flp_fence_oper_cmd(unsigned int cmd, const char *usr_buff, size_t len)
{
	struct read_info rd = {0};
	int ret;
...
static int flp_iomcu_template(struct flp_port_t *flp_port, unsigned long arg, unsigned int iomcu_sub_cmd)
{
	...
	if (iomcu_sub_cmd == SUB_CMD_FLP_CELLFENCE_OPT_REQ)
		ret = flp_fence_oper_cmd(iomcu_sub_cmd, usr, (unsigned long)cfence_hdr.len);
	else
		ret = inputhub_wrapper_pack_and_send_cmd(TAG_FLP, CMD_CMN_CONFIG_REQ, iomcu_sub_cmd,
 ...
 */
static int cellfence_operate(struct flp_port_t *flp_port, unsigned long arg)
{
...
static int cellfence_inject_result(struct flp_port_t *flp_port, unsigned long arg)
{
...
static int flp_shmem_send_async(unsigned short int cmd, const void __user *buf,
			unsigned int length)
{
...
static int flp_shmem_send_sync(unsigned short int cmd, const void __user *buf,
			unsigned int length)
{
	int ret;
...
static int flp_oper_wififence(struct flp_port_t *flp_port, unsigned long arg, unsigned int cmd)
{
	...
	if (cmd == SUB_CMD_FLP_REMOVE_WIFENCE_REQ) {
		ret = flp_fence_oper_cmd(cmd, (char *)cmd_data, (size_t)config.length);
		kfree(cmd_data);
		return ret;
	}
...
static bool flp_ioctl_result_is_negation(unsigned int cmd)
{
	switch (cmd) {
	case FLP_IOCTL_GEOFENCE_ADD:
	case FLP_IOCTL_GEOFENCE_REMOVE:
...
static long flp_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	if (ret < 0 && flp_ioctl_result_is_negation(cmd)) {
		pr_info("flp ioctl result negation enter\n");
		return (0 - ret); /* positive number is required by FlpHifenceSendCmd of hal */
	}
	return ret;
}
/*lint +e732*/
...
static int flp_mmap_impl(struct file *filp, struct vm_area_struct *vma)
{
	...
	offset = vma->vm_pgoff;
	vma->vm_pgoff = 0;
	pr_debug("[%s]:line[%d] enter, vm_start.0x%lx, size.0x%lx, vm_end.0x%lx, offset.0x%lx\n",
