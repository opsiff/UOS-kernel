...
{
	int ret, i;
	unsigned int ddr_loadmonitor_size;
	unsigned int ddr_loadmonitor_base_ap;
	...
	if (left == 0) {
		...
		ddr_loadmonitor_base_ap = dts_ddr_addr_get(DTSI_DDR_LOADMONITOR_PHYMEM_BASE_AP);
		ddr_loadmonitor_size = dts_ddr_size_get(DTSI_DDR_LOADMONITOR_PHYMEM_BASE_AP);
#else
		ddr_loadmonitor_base_ap = DDR_LOADMONITOR_PHYMEM_BASE_AP;
		ddr_loadmonitor_size = DDR_LOADMONITOR_PHYMEM_SIZE;
#endif
		if (dt_len > (size_t)ddr_loadmonitor_size)
			dt_len = (size_t)ddr_loadmonitor_size;
		if (p_data == NULL) {
			p_data = (char *)ioremap_wc((size_t)ddr_loadmonitor_base_ap, ddr_loadmonitor_size);
			if (p_data == NULL) {
