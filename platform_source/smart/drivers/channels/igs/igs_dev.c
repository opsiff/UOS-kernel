...
static unsigned int g_screen_state = 0;
...
static int npu_get_avs_volt(struct device_node *np)
{
	...
	if (of_property_read_u32_array(np, npu_avs_name, g_igs_avs_data,  (size_t)nr)) {
	...
	if (of_property_read_u32_array(np, margin_code_name, g_igs_pa_margin,  (size_t)nr)) {
		igs_log_err("%s: property:%s array err\n", __func__, margin_code_name);
		return -EINVAL;
	}
...
static int npu_get_profvolt_margin(struct device_node *np)
{
	...
	if (of_property_read_u32_array(np, npu_profvolt_margin, g_prof_margin, (size_t)nr)) {
		igs_log_err("%s: property:%s array err\n", __func__, npu_profvolt_margin);
		return -EINVAL;
	}
...
static int get_npu_svfd_code(struct device_node *np)
{
	...
	if (of_property_read_u32_array(np, svfd_code_name, g_igs_svfd_data, (size_t)nr)) {
	...
	if (of_property_read_u32_array(np, svfd_para_name, g_igs_svfd_para, (size_t)nr)) {
...
static int igs_notifier(struct notifier_block *nb, unsigned long action, void *data)
{
	...
	old_screen_state = g_screen_state;
	switch (blank) {
	case FB_BLANK_UNBLANK:
		mutex_lock(&g_igs_priv.wklock_mutex);
		/* screen on */
		igs_log_info("[%s] panel_id[%d] screen on\n", __func__, event->panel_id);
		g_screen_state |= (1 << (unsigned char)event->panel_id);
