...
void mailbox_destory_platform(void)
{
	...
	size_t count = ARRAY_SIZE(g_mailbox_local_proc_tbl);
	...
	wakeup_source_unregister(g_mb_lpwr_lock);
	g_mb_lpwr_lock = NULL;
}
...
int mailbox_init_platform(void)
{
	...
	g_mb_lpwr_lock = wakeup_source_register(NULL, "mailbox_low_power_wake_lock");
#else
	g_mb_lpwr_lock = wakeup_source_register("mailbox_low_power_wake_lock");
#endif
	if (g_mb_lpwr_lock == NULL)
		return -ENOMEM;
...
static uint32_t mailbox_ipc_process(struct mb_local_work *local_work,
