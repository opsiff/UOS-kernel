...
static uint32_t mailbox_init_mem(const struct mb_cfg *cfg)
{
	...
	void *usr_data)
{
	...
	dst_id = mailbox_get_dst_id(mail_code);

	AUDIO_LOGI("local_id = %u dst_id = %u", mb->local_id, dst_id);

	if (mb->local_id != dst_id)
		return mb_logerro_p1(MB_E_GUT_INVALID_TARGET_CPU, mail_code);

	mbuf = mailbox_get_channel_handle(mb, mail_code);
	if (mbuf == NULL)
		return mb_logerro_p1(MB_E_GUT_INVALID_CHANNEL_ID, mail_code);

	read_cb = &mbuf->read_cb[mailbox_get_use_id(mail_code)];
