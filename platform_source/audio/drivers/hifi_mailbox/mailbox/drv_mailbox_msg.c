...
static void mailbox_msg_receiver(void *mb_buf, void *handle, void *data)
{
...
static_t uint32_t mailbox_try_send_msg(uint32_t mailcode, const void *pdata,
	uint32_t length)
{
	...
	uint32_t *size)
{
	...
	if ((q == NULL) || (buff == NULL) || (size == NULL))
		return (uint32_t)mb_logerro_p1(MB_E_GUT_INPUT_PARAMETER, 0);

	if (q->size > *size)
		return (uint32_t)mb_logerro_p1(MB_E_GUT_USER_BUFFER_SIZE_TOO_SMALL, *size);

	/* Check the validity of the queue handle returned by the user */
	if ((q->length == 0) || ((uint32_t)(q->front - q->base) > q->length) ||
		((uint32_t)(q->rear - q->base) > q->length))
		return (uint32_t)mb_logerro_p1(MB_F_GUT_INVALID_USER_MAIL_HANDLE, q);
