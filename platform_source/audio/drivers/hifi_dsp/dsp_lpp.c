...
static int try_fill_user_buf(char __user *buf, size_t count)
{
	...
	if (len > count) {
		...
		kfree_sensitive(g_lpp_priv.to_user_buf);
#else
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5,10,0))
static const struct proc_ops dsp_proc_fops = {
	.proc_read = dsp_lpp_proc_read,
};

static const struct proc_ops dsp_pcm_read_fops = {
	.proc_read = dsp_lpp_pcm_read,
};

static const struct proc_ops dsp_volume_meter_fops = {
	.proc_write = dsp_lpp_wired_para_write,
};
#else
...
static void wakeup_source_deinit(void)
{
	wakeup_source_unregister(g_lpp_priv.wakelock);
	wakeup_source_unregister(g_lpp_priv.update_buff_wakelock);
}
...
int dsp_lpp_init(unsigned char *base_virt)
{
	...
	g_lpp_priv.wakelock = wakeup_source_register(NULL, "hifi_wakelock");
	if (g_lpp_priv.wakelock == NULL)
		return -ENOMEM;
	g_lpp_priv.update_buff_wakelock = wakeup_source_register(NULL, "update_buff_wakelock");
	if (g_lpp_priv.update_buff_wakelock == NULL) {
		wakeup_source_unregister(g_lpp_priv.wakelock);
		return -ENOMEM;
	}
#else
	g_lpp_priv.wakelock = wakeup_source_register("hifi_wakelock");
	if (g_lpp_priv.wakelock == NULL)
		return -ENOMEM;
	g_lpp_priv.update_buff_wakelock = wakeup_source_register("update_buff_wakelock");
	if (g_lpp_priv.update_buff_wakelock == NULL) {
		wakeup_source_unregister(g_lpp_priv.wakelock);
		return -ENOMEM;
	}
