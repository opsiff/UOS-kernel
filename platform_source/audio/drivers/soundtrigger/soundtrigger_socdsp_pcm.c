...
static void wakeup_buffer_init(uint32_t wakeup_mode, uint32_t *fast_buf, uint32_t *fast_size,
	uint32_t *elapsed_buf, uint32_t *elapsed_size)
{
...
static int soundtrtgger_socdsp_pcm_buffer_init(uint32_t wakeup_mode)
{
	...
	wakeup_buffer_init(wakeup_mode, &fast_buf, &fast_size, &elapsed_buf, &elapsed_size);
...
int32_t soundtrigger_socdsp_pcm_probe(struct device *dev)
{
	int32_t ret;
	uint32_t channel_num = ONEMIC_CHN;
	struct device_node *np = dev->of_node;

	ret = of_property_read_u32(np, "sochifi_wakeup_upload_chn_num", &channel_num);
	if (ret == 0)
