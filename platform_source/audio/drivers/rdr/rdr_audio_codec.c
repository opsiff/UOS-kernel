...
#define RECORD_FILE_LEN 64
#define MODID_AP_S_PANIC_AUDIO_CODEC 0x80000029 /* must be same with rdr_platform.h */
...
static unsigned char rdr_audio_get_codec_err_times(void)
{
	...
	mm_segment_t fs = 0;
	char file_name[RECORD_FILE_LEN] = {0};

	snprintf(file_name, sizeof(file_name) - 1, "%s%s", PATH_ROOT, "codecerr_times.log");
	fp = filp_open(file_name, O_RDONLY, 0660);
	if (IS_ERR(fp)) {
...
static void rdr_audio_set_codec_err_times(unsigned char times)
{
	...
	snprintf(file_name, sizeof(file_name) - 1, "%s%s", PATH_ROOT, "codecerr_times.log");
	fp = filp_open(file_name, O_CREAT | O_RDWR, 0660);
	if (IS_ERR(fp)) {
...
int rdr_audio_codec_init(void)
{
	...
	g_priv_des.rdr_wl = wakeup_source_register(NULL, "rdr_codechifi");
#else
	g_priv_des.rdr_wl = wakeup_source_register("rdr_codechifi");
#endif
	if (g_priv_des.rdr_wl == NULL) {
...
create_dump_thead_error:
	wakeup_source_unregister(g_priv_des.rdr_wl);
	g_priv_des.rdr_wl = NULL;
