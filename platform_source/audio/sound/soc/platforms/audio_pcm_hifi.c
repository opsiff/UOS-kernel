...
#define AUIDO_PCM_CYCLES_1S   32768    /* 32k clock */
...
static struct log_control g_log_control;
...
static bool is_pcm_device_using_thread(int pcm_device)
{
	if (priv_cust == NULL) {
	...
	if (priv_cust->dev_lowlatency == (uint32_t)pcm_device)
		return true;

	return false;
}

static bool is_log_limit(unsigned int curr_time)
{
	if (!priv_cust->is_cdc_device)
		return false;

	spin_lock(&(g_log_control.lock));
	if (g_log_control.logs_count != 0 && (curr_time > (g_log_control.last_print_time + AUIDO_PCM_CYCLES_1S)))
		g_log_control.logs_count = 0;

	if (g_log_control.logs_count >= 3) {
		spin_unlock(&(g_log_control.lock));
		return true;
	}

	g_log_control.logs_count++;
	g_log_control.last_print_time = curr_time;
	spin_unlock(&(g_log_control.lock));
	return false;
}

static void print_pcm_underrun(struct snd_pcm_substream *substream,
	snd_pcm_sframes_t avail, snd_pcm_sframes_t rt_period_size)
{
	...
	if (is_log_limit(curr_time))
		return;

	AUDIO_LOGW("underrun, device is %d, mode is %d, avail[%ld] < period size[%ld]",
		substream->pcm->device, substream->stream, avail, rt_period_size);
}
...
static int pcm_notify_set_buf(struct snd_pcm_substream *substream)
{
	...
	if (pcm_device == priv_cust->dev_mmap) {
		...
		if (mmap_buf == NULL || mmap_buf->buf_addr == NULL) {
			AUDIO_LOGE("mmap buf or buf_addr is null");
			return -ENOMEM;
		}
...
static void update_pcm_buff(struct snd_pcm_substream *substream, bool discard)
{
	...
	if (avail < rt_period_size) {
		print_pcm_underrun(substream, avail, rt_period_size);
		return;
	}
	...
	bool *should_schedule)
{
	...
	for (device = 0; device < priv_cust->dev_num; device++) {
		...
		for (stream = 0; stream < PCM_STREAM_MAX; stream++) {
			spin_lock_bh(&pcm_lock[device][stream]);
			if (pcm_stream_status[device][stream] == 0) {
				spin_unlock_bh(&pcm_lock[device][stream]);
				continue;
			}
...
static void pcm_thread_stop(struct snd_pcm *pcm)
{
	...
	struct audio_pcm_data *pdata = snd_soc_component_get_drvdata(component);
	struct pcm_thread_info *thread_info = &pdata->thread_info;
	bool is_stream_open = (pcm->streams[SNDRV_PCM_STREAM_PLAYBACK].substream_opened != 0) ||
		(pcm->streams[SNDRV_PCM_STREAM_CAPTURE].substream_opened != 0);
	...
	int pcm_device, int pcm_mode)
{
	...
	if (delay_time > (AUIDO_PCM_WORK_DELAY_1MS * time_delay) && !is_log_limit(curr_time))
		AUDIO_LOGW("%s, delay time stamp diff is %u, pcm device %d pcm mode %d",
			print_type, delay_time, pcm_device, pcm_mode);
}
	...
	struct mb_queue *mail_handle, unsigned int mail_len)
{
	...
	spin_lock_bh(&pcm_lock[pcm_device][pcm_mode]);
	substream = g_substream[pcm_device][pcm_mode];
	if (!is_valid_substream(substream)) {
		spin_unlock_bh(&pcm_lock[pcm_device][pcm_mode]);
	...
	case HI_CHN_MSG_PCM_PERIOD_ELAPSED:
		print_pcm_timeout(mail_buf.msg_timestamp,
			"ap respond elapsed msg timeout", RESPOND_MAX_TIME, pcm_device, pcm_mode);
		start_time = (unsigned int)mailbox_get_timestamp();
		pcm_isr_handle(substream, mail_buf.frame_num, (bool)mail_buf.discard);
		print_pcm_timeout(start_time, "ap process elapsed msg timeout", PROCESS_MAX_TIME, pcm_device, pcm_mode);
		break;
	...
	const struct audio_pcm_runtime_data *prtd, struct hifi_chn_pcm_trigger msg_body)
{
	...
	if (prtd->using_thread_flag && (cmd == SNDRV_PCM_TRIGGER_START)) {
		...
		AUDIO_LOGI("lowlatency check frame thread wake up %s", ret != 0 ? "success" : "fail");
	...
	struct snd_pcm_hw_params *params)
{
	int ret = 0;
	struct audio_pcm_runtime_data *prtd = substream->runtime->private_data;
	uint32_t bytes = params_buffer_bytes(params);
	...
	if (device == priv_cust->dev_mmap) {
		...
		if (ret != 0) {
			AUDIO_LOGE("alloc mmap share buffer size %u fail, ret %d", bytes, ret);
...
static int audio_pcm_open(struct snd_pcm_substream *substream)
{
	...
	AUDIO_LOGI("pcm open, device %d, mode %d", pcm_device, pcm_mode);
	...
	if (!hw_info) {
		AUDIO_LOGW("pcm device %d, pcm mode %d hw_info is not found, set default para",
			pcm_device, pcm_mode);
		if (priv_cust->is_cdc_device)
			return -EINVAL;
		else
	...
	spin_lock_init(&(g_log_control.lock));
...
static int audio_pcm_close(struct snd_pcm_substream *substream)
{
	...
	spin_lock_bh(&pcm_lock[device][substream->stream]);
	pcm_stream_status[device][substream->stream] = 0;
	g_substream[device][substream->stream] = NULL;
	spin_unlock_bh(&pcm_lock[device][substream->stream]);
