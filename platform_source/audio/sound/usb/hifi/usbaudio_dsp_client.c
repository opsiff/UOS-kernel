...
static bool need_blue_microphones_workaround(const struct audioformat *fp,
	...
	struct usb_host_interface *alts, unsigned int num)
{
	...
	int iface_no, struct list_head *fp_list)
{
	...
	for (i = 0; i < num; i++) {
		...
		/* check direction */
		stream = ((get_endpoint(alts, 0)->bEndpointAddress & USB_DIR_IN) != 0) ?
...
static bool is_customized_headset(u32 usb_id)
{
	unsigned int i;
...
static bool is_forced_match_hifi_headset(u32 usb_id)
{
	unsigned int i;
...
static_t bool hifi_samplerate_compare(struct audioformat *fp, struct usbaudio_pcm_cfg *pcm_cfg)
{
	int i;
	bool found = false;
...
static_t bool hifi_formats_compare(struct audioformat *fp, struct usbaudio_pcm_cfg *pcm_cfg)
{
	unsigned int i;
	bool found = false;
...
static_t bool hifi_channel_compare(struct audioformat *fp, struct usbaudio_pcm_cfg *pcm_cfg)
{
	unsigned int i;
	bool found = false;
...
static_t bool hifi_usbaudio_protocal_compare(struct usb_device *dev, struct audioformat *fp, u32 usb_id)
{
	struct usb_host_interface *alts = NULL;
	struct usb_interface_descriptor *altsd = NULL;
	struct usb_interface *iface = NULL;
...
static unsigned int pcm_fomat_convert(unsigned long long format)
{
	unsigned int pcm_format = SNDRV_PCM_FORMAT_S16_LE;
...
static_t bool is_headphone(struct list_head *fp_list)
{
	struct audioformat *fp = NULL;
	bool is_hp = true;
...
static bool match_hifi_format(struct usbaudio_dsp *pdata, struct list_head *fp_list,
	struct usbaudio_pcms *pcms, u32 usb_id)
{
	...
	struct usb_device *dev = pdata->usb_dev;
	...
	if (pdata->only_support_capture_microphone) {
		AUDIO_LOGI("play: %d, capture :%d", playback_match, capture_match);
	...
	struct usb_host_interface *ctrl_intf, int ctrlif, struct usbaudio_pcms *pcms)
{
	...
	return match_hifi_format(pdata, &fp_list, pcms, usb_id);
}

static bool is_usbaudio_device(struct usbaudio_dsp *pdata)
{
	...
	int nintf;
	int i;
	struct usb_device *udev = pdata->usb_dev;
	...
	if (pdata->only_support_capture_microphone) {
		if ((audio_intf_num == 1) && (other_intf_num == 0)) {
...
static bool is_forced_match_arm_usbid(u32 usb_id)
{
	unsigned int i;
	...
	struct usb_host_interface *ctrl_intf, int ctrlif, struct usbaudio_pcms *pcms)
{
	...
	if (is_usbaudio_device(pdata) && !is_forced_match_arm_usbid(usb_id) &&
		is_match_hifi_format(pdata, ctrl_intf, ctrlif, pcms) &&
		!suport_usbvoice_status) {
		if (usb_using_hifi_usb(pdata->usb_dev)) {
			/* set volume for audio headset */
			set_customsized_headset_volume(pdata->usb_dev, usb_id);
			pdata->usbaudio_connect_state = USBAUDIO_DSP;
			AUDIO_LOGI("using hifiusb start, other will force match armusb");
			return false;
		}
		...
		if (need_reset_vbus(usb_id)) {
			if (usb_start_hifi_usb_reset_power(pdata->usb_dev) == 0)
				return true;
		} else if (usb_start_hifi_usb(pdata->usb_dev) == 0) {
			return true;
		} else {
	...
	if (usb_using_hifi_usb(pdata->usb_dev)) {
		...
		usb_stop_hifi_usb(pdata->usb_dev);
		return true;
	}
...
bool send_usbaudioinfo2hifi(struct snd_usb_audio *chip, struct usbaudio_pcms *pcms)
{
	...
	if (usb_using_hifi_usb(chip->dev)) {
		...
		for (i = 0; i < USBAUDIO_PCM_NUM; i++) {
			AUDIO_LOGI("begin");
			AUDIO_LOGI("formats %x channels %u fmt_type %u frame_size %u iface %d altsetting %u altset_idx %u \
			attributes %u endpoint %u ep_attr %u datainterval %u protocol %u maxpacksize %u rates %u clock %u",
			pcms->fmts[i].formats,
			...
			AUDIO_LOGI("bLength %u bDescriptorType %u bInterfaceNumber %u bAlternateSetting %u bNumEndpoints %u bInterfaceClass  %u bInterfaceSubClass  %u bInterfaceProtocol %u iInterface %u",
			...
			AUDIO_LOGI("bLength %u bDescriptorType %u bEndpointAddress %u bmAttributes %u wMaxPacketSize %u bInterval %u bRefresh %u bSynchAddress %u",
