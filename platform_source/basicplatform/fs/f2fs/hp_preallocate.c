...
static int f2fs_curseg_set(struct f2fs_sb_info *sbi, int type, unsigned int segno,
...
static void f2fs_curseg_restore(struct f2fs_sb_info *sbi, int type,
			 struct curseg_info *backupseg)
{
	struct curseg_info *curseg = CURSEG_I(sbi, type);
...
static unsigned int least_secnum_needed(struct f2fs_sb_info *sbi, unsigned int needed)
{
	unsigned int node_secs = (unsigned int)get_blocktype_secs(sbi, F2FS_DIRTY_NODES);
	unsigned int dent_secs = (unsigned int)get_blocktype_secs(sbi, F2FS_DIRTY_DENTS);
	unsigned int imeta_secs = (unsigned int)get_blocktype_secs(sbi, F2FS_DIRTY_IMETA);

	return (node_secs + dent_secs + imeta_secs + dent_secs +
		(unsigned int)reserved_sections(sbi) + needed);
}

static unsigned int f2fs_find_continuous_secs(struct f2fs_sb_info *sbi, int type,
				       unsigned int secnum)
{
...
void f2fs_hp_set_secs(struct f2fs_sb_info *sbi, int type,
			unsigned int secno, unsigned int secnum)
{
	unsigned int i;
...
static int f2fs_hp_file_expand(struct inode *inode, unsigned long long len)
{
	...
	secnum = (unsigned int)(pg_end / BLKS_PER_SEC(sbi));
	...
	target_secno = f2fs_find_continuous_secs(sbi, CURSEG_COLD_DATA, secnum);
	...
	if (err) {
		for (i = target_segno; i < target_segno + pg_end / sbi->blocks_per_seg; i++)
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5,10,0))
	...
	if (err) {
		pr_err_ratelimited("[HP] map blocks err:%d\n", err);
		for (i = target_segno; i < target_segno + pg_end / sbi->blocks_per_seg; i++)
			__set_test_and_free(sbi, i, false);
	}
