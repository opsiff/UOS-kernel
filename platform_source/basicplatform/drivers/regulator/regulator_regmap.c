...
#define PMIC_DUAL_VSET_LOW 0
#define PMIC_DUAL_VSET_HIGH 1
...
struct regulator_register_info {
	...
	u32 vset_reg;
	u32 vset_mask;
	u32 dual_vset_select_reg;
	u32 dual_vset_select_mask;
};
...
static void dual_vset_select_high(struct regmap_regulator *sreg,
	struct regmap *regmap)
{
	int ret;

	ret = regmap_write_bits(regmap,
		sreg->register_info.dual_vset_select_reg,
		sreg->register_info.dual_vset_select_mask,
		sreg->register_info.dual_vset_select_mask);
	if (ret < 0)
		pr_err("map_write_bits high dual_vset_select_reg 0x%x failed\n",
			sreg->register_info.dual_vset_select_reg);
}

static void dual_vset_select_low(struct regmap_regulator *sreg,
	struct regmap *regmap)
{
	int ret;
	...
	ret = regmap_write_bits(regmap,
		...
		0);
	if (ret < 0)
		pr_err("map_write_bits low dual_vset_select_reg 0x%x failed\n",
			sreg->register_info.dual_vset_select_reg);
}
...
static int regmap_dt_parse_dual_vset_select_high(struct regmap_regulator *sreg,
	struct platform_device *pdev)
{
	...
	if (!of_property_read_u32(np, "pmic-n-voltages-high", &sreg->rdesc.n_voltages)) {
		v_table = devm_kzalloc(
			dev, sizeof(unsigned int) * sreg->rdesc.n_voltages, GFP_KERNEL);
		if (!v_table) {
			ret = -ENOMEM;
		...
		if (!of_property_read_u32_array(np, "pmic-vset-table-high",
			v_table, sreg->rdesc.n_voltages))
			sreg->rdesc.volt_table = v_table;
	}
	return 0;
...
static int regmap_dt_parse_dual_vset_select(struct regmap_regulator *sreg,
	struct platform_device *pdev)
{
	struct device *dev = &pdev->dev;
	struct device_node *np = dev->of_node;
	...
	ret = of_property_read_u32(np, "pmic_dual_vset_level", &level);
	if (ret)
		return 0;

	dev_info(dev, "pmic_dual_vset_level is %d\n", level);

	ret = of_property_read_u32_array(np, "pmic_dual_vset_crtl",
			register_info, 2);
	if (ret) {
		dev_err(dev, "get array pmic_dual_vset_crtl fail\n");
		return ret;
	}
	sreg->register_info.dual_vset_select_reg = register_info[0];
	sreg->register_info.dual_vset_select_mask = register_info[1];
	...
	if (level == PMIC_DUAL_VSET_LOW) {
		dual_vset_select_low(sreg, reg_map);
		return 0;
	}

	dual_vset_select_high(sreg, reg_map);

	ret = regmap_dt_parse_dual_vset_select_high(sreg, pdev);
	if (ret) {
		dev_err(dev, "regmap_dt_parse_dual_vset_select_high fail\n");
		return ret;
	}
	return 0;
}
				...
				struct platform_device *pdev)
{
	...
	ret = regmap_dt_parse_dual_vset_select(sreg, pdev);
	if (ret) {
		dev_err(dev, "failure in regmap_dt_dual_vset_select\n");
		devm_kfree(dev, v_table);
		return ret;
	}
	return ret;
