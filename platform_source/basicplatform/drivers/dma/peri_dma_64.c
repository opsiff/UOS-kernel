...
static irqreturn_t peri_dma_int_handler(int irq, void *dev_id)
{
	...
	if (d->dma_irq_flag == 0) {
		writel(tc1, d->base + INT_TC1_RAW);
		writel(err1, d->base + INT_ERR1_RAW);
		writel(err2, d->base + INT_ERR2_RAW);
		dev_info(d->slave.dev, "clear abnormal dma irq\n");
		return IRQ_HANDLED;
	}
...
#ifdef CONFIG_DMA_SMMU_ENABLE
	peri_dma_smmu_init(op, d);
#endif
...
#ifdef CONFIG_DMA_SMMU_ENABLE
	struct platform_device *op = container_of(dev, struct platform_device, dev);
	dma_smmu_suspend(op, d);
#endif
	dev_info(d->slave.dev, "%s: suspend -\n", __func__);
	return 0;
}
