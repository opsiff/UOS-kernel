...
static inline void init_hpb_radix_tree(void)
{
	...
	node_tree.xa_flags = GFP_ATOMIC | __GFP_NOWARN;
	node_tree.xa_head = NULL;
#else
...
static void fill_rb_cdb_info(struct ufs_hba *hba, u8 *cdb, int region_id)
{
...
static void ufshpb_active_hpb_entry(struct ufs_hba *hba,
					   struct ndlist_data *ndlist_data)
{
	struct subregion_node *node = NULL;
	int err;
			    ...
			    u64 ppn)
{
	...
	/*
	 * ufshpb_ppn_prep always regard request as read10.
	 * transfer_len should be retrieved from request if it's read16.
	 */
	if (unlikely(lrbp->cmd->cmnd[0] == READ_16))
		transfer_len = sectors_to_logical(lrbp->cmd->device,
					  blk_rq_sectors(lrbp->cmd->request));
	else
...
static void fill_hpb_unit_desc(struct ufs_hba *hba, u8 *unit_desc_buf)
{
...
static void fill_hpb_info(struct ufs_hba *hba, u8 *device_desc_buf,
...
static int control_init_per_lun(struct ufs_hba *hba, unsigned int lun)
{
	struct ufshpb_lu_control *hpb_lu_control = NULL;
	int ret;
...
static int hpb_reset_process(struct ufs_hba *hba, bool *hpb_after_reset)
{
	int ret;
	int retry_count = 0;
	...
	*hpb_after_reset = true;
	...
	while (*hpb_after_reset) {
		...
		ufshcd_query_flag_retry(hba, UPIU_QUERY_OPCODE_READ_FLAG,
					QUERY_FLAG_IDN_HPB_RESET, 0, hpb_after_reset);
...
static inline void empty_node_update_process(
	struct subregion_node *subregion_node,
	struct ndlist_data *ndlist_data,
	u64 subregion)
{
...
static void new_node_update_process(
	struct subregion_node *subregion_node,
	struct ndlist_data *ndlist_data,
	u64 subregion)
{
