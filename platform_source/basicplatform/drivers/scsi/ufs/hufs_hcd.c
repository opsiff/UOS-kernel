...
void ufshcd_hufs_host_memory_configure(struct ufs_hba *hba)
{
	...
	if (is_dfa_enabled(hba))
		return dfa_host_memory_configure(hba);
#endif
	response_offset =
...
int ufshcd_hufs_uic_change_pwr_mode(struct ufs_hba *hba, u8 mode)
{
	...
	ufshcd_switch_latency_check(hba, UFS_DSS_LATENCY_SCENARIO_REG_DONE, 0, 0);
#endif
retry:
...
out:
#ifdef CONFIG_SCSI_UFS_GEAR_CTRL
	ufshcd_switch_latency_check(hba, UFS_DSS_LATENCY_SCENARIO_INT_BACK, 0, 0);
#endif
	return ret;
}
...
irqreturn_t ufshcd_ufs_intr_hub_intr(int ufs_intr_hub_irq, void *__hba)
{
	...
	if (host->host_id == 1) {
		intr_stat = readl(host->hsdt1_intr_hub +
