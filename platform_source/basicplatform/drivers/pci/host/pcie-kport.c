				     ...
				     struct platform_device *pdev)
{
	...
	dtsinfo->pcie_share_clock_support = of_property_read_bool(np, "pcie_share_clock_support");
#endif
}
...
int pcie_kport_register_event(struct pcie_kport_register_event *reg)
{
	...
	mutex_lock(&pcie->event_reg_lock);
	pcie->event_reg = reg;
	mutex_unlock(&pcie->event_reg_lock);
...
static int pcie_probe(struct platform_device *pdev)
{
	...
	mutex_init(&pcie->event_reg_lock);
...
#ifdef CONFIG_PCIE_KPORT_JUN
	pcie_register_dsm_client(pcie->dtsinfo.ep_device_type);
#endif
