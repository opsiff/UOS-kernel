...
static DEFINE_MUTEX(tcu_init_lock);
...
static int pcie_refclk_on(struct pcie_kport *pcie)
{
	...
	if (pcie->dtsinfo.pcie_share_clock_support && (atomic_read(&pcie->is_clock_on) != 0))
		return 0;
#endif
...
#ifdef CONFIG_HUAWEI_EXTERNAL_CAMERA
	if (pcie->dtsinfo.pcie_share_clock_support)
		atomic_set(&pcie->is_clock_on, 1);
#endif
...
static int pcie_smmu_init(struct pcie_kport *pcie, u32 enable)
{
	...
	mutex_lock(&tcu_init_lock);
	ret = pcie_smmu_cfg(pcie, enable);
	mutex_unlock(&tcu_init_lock);
			...
			int fixed, enum pcie_cof_preset_index p_index)
{
	...
	if (pcie->phy->version == PCIE_PHY_VERSION_1) {
...
static void pcie_ctrl_ffe_cfg(struct pcie_kport *pcie)
{
	...
	if (pcie->phy->version == PCIE_PHY_VERSION_1) {
		...
		pcie_config_mac_ffe_map_test(pcie->rc_id, FIXED, P_4);
	}
}
...
HSDTSCTRL_BASE_UNMAP:
	iounmap(pcie->hsdtsctrl_base);
	return -1;
}
