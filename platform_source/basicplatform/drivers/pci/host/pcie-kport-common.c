...
/*
 * pcie_kport_set_devid - set device id.
 */
void pcie_kport_set_devid(struct pcie_kport *pcie)
{
	u32 val = pcie_read_dbi(pcie->pci, pcie->pci->dbi_base, 0, 4); /* dbi size is 4 */

#ifdef CONFIG_PCIE_KPORT_TEST
	/* for product safety */
#ifdef CONFIG_PCIE_KPORT_BURB
	val = ( val & PCI_VENDOR_ID_MASK ) | (PCIE_DENVER_DEVICE_ID << 16);
#else
	val = ( val & PCI_VENDOR_ID_MASK ) | (PCIE_DEFAULT_DEVICE_ID << 16);
#endif
#endif
...
void pcie_ep_clear_intr(struct pcie_kport *pcie)
{
	...
	val |= PCIE_PERST_IN_INTR_MASK;
	pcie_apb_ctrl_writel(pcie, val, SOC_PCIECTRL_CTRL6_ADDR);
}

/* check perst_in_n, whether to reinitialize */
void pcie_ep_check_perst_in(struct pcie_kport *pcie)
{
	u32 val;

	val = pcie_apb_ctrl_readl(pcie, SOC_PCIECTRL_STATE4_ADDR);
	if (get_bit(val, PCIE_PERST_IN_MASK_BIT) == 0 && get_bit(val, PCIE_PAD_PERST_IN_BIT) == 0) {
		pcie_power_ctrl(pcie, RC_POWER_OFF);
		pcie_power_ctrl(pcie, RC_POWER_ON);
	}
}
