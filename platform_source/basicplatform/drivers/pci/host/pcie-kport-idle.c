...
static void pcie_idle_sleep_init(struct pcie_idle_sleep *idle_sleep)
{
	...
	pcie = get_pcie_by_id(idle_sleep->host_id);
	if (!pcie) {
		PCIE_PR_E("%s: pcie is null, return\n", __func__);
		return;
	}
#endif
	...
	if (pcie->dtsinfo.pcie_share_clock_support) {
		enable_irq(idle_sleep->irq[IRQ_IDLE_WAKE]);
		enable_irq(idle_sleep->irq[IRQ_IDLE_TIMEOUT]);
	}
#endif
...
#ifdef CONFIG_HUAWEI_EXTERNAL_CAMERA
	if (pcie->dtsinfo.pcie_share_clock_support) {
		disable_irq(idle_sleep->irq[IRQ_IDLE_WAKE]);
		disable_irq(idle_sleep->irq[IRQ_IDLE_TIMEOUT]);
	}
#endif
	/* clear pcie idle timeout int */
	val = idle_reg_readl(idle_sleep, idle_sleep->idle_timeout_clr_addr);
