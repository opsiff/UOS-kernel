...
#include <platform_include/basicplatform/linux/mmc/sdhci_zodiac_mmc_aquila.h>
...
static void sdhci_zodiac_clock_enable(struct device *dev, struct sdhci_zodiac_data *sdhci_zodiac)
{
	...
	if (!IS_ERR(sdhci_zodiac->tiu_clk)) {
		if (clk_prepare_enable(sdhci_zodiac->tiu_clk))
			dev_err(dev, "Unable to enable tiu clock.\n");
	}
}
...
static void sdhci_zodiac_clock_disable(struct sdhci_zodiac_data *sdhci_zodiac)
{
	...
	if (!IS_ERR(sdhci_zodiac->tiu_clk))
		clk_disable_unprepare(sdhci_zodiac->tiu_clk);
}

void sdhci_zodiac_gpio_value_set(int value, int gpio_num)
{
	int err;
...
static void sdhci_zodiac_power_up(struct sdhci_host *host)
{
	...
	if (!IS_ERR(mmc->supply.vqmmc)) {
		if (sdhci_zodiac->mux_sdsim && !sdhci_zodiac->lever_shift) {
...
static void sdhci_zodiac_set_power(struct sdhci_host *host, unsigned char mode, unsigned short vdd)
{
	...
	case MMC_POWER_UP:
		...
		if (sdhci_zodiac->tbu_sd_bypass)
			sdhci_zodiac_qos_config(host); /* HSDT1QIC_QCP */
...
static int sdhci_zodiac_suspend(struct device *dev)
{
	...
	if (sdhci_zodiac->tbu_sd_bypass) {
		sdhci_zodiac_qic_idle_config(host, ENABLE);
		sdhci_zodiac_tbu_config(host, DISABLE);
	}
...
static int sdhci_zodiac_resume(struct device *dev)
{
	...
	if (sdhci_zodiac->tbu_sd_bypass) {
		sdhci_zodiac_tbu_config(host, ENABLE);
		sdhci_zodiac_qic_idle_config(host, DISABLE);
	}
...
static void sdhci_zodiac_populate_dt(struct platform_device *pdev)
{
	...
	if (of_find_property(np, "tbu_sd_bypass", NULL)) {
		dev_info(&pdev->dev, "%s %s tbu_sd_bypass found\n", dev_name(sdhci_zodiac->dev), __func__);
		sdhci_zodiac->tbu_sd_bypass = ENABLE;
	}
...
static int sdhci_zodiac_clock_init(struct sdhci_host *host, struct platform_device *pdev, struct sdhci_zodiac_data *sdhci_zodiac)
{
	...
	if (sdhci_zodiac->tbu_sd_bypass) {
		sdhci_zodiac->tiu_clk = devm_clk_get(&pdev->dev, "tiu");
		if (IS_ERR(sdhci_zodiac->tiu_clk)) {
			dev_err(&pdev->dev, "tiu clock not found.\n");
			ret = PTR_ERR(sdhci_zodiac->tiu_clk);
			return ret;
		}
	}
...
static int sdhci_zodiac_probe(struct platform_device *pdev)
{
	...
	if (sdhci_zodiac->tbu_sd_bypass) {
		sdhci_zodiac_tbu_config(host, ENABLE); /* TBU_SD bypass */
		sdhci_zodiac_qic_idle_config(host, DISABLE);
	}
	sdhci_zodiac_hardware_reset(host);
	sdhci_zodiac_hardware_disreset(host);
...
static const struct of_device_id sdhci_zodiac_of_match[] = {
	...
	{.compatible = "hisilicon,antlia-sdhci"},
	{.compatible = "hisilicon,aquila-sdhci"},
	{}
};
