...
int ipc_rproc_xfer_sync_no_ack_npu(rproc_id_t rproc_id, rproc_msg_t *msg,
	rproc_msg_len_t len)
{
	struct ipc_rproc_info *rproc = NULL;
	struct ipc_mbox *mbox = NULL;
	...
	ret = ipc_mbox_msg_send_sync_without_ack(mbox, msg, len, ack_type);
	if (ret)
		rproc_pr_err("fail to sync send, id %d", rproc_id);
...
EXPORT_SYMBOL(ipc_rproc_xfer_sync_no_ack_npu);
#endif
