...
static int ipc_mbox_task_send_sync_without_ack(
	struct ipc_mbox_device *mdev, struct ipc_mbox_task *tx_task)
{
	unsigned long tx_timeout;
	unsigned long timeout;
...
#ifdef CONFIG_NPU_PLATFORM_DRIVER
int ipc_mbox_msg_send_sync_without_ack(struct ipc_mbox *mbox, mbox_msg_t *tx_buffer,
	mbox_msg_len_t tx_buffer_len, int need_auto_ack)
{
	struct ipc_mbox_device *mdev = NULL;
	struct ipc_mbox_task tx_task;
	size_t src_len, copy_len;
	...
	/* send */
	mutex_lock(&mdev->dev_lock);
	ret = ipc_mbox_task_send_sync_without_ack(mdev, &tx_task);
	if (ret)
		mbox_pr_err("send fail");
