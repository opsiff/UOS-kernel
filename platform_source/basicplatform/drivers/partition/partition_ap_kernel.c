...
static const struct partition *get_ptable_src(void)
{
	...
	else if (dev_type == BOOT_DEVICE_NOR_FLASH)
		return partition_table_ufs;
...
/*
 * Get partition entry in total partitions(all lu), not the current LU
 */
int flash_get_ptn_entry(const char *ptn_name, struct partition *entry)
{
	int n;
	const struct partition *ptable_src = NULL;
	char part_nm_tmp[MAX_PARTITION_NAME_LENGTH] = {0};
	...
	if (!strcmp(PART_XLOADER, ptn_name)) {
		pr_err("%s: This is boot partition\n", __func__);
		return PTN_ERROR;
	}
	...
	/* normal partition */
	for (n = 0; n < ptn_num; n++) {
		if (!strcmp((ptable_src + n)->name, ptn_name)) {
			entry->start = ((ptable_src + n)->start) * 1024;
			entry->length = ((ptable_src + n)->length) * 1024;
			entry->flags = (ptable_src + n)->flags;
			return 0;
		}
	}

	if (transform_ptn_name(ptn_name, part_nm_tmp, sizeof(part_nm_tmp))) {
		pr_err("%s:%d transform ptn name fail!\n", __func__, __LINE__);
		return PTN_ERROR;
	}
	...
	for (n = 0; n < ptn_num; n++) {
		if (!strcmp((ptable_src + n)->name, part_nm_tmp)) {
			entry->start = (ptable_src + n)->start;
			entry->length = (ptable_src + n)->length;
			entry->flags = (ptable_src + n)->flags;
			return 0;
		}
	}
