...
static int _combophy_v2_init(struct phy_priv *priv)
{
	...
	/* DP use 4 lane, disable usb3 */
	if (priv->phy_mode == TCPC_DP && !priv->is_hiusbc)
			     ...
			     struct phy_priv *priv)
{
	...
	if (IS_ERR(priv->usb_tca))
		return (int)PTR_ERR(priv->usb_tca);
...
static int get_clk_resource(struct phy_priv *priv)
{
	...
	if (priv->is_combophy_v2 && !priv->is_hiusbc) {
...
static int get_dts_resource(struct platform_device *pdev, struct phy_priv *priv)
{
	...
	priv->is_hiusbc = of_property_read_bool(np, "is-hiusbc");
	pr_info("is_hiusbc %s\n", priv->is_hiusbc ? "true" : "false");
...
static int create_chip_combophy(struct phy_priv *priv)
{
...
#ifdef CONFIG_USB_HIUSB
	phy_provider = devm_of_phy_provider_register(priv->dev, of_phy_simple_xlate);
	if (IS_ERR(phy_provider))
		pr_err("phy provider register failed\n");
#else
	/* hiusb no need combophy_config_ops */
	...
	phy_provider = devm_of_phy_provider_register(priv->dev,
						     combophy_of_xlate);
	if (IS_ERR(phy_provider))
		return (int)PTR_ERR(phy_provider);
#endif
	return 0;
}
