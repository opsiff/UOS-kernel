...
static int uart_v500_startup(struct uart_port *port)
{
	...
	if (uap->uart_sleep_use_doze)
		vote_disable_doze(uap->doze_id);
	...
	struct ktermios *termios, struct ktermios *old)
{
	...
	quot |= quotn;
	if ((uap->port.line == 0) && (uap->bbic_flag))
		dev_err(port->dev, "%s set baud %u\n", __func__, baud);
...
static void uart_v500_get_doze_id(struct platform_device *pdev,
	struct uart_v500_port *uap)
{
	if (of_property_read_u32(pdev->dev.of_node, "use-doze-to-sleep",
		&uap->uart_sleep_use_doze))
		uap->uart_sleep_use_doze = 0;

	if (uap->uart_sleep_use_doze) {
		if (of_property_read_u32(pdev->dev.of_node, "doze-id",
			&uap->doze_id)) {
			pr_err("%s: uart doze id get err\n", __func__);
			uap->uart_sleep_use_doze = 0;
		}
	}
}
...
static int uart_v500_probe(struct platform_device *pdev)
{
	...
	if (of_property_read_u32(pdev->dev.of_node, "cts-flag",
		&uap->cts_flag))
		uap->cts_flag = 0;
	if (of_property_read_u32(pdev->dev.of_node, "bbic-flag",
		&uap->bbic_flag))
		uap->bbic_flag = 0;

	uart_v500_get_doze_id(pdev, uap);
...
#else
/*
 * LDK requires a subsys_initcall to ensure that the clock is initialized
 * first, otherwise SError issues will be triggered when the clock rate is
 * set.
 */
subsys_initcall(uart_v500_init);
#endif
