	...
	struct uart_v500_port *uap)
{
	...
	if (console_fifo_cmd_status == 1) {
		...
		fifo_cpuon = CONSOLE_FIFO_CPUON_DEFAULT;
	...
	unsigned int count)
{
	...
	struct uart_async_print_unit *unit = NULL;
	unsigned long flag;
	...
	if (!uap) {
		printk(KERN_ERR "%s get_uart_ports_uap failed\n", __func__);
		return;
	}

	unit = &uap->async_print_unit;
	...
	unsigned int count)
{
	...
	new_cfg = old_cfg | UART_V500_CFG_UART_EN | UART_V500_CFG_TX_EN | UART_V500_CFG_FIFO_EN;
