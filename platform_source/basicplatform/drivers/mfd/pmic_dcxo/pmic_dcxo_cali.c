...
static int pmu_dcxo_resp_to_modem(void *msg, int result)
{
	...
	msg_resp.result = (unsigned int)result;
	ret = RPROC_ASYNC_SEND(IPC_ACPU_MODEM_MBX_1, (mbox_msg_t *)(&msg_resp),
...
static int pmu_dcxo_recv_notifier(struct notifier_block *nb, unsigned long len, void *msg)
{
	...
	ret = memcpy_s(&(di->dcxo_para_msg[0]), sizeof(u32) * MSG_LEN_MAX, msg, len);
	if (ret) {
		pr_info("[%s] mailbox msg copy failed, ret %d\n", __func__, ret);
		return ret;
	}
...
static void pmu_dcxo_ipc_recv_register(struct pmic_dcxo_cali_dev *di)
{
	...
	ret = RPROC_MONITOR_REGISTER(IPC_MODEM_ACPU_MBX_1, &di->pmu_dcxo_nb);
	if (ret)
