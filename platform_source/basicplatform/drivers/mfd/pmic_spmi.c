...
u32 sub1_pmic_read(struct vendor_pmic *pmic, int reg)
{
	return _spmi_pmic_read(g_sub1_pmic, reg);
}

void sub1_pmic_write(struct vendor_pmic *pmic, int reg, u32 val)
{
	_spmi_pmic_write(g_sub1_pmic, reg, val);
}
...
unsigned int sub1_pmic_reg_read(int addr)
{
	return _spmi_pmic_read(g_sub1_pmic, addr);
}
EXPORT_SYMBOL(sub1_pmic_reg_read);

void sub1_pmic_reg_write(int addr, int val)
{
	_spmi_pmic_write(g_sub1_pmic, addr, val);
}
EXPORT_SYMBOL(sub1_pmic_reg_write);
...
void pmic_set_buckboost_mode(int mode)
{
	...
	pr_info("%s mode is %d, reg(0x%x) = 0x%x\n", __func__, mode, BB_CFG_REG, val);
...
success:
	if (of_device_is_compatible(np, SPMI_MMW_PMIC_COMP))
		...
		g_sub_pmic = pmic;
	else if (of_device_is_compatible(np, SPMI_SUB1_PMIC_COMP))
		g_sub1_pmic = pmic;
...
static const struct of_device_id of_spmi_pmic_match_tbl[] = {
	...
	{ .compatible = SPMI_SUB1_PMIC_COMP, },
