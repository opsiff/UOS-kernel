	...
	struct i2c_client *client, struct pmic_i2c_priv *pmic)
{
	...
	ret = of_property_read_u32_array(
		np, "pmic-irq-array", &(pmic->irqarray), 1);
	if (ret) {
	...
	struct i2c_client *client, struct pmic_i2c_priv *pmic)
{
	...
	ret = of_property_read_string(np, "pmic-irq-name",
		(const char **)&pmic->irq_name);
	if (ret < 0) {
		pr_err("no pmic-irq-name\n");
		return -ENODEV;
	}
	...
	ret = of_property_read_string(np, "hisilicon,pmic-chip-irq-name",
		(const char **)&pmic->chip_irq_name);
	if (ret < 0) {
		pr_err("no pmic-chip-irq-name\n");
		return -ENODEV;
	}
	return 0;
}
...
int pmic_test_read(unsigned int reg)
{
...
int pmic_test_write(unsigned int reg, unsigned int val)
{
	struct pmic_i2c_priv *pmic = g_pmic;
	int ret;
	...
	const struct i2c_device_id *id)
{
	...
	if (ret)
		dev_err(pmic->dev, "no pmic_fpga_flag!\n");

	if (fpga_flag == PMIC_I2C_FPGA_FLAG){
		goto success;
	}
