...
#ifdef CONFIG_UFS_RPMB_SUPPORT_FROM_TEEOS_IN_KERNEL
void ufs_rpmb_cmd_enter(void)
{
	mutex_lock(&rpmb_ufs_cmd_lock);
}

void ufs_rpmb_cmd_exit(void)
{
				  ...
				  bool is_write, enum rpmb_req_source source)
{
	...
	trace_ldk_rvh_ufs_bsg_ioctl_rpmb(rdata, is_write, id, source, &ret);
	return ret;
}
#else
static int ufs_bsg_ioctl_rpmb_cmd(enum func_id id,
...
static int32_t ufs_rpmb_key_status(void)
{
	...
	for (i = 0; i < MAX_KEY_STATUS_NUM; i++) {
		...
		ufs_rpmb_get_counter(request, rpmb_data, sense_buffer,
				     transfer_frame);
#ifdef CONFIG_UFS_RPMB_SUPPORT_FROM_TEEOS_IN_KERNEL
		ret = ufs_bsg_ioctl_rpmb_cmd_teeos(RPMB_FUNC_ID_SE, rpmb_data, false);
#else
		ret = ufs_bsg_ioctl_rpmb_cmd(RPMB_FUNC_ID_SE, rpmb_data, false, RPMB_FROM_AUTH_KEY);
#endif
		if (ret) {
			       ...
			       uint8_t *sense_buffer[])
{
	...
	case RPMB_OP_WR_DATA:
		...
		write_flag[1] = 1;
		break;
	case RPMB_OP_RD_PROTECT:
		/* include basic request(0), result(1) frmaes */
		frame_cnt = 2;
		write_flag[0] = 1;
		break;
	case RPMB_OP_WR_PROTECT:
		/* include basic request(0), status(1), result(2) frmaes */
		frame_cnt = 3;
		write_flag[0] = 1;
		write_flag[1] = 1;
			    ...
			    struct storage_blk_ioc_rpmb_data *storage_data)
{
	...
	if (operation == RPMB_OP_RD || operation == RPMB_OP_RD_PROTECT)
...
#ifdef CONFIG_UFS_RPMB_SUPPORT_FROM_TEEOS_IN_KERNEL
	ret = ufs_bsg_ioctl_rpmb_cmd_teeos(id, rpmb_data,
				     (operation == RPMB_OP_WR_DATA || operation == RPMB_OP_WR_PROTECT));
#else
