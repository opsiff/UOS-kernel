...
{
#ifdef CONFIG_RPMB_FFA_SUPPORT
	return ffa_rpmbmspc_send_msg(_function_id, _arg0, _arg1, _arg2);
#else
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5,4,0))
...
#ifdef CONFIG_HISYSEVENT
	event = hisysevent_create("STORAGE", "RPMB_DMD_REPORT_ID", FAULT);
	if (event == NULL) {
		pr_err("create hisysevent failed: RPMB_DMD_REPORT_ID \n");
		return;
	}

	hisysevent_put_integer(event, "TYPE", rpmb_info_record.type);
	hisysevent_put_integer(event, "STATE", rpmb_info_record.state);
	hisysevent_put_integer(event, "ERRNO", rpmb_info_record.errno);
	hisysevent_put_integer(event, "OCCUR", g_atf_start_time);
	hisysevent_put_integer(event, "START", g_mspc_start_time);
	hisysevent_put_integer(event, "FINISH", g_mspc_end_time);
	hisysevent_put_integer(event, "CALLBACK", g_atf_end_time);
...
static void rpmb_get_privilege(struct cred **kern_cred, const struct cred **old_cred)
{
	struct cred *new_cred = NULL;

	new_cred = prepare_kernel_cred(&init_task);
	if (!new_cred) {
		pr_err("prepare kernel cred failed\n");
	} else {
		*kern_cred = new_cred;
...
static void rpmb_release_privilege(struct cred *kern_cred, const struct cred *old_cred)
{
	if (kern_cred) {
			      ...
			      const char *buf, size_t count)
{
	...
	/* We are about to try to access the bsg-class device file. Because we are called by
	 * atcmdserver which does not have sufficient privileges, we use the kernel credentials
	 * to read the file.
	 */
	rpmb_get_privilege(&kern_cred, &old_cred);
	if (rpmb_ops && rpmb_ops->key_store)
		ret = rpmb_ops->key_store(dev, request);
	rpmb_release_privilege(kern_cred, old_cred);
...
__attribute__((weak)) int wait_rpmb_device_ready(void)
{
	int i;
	int32_t ret;
	/*lint -e501*/
...
int get_rpmb_device_init_status(void)
{
	return get_rpmb_device_init_status;
}
#endif
