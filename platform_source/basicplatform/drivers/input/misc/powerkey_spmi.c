...
#define IRQ_L3_PWRON_RPT 0x1A2
#define PWRON_DOWN       0x1
#define PWRON_UP         0x2
#define PWRON_DOWN_1S    0x4
#define PWRON_DOWN_6S    0x8

static irqreturn_t powerkey_irq_hpd(int irq, void *data);
#endif

#if defined(CONFIG_HIBERNATION) && defined(CONFIG_BCI_BATTERY)
#include <linux/power/platform/bci_battery.h>

static bool power_charged;
static bool s4_happend;
#endif
...
static irqreturn_t powerkey_irq_hpd(int irq, void *data)
{
	...
	key_value = pmic_read_reg(IRQ_L3_PWRON_RPT);
	pmic_write_reg(IRQ_L3_PWRON_RPT, key_value);
	if (key_value & PWRON_DOWN) {
		powerkey_down_hdl(irq, data);
		pr_err("[%s] power key press interrupt!\n", __func__);
	} else if (key_value & PWRON_UP) {
		powerkey_up_hdl(irq, data);
		pr_err("[%s]power key release interrupt!\n", __func__);
	} else if (key_value & PWRON_DOWN_1S) {
		powerkey_1s_hdl(irq, data);
		pr_err("[%s]power key press 1s interrupt!\n", __func__);
	} else if (key_value & PWRON_DOWN_6S) {
		powerkey_6s_hdl(irq, data);
		pr_err("[%s]power key press 6s interrupt!\n", __func__);
	} else {
		pr_err("[%s]no powerkey irq!\n", __func__);
		return IRQ_NONE;
	}

	pr_err("[%s]key release interrupt!\n", __func__);
	return IRQ_HANDLED;
}

static int keyirq_requst(struct spmi_device *pdev, struct powerkey_info *info)
{
	struct device *dev = &pdev->dev;
	int irq, ret;

	irq = spmi_get_irq_byname(pdev, NULL, "powerkey");
	if (irq < 0)
		return -1;

	ret = devm_request_irq(dev, irq, powerkey_irq_hpd,IRQF_SHARED | IRQF_NO_SUSPEND, "powerkey", info);
	if (ret < 0) {
		pr_err("%s: request powerkey_connect irq err, ret %d\n", __func__, ret);
		return -ENOENT;
	}
	pr_err("powerkey irq request, ret %d\n", ret);
	return ret;
}
#endif
...
#if defined(CONFIG_HIBERNATION) && defined(CONFIG_BCI_BATTERY)
static int powerkey_suspend(struct device *dev)
{
	pr_info("[%s]suspend successfully\n", __func__);
	return 0;
}

static int powerkey_resume(struct device *dev)
{
	pr_info("[%s]resume successfully\n", __func__);
	return 0;
}

static int psy_change_s4_notifier_call(struct notifier_block *self,
					unsigned long event, void *data)
{
	...
	case VCHRG_START_CHARGING_EVENT:
		power_charged = true;
		pr_info("[%s]battery begin to charge, %d\n", __func__, power_charged);
		break;
	case VCHRG_STOP_CHARGING_EVENT:
		power_charged = false;
		pr_info("[%s]battery stops charging, %d\n", __func__,power_charged);
		break;
	default:
		break;
	}
...
struct notifier_block psy_change_s4_event_nb = {
	.notifier_call = psy_change_s4_notifier_call,
};

static int __init psy_change_s4_init(void)
{
	bci_register_notifier(&psy_change_s4_event_nb, 1);
	return 0;
}

late_initcall(psy_change_s4_init);

static int s4_happend_event(struct device *dev)
{
	s4_happend = true;
	pr_info("[%s]S4 turn on flag successfully\n", __func__);
	return 0;
}

static void s4_powerkey_complete(struct device *dev)
{
	pr_info("[%s]resume, trying to call powerkey to wakeup\n", __func__);
	struct powerkey_info *info = dev_get_drvdata(dev);
 
	if (!power_charged && s4_happend) {
		input_report_key(info->idev, KEY_POWER, POWER_KEY_PRESS);
		input_sync(info->idev);
		pr_info("[%s]report press powerkey\n", __func__);
		input_report_key(info->idev, KEY_POWER, POWER_KEY_RELEASE);
		input_sync(info->idev);
	}
	pr_info("[%s]S4 powerkey event is over or not happend: %d\n", __func__, s4_happend);

	if (s4_happend)
		s4_happend = false;
}

const struct dev_pm_ops s4_powerkey_pm_ops = {
	.restore_noirq = s4_happend_event,
	.complete  = s4_powerkey_complete,
