...
#define LINEAR_VIBRATOR_FREQ_CALCU_BASE 1000000000
#define LINEAR_VIBRATOR_FREQ_TIME       13
...
static int linear_vibrator_frep_convert(void)
{
	...
	if ((g_vdev->linear_vibrator_freq * LINEAR_VIBRATOR_FREQ_TIME) == 0) { // calc f0 val
		dev_err(g_vdev->dev, "freq calc err, pmu not init");
		return 0;
	}
	f0 = LINEAR_VIBRATOR_FREQ_CALCU_BASE / (g_vdev->linear_vibrator_freq * LINEAR_VIBRATOR_FREQ_TIME);
	return f0;
}
...
static void haptics_num_set(u32 type)
{
	...
	case LINEAR_VIBRATOR_HAPTIC_6:
		...
		linear_vibrator_write_u8(
			LINEAR_VIBRATOR_SPEL_TIME_VALUE3_6,
			LINEAR_VIBRATOR_SPEL_TIME_REG3);
		dev_info(g_vdev->dev, "%s 600 cycle\n",  __func__);
		break;
#endif
	default:
...
#ifdef CONFIG_PMIC_21V900_PMU
	linear_vibrator_write_u8(LINEAR_VIBRATOR_CALI_RESET, LINEAR_VIBRATOR_RESET_REG);
	linear_vibrator_write_u8(LINEAR_VIBRATOR_CALI_DEASSERT_RESET, LINEAR_VIBRATOR_RESET_REG);
#endif
