...
static const struct proc_ops key_mask_fops = {
	.proc_read = key_mask_read,
	.proc_write = key_mask_write,
};
#endif
...
{
	struct device *dev = &keypad_key_dev->pdev->dev;

	/* initial work before we use it. */
	INIT_DELAYED_WORK(&(keypad_key_dev->gpio_key_work),
		keypad_key_dev->key_work);

	keypad_key_dev->gpio_key_lock = 
		wakeup_source_register(dev, keypad_key_dev->work_init_name);
...
static int keypad_init_pin(struct keypad_key_devices *keypad_key_dev)
{
	...
	struct device_node *np = keypad_key_dev->pdev->dev.of_node;
	int ret;

	ret = of_property_read_u32(np, "is_emu", &keypad_key_dev->is_emu);
	if (ret < 0)
		pr_err("[%s]: is_emu failed, ret = %d\n", __func__, ret);

	if (keypad_key_dev->is_emu)
		return 0;
	...
	struct device_node *of_node)
{
	...
	if (keypad_key_dev->is_emu) {
		dev_err(&keypad_key_dev->pdev->dev, "emu is not get gpio, set default[0x%x]",
			keypad_key_dev->is_emu);
	} else {
...
#endif
req_irq_err:
	wakeup_source_unregister(keypad_key_dev->gpio_key_lock);
