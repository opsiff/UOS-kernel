...
#elif (defined CONFIG_PMIC_26V100_PMU)
#define PMIC_GPIO_KEYUP_DATA 0x08
#define PMIC_GPIO_KEYDOWN_DATA 0x08
#define PMIC_KEYPAD_UP (1 << 1)
#define PMIC_KEYPAD_DOWN (1 << 2)
#define PMIC_KEYPAD_UP_DATA 0x2
#define PMIC_KEYPAD_DOWN_DATA 0x4
#define PMIC_GPIO_KEY_INT 0x4519
#define PMIC_KEYUP_INT_CLEAR 0
#define PMIC_KEYPAD_UP_DATA_BIT 1
#define PMIC_KEYPAD_DOWN_DATA_BIT 2
#define PMIC_KEYPAD_UP_UP_INT 0
...
#define PMIC_KEYPAD_DOWN_UP_INT 2
#define PMIC_KEYPAD_DOWN_DOWN_INT 3
#elif (defined CONFIG_PMIC_55V500_PMU)
...
#define PMIC_KEYPAD_DOWN_UP_INT 2
#define PMIC_KEYPAD_DOWN_DOWN_INT 3
#elif (defined CONFIG_PMIC_55V300_PMU)
...
#if defined(CONFIG_PMIC_21V900_PMU) || defined(CONFIG_PMIC_55V500_PMU) || defined(CONFIG_PMIC_26V100_PMU)
...
struct keypad_pmic_key {
	...
	struct wakeup_source *volume_up_key_lock;
	struct wakeup_source *volume_down_key_lock;
...
#if defined(CONFIG_HUAWEI_DSM)
static void dsm_gpio_key_timer_func(struct timer_list *data)
{
...
static void keypad_pmic_keyup_work(struct work_struct *work)
{
	...
	if (keyup_value == GPIO_HIGH_VOLTAGE)
		__pm_relax(gpio_key->volume_up_key_lock);
}
...
static void keypad_pmic_keydown_work(struct work_struct *work)
{
	...
	if (keydown_value == GPIO_HIGH_VOLTAGE)
		__pm_relax(gpio_key->volume_down_key_lock);
}

static void keypad_pmic_keyup_timer(struct timer_list *data)
{
	unsigned int keyup_value;
	struct keypad_pmic_key *gpio_key = from_timer(gpio_key, data, key_up_timer);
#if defined(CONFIG_HUAWEI_DSM)
	...
	/* judge key is pressed or released. */
	if (keyup_value == GPIO_LOW_VOLTAGE) {
		__pm_stay_awake(gpio_key->volume_up_key_lock);
#if defined(CONFIG_HUAWEI_DSM)
...
static void keypad_pmic_keydown_timer(struct timer_list *data)
{
	u32 keydown_value;
	struct keypad_pmic_key *gpio_key = from_timer(gpio_key, data, key_down_timer);
#if defined(CONFIG_HUAWEI_DSM)
	char *rcd_msg = "volume down key trigger on the abnormal style.\n";
#endif
	...
	/* judge key is pressed or released. */
	if (keydown_value == GPIO_LOW_VOLTAGE) {
		__pm_stay_awake(gpio_key->volume_down_key_lock);
...
static irqreturn_t keypad_pmic_key_irq_handler(int irq, void *dev_id)
{
	...
	if ((irq == gpio_key->gpiokey_irq[0]) ||
		...
		__pm_wakeup_event(gpio_key->volume_up_key_lock,
		...
		mod_timer(&(gpio_key->key_down_timer),
			jiffies + msecs_to_jiffies(TIMER_DEBOUNCE));
		__pm_wakeup_event(gpio_key->volume_down_key_lock,
			jiffies_to_msecs(KEY_IRQ_WAKEUP_INTERVAL));
...
static const struct proc_ops key_mask_fops = {
	.proc_read = keypad_pmic_key_mask_read,
	.proc_write = keypad_pmic_key_mask_write,
};
#endif
...
{
	struct device *dev = gpio_key->dev;
	INIT_DELAYED_WORK(
		(struct delayed_work *)(uintptr_t)(&(gpio_key->gpio_keyup_work)),
		keypad_pmic_keyup_work);
	INIT_DELAYED_WORK(
		(struct delayed_work *)(uintptr_t)(&(gpio_key->gpio_keydown_work)),
		keypad_pmic_keydown_work);
	gpio_key->volume_down_key_lock = wakeup_source_register(dev, "key_down_wake_lock");
	gpio_key->volume_up_key_lock = wakeup_source_register(dev, "key_up_wake_lock");
...
{
	timer_setup(&(gpio_key->key_up_timer), keypad_pmic_keyup_timer, 0);
	timer_setup(&(gpio_key->key_down_timer), keypad_pmic_keydown_timer, 0);

#if defined(CONFIG_HUAWEI_DSM)
	timer_setup(&dsm_gpio_key_timer, dsm_gpio_key_timer_func, 0);
#endif
}
...
err_register_dev:
	wakeup_source_unregister(gpio_key->volume_down_key_lock);
	wakeup_source_unregister(gpio_key->volume_up_key_lock);
