...
#if defined(CONFIG_SR_HIBERNATION_WATCHDOG) && (LINUX_VERSION_CODE >= KERNEL_VERSION(5,10,0))
/* s4 set watchdog timeout */
void watchdog_s4_timeout_set(unsigned int timeout) {
	struct hiv500_wdt *wdt = g_wdt;
	int ret;
	...
	if ((timeout < wdt->kick_time) || (timeout > (DEFAULT_TIMEOUT * WDG_TIMEOUT_ADJUST))) {
		pr_err("[%s]timeout invalid[%u, %u]\n", __func__, timeout, wdt->kick_time);
		return;
	}
...
#if defined(CONFIG_SR_HIBERNATION_WATCHDOG) && (LINUX_VERSION_CODE >= KERNEL_VERSION(5,10,0))
int watchdog_get_default_timeout(void)
{
	if (!g_wdt || !g_wdt->pdev) {
		pr_err("[%s] wdt device not init\n", __func__);
		return -ENOENT;
	}
	struct platform_device *pdev = g_wdt->pdev;
	int config_time = 0;
	struct device_node *np = NULL;
	int ret;
	...
	ret = of_property_read_u32_index(np, "default-timeout", 0, &config_time);
	if (ret) {
		dev_warn(&pdev->dev,
			"find default-timeout property fail, Use the default value: %us\n", DEFAULT_TIMEOUT);
		return DEFAULT_TIMEOUT;
