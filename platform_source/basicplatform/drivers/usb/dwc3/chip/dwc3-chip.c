 ...
 */
static int event_dequeue(struct chip_dwc3_device *chip_dwc3,
...
static void handle_start_arm_usb_event(struct chip_dwc3_device *chip_dwc)
{
	...
	case USB_STATE_OFF:
		...
		chip_dwc->state = USB_STATE_HOST;
		if (!chip_dwc->usb_host_wakelock_disable)
			chip_dwc3_wake_lock(chip_dwc);
		usb_dbg("usb_status: OFF -> HOST\n");
		break;
	default:
...
static int get_resource(struct chip_dwc3_device *chip_dwc3)
{
	...
	if (of_property_read_u32(dev->of_node, "usb_host_wakelock_disable", &(chip_dwc3->usb_host_wakelock_disable)))
		chip_dwc3->usb_host_wakelock_disable = 0;
...
static void dwc3_handle_wakeup_event(struct device *dev)
{
...
static int dwc3_request_wakeup_irq(struct device *dev, struct chip_dwc3_device *chip_dwc3)
{
	int retval;
...
static int dwc3_free_wakeup_irq(struct device *dev, struct chip_dwc3_device *chip_dwc3)
{
	if (chip_dwc3->usb_wakeup.irq == 0)
		return 0;
