...
void usb_notify_set_address_err(struct usb_device *udev)
{
	blocking_notifier_call_chain(&usb_fault_notifier_list, USB_SET_ADDRESS_ERR, udev);
}

void usb_notify_get_descriptor_err(struct usb_device *udev)
{
	blocking_notifier_call_chain(&usb_fault_notifier_list, USB_GET_DESCRIPTOR_ERR, udev);
}

void usb_notify_enum_dev_unknown_err(struct usb_device *udev)
{
	blocking_notifier_call_chain(&usb_fault_notifier_list, USB_ENUM_UNKNOWN_ERR, udev);
}

void usb_notify_transfer_err(struct usb_device *udev)
{
	blocking_notifier_call_chain(&usb_fault_notifier_list, USB_TRANSFER_ERR, udev);
}
...
void vendor_usb_urb_complete(struct usb_device *udev, struct urb *urb,
	int status)
{
#ifdef CONFIG_FMEA_FAULT_INJECTION
