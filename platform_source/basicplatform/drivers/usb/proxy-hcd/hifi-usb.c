...
static void usb_stop_hifi_usb_reset_power_by_portid(int portid)
{
	info("\n");
...
static void hifi_usb_proxy_free_dev(struct proxy_hcd_client *client, int slot_id)
{
	struct hifi_usb_proxy *proxy = client->client_priv;
	struct hifi_usb_op_msg *op_msg = &proxy->op_msg;
	...
	if (!timer_pending(&proxy->confirm_udev_timer))
		mod_timer(&proxy->confirm_udev_timer, jiffies + proxy->reconnect_time);

	mutex_unlock(&proxy->msg_lock);
}

static int hifi_usb_proxy_enable_device(struct proxy_hcd_client *client, int slot_id, int speed, int ep0_maxpks)
{
	struct hifi_usb_proxy *proxy = client->client_priv;
	struct hifi_usb_op_msg *op_msg = &proxy->op_msg;
	int ret;
...
static int hifi_usb_proxy_reset_device(struct proxy_hcd_client *client, int slot_id)
{
	struct hifi_usb_proxy *proxy = client->client_priv;
	struct hifi_usb_op_msg *op_msg = &proxy->op_msg;
	int ret;
...
static int hifi_usb_proxy_address_device(struct proxy_hcd_client *client, int slot_id)
{
	struct hifi_usb_proxy *proxy = client->client_priv;
	struct hifi_usb_op_msg *op_msg = &proxy->op_msg;
	int ret;
...
static int hifi_usb_proxy_update_device(struct proxy_hcd_client *client, int slot_id, struct usb_ext_cap_descriptor *ext_cap)
{
	struct hifi_usb_proxy *proxy = client->client_priv;
	struct hifi_usb_op_msg *op_msg = &proxy->op_msg;
	int ret;
...
static int hifi_usb_proxy_hub_control(struct proxy_hcd_client *client, struct usb_ctrlrequest *cmd, char *buf)
{
	struct hifi_usb_proxy *proxy = client->client_priv;
	int ret;
...
static int hifi_usb_proxy_urb_enqueue(struct proxy_hcd_client *client, struct urb_msg *urb_msg)
{
	struct hifi_usb_proxy *proxy = client->client_priv;
	struct hifi_usb_op_msg usb_op_msg = {0};
	struct hifi_usb_op_msg *op_msg = &usb_op_msg;
...
static void hifi_usb_dts_config(struct hifi_usb_proxy *hifiusb)
{
	...
	if (device_property_read_bool(dev, "hifiusb,reconnect_short_time"))
		hifiusb->reconnect_time = HIFI_USB_CONFIRM_RECONNECT_SHORT_TIME;
	else
		hifiusb->reconnect_time = HIFI_USB_CONFIRM_RECONNECT_LONG_TIME;
...
static int hifi_usb_suspend(struct proxy_hcd_client *client)
{
	struct hifi_usb_proxy *proxy = client->client_priv;
...
static void hifi_usb_resume(struct proxy_hcd_client *client)
{
	struct hifi_usb_proxy *proxy = client->client_priv;
