...
static struct proxy_hcd_client *find_client_by_name(const char *name)
{
...
static int phcd_plat_prepare(struct device *dev)
{
	info("+\n");
...
static void phcd_plat_complete(struct device *dev)
{
	info("+\n");
	info("-\n");
}

static int phcd_plat_suspend_late(struct device *dev)
{
	dbg("+\n");
	dbg("-\n");
	return 0;
}
static int phcd_plat_resume_early(struct device *dev)
{
	dbg("+\n");
	dbg("-\n");
	return 0;
