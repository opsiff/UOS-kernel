...
static bool is_usbaudio_device(struct usb_device *udev, int configuration)
{
	...
	info("audio_intf_num %d, hid_intf_num %d, other_intf_num %d, speed %d\n",
		audio_intf_num, hid_intf_num, other_intf_num, udev->speed);
...
static bool is_specific_earphone(struct usb_device *udev)
{
	if (udev->parent == NULL) {
		WARN_ON(1);
		return false;
	...
	if (udev->speed == USB_SPEED_FULL) {
		info("this is specific earphone\n");
		return true;
	}
...
bool stop_hifi_usb_when_non_usbaudio(struct usb_device *udev, int configuration)
{
	...
	if (is_specific_earphone(udev))
#ifdef CONFIG_USB_HIUSB
		(void)hiusb_usb20_phy_calibrate(udev);
#else
