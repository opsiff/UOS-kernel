...
static int do_read_toc(struct fsg_common *common, struct fsg_buffhd *bh)
{
	...
	if (response_length < common->data_size_from_cmnd)
		common->data_size_from_cmnd = (u32)response_length;
				...
				struct device_attribute *attr, char *buf)
{
	...
	return snprintf_s(buf, PAGE_SIZE, sizeof(opts->common->inquiry_string) - 1, "%s\n", opts->common->inquiry_string);
}
		...
		struct device_attribute *attr, const char *buf, size_t size)
{
	...
	ret = strncpy_s(opts->common->inquiry_string, sizeof(opts->common->inquiry_string), buf, len);
	if (ret != EOK)
		pr_info("mass storage inquiry store failed\n");
