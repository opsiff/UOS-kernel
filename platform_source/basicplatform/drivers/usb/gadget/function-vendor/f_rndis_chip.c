...
#include <linux/platform_drivers/usb/android_gadget.h>
#include "u_ether.h"
#include <securec.h>
...
{
	return snprintf_s(buf, PAGE_SIZE, sizeof(manufacturer) - 1, "%s\n", manufacturer);
}
		...
		struct device_attribute *attr, const char *buf, size_t size)
{
	...
	ret = strncpy_s(manufacturer, sizeof(manufacturer), buf, len);
	if (ret != EOK)
		pr_info("[%s]rndis manufacturer store failed\n", __func__);
...
static ssize_t rndis_wceis_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	return snprintf_s(buf, PAGE_SIZE, sizeof(wceis), "%d\n", wceis);
}
		...
		struct device_attribute *attr, const char *buf, size_t size)
{
	...
	/* ETH_ALEN is 6 */
	if (sscanf_s(buf, "%02x:%02x:%02x:%02x:%02x:%02x\n",
...
static int get_ether_addr_str(u8 dev_addr[ETH_ALEN], char *str, int len)
{
	...
	ret = snprintf_s(str, (size_t)len, (size_t)(len - 1), "%pM", dev_addr);
	if (ret == -1)
		pr_info("%s get ether add failed\n", __func__);
	return HOST_ADD_LEN;
}
...
static void rndis_set_host_ethaddr(struct f_rndis_opts *opts)
{
	...
	if (get_ether_addr_str(host_ethaddr_record, addr, HOST_ADD_LEN) > 0) {
		if (gether_set_host_addr(opts->net, addr) == 0)
