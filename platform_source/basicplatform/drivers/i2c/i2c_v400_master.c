...
static void iomcu_i2c_reset_controller(struct hs_i2c_dev *dev)
{
#ifdef CONFIG_I2C_FFA_SUPPORT
	int ret;
	...
	args.data1 = dev->mapbase;

	ret = ffa_platdrv_send_msg(&args);
	if (ret != 0)
		dev_err(dev->dev, "%s failed to send_msg, ret:%d!\n", __func__, ret);
#else
	unused(dev);
#endif
}
...
static int hs_i2c_reset_controller(struct hs_i2c_dev *dev)
{
	...
	if(dev->iomcu_i2c) {
		iomcu_i2c_reset_controller(dev);
		return 0;
	}
...
{
	struct hs_i2c_dev *dev = i2c_get_adapdata(adap);
	struct hs_i2c_sda_iomg *sda_ioc = NULL;
	...
	if ((dev->iomg_base == NULL) || (dev->iomg_pad_base == NULL)) {
		dev_err(dev->dev, "%s:i2c iomg doesn't be remap\n", __func__);
		return;
	}

	sda_ioc = &dev->sda_ioc;
	if ((sda_ioc->iomg_func_i2c == 0) || (sda_ioc->iomg_pad_func_i2c == 0)) {
		dev_err(dev->dev, "%s:i2c iomg_func or iomg_pad_func err\n", __func__);
		return;
	}
	dev_info(dev->dev, "bus prepares recovery ...\n");

	/* switch sda to gpio */
	writel(sda_ioc->iomg_func_gpio, dev->iomg_base + sda_ioc->iomg_offset);
	writel(sda_ioc->iomg_func_gpio, dev->iomg_pad_base + sda_ioc->iomg_offset);

	udelay(10);
	/* config slave address to 0x7F */
	regmap_write(dev->map, SLV_ADDR, I2C_V400_TAR_RECOVERY_CFG);

	/* recived data from bus */
	regmap_write(dev->map, CMD_TXDATA, I2C_V400_RD_CFG);

	msleep(I2C_RECOVER_SLEEP_MS);

	/* switch sda to i2c */
	writel(sda_ioc->iomg_func_i2c, dev->iomg_base + sda_ioc->iomg_offset);
	writel(sda_ioc->iomg_pad_func_i2c, dev->iomg_pad_base + sda_ioc->iomg_offset);
...
static int hs_i2c_wait_bus_not_busy(struct hs_i2c_dev *dev)
{
	...
	dev_err(dev->dev, "timeout waiting for bus ready 0x%x\n", stat);
	...
	struct i2c_msg msgs[], int num)
{
	...
	r = hs_i2c_wait_bus_not_busy(dev);
	if (r)
		return r;
...
static void hs_i2c_dev_recover_cfg(struct hs_i2c_dev *idev,
	struct device *dev)
{
	int ret;
	u32 data[I2C_RECOVER_VAL_CNT] = {0};
	...
	if (ret) {
		...
		idev->iomg_base = devm_ioremap(dev, data[1], data[3]);
		dev_info(dev, "i2c ioc register addr is: %x\n", data[1]);
	}

	ret = of_property_read_u32_array(dev->of_node, "i2c-pad-ioc-base",
		&data[0], ARRAY_SIZE(data));
	if (ret) {
		dev_err(dev, "doesn't have i2c-pad-ioc-base property!\n");
		return;
	} else {
		idev->iomg_pad_base = devm_ioremap(dev, data[1], data[3]);
		dev_info(dev, "i2c pad ioc register addr is: %x\n", data[1]);
	}
	...
	idev->sda_ioc.iomg_offset = data[0];
	idev->sda_ioc.iomg_func_i2c = data[1];
	idev->sda_ioc.iomg_pad_func_i2c = data[2];
	idev->sda_ioc.iomg_func_gpio = data[3];
