...
#include <power/spmi_sec_whitelist.h>
...
#if defined(CONFIG_PLATDRV_SUPPORT_FFA)
static int ffa_sync_send_receive_request(u64 _function_id, u64 _type, u64 _arg0, u64 _arg1, u64 _arg2)
{
	int ret;
...
static int spmi_smc(u64 _function_id, u64 _arg0, u64 _arg1, u64 _arg2, u64 _arg3)
{
#if defined(CONFIG_PLATDRV_SUPPORT_FFA)
	return ffa_sync_send_receive_request(_function_id, _arg0, _arg1, _arg2, _arg3);
#endif
	return atfd_spmi_smc(_function_id, _arg0, _arg1, _arg2, _arg3);
}

static bool is_spmi_sec_reg(u8 sid, u16 addr)
{
#if defined(CONFIG_PLATDRV_SUPPORT_FFA)
	u32 i;

	if (sid >= SPMI_SLAVEID_EOF)
		return false;

	struct spmi_sec_reg * whitelist = spmi_sec_whitelist[sid];
	if (whitelist == NULL)
		return false;

	for (i = 0; ; i++) {
		if (whitelist[i].sid == SPMI_SLAVEID_EOF)
			return false;

		if (whitelist[i].sid == sid && whitelist[i].addr == addr)
			return true;
	}
#endif
	return false;
	...
	u8 opcode, u8 sid, u16 addr, u8 bc, u8 *buf)
{
	...
	if (ctrl->always_sec || is_spmi_sec_reg(sid, addr)) {
		spin_lock_irqsave(&ctrl->sec_lock, flags);
		for (i = 0; i < bc; i++) {
			ret = spmi_smc((u64)SPMI_FN_MAIN_ID, (u64)SPMI_READ,
	...
	u8 opcode, u8 sid, u16 addr, u8 bc, u8 *buf)
{
	...
	if (ctrl->always_sec || is_spmi_sec_reg(sid, addr)) {
		spin_lock_irqsave(&ctrl->sec_lock, flags);
		for (i = 0; i < bc; i++) {
			ret = spmi_smc((u64)SPMI_FN_MAIN_ID, (u64)SPMI_WRITE,
