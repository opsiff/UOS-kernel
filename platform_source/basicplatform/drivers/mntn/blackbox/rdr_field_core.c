...
#include "rdr_inner.h"
#include "rdr_print.h"
#ifdef CONFIG_MNTN_BBOX_LOG_MAP
#include "mntn_bbox_log.h"
#endif
...
void *get_log_addr(void)
{
#ifdef CONFIG_MNTN_BBOX_LOG_MAP
	g_tmp_pbb = get_bbox_log_vaddr();
#else
...
int rdr_field_init(void)
{
	...
	g_pbb = dfx_bbox_map(rdr_reserved_phymem_addr(),
			rdr_reserved_phymem_size());
	if (g_pbb == NULL) {
		BB_PRINT_ERR("dfx_bbox_map g_pbb faild\n");
		ret = -1;
		goto out;
	}

	g_tmp_pbb = get_log_addr();
	if (g_tmp_pbb == NULL) {
		BB_PRINT_ERR("get g_tmp_pbb faild\n");
		ret = -1;
		dfx_bbox_unmap(g_pbb);
		g_pbb = NULL;
		goto out;
	}
...
#endif
#ifndef CONFIG_MNTN_BBOX_LOG_MAP
	...
	if ((rdr_get_reboot_type() == AP_S_COLDBOOT || rdr_get_reboot_type() == COLDBOOT) && (fpga_flag != 1)) {
