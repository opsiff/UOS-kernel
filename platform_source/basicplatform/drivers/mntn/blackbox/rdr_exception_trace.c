				...
				u32 e_exce_type, u32 e_exce_subtype)
{
	...
	arch_timer_func_ptr   func_ptr;

	BB_PRINT_START();

	func_ptr = get_timer_func_ptr();
	if (!rdr_init_done()) {
...
{
	u32 min_size = (u32)(sizeof(struct dfx_ap_ringbuffer_s) + sizeof(rdr_exception_trace_t));
...
int rdr_exception_trace_init(void)
{
	...
	if (g_exception_trace_addr == NULL) {
		g_exception_trace_addr = (u8 *)dfx_bbox_map(g_current_info.log_addr, g_current_info.log_len);
		if (unlikely(!g_exception_trace_addr)) {
			BB_PRINT_ERR("[%s], dfx_bbox_map fail!\n", __func__);
			goto error;
		}
	}
