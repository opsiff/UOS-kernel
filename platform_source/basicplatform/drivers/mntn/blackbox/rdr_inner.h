...
void rdr_set_reboot_type(u32 reboot_type);
...
#ifdef CONFIG_DUMP_HMLOG_ON_EXCEPTION
void dump_hmlog_on_exception(const char *path);
#else
static inline void dump_hmlog_on_exception(const char *path)
{
	unused(path);
}
#endif /* CONFIG_DUMP_HMLOG_ON_EXCEPTION */
