...
static bool g_saved_dfx2file = false;
...
/* Execption information is in dfx partition if the execption occur in XLOADER~FASTBOOT.
   Save dfx information already include the process of save history.log record.
   Need to save history.log record actively if no information int dfx partition */
bool saved_dfx2file_done(void)
{
	return g_saved_dfx2file;
}
