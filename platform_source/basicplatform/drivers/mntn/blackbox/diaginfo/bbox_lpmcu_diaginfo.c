...
static int bbox_lpmcu_diaginfo_save(struct lpmcu_diaginfo *pmsg)
{
	...
	for (i = 0; i < map_size; i++) {
		if (g_lpmcu_diaginfo_map[i].err_id == (unsigned int)pmsg->diaginfo[0])
			break;
	}
	...
	for (i = 1; i <= min((unsigned int)(g_lpmcu_diaginfo_map[index].param_num), MAX_PARAM_NUM); i++) {
		...
		ret = strncat_s(pdata, MAX_DIAGINFO_LEN, temp, strlen(temp));
		if (ret != EOK) {
