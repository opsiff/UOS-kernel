	...
	const char __user *userbuf, size_t bytes, loff_t *off)
{
	...
	if (strstr(reason, "SHUTDOWN") != NULL) {
		ret = snprintf_s(buf, sizeof(buf), sizeof(buf) - 1,
			"system exception core [AP], reason [%s], time [%s], sysreboot [true], bootup_keypoint [%u], category [SHUTDOWN]\n",
			reason, date, get_boot_keypoint());
	} else {
		ret = snprintf_s(buf, sizeof(buf), sizeof(buf) - 1,
			"system exception core [AP], reason [%s], time [%s], sysreboot [true], bootup_keypoint [%u], category [NORMALBOOT]\n",
			reason, date, get_boot_keypoint());
	}
...
static const struct proc_ops poweroff_reason_fops = {
	.proc_write = poweroff_reason_write,
#else
