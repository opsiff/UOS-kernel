...
static void rdr_save_logbuf_log(const char *path)
{
	...
	ret = rdr_savebuf2fs(path, "kernel_logbuff.bin", dst, DST_LOG_BUFF_LEN, 0);
	if (ret < 0) {
...
static void rdr_save_logbuf_log(const char *path)
{
	...
	ret = rdr_savebuf2fs(path, "kernel_logbuff.bin", dst, lenth, 0);
	if (ret < 0) {
		BB_PRINT_ERR("%s(): rdr_savelogbuf2fs error = %d\n", __func__, ret);
		goto err;
	}
	...
	ret = rdr_savebuf2fs(path, "kernel_logbuff.bin", buffer, sizeof(lenth), 1);
	if (ret < 0) {
