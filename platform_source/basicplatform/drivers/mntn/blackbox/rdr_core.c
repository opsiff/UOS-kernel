...
#ifdef CONFIG_DFX_BB_SHUTDOWN_REASON
...
#include "../../ddrc/ddr_subreason/ddr_subreason_monoceros.h"
#endif
#endif
...
{
	u32 reboot_times;
	u32 max_reboot_times = rdr_get_reboot_times();
	u32 mod_id = p->modid;
	...
	if (p_exce_info->e_reset_core_mask & RDR_AP) {
		rdr_record_reboot_times2mem();
		reboot_times = (u32)rdr_record_reboot_times2file();
...
static int rdr_main_thread_body(void *arg)
{
	...
	while (!kthread_should_stop()) {
		jiffies_time = (long)msecs_to_jiffies(5000);
