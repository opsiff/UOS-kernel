...
#if defined(CONFIG_DFX_MNTN_PC) || defined(CONFIG_DFX_OHOS) || !defined(CONFIG_DFX_BB_DEBUG)
	BB_PRINT_PN("[%s], NO version on system!\n", __func__);
	return;
#endif
...
int dfx_ap_nmi_notify_init(void)
{
	struct device_node *np = NULL;
	u32 sctrl_base;
	u32 lpctrl_offset, ret;

	np = of_find_compatible_node(NULL, NULL, "hisilicon,sctrl_address");
	if (!np) {
		BB_PRINT_ERR("%s: no compatible node found\n", __func__);
		return -1;
	}

	ret = of_property_read_u32(np, "base_addr", &sctrl_base);
	if (ret) {
		BB_PRINT_ERR("%s , get sctrl base compatible node err\n", __func__);
		return -1;
	}

	ret = of_property_read_u32(np, "lpctrl_offset", &lpctrl_offset);
	if (ret) {
		BB_PRINT_ERR("%s , get lpctrl_offset compatible node err\n", __func__);
		return -1;
	}

	if (g_mi_notify_lpm3_addr == NULL) {
		g_mi_notify_lpm3_addr = ioremap(sctrl_base + lpctrl_offset, REG_MAP_SIZE); /*lint !e446*/
		if (g_mi_notify_lpm3_addr == NULL) {
	...
	if (g_reset_keypoint_addr == NULL) {
		/*lint -e446*/
		u32 keypoint_offset;
		ret = of_property_read_u32(np, "bakdata10_offset", &keypoint_offset);
		if (ret) {
			BB_PRINT_ERR("%s , get bakdata10_offset compatible node err\n", __func__);
			return -1;
		}

		g_reset_keypoint_addr =
			ioremap(sctrl_base + keypoint_offset, REG_MAP_SIZE);
		/*lint +e446*/
		if (g_reset_keypoint_addr == NULL) {
			BB_PRINT_ERR("[%s]", __func__);
			return -1;
...
void rdr_ap_reset(u32 modid, u32 etype, u64 coreid)
{
	...
	flush_logbuff_range();
#ifdef CONFIG_LIBLINUX_REBOOT_WITH_SNAPSHOT
	if ((modid == MODID_AP_S_COMBINATIONKEY) && (etype == AP_S_COMBINATIONKEY)) {
		preempt_enable();
		reboot_with_snapshot("AP_S_COMBINATIONKEY");
		BB_PRINT_PN("%s end\n", __func__);
		return;
	}
#endif
...
int record_reason_task(void *arg)
{
	...
	while (rdr_wait_partition(PATH_MNTN_PARTITION, RDR_WAIT_PARTITION_TIME) != 0);
	if (((temp.e_exce_type >= REBOOT_REASON_LABEL4 && temp.e_exce_type < REBOOT_REASON_LABEL5)) &&
		(need_save_dfx2file() || saved_dfx2file_done())) {
		BB_PRINT_PN("save dfx partition msg inclule save history log\n");
		return 0;
	}
