...
static bool need_run_record_task(void)
{
	u32 reboot_type;
	u32 bootup_keypoint;
	...
	if (reboot_type < REBOOT_REASON_LABEL1 &&
		...
		return true;
	if (reboot_type >= REBOOT_REASON_LABEL4 && reboot_type < REBOOT_REASON_LABEL5)
		return true;
	return false;
}
/*
...
static void rdr_log_buf_notify_bl31(void)
{
...
void rdr_kernel_early_panic(void)
{
	...
	set_reboot_reason_subtype(AP_S_PANIC, HI_APPANIC_EARLY_PANIC);
	if (check_mntn_switch(MNTN_PANIC_INTO_LOOP))
		do {} while (1);
