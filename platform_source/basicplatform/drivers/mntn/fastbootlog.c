...
#define FASTBOOT_DUMP_LOG_ADDR SUB_RESERVED_FASTBOOT_LOG_PYHMEM_BASE
#define FASTBOOT_DUMP_LOG_SIZE SUB_RESERVED_FASTBOOT_LOG_PYHMEM_SIZE
					...
					size_t bytes, loff_t *off)
{
	...
	copy = (ssize_t)(uintptr_t) min(bytes, (size_t) (s_last_fastbootlog_size - *off));
					...
					loff_t *off)
{
	...
	copy = (ssize_t)(uintptr_t) min(bytes, (size_t) (s_fastbootlog_size - *off));
...
static const struct proc_ops last_fastbootlog_dump_file_fops = {
	.proc_read = last_fastbootlog_dump_file_read,
#else
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5,10,0))
static const struct proc_ops fastbootlog_dump_file_fops = {
	.proc_read = fastbootlog_dump_file_read,
#else
...
static int dump_init_save_log(struct fastbootlog_head *head, const char *fastbootlog_buff)
{
	...
	if (head->log_offset < head->log_start) {
		...
		log_size = tmp_len + head->log_offset - (unsigned int)sizeof(*head);
...
static int dump_init_save_lastlog(struct fastbootlog_head *head, const char *fastbootlog_buff)
{
	...
	if (head->lastlog_offset < head->lastlog_start) {
		...
		lastlog_size = tmp_len + head->lastlog_offset - (unsigned int)sizeof(*head);
