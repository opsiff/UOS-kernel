...
static enum mspc_report_data_index g_index;
static struct mspc_diaginfo g_mspc_diaginfo[MSPC_INDEX_MAX];
static struct timespec64 g_time_base;

static int mspc_record_mspc_diaginfo(const struct mspc_diaginfo *diaginfo);
#endif

#ifdef CONFIG_CRYPTO_CORE_FFA_SUPPORT
struct crypto_core_logbuf_info {
	uintptr_t base_addr;
	uint64_t base_size;
};

static struct crypto_core_logbuf_info g_mntn;
#endif
...
#ifdef CONFIG_CRYPTO_CORE_V2
enum hisee_int_id {
	HISEE_OTPC_INT_ID = BIT(0),
	HISEE_TRNG_INT_ID = BIT(1),
	HISEE_TSENSOR0_INT_ID = BIT(2),
	HISEE_TSENSOR1_INT_ID = BIT(3),
	HISEE_RST_INT_ID = BIT(4),
	HISEE_LOCKUP_INT_ID = BIT(5),
	HISEE_SENC2AP0_INT_ID = BIT(6),
	HISEE_SENC2AP1_INT_ID = BIT(7),
	HISEE_DS2AP_INT_ID = BIT(8),
	HISEE_AS2AP_INT_ID = BIT(9),
#ifdef CONFIG_GENERAL_SEE_MNTN_DDR
	HISEE_SOC_ALARM_INT_ID = BIT(10),
	HISEE_CORE_PARITY_INT_ID = BIT(11),
	HISEE_SUBSYS_ALARM_INT_ID = BIT(12),
	HISEE_DUAL_VIOLATION_INT_ID = BIT(13),
	HISEE_INT_TRAP_INT_ID = BIT(14),
#endif
	HISEE_INVALID_INT_ID,
};
...
static struct hisee_exc_trans_s g_hisee_exc_trans[] = {
	...
	{ HISEE_TSENSOR0_INT_ID, MODID_GENERAL_SEE_EXC_TSENSOR0 },
...
#ifdef CONFIG_GENERAL_SEE_MNTN_DDR
	{ HISEE_SOC_ALARM_INT_ID, MODID_GENERAL_SEE_EXC_SOC_ALARM_IRQ },
	{ HISEE_CORE_PARITY_INT_ID, MODID_GENERAL_SEE_EXC_CORE_PARITY_IRQ },
	{ HISEE_SUBSYS_ALARM_INT_ID, MODID_GENERAL_SEE_EXC_SUBSYS_ALARM_IRQ },
	{ HISEE_DUAL_VIOLATION_INT_ID, MODID_GENERAL_SEE_EXC_DUAL_VIOLATION_IRQ },
	{ HISEE_INT_TRAP_INT_ID, MODID_GENERAL_SEE_EXC_INT_TRAP_IRQ },
#endif
...
#ifdef CONFIG_CRYPTO_CORE_APM_KCA_NOTIFY
static struct task_struct *g_apm_thread = NULL;
#endif
...
#ifdef CONFIG_CRYPTO_CORE_FFA_SUPPORT
#define NEED_COPY_SECOND_BLOCK 1

static noinline int copy_logdata_to_log_phy(u64 source_addr, u64 size)
{
	u64 recv_size;
	u8 *log_write_start = (u8 *)(g_logbuf_info.addr + GENERAL_SEE_LOGBUF_HEADER_LEN +
				     g_logbuf_info.header->real_size);
	u8 *dest_addr = log_write_start;
	size_t dest_max = (size_t)g_hisee_info.log_len - GENERAL_SEE_LOGBUF_HEADER_LEN;
	struct ffa_send_direct_data args = {
		.data0 = HISEE_MNTN_ID,
		.data1 = HISEE_SMC_GET_LOG,
		.data2 = 0,
		.data3 = 0,
		.data4 = 0,
	};
	...
	if (size > dest_max) {
		pr_err("%s: size %llx too long!! dest_max: %lx.\n", __func__, size, dest_max);
		return HISEE_ERROR;
	}

	while (size > 0) {
		recv_size = size < FFA_LOCAL_BUFFER_MAX ? size : FFA_LOCAL_BUFFER_MAX;
		args.data2 = source_addr;
		args.data4 = recv_size;

		if (ffa_rpmbmspc_send_msg_with_shmem(&args, dest_addr, SHMEM_OUT) != 0) {
			return HISEE_ERROR;
		}
		dest_addr += recv_size;
		source_addr += recv_size;
		size -= recv_size;
	}

	g_logbuf_info.header->real_size = dest_addr - log_write_start;
	return HISEE_OK;
}

static noinline u64 tzsp_hisi_service_hisee_mntn_ffa(u64 _function_id, u64 _arg0, u64 _arg1, u64 _arg2)
{
	int ret;
	struct ffa_send_direct_data args = {
		.data0 = _function_id,
	...
	switch (_arg0) {
	case HISEE_SMC_INIT:
	case HISEE_SMC_GET_ATF_VOTE:
	case HISEE_SMC_GET_TEE_VOTE:
		ret = ffa_rpmbmspc_send_msg_with_args(&args);
		if (ret) {
			pr_err("ffa send fail:%d\n", ret);
			return ret;
		}
		return args.data1;
	case HISEE_SMC_GET_LOG:
		g_logbuf_info.header->real_size = 0;
		return copy_logdata_to_log_phy(g_mntn.base_addr,
					       g_mntn.base_size);
	case HISEE_SMC_LOG_OUT:
		ret = ffa_rpmbmspc_send_msg_with_args(&args);
		if (ret) {
			pr_err("ffa send fail:%d\n", ret);
			return ret;
		...
		g_logbuf_info.header->real_size = 0;
		ret = copy_logdata_to_log_phy(args.data0, args.data1);
		if(ret != HISEE_OK)
			return ret;

		if (args.data2 == NEED_COPY_SECOND_BLOCK) {
			ret = copy_logdata_to_log_phy(args.data3, args.data4);
			if(ret != HISEE_OK)
				return ret;
		}
		return ret;
...
static noinline u64 hisee_mntn_svc(u64 _function_id, u64 _arg0, u64 _arg1, u64 _arg2)
{
	pr_err("%s[%d] enter!!!.\n", __func__, __LINE__);
#ifdef CONFIG_CRYPTO_CORE_FFA_SUPPORT
	return tzsp_hisi_service_hisee_mntn_ffa(_function_id, _arg0, _arg1, _arg2);
#else
	return atfd_hisi_service_hisee_mntn_smc(_function_id, _arg0, _arg1, _arg2);
#endif
}
...
void rdr_hisee_save_print_log(void)
{
	...
	(void)hisee_mntn_svc(HISEE_MNTN_ID, (u64)HISEE_SMC_LOG_OUT,
	...
	/* save hisee log to CONFIG_BBOX_LOG_DIR+time/hisee_printlog */
...
int rdr_hisee_thread(void *arg)
{
	...
	while (!kthread_should_stop()) {
		...
		case HISEE_STATE_HISEE_EXC:
			...
			g_vote_val_atf = (u32)hisee_mntn_svc(HISEE_MNTN_ID,
				(u64)HISEE_SMC_GET_ATF_VOTE, (u64)0, (u64)0);
			g_vote_val_tee = hisee_mntn_svc(HISEE_MNTN_ID,
	...
	while (!kthread_should_stop()) {
		...
		case HISEE_STATE_HISEE_EXC:
			...
			if (rdr_hisee_judge_new_misc() == HISEE_OK) {
				...
				pr_err("msg.data[0] = %u msg.data[1] = %u\n",
				       g_msg.data[HISEE_MNTN_IPC_DATA0], g_msg.data[HISEE_MNTN_IPC_DATA1]);
#ifdef CONFIG_GENERAL_SEE_DMD_RECORD
				/* not care record result */
				(void)bbox_diaginfo_record(MSPC_DIAG_INFO, NULL,
					"InSE error - mod_id %x; fi %x; fv %x; info0 %x; info1 %x",
					g_hisee_exception_modid, g_msg.data[HISEE_MNTN_IPC_DATA2],
					g_msg.data[HISEE_MNTN_IPC_DATA3], g_msg.data[HISEE_MNTN_IPC_DATA4],
					g_msg.data[HISEE_MNTN_IPC_DATA5]);
#endif
...
static void clear_mspc_diaginfo(void)
{
	g_index = MSPC_INDEX_0;

	(void)memset_s((void *)g_mspc_diaginfo, sizeof(g_mspc_diaginfo),
				   INVAILD, sizeof(g_mspc_diaginfo));
}

static int save_mspc_diaginfo(const struct mspc_diaginfo *diaginfo)
{
	enum mspc_report_data_index index;
	struct timespec64 ts_delta;

	ktime_get_ts64(&ts_delta);
	if (g_index != MSPC_INDEX_0 && timespec64_sub(ts_delta, g_time_base).tv_sec >= REPORT_TIME) {
		pr_err("entry time over %lld\n", timespec64_sub(ts_delta, g_time_base).tv_sec);
		clear_mspc_diaginfo();
	}
	if (g_index == MSPC_INDEX_0) {
		if (memcpy_s((void *)(&g_time_base), sizeof(struct timespec64),
			(void *)&ts_delta, sizeof(struct timespec64)) != EOK)
			return -1;
	}	
	if (g_index == MSPC_INDEX_MAX)
		return -1;
	for (index = MSPC_INDEX_0; index < g_index; index++) {
		if (g_mspc_diaginfo[index].info0 == diaginfo->info0 &&
		    g_mspc_diaginfo[index].info1 == diaginfo->info1) {
			pr_err("%s:report data repeat!\n", __func__);
			return -1;
		}
	}
	if (memcpy_s((void *)(&g_mspc_diaginfo[g_index]), sizeof(struct mspc_diaginfo),
				 (void *)diaginfo, sizeof(struct mspc_diaginfo)) != EOK) {
		pr_err("%s:memcpy data error!\n", __func__);
		return -1;
	}
	g_index++;
	return 0;
...
static int mspc_record_mspc_diaginfo(const struct mspc_diaginfo *diaginfo)
{
	...
	ret = save_mspc_diaginfo(diaginfo);
	if (ret != 0)
		return ret; 
			   ...
			   const char *pathname, pfn_cb_dump_done pfn_cb)
{
	...
	hisee_mntn_svc(HISEE_MNTN_ID, (u64)HISEE_SMC_GET_LOG,
					 (u64)0x0, (u64)0x0);
	ret = snprintf_s(path, HISEE_MNTN_PATH_MAXLEN,
			 HISEE_MNTN_PATH_MAXLEN - 1,
	...
	/* save hisee log to CONFIG_BBOX_LOG_DIR+time/hisee_log */
...
void hisee_mntn_make_data_full(void)
{
	...
	if (p_buf) { /* 0xab is clean value */
		...
		while (1) {
			/* save hisee log to CONFIG_BBOX_LOG_DIR+hisee/hisee_log */
...
#ifdef CONFIG_CRYPTO_CORE_FFA_SUPPORT
static int get_mspc_mntn_memory_data_from_dts(void)
{
	int ret;
	struct device_node *np = NULL;
	/*
	 * data[0] is the mspc mntn base address
	 * data[1] is the size of mspc mntn
	 */
	u32 data[2] = { 0 };

	np = of_find_compatible_node(NULL, NULL, "hisee-mntn");
	if (!np) {
		pr_err("%s: no hisee-mntn compatible node found.\n",
		       __func__);
		return -EPERM;
	}
	...
	if (ret != 0) {
		pr_err("%s: get reg node err.\n", __func__);
		return -EPERM;
	}

	g_mntn.base_addr = data[0];
	g_mntn.base_size = data[1];
...
#ifdef CONFIG_CRYPTO_CORE_FFA_SUPPORT
	ret = get_mspc_mntn_memory_data_from_dts();
	if (ret != 0) {
		pr_err("%s:get dts node failed\n", __func__);
		return ret;
	}
#endif
...
#ifdef CONFIG_CRYPTO_CORE_APM_KCA_NOTIFY
	/* init thread for apm */
	g_apm_thread = kthread_run(hisee_apm_ta_thread, NULL, "hisee_apm_ta_thread");
	if (IS_ERR(g_apm_thread))
		pr_err("apm kthread_create failed, ret=%ld\n", PTR_ERR(g_apm_thread));
#endif
