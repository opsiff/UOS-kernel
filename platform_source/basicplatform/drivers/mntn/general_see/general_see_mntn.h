 ...
 * Create: 2020-02-17
 */
#ifndef GENERAL_SEE_MNTN_H
#define GENERAL_SEE_MNTN_H
...
struct general_see_logbuf_header {
	u64 real_size;
};

struct general_see_logbuf_info {
	void *addr;
	u64 max_size;
	struct general_see_logbuf_header *header;
};
...
#ifdef CONFIG_CRYPTO_CORE_FFA_SUPPORT
#define HISEE_MNTN_ID           CRYPTO_CORE_FID_MNTN_VALUE
#else
...
#define LPM3_HISEE_MNTN    IPC_CMD(OBJ_AP, OBJ_GENERAL_SE, CMD_INQUIRY, HISEE_MNTN_TYPE)
#define HISEE_EXCEPTION    IPC_CMD(OBJ_LPM3, OBJ_GENERAL_SE, CMD_NOTIFY, HISEE_MSG_EXCEPTION)
#define HISEE_IRQ          IPC_CMD(OBJ_LPM3, OBJ_GENERAL_SE, CMD_NOTIFY, HISEE_MSG_IRQ)
#define HISEE_TIME         IPC_CMD(OBJ_LPM3, OBJ_GENERAL_SE, CMD_NOTIFY, HISEE_MSG_SYNC_TIME)
#define HISEE_LOG_OUT      IPC_CMD(OBJ_LPM3, OBJ_GENERAL_SE, CMD_NOTIFY, HISEE_MSG_LOG_OUT)
#define HISEE_VOTE_RES     IPC_CMD(OBJ_LPM3, OBJ_GENERAL_SE, CMD_NOTIFY, HISEE_MSG_VOTE_RES)
#define MSPC_RECORD        IPC_CMD(OBJ_LPM3, OBJ_GENERAL_SE, CMD_NOTIFY, MSPC_MSG_RECORD)
#define MSPC_SECFLASH_ERR  IPC_CMD(OBJ_LPM3, OBJ_GENERAL_SE, CMD_NOTIFY, MSPC_MSG_SECFLASH_ERR)
#define MSPC_SECFLASH_WARN IPC_CMD(OBJ_LPM3, OBJ_GENERAL_SE, CMD_NOTIFY, MSPC_MSG_SECFLASH_WARN)
#define MSPC_APM_KCA_NOTIFY IPC_CMD(OBJ_LPM3, OBJ_GENERAL_SE, CMD_NOTIFY, MSPC_MSG_APM_KCA_NOTIFY)
...
#else
static inline void hisee_mntn_debug_dump(void)
{
}

static inline int hisee_mntn_record_dmd_info(long dmd_no, const char *dmd_info)
{
	(void)dmd_no;
	(void)dmd_info;
	return 0;
