...
static u32 g_bl31_excep_offset;
...
u32 get_bl31_exception_flag(void)
{
	...
	if (sctrl_base)
		val = readl(sctrl_base + g_bl31_excep_offset);
...
static int __init dfx_bl31_panic_init(void)
{
	...
	uint32_t data[2] = { 0 };
	uint32_t bl31_smem_base, sctrl_addr_phy;
	...
	if (!np) {
		BB_PRINT_ERR("%s: no compatible sctrl_address node found\n", __func__);
		return -EPERM;
	}
	/* read bl31 crtl addr ande size from dts , 2 is addr and size count */
	ret = of_property_read_u32(np, "base_addr", &sctrl_addr_phy);
	if (ret) {
		BB_PRINT_ERR("%s , get sctrl base addr err\n", __func__);
		return -EPERM;
	}

	ret = of_property_read_u32(np, "bakdata5_offset", &g_bl31_excep_offset);
	if (ret) {
		BB_PRINT_ERR("%s , get sctrl base addr err\n", __func__);
		return -EPERM;
	}

	sctrl_base = (void *)ioremap(sctrl_addr_phy, SCTRL_REG_SIZE);
	if (!sctrl_base) {
	...
	if (!np) {
		BB_PRINT_ERR("%s: no compatible node bl31_mntn found\n", __func__);
		goto iounmap_sctrl;
	}
        /* read bl31 crtl addr ande size from dts , 2 is addr and size count */
	...
	if (!np) {
		BB_PRINT_ERR("%s: no compatible node bl31_share_address found\n", __func__);
		goto iounmap_sctrl;
	}

	ret = of_property_read_u32(np, "base_addr", &bl31_smem_base);
	if (ret) {
		BB_PRINT_ERR("%s , get bl31 mem base compatible node err\n", __func__);
		goto iounmap_sctrl;
	}
