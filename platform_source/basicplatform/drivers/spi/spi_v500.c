...
#include "sts.h"
static sts_sys_status_enum g_iodie_state = STS_ST_PRE_INITED;
static unsigned int g_sts_spi_cfg = 0;
#endif
...
static int iodie_sts_spi_setup(struct hs_spi_controller *spi_control)
{
	int ret = 0;
	struct chip_data *chip = spi_control->cur_chip;
	sts_spi_config_t spi_config = {0};
	unsigned int reg_val = 0;

	if (g_iodie_state != STS_ST_INITED) {
		pr_err("iomcu sts is not ready\n");
		if (spi_control->sctrlbase != NULL) {
			reg_val = readl(spi_control->sctrlbase + STS_CFG_BAKDATA_OFFSET);
			chip->cr = (chip->cr & SPI_RATIO_CLEAR) | (reg_val << SPI_RATIO_START);
		}
		return ret;
	}

	if (g_sts_spi_cfg == chip->cr)
		return ret;

	g_sts_spi_cfg = chip->cr;
	spi_config.speed = (chip->cr & SPI_RATIO_MASK) >> SPI_RATIO_START;

	ret = sts_spi_config(&spi_config);
	if (ret != 0) {
		pr_err("iodie sts spi config fail\n");
		return ret;
	}
	if (spi_control->sctrlbase != NULL)
		writel((spi_config.speed | STS_CFG_BAKDATA_HIGH_MASK),
			spi_control->sctrlbase + STS_CFG_BAKDATA_OFFSET);
...
#ifdef CONFIG_TCG_TIS_SPI
		if (message->state != STATE_DONE && spi_control->cur_transfer->cs_change)
			hs_spi_cs_control(spi_control, SSP_CHIP_DESELECT);
#endif
	}
out:
					...
					struct spi_message *msg)
{
	...
	if (spi_control->support_iodie) {
		if ((g_iodie_state == STS_ST_ERROR) || (iodie_sts_spi_setup(spi_control) != 0)) {
			msg->status = -EIO;
			giveback(spi_control);
			return -EIO;
		}
...
#ifdef CONFIG_TCG_TIS_SPI
	spi_control->cur_cs = msg->spi->cs_gpio;
#endif
	...
	struct hs_spi_controller *spi_ctrl)
{
	...
	if (sctrl_np == NULL) {
		dev_err(&pdev->dev, "fail to find sctrl node\n");
		return;
	}

	spi_ctrl->sctrlbase = of_iomap(sctrl_np, 0);
	if (spi_ctrl->sctrlbase == NULL) {
...
err_no_ioremap:
	if (spi_control->sctrlbase != NULL) {
		iounmap(spi_control->sctrlbase);
		spi_control->sctrlbase = NULL;
	}
