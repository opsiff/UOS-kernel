...
struct opt_regs g_v511_lvc_opt_regs[] = {
	/* reg, mask, shift, val, before,after */
	reg_cfg(0x2d3, 0xff, 0, 0x15, 0, 0),
	reg_cfg(0x2d4, 0xff, 0, 0x95, 0, 0),
	reg_cfg(0x2e1, 0xff, 0, 0x6C, 0, 0),
...
struct opt_regs g_v511_lvc_extra_opt_regs_1moh[] = {
	reg_cfg(0x2b9, 0xff, 0, 0x3c, 0, 0),
};
...
struct opt_regs g_v510_sc_opt_regs_after_enabled[] = {
	...
	reg_cfg(0x2bb, 0xff, 0, 0xF3, 20, 0),
};

struct opt_regs g_v511_sc_opt_regs[] = {
	/* reg, mask, shift, val, before,after */
	reg_cfg(0x2d3, 0xff, 0, 0x15, 0, 0),
	reg_cfg(0x2d4, 0xff, 0, 0x95, 0, 0),
	reg_cfg(0x2e1, 0xff, 0, 0x6C, 0, 0),
...
struct opt_regs g_v511_sc_opt_regs_after_enabled[] = {
	reg_cfg(0x2b9, 0xff, 0, 0x50, 0, 0),
	reg_cfg(0x2bb, 0xff, 0, 0xF3, 20, 0),
};
...
static void hi6526_dc_ibat_ocp_cfg(void)
{
	struct hi6526_device_info *di = get_hi6526_device();
	...
	if (di->param_dts.r_coul_uohm == R_UOHM_2K)
		hi6526_write_mask(CHG_IBAT_OCP_REG, CHG_IBAT_OCP_MSK,CHG_IBAT_OCP_SHIFT, 0);

	if (di->param_dts.r_coul_uohm == R_UOHM_1K)
		hi6526_write_mask(CHG_IBAT_OCP_REG, CHG_IBAT_OCP_MSK,CHG_IBAT_OCP_SHIFT, 1);
}
...
static void hi6526_lvc_opt_regs(void)
{
	...
	if ((di->hi6526_version == CHIP_VERSION_V100) || (di->hi6526_version == CHIP_VERSION_V200)){
		...
		hi6526_opt_regs_set(g_v511_lvc_opt_regs, ARRAY_SIZE(g_v511_lvc_opt_regs));
		hi6526_opt_regs_set(g_v511_lvc_extra_opt_regs_1moh, ARRAY_SIZE(g_v511_lvc_extra_opt_regs_1moh));
		hi6526_dc_ibat_ocp_cfg();
	}
}
...
static void hi6526_sc_opt_regs(void)
{
	...
	if ((di->hi6526_version == CHIP_VERSION_V100) || (di->hi6526_version == CHIP_VERSION_V200))
		...
		hi6526_opt_regs_set(g_v511_sc_opt_regs, ARRAY_SIZE(g_v511_sc_opt_regs));
}

static void hi6526_set_dc_vbat_ovp(void)
{
	u8 reg_val;
	struct hi6526_device_info *di = get_hi6526_device();
	...
	/* when sc_vbat_ovp is 0 use default configuration */
	if (di->param_dts.sc_vbat_ovp) {
		reg_val = (u8)((di->param_dts.sc_vbat_ovp - V510_CHG_DET_TOP_CFG_BASE) / V510_CHG_DET_TOP_CFG_STEP);
		hi6526_write_mask(V510_CHG_DET_TOP_CFG_REG, V510_CHG_DET_TOP_CFG_MASK, V510_CHG_DET_TOP_CFG_SHIFT, reg_val);
	}
}
					...
					int enable)
{
	...
	if (enable) {
		if (di->chg_mode == LVC) {
			...
			if ((di->hi6526_version == CHIP_VERSION_V100) || (di->hi6526_version == CHIP_VERSION_V200))
				hi6526_opt_regs_set(g_sc_opt_regs_after_enabled,ARRAY_SIZE(g_sc_opt_regs_after_enabled));
			else if (di->hi6526_version == CHIP_VERSION_V510)
				hi6526_opt_regs_set(g_v510_sc_opt_regs_after_enabled,ARRAY_SIZE(g_v510_sc_opt_regs_after_enabled));
			else
				hi6526_opt_regs_set(g_v511_sc_opt_regs_after_enabled,ARRAY_SIZE(g_v511_sc_opt_regs_after_enabled));
		} else {
