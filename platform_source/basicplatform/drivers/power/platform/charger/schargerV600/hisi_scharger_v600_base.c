...
void parse_dts(struct device_node *np, struct hi6526_device_info *di)
{
	...
	ret = of_property_read_u32(np, "sc_vbat_ovp",
				   (u32 *)&(di->param_dts.sc_vbat_ovp));
	if (ret) {
		scharger_err("get sc_vbat_ovp failed\n");
		di->param_dts.sc_vbat_ovp = 0;
	}
	scharger_inf("prase_dts sc_vbat_ovp=%d\n",
		     di->param_dts.sc_vbat_ovp);
