...
static inline void __chg_wdt_kick(const void __iomem *base)
{
...
void charge_feed_sys_wdt(unsigned int timeout)
{
	...
	watchdog_kick_cpu = (unsigned int)get_cpu();
	put_cpu();
	watchdog_kick_time = dfx_getcurtime();
...
static int hisi_chg_wdg_probe(struct platform_device *pdev)
{
	...
	if (!strstr(saved_command_line, "androidboot.swtype=factory") &&
		check_mntn_switch(MNTN_GLOBAL_WDT))
		g_watchdog_enable = 1;
	else
		g_watchdog_enable = 0;
