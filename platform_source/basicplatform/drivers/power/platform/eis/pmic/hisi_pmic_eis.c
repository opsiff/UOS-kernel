...
/*
 * eacr freq method imag coefficient when vol gain is 1, if vol gain is N, then
 * each term in this list would miltiply 1 / N, magnified 100000 TIMES
 */
static long long eacr_k_real[] = {
	-109890, -109890, 109890, 109890, -109890, -109890,
	109890, 109890, -109890, -109890, 109890, 109890
};
...
static void eis_pmic_set_work_flag(int flag)
{
	atomic_set(&g_work_flag, flag);
}

static int eis_pmic_read_work_flag(void)
{
...
_STATIC void eis_pmic_set_i_bat_gain(unsigned int i_gain)
{
	...
	if (locate_in_array(i_gain_ptr, i_gain_len, i_gain) == -1) {
		eis_err("i_gain = %d, invalid input, use max(default) gain\n", i_gain);
		return;
	}
...
_STATIC void eis_pmic_freq_set_curt_sample_thres(int batt, int cur_offset)
{
	int low, high;
	...
	} else if (r_uohm == R_OHM_500) {
		get_calibrated_ana_cur(CUR_FREQ_THRES_H_ERR_500UOHM, &high_err);
		get_calibrated_ana_cur(CUR_FREQ_THRES_L_ERR_500UOHM, &low_err);
	}else {
...
_STATIC void eis_pmic_irq_seq_clear(enum eis_irq_range irq)
{
	int ret;
	unsigned int i;
...
_STATIC int eis_pmic_irq_seq_read(enum eis_irq_range irq)
{
	u8 reg_val;
	unsigned int start, end;
...
_STATIC int eis_pmic_get_eis_det_flag(void)
{
...
_STATIC void eis_pmic_eis_enable(bool enable)
{
	u8 reg_val;
...
static void eis_pmic_get_raw_eis_ri(long long *p_r, long long *p_i,
	int v_gain, const int *raw_sample_vol_data, unsigned int len)
{
	...
	ret = eis_pmic_get_pure_vol_digit_data(
...
static void eis_pmic_eacr_correct_vol_data(int *pure_sample_vol_data,
	int *correct_sample_vol_data, int raw_len, int correct_len)
{
	int i, delta, delta_idx;

	delta = pure_sample_vol_data[correct_len / 2 - 1] -
		pure_sample_vol_data[raw_len / 2 - 1];
	delta_idx = (raw_len - correct_len) / 2;

	for (i = 0; i < correct_len / 2; i++)
		correct_sample_vol_data[i] = pure_sample_vol_data[i];

	for (i = correct_len / 2; i < correct_len; i++)
		correct_sample_vol_data[i] = pure_sample_vol_data[i + delta_idx] + delta;
}

static int eis_pmic_get_raw_eacr(
	int v_gain, const int *raw_sample_vol_data, unsigned int len)
{
	int ret;
	int j;
	long long r_t = 0;
	int v_tmp[V_BAT_FIFO_DEPTH];
	int v_correct[EACR_V_BAT_FIFO_DEPTH];

	if (raw_sample_vol_data == NULL	|| len > V_BAT_FIFO_DEPTH || v_gain == 0) {
		eis_err("input null or error\n");
		return -1;
	}
	...
	eis_pmic_eacr_correct_vol_data(v_tmp, v_correct, len, EACR_V_BAT_FIFO_DEPTH);
	for (j = 0; j < EACR_V_BAT_FIFO_DEPTH; j++)
		r_t += eacr_k_real[j] / v_gain * v_correct[j];
	return r_t;
}

_STATIC void eis_pmic_freq_start_sampling_init(unsigned int batt)
{
	unsigned int i;
	int ret;
...
static struct eis_freq_device_ops hisi_pmic_freq_ops = {
	...
	.set_work_flag = eis_pmic_set_work_flag,
	.read_work_flag = eis_pmic_read_work_flag,
	.get_raw_eis_ri = eis_pmic_get_raw_eis_ri,
	.get_raw_eacr_real = eis_pmic_get_raw_eacr,
};
...
static int eis_pmic_freq_chip_init(struct eis_pmic_device *di)
{
	...
	ret = eis_core_drv_ops_register(&hisi_pmic_freq_ops, FREQ_DRV_OPS, 0);
	if (ret != _SUCC) {
...
static int hisi_eis_core_chip_init(struct eis_pmic_device *di)
{
	...
	ret = eis_core_drv_ops_register(&hisi_eis_ops, EIS_OPS, 0);
	if (ret != _SUCC) {
...
eis_probe_err:
	...
	(void)eis_core_drv_ops_register(NULL, FREQ_DRV_OPS, 0);
	(void)eis_core_drv_ops_register(NULL, EIS_OPS, 0);
	eis_err("fail!\n");
	return -ENODEV;
}
