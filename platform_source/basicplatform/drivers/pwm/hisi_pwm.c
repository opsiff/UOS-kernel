...
static bool config_duty_check(const struct pwm_state *state, bool enabled,
...
static int hisi_pwm_apply_state(struct pwm_chip *chip, struct pwm_device *pwm,
	const struct pwm_state *state)
{
	int ret;

	ret = hisi_pwm_apply(chip, pwm, state);
	if ((ret == AUTO_CONFIG_DUTY) || (ret == NORMAL_FINISH))
		return 0;

	return ret;
}
#else
static int hisi_pwm_apply_read_state(struct pwm_chip *chip, struct pwm_device *pwm,
	struct pwm_state *state)
{
	int ret;
...
#if KERNEL_VERSION(5, 10, 0) <= LINUX_VERSION_CODE
	.apply = hisi_pwm_apply_state,
#else
	.apply = hisi_pwm_apply_read_state,
#endif
	.owner = THIS_MODULE,
};
