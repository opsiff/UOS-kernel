...
bool media_bypass_check(const char **bypass_chips, int bp_chip_num)
{
	uint32_t i;
	int32_t ret;
	const char *soc_spec = NULL;

	if (bypass_chips == NULL)
		return false;
	...
	if (ret) {
		pr_err("[%s] of_property_read_string soc_spec_set fail!\n", __func__);
		return false;
	}

	for (i = 0; i < bp_chip_num; i++) {
		if (bypass_chips[i] == NULL)
			continue;

		ret = strncmp(soc_spec, bypass_chips[i], strlen(bypass_chips[i]));
		if (ret == 0) {
			pr_err("[%s] chip: %s, need bypass media!\n", __func__, bypass_chips[i]);
			return true;
		}
	}
	return false;
