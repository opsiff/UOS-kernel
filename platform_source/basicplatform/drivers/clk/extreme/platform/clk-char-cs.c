...
#include "clk_suspend_resume.h"

#ifdef CONFIG_HIBERNATION
/**
 * @brief One notifier_block callback func to save reg data in
 * suspend stage, the other one is to store reg vale in resume
 * 
 */
static struct notifier_block clk_hibernate_nb_suspend = {
	.notifier_call = clk_hibernate_notify_suspend,
	.priority=-1
};

static struct notifier_block clk_hibernate_nb_resume = {
	.notifier_call = clk_hibernate_notify_resume,
	.priority=100
};
#endif
...
static const struct gate_clock crgctrl_gate_clks[] = {
	...
	{ CLK_GATE_BLPWM3, "clk_blpwm3", "clk_ptp_div", 0x20, 0x1000000, 0,
		NULL, 0, {0}, {0}, 0, 0, 0, "clk_blpwm3" },
	{ CLK_GATE_BLPWM4, "clk_blpwm4", "clk_ptp_div", 0x20, 0x800000, 0,
...
static const struct dvfs_clock perimem_dvs_clks[] = {
	...
	{ CLK_GATE_VDEC_MEM, "clk_vdec_mem", "clk_vdec_freq", 121, -1, 1, 1,
	...
	{ ACLK_GATE_VENC_MEM, "aclk_venc_mem", "aclk_venc_freq", 139, -1, 1, 1,
...
static const struct fast_dvfs_clock fast_dvfs_clks_media1[] = {
	...
	{ CLK_GATE_VDEC_FREQ, "clk_vdec_freq", { 0x064, 0x3c0, 6 }, { 0x064, 0x3f, 0 },
		{ 0x084, 1 }, { 0x000, 2 }, media1crg_plls, ARRAY_SIZE(media1crg_plls),
		{ 104000, 208000, 334000, 480000, 642000 }, 4, { 1, 1, 1, 2, 0 },
		{ 0x03C00080, 0x03C00080, 0x03C00080, 0x03C00100, 0x03C00040 }, { 16, 8, 5, 3, 1 },
		{ 0x003F000F, 0x003F0007, 0x003F0004, 0x003F0002, 0x003F0000 }, NO_ALWAYS_ON, "clk_vdec_freq" },
};
...
static struct media_pll_info fll_m2_info = MEDIA_PLL_INFO("clk_fll_src", 208906485);
static struct media_pll_info *media2crg_plls[] = {
	&ppll2b_m2_info, &ppll0_m2_info, &ppll1_m2_info, &ppll2_m2_info,
};

static struct media_pll_info *aclk_venc_plls[] = {
	&ppll2b_m2_info, &ppll0_m2_info, &ppll1_m2_info, &fll_m2_info,
};
...
static const struct fast_dvfs_clock fast_dvfs_clks_media2[] = {
	...
	{ ACLK_GATE_VENC_FREQ, "aclk_venc_freq", { 0x088, 0xF, 0 }, { 0x098, 0x3F, 0 },
		{ 0x0B8, 3 }, { 0x000, 4 }, aclk_venc_plls, ARRAY_SIZE(aclk_venc_plls),
		{ 104000, 334000, 417000, 642000 }, 3, { 1, 1, 1, 0 },
		{ 0x000F0002, 0x000F0002, 0x000F0002, 0x000F0001 }, { 16, 5, 4, 1 },
		{ 0x003F000F, 0x003F0004, 0x003F0003, 0x003F0000 }, NO_ALWAYS_ON, "aclk_venc_freq" },
};
...
static const char *media2_bypass_product[] = {
	"lite_pc",
};
...
#ifdef CONFIG_HIBERNATION
	int ret;
	ret=register_pm_notifier(&clk_hibernate_nb_suspend);
	if (ret) 
		pr_err("%s Camera clk_hibernate notifier suspend regist failed! ret: %d\n", __func__, ret);

	ret=register_pm_notifier(&clk_hibernate_nb_resume);
	if (ret) 
		pr_err("%s Camera clk_hibernate notifier resume regist failed! ret: %d\n", __func__, ret);
#endif
...
static void clk_media2_init(struct device_node *np)
{
	...
	bool media2_bypass = media_bypass_check(media2_bypass_product, ARRAY_SIZE(media2_bypass_product));
	if (no_media || media2_bypass)
