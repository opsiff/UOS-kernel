...
#ifdef CONFIG_CLK_LOW_TEMPERATURE_JUDGE_BY_VOLT
/* for get temprature */
...
static int clk_ip_avs_poll(struct peri_dvfs_clk *dfclk, bool flag)
{
	...
	clk_log_dbg("val = 0x%x, avs id = %d, clk id = %d, SCDATA24 = 0x%x, flag = %d!\n",
		val, dfclk->avs_poll_id, dfclk->id,
...
static int wait_avs_complete(struct peri_dvfs_clk *dfclk)
{
	...
	if (!val) {
		pr_err("[%s]:clk prepare wait for avs bitmask timeout, loop = %d, clk dfclk id = %d,"
			"pmctrl 0x350 = 0x%x, 0x354 = 0x%x, SCData24 = 0x%x!\n",
			__func__, loop, dfclk->id, readl(pvp->addr_0), readl(pvp->addr),
	...
	unsigned int volt)
{
	...
	if (dfclk->avs_poll_id > 0)
		clk_log_dbg("IP %s Old rate = %lu, Current rate= %lu!\n",
			clk_hw_get_name((struct clk_hw *)&(dfclk->hw)), freq_old, rate);
...
static int peri_dvfs_prepare(struct peri_dvfs_clk *pclk)
{
	...
	if (pclk->avs_poll_id > 0)
		clk_log_dbg("IP %s will be enable, current rate = %lu!\n",
			clk_hw_get_name((struct clk_hw *)&(pclk->hw)), cur_rate);
	...
	struct peri_dvfs_clk *devfreq_clk)
{
	...
	for (i = 0; i < dvfs->sensitive_level; i++) {
