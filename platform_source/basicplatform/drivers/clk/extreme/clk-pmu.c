	...
	struct hi3xxx_clkpmu *pclk)
{
	...
	np = of_find_compatible_node(NULL, NULL, "hisilicon,clk-extreme-pmu");
	if (np) {
		if (of_property_read_u32(np, "abb_scbakdata", &abb_scbakdata)) {
			pr_err("[%s] %s node doesn't have abb_scbakdata property!\n",
				__func__, np->name);
			return -ENODEV;
		}
		pclk->sctrl += abb_scbakdata; /* only for abb clk */
	} else {
		pr_err("[%s] fail to find clk-extreme-pmu node!\n", __func__);
		return -ENODEV;
	}
