	...
	struct hi3xxx_periclk *pclk)
{
	...
	if (pclk->peri_dvfs_sensitive) {
		...
		for (i = 0; i < pclk->sensitive_level; i++) {
...
#ifndef CONFIG_LIBLINUX
static void plat_clk_scgt_always_on(struct clk_gate *gate, int set, bool *ret)
{
...
#ifndef CONFIG_LIBLINUX
	if (!hck_flag) {
		REGISTER_HCK_VH(clk_scgt_always_on, plat_clk_scgt_always_on);
		hck_flag = true;
	}
#endif
}
