...
#include "clk-ppll.h"
#include <securec.h>
#ifdef CONFIG_PLL_VOTE_SEC
#include "clk-smc.h"
#endif
#ifdef CONFIG_PLL_LOCK_RETRY
...
static void ppll_disable_set_bypass_b1(struct hi3xxx_ppll_clk *ppll_clk)
{
	...
	case AUPLL:
		...
		break;
	default:
		pr_err("[%s]: A wrong PPLL-%u is set_bypass\n", __func__, ppll);
		break;
	}
}
...
static void timer_print_info(struct timer_list *t)
{
...
{
	__init_timer(&g_dmd_timer, timer_fn, 0);
