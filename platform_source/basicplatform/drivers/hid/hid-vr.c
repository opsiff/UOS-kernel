					...
					unsigned char report_type)
{
	...
	if (copy_to_user(buffer, buf, (size_t)len)) {
		ret = -EFAULT;
		goto out_free;
	}
...
static int hw_report_event(struct hid_device *hid, u8 *data, int len)
{
	struct hidraw *dev = NULL;
	struct hidraw_list *list = NULL;
	int ret = 0;
	...
	list_for_each_entry(list, &dev->list, node) {
		...
		list->buffer[list->head].value = kmemdup(data, (size_t)len, GFP_ATOMIC);
...
static int hw_connect(struct hid_device *hid)
{
	unsigned int minor;
	int result = 0;
	struct hidraw *dev = NULL;
...
static void hw_disconnect(struct hid_device *hid)
{
				...
				void __user *user_arg, struct file *file)
{
	...
	if (_IOC_NR(cmd) == _IOC_NR(HIDIOCGRAWNAME(0))) {
		...
		if (len > _IOC_SIZE(cmd))
			len = _IOC_SIZE(cmd);
		ret = (copy_to_user(user_arg, hid->name, len) != 0) ?
			-EFAULT : (long)len;
		return ret;
	}
	...
	if (_IOC_NR(cmd) == _IOC_NR(HIDIOCGRAWPHYS(0))) {
		...
		if (len > _IOC_SIZE(cmd))
			len = _IOC_SIZE(cmd);
		ret = (copy_to_user(user_arg, hid->phys, len) != 0) ?
			-EFAULT : (long)len;
		return ret;
	}
	return -ENOTTY;
