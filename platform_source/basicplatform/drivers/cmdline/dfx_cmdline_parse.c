...
/*
 * Description: Save spec info to kernel.
 *              The spec info passed by cmd line from bootloader.
 * Input:
 *        char *p -- cmd line node
 * Return:
 *        spec info
 */
static int __init early_parse_spec_info(char *p)
{
	return early_parse_sepc_info(p);
}
early_param("spec_info", early_parse_spec_info);
