...
static void wr_port_25_update_fail_logs_dbg(void __iomem *viraddr)
{
	...
	platform_dump_jpeg_regs("CVDR", viraddr, cvdr_offsets, ARRAY_SIZE(cvdr_offsets));
}

static void wr_port_25_update_suc_logs_dbg(void __iomem *viraddr)
{
	uint32_t cvdr_offsets[] = {
...
static void wr_port_id0_logs_dbg(void __iomem *viraddr)
{
	uint32_t cvdr_offsets[] = {
		...
		CVDR_AXI_JPEG_DEBUG_0_OFFSET,
		CVDR_AXI_JPEG_DEBUG_1_OFFSET,
		CVDR_AXI_JPEG_DEBUG_2_OFFSET,
		CVDR_AXI_JPEG_DEBUG_3_OFFSET,
...
static void wr_port_id30_logs_dbg(void __iomem *viraddr)
{
	uint32_t cvdr_offsets[] = {
...
static void wr_port_25_update_fail_logs_info(void __iomem *viraddr)
{
	uint32_t cvdr_offsets[] = {
		CVDR_SRT_VP_WR_CFG_25_OFFSET_UPDATE,
		CVDR_SRT_VP_WR_AXI_FS_25_OFFSET_UPDATE,
...
static void wr_port_id30_logs_info(void __iomem *viraddr)
{
	uint32_t cvdr_offsets[] = {
		CVDR_AXI_JPEG_VP_WR_CFG_30_OFFSET,
		CVDR_AXI_JPEG_VP_WR_AXI_FS_30_OFFSET,
...
static void wr_port_id4_logs_info(void __iomem *viraddr)
{
	uint32_t cvdr_offsets[] = {
		CVDR_AXI_JPEG_NR_RD_CFG_4,
...
int dump_cvdr_debug_reg(hjpeg_hw_ctl_t *hw_ctl)
{
	...
	if (!hw_ctl) {
		cam_err("%s:hw_ctl is NULL", __func__);
		return -1;
	}
	...
	if (hw_ctl->use_platform_config) {
		platform_dump_cvdr_debug_reg(hw_ctl);
		return 0;
	}
...
static void hjpeg_vp_wr_axi_line_config(void __iomem *cvdr_srt_base,
...
static void hjpeg_vp_wr_axi_line_update(void __iomem *cvdr_srt_base,
	cvdr_wr_fmt_desc_t *desc, u32 offset)
{
...
static void hjpeg_vp_wr_axi_line_update3(void __iomem *cvdr_srt_base,
	cvdr_wr_fmt_desc_t *desc, u32 offset)
{
...
static void hjpeg_config_axi_line(hjpeg_hw_ctl_t *hw_ctl, cvdr_wr_fmt_desc_t *desc)
{
	int wr_port_flag;
	...
	default: {
		cam_err("%s, invalid rd port[%d]", __func__,
...
static void hjpeg_config_vp_wr_axi_fs_for32(void __iomem *cvdr_srt_base,
	u32 offset, u32 addr)
{
...
static void hjpeg_config_vp_wr_axi_fs_for34(void __iomem *cvdr_srt_base,
	u32 offset, u32 addr)
{
...
static void hjpeg_config_axi_fs(hjpeg_hw_ctl_t *hw_ctl, jpgenc_config_t *config)
{
	u32 buf_addr;
	u32 wr_port;
...
static void hjpeg_vp_wr_cfg_for32(void __iomem *cvdr_srt_base,
	cvdr_wr_fmt_desc_t *desc, u32 offset)
{
	u_vp_wr_cfg tmp_cfg;
...
static void hjpeg_vp_wr_cfg_for34(void __iomem *cvdr_srt_base,
	cvdr_wr_fmt_desc_t *desc, u32 offset)
{
...
static void hjpeg_vp_wr_cfg_for34_fmt_update(void __iomem *cvdr_srt_base,
	cvdr_wr_fmt_desc_t *desc, u32 offset)
{
...
static void hjpeg_config_vp_wr(hjpeg_hw_ctl_t *hw_ctl, cvdr_wr_fmt_desc_t *desc)
{
	int iova_update_flag;
	u32 wr_port;
...
static int set_nr_rd_config(hjpeg_hw_ctl_t *hw_ctl)
{
	...
	tmp.reg32 = get_reg_val((void __iomem *)((char *)cvdr_srt_base +
		nr_rd_cfg_offset));
	cam_info("%s: get CVDR reg: 0x%08x=0x%08x", __func__,
			 nr_rd_cfg_offset, tmp.reg32);
	lmt.reg32 = get_reg_val((void __iomem *)((char *)cvdr_srt_base +
		limiter_nr_rd_offset));
	cam_info("%s: get CVDR reg: 0x%08x=0x%08x", __func__,
			 limiter_nr_rd_offset, lmt.reg32);
	if ((hw_ctl->cvdr_prop.flag != 0) && rd_port == RD_PORT_4) {
	...
	cam_info("%s: set CVDR reg: 0x%08x=0x%08x",
			 __func__, nr_rd_cfg_offset, tmp.reg32);
	...
	cam_info("%s: set CVDR reg: 0x%08x=0x%08x",
			 __func__, limiter_nr_rd_offset, lmt.reg32);

	return 0;
}

static void config_cvdr_max_wr_id(void __iomem *cvdr_base_addr)
{
	u32 tmpVal;
	...
	cam_info("%s: CVDR reg: 0x%08x=0x%08x",
		__func__, CVDR_AXI_JPEG_CVDR_CFG,
		get_reg_val((void __iomem *)(
		(char *)cvdr_base_addr + CVDR_AXI_JPEG_CVDR_CFG)));
}

static void config_cvdr_wr_qos(void __iomem *cvdr_base_addr)
{
...
static void config_cvdr_rd_qos(void __iomem *cvdr_base_addr)
{
...
void hjpeg_config_cvdr(hjpeg_hw_ctl_t *hw_ctl, jpgenc_config_t *config)
{
	...
	if (hw_ctl->use_platform_config != 0) {
		cam_info("%s: use platform config cvdr", __func__);
		platform_config_jpeg_cvdr(hw_ctl, config);
		return;
	}
	...
	cvdr_base_addr = hw_ctl->cvdr_viraddr;
	if (cvdr_base_addr) {
		// set jpeg cvdr 0x131F2000 for hisp230 performance
		set_reg_val((void __iomem*)((char*)cvdr_base_addr + CVDR_AXI_JPEG_CVDR_CFG) , 0x131F2000);
	}
#endif
