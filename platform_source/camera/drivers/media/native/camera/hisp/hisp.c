...
struct daemon_ref_count {
	pid_t pid;
	int ref_counts;
};
static DEFINE_MUTEX(client_maps_mtx);
static struct daemon_ref_count hisp_client_id_to_daemon_pid[MAX_HISP_CLIENT];
static int hisp_allocate_client_id(pid_t pid)
{
	int cid;
	int rc = -EBUSY;
	mutex_lock(&client_maps_mtx);
	for (cid = 0; cid < MAX_HISP_CLIENT; ++cid)
		if (hisp_client_id_to_daemon_pid[cid].pid == pid) {
			rc = 0;
			goto out;
		}

	for (cid = 0; cid < MAX_HISP_CLIENT; ++cid)
		if (hisp_client_id_to_daemon_pid[cid].pid == 0) {
			hisp_client_id_to_daemon_pid[cid].pid = pid;
			rc = 0;
			goto out;
		}
...
out:
	if (!rc) {
		++hisp_client_id_to_daemon_pid[cid].ref_counts;
		cam_info("allocate daemon %d client_id: %d refs: %d", pid, cid,
				hisp_client_id_to_daemon_pid[cid].ref_counts);
	} else {
		cam_warn("allocate daemon %d client_id failed, client full", pid);
	}
	mutex_unlock(&client_maps_mtx);
	return rc;
}

static void hisp_free_client_id(pid_t pid)
{
	int cid;
	mutex_lock(&client_maps_mtx);
	for (cid = 0; cid < MAX_HISP_CLIENT; ++cid)
		if (hisp_client_id_to_daemon_pid[cid].pid == pid) {
			--hisp_client_id_to_daemon_pid[cid].ref_counts;
			if (hisp_client_id_to_daemon_pid[cid].ref_counts == 0)
				hisp_client_id_to_daemon_pid[cid].pid = 0;
			cam_info("free daemon %d client_id: %d refs: %d", pid, cid, hisp_client_id_to_daemon_pid[cid].ref_counts);
		}
	mutex_unlock(&client_maps_mtx);
}
...
int hisp_get_client_refs(pid_t pid)
{
	...
	for (cid = 0; cid < MAX_HISP_CLIENT; ++cid)
		if (hisp_client_id_to_daemon_pid[cid].pid == pid) {
			refs = hisp_client_id_to_daemon_pid[cid].ref_counts;
			mutex_unlock(&client_maps_mtx);
			return refs;
		}
	mutex_unlock(&client_maps_mtx);
...
static bool hisp_is_valid_daemon(pid_t pid)
{
	int cid;
	mutex_lock(&client_maps_mtx);
	for (cid = 0; cid < MAX_HISP_CLIENT; ++cid)
...
static void hisp_notify_rpmsg_cb(hisp_notify_intf_t *i, hisp_event_t *hisp_ev)
{
	...
	client_id = hisp_ev->kind >> HISP_MESSAGE_ID_CLIENT_BIT;
	cam_debug("enter %s, client_id: %d", __func__, client_id);
	...
	ev.id = HISP_HIGH_PRIO_EVENT | (client_id << HISP_MESSAGE_ID_CLIENT_BIT);

	req->kind = hisp_ev->kind & ((1 << HISP_MESSAGE_ID_CLIENT_BIT) - 1);
...
static long hisp_ioctl_select(unsigned int cmd, hisp_t *isp, void *arg)
{
	...
	case HISP_IOCTL_SEND_RPMSG:
		...
		if (isp && isp->hw && isp->hw->vtbl) {
			if (!hisp_is_valid_daemon(current->tgid)) {
				cam_err("invalid SEND_RPMSG usr(cur %d)", current->tgid);
				return -EINVAL;
			}
	...
	case HISP_IOCTL_RECV_RPMSG:
		...
		if (isp && isp->hw && isp->hw->vtbl) {
			if (!hisp_is_valid_daemon(current->tgid)) {
				cam_err("invalid RECV_RPMSG usr(cur %d)", current->tgid);
				return -EINVAL;
			}
...
__weak void hisp_clear_msgs(int client_id)
{
	(void)client_id;
}
									   ...
									   struct v4l2_fh *fh, struct v4l2_event_subscription *sub)
{
	...
	cam_info("enter %s, client_id: %d", __func__, client_id);
	sub->id |= (client_id << HISP_MESSAGE_ID_CLIENT_BIT);
	hisp_clear_msgs(client_id);
	/* 128:size */
										 ...
										 struct v4l2_fh *fh, struct v4l2_event_subscription *sub)
{
	...
	hisp_notify_intf_rpmsg_cb(&isp->notify, &isp_ev);
...
static int hisp_subdev_open(struct v4l2_subdev *sd, struct v4l2_subdev_fh *fh)
{
	...
	if (!rc)
		rc = hisp_allocate_client_id(current->tgid);

	if (rc && isp->hw && isp->hw->vtbl && isp->hw->vtbl->close)
		isp->hw->vtbl->close(isp->hw);
...
static int hisp_subdev_close(struct v4l2_subdev *sd, struct v4l2_subdev_fh *fh)
{
	...
	hisp_free_client_id(current->tgid);
...
static int get_isp_dt_aclk(struct device_node *dev_node, hisp_dt_data_t *dt,
					struct device *dev)
{
	int ret;
...
static int get_isp_dt_aclk_dss(struct device_node *dev_node, hisp_dt_data_t *dt,
						struct device *dev)
{
	int ret;
	const char *clk_name = NULL;
...
static int get_isp_dt_pclk_dss(struct device_node *dev_node, hisp_dt_data_t *dt,
						struct device *dev)
{
	int ret;
	const char *clk_name = NULL;
...
static int set_isp_dt_parameter(struct device *dev, hisp_dt_data_t *dt)
{
	int ret = 0;
