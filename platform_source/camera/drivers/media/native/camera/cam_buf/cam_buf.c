...
static void *cam_buf_map_kernel(int fd)
{
...
static void cam_buf_unmap_kernel(int fd)
{
	struct device *dev = NULL;
...
static int cam_buf_sync(int fd, struct sync_format *fmt)
{
	if (fd < 0 || !fmt) {
		cam_err("%s: fd or fmt invalid", __func__);
		return -EINVAL;
...
static int cam_buf_local_sync(int fd, struct local_sync_format *fmt)
{
	if (fd < 0 || !fmt) {
		cam_err("%s: fd or fmt invalid", __func__);
		return -EINVAL;
...
static int cam_buf_open_security_ta(void)
{
...
static int cam_buf_set_secboot(struct cam_buf_cfg *cfg)
{
	if (g_cam_secmem_type != 1) {
		cam_err("%s: sec_mem_type = %d", __func__,
				g_cam_secmem_type);
...
static int cam_buf_release_secboot(struct cam_buf_cfg *cfg)
{
	(void)cfg;
	if (g_cam_secmem_type != 1) {
		cam_err("%s: sec_mem_type = %d", __func__,
...
#if (defined(HISP300_CAMERA) || defined(HISP350_CAMERA) || defined(HISP500_CAMERA)) && \
...
static struct cam_buf_device *cam_buf_device_create(struct platform_device *pdev)
{
	int ret;
...
static void cam_buf_device_destroy(struct cam_buf_device *idev)
{
...
static ssize_t cam_buf_info_show(struct device *dev,
