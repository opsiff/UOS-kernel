...
int hw_buck_get_dt_data(struct hw_buck_ctrl_t *buck_ctrl)
{
	...
	rc = of_property_read_u32(dev_node, "vendor,en-pin", &buck_info->en_pin);
	cam_info("%s vendor,en-pin %d", __func__, buck_info->en_pin);
	if (rc < 0)
		cam_err("%s, get en-pin failed %d", __func__, __LINE__);

	rc = of_property_read_u32(dev_node, "vendor,vsel-pin", &buck_info->pin_vsel);
	cam_info("%s vendor,vsel-pin %d", __func__, buck_info->pin_vsel);
	if (rc < 0)
		cam_err("%s, get vsel-pin failed %d", __func__, __LINE__);

	(void)of_property_read_u32(dev_node, "vendor,vsel-range", &buck_info->vsel_range);
	cam_info("%s vendor,vsel-range %d", __func__, buck_info->vsel_range);
