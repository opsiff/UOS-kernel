...
int __weak lpm_adc_get_current(int adc_channel)
{
	pr_err("lpm_adc_get_current is invalide;\n");
	return -1;
}
	...
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	for (i = 0; i < p_ldo->ldo_num; i++) {
		...
		for (j = 0; j < LDO_RUN_COUNT; j++) {
			cur_val = lpm_adc_get_current(p_ldo->ldo_channel[i]);
			if (cur_val < 0) {
...
static int hw_rt_get_ldo_data(void)
{
	...
	if (runmode_is_factory()) {
		...
		of_node = of_find_node_by_path("/vendor,camera_ldo");
		if (!of_node)
			return -1;
		...
		if (p_ldo->ldo_num > 0) {
			ret = of_property_read_u32_array(of_node,
				"rear-ldo-channel",
				(u32 *)&p_ldo->ldo_channel[0], p_ldo->ldo_num);
			if (ret < 0) {
				cam_err("%s failed %d\n", __func__, __LINE__);
				return ret;
			}
			ret = of_property_read_u32_array(of_node,
				"rear-ldo-threshold",
				(u32 *)&p_ldo->ldo_threshold[0], p_ldo->ldo_num);
			if (ret < 0) {
				cam_err("%s failed %d\n", __func__, __LINE__);
				return ret;
			}
