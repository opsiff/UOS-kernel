...
#define KTD2688A_FLASH_TIMEOUT_TIME 0x0F /* 1600ms KTD2687 MAX time-out */
...
#define flash_current_to_level_ktd2688a(x) (((32 * (x)) / 375) - 1)
#define torch_current_to_level_ktd2688a(x) (((128 * (x)) / 375) - 1)
	...
	struct hw_flash_cfg_data *cdata)
{
	...
	case KTD2687:
		...
		break;
	case KTD2688A:
		led1_cur_calc = flash_current_to_level_ktd2688a(led1_cur);
		led2_cur_calc = flash_current_to_level_ktd2688a(led2_cur);
		break;
	case LM3644TT:
	...
	struct hw_lm3644tt_mix_ir_private_data_t *pdata)
{
	...
	/* driver ic: LM3644TT: 1, AW3644: 2, KTD2687: 3, KTD2688A: 4 */
	cam_info("%s led_type = %d, flash driver ic is %u", __func__,
		pdata->led_type, g_lm3644tt_mix_ir_pdata.chip_type);
	...
	case KTD2688A:
		led1_cur_calc =
			torch_current_to_level_ktd2688a(led1_cur);
		led2_cur_calc =
			torch_current_to_level_ktd2688a(led2_cur);
		break;
	case LM3644TT:
	...
	unsigned char reg_mode)
{
	...
	case KTD2687:
		...
		break;
	case KTD2688A:
		flash_time = KTD2688A_FLASH_TIMEOUT_TIME;
		break;
	case LM3644TT:
	case AW3644:
...
static int hw_lm3644tt_mix_ir_match(struct hw_flash_ctrl_t *flash_ctrl)
{
	...
	if (id == LM3644TT_CHIP_ID) {
		...
		if (mode == MODE_TEMP_VALUE) {
			...
			int ktd_ic_type = -1;
			int rc = of_property_read_u32(flash_ctrl->dev->of_node, "vendor,ktd_ic_type",
				(u32 *)&ktd_ic_type);
			if (ktd_ic_type == KTD2688A) {
				g_lm3644tt_mix_ir_pdata.chip_type = KTD2688A;
				cam_info("%s 0x%x mode=0x%x chip type is KTD2688A",
					__func__, id, mode);
			} else {
