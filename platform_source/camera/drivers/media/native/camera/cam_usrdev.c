 ...
 * Description: cam usr dev source file.
 * Create: 2016-04-01
 *
...
#include <securec.h>
#if (KERNEL_VERSION(4, 4, 0) <= LINUX_VERSION_CODE) /* 4: kernel version */
...
static int create_media_device(struct device *dev, struct video_device *vdev,
...
#if (KERNEL_VERSION(4, 9, 0) <= LINUX_VERSION_CODE) /* 4,9: kernel version */
...
static void destroy_media_device(struct video_device *vdev,
						  struct media_device *mdev)
{
...
static void cam_dev_format_name(struct video_device *vdev, const char *prefix)
{
	int rc;
					  ...
					  struct v4l2_device *v4l2, cam_dev_t *cam)
{
	...
	strlcpy(vdev->name + strlen(vdev->name), "cam-userdev",
			sizeof(vdev->name) - strlen(vdev->name));
#if (KERNEL_VERSION(4, 9, 0) <= LINUX_VERSION_CODE) /* 4,9: kernel version */
...
static int create_video_device(struct device *dev, struct video_device **pvdev,
						struct v4l2_device *v4l2, cam_dev_t *cam,
						struct media_device *mdev)
{
	int rc;
