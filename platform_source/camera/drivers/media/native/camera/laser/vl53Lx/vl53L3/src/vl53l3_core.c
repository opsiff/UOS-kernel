	...
	VL53L3_DEV        Dev)
{
	...
	pdev->version.ll_major    = VL53L3_LL_API_IMPLEMENTATION_VER_MAJOR;
	pdev->version.ll_minor    = VL53L3_LL_API_IMPLEMENTATION_VER_MINOR;
	pdev->version.ll_build    = VL53L3_LL_API_IMPLEMENTATION_VER_SUB;
	pdev->version.ll_revision = VL53L3_LL_API_IMPLEMENTATION_VER_REVISION;
}


void  VL53L3_init_ll_driver_state(
	VL53L3_DEV         Dev,
	VL53L3_DeviceState device_state)
{
	...
	pstate->cfg_gph_id        = VL53L3_GROUPEDPARAMETERHOLD_ID_MASK;
	pstate->cfg_timing_status = 0;
	pstate->cfg_zone_id       = 0;

	pstate->rd_device_state   = device_state;
	pstate->rd_stream_count   = 0;
	pstate->rd_gph_id         = VL53L3_GROUPEDPARAMETERHOLD_ID_MASK;
...
VL53L3_Error  VL53L3_update_ll_driver_rd_state(
	VL53L3_DEV         Dev)
{
	...
	if ((pdev->sys_ctrl.system__mode_start &
		VL53L3_DEVICEMEASUREMENTMODE_MODE_MASK) == 0x00) {

		pstate->rd_device_state  = VL53L3_DEVICESTATE_SW_STANDBY;
		pstate->rd_stream_count  = 0;
		...
		status = VL53L3_update_internal_stream_counters(Dev,
			pstate->rd_stream_count,
		...
		case VL53L3_DEVICESTATE_SW_STANDBY:

			if ((pdev->dyn_cfg.system__grouped_parameter_hold &
				VL53L3_GROUPEDPARAMETERHOLD_ID_MASK) > 0) {
				...
				if (pstate->rd_zone_id >=
					...
					pstate->rd_device_state =
					VL53L3_DEVICESTATE_RANGING_OUTPUT_DATA;
				else
					pstate->rd_device_state =
					VL53L3_DEVICESTATE_RANGING_GATHER_DATA;
			}
		...
		case VL53L3_DEVICESTATE_RANGING_WAIT_GPH_SYNC:
			pstate->rd_stream_count = 0;
			pstate->rd_internal_stream_count = 0;
			pstate->rd_internal_stream_count_val = 0;
			pstate->rd_zone_id      = 0;
...
VL53L3_Error VL53L3_check_ll_driver_rd_state(
	VL53L3_DEV         Dev)
{
	...
	VL53L3_ll_driver_state_t  *pstate       = &(pdev->ll_state);
	VL53L3_system_results_t   *psys_results = &(pdev->sys_results);
	VL53L3_histogram_bin_data_t *phist_data = &(pdev->hist_data);
	VL53L3_zone_private_dyn_cfgs_t *pZ = &(pres->zone_dyn_cfgs);
	...
	device_gph_id = (psys_results->result__interrupt_status &
		VL53L3_INTERRUPT_STATUS__GPH_ID_INT_STATUS_MASK) >> 4;
	...
	if (pstate->rd_device_state ==
		...
		if (histogram_mode == 0) {
			...
			VL53L3_DEVICEERROR_GPHSTREAMCOUNT0READY)
				status =
				VL53L3_ERROR_GPH_SYNC_CHECK_FAIL;

		}
	} else {
		if (pstate->rd_stream_count != device_stream_count)
			status = VL53L3_ERROR_STREAM_COUNT_CHECK_FAIL;


		if (pstate->rd_gph_id != device_gph_id)
			status = VL53L3_ERROR_GPH_ID_CHECK_FAIL;




		expected_stream_count =
		pZ->VL53L3_p_002[pstate->rd_zone_id].expected_stream_count;
		expected_gph_id =
		pZ->VL53L3_p_002[pstate->rd_zone_id].expected_gph_id;
		...
		if (expected_stream_count != device_stream_count) {
			...
			if (!((pdev->zone_cfg.active_zones == 0) &&
				...
				status =
				VL53L3_ERROR_ZONE_STREAM_COUNT_CHECK_FAIL;
		...
		if (expected_gph_id != device_gph_id)
			status = VL53L3_ERROR_ZONE_GPH_ID_CHECK_FAIL;
...
VL53L3_Error  VL53L3_update_ll_driver_cfg_state(
	VL53L3_DEV         Dev)
{
	...
	if ((pdev->sys_ctrl.system__mode_start &
		...
		pstate->cfg_device_state  = VL53L3_DEVICESTATE_SW_STANDBY;
		pstate->cfg_stream_count  = 0;
		...
		status = VL53L3_update_internal_stream_counters(
			Dev,
			pstate->cfg_stream_count,
		...
		case VL53L3_DEVICESTATE_SW_STANDBY:
			...
			pstate->cfg_device_state =
					VL53L3_DEVICESTATE_RANGING_DSS_AUTO;
			break;

		case VL53L3_DEVICESTATE_RANGING_DSS_AUTO:
			pstate->cfg_zone_id++;
			if (pstate->cfg_zone_id >
				pdev->zone_cfg.active_zones) {
		...
		case VL53L3_DEVICESTATE_RANGING_DSS_MANUAL:
			pstate->cfg_zone_id++;
			if (pstate->cfg_zone_id >
				pdev->zone_cfg.active_zones) {
				pstate->cfg_zone_id = 0;
		...
		default:
			...
			pstate->cfg_internal_stream_count_val = 0;
			pstate->cfg_gph_id =
					VL53L3_GROUPEDPARAMETERHOLD_ID_MASK;
			pstate->cfg_timing_status = 0;
			pstate->cfg_zone_id       = 0;
			break;
		}
	...
	if (pdev->zone_cfg.active_zones == 0) {
		...
		pZ->VL53L3_p_002[prev_cfg_zone_id].expected_stream_count
			= prev_cfg_stream_count - 1;

		pZ->VL53L3_p_002[pstate->rd_zone_id].expected_gph_id =
			prev_cfg_gph_id ^ VL53L3_GROUPEDPARAMETERHOLD_ID_MASK;
	} else {
		pZ->VL53L3_p_002[prev_cfg_zone_id].expected_stream_count
			= prev_cfg_stream_count;
		pZ->VL53L3_p_002[prev_cfg_zone_id].expected_gph_id =
			prev_cfg_gph_id;
	}
...
void VL53L3_copy_rtn_good_spads_to_buffer(
...
void VL53L3_init_system_results(
...
void VL53L3_init_zone_results_structure(
	uint8_t                 active_zones,
	VL53L3_zone_results_t  *pdata)
{
	...
	for (z = 0; z < pdata->max_zones; z++) {
		pobjects = &(pdata->VL53L3_p_002[z]);
		pobjects->cfg_device_state = VL53L3_DEVICESTATE_SW_STANDBY;
		pobjects->rd_device_state  = VL53L3_DEVICESTATE_SW_STANDBY;
		pobjects->max_objects      = VL53L3_MAX_RANGE_RESULTS;
		pobjects->active_objects   = 0;
	}
}

void VL53L3_init_zone_dss_configs(
	VL53L3_DEV              Dev)
{
	...
	VL53L3_LLDriverResults_t  *pres =
			VL53L3DevStructGetLLResultsHandle(Dev);
	uint8_t  z = 0;
	uint8_t max_zones    = VL53L3_MAX_USER_ZONES;
	VL53L3_zone_private_dyn_cfgs_t *pdata = &(pres->zone_dyn_cfgs);

	for (z = 0; z < max_zones; z++) {
		pdata->VL53L3_p_002[z].dss_mode =
			VL53L3_DSS_CONTROL__MODE_TARGET_RATE;
		pdata->VL53L3_p_002[z].dss_requested_effective_spad_count = 0;
	}
}


void VL53L3_init_histogram_config_structure(
	uint8_t   even_bin0,
	uint8_t   even_bin1,
	uint8_t   even_bin2,
	uint8_t   even_bin3,
	...
	uint8_t   odd_bin4,
	uint8_t   odd_bin5,
	VL53L3_histogram_config_t  *pdata)
{
...
void VL53L3_init_histogram_multizone_config_structure(
	uint8_t   even_bin0,
	uint8_t   even_bin1,
	uint8_t   even_bin2,
	uint8_t   even_bin3,
...
void VL53L3_init_xtalk_bin_data_struct(
	uint32_t                        bin_value,
	uint16_t                        VL53L3_p_024,
	VL53L3_xtalk_histogram_shape_t *pdata)
{
	...
	pdata->VL53L3_p_023               = VL53L3_XTALK_HISTO_BINS;
	pdata->VL53L3_p_024            = (uint8_t)VL53L3_p_024;
...
void VL53L3_i2c_encode_uint16_t(
	uint16_t    ip_value,
	uint16_t    count,
	uint8_t    *pbuffer)
{
	...
	uint16_t   i    = 0;
	uint16_t   VL53L3_p_002 = 0;

	VL53L3_p_002 =  ip_value;

	for (i = 0; i < count; i++) {
		pbuffer[count-i-1] = (uint8_t)(VL53L3_p_002 & 0x00FF);
		VL53L3_p_002 = VL53L3_p_002 >> 8;
	}
}

uint16_t VL53L3_i2c_decode_uint16_t(
	uint16_t    count,
	uint8_t    *pbuffer)
{
...
void VL53L3_i2c_encode_int16_t(
	int16_t     ip_value,
	uint16_t    count,
	uint8_t    *pbuffer)
{
...
int16_t VL53L3_i2c_decode_int16_t(
	uint16_t    count,
	uint8_t    *pbuffer)
{
...
void VL53L3_i2c_encode_uint32_t(
	uint32_t    ip_value,
	uint16_t    count,
	uint8_t    *pbuffer)
{
...
uint32_t VL53L3_i2c_decode_uint32_t(
	uint16_t    count,
	uint8_t    *pbuffer)
{
...
uint32_t VL53L3_i2c_decode_with_mask(
	uint16_t    count,
	uint8_t    *pbuffer,
	uint32_t    bit_mask,
	uint32_t    down_shift,
...
void VL53L3_i2c_encode_int32_t(
	int32_t     ip_value,
	uint16_t    count,
	uint8_t    *pbuffer)
{
...
int32_t VL53L3_i2c_decode_int32_t(
	uint16_t    count,
	uint8_t    *pbuffer)
{
...
VL53L3_Error VL53L3_start_test(
	VL53L3_DEV    Dev,
...
VL53L3_Error VL53L3_set_firmware_enable_register(
	VL53L3_DEV    Dev,
	uint8_t       value)
{
	...
	status = VL53L3_WrByte(
				Dev,
				VL53L3_FIRMWARE__ENABLE,
				pdev->sys_ctrl.firmware__enable);

	return status;
}

VL53L3_Error VL53L3_enable_firmware(
	VL53L3_DEV    Dev)
{
	...
	status = VL53L3_set_firmware_enable_register(Dev, 0x01);
...
VL53L3_Error VL53L3_disable_firmware(
	VL53L3_DEV    Dev)
{
	...
	status = VL53L3_set_firmware_enable_register(Dev, 0x00);
...
VL53L3_Error VL53L3_set_powerforce_register(
	VL53L3_DEV    Dev,
	uint8_t       value)
{
	...
	status = VL53L3_WrByte(
			Dev,
			VL53L3_POWER_MANAGEMENT__GO1_POWER_FORCE,
...
VL53L3_Error VL53L3_enable_powerforce(
	VL53L3_DEV    Dev)
{
	...
	status = VL53L3_set_powerforce_register(Dev, 0x01);
...
VL53L3_Error VL53L3_disable_powerforce(
	VL53L3_DEV    Dev)
{
	...
	status = VL53L3_set_powerforce_register(Dev, 0x00);
...
VL53L3_Error VL53L3_clear_interrupt(
	VL53L3_DEV    Dev)
{
	...
	pdev->sys_ctrl.system__interrupt_clear = VL53L3_CLEAR_RANGE_INT;

	status = VL53L3_WrByte(
				Dev,
				VL53L3_SYSTEM__INTERRUPT_CLEAR,
...
VL53L3_Error VL53L3_force_shadow_stream_count_to_zero(
	VL53L3_DEV    Dev)
{
	...
	if (status == VL53L3_ERROR_NONE) {
		status = VL53L3_WrByte(
				Dev,
				VL53L3_SHADOW_RESULT__STREAM_COUNT,
				0x00);
	}
...
uint32_t VL53L3_calc_macro_period_us(
	...
	uint8_t   VL53L3_p_009)
{
	...
	pll_period_us = VL53L3_calc_pll_period_us(fast_osc_frequency);



	VL53L3_p_031 = VL53L3_decode_vcsel_period(VL53L3_p_009);



	macro_period_us =
			(uint32_t)VL53L3_MACRO_PERIOD_VCSEL_PERIODS *
			pll_period_us;
	macro_period_us = macro_period_us >> 6;

	macro_period_us = macro_period_us * (uint32_t)VL53L3_p_031;
	macro_period_us = macro_period_us >> 6;
...
uint16_t VL53L3_calc_range_ignore_threshold(
...
uint32_t VL53L3_calc_timeout_mclks(
	uint32_t timeout_us,
...
uint16_t VL53L3_calc_encoded_timeout(
	uint32_t timeout_us,
	uint32_t macro_period_us)
{
	...
	timeout_mclks   =
		VL53L3_calc_timeout_mclks(timeout_us, macro_period_us);

	timeout_encoded =
		VL53L3_encode_timeout(timeout_mclks);
...
uint32_t VL53L3_calc_timeout_us(
...
uint32_t VL53L3_calc_crosstalk_plane_offset_with_margin(
...
uint32_t VL53L3_calc_decoded_timeout_us(
	...
	uint32_t macro_period_us)
{
	...
	timeout_mclks =
		VL53L3_decode_timeout(timeout_encoded);

	timeout_us    =
		VL53L3_calc_timeout_us(timeout_mclks, macro_period_us);
...
uint16_t VL53L3_encode_timeout(uint32_t timeout_mclks)
{
...
uint32_t VL53L3_decode_timeout(uint16_t encoded_timeout)
{
...
VL53L3_Error VL53L3_calc_timeout_register_values(
	uint32_t                 phasecal_config_timeout_us,
	uint32_t                 mm_config_timeout_us,
	uint32_t                 range_config_timeout_us,
	uint16_t                 fast_osc_frequency,
	VL53L3_general_config_t *pgeneral,
	VL53L3_timing_config_t  *ptiming)
{
	...
	if (fast_osc_frequency == 0) {
		...
		timeout_encoded = VL53L3_calc_encoded_timeout(
							range_config_timeout_us,
							macro_period_us);
...
uint8_t VL53L3_encode_vcsel_period(uint8_t VL53L3_p_031)
{


	uint8_t vcsel_period_reg = 0;

	vcsel_period_reg = (VL53L3_p_031 >> 1) - 1;

	return vcsel_period_reg;
}


uint32_t VL53L3_decode_unsigned_integer(
	uint8_t  *pbuffer,
	uint8_t   no_of_bytes)
{
...
void VL53L3_encode_unsigned_integer(
	uint32_t  ip_value,
	uint8_t   no_of_bytes,
	uint8_t  *pbuffer)
{
	...
	VL53L3_p_002 = ip_value;
	for (i = 0; i < no_of_bytes; i++) {
		pbuffer[no_of_bytes-i-1] = VL53L3_p_002 & 0x00FF;
		VL53L3_p_002 = VL53L3_p_002 >> 8;
	}
}


VL53L3_Error  VL53L3_hist_copy_and_scale_ambient_info(
	VL53L3_zone_hist_info_t       *pidata,
	VL53L3_histogram_bin_data_t   *podata)
{
...
void  VL53L3_hist_get_bin_sequence_config(
	VL53L3_DEV                     Dev,
	VL53L3_histogram_bin_data_t   *pdata)
{
	...
	for (i = 0; i < VL53L3_MAX_BIN_SEQUENCE_LENGTH; i++)
		pdata->bin_rep[i] = 1;
...
VL53L3_Error  VL53L3_hist_phase_consistency_check(
	VL53L3_DEV                   Dev,
	VL53L3_zone_hist_info_t     *phist_prev,
	VL53L3_zone_objects_t       *prange_prev,
	VL53L3_range_results_t      *prange_curr)
{
	...
	VL53L3_DeviceError  range_status = 0;
	...
	for (lc = 0; lc < prange_curr->active_results; lc++) {
		...
		if (!((prange_curr->VL53L3_p_002[lc].range_status ==
			VL53L3_DEVICEERROR_RANGECOMPLETE) ||
			(prange_curr->VL53L3_p_002[lc].range_status ==
		...
		if (prange_prev->active_objects == 0)
			prange_curr->VL53L3_p_002[lc].range_status =
			VL53L3_DEVICEERROR_PREV_RANGE_NO_TARGETS;
		else
			prange_curr->VL53L3_p_002[lc].range_status =
				VL53L3_DEVICEERROR_PHASECONSISTENCY;
		...
		for (p = 0; p < prange_prev->active_objects; p++) {
			...
			if (prange_curr->VL53L3_p_002[lc].VL53L3_p_014 >
				prange_prev->VL53L3_p_002[p].VL53L3_p_014) {
				phase_delta =
				prange_curr->VL53L3_p_002[lc].VL53L3_p_014 -
				prange_prev->VL53L3_p_002[p].VL53L3_p_014;
			} else {
				phase_delta =
				prange_prev->VL53L3_p_002[p].VL53L3_p_014 -
			...
			if (phase_delta < phase_tolerance) {
				...
				if (status == VL53L3_ERROR_NONE)
					status =
					VL53L3_hist_events_consistency_check(
					event_sigma,
					event_min_spad_count,
					phist_prev,
					&(prange_prev->VL53L3_p_002[p]),
					&(prange_curr->VL53L3_p_002[lc]),
					&events_tolerance,
					&events_delta,
					&range_status);
				...
				if (status == VL53L3_ERROR_NONE &&
					...
					VL53L3_DEVICEERROR_RANGECOMPLETE)
					status =
					VL53L3_hist_merged_pulse_check(
					min_max_tolerance,
					&(prange_curr->VL53L3_p_002[lc]),
					&range_status);
...
VL53L3_Error  VL53L3_hist_events_consistency_check(
	uint8_t                      event_sigma,
	uint16_t                     min_effective_spad_count,
	VL53L3_zone_hist_info_t     *phist_prev,
	VL53L3_object_data_t        *prange_prev,
	VL53L3_range_data_t         *prange_curr,
	int32_t                     *pevents_tolerance,
	int32_t                     *pevents_delta,
	VL53L3_DeviceError          *prange_status)
{
	...
	if (event_sigma == 0) {
		*prange_status = VL53L3_DEVICEERROR_RANGECOMPLETE;
		return status;
	}
	...
	tmpc *= (int64_t)prange_curr->VL53L3_p_006;
	...
	c_signal_events  = (int64_t)prange_curr->VL53L3_p_021;
	c_signal_events -= (int64_t)prange_curr->VL53L3_p_020;
	c_signal_events *= (int64_t)events_scaler;
	c_signal_events += 2048;
	c_signal_events /= 4096;

	c_sig_noise_sq  = (int64_t)events_scaler_sq;
	c_sig_noise_sq *= (int64_t)prange_curr->VL53L3_p_021;
	c_sig_noise_sq += 2048;
	c_sig_noise_sq /= 4096;

	c_amb_noise_sq  = (int64_t)events_scaler_sq;
	c_amb_noise_sq *= (int64_t)prange_curr->VL53L3_p_020;
	c_amb_noise_sq += 2048;
	c_amb_noise_sq /= 4096;
	...
	p_amb_noise_sq  =
		(int64_t)prange_prev->VL53L3_p_020;
	...
	noise_sq_sum =
		(uint32_t)prange_prev->VL53L3_p_021 +
		(uint32_t)c_sig_noise_sq +
		(uint32_t)p_amb_noise_sq +
		(uint32_t)c_amb_noise_sq;

	*pevents_tolerance =
		(int32_t)VL53L3_isqrt(noise_sq_sum * 16);

	*pevents_tolerance *= (int32_t)event_sigma;
	*pevents_tolerance += 32;
	*pevents_tolerance /= 64;

	p_signal_events  = (int32_t)prange_prev->VL53L3_p_021;
	p_signal_events -= (int32_t)prange_prev->VL53L3_p_020;
	...
	if (*pevents_delta > *pevents_tolerance &&
		prange_curr->VL53L3_p_006 > min_effective_spad_count)
		*prange_status = VL53L3_DEVICEERROR_EVENTCONSISTENCY;
	else
		*prange_status = VL53L3_DEVICEERROR_RANGECOMPLETE;
...
VL53L3_Error  VL53L3_hist_merged_pulse_check(
	...
	VL53L3_DeviceError          *prange_status)
{
	...
	if (min_max_tolerance_mm > 0 &&
		...
		*prange_status = VL53L3_DEVICEERROR_RANGECOMPLETE_MERGED_PULSE;
	else
		*prange_status = VL53L3_DEVICEERROR_RANGECOMPLETE;
...
VL53L3_Error  VL53L3_hist_xmonitor_consistency_check(
	VL53L3_DEV                   Dev,
	VL53L3_zone_hist_info_t     *phist_prev,
	VL53L3_zone_objects_t       *prange_prev,
	VL53L3_range_data_t         *prange_curr)
{
	...
	if (prange_curr->range_status == VL53L3_DEVICEERROR_RANGECOMPLETE ||
		prange_curr->range_status ==
			VL53L3_DEVICEERROR_RANGECOMPLETE_NO_WRAP_CHECK ||
		prange_curr->range_status ==
				VL53L3_DEVICEERROR_EVENTCONSISTENCY) {
...
VL53L3_Error  VL53L3_hist_wrap_dmax(
	VL53L3_hist_post_process_config_t  *phistpostprocess,
	VL53L3_histogram_bin_data_t        *pcurrent,
	int16_t                            *pwrap_dmax_mm)
{
	...
	*pwrap_dmax_mm = 0;
	...
	if (pcurrent->VL53L3_p_019 != 0) {



		pll_period_mm =
			VL53L3_calc_pll_period_mm(
				pcurrent->VL53L3_p_019);
...
void VL53L3_hist_combine_mm1_mm2_offsets(
	...
	VL53L3_additional_offset_cal_data_t  *pcal_data,
	uint8_t                              *pgood_spads,
...
VL53L3_Error VL53L3_hist_xtalk_extract_calc_window(
	...
	VL53L3_hist_xtalk_extract_data_t   *pxtalk_data)
{
	...
	pxtalk_data->pll_period_mm =
		VL53L3_calc_pll_period_mm(phist_bins->VL53L3_p_019);
	...
	pxtalk_data->VL53L3_p_015 =
		(uint8_t)(pxtalk_data->xtalk_start_phase / 2048);


	pxtalk_data->VL53L3_p_016 =
	...
	if (pxtalk_data->VL53L3_p_016 > (pxtalk_data->target_start-1))
		pxtalk_data->VL53L3_p_016 = pxtalk_data->target_start-1;


	pxtalk_data->effective_width =
			(2048 * ((int32_t)pxtalk_data->VL53L3_p_016+1));
...
VL53L3_Error VL53L3_hist_xtalk_extract_calc_event_sums(
	VL53L3_histogram_bin_data_t        *phist_bins,
	VL53L3_hist_xtalk_extract_data_t   *pxtalk_data)
{
	...
	for (lb  = pxtalk_data->VL53L3_p_015;
		 lb <= pxtalk_data->VL53L3_p_016;
		 lb++) {


		i = (lb + phist_bins->number_of_ambient_bins +
			phist_bins->VL53L3_p_024) %
					phist_bins->VL53L3_p_024;


		pxtalk_data->signal_events_sum += phist_bins->bin_data[i];
		pxtalk_data->signal_events_sum -=
				phist_bins->VL53L3_p_004;
	}



	for (lb  = 0; lb < VL53L3_XTALK_HISTO_BINS  &&
			lb < phist_bins->VL53L3_p_024; lb++) {
...
VL53L3_Error VL53L3_hist_xtalk_extract_calc_rate_per_spad(
	VL53L3_hist_xtalk_extract_data_t   *pxtalk_data)
{
...
VL53L3_Error VL53L3_hist_xtalk_extract_calc_shape(
	VL53L3_hist_xtalk_extract_data_t  *pxtalk_data,
	VL53L3_xtalk_histogram_shape_t    *pxtalk_shape)
{
	...
	pxtalk_shape->VL53L3_p_022      = 0;
	pxtalk_shape->VL53L3_p_023    = VL53L3_XTALK_HISTO_BINS;
	pxtalk_shape->VL53L3_p_024 = VL53L3_XTALK_HISTO_BINS;
	...
	for (lb = 0; lb < VL53L3_XTALK_HISTO_BINS; lb++) {

		if ((lb < (int32_t)pxtalk_data->VL53L3_p_015 ||
			 lb > (int32_t)pxtalk_data->VL53L3_p_016)  ||
...
VL53L3_Error VL53L3_hist_xtalk_shape_model(
	...
	VL53L3_xtalk_histogram_shape_t  *pxtalk_shape)
{
	...
	for (lb = 0; lb < VL53L3_XTALK_HISTO_BINS; lb++) {
		...
		if (lb == bin_start && lb == bin_stop) {
			VL53L3_p_008 =
			VL53L3_hist_xtalk_shape_model_interp(
				events_per_bin,
		...
		pxtalk_shape->bin_data[lb] = VL53L3_p_008;
	}
...
uint16_t VL53L3_hist_xtalk_shape_model_interp(
	uint16_t      events_per_bin,
	uint32_t      phase_delta)
{


	uint32_t  VL53L3_p_008  = 0;

	LOG_FUNCTION_START("");


	VL53L3_p_008  = (uint32_t)events_per_bin * phase_delta;
	VL53L3_p_008 +=  1024;
	VL53L3_p_008 /=  2048;


	if (VL53L3_p_008 > 0xFFFFU)
		VL53L3_p_008 = 0xFFFFU;

	LOG_FUNCTION_END(0);

	return (uint16_t)VL53L3_p_008;
}


void VL53L3_spad_number_to_byte_bit_index(
	uint8_t  spad_number,
	uint8_t *pbyte_index,
	uint8_t *pbit_index,
	uint8_t *pbit_mask)
...
void VL53L3_decode_zone_limits(
	...
	int16_t  *py_ur)
{
	...
	*px_ur = *px_ll + (int16_t)width;
	if (*px_ur > (VL53L3_SPAD_ARRAY_WIDTH-1))
		*px_ur = VL53L3_SPAD_ARRAY_WIDTH-1;
	...
	*py_ur = *py_ll + (int16_t)height;
	if (*py_ur > (VL53L3_SPAD_ARRAY_HEIGHT-1))
		*py_ur = VL53L3_SPAD_ARRAY_HEIGHT-1;
}


uint8_t VL53L3_is_aperture_location(
	uint8_t row,
	uint8_t col)
{
...
void VL53L3_calc_max_effective_spads(
	uint8_t     encoded_zone_centre,
	uint8_t     encoded_zone_size,
	uint8_t    *pgood_spads,
	uint16_t    aperture_attenuation,
	...
	*pmax_effective_spads = 0;
	...
	for (y = zone_y_ll; y <= zone_y_ur; y++) {
		for (x = zone_x_ll; x <= zone_x_ur; x++) {
			...
			VL53L3_spad_number_to_byte_bit_index(
				spad_number,
				&byte_index,
				&bit_index,
				&bit_mask);
			...
			if ((pgood_spads[byte_index] & bit_mask) > 0) {
				...
				is_aperture = VL53L3_is_aperture_location(
					(uint8_t)y,
					(uint8_t)x);
...
void VL53L3_calc_mm_effective_spads(
	uint8_t     encoded_mm_roi_centre,
	uint8_t     encoded_mm_roi_size,
	uint8_t     encoded_zone_centre,
	uint8_t     encoded_zone_size,
...
void VL53L3_hist_copy_results_to_sys_and_core(
	VL53L3_histogram_bin_data_t      *pbins,
	VL53L3_range_results_t           *phist,
	...
	VL53L3_core_results_t            *pcore)
{
	...
	VL53L3_init_system_results(psys);
	...
	for (i = 0; i < phist->active_results; i++) {
		...
		case 0:
			...
			psys->result__sigma_sd0 = pdata->VL53L3_p_005;
			psys->result__phase_sd0 = pdata->VL53L3_p_014;
		...
		case 1:
			...
			psys->result__sigma_sd1 = pdata->VL53L3_p_005;
			psys->result__phase_sd1 = pdata->VL53L3_p_014;
...
VL53L3_Error VL53L3_sum_histogram_data(
		VL53L3_histogram_bin_data_t *phist_input,
		VL53L3_histogram_bin_data_t *phist_output)
{
	...
	if (status == VL53L3_ERROR_NONE) {
		if (phist_output->VL53L3_p_024 >=
				phist_input->VL53L3_p_024)
			smallest_bin_num = phist_input->VL53L3_p_024;
		else
			smallest_bin_num = phist_output->VL53L3_p_024;
	}
	...
	if (status == VL53L3_ERROR_NONE)
		phist_output->VL53L3_p_004 +=
			phist_input->VL53L3_p_004;
...
VL53L3_Error VL53L3_avg_histogram_data(
			uint8_t no_of_samples,
			VL53L3_histogram_bin_data_t *phist_sum,
			VL53L3_histogram_bin_data_t *phist_avg)
{
	...
	if (status == VL53L3_ERROR_NONE) {
		for (i = 0; i < phist_sum->VL53L3_p_024; i++) {
	...
	if (status == VL53L3_ERROR_NONE) {
		if (no_of_samples > 0)
			phist_avg->VL53L3_p_004 =
				phist_sum->VL53L3_p_004 /
					(int32_t)no_of_samples;
		else
			phist_avg->VL53L3_p_004 =
					phist_sum->VL53L3_p_004;
...
VL53L3_Error VL53L3_save_cfg_data(
	VL53L3_DEV  Dev)
{
...
VL53L3_Error VL53L3_dynamic_zone_update(
	VL53L3_DEV  Dev,
	VL53L3_range_results_t *presults)
{
	...
	uint16_t eff_spad_cnt =
		pZ->VL53L3_p_002[zone_id].dss_requested_effective_spad_count;
#endif

	LOG_FUNCTION_START("");

	pZ->VL53L3_p_002[zone_id].dss_requested_effective_spad_count = 0;

	trace_print(VL53L3_TRACE_LEVEL_DEBUG,
		"    DYNZONEUPDATE: peak signal count rate mcps:");

	trace_print(VL53L3_TRACE_LEVEL_DEBUG,
		"%u actual effective spads: %u\n",
		presults->VL53L3_p_002[0].peak_signal_count_rate_mcps,
		presults->VL53L3_p_002[0].VL53L3_p_006);
	...
	max_total_rate_per_spads =
		presults->VL53L3_p_002[0].total_rate_per_spad_mcps;
	...
	for (i = 1; i < presults->active_results; i++) {
		...
		presults->VL53L3_p_002[i].total_rate_per_spad_mcps);

		if (presults->VL53L3_p_002[i].total_rate_per_spad_mcps >
			max_total_rate_per_spads)
			max_total_rate_per_spads =
			presults->VL53L3_p_002[i].total_rate_per_spad_mcps;
...
VL53L3_Error VL53L3_multizone_hist_bins_update(
	VL53L3_DEV  Dev)
{
	...
	VL53L3_histogram_config_t *phist_cfg = &(pdev->hist_cfg);
	VL53L3_histogram_config_t *pmulti_hist =
	...
	if (pzone_cfg->bin_config[pdev->ll_state.cfg_zone_id] ==
		...
		VL53L3_ZONECONFIG_BINCONFIG__MIDAMB) {
		trace_print (VL53L3_TRACE_LEVEL_DEBUG,
	...
	if (status == VL53L3_ERROR_NONE) {
		VL53L3_copy_hist_bins_to_static_cfg(
			phist_cfg,
			&(pdev->stat_cfg),
			&(pdev->tim_cfg));
	}
...
VL53L3_Error VL53L3_update_internal_stream_counters(
	VL53L3_DEV  Dev,
...
VL53L3_Error VL53L3_set_histogram_multizone_initial_bin_config(
	VL53L3_zone_config_t		*pzone_cfg,
	VL53L3_histogram_config_t	*phist_cfg,
	VL53L3_histogram_config_t	*pmulti_hist)
{
	VL53L3_Error  status = VL53L3_ERROR_NONE;
...
uint8_t	VL53L3_encode_GPIO_interrupt_config(
...
VL53L3_GPIO_interrupt_config_t VL53L3_decode_GPIO_interrupt_config(
	uint8_t		system__interrupt_config)
{
...
VL53L3_Error VL53L3_set_GPIO_distance_threshold(
	VL53L3_DEV                      Dev,
...
VL53L3_Error VL53L3_set_GPIO_rate_threshold(
	VL53L3_DEV                      Dev,
	uint16_t			threshold_high,
	uint16_t			threshold_low)
{
...
VL53L3_Error VL53L3_set_GPIO_thresholds_from_struct(
	VL53L3_DEV                      Dev,
	VL53L3_GPIO_interrupt_config_t *pintconf)
{
	VL53L3_Error  status = VL53L3_ERROR_NONE;
	...
	status = VL53L3_set_GPIO_distance_threshold(
			Dev,
	...
	if (status == VL53L3_ERROR_NONE) {
		status =
			VL53L3_set_GPIO_rate_threshold(
				Dev,
...
VL53L3_Error VL53L3_set_ref_spad_char_config(
	VL53L3_DEV    Dev,
	...
	uint16_t      fast_osc_frequency)
{
	...
	if (status == VL53L3_ERROR_NONE)
		status =
			VL53L3_WrByte(
				Dev,
				VL53L3_PHASECAL_CONFIG__TIMEOUT_MACROP,
				pdev->gen_cfg.phasecal_config__timeout_macrop);
	...
	if (status == VL53L3_ERROR_NONE)
		status =
			VL53L3_WrByte(
				Dev,
				VL53L3_RANGE_CONFIG__VCSEL_PERIOD_A,
	...
	if (status == VL53L3_ERROR_NONE)
		status =
			VL53L3_WrWord(
				Dev,
				VL53L3_REF_SPAD_CHAR__TOTAL_RATE_TARGET_MCPS,
				total_rate_target_mcps);
	...
	if (status == VL53L3_ERROR_NONE)
		status =
			VL53L3_WrWord(
				Dev,
				VL53L3_RANGE_CONFIG__SIGMA_THRESH,
	...
	if (status == VL53L3_ERROR_NONE)
		status =
			VL53L3_WrWord(
			Dev,
			VL53L3_RANGE_CONFIG__MIN_COUNT_RATE_RTN_LIMIT_MCPS,
...
VL53L3_Error VL53L3_set_ssc_config(
	VL53L3_DEV            Dev,
	VL53L3_ssc_config_t  *pssc_cfg,
	uint16_t              fast_osc_frequency)
{
	...
	if (status == VL53L3_ERROR_NONE)
		status =
			VL53L3_WrByte(
				Dev,
				VL53L3_CAL_CONFIG__VCSEL_START,
	...
	if (status == VL53L3_ERROR_NONE)
		status =
			VL53L3_WrByte(
				Dev,
				VL53L3_GLOBAL_CONFIG__VCSEL_WIDTH,
	...
	buffer[2] = pssc_cfg->VL53L3_p_009;
	...
	if (status == VL53L3_ERROR_NONE)
		status =
			VL53L3_WriteMulti(
				Dev,
				VL53L3_RANGE_CONFIG__TIMEOUT_MACROP_B_HI,
				buffer,
				5);



	buffer[0] = pssc_cfg->VL53L3_p_009;
	buffer[1] = pssc_cfg->VL53L3_p_009;
...
VL53L3_Error VL53L3_get_spad_rate_data(
	VL53L3_DEV                Dev,
	VL53L3_spad_rate_data_t  *pspad_rates)
{
	...
	uint8_t  VL53L3_p_002[512];
	uint8_t *pdata = &VL53L3_p_002[0];
	...
	if (status == VL53L3_ERROR_NONE)
		status =
			VL53L3_ReadMulti(
				Dev,
				VL53L3_PRIVATE__PATCH_BASE_ADDR_RSLV,
				pdata,
				512);


	pdata = &VL53L3_p_002[0];
	for (i = 0; i < VL53L3_NO_OF_SPAD_ENABLES; i++) {
		pspad_rates->rate_data[i] =
			(uint16_t)VL53L3_decode_unsigned_integer(pdata, 2);
		pdata += 2;
	}



	pspad_rates->VL53L3_p_023     = VL53L3_NO_OF_SPAD_ENABLES;
	pspad_rates->no_of_values    = VL53L3_NO_OF_SPAD_ENABLES;
...
VL53L3_Error VL53L3_dynamic_xtalk_correction_calc_required_samples(
	VL53L3_DEV                          Dev
	)
{
	...
	VL53L3_smudge_corrector_config_t *pconfig =
				&(pdev->smudge_correct_config);
	VL53L3_smudge_corrector_internals_t *pint =
				&(pdev->smudge_corrector_internals);

	VL53L3_range_results_t *presults = &(pres->range_results);
	VL53L3_range_data_t *pxmonitor = &(presults->xmonitor);
	...
	temp64a = pxmonitor->VL53L3_p_021 +
		pxmonitor->VL53L3_p_020;
	temp64a = do_division_u((temp64a * 1000), peak_duration_us);
	temp64a = do_division_u((temp64a * 1000), peak_duration_us);

	temp64z = pconfig->noise_margin * pxmonitor->VL53L3_p_006;
	if (temp64z == 0)
		temp64z = 1;
...
VL53L3_Error VL53L3_dynamic_xtalk_correction_calc_new_xtalk(
	VL53L3_DEV				Dev,
	uint32_t				xtalk_offset_out,
	VL53L3_smudge_corrector_config_t	*pconfig,
	VL53L3_smudge_corrector_data_t		*pout,
	uint8_t					add_smudge,
	uint8_t					soft_update
	)
{
	...
	VL53L3_xtalk_config_t  *pX = &(pdev->xtalk_cfg);
	VL53L3_xtalk_calibration_results_t  *pC = &(pdev->xtalk_cal);
	uint32_t *pcpo;
...
#define CONT_RESET	2
VL53L3_Error VL53L3_dynamic_xtalk_correction_corrector(
	VL53L3_DEV                          Dev
	)
{
	...
	VL53L3_range_results_t  *pR = &(pres->range_results);
	VL53L3_xtalk_config_t  *pX = &(pdev->xtalk_cfg);
	...
	VL53L3_dynamic_xtalk_correction_output_init(pres);
	...
	if ((pR->xmonitor.range_status
		...
		for (i = 0; i < pR->active_results; i++) {
			if (pR->VL53L3_p_002[i].range_status ==
				VL53L3_DEVICEERROR_RANGECOMPLETE) {
				if (pR->VL53L3_p_002[i].median_range_mm
						<=
		...
		amr =
		pR->VL53L3_p_002[nodetect_index].ambient_count_rate_mcps;
	...
	if (run_smudge_detection) {
		...
		VL53L3_dynamic_xtalk_correction_calc_required_samples(Dev);


		xtalk_offset_in =
			pR->xmonitor.VL53L3_p_012;
		...
		if (continue_processing == CONT_CONTINUE) {
			...
			VL53L3_dynamic_xtalk_correction_calc_new_xtalk(
				Dev,
				xtalk_offset_out,
				pconfig,
				pout,
...
VL53L3_Error VL53L3_dynamic_xtalk_correction_data_init(
	VL53L3_DEV                          Dev
	)
{
	...
	pdev->smudge_correct_config.smudge_corr_single_apply  =
		VL53L3_TUNINGPARM_DYNXTALK_SMUDGE_COR_SINGLE_APPLY_DEFAULT;

	pdev->smudge_correct_config.smudge_margin =
		VL53L3_TUNINGPARM_DYNXTALK_SMUDGE_MARGIN_DEFAULT;
	pdev->smudge_correct_config.noise_margin =
		VL53L3_TUNINGPARM_DYNXTALK_NOISE_MARGIN_DEFAULT;
	pdev->smudge_correct_config.user_xtalk_offset_limit =
		VL53L3_TUNINGPARM_DYNXTALK_XTALK_OFFSET_LIMIT_DEFAULT;
	pdev->smudge_correct_config.user_xtalk_offset_limit_hi =
		VL53L3_TUNINGPARM_DYNXTALK_XTALK_OFFSET_LIMIT_HI_DEFAULT;
	pdev->smudge_correct_config.sample_limit =
		VL53L3_TUNINGPARM_DYNXTALK_SAMPLE_LIMIT_DEFAULT;
	pdev->smudge_correct_config.single_xtalk_delta =
		VL53L3_TUNINGPARM_DYNXTALK_SINGLE_XTALK_DELTA_DEFAULT;
	pdev->smudge_correct_config.averaged_xtalk_delta =
		VL53L3_TUNINGPARM_DYNXTALK_AVERAGED_XTALK_DELTA_DEFAULT;
	pdev->smudge_correct_config.smudge_corr_clip_limit =
		VL53L3_TUNINGPARM_DYNXTALK_CLIP_LIMIT_DEFAULT;
	pdev->smudge_correct_config.smudge_corr_ambient_threshold =
		VL53L3_TUNINGPARM_DYNXTALK_XTALK_AMB_THRESHOLD_DEFAULT;
	pdev->smudge_correct_config.scaler_calc_method =
		0;
	pdev->smudge_correct_config.x_gradient_scaler =
		VL53L3_TUNINGPARM_DYNXTALK_XGRADIENT_SCALER_DEFAULT;
	pdev->smudge_correct_config.y_gradient_scaler =
		VL53L3_TUNINGPARM_DYNXTALK_YGRADIENT_SCALER_DEFAULT;
	pdev->smudge_correct_config.user_scaler_set =
		VL53L3_TUNINGPARM_DYNXTALK_USER_SCALER_SET_DEFAULT;
	pdev->smudge_correct_config.nodetect_ambient_threshold =
		VL53L3_TUNINGPARM_DYNXTALK_NODETECT_AMB_THRESHOLD_KCPS_DEFAULT;
	pdev->smudge_correct_config.nodetect_sample_limit =
		VL53L3_TUNINGPARM_DYNXTALK_NODETECT_SAMPLE_LIMIT_DEFAULT;
	pdev->smudge_correct_config.nodetect_xtalk_offset =
		VL53L3_TUNINGPARM_DYNXTALK_NODETECT_XTALK_OFFSET_KCPS_DEFAULT;
	pdev->smudge_correct_config.nodetect_min_range_mm =
		VL53L3_TUNINGPARM_DYNXTALK_NODETECT_MIN_RANGE_MM_DEFAULT;
	pdev->smudge_correct_config.max_smudge_factor =
		VL53L3_TUNINGPARM_DYNXTALK_MAX_SMUDGE_FACTOR_DEFAULT;
...
VL53L3_Error VL53L3_dynamic_xtalk_correction_output_init(
	VL53L3_LLDriverResults_t *pres
	)
{
	...
	VL53L3_smudge_corrector_data_t *pdata;
...
VL53L3_Error VL53L3_xtalk_cal_data_init(
	VL53L3_DEV                          Dev
	)
{
...
VL53L3_Error VL53L3_low_power_auto_data_init(
	VL53L3_DEV                          Dev
	)
{
	...
	pdev->low_power_auto_data.vhv_loop_bound =
		VL53L3_TUNINGPARM_LOWPOWERAUTO_VHV_LOOP_BOUND_DEFAULT;
...
VL53L3_Error VL53L3_low_power_auto_data_stop_range(
	VL53L3_DEV                          Dev
	)
{
...
VL53L3_Error VL53L3_config_low_power_auto_mode(
	VL53L3_general_config_t   *pgeneral,
	VL53L3_dynamic_config_t   *pdynamic,
	...
	)
{
	...
	pdynamic->system__sequence_config =
			VL53L3_SEQUENCE_VHV_EN |
			VL53L3_SEQUENCE_PHASECAL_EN |
			VL53L3_SEQUENCE_DSS1_EN |



			VL53L3_SEQUENCE_RANGE_EN;


	pgeneral->dss_config__manual_effective_spads_select = 200 << 8;
	pgeneral->dss_config__roi_mode_control =
		VL53L3_DEVICEDSSMODE__REQUESTED_EFFFECTIVE_SPADS;
...
VL53L3_Error VL53L3_low_power_auto_setup_manual_calibration(
	VL53L3_DEV        Dev)
{
...
VL53L3_Error VL53L3_low_power_auto_update_DSS(
	VL53L3_DEV        Dev)
{
	...
	VL53L3_system_results_t *pS = &(pdev->sys_results);
	...
	if (status == VL53L3_ERROR_DIVISION_BY_ZERO) {
...
VL53L3_Error VL53L3_compute_histo_merge_nb(
	VL53L3_DEV        Dev,	uint8_t *histo_merge_nb)
{
	VL53L3_LLDriverData_t *pdev = VL53L3DevStructGetLLDriverHandle(Dev);
	VL53L3_Error  status = VL53L3_ERROR_NONE;
	uint8_t i, timing;
