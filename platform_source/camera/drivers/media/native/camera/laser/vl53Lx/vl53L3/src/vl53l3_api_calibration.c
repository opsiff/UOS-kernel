...
VL53L3_Error VL53L3_run_ref_spad_char(
	VL53L3_DEV        Dev,
	VL53L3_Error     *pcal_status)
{
	...
	VL53L3_refspadchar_config_t *prefspadchar  = &(pdev->refspadchar);
	...
	if (status == VL53L3_ERROR_NONE)
		status = VL53L3_enable_powerforce(Dev);



	if (status == VL53L3_ERROR_NONE)
		status =
		VL53L3_set_ref_spad_char_config(
			Dev,
			prefspadchar->VL53L3_p_009,
	...
	if (status == VL53L3_ERROR_NONE)
		...
		VL53L3_ReadMulti(
			Dev,
			VL53L3_REF_SPAD_CHAR_RESULT__NUM_ACTUAL_REF_SPADS,
			comms_buffer,
			2);
	...
	if (status == VL53L3_ERROR_NONE)
		VL53L3_print_customer_nvm_managed(
			...
			VL53L3_TRACE_MODULE_REF_SPAD_CHAR);
#endif
	...
	if (status == VL53L3_ERROR_NONE) {
		...
		case VL53L3_DEVICEERROR_REFSPADCHARNOTENOUGHDPADS:
			status = VL53L3_WARNING_REF_SPAD_CHAR_NOT_ENOUGH_SPADS;
			break;

		case VL53L3_DEVICEERROR_REFSPADCHARMORETHANTARGET:
			status = VL53L3_WARNING_REF_SPAD_CHAR_RATE_TOO_HIGH;
			break;

		case VL53L3_DEVICEERROR_REFSPADCHARLESSTHANTARGET:
			status = VL53L3_WARNING_REF_SPAD_CHAR_RATE_TOO_LOW;
			break;
		}
	}
	...
	*pcal_status = status;
	...
	IGNORE_STATUS(
		...
		VL53L3_WARNING_REF_SPAD_CHAR_NOT_ENOUGH_SPADS,
		status);

	IGNORE_STATUS(
		IGNORE_REF_SPAD_CHAR_RATE_TOO_HIGH,
		VL53L3_WARNING_REF_SPAD_CHAR_RATE_TOO_HIGH,
		status);

	IGNORE_STATUS(
		IGNORE_REF_SPAD_CHAR_RATE_TOO_LOW,
		VL53L3_WARNING_REF_SPAD_CHAR_RATE_TOO_LOW,
		status);
	...
	VL53L3_Error                       *pcal_status)
{
	...
	VL53L3_xtalkextract_config_t *pX = &(pdev->xtalk_extract_cfg);
	VL53L3_xtalk_config_t *pC = &(pdev->xtalk_cfg);
	VL53L3_xtalk_calibration_results_t *pXC = &(pdev->xtalk_cal);
	...
	uint8_t measurement_mode = VL53L3_DEVICEMEASUREMENTMODE_BACKTOBACK;
	...
	VL53L3_init_histogram_bin_data_struct(
			0,
			(uint16_t)VL53L3_HISTOGRAM_BUFFER_SIZE,
	...
	if (status == VL53L3_ERROR_NONE)
		status =
		VL53L3_set_preset_mode(
		Dev,
		VL53L3_DEVICEPRESETMODE_HISTOGRAM_XTALK_PLANAR,
	...
	pdev->xtalk_results.max_results    = VL53L3_MAX_XTALK_RANGE_RESULTS;
	...
	for (i = 0; i < pdev->xtalk_results.max_results; i++) {
		pdev->xtalk_results.VL53L3_p_002[i].no_of_samples           = 0;
		pdev->xtalk_results.VL53L3_p_002[i].signal_total_events_avg = 0;
		pdev->xtalk_results.VL53L3_p_002[i].signal_total_events_sum = 0;
		pdev->xtalk_results.VL53L3_p_002[i].rate_per_spad_kcps_sum  = 0;
		pdev->xtalk_results.VL53L3_p_002[i].rate_per_spad_kcps_avg  = 0;
		pdev->xtalk_results.VL53L3_p_002[i].sigma_mm_sum            = 0;
		pdev->xtalk_results.VL53L3_p_002[i].sigma_mm_avg            = 0;

		pdev->xtalk_results.VL53L3_p_002[i].median_phase_sum        = 0;
		pdev->xtalk_results.VL53L3_p_002[i].median_phase_avg        = 0;
	...
	if (status == VL53L3_ERROR_NONE) {
		...
		status =
		VL53L3_get_and_avg_xtalk_samples(
		Dev,
	...
	if (status == VL53L3_ERROR_NONE)
		if ((pdev->xtalk_results.VL53L3_p_002[4].no_of_samples == 0) ||
			(pdev->xtalk_results.VL53L3_p_002[4].sigma_mm_avg >
...
#ifdef VL53L3_LOG_ENABLE
	if (status == VL53L3_ERROR_NONE)
		VL53L3_print_xtalk_range_results(
			&(pdev->xtalk_results),
			"pdev->xtalk_results",
			VL53L3_TRACE_MODULE_CORE);
#endif

	if ((status == VL53L3_ERROR_NONE) && (results_invalid == 0)) {

		status =
			VL53L3_ipp_xtalk_calibration_process_data(
					Dev,
					&(pdev->xtalk_results),
					&(pdev->xtalk_shapes),
					&(pdev->xtalk_cal));
	...
	if (status == VL53L3_ERROR_NONE) {
		...
		for (i = 0; i < pdev->xtalk_results.max_results; i++) {
			...
			if (pdev->xtalk_results.VL53L3_p_002[i].no_of_samples !=
			...
			if (pdev->xtalk_results.VL53L3_p_002[i].sigma_mm_avg >
				((uint32_t)tmp16 << 5)) {
	...
	if (results_invalid > 0) {
		...
		if (pdev->xtalk_results.VL53L3_p_002[4].no_of_samples == 0) {
			...
			if (pdev->xtalk_results.VL53L3_p_002[4].sigma_mm_avg >
			(((uint32_t)pX->algo__crosstalk_extract_max_sigma_mm)
				<< 5)) {
				status =
		...
		if (pdev->xtalk_results.zero_samples_status != 0x00) {
			status = VL53L3_WARNING_XTALK_NO_SAMPLES_FOR_GRADIENT;
		} else {
			if (pdev->xtalk_results.max_sigma_status != 0x00) {
				status =
				VL53L3_WARNING_XTALK_SIGMA_LIMIT_FOR_GRADIENT;
			} else {
				if (pdev->xtalk_results.num_of_samples_status !=
						0x00)
					status =
					VL53L3_WARNING_XTALK_MISSING_SAMPLES;
			}
		}
	}
	...
	*pcal_status = pdev->xtalk_results.cal_status;
	...
	IGNORE_STATUS(
		...
		VL53L3_ERROR_XTALK_EXTRACTION_NO_SAMPLE_FAIL,
		status);
	...
	IGNORE_STATUS(
		...
		VL53L3_WARNING_XTALK_NO_SAMPLES_FOR_GRADIENT,
		status);
	...
	VL53L3_print_customer_nvm_managed(
		...
		VL53L3_TRACE_MODULE_XTALK_DATA);

	VL53L3_print_xtalk_config(
		&(pdev->xtalk_cfg),
		"run_xtalk_extraction():pdev->lldata.xtalk_cfg.",
		VL53L3_TRACE_MODULE_XTALK_DATA);

	VL53L3_print_xtalk_extract_config(
	...
	VL53L3_print_xtalk_histogram_data(
		&(pdev->xtalk_shapes),
...
VL53L3_Error VL53L3_get_and_avg_xtalk_samples(
		...
		VL53L3_xtalk_range_results_t *pXR,
		...
		VL53L3_histogram_bin_data_t  *pavg_histo)
{
	...
	VL53L3_range_results_t      *prs =
			(VL53L3_range_results_t *) pdev->wArea1;

	VL53L3_range_data_t         *prange_data;
	VL53L3_xtalk_range_data_t   *pxtalk_range_data;
	...
	status = VL53L3_dynamic_xtalk_correction_disable(Dev);
	...
	for (i = 0; i <= (final_zone*num_of_samples); i++) {
		...
		if (status == VL53L3_ERROR_NONE)
			status = VL53L3_wait_for_range_completion(Dev);



		if (status == VL53L3_ERROR_NONE)
			status =
				VL53L3_get_device_results(
					Dev,
		...
		if (status == VL53L3_ERROR_NONE &&
			...
			prange_data       = &(prs->VL53L3_p_002[0]);


			if (prs->active_results > 1) {
				for (j = 1;
				j < prs->active_results; j++) {
					if (prs->VL53L3_p_002[j].median_range_mm
						<
			...
			pxtalk_range_data = &(pXR->VL53L3_p_002[zone_id]);
			...
			if (valid_result == 1) {
				...
				pxtalk_range_data->signal_total_events_sum +=
					prange_data->VL53L3_p_013;

				pxtalk_range_data->sigma_mm_sum +=
					(uint32_t)prange_data->VL53L3_p_005;



				pxtalk_range_data->median_phase_sum +=
					(uint32_t)prange_data->VL53L3_p_014;
			...
			if ((valid_result == 1) && (zone_id >= 4)) {
				status = VL53L3_sum_histogram_data(
						&(pdev->hist_data),
						psum_histo);



				if (prange_data->VL53L3_p_015 <
					pXR->central_histogram__window_start)
					pXR->central_histogram__window_start =
					prange_data->VL53L3_p_015;


				if (prange_data->VL53L3_p_016 >
					pXR->central_histogram__window_end)
					pXR->central_histogram__window_end =
						prange_data->VL53L3_p_016;
		...
		if (status == VL53L3_ERROR_NONE)
			status = VL53L3_wait_for_firmware_ready(Dev);



		if (status == VL53L3_ERROR_NONE)
			status =
				VL53L3_clear_interrupt_and_enable_next_range(
					Dev,
					measurement_mode);
	...
	for (i = 0; i < (pdev->zone_cfg.active_zones+1); i++) {
		...
		pxtalk_range_data = &(pXR->VL53L3_p_002[i+xtalk_result_id]);
	...
	if (status == VL53L3_ERROR_NONE) {
		...
		pxtalk_range_data = &(pXR->VL53L3_p_002[xtalk_histo_id]);

		status = VL53L3_avg_histogram_data(
			pxtalk_range_data->no_of_samples,
			psum_histo,
			pavg_histo);
	}
	...
	if (status == VL53L3_ERROR_NONE) {
		if (smudge_corr_en == 1)
			status = VL53L3_dynamic_xtalk_correction_enable(Dev);
	}
	...
	VL53L3_Error                 *pcal_status)
{
	...
	VL53L3_DevicePresetModes device_preset_modes[
				VL53L3_MAX_OFFSET_RANGE_RESULTS];

	VL53L3_range_results_t     *prange_results =
			(VL53L3_range_results_t *) pdev->wArea1;

	VL53L3_range_data_t        *pRData = NULL;
	VL53L3_offset_range_data_t *pfs     = NULL;
	VL53L3_general_config_t *pG = &(pdev->gen_cfg);
	VL53L3_additional_offset_cal_data_t *pAO = &(pdev->add_off_cal_data);
	...
	uint8_t num_of_samples[VL53L3_MAX_OFFSET_RANGE_RESULTS];
	...
	case VL53L3_OFFSETCALIBRATIONMODE__MM1_MM2__HISTOGRAM:
	case VL53L3_OFFSETCALIBRATIONMODE__MM1_MM2__HISTOGRAM_PRE_RANGE_ONLY:
		device_preset_modes[0] =
			VL53L3_DEVICEPRESETMODE_HISTOGRAM_RANGING;
		device_preset_modes[1] =
			VL53L3_DEVICEPRESETMODE_HISTOGRAM_RANGING_MM1_CAL;
		device_preset_modes[2] =
			VL53L3_DEVICEPRESETMODE_HISTOGRAM_RANGING_MM2_CAL;
	break;
	...
	default:
		...
		device_preset_modes[1] =
			VL53L3_DEVICEPRESETMODE_STANDARD_RANGING_MM1_CAL;
		device_preset_modes[2] =
			VL53L3_DEVICEPRESETMODE_STANDARD_RANGING_MM2_CAL;
	break;
	}
	...
	case VL53L3_OFFSETCALIBRATIONMODE__MM1_MM2__STANDARD_PRE_RANGE_ONLY:
	case VL53L3_OFFSETCALIBRATIONMODE__MM1_MM2__HISTOGRAM_PRE_RANGE_ONLY:
	...
	for (m = 0; m <  VL53L3_MAX_OFFSET_RANGE_RESULTS; m++) {

		pfs = &(pdev->offset_results.VL53L3_p_002[m]);
		pfs->preset_mode         = 0;
		pfs->no_of_samples       = 0;
	...
	for (m = 0; m < pdev->offset_results.active_results; m++) {
		...
		for (i = 0; i <= (num_of_samples[m]+2); i++) {
			...
			if (status == VL53L3_ERROR_NONE)
				status =
					VL53L3_wait_for_range_completion(Dev);
			...
			pRData  = &(prange_results->VL53L3_p_002[0]);
			...
			if ((prange_results->active_results > 0 &&
				...
				pfs->no_of_samples++;
				pfs->effective_spads +=
				(uint32_t)pRData->VL53L3_p_006;
				pfs->peak_rate_mcps  +=
				(uint32_t)pRData->peak_signal_count_rate_mcps;
				pfs->VL53L3_p_005        +=
					(uint32_t)pRData->VL53L3_p_005;
				pfs->median_range_mm +=
			...
			if (status == VL53L3_ERROR_NONE)
				status =
					VL53L3_wait_for_firmware_ready(Dev);
		...
		if (pfs->no_of_samples > 0) {
			...
			pfs->VL53L3_p_005        += (pfs->no_of_samples/2);
			pfs->VL53L3_p_005        /= pfs->no_of_samples;
	...
	case VL53L3_OFFSETCALIBRATIONMODE__MM1_MM2__HISTOGRAM_PRE_RANGE_ONLY:
		...
		(int16_t)pdev->offset_results.VL53L3_p_002[0].range_mm_offset;
	...
	default:
		...
		pdev->customer.mm_config__inner_offset_mm =
		(int16_t)pdev->offset_results.VL53L3_p_002[1].range_mm_offset;
		pdev->customer.mm_config__outer_offset_mm =
		(int16_t)pdev->offset_results.VL53L3_p_002[2].range_mm_offset;
		pdev->customer.algo__part_to_part_range_offset_mm = 0;



		pAO->result__mm_inner_actual_effective_spads =
		(uint16_t)pdev->offset_results.VL53L3_p_002[1].effective_spads;
		pAO->result__mm_outer_actual_effective_spads =
		(uint16_t)pdev->offset_results.VL53L3_p_002[2].effective_spads;

		pAO->result__mm_inner_peak_signal_count_rtn_mcps =
		(uint16_t)pdev->offset_results.VL53L3_p_002[1].peak_rate_mcps;
		pAO->result__mm_outer_peak_signal_count_rtn_mcps =
		(uint16_t)pdev->offset_results.VL53L3_p_002[2].peak_rate_mcps;
	...
	pdev->cust_dmax_cal.ref__actual_effective_spads =
		(uint16_t)pdev->offset_results.VL53L3_p_002[0].effective_spads;
	pdev->cust_dmax_cal.ref__peak_signal_count_rate_mcps =
		(uint16_t)pdev->offset_results.VL53L3_p_002[0].peak_rate_mcps;
	...
	for (m = 0; m < pdev->offset_results.active_results; m++) {
		...
		if (status == VL53L3_ERROR_NONE) {
			...
			if (pfs->no_of_samples < num_of_samples[m])
				status =
				VL53L3_WARNING_OFFSET_CAL_MISSING_SAMPLES;


			if (m == 0 && pfs->VL53L3_p_005 >
				((uint32_t)VL53L3_OFFSET_CAL_MAX_SIGMA_MM << 5))
				status =
				VL53L3_WARNING_OFFSET_CAL_SIGMA_TOO_HIGH;

			if (pfs->peak_rate_mcps >
				VL53L3_OFFSET_CAL_MAX_PRE_PEAK_RATE_MCPS)
				status =
				VL53L3_WARNING_OFFSET_CAL_RATE_TOO_HIGH;

			if (pfs->dss_config__manual_effective_spads_select <
				VL53L3_OFFSET_CAL_MIN_EFFECTIVE_SPADS)
				status =
				VL53L3_WARNING_OFFSET_CAL_SPAD_COUNT_TOO_LOW;

			if (pfs->dss_config__manual_effective_spads_select == 0)
				status =
				VL53L3_ERROR_OFFSET_CAL_NO_SPADS_ENABLED_FAIL;
	...
	*pcal_status = pdev->offset_results.cal_status;
	...
	VL53L3_print_customer_nvm_managed(
		...
		VL53L3_TRACE_MODULE_OFFSET_DATA);
	...
	VL53L3_print_offset_range_results(
		&(pdev->offset_results),
	...
	uint16_t               *pzero_distance_phase)
{
	...
	for (m = 0; m < phasecal_num_of_samples; m++) {
		...
		if (status == VL53L3_ERROR_NONE) {
			...
			period = 2048 *
				(uint32_t)VL53L3_decode_vcsel_period(
					pdev->hist_data.VL53L3_p_009);

			VL53L3_p_017  = period;
			VL53L3_p_017 += (uint32_t)(
			...
			if (period != 0) {
				VL53L3_p_017  = VL53L3_p_017 % period;
			}
			else {
				status =
				VL53L3_ERROR_DIVISION_BY_ZERO;
				VL53L3_p_017 = 0;
			}
			...
			zero_distance_phase += (uint32_t)VL53L3_p_017;
		}
	}
	VL53L3_unload_patch(Dev);
	...
	if (status == VL53L3_ERROR_NONE && samples > 0) {
	...
	VL53L3_Error                 *pcal_status)
{
	...
	VL53L3_range_results_t         *pRR =
			(VL53L3_range_results_t *) pdev->wArea1;
	VL53L3_range_data_t            *prange_data = NULL;
	VL53L3_zone_calibration_data_t *pzone_data  = NULL;
	...
	VL53L3_OffsetCorrectionMode  offset_cor_mode =
			VL53L3_OFFSETCORRECTIONMODE__NONE;
	...
	if (zone_preset == VL53L3_DEVICEZONEPRESET_CUSTOM) {

		if (status == VL53L3_ERROR_NONE)
			status =
				VL53L3_set_zone_config(
					Dev,
					pzone_cfg);

	} else if (zone_preset != VL53L3_DEVICEZONEPRESET_NONE) {

		if (status == VL53L3_ERROR_NONE)
			status =
				VL53L3_set_zone_preset(
					Dev,
					zone_preset);
	}
	...
	for (i = 0; i < VL53L3_MAX_USER_ZONES; i++) {
		...
		pres->zone_cal.VL53L3_p_002[i].VL53L3_p_014    = 0;
		pres->zone_cal.VL53L3_p_002[i].VL53L3_p_005        = 0;
	...
	status =
		VL53L3_get_offset_correction_mode(
			Dev,
			&offset_cor_mode);

	if (status == VL53L3_ERROR_NONE)
		status =
			VL53L3_set_offset_correction_mode(
				Dev,
				VL53L3_OFFSETCORRECTIONMODE__NONE);
	...
	for (i = 0; i <= m; i++) {
		...
		prange_data  = &(pRR->VL53L3_p_002[0]);
	...
	if (status == VL53L3_ERROR_NONE) {
		...
		for (z = 0; z < pres->zone_cal.active_zones; z++) {
			...
			pzone_data = &(pres->zone_cal.VL53L3_p_002[z]);
			...
			if (pzone_data->no_of_samples > 0) {
				...
				VL53L3_range_maths(
				pdev->stat_nvm.osc_measured__fast_osc__frequency
				, (uint16_t)pzone_data->VL53L3_p_014,
				pres->zone_cal.zero_distance_phase,
				2,
				0x0800,
				0);
				...
				if (pzone_data->no_of_samples <
					...
					status =
					VL53L3_WARNING_ZONE_CAL_MISSING_SAMPLES;


				if (pzone_data->VL53L3_p_005 >
					((uint32_t)VL53L3_ZONE_CAL_MAX_SIGMA_MM
							<< 5))
					status =
					VL53L3_WARNING_ZONE_CAL_SIGMA_TOO_HIGH;

				if (pzone_data->peak_rate_mcps >
					VL53L3_ZONE_CAL_MAX_PRE_PEAK_RATE_MCPS)
					status =
					VL53L3_WARNING_ZONE_CAL_RATE_TOO_HIGH;

			} else {
				status = VL53L3_ERROR_ZONE_CAL_NO_SAMPLE_FAIL;
			}
		}
	}
	...
	*pcal_status = pres->zone_cal.cal_status;
	...
	VL53L3_print_zone_calibration_results(
		&(pres->zone_cal),
	...
	VL53L3_spad_rate_data_t   *pspad_rate_data)
{
	...
	if (status == VL53L3_ERROR_NONE)
		status =
			VL53L3_get_spad_rate_data(
				Dev,
				pspad_rate_data);

	if (device_test_mode == VL53L3_DEVICETESTMODE_LCR_VCSEL_ON)
	...
	if (status == VL53L3_ERROR_NONE)
		status = VL53L3_disable_powerforce(Dev);

#ifdef VL53L3_LOG_ENABLE


	if (status == VL53L3_ERROR_NONE) {
		VL53L3_print_spad_rate_data(
			pspad_rate_data,
			"run_spad_rate_map():",
			VL53L3_TRACE_MODULE_SPAD_RATE_MAP);
		VL53L3_print_spad_rate_map(
			pspad_rate_data,
			"run_spad_rate_map():",
			VL53L3_TRACE_MODULE_SPAD_RATE_MAP);
	}
	...
	VL53L3_DeviceTestMode  device_test_mode)
{
	...
	if (status == VL53L3_ERROR_NONE)
		status = VL53L3_start_test(
					Dev,
					device_test_mode);


	if (status == VL53L3_ERROR_NONE)
		status = VL53L3_wait_for_test_completion(Dev);
	...
	if (status == VL53L3_ERROR_NONE) {
		...
		if (status == VL53L3_ERROR_NONE)
			status = VL53L3_clear_interrupt(Dev);
	}
	...
	VL53L3_hist_xtalk_extract_data_t *pxtalk_data)
{
	...
	pxtalk_data->VL53L3_p_015             = 0U;
	pxtalk_data->VL53L3_p_016               = 0U;
	...
	VL53L3_hist_xtalk_extract_data_t   *pxtalk_data)
{
	...
	status =
		VL53L3_hist_xtalk_extract_calc_window(
			target_distance_mm,
	...
	if (status == VL53L3_ERROR_NONE) {
		status =
			VL53L3_hist_xtalk_extract_calc_event_sums(
				phist_bins,
				pxtalk_data);
	}
	...
	VL53L3_xtalk_histogram_shape_t     *pxtalk_shape)
{
	...
	VL53L3_Error  status = VL53L3_ERROR_NONE;
	VL53L3_xtalk_calibration_results_t *pX = pxtalk_cal;
	...
	if (pxtalk_data->sample_count > 0) {
		...
		status =
			VL53L3_hist_xtalk_extract_calc_rate_per_spad(
				pxtalk_data);
		...
		if (status == VL53L3_ERROR_NONE) {
			...
			status =
				VL53L3_hist_xtalk_extract_calc_shape(
					pxtalk_data,
					pxtalk_shape);
			...
			pxtalk_shape->VL53L3_p_019 =
				phist_bins->VL53L3_p_019;
		}
	...
	VL53L3_Error                       *pcal_status)
{
	...
	for (k = 0; k < nbloops; k++) {
		...
		VL53L3_hist_xtalk_extract_data_init(
				&(pdev->xtalk_extract));
		VL53L3_set_tuning_parm(Dev,
				VL53L3_TUNINGPARM_HIST_MERGE_MAX_SIZE,
				k * MaxId + 1);
		...
		for (i = 0; i <= pX->num_of_samples; i++) {
			if (status == VL53L3_ERROR_NONE)
				status = VL53L3_wait_for_range_completion(Dev);
			if (status == VL53L3_ERROR_NONE)
				status = VL53L3_get_device_results(Dev,
					VL53L3_DEVICERESULTSLEVEL_FULL,
					prange_results);
			Very1stRange =
				(pdev->ll_state.rd_device_state ==
			...
			if (wait_for_accumulation)
				...
				if ((status == VL53L3_ERROR_NONE) &&
					(!Very1stRange)) {
					status =
					VL53L3_hist_xtalk_extract_update(
						cal_distance_mm,
						OVERSIZE,
						&(pdev->hist_data),
						&(pdev->xtalk_extract));
		...
		if (status == VL53L3_ERROR_NONE)
			status =
			VL53L3_hist_xtalk_extract_fini(
				&(pdev->hist_data),
				&(pdev->xtalk_extract),
				&(pdev->xtalk_cal),
	...
	VL53L3_stop_range(Dev);

	VL53L3_set_tuning_parm(Dev, VL53L3_TUNINGPARM_HIST_MERGE_MAX_SIZE,
			initMergeSize);
	VL53L3_unload_patch(Dev);

	if (status != VL53L3_ERROR_NONE)
		status = VL53L3_ERROR_XTALK_EXTRACTION_SIGMA_LIMIT_FAIL;
