...
VL53L3_Error VL53L3_xtalk_calibration_process_data(
	...
	VL53L3_xtalk_calibration_results_t	*pxtalk_cal)
{
	...
	VL53L3_xtalk_algo_data_t xtalk_debug;
	VL53L3_xtalk_algo_data_t *pdebug      = &xtalk_debug;
	VL53L3_xtalk_range_data_t *pxtalk_data = NULL;

	VL53L3_histogram_bin_data_t avg_bins;
	VL53L3_histogram_bin_data_t *pavg_bins   = &avg_bins;
	...
	if (status == VL53L3_ERROR_NONE)
		VL53L3_init_histogram_bin_data_struct(
			0, 0, &(pdebug->VL53L3_p_057));

	if (status == VL53L3_ERROR_NONE)
		VL53L3_init_histogram_bin_data_struct(
			0, 0, &(pdebug->VL53L3_p_058));
	...
	pxtalk_data = &(pxtalk_results->VL53L3_p_002[4]);
...
VL53L3_Error VL53L3_generate_dual_reflectance_xtalk_samples(
	VL53L3_xtalk_range_results_t	*pxtalk_results,
	uint16_t			expected_target_distance_mm,
	uint8_t					higher_reflectance,
	VL53L3_histogram_bin_data_t	*pxtalk_avg_samples
)
{
	...
	VL53L3_histogram_bin_data_t *pzone_avg_1   =
			&(pxtalk_results->histogram_avg_1[0]);
	VL53L3_histogram_bin_data_t *pzone_avg_2   =
			&(pxtalk_results->histogram_avg_2[0]);

	VL53L3_histogram_bin_data_t *pxtalk_output = pxtalk_avg_samples;
...
VL53L3_Error VL53L3_f_050(
	VL53L3_histogram_bin_data_t	*pzone_avg_1,
	VL53L3_histogram_bin_data_t	*pzone_avg_2,
	uint16_t			expected_target_distance,
	uint8_t				subtract_amb,
	uint8_t				higher_reflectance,
	VL53L3_histogram_bin_data_t	*pxtalk_output
)
{
	...
	VL53L3_histogram_bin_data_t  zone_avg_realigned;
	...
	if ((status == VL53L3_ERROR_NONE) && (subtract_amb == 0x01)) {
		VL53L3_f_037(
				pzone_avg_1,
				pzone_avg_1->VL53L3_p_004);


		pzone_avg_1->VL53L3_p_004 = 0x0;
	}

	if ((status == VL53L3_ERROR_NONE) && (subtract_amb == 0x01)) {
		VL53L3_f_037(
			pzone_avg_2,
			pzone_avg_2->VL53L3_p_004);


		pzone_avg_2->VL53L3_p_004 = 0x0;
	}
...
VL53L3_Error VL53L3_f_049(
		VL53L3_histogram_bin_data_t        *pavg_bins,
		VL53L3_xtalk_algo_data_t           *pdebug,
		VL53L3_xtalk_range_data_t          *pxtalk_data,
	...
	)
{
	...
	VL53L3_xtalk_range_data_t  *presults_int = NULL;

	int          i                   = 0;

	uint32_t xtalk_per_spad[4];
	int32_t  VL53L3_p_060         = 0;
	int32_t  VL53L3_p_061         = 0;
	...
	*ygradient = 0;
	...
	for (i = 0; i < 4; i++) {
		...
		if (status == VL53L3_ERROR_NONE) {
			...
			presults_int = &(pxtalk_results->VL53L3_p_002[i]);
	...
	if ((status == VL53L3_ERROR_NONE) && (result_invalid == 0)) {



		if (status == VL53L3_ERROR_NONE) {
			VL53L3_p_060 = ((int32_t)xtalk_per_spad[1]
				- (int32_t)xtalk_per_spad[0]) / (8);
			VL53L3_p_061 = ((int32_t)xtalk_per_spad[3]
		...
		if (status == VL53L3_ERROR_NONE) {
			...
			pdebug->VL53L3_p_060 = (int16_t)VL53L3_p_060;
			pdebug->VL53L3_p_061 = (int16_t)VL53L3_p_061;
		}
		...
		pdebug->VL53L3_p_060 = 0;
		pdebug->VL53L3_p_061 = 0;
	}



	if (status == VL53L3_ERROR_NONE) {
		*xgradient = (int16_t)VL53L3_p_060;
		*ygradient = (int16_t)VL53L3_p_061;
	}
	...
	)
{
	...
	*xtalk_mean_offset_kcps          = 0;
	...
	if ((status == VL53L3_ERROR_NONE) && (result_invalid == 0)) {
		...
		pdebug->VL53L3_p_059 = xtalk_per_spad;
...
VL53L3_Error VL53L3_f_053(
	VL53L3_histogram_bin_data_t	*phist_data,
	VL53L3_xtalk_range_data_t      *pxtalk_data,
	VL53L3_xtalk_algo_data_t       *pdebug,
	VL53L3_xtalk_histogram_shape_t *pxtalk_histo
	)
{
	...
	int32_t tmpi32;
	uint8_t i = 0;
	uint64_t bin_data[VL53L3_XTALK_HISTO_BINS];
	...
	pxtalk_histo->VL53L3_p_023             =
			phist_data->VL53L3_p_023;
	pxtalk_histo->cal_config__vcsel_start =
			phist_data->cal_config__vcsel_start;
	pxtalk_histo->VL53L3_p_019     =
			phist_data->VL53L3_p_019;
	pxtalk_histo->VL53L3_p_022               =
			phist_data->VL53L3_p_022;
	...
	memcpy(&(pdebug->VL53L3_p_058),
		phist_data, sizeof(VL53L3_histogram_bin_data_t));
	...
	if (pxtalk_data->signal_total_events_avg == 0) {
		for (i = 0; i < pxtalk_histo->VL53L3_p_024; i++)
			bin_data[i] = 0;
		goto FAIL;
	}
...
FAIL:
	...
	for (i = 0; i < pxtalk_histo->VL53L3_p_024; i++)
		pdebug->VL53L3_p_063[i] = pxtalk_histo->bin_data[i];
...
VL53L3_Error VL53L3_f_054(
	VL53L3_customer_nvm_managed_t *pcustomer,
	VL53L3_dynamic_config_t       *pdyn_cfg,
	VL53L3_xtalk_histogram_data_t *pxtalk_shape,
	VL53L3_histogram_bin_data_t   *pip_hist_data,
	VL53L3_histogram_bin_data_t   *pop_hist_data,
	VL53L3_histogram_bin_data_t   *pxtalk_count_data)
{
...
VL53L3_Error VL53L3_f_041(
	VL53L3_histogram_bin_data_t    *phist_data,
	VL53L3_xtalk_histogram_shape_t *pxtalk_data,
	uint32_t                        xtalk_rate_kcps,
	VL53L3_histogram_bin_data_t    *pxtalkcount_data
	)
{
	...
	for (i = 0; i < pxtalk_data->VL53L3_p_024; i++) {
...
VL53L3_Error VL53L3_f_055(
	VL53L3_histogram_bin_data_t	*phist_data,
	VL53L3_histogram_bin_data_t	*pxtalk_data,
	...
	uint8_t						VL53L3_p_026)
{
	...
	int32_t realign_bin_data[VL53L3_HISTOGRAM_BUFFER_SIZE];

	LOG_FUNCTION_START("");



	for (i = 0 ; i < VL53L3_HISTOGRAM_BUFFER_SIZE ; i++)
		realign_bin_data[i] = 0;

	first_bin_int = VL53L3_p_022;
	last_bin_int  = VL53L3_p_026;





	VL53L3_hist_remove_ambient_bins(pxtalk_data);



	first_bin_int = (first_bin_int) %
				pxtalk_data->VL53L3_p_024;

	last_bin_int = (last_bin_int) %
				pxtalk_data->VL53L3_p_024;

	first_bin_inc = (first_bin_int + 1) % pxtalk_data->VL53L3_p_024;
	...
	if (first_bin_inc > last_bin_int) {
		...
		realign_bin = pxtalk_data->VL53L3_p_024 - first_bin_inc;
	...
	if (realign_bin > 0) {
		...
		for (i = 0; i < pxtalk_data->VL53L3_p_024; i++) {
			realign_index = (pxtalk_data->VL53L3_p_024 -
				realign_bin + i)
				% pxtalk_data->VL53L3_p_024;
		...
		uint32_t                    *ambient_noise)
{
	...
	if (VL53L3_p_004 <= 0)
		...
		ambient_events_per_bin_int = (uint32_t)VL53L3_p_004;

	*ambient_noise =  VL53L3_isqrt(ambient_events_per_bin_int);
