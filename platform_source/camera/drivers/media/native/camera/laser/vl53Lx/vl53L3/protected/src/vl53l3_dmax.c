	...
	int16_t                              *pambient_dmax_mm)
{
	...
	pdata->VL53L3_p_006     = 0x0000;
	pdata->VL53L3_p_033 = 0x0000;
	pdata->VL53L3_p_001          = 0x0000;
	pdata->VL53L3_p_012    = 0x0000;
	pdata->VL53L3_p_004     = 0x0000;
	pdata->VL53L3_p_034 = 0x0000;
	pdata->VL53L3_p_035             = 0;
	pdata->VL53L3_p_007            = 0;

	*pambient_dmax_mm  = 0;


	if ((pbins->VL53L3_p_019        != 0) &&
		(pbins->total_periods_elapsed      != 0)) {



		pll_period_us   =
			VL53L3_calc_pll_period_us(pbins->VL53L3_p_019);
		...
		pdata->VL53L3_p_037 = pcfg->max_effective_spads;
		pdata->VL53L3_p_006  = pcfg->max_effective_spads;

		if (pdata->VL53L3_p_033 > 0) {
			tmp64   =
			(uint64_t)pcfg->dss_config__target_total_rate_mcps;
			tmp64  *= 1000;
			tmp64 <<= (11+1);
			tmp32 = pdata->VL53L3_p_033/2;
			tmp64  += (uint64_t)tmp32;
			tmp64 = do_division_u(tmp64,
				(uint64_t)pdata->VL53L3_p_033);

			if (tmp64 < (uint64_t)pcfg->max_effective_spads)
				pdata->VL53L3_p_006 = (uint16_t)tmp64;
		}
	}
	...
	if ((pcal->ref__actual_effective_spads != 0) &&
		...
		pdata->VL53L3_p_012   = (uint32_t)tmp64;
		pdata->VL53L3_p_012 <<= 4;



		tmp64   = (uint64_t)pdata->VL53L3_p_036;
		tmp64  *= (uint64_t)pdata->VL53L3_p_033;
		tmp64  *= (uint64_t)pdata->VL53L3_p_006;
		tmp64  += (1<<(11+7));
		tmp64 >>= (11+8);
		tmp64  +=  500;
		...
		pdata->VL53L3_p_004 = (uint32_t)tmp64;



		tmp64   = (uint64_t)pdata->VL53L3_p_036;
		tmp64  *= (uint64_t)pdata->VL53L3_p_012;
		tmp64  *= (uint64_t)pdata->VL53L3_p_006;
		tmp64  += (1<<(11+7));
		tmp64 >>= (11+8);
		...
		pdata->VL53L3_p_034 = (uint32_t)tmp64;



		tmp32  = VL53L3_isqrt(pdata->VL53L3_p_004 << 8);
		tmp32 *= (uint32_t)pcfg->ambient_thresh_sigma;



		if (pdata->VL53L3_p_004 <
			(uint32_t)pcfg->min_ambient_thresh_events) {

			amb_thres_delta =
				pcfg->min_ambient_thresh_events -
				(uint32_t)pdata->VL53L3_p_004;
		...
		tmp32  = (uint32_t)pdata->VL53L3_p_034;
		...
		if (pdata->VL53L3_p_035 < pdata->VL53L3_p_007)
			*pambient_dmax_mm = pdata->VL53L3_p_035;
		else
			*pambient_dmax_mm = pdata->VL53L3_p_007;
	...
	uint32_t     signal_thresh_sigma)
{
	...
	tmp32  = VL53L3_isqrt(tmp32);
	tmp32 += (uint32_t)signal_thresh_sigma;



	range_mm =
		(uint32_t)VL53L3_isqrt(ref_signal_events << 4);
