	...
	VL53L3_decoded_nvm_data_t          *pdata)
{
	...
	if (status == VL53L3_ERROR_NONE)
		VL53L3_print_decoded_nvm_data(
			pdata,
			"get_nvm_debug_data():pnvm_info.",
			VL53L3_TRACE_MODULE_NVM_DATA);
#endif
	...
	char      *pbuffer)
{
	...
	char  fmt[VL53L3_MAX_STRING_LENGTH];
	...
	uint32_t                       trace_flags)
{
	...
	char  fp_text[VL53L3_MAX_STRING_LENGTH];
	...
	VL53L3_signed_fixed_point_sprintf(
		(int32_t)pdata->osc_measured__fast_osc__frequency,
		12,
		VL53L3_MAX_STRING_LENGTH,
		fp_text);
	...
	uint32_t                       trace_flags)
{
	...
	trace_print(VL53L3_TRACE_LEVEL_INFO,
		"%s%s = %u\n",
		pprefix,
	...
	uint32_t                     trace_flags)
{
	...
	char  fp_text[VL53L3_MAX_STRING_LENGTH];
	char  pre_text[VL53L3_MAX_STRING_LENGTH];
	...
	uint32_t                       trace_flags)
{
	...
	VL53L3_print_xtalk_histogram_shape_data(
		&(pdata->xtalk_shape),
	...
	uint32_t             trace_flags)
{
	...
	VL53L3_signed_fixed_point_sprintf(
		(int32_t)pdata->width,
		 4,	VL53L3_MAX_STRING_LENGTH, fp_text);
	...
	VL53L3_signed_fixed_point_sprintf(
		...
		12,	VL53L3_MAX_STRING_LENGTH, fp_text);
	...
	VL53L3_signed_fixed_point_sprintf(
		(int32_t)pdata->zero_distance_phase,
		11, VL53L3_MAX_STRING_LENGTH, fp_text);
	...
	VL53L3_signed_fixed_point_sprintf(
		(int32_t)pdata->VL53L3_p_006,
		8, VL53L3_MAX_STRING_LENGTH, fp_text);
	...
	VL53L3_signed_fixed_point_sprintf(
		...
		7, VL53L3_MAX_STRING_LENGTH, fp_text);
	...
	VL53L3_signed_fixed_point_sprintf(
		...
		13, VL53L3_MAX_STRING_LENGTH, fp_text);
	...
	VL53L3_signed_fixed_point_sprintf(
		(int32_t)pdata->VL53L3_p_005,
		 2,	VL53L3_MAX_STRING_LENGTH, fp_text);
	...
	uint32_t                trace_flags)
{
	...
	for (i = 0; i < pdata->active_results; i++) {
		...
		VL53L3_print_range_data(
			&pdata->VL53L3_p_002[i],
			ppre_text, trace_flags);
	}
	...
	uint32_t                       trace_flags)
{
	...
	for (i = 0; i < pdata->active_results; i++) {
		sprintf(ppre_text, "%sdata[%u].", pprefix, i);
		VL53L3_print_offset_range_data(
			&(pdata->VL53L3_p_002[i]),
			ppre_text, trace_flags);
	}
}
	...
	uint32_t                 trace_flags)
{
	...
	sprintf(ppre_text, "%s VL53L3_p_010.", pprefix);
		VL53L3_print_histogram_bin_data(
			&pdata->VL53L3_p_010,
			ppre_text, trace_flags);
	...
	uint32_t                           trace_flags)
{
	...
	for (i = 0; i < pdata->active_zones; i++) {
		sprintf(ppre_text, "%sdata[%u].", pprefix, i);
		VL53L3_print_zone_calibration_data(
			&(pdata->VL53L3_p_002[i]),
			ppre_text, trace_flags);
	}
}
	...
	uint32_t                      trace_flags)
{
	...
	VL53L3_histogram_bin_data_t *pbin_data;
	...
	uint32_t                   trace_flags)
{
	...
	VL53L3_print_gain_calibration_data(
		&(pdata->gain_cal),
		ppre_text, trace_flags);
	...
	uint32_t                  trace_flags)
{
	...
	for (col = 0;  col < VL53L3_SPAD_ARRAY_WIDTH; col++)
		trace_print(
			VL53L3_TRACE_LEVEL_INFO,
			",%8u",
			col);
	...
	for (row = 0;  row < VL53L3_SPAD_ARRAY_HEIGHT; row++) {
		...
		for (col = 0;  col < VL53L3_SPAD_ARRAY_HEIGHT; col++) {
