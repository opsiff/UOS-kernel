	...
	struct tof8801_BL_application *BL_app, int num_retries)
{
	...
	do {
		num_retries -= 1;
		error = tof_i2c_read(client, TOF8801_CMD_STAT, rbuf, TOF8801_I2C_HEADER_SIZE);
		if (error)
			continue;
		if (TOF8801_BL_IS_CMD_BUSY(*status)) {
			...
			if (num_retries <= 0) {
				tof8801_infomsg("BL application is busy: %#04x", *status);
				error = -EBUSY;
			}
			continue;
		}
		...
		if (*rdata_size >= 0) {
			...
			if ((chksum != TOF_VALID_CHKSUM) && (num_retries <= 0)) {
				tof8801_infomsg("Checksum verification of Response failed: %#04x", chksum);
				return -EIO;
			}
	...
	tof8801_dbgmsg("BL application wait for response: \'%d\'", error);
	return error;
}
 ...
 * @BL_app: pointer to BL application struct
 */
int tof8801_BL_short_cmd(struct i2c_client *client, struct tof8801_BL_application *BL_app,
...
int tof8801_BL_addr_ram(struct i2c_client *client,
	struct tof8801_BL_application *BL_app, int addr)
{
...
int tof8801_BL_read_ram(struct i2c_client *client,
	struct tof8801_BL_application *BL_app, char *rbuf, int len)
{
	int error = 0;
	int rc;
...
int tof8801_BL_write_ram(struct i2c_client *client,
	struct tof8801_BL_application *BL_app, const char *buf, int len)
{
