...
typedef struct _intelHexRecord {
	/* ULBA = upper linear base address = higher 16 bits of the 32-bit address */
	unsigned int ulba;
	/* lower 16-bit of the 32-bit address - old intel formats used just 16-bit addresses */
...
/* converted ascii intel hex input into an ready to program data record */
static intelRecord intelHexRecord;
	...
	char lineLength, const char *line, char verify)
{
	...
	if (lineLength && line[0] == ':') { /* intel hex records must start with a colon */
		...
		*/
			...
			if ((intelHexRecord.length * 2) > lineLength) {
				...
				if (result != INTEL_HEX_CONTINUE) {
					...
					if (type == INTEL_HEX_TYPE_DATA) {
						...
						intelHexRecord.ulba = 0xFFFFFFFFUL;
		...
		/* if it is an eof record we do not want to call ramWrite */
		if ((result == INTEL_HEX_CONTINUE)
...
static char ramWrite(void *client, struct tof8801_BL_application *BL_app, char verify)
{
	...
	if (intelHexRecord.length) { /* have data to write */
		/* continues write now - cause either we set up the new address or it was continues; */
		result = tof8801_BL_write_ram(client, BL_app,
			intelHexRecord.data, intelHexRecord.length);
		if (result)
			return result;
