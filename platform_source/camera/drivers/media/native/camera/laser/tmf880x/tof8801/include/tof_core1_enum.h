...
typedef enum {
	/* GPIO intterupt occurred during the proximity light collection, no result possible */
	resultStatusNone = 0,
	/* GPIO interrupt occurred during the distance light measurement, Proximity result present */
	resultStatusProx = 1,
	/* GPIO interrupt occurred during the distance algorithm, result complete */
	resultStatusProxDist = 2,
...
typedef struct tof_core1_measure_info {
	...
	union {
		struct {
			/* This bit indicates that a measurement is ongoing. This bit can be read at any time */
			uint8_t tdcBusy : 1;
			/* TDC threshold has been exceeded in previous measurement cycle. If this bit is asserted,
			* then the measurement has stopped not because tdc_count has been reached but because the
			* threshold for "automatic exposure control" was surpassed. Value is only valid after a measurement */
			uint8_t thresholdExceeded : 1;
			/* The cp_overload bit was flagged during the previous measurement. If enabled in cp_stops_tdc,
			* then this is (can be) the reason for a premature end of the measurement.
			* Note: the live value of cp_overload can be seen in the I2C register map. */
			uint8_t chargePumpOverload : 1;
		} PACKED;
		uint8_t tdcifStatus; /* OL_TDCIF_STATUS */
	} PACKED;

	/* Number of times the TDC measurement was initiated to attempt to complete all of the requested iterations */
	uint8_t collectionCycles;
...
typedef union _appStatus {
	struct {
		...
		uint16_t measureInterrupt : 1; /* Indicates the current measurement was delayed due to external trigger */
