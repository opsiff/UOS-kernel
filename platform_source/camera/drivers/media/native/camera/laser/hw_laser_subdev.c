...
void laser_notify_data_event(hw_laser_ctrl_t *ctrl, struct laser_event_t *laser_event)
{
	struct video_device *vdev = NULL;
	struct laser_event_t *req = NULL;
	struct v4l2_event ev;

	if (!ctrl || !(ctrl->sd) || !laser_event) {
		cam_err("param null!");
		return;
	}
	...
	if (err != EOK)
		cam_err("func %s: memset_s fail\n", __func__);
	req = (struct laser_event_t *)ev.u.data;

	vdev = ctrl->sd->devnode;
	ev.type = V4L2_EVENT_CTRL;
	ev.id = 0;
	req->kind = laser_event->kind;
	cam_info("%s event kind: %d", __func__, req->kind);
