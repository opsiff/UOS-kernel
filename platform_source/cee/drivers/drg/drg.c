...
static int drg_rule_get_master_state(struct drg_rule *ruler)
{
	...
	for (i = 0; i < ruler->nr_master; i++) {
		...
		tmp_state = master->freq_dev->idle ? 0 : master->state;
		if (tmp_state < 0)
			continue;

		if (state < 0 ||
		    (ruler->master_vote_max && tmp_state > state) ||
		    (!ruler->master_vote_max && tmp_state < state))
			state = tmp_state;
	}
...
static bool is_effective_master(struct drg_freq_device *freq_dev)
{
	struct drg_device *master = NULL;
	list_for_each_entry(master, &freq_dev->master_list, node)
		if (master->ruler->enable)
			return true;
	return false;
}

static bool is_master_state_lowest(struct drg_freq_device *freq_dev)
{
	struct drg_device *master = NULL;
	list_for_each_entry(master, &freq_dev->master_list, node)
		if (master->state > 0)
			       ...
			       void *data)
{
	...
	freq_dev = this_cpu_read(cpufreq_dev);
	if (!freq_dev)
		goto out;

	if (!is_effective_master(freq_dev))
		goto out;
	...
	case CPU_PM_ENTER:
		...
		if (!is_master_state_lowest(freq_dev))
			mod_timer(&g_drg_idle_update_timer, jiffies +
				msecs_to_jiffies(IDLE_UPDATE_SLACK_TIME));
