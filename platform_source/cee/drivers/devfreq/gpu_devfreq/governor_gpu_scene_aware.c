...
#include <platform_include/trace/events/gpu_scene_aware.h>
...
#define DEFAULT_LOAD_POLICY		WINDOW_STATS_AVG
...
static inline unsigned long gpu_min_freq(struct devfreq *df)
{
	...
	qos_min_freq = dev_pm_qos_read_value(df->dev.parent, DEV_PM_QOS_MIN_FREQUENCY);

	return max(min_freq, (unsigned long)HZ_PER_KHZ * qos_min_freq);
}
#endif
...
static inline unsigned long gpu_max_freq(struct devfreq *df)
{
	...
	max_freq = freq_table[df->profile->max_state - 1];
	qos_max_freq = dev_pm_qos_read_value(df->dev.parent, DEV_PM_QOS_MAX_FREQUENCY);

	return min(max_freq, (unsigned long)HZ_PER_KHZ * qos_max_freq);
}
#endif
						 ...
						 unsigned long *freq)
{
	...
	trace_devfreq_gpu_scene_aware_apply_limits(data->cl_boost,
		data->cl_boost_freq, data->user_set_freq, *freq);
}
					...
					unsigned long *freq)
{
	...
	*freq = stat.current_frequency;
...
#ifdef trace_perf
	trace_perf(clock_set_rate, "gpuload", util, 0);
#endif
	...
	data->buffer[data->window_idx] = a;
	data->window_idx += 1;
	data->window_idx = data->window_idx %
			   data->cur_policy->para[LOADING_WINDOW];

	switch (data->cur_policy->para[LOAD_POLICY]) {
	case WINDOW_STATS_RECENT:
		break;
	case WINDOW_STATS_MAX:
		for (i = 0; i < data->window_counter; i++) {
			if (data->buffer[i] > a)
				a = data->buffer[i];
		}
		break;
	case WINDOW_STATS_MAX_RECENT_AVG:
		a = max(a, div_u64(data->util_sum, data->window_counter));
		break;
	...
	trace_devfreq_gpu_scene_aware(data->cur_policy->para[INDEX],
		util, data->util_sum, data->window_counter,
		data->vsync, targetload, *freq);
check_barrier:
...
static int gpu_scene_aware_init(struct devfreq *devfreq)
{
	...
	data->cur_policy->para[LOAD_POLICY] = DEFAULT_LOAD_POLICY;
					   ...
					   void *data)
{
	...
	case DEVFREQ_GOV_START:
		...
		if (ret == 0) {
			...
			devfreq_monitor_suspend(devfreq);
#ifdef CONFIG_HECA
			heca_register_dev_freq_notifier(HECA_DEVFREQ_GPU, devfreq);
#endif
		}
		break;
...
#ifdef CONFIG_HECA
		heca_unregister_dev_freq_notifier(HECA_DEVFREQ_GPU, devfreq);
#endif
		break;
