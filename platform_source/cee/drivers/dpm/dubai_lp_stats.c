...
#define DDR_AP_VOTE_DATA_MAX	1764
#define DDR_AP_VOTE_NUM_MAX	(2 * PLATFORM_QOS_DEBUG_MAX_NUM)
static unsigned int g_ddr_ap_vote_num;
static unsigned int g_ddr_ap_vote_data_size;
struct dubai_transmit_t *g_ddr_ap_vote_data;
...
{
	u32 latency_num = 0;
	u32 throughput_num = 0;

	if (argp == NULL || g_ddr_ap_vote_data == NULL)
		return -EFAULT;

	if (platform_qos_debug_record(PLATFORM_QOS_MEMORY_LATENCY, &latency_num,
				(struct platform_qos_req_data *)(g_ddr_ap_vote_data->data), PLATFORM_QOS_DEBUG_MAX_NUM) != 0) {
		pr_err("update_ddr_ap_vote_latency failed!\n");
		return -EFAULT;
	}

	if (platform_qos_debug_record(PLATFORM_QOS_MEMORY_THROUGHPUT, &throughput_num,
			(struct platform_qos_req_data *)(g_ddr_ap_vote_data->data) + latency_num, PLATFORM_QOS_DEBUG_MAX_NUM) != 0) {
		pr_err("update_ddr_ap_vote_throughput failed!\n");
		return -EFAULT;
	}

	g_ddr_ap_vote_num = latency_num + throughput_num;
	if (copy_to_user(argp, &g_ddr_ap_vote_num, sizeof(g_ddr_ap_vote_num)) != 0) {
		pr_err("update_ddr_vote_num failed!\n");
		return -EFAULT;
	}
	return OK;
...
int ioctrl_ddr_ap_vote_data(void __user *argp)
{
	...
	if (argp == NULL || g_ddr_ap_vote_data == NULL || g_ddr_ap_vote_num == 0 ||
		g_ddr_ap_vote_data_size > DDR_AP_VOTE_DATA_MAX)
		return -EINVAL;

	g_ddr_ap_vote_data->length = g_ddr_ap_vote_data_size;
	if (copy_to_user(argp, g_ddr_ap_vote_data, g_ddr_ap_vote_data_size) != 0) {
		pr_err("%s failed!\n", __func__);
		return -EFAULT;
	}
...
static int malloc_vote_time_data(void)
{
	...
	g_ddr_ap_vote_data_size = sizeof(struct dubai_transmit_t) +
			       sizeof(struct platform_qos_req_data) * DDR_AP_VOTE_NUM_MAX;
	g_ddr_ap_vote_data = (struct dubai_transmit_t *)kzalloc(g_ddr_ap_vote_data_size, GFP_KERNEL);
	if (g_ddr_ap_vote_data == NULL) {
		ret = -ENOMEM;
		goto fail_malloc_mem;
	}
#ifdef CONFIG_LOWPM_DDR_STATE
