...
#include <platform_include/cee/linux/freq_qos_debug.h>

#if !defined(CONFIG_ARCH_PLATFORM) || !defined(CONFIG_AB_APCP_NEW_INTERFACE)
#include <platform_include/cee/linux/hisi_cpufreq_req.h>
#endif
#ifdef CONFIG_DRG
...
int lpcpu_cpufreq_init_req(struct cpufreq_req *req, int cpu)
{
	...
	freq_qos_add_request_name(req_wrap->data.request);
	return 0;
