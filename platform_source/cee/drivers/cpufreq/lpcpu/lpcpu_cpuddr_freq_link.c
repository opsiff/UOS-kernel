...
static int cpuddr_freq_link_notify(struct notifier_block *nb __maybe_unused,
				   unsigned long val, void *v __maybe_unused)
{
	int ret;
...
static __init int hisi_cpuddr_freq_link_init(void)
{
	...
	ret = perf_mode_register_notifier(&cpuddr_freq_link_notifier);
	if (ret != 0)
