...
static int get_device_tree_data(struct platform_device *pdev)
{
	...
	get_clustermask_cnt(lite_inf.oncpucluster, MAX_THERMAL_CLUSTER_NUM,
			    &(lite_inf.cpuclustercnt), &(lite_inf.gpuclustercnt), &(lite_inf.npuclustercnt));
#endif
...
#ifdef CONFIG_THERMAL_SUPPORT_LITE
	rc = update_sens_num(inf.tsens_num_sensors, lite_inf.cpuclustercnt, lite_inf.gpuclustercnt, lite_inf.npuclustercnt);
	if (rc != 0)
		return -ENODEV;
#endif
	for (i = 0; i < inf.tsens_num_sensors; i++) {
#ifdef CONFIG_THERMAL_SUPPORT_LITE
		rc = lite_sensor_exist(of_node, i, lite_inf.cpuclustercnt, lite_inf.gpuclustercnt, lite_inf.npuclustercnt);
		if (rc != 0)
			continue;  /*lint !e605*/
		rc = of_property_read_string_index(of_node, "ithermal,tsensor_name", i, &g_tsensor_name[j++]);
#else                  /*lint !e605*/
