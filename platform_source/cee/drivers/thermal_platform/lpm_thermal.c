...
int gpufreq_update_policies(bool updatepolicy)
{
	...
	list_for_each_entry(cdev, &thermal_cdev_list, node) {
		...
		if (!strncmp(cdev->np->name, "gpufreq", strlen("gpufreq"))) {
			...
			dfc->cooling_state = freq_get_state(dfc, freq);
		}
	}
...
int cpufreq_update_policies(bool updatepolicy)
{
	...
	list_for_each_entry(cpufreq_cdev, &cpufreq_cdev_list, node) {
		...
		cpufreq_cdev->cpufreq_state = get_level(cpufreq_cdev, freq);
	}
 ...
 */
unsigned long freq_get_state(struct devfreq_cooling_device *dfc, unsigned long freq)
{
	int i;
