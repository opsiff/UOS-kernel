...
inline void enable_mask_bit(enum LP_REG_DOMAIN reg_domain, int offset, unsigned int nr)
{
	writel(hm_en(nr), g_sysreg_base[reg_domain].addr + offset);
}

inline void disable_mask_bit(enum LP_REG_DOMAIN reg_domain, int offset, unsigned int nr)
{
	writel(hm_dis(nr), g_sysreg_base[reg_domain].addr + offset);
}
