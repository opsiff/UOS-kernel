...
static void set_ap_suspend_flag(void)
{
	...
	enable_bit(SCTRL, REG_AP_SUSPEND_OFFSET, GUEST_SUSPEND_BIT);
#else
...
{
#ifdef CONFIG_PRODUCT_CDC_ACE
	disable_bit(SCTRL, REG_AP_SUSPEND_OFFSET, GUEST_SUSPEND_BIT);
#else
...
static int sr_psci_suspend(unsigned long index)
{
	...
	/*
	 * if psci version is 0.1 or 0.2, not support psci_system_suspend
	 * if psci version is larger than 1.0, psci_system_suspend is support
	 */
	if (psci_ops.get_version && PSCI_VERSION_MAJOR(psci_ops.get_version()) >= 1)
		return psci_system_suspend(index);
