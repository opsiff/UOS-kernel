...
#endif
#include "securec.h"
#if defined(CONFIG_DDR_CHAMAELEON) || defined(CONFIG_DDR_PISCES)
...
#define DDR_QICE_INT_RCD_BASE (RESERVED_MNTN_PHYMEM_BASE + RESERVED_MNTN_PHYMEM_SIZE - RDR_DDR_MEM_SIZE)
#define DDR_QICE_INT_RCD_SIZE 0x4
...
bool check_qice_intr(void)
{
	...
	vir_addr = ioremap_wc(DDR_QICE_INT_RCD_BASE, DDR_QICE_INT_RCD_SIZE);
	if (!vir_addr) {
