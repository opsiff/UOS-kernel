...
static void hisi_fbex_start_fn(struct work_struct *work)
{
	...
	if (g_fbex_data.cmd >= (u32)ARRAY_SIZE(dispatch)) {
...
static u32 hisi_fbex_copy_out(u32 cmd, struct fbex_req *from, void __user *to)
{
	...
	ret = (u32)copy_to_user(to, from, sizeof(g_fbex_data.req));
	if (ret) {
...
static u32 hisi_fbex_cmd_dispatch(u32 cmd, void __user *argp)
{
	...
	ret = (u32)copy_from_user(&g_fbex_data.req, argp, sizeof(g_fbex_data.req));
	if (ret) {
				    ...
				    unsigned long arg)
{
	...
	ret = (int)hisi_fbex_cmd_dispatch(cmd, argp);
...
static void __exit hisi_fbex_module_exit(void)
{
	if (g_fbex_dev_cmd.list.prev)
		misc_deregister(&g_fbex_dev_cmd);
	fbex_cleanup_fscrypto_dev();
	fbex_cleanup_debugfs();
}

static int __init hisi_fbex_module_init(void)
{
	int ret;
	...
	ret = fbex_init_fscrypto_dev();
	if (ret != 0) {
		pr_err("failed to register fbex fscrypto dev\n");
		goto out;
	}
