...
#define FBEX_UDK_CMD_ENCRYPT_SLOT 6
static void fbex_sync_ufs_lock(void)
{
#if defined(CONFIG_LIBLINUX) && defined(CONFIG_FSRV_ENCRYPTION_HW_SDP_ENCRYPTION)
	int fd;
	int ret;

	fd = liblinux_pal_vfs_open("/dev/udk_sdproxy", O_RDONLY, 0);
	if (fd < 0) {
		pr_err("%s, open device failed, ret: %d\n", __func__, fd);
		return;
	}
	ret = liblinux_pal_vfs_ioctl(fd, FBEX_UDK_CMD_ENCRYPT_SLOT, g_ufs_lock, sizeof(g_ufs_lock));
	if (ret)
		pr_err("%s, sync ufs lock failed, ret: %d\n", __func__, ret);
	(void)liblinux_pal_vfs_close(fd);
#endif
}
...
#ifdef CONFIG_LIBLINUX
/* Because fs encryption is not enabled, screen lock and unloack hack temporary. */
void fbe3_lock_in(u32 user)
{
	liblinux_vfs_lock_in(user);
}

u32 fbe3_unlock_in(u32 user, u32 file, u8 *iv, u32 iv_len)
{
	return liblinux_vfs_unlock_in(user, file, iv, iv_len);
}
#endif
