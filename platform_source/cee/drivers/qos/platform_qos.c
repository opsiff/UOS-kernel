...
#define PLATFORM_QOS_MEMORY_THROUGHPUT_UP_THRESHOLD_DEFAULT_VALUE	61440
#endif
...
static void fill_req_data(struct platform_qos_req_data *data, struct platform_qos_request *req)
{
	char *p = NULL;
	char *next_p = NULL;
	char owner[MAX_PLATFORM_QOS_OWNER_LENGTH] = {0};
	strncpy_s(owner, sizeof(owner), req->owner, sizeof(req->owner) - 1);
	p = strtok_s(owner, ":", &next_p);
	p = strtok_s(NULL, " ", &next_p);
	if (p)
		strncpy_s(data->name, sizeof(data->name), p, strlen(p));

	p = strtok_s(NULL, ":", &next_p);
	p = strtok_s(NULL, " ", &next_p);
	if (p)
		kstrtoint(p, 10, &data->pid);

	p = strtok_s(NULL, ":", &next_p);
	p = strtok_s(NULL, " ", &next_p);
	if (p)
		kstrtoint(p, 10, &data->tid);

	p = strtok_s(NULL, ":", &next_p);
	p = strtok_s(NULL, " ", &next_p);
	if (p)
		kstrtoll(p, 10, &data->timestamp);

	data->vote_class = req->platform_qos_class;
	data->prio = (req->node).prio;
}
		...
		int *active_nums, struct platform_qos_req_data *data, int data_size)
{
	...
	plist_for_each_entry(req, &c->list, node) {
		if (req->node.prio != c->default_value && active_reqs < data_size) {
			active_reqs++;
#ifdef CONFIG_FREQ_PLATFORM_QOS_DEBUG
			pr_info("%d: class:%d prio:%d %s\n",
				active_reqs, req->platform_qos_class, (req->node).prio, req->owner);
			fill_req_data((struct platform_qos_req_data *)data + active_reqs - 1, req);
			*active_nums = active_reqs;
#else
				   ...
				   void *unused __maybe_unused)
{
	...
	plist_for_each_entry(req, &c->list, node) {
		...
		tot_reqs++;
#ifdef CONFIG_FREQ_PLATFORM_QOS_DEBUG
		seq_printf(s, "%d: class:%d prio:%d %s state:%s\n",
			tot_reqs, req->platform_qos_class, (req->node).prio, req->owner, state);
		pr_info("%d: class:%d prio:%d %s state:%s\n",
			tot_reqs, req->platform_qos_class, (req->node).prio, req->owner, state);
#else
		seq_printf(s, "%d: %d: %s\n", tot_reqs,
			(req->node).prio, state);
			      ...
			      int platform_qos_class, s32 value)
{
	...
	(void)snprintf_s(req->owner, sizeof(req->owner) - 1, sizeof(req->owner) - 1,
			"owner:%s pid:%d tid:%d time:%ld", current->comm, task_tgid_nr(current), task_pid_nr(current), ts.tv_sec);
#endif
}
