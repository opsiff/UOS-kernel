...
static int enable_monitor(void)
{
	if (g_loadmonitor_status == LOADMONITOR_OFF) {
		...
		g_monitor_info.monitor_delay = (u64)(g_dpm_period / LOADMONITOR_PCLK);
		queue_delayed_work(g_monitor_dpm_wq, &g_dpm_delayed_work,
				   msecs_to_jiffies((unsigned int)(g_monitor_info.monitor_delay / 1000U)));
...
static int ioctrl_get_ip_len(void __user *argp)
{
	...
	g_dpm_data->length = MAX_NUM_LOGIC_IP_DEVICE + 1UL;
	dpm_ipnum = (u32)(MAX_NUM_LOGIC_IP_DEVICE + 1UL);
...
static int ioctrl_get_sample_time(void __user *argp)
{
	...
	if (dpm_switch == DPM_GET_MAX_PERIOD) {
		period = DPM_PERIOD_MAX / (LOADMONITOR_PCLK * 1000U);
		if (copy_to_user(argp, &period, sizeof(period)) != 0) {
			pr_err("dpm_period get version failed\n");
			ret = -EFAULT;
		}
	} else if (dpm_switch == DPM_GET_MIN_PERIOD) {
		period = DPM_PERIOD_MIN / (LOADMONITOR_PCLK * 1000U);
		if (copy_to_user(argp, &period, sizeof(period)) != 0) {
			pr_err("dpm_period get version failed\n");
			ret = -EFAULT;
		}
...
static int ioctrl_get_ddr_freq_info(void __user *argp)
{
	...
	char *ddr_data = NULL;
	int ddr_freq_num, j;
	size_t ddr_data_len, i, ddr_info_name_len;
	size_t pos = 0;
	u64 ddr_data_temp;
	...
	for (i = 0; i < DDR_INFO_LEN; i++) {
		...
		ddr_info_name_len = strlen(g_ddr_info_name[i]);
		ret = strncpy_s(ddr_data + pos, IP_NAME_SIZE, g_ddr_info_name[i],
				min_t(size_t, ddr_info_name_len, IP_NAME_SIZE - 1U));
		if (ret != EOK) {
...
static int ioctrl_set_sample_time(const void __user *argp)
{
	...
	g_monitor_delay_value.peri0 = period * (LOADMONITOR_PERI_FREQ_CS / 1000U);
	g_monitor_delay_value.peri1 = period * (LOADMONITOR_PERI_FREQ_CS / 1000U);
	g_monitor_delay_value.ao = period * (LOADMONITOR_AO_FREQ / 1000U);
	g_dpm_period = period * LOADMONITOR_PCLK * 1000U;
	pr_err("%s flag:0x%x,sample_time:%ums,dpm_period:%u,peri0:%u,peri1:%u,ao:%u\n", __func__,
	       g_monitor_delay_value.monitor_enable_flags,
	       period, g_dpm_period, g_monitor_delay_value.peri0,
...
static struct ioctrl_dpm_ldm_t dpm_ldm_func_table[] = {
	...
	{ DPM_IOCTL_GET_CPU_POWER, ioctl_dpm_get_cpu_data },
#endif
...
static int dpmldm_pm_callback(struct notifier_block *nb, unsigned long action, void *ptr)
{
	...
	case PM_HIBERNATION_PREPARE:
		pr_info("loadmonitor suspend +:status%u, action:%u\n",
			g_loadmonitor_status, action);
		if (g_loadmonitor_status == LOADMONITOR_ON) {
			cancel_delayed_work_sync(&g_dpm_delayed_work);
			sec_loadmonitor_switch_disable(
				g_monitor_delay_value.monitor_enable_flags,
	...
	case PM_POST_HIBERNATION:
		pr_info("loadmonitor resume +:status%u, action:%u\n", g_loadmonitor_status, action);
		if (g_loadmonitor_status == LOADMONITOR_SUSPEND) {
			sec_loadmonitor_switch_enable(g_open_sr_ao_monitor);
			g_monitor_info.monitor_delay = g_dpm_period / LOADMONITOR_PCLK;
			queue_delayed_work(g_monitor_dpm_wq, &g_dpm_delayed_work,
					   msecs_to_jiffies((u32)(g_monitor_info.monitor_delay / 1000U)));
				    ...
				    unsigned int dev_index, unsigned int sensor_index)
{
	...
	idle_nor_time = 1000UL * (u64)status->idle / freq;
	busy_nor_time = 1000UL * (u64)status->busy_norm_volt / freq;
	busy_low_time = 1000UL * (u64)status->buys_low_volt / freq;
	...
	if (is_ao_data(info)) {
		...
		if (idle_nor_time + busy_nor_time + busy_low_time > 1000) {
			...
			off_time = 0U;
			if (g_print_err_data == -1 ||
			    (u64)g_print_err_data <= MAX_MONITOR_IP_DEVICE) {
			...
			off_time = 1000UL - idle_nor_time - busy_nor_time -
				   busy_low_time;
		}
	}
...
void update_dpm_data(struct work_struct *work)
{
	...
	calc_ddr_bw();
	g_dpm_data->length = MAX_NUM_LOGIC_IP_DEVICE + 1U;
	for (i = 0; i < MAX_NUM_LOGIC_IP_DEVICE; i++) {
	...
	g_dpm_data->data[MAX_NUM_LOGIC_IP_DEVICE].busy_low_time = 0UL;
	g_dpm_data->data[MAX_NUM_LOGIC_IP_DEVICE].idle_nor_time = 0UL;
	...
	up(&g_rw_loadmonitor);
	queue_delayed_work(g_monitor_dpm_wq, &g_dpm_delayed_work,
			   msecs_to_jiffies(g_monitor_info.monitor_delay / 1000U));
}
...
#if defined (CONFIG_PLATDRV_SUPPORT_FFA) && defined (CONFIG_FREQDUMP_SUPPORT_FFA)
static int freqdump_loadmonitor_buff_init(void)
{
	g_freqdump_virt_addr = kzalloc(SHARED_MEMORY_SIZE, GFP_KERNEL);
	if (g_freqdump_virt_addr == NULL) {
		pr_err("%s %d kzalloc fail\n", __func__, __LINE__);
		return -ENOMEM;
	}
	g_loadmonitor_virt_addr = g_freqdump_virt_addr;
	freqdump_phy_addr = g_freqdump_virt_addr;
	loadmonitor_phy_addr = g_freqdump_virt_addr;
	return 0;
}
#else
static int freqdump_loadmonitor_buff_init(void)
...
static int __init dpmldm_init(void)
{
	...
	dev_t devno = MKDEV(g_dpmldm_major, 0U);
	...
	pdevice = device_create(dpmldm_class, NULL,
				(dev_t)MKDEV(g_dpmldm_major, 0U), NULL,
				DPM_DEV_NAME);
	...
	ret = freqdump_loadmonitor_buff_init();
	if (ret != 0) {
		pr_err("%s freqdump_loadmonitor_buff_init err\n", __func__);
...
fail_to_alloc_workqueue:
	...
	device_destroy(dpmldm_class, (dev_t)MKDEV(g_dpmldm_major, 0U));
...
static void __exit dpmldm_exit(void)
{
	if (g_freqdump_virt_addr != NULL) {
#if defined (CONFIG_PLATDRV_SUPPORT_FFA) && defined (CONFIG_FREQDUMP_SUPPORT_FFA)
		kfree(g_freqdump_virt_addr);
#else
		iounmap(g_freqdump_virt_addr);
#endif
		g_freqdump_virt_addr = NULL;
		g_loadmonitor_virt_addr = NULL;
		freqdump_phy_addr = 0;
		loadmonitor_phy_addr = 0;
	}
	free_dpm_ldm_data();
	(void)unregister_pm_notifier(&g_dpmldm_pm_notif_block);
