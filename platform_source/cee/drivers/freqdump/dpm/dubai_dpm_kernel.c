...
#ifdef CONFIG_DPM_DUBAI_STUB
	static long long g_stub_energy = 1;

	static void stub_energy_acc()
	{
		g_stub_energy++;
		if (g_stub_energy > 65535)
			g_stub_energy = 0;
	}
#endif
...
int ioctl_dpm_get_cpu_data(void __user *argp)
{
	int ret = 0;
	...
	for (i = 0; i < CORE_NUMBER; i++)
		g_dpm_cpu_power_data.power[i] = 10000 + g_stub_energy;
	stub_energy_acc();
#else
	ret = get_cpu_dpm_chdmod_power();
#endif
	if (copy_to_user(argp, &g_dpm_cpu_power_data,
			 g_dpm_cpu_power_data_size) != 0) {
		return -EFAULT;
	}
