...
static int send_ipc(unsigned char cmd_type)
{
	...
	index = (int)(g_ipc_cnt * (MAX_MAIL_SIZE - 1U));
	max_freq_num = ((unsigned int)g_ddr_freq_num - ((unsigned int)(MAX_MAIL_SIZE - 1U) * g_ipc_cnt)) /
		       (MAX_MAIL_SIZE - 1U) > 0 ?
		       MAX_MAIL_SIZE - 1U : g_ddr_freq_num % (MAX_MAIL_SIZE - 1U);
...
int monitor_ipc_send(void)
{
	...
	max_ipc_cnt = ((unsigned int)g_ddr_freq_num / (unsigned int)(MAX_MAIL_SIZE - 1U) + 1U);
