				 ...
				 struct cpuidle_driver *drv, int idx)
{
	...
	if (ret == 0) {
		...
		/*
		 * We must check need_resched again at last since cpu_pm_enter
		 * may call wakeup. Cpu can't enter idle with need_resched
		 * flag set by itself while no ipi is raised.
		 */
		if (!need_resched()) {
			/*
